...
- **Rhetorical shape.** Parallelism, negation-first framing and epigram: _"Nothing leaves this
  machine and nothing arrives until you ask for it."_, _"Nothing is looked at until you ask."_,
  _"What you time is what plays."_ Say the fact once: _"Nothing is sent or fetched except by an
  explicit action."_
...
- **Second person as narrator.** `you` / 「你」 is for an instruction the author is about to carry out
  ("Press F1"), not for describing what the product does to their data. _"Your project files are left
  untouched until you unfreeze it"_ → _"Project files are not written until the project is unfrozen."_
...
1. **Help popover (`F1`)** — anchored, transient, closes on Escape or a click outside. It resolves
   ...
   position), so it answers "what is _this_" without the author naming anything. Following a
