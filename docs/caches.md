...
| | 全局缓存 | 项目缓存 |
|---|---|---|
| 位置 | `<缓存根>/<CacheNamespace>/`，见下一节 | 项目内 `editor/cache/` |
| 归谁 | 这台机器 | 这一个项目 |
...
项目缓存**必须**放在 `editor/cache/` 下，因为 `workingSet` 正是按这个前缀排除的：
...
根锚定（只在项目根排除，因为这些是普通英文词，作者可能拿来当内容目录名）：
...
⚠ `editor/assets/remote/` 是唯一一个**不在 `editor/cache/` 下的项目缓存**。
它单独出现在排除表里。新加项目缓存请放 `editor/cache/`，不要再增加一个例外。
...
1. **项目缓存没有任何界面**。设置里的缓存清单只覆盖全局。一个项目的
