...
跨蓝图直接调用的作用域函数系统：Fn 头节点在事件图中声明函数（fn 身份 = 头节点 id），Call Fn 通过下拉框选择可见的 Fn 并等待其执行完毕后取回返回值。

| 节点 | 类型 ID 建议 | 说明 |
| --- | --- | --- |
| Fn | `blueprint.fn.head` | **已实现**。函数声明头节点；Inspector `Fn Name` 设置函数名；`Add parameter` 追加参数（数据输出引脚，可改名、可选类型：string / integer / float / boolean / json / any）；参数值由调用方传入。可声明于 Global / Surface / 控件蓝图。 |
| Call Fn | `blueprint.fn.call` | **已实现**。下拉框选择可见的 Fn（标签按作用域消歧，如 `MyFn — Button2`）；选中后自动生成与签名匹配的输入 / 输出数据引脚（签名快照缓存在节点参数中，声明变更后同图自动同步、跨图打开时修复并有 `fn.call_signature_stale` 警告兜底）。Latent：等待函数体执行完毕（含 Delay 等）后携带返回值走 `next`。Blueprint Value 图可以宿主控件身份调用。 |
| Fn Return | `blueprint.fn.return` | **已实现**。结束函数体并提供返回值；`Add return value` 追加类型化返回引脚；必须从恰好一个 Fn 头节点 exec 可达；同一 Fn 的多个 Return 签名必须一致。 |

作用域规则：Surface / 控件蓝图声明的 Fn 仅同 Surface 内可调用（surfaceMain + 该 Surface 上的 widgetMain / widgetValue）；Global 蓝图声明的 Fn 处处可调用；Global 调用方只能看到 Global Fn；组件蓝图（componentWidgetMain）暂不参与。复制粘贴：Fn 头与 Call Fn 一同粘贴时调用自动改指粘贴出的新 Fn；单独把 Call Fn 粘贴到其他 Surface 时保留原引用并由校验上报 `fn.call_target_not_found` 错误（节点红框 + 诊断面板）。运行时递归深度上限 32；静态自环有 `fn.recursive_call` 警告。
