...
- ~~**resolve 面板没有做过目视验收**~~ —— **2026-08-25 做了**，用两份 clone 对同一行台词和同一个
  ...
  冲突标记清干净）。**说话的那一半有五处欠账，三处已修（2026-08-25）**：
  ...
  3. ✅ 名字被截成 `s…` 随 1 一起消失。
  4. ⚠ **只修了门，没修屋子。** 合并期间作者的文件在磁盘上带着 `<<<<<<< ours` /
     `||||||| original` / `>>>>>>> theirs`，这是设计（见 §4.23 与 merge 冻结的注释）。
     **恢复模式不再被提议**——它是为「没人说得清的损坏」准备的，而合并说得清自己，出路是把合并
     做完或放弃（`useRecoveryOffer` 现在问仓库有没有未完成的合并，**不是问冻结**：冻结在开工程时
     才 armed，答不了「刚刚同步出冲突的这个窗口」）。
     **仍然欠着的是**：一次产生冲突的同步之后，这个窗口按原样重读那些带标记的文件，于是故事面板空白、
     仪表盘 0 场景；而**重新打开同一个工程是好的**——那条路 armed 了 merge 冻结、编辑器改从
     `@/lib/app/mergeConflictReads` 读作者自己那一侧，内容照常显示。让同步后的重读走进同一个状态
     是下一步。
  5. ⚠ 4 修好之后那两条红色通知不再出现（它们本来就是 4 的后果），**剩下一处自相矛盾**：
     合并完成后标签页头仍写「该工程的两个版本正在合并」而正文写「该工程没有正在进行的合并」。
