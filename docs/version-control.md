...
- ~~**resolve 面板没有做过目视验收**~~ —— **2026-08-25 做了**，用两份 clone 对同一行台词和同一个
  ...
  冲突标记清干净）。**说话的那一半有五处欠账**：
  1. **索引与详情按文件命名**（`storydoc.json` / `editor/story/stories/48bb82a5-…`），比较界面
     早已改成作者的名字（§9.1、`documentName.ts`），resolve 这一侧没跟上——这是
     「不向作者展示裸数据文件」那条常设裁决的现场。⚠ 移植时有一处真问题：合并期间
     `editor/story/index.json` 自己也可能是冲突的，所以「读作者起的名字」可能没有答案，
     得靠 `DocumentName` 的 `unnamed` 那一态。
  2. **冲突的台词按序列化 JSON 呈现**（`text {"textId":"eac3208a-…","role":"narration","value":"…"}`），
     作者要读 JSON 才能选边；同一份故事在分屏比较里已经画成剧本了。
  3. 第一行的文件名被截成 `s…`——列宽不够时 basename 先输，等于没有名字。
  4. **合并期间作者的文件在磁盘上带着 `<<<<<<< ours` / `||||||| original` / `>>>>>>> theirs`**，
     于是 Studio 自己读不动它们，工作区弹「该工程未能正常加载」并**建议以恢复模式打开**——
     而此刻正确的下一步是把合并做完。
  5. **合并完成后那两条红色通知不会消失**，内容全部变成假的（说文件读不了、说去选边）；
     同一屏上标签页头写「该工程的两个版本正在合并」而正文写「该工程没有正在进行的合并」。
     工程内容也没有重新读取，仪表盘停在 0 场景。
