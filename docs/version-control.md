...
- ~~**resolve 面板没有做过目视验收**~~ —— **2026-08-25 做了**，用两份 clone 对同一行台词和同一个
  ...
  冲突标记清干净）。**说话的那一半原有五处欠账。2026-08-25 了结四处，第 5 处只剩一句自相矛盾，目视验收又添了第 6 处**：
  ...
  6. ⚠ **同步的冲突通知仍按文件命名**：`workspace.shell.versionControl.syncConflictDetailMany`
     把 `editor/story/stories/48bb…/storydoc.json` 原样列给作者。索引与详情已经改成作者的名字
     （见 1），这条通知是同一条常设裁决下最后一处漏网的地方。
