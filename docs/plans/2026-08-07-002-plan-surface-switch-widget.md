...
> **进度（2026-08-07）：M1–M4 全部完成并验收，本卡收工。**
> M3/M4 的实机验收 11 条断言全绿，脚本 `tools/ui-verify/scenarios/switch-widget-m3m4.js`
> （同样写在实现之前）。M3 的判据是**自相矛盾构造**：作者的 `props.checked` 为 false、
> 绑定图返回 true，于是渲染成开的开关只可能是绑定生效了。
> M4 的拖拽实测：拖过半程切换、拖不过半程不切换、原地按一下仍然切换（点击没被拖拽吃掉）。
>
> **一条与本卡无关、但被验收撞出来的产品缺陷（已另开卡）**：Dev Mode 运行时保存 uidoc
> ——**哪怕一个字都没改**——会让运行中的游戏画面变空（18 个节点掉到 2 个），
> 且页面不报任何错，只有停掉再起 Dev Mode 才恢复。它的坏处不止于此：
> 保存发生在哪一步、空屏就在**下一步**才被观测到，于是失败看起来像「控件不见了」。
> 本卡的 M3/M4 脚本因此把 seed 改成真正幂等（已绑定就不再 ensure、不再 save）。
>
> ---
>
> **M1 + M2（较早完成）**
