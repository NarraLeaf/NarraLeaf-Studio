...
**结果：19 绿 / 1 红。已合并 develop 并推送（merge `79149c39`，随后 `2e549a90` 并入上游 revert）。**

方法：`git archive` 出只含被测分支的隔离树（`D:/Temp/nls-u4-final`，node_modules junction），
**新起实例**（CDP 9228），驱动到 §4 指定状态，逐条跑断言 + 亲自读图。
合并后**又在合并结果上重跑了一整轮**——合并本身改了代码（见 §7.2），只验分支不算数。

夹具指纹：按**内容**比对（不是 mtime），`First Day` / `At the Station` / `The Forest` 三个场景
逐块 payload 未变，共享 demo3 仍是 3 个场景、没有夹具泄漏进去。

### 7.1 唯一的红：A-9，且不是实现的错

`Repeat` 那一级显示的是**声明的**次数 `×3`，不是**当前第几轮**。我自己在引擎源码里核过，
不是采信执行者的说法：`StackModel.snapshot()` 把 `loop` 挂在它返回的那个对象上，
但嵌套的循环只能经由

```js
r.branches = i.wait.stackModels.map(s => s.snapshot().frames)
```

进入 `getStackSnapshot()`——只取 `.frames`，**`.loop` 被丢掉**。`/repeat` 编译出来的正是
这样一个嵌套 StackModel，所以现有引擎面上拿不到活轮次。

**这是我写判据时的错**：我照着 `StackSnapshot.loop` 的类型声明写了 `n/3`，
没有验证这个字段在嵌套后还活着。**引擎卡待开**（`StackFrameSnapshot.branches`
应携带整个 snapshot 而不只是 `frames`）；在那之前这条就是欠着的，不改判据、不当它绿。

### 7.2 合并时抓到的一处「只有合并才会坏」

develop 在本分支切出之后，把 `describeBlock` 里 `plugin` 那一条加了回来；而本分支把
`describeBlock` 整个搬进了共享投影。**照单接受重构会静默丢掉这一条**——插件行会掉进
effect 分支、把 `undefined` 渲染出来。lint 不会报，测试不会红，截图看不出来。
合并时补进了投影的新家。

随后上游 `b95e61ad` 又把整个插件 API 提交 revert 了（`action: "plugin"` 从
`StoryActionPayload` 里整个消失），这一条于是又必须撤掉，否则投影编译不过。
两次都在合并里处理完并重新 lint + 重跑验收。

### 7.3 标定时改写过的四条判据（都是判据错，不是实现错）

| 断言 | 原写法 | 为什么错 |
|---|---|---|
| A-1 | 12 行逐字相同 | 编辑器故意把说话人单独成行、续行省略；一刀切会逼时间线抄它的两行布局。拆成 A-1a/A-1b |
| A-4 | 12/12 行有类别色 | 编辑器**按设计**只给非对白动作行上色，旁白/对白零 chrome。改成"编辑器上了色的行" |
| A-13 | 任意行落点精确 | 跳到 `character enter` 落在下一行是对的：冷跳"进入该行并向前播"，非阻塞动作不停播放头 |
| A-8/A-9 | 先找 tab 再推进 | 相反了：relaunch 后播放头在根行、栈是空的，检查直接短路，两条驱动路径一次都没跑过 |

驱动脚本本身也修了两处同族毛病：按**固定点击次数**推进（重复体已经走了几轮是不确定的，
猜次数就是读错状态）改成**按条件推进**；以及把"场景加载完成"误当成"我的点击生效了"，
在冷实例上让每一步都慢一拍——加了上台守卫。

### 7.4 还欠着的

- **A-9 的活轮次**：等引擎卡。
- **菜单/选项容器链**：CDP 合成点击选不中选项（点击确实落在选项上、`elementFromPoint`
  也确认命中，菜单就是不响应）。夹具已改成不依赖它，但这一情形**没有人验过**。
- 执行者报告 §7 自列了八条 gap，其中值得下一轮看的：空文本行会把编辑器的
  `Double-click to enter narration` 占位符原样搬到时间线上（与 U5 的"不要解释性文字"冲突）；
  容器行读作 `Repeat 3 times` 而编辑器 `innerText` 读作 `Repeat times`（次数在 `<input value>` 里）；
  `displayable/transform` 是已知最后一处编辑器/时间线分歧。
