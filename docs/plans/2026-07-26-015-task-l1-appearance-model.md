...
F6 的三条出路：(a) 迁移时读角色库，用 `defaultForm` 补出 `(form, variant)`——放弃"两个迁移互不依赖"
这条设计，但结果最准；(b) 写一个解析不了的哨兵 pose id，让编译器如实报"这一行的差分没了"；
(c) 维持现状。**建议 (a) 有则用、无则退 (b)**：`defaultForm` 就在同一份 store 里，
拿不到时再退化成显式诊断，两种情况都不会静默换图。
