...
- **B11 canonical 拼写永远英文、永远可输入**、`/` 与 `#` 仅在行首触发、提交后
  ...
  > **2026-08-01 修订。** 原文为「token 不翻译」。现在指令名与参数名都有一张
  > **派生的**本地化别名表：指令的取自 `story.command.<id>.label`
  > （`commands/registry.ts`），参数的取自 `story.paramHint.<key>`
  > （`commands/localizedParams.ts`）。所以 `/背景` 解析为 `bg`、`位置=left`
  > 解析为 `at=left`。
  >
  > 修订的是「译名能不能输入」，不是「canonical 是什么」：英文拼写在任何语言下
  > 都先于译名匹配，脚本因此跨语言可读。译名为空、回显 key（未翻译）、含空格、
  > 或已经拼成某个 canonical 拼写时一律不收录，表是派生的、不手写。
  >
  > 枚举值（`fade` / `left` / `zoom`）**仍不翻译**——它们没有这样一张表，翻译
  > 它们等于教一门解析器不认识的语言。
  >
  > 显示语言由 `editor.commandLanguage` 单独决定（默认跟随界面语言），因为
  > 「我读什么语言」和「我敲什么语言」是两个问题。
...
0.16.1）**：整屏 kind 在编译期实例化为引擎的转场"引擎 + 几何词表"——`Reveal` 承载遮罩几何
（`Mask.wipe / iris / blinds / barnDoor / clock / fan / dots`），`ThroughColor` 承载"过色"，
`Dissolve` / `FadeIn` / `BlurDissolve` / `Push` / `Darkness` 各自成引擎。舞台对象的 reveal/conceal
走 Transform 预设，不属于遮罩词表，故新几何转场（barn-door / clock / fan / dots）**仅**整屏可用。
...
`darkness`：整屏压暗转场，编译期实例化引擎的 `Darkness`（0.16.1 起进入公开导出面）——直接换到
新图片并把它的亮度从"起始暗度"动到"结束暗度"，两者均 0–1，默认 `1 → 0`（从黑场中浮现），
在检查器可调。
