...
**未完成**（下方 12.1 表为准，此处给阻塞关系）：

- **W0**（Phase 0 实机验证）：未做，需真机，清单见 §12.4。不阻塞任何代码。
- **S1–S4**（Studio-Shell 壳仓库）：未做。**仓库已创建：
  `https://github.com/NarraLeaf/Studio-Shell.git`（空仓）**。这是当前**唯一的总
  卡点**——A2/A6 接线/A7/B1 protocol/B2 全部下游依赖它产出的模板与 manifest。
- **A2**（模板分发）：卡 S4（npm 包需先存在）。
- **A6 接线**（protocol.ts mobile job、runGameBuild.ts 分发、GameBuildManager
  移动分支）：repack 核心已就位，**只差接线**；卡 A2 + 真实 staging-web 产物。
- **A7**（UI/i18n/preflight 暴露）：卡 A6。
- **B1 剩余**（protocol ios 块）+ **B2**（iOS UI 暴露）：卡 A6。
- **M4 加密 / M5+**：首轮非目标。
