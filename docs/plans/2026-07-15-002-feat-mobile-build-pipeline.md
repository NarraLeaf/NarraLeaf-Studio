...
- 加密：与 web 相同——移动目标 + `encryptAssets` 开启时输出同款
  console 提示（"资产保护不适用于移动导出"），载荷明文。§8 的拦截点不改变
  这一点，只是留门。
...
- 壳的 serving 层统一经过 `ContentDecoder`（默认恒等）。接口形状通用（字节/
  流变换），**不包含任何机制语义**。
- 后续批次由 NarraLeaf-Encryption 私有仓产出 Android/iOS 原生库替换默认实现，
  接入方式届时在私有仓内文档化。本仓库与 Studio-Shell 仓库不描述其内部。
- 威胁模型对齐桌面：目标是把攻击成本从"解压即得"抬到"逆向原生代码/内存
  dump"级别；配合 5.1 的调试开关关闭。
...
| M4+ | 原生桥 v2、release 签名、AAB、加密接入、商店化、工程导出逃生舱 | 各仓 |
