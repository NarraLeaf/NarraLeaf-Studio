...
| ~~D5~~ | ~~`deleteChapter`~~ | | | **Done** — same snapshot, and it covers the cascade (every scene in the chapter) plus the entry-scene re-point. Found while doing it: nothing called `deleteChapter` at all — the story panel had no "delete chapter", so the cascade was unreachable. The chapter row now has a context menu (right-click the header, or its kebab) with New scene / Rename / Delete, so it is reachable and undoable together. |
