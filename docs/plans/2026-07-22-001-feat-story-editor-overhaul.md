...
## 10. 追加裁决（2026-07-23，用户已确认）

**变量注册表（bible §3.1 ↔ §3.4b 和解）**：项目级 `VariableRegistryService` **只收蓝图声明的 persistent**（§3.4b 明文的"唯一不以行呈现的类别"）；故事侧 `/persis` 维持 v6 声明行模型（块 id 即 variableId、删行即删变量）。编译器校验、蓝图成员树、变量面板读**合并视图**（注册表 + 声明行扫描），同名冲突产出编译诊断。§3.1 的"持有全部 persistent 声明"据此修正为"持有全部**蓝图侧** persistent 声明"；其原始动机（蓝图寄生、静默失败、undo 分裂）在本方案下全部达成。M-VAR 专属里程碑卡按此裁决编写。

**persistent 严格校验**（2026-07-23 确认）：不存在"宿主注入、Studio 未声明"的用法，未声明引用 warn+skip 维持。

**`Control.any` 败者分支不掐断**（引擎 M4 观察，≈Promise.race 语义）：暂定维持；若 M7 演出透镜需要"胜出即中止其余"，作为引擎语义变更单独立项。
