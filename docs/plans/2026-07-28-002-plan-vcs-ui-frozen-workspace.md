...
## 4.5 界面第二轮：轨道的四个接缝（2026-07-30 全部落地）

四个接缝当初是有意留空的，不是没做完；每一个的填法都记在这里，因为理由比代码难重建。

| 接缝 | 落地 | 为什么是这个形状 |
|---|---|---|
| `commit-form` | 提交表单 | **不可提交时整个不渲染，而不是禁用**——冻结态下一个点不动的 Commit 正是 `freezeGuard` 唯一禁止的东西，这也是它当初被留空的原因。**不拿变更数当闸门**：`status` 在有人扫描前是 `null`，为了决定画不画按钮去扫描就是 §4.17 禁止的隐式扫描；树没变时后端会答，那对按了 Commit 的人**就是**答案。空消息允许（后端有默认，轨道也早就能渲染无消息的修订）。Ctrl/Cmd+Enter。**提交后必须重读身份**——否则轨道停在旧编号 |
| `change-list` | 逐文件清单 | 文件名显眼、目录让位并**从头部截断**（本工程的路径在尾巴上分岔，普通省略号正好吃掉那一个字符）。**50 行上限并明说漏了多少**，且**排序在截断之前**（否则挡住提交的冲突可能正是被切掉那行）。**看历史版本时整段不渲染**：它描述的是工作树而屏幕上是修订，那里的扫描本来就跳过，所以画出来的数必然陈旧（真 app 上看到过） |
| `VERSION_HISTORY_PAGE` | 翻页 | **加大 limit 重读**，因为后端没有游标（`readRevisionGraph(globals, limit)` 就是全部）。代价在主进程消掉：`VcsSession.details` 按 revision 缓存元数据（修订不可变），**只缓存明细不缓存图**。**「还有更多吗」看原始条目数不看行数**——压平丢掉被合并分支的修订，折叠藏掉检查点，满是检查点的工程读满 50 条只画 3 行 |
| `versionStatusText` | 非默认分支时显示分支名 | 三处界面（状态栏 / 顶栏控件 / 轨道）合用一个 `versionFace`——三份实现迟早互相矛盾，这个功能已经发生过一次。默认分支 `main` 是**实测**并被集成测试钉住的。读分支必须是纯读，所以走 `repositoryStatus(scan:false, revisionOnly:true)`；顺手删掉了 `getInfo` 为一个**无人读取**的 `revisionCount` 走整棵修订图这件事 |

**顺带修掉的两个真缺陷**（都是跑真 app 才暴露的）：

1. **提交后状态栏停在旧编号**。三个版本界面各自读 head（状态栏那一位不在轨道的树里，是有意的），
   所以一个界面造成的修订对另外两个不可见。修法是 `VersionControlService` 广播
   `revisionRecorded`，三处一起重读。**对自动检查点更重要**——那时没人按过任何东西。
2. **`notVersioned` 在窄界面被截断成 "No version hist…"**。它和 `noHistory` 还撞了语义（一个是
   「没启用」，一个是「启用了但空」）。两串都改短并分开。

**`.nlspkg` 导出**现在排除 `.lore/` 与 `.nlstudio/services`（裁决见 §6）。
...
- **轨道展开状态跨会话持久化**：**保持持久化**（`ui.versionRail.expanded`，本来就是）。提交表单
  落进面板之后，面板是日常提交入口而不只是冻结态的指示器；「你在看历史版本」的强提示是 48px 条。
- **`.nlspkg` 导出**：**排除 `.lore/` 与 `.nlstudio/services`**。导出包是「一份工程的副本」，
  不是「我的全部历史」，也不是「发送方窗口是怎么摆的」。`.loreignore` 仍随包走——它是策略文件，
  很小，而且到哪儿都对这个工程有效。
- **状态栏那一位**：**只在非默认分支时显示分支名**（`feature · #12`），默认分支上维持 `#12`。
  合并标记**不加**——Studio 里现在建不出分支也做不出合并，等 V5 协作真做了再一次定形。
