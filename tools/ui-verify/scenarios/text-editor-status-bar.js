...
async function openAssetsPanel(d) {
  ...
  if (!(await rendered()))
    throw new Error("SETUP GUARD: the Assets panel rendered no category sections");
}
...
async function ensureTextTab(d) {
  ...
  if (!open) {
    ...
    if (!(await findRow(ASSET_NAME)).found)
      throw new Error(`SETUP GUARD: ${ASSET_NAME} is not in the sidebar`);
    await A.sleep(500);
    const row = await measureRow(ASSET_NAME);
    if (!row.reachable)
      throw new Error(
        `SETUP GUARD: ${ASSET_NAME} has a rect but nothing paints there — ${JSON.stringify(row)}`
      );
...
async function phaseStrip() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    const shape = await A.call(d, function () {
      ...
      const stripText = (root.innerText || "")
        .replace(editor.innerText || "", "")
        .replace(/\s+/g, " ")
        .trim();
      return {
        ...
        pluginControls: root.querySelectorAll(
          "[data-text-editor-preview-id], [data-text-editor-action-id]"
        ).length,
    ...
    run.check(
      "S-1",
      "the tab has no bottom strip of its own (editor reaches the bottom of the tab)",
    ...
    run.check(
      "S-2",
      "the encoding control no longer lives inside the tab",
      shape.hasEncodingToken === false,
      `token inside tab: ${shape.hasEncodingToken}`
    );
...
async function phasePlacement() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    if (!bar.cells.length)
      throw new Error(
        "SETUP GUARD: no [data-status-bar-entry-id] cells — add the hook or fix the selector"
      );
    ...
    run.check(
      "P-1",
      "the file name is shown in the status bar",
      Boolean(fileCell),
      JSON.stringify(bar.cells.map((c) => `${c.id}@${c.x}`))
    );
    run.check(
      "P-2",
      "the file name is the RIGHTMOST cell of the left cluster",
    ...
    run.check(
      "P-3",
      "selection, line ending and encoding are the three cells nearest the centre on the right",
      firstThree.length === 3 &&
...
async function phaseSplit() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    const textCells = withText.cells.filter((c) =>
      /encoding|eol|line-?ending|selection|cursor|file-?name/i.test(c.id)
    );
    run.check(
      "T-1",
      "the four cells are present while a text tab is active",
      textCells.length === 4,
    ...
    const switched = await A.call(d, function () {
      ...
      const other = tabs.find(
        (t) => !/text-editor/.test(t.getAttribute("data-editor-tab-id") || "")
      );
    ...
    const leftover = without.cells.filter((c) =>
      /encoding|eol|line-?ending|selection|cursor|file-?name/i.test(c.id)
    );
    run.check(
      "T-2",
      "switching to a non-text tab removes all four",
      leftover.length === 0,
      JSON.stringify(leftover.map((c) => c.id))
    );
...
async function phaseMenu() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    run.check(
      "M-1",
      "the menu offers reopen-with and save-with",
      menu.rows.filter((r) => /reopen|重新打开|save|保存/i.test(r)).length >= 2,
      JSON.stringify(menu.rows)
    );
    run.check(
      "M-2",
      "the menu opens upward and is fully on screen (not clipped by the window edge)",
...
async function phaseSelection() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    run.check(
      "X-1",
      "with no selection the cell reads a caret position",
      Boolean(noSelection) && /\d/.test(noSelection),
      String(noSelection)
    );
    run.check(
      "X-2",
      "selecting text makes the cell report the selected count",
...
async function phaseEol() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    run.check(
      "L-0",
      "a new file on Windows is written with the OS line ending",
    ...
    run.check(
      "L-1",
      "switching to LF removes every CRLF from the bytes on disk",
...
async function phasePersist() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    run.check(
      "R-1",
      "the chosen encoding is written into the asset record (so it travels through VCS)",
      Boolean(record) && /gbk/i.test(JSON.stringify(record)),
      JSON.stringify(record).slice(0, 200)
    );

    // Close and reopen: the file must come back decoded, not as mojibake.
    await A.call(d, function () {
      const t = Array.from(document.querySelectorAll("[data-editor-tab-id]")).find((e) =>
        /text-editor/.test(e.getAttribute("data-editor-tab-id") || "")
      );
    ...
    const state = await A.call(d, function () {
      ...
      const cells = Array.from(document.querySelectorAll("[data-status-bar-entry-id]")).filter(
        (e) => /encoding/i.test(e.getAttribute("data-status-bar-entry-id"))
      );
      return {
    ...
    run.check(
      "R-2",
      "reopening the asset uses the remembered encoding, with no replacement characters",
      state.lossy === false && /gbk/i.test(state.encoding || ""),
  ...
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    const m = await A.call(d, function () {
      ...
      return {
        ...
        surfaceOpacityVar: getComputedStyle(document.documentElement)
          .getPropertyValue("--nl-editor-surface-opacity")
          .trim(),
        topGap:
          editor && firstLine
    ...
    run.check(
      "U-1",
      "the first line is not flush against the top of the editor",
      m.topGap !== null && m.topGap >= 4,
      `${m.topGap}px`
    );
    run.check(
      "U-2",
      "the editor surface carries the workspace surface opacity rather than being a solid slab",
...
async function phaseRegress() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    const errors = await A.call(d, function () {
      return (window.__nlsErr || []).slice(0, 5);
    });
    run.check(
      "G-1",
      "no page errors collected while driving the text editor",
      errors.length === 0,
