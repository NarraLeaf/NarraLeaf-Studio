...
function phaseCatalog() {
    ...
    for (const file of fs.readdirSync(dir).filter((f) => f.endsWith('.ts'))) {
        fs.readFileSync(path.join(dir, file), 'utf8').split(/\r?\n/).forEach((line, i) => {
            if (/No [^"']*\byet\b/.test(line) || /Nothing on /.test(line) || /No item selected/.test(line)) {
                hits.push({ where: `${file}:${i + 1}`, text: line.trim().slice(0, 90) });
            }
        });
    }
    const strays = hits.filter((h) => !KEEP.some((k) => h.text.includes(k)));
    run.check('B-1', 'only the two deliberate survivors remain in the en catalogue', strays.length === 0,
        `${hits.length} total, ${strays.length} off the keep-list: ${JSON.stringify(strays.slice(0, 8))}`);

    let parity = 'not run';
    try {
        execFileSync('yarn', ['vitest', 'run', 'src/shared/i18n', '--silent'], { cwd: REPO, encoding: 'utf8', shell: true, stdio: 'pipe' });
        parity = 'green';
    } catch (e) {
        parity = `RED: ${String(e.stdout || e.message).split('\n').filter((l) => /FAIL|✕|Error/.test(l)).slice(0, 4).join(' | ')}`;
    }
...
async function phaseInspector() {
    await D.onWindow('workspace', A.WINDOWS.workspace, async (d) => {
        ...
        const select = async (index) => {
            ...
            if (!row.reachable) throw new Error(`SETUP GUARD: story row ${index} has a rect but is not reachable`);
        ...
        run.check('B-3', 'no "Stage name" in the story inspector', !/Stage name/.test(character), character.slice(0, 200));

        // Row 9 is a dialogue — the one that carried a bare uuid as "Text ID".
        const dialogue = await select(8);
        const uuid = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/;
        run.check('B-4', 'no bare uuid shown in the inspector by default', !uuid.test(dialogue), dialogue.slice(0, 200));
...
async function phaseDashboard() {
    await D.onWindow('workspace', A.WINDOWS.workspace, async (d) => {
        ...
        await A.sleep(1800);
        if (!(await A.call(d, function () { return /Scale|Writing activity/.test(document.body.innerText || ''); }))) {
        ...
        run.check('B-5', 'dashboard first screen has at most 6 one-integer tiles', tiles.length <= 6,
            `${tiles.length}: ${JSON.stringify(tiles)}`);

        const truncated = await A.call(d, A.TRUNCATED_LABELS);
        run.check('B-6', 'no label is truncated on the dashboard first screen', truncated.length === 0,
        ...
        const numbers = await A.call(d, function () {
            ...
            return { scenes: grab('Scenes'), assets: grab('Assets'), variables: grab('Variables'), characters: grab('Characters') };
        });
        run.note(`B-11 dashboard numbers: ${JSON.stringify(numbers)} (compare against the pre-change run)`);

        const surface = await A.panelSurface(d, 'Properties');
        run.check('B-7-dashboard', 'dashboard right rail sits on an opaque surface',
            surface.ok && A.alphaOf(surface.backgroundColor) === 1, JSON.stringify(surface));
    ...
    await D.onWindow('workspace', A.WINDOWS.workspace, async (d) => {
        ...
        const background = await A.call(d, function () {
            const el = Array.from(document.querySelectorAll('*'))
                .find((e) => /url\(/.test(getComputedStyle(e).backgroundImage));
            return el ? getComputedStyle(el).backgroundImage.slice(0, 60) : null;
        });
        if (!background) throw new Error('SETUP GUARD: no workspace background image is set — this is measured with it ON');
        run.note(`background image on: ${background}`);

        for (const [id, panel] of [['assets', '^Assets$'], ['characters', '^Characters$']]) {
            await A.clickNamed(d, '[aria-label]', panel);
            await A.sleep(1600);
            const surface = await A.panelSurface(d, 'Properties');
            run.check(`B-7-${id}`, `${id} inspector sits on an opaque surface`,
                surface.ok && A.alphaOf(surface.backgroundColor) === 1, JSON.stringify(surface));
        }
...
async function phaseNames() {
    await D.onWindow('workspace', A.WINDOWS.workspace, async (d) => {
        ...
        run.check('B-8', 'no visible button is without an accessible name', total === 0, JSON.stringify(seen));
    });
}
...
async function phaseTimeline() {
    await D.onWindow('dev-mode', A.WINDOWS.devmode, async (d) => {
        ...
        run.check('B-9', 'the editor placeholder does not appear on the read-only timeline',
            !/Double-click to enter/i.test(text), text.slice(0, 200));
    });
}
