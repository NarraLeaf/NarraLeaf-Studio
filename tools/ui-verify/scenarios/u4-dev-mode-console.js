...
const READ_EDITOR = function () {
    ...
    const rows = nodes.map((el) => {
        ...
        const categoryHues = Array.from(el.querySelectorAll('*')).map((n) => {
            ...
            return { bg: cs.backgroundColor, w: Math.round(n.getBoundingClientRect().width), op: cs.opacity };
        }).filter((s) => s.bg !== 'rgba(0, 0, 0, 0)' && s.w === 3 && Number(s.op) < 1).map((s) => s.bg);
        return { id: el.getAttribute('data-story-row-block-id'), y: Math.round(r.y), sentence, categoryHues };
    }).sort((a, b) => a.y - b.y);
    const onScreen = nodes.find((n) => {
        const r = n.getBoundingClientRect();
        return r.y >= 0 && r.y + r.height <= innerHeight;
    });
    if (!onScreen) return { ok: true, reachable: false, why: 'no row is fully inside the viewport (list scrolled)', count: rows.length, rows };
    ...
    const rows = Array.from(panel.querySelectorAll('li, [data-timeline-row], [role="listitem"]'))
        .map((el) => {
            ...
            return {
                ...
                sentence: raw.replace(/^\d+\s*/, '').replace(/\s*▶\s*$/, '').trim(),
                hues: hues.filter((h) => h.bg !== 'rgba(0, 0, 0, 0)' && h.w > 0 && h.w <= 6).map((h) => h.bg)
                    .concat(hues.filter((h) => h.w > 0 && h.w <= 20 && h.h > 0 && h.h <= 20).map((h) => h.color)),
                y: Math.round(r.y),
            };
...
const CONTEXT_TAB = function () {
    const tabs = document.querySelector('[role="tablist"]');
    if (!tabs) return null;
    const labels = Array.from(tabs.querySelectorAll('[role="tab"]')).map((t) => (t.textContent || '').trim());
...
async function phaseEditor(baseline) {
    return D.onWindow('workspace', A.WINDOWS.workspace, async (d) => {
        if (!(await A.call(d, function () { return Boolean(document.querySelector('[data-story-row-block-id]')); }))) {
        ...
        if (!res.reachable) throw new Error(`SETUP GUARD: editor rows present but not reachable — ${res.why}`);
        if (res.count !== 12) throw new Error(`SETUP GUARD: expected 12 "${D.SCENE}" rows, saw ${res.count}`);
        await d.screenshot('u4-editor');

        if (baseline && baseline.editor) {
            const diffs = res.rows
                .map((r, i) => (norm(r.sentence) === norm(baseline.editor[i].sentence)
                    ? null : `${i + 1}: "${baseline.editor[i].sentence}" -> "${r.sentence}"`))
                .filter(Boolean);
            run.check('A-6', 'editor sentences unchanged vs the recorded baseline', diffs.length === 0, diffs);
        } else {
...
async function phaseTimeline(editorRows) {
    return D.onWindow('dev-mode', A.WINDOWS.devmode, async (d) => {
        ...
        if (tl.count !== 12) throw new Error(`SETUP GUARD: expected 12 timeline rows, saw ${tl.count} — is the running scene "${D.SCENE}"?`);
        ...
        const SPEAKERS = { 3: 'Youk', 4: 'YouKi', 5: 'Nattou', 6: 'YouKi', 7: 'YouKi', 8: 'YouKi', 9: 'Narra' };
        ...
        run.check('A-1a', 'non-dialogue rows: timeline sentence == editor sentence verbatim', plain.length === 0, plain.length ? plain : '5/5 equal');
        ...
        run.check('A-1b', 'dialogue rows: timeline == "<speaker>: <editor body>"', dlg.length === 0, dlg.length ? dlg : '7/7 equal');

        run.check('A-3b', 'row 9 keeps its inline variable reference', /\bOK\b.*\ba\b/.test(norm(tl.rows[8].sentence)), norm(tl.rows[8].sentence));
        run.check('A-2', 'row 2 carries no engine vocabulary',
            !/character enter/i.test(norm(tl.rows[1].sentence)) && !/·\s*character\b/.test(norm(tl.rows[1].sentence)), norm(tl.rows[1].sentence));
        run.check('A-3', 'row 12 names the asset, not the enum',
            /outside_s\.jpg/.test(norm(tl.rows[11].sentence)) && !/\bsetBackground\b/.test(norm(tl.rows[11].sentence)), norm(tl.rows[11].sentence));
        ...
        const hued = editorRows.map((r, i) => ({ line: i + 1, editor: r.categoryHues || [] })).filter((p) => p.editor.length);
        ...
        run.check('A-4', 'every row the editor hues carries the same hue on the timeline',
            hued.length > 0 && matched.length === hued.length, `${matched.length}/${hued.length}; ${JSON.stringify(pairs)}`);
        ...
        run.check('A-5', 'dialogue rows 3-9 name their speaker', named.length === 7, `${named.length}/7`);
        ...
        run.check('A-10', 'no engine vocabulary anywhere in the panel', hits.length === 0, hits.length ? hits : 'clean');
        return tl.rows;
    });
}
...
async function phaseContext() {
    await D.onWindow('dev-mode', A.WINDOWS.devmode, async (d) => {
        ...
        run.check('A-7a', 'an execution-context tab is present (not hidden when the stack is empty)', Boolean(tab), `tab=${tab}`);
        if (!tab) {
        ...
        run.check('A-7', `context names the running scene "${D.SCENE}"`, new RegExp(D.SCENE).test(text), text.slice(0, 200));
    });
}
...
async function phaseScenes(baseline) {
    await D.onWindow('dev-mode', A.WINDOWS.devmode, async (d) => {
        ...
        run.check('A-11', 'scene node title renders at >= 11px', worst >= 11,
        ...
        run.check('A-12', 'every scene node is fully inside the pane', inside === g.nodes.length, `${inside}/${g.nodes.length}`);
    });

    return D.onWindow('workspace', A.WINDOWS.workspace, async (d) => {
        const flowTab = await A.probe(d, '[data-editor-tab-id]', 'Scene Flow', 'i', 0);
        if (flowTab.found && flowTab.reachable) { await d.click(flowTab.cx, flowTab.cy); await A.sleep(1600); }
        const g = await A.call(d, A.SCENE_GRAPH);
        if (!g.ok) { run.note(`A-14 skipped: ${g.why} — open the workspace Scene Flow tab first`); return null; }
        const fonts = g.nodes.map((n) => n.cssFont);
        if (baseline && baseline.workspaceFlowFonts) {
            run.check('A-14', 'workspace story-flow node font unchanged (the Dev Mode fix must not leak)',
...
async function phaseJump() {
    await D.onWindow('dev-mode', A.WINDOWS.devmode, async (d) => {
        ...
        for (const line of [10, 5, 11, 3, 9]) {
            const target = await A.call(d, function (n) {
                const tabs = document.querySelector('[role="tablist"]');
                const rows = Array.from(tabs.parentElement.querySelectorAll('li, [data-timeline-row], [role="listitem"]'))
            ...
            if (!target || !target.reachable) { landed.push({ target: line, got: target ? 'unreachable' : 'not found' }); continue; }
        ...
        run.check('A-13', 'timeline jump lands on the clicked row (5 blocking rows)', ok === 5, JSON.stringify(landed));
    });
}
...
async function phaseFixture() {
    await D.onWindow('dev-mode', A.WINDOWS.devmode, async (d) => {
        ...
        const node = await A.call(d, function () {
            const n = Array.from(document.querySelectorAll('.react-flow__node')).find((e) => /Nesting Lab/.test(e.innerText || ''));
            if (!n) return null;
            const r = n.getBoundingClientRect();
            const cx = Math.round(r.x + r.width / 2);
            const cy = Math.round(r.y + r.height / 2);
        ...
        if (!node) throw new Error('SETUP GUARD: no "Nesting Lab" node — run fixtures/nesting-lab.js against the project copy');
        if (!node.reachable) throw new Error('SETUP GUARD: the Nesting Lab node has a rect but is occluded');
        ...
        for (let t = 0; t < 40; t += 1) {
            ...
            if (t === 39) throw new Error('SETUP GUARD: Nesting Lab never reached the stage after relaunch');
        }
        await A.sleep(800);
        ...
        run.check('A-9', 'context shows the live repeat round as n/3',
            Boolean(repeat.text) && /\b[1-3]\s*\/\s*3\b/.test(repeat.text), repeat.text || repeat.why);
        ...
        run.check('A-8b', 'the parallel lists both branches by sentence, running one marked',
            Boolean(parallel.text) && /Parallel branch A\./.test(parallel.text)
                && /Parallel branch B\./.test(parallel.text) && /▸/.test(parallel.text), parallel.text || parallel.why);
        const engineWords = [/actionType/, /menu:action/, /\bbranchWaitType\b/, /\b[0-9a-f]{8}…/];
        const hits = parallel.text ? engineWords.filter((re) => re.test(parallel.text)).map(String) : ['(no context text)'];
        run.check('A-10b', 'no engine vocabulary in the context tab while inside a parallel', hits.length === 0, hits.length ? hits : 'clean');
    });
}
...
(async () => {
    ...
    if (phase === 'all' || phase === 'editor' || phase === 'timeline') record.editor = await phaseEditor(baseline);
    if (phase === 'all' || phase === 'timeline') await phaseTimeline(record.editor);
    if (phase === 'all' || phase === 'context') await phaseContext();
    if (phase === 'all' || phase === 'scenes') record.workspaceFlowFonts = await phaseScenes(baseline);
