...
const SNAPSHOT = function () {
  ...
  for (const row of document.querySelectorAll("[data-story-row-block-id]")) {
    out[row.getAttribute("data-story-row-block-id")] = (row.innerText || "")
      .replace(/\s+/g, " ")
      .trim();
  }
  return out;
...
function print(report) {
  ...
  for (const entry of report) {
    ...
    console.log(
      `   glyph top (row-relative)  cold=${round(glyph(a))}  hover=${round(glyph(b))}  editing=${round(glyph(c))}  escaped=${round(glyph(dd))}  cold2=${round(glyph(e))}`
    );
    console.log(
      `   body cell height          read=${round(b && b.cell && b.cell.height)}  editing=${round(c && c.cell && c.cell.height)}`
    );
    console.log(
      `   body cell width           read=${round(b && b.cell && b.cell.width)}  editing=${round(c && c.cell && c.cell.width)}`
    );
    console.log(`   rendered lines            read=${lines(b)}  editing=${lines(c)}`);
    console.log(
      `   row height                read=${round(b && b.row.height)}  editing=${round(c && c.row.height)}`
    );
  }
  console.log("");
}
...
function checkPhase(phase, report) {
  ...
  const check = (name, pick, unit) => {
    ...
    record(
      `[${phase}] ${name}`,
      measured.length > 0 && bad.length === 0,
      measured.length === 0
        ? "NOTHING MEASURED"
        : bad.length === 0
          ? `worst=${worst(pick)}${unit} over ${measured.length} rows`
          : JSON.stringify(
              bad.map((entry) => ({ id: entry.id.slice(0, 8), px: round(entry.delta) }))
            )
    );
  };

  record(
    `[${phase}] every sampled row measured in both modes`,
    measured.length === report.length && report.length > 0,
    `measured=${measured.length}/${report.length}`
  );
  record(
    `[${phase}] clicking the body opened the row for editing`,
  ...
  record(
    `[${phase}] Escape closed the editor again`,
  ...
  check(
    "hovering a row does not move its words",
    (entry) => entry.b.glyph.top - entry.a.glyph.top,
    "px"
  );
  check(
    "opening a row for editing does not move its words",
    (entry) => entry.c.glyph.top - entry.b.glyph.top,
    "px"
  );
  check(
    "opening a row for editing does not re-wrap its words",
    (entry) => entry.c.cell.width - entry.b.cell.width,
    "px"
  );
  check(
    "the words are back where they started after Escape",
    (entry) => entry.e.glyph.top - entry.a.glyph.top,
    "px"
  );
  ...
  check(
    "the editor cell is the same box as the read-only cell",
    (entry) => entry.c.cell.height - entry.b.cell.height,
    "px"
  );
  ...
  await withDriver({ target: "workspace", port: PORT }, async (d) => {
    ...
    for (const row of rows.slice(0, 6)) {
      ...
      if (!entry) {
        ...
        record(
          `row ${row.id.slice(0, 6)} body is reachable`,
          false,
    ...
    record(
      "narrowing the window wrapped some rows",
      wrapped.length > 0,
      `wrappedRows=${wrapped.length} at ${NARROW_WIDTH}px`
    );
    ...
    record(
      "the drive changed no row text",
      shared.length > 0 && changed.length === 0,
      `compared ${shared.length} rows present in both samples, changed=${changed.length}` +
        (changed.length
          ? ` ${JSON.stringify(changed.slice(0, 2).map((id) => [before[id], after[id]]))}`
          : "")
    );
  });
