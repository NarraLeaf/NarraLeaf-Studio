...
async function main() {
  const seeded = await onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    run.check(
      "M1",
      'the binding was stored with valueType "boolean"',
      result.bindingValueType === "boolean",
      result.bindingValueType
    );
    run.check(
      "M2",
      "the bound switch is authored OFF, so only the binding can turn it on",
      result.authoredChecked !== true,
      result.authoredChecked
    );
    return result;
  });
  ...
  await onWindow("dev-mode", A.WINDOWS.devmode, async (d) => {
    ...
    run.check(
      "M3a",
      "both seeded switches are on screen in the running game",
      Boolean(bound && bound.onScreen && plain && plain.onScreen),
      { bound, plain }
    );
    ...
    run.check(
      "M3b",
      "the bound switch renders CHECKED although its authored value is false",
      bound.checked === "true",
      { authored: seeded.authoredChecked, rendered: bound.checked }
    );
    run.check(
      "M3c",
      "the unbound switch beside it is still off (the binding is not global)",
      plain.checked === "false",
      plain.checked
    );
    ...
    run.check(
      "M4b",
      "dragging across the track toggles it on",
      now.checked === "true",
      now.checked
    );
    ...
    run.check(
      "M4c",
      "dragging back across turns it off again",
      now.checked === "false",
      now.checked
    );
    ...
    run.check(
      "M4d",
      "a drag that stops short of halfway does NOT toggle",
      now.checked === "false",
      now.checked
    );
    ...
    run.check(
      "M4e",
      "a press with no movement still toggles (click not regressed)",
      now.checked === "true",
      now.checked
    );
  });
