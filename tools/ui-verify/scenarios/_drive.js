...
async function driveToDevMode() {
  ...
  if (!(await listTargets({ port: PORT })).find((t) => /dev-mode/i.test(`${t.title}${t.url}`))) {
    await onWindow("workspace", A.WINDOWS.workspace, (d) =>
      A.clickNamed(d, "[aria-label]", "^Run Dev Mode$")
    );
  ...
  await onWindow("dev-mode", A.WINDOWS.devmode, async (d) => {
    ...
    const findMenu = () =>
      A.call(d, function () {
        const el = Array.from(document.querySelectorAll("*")).find(
          (e) => (e.textContent || "").trim() === "New Game" && e.children.length === 0
        );
        if (!el) return null;
    ...
    throw new Error(
      `SETUP GUARD: the story never reached the stage (clicked "New Game" ${clicks}x); stage="${last.slice(0, 160)}"`
    );
  });
}
  ...
  const menuOpen = await A.call(d, function () {
    return Boolean(
      Array.from(document.querySelectorAll("[aria-label]")).find(
        (e) => e.getAttribute("aria-label") === "Close preview debug tools menu"
      )
    );
  });
  if (!menuOpen) {
