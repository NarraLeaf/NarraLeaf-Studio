...
function onWorkspace(fn) {
  return withDriver(
    { target: "workspace", port: PORT, outDir: OUT, prefix: "2026-07-29-camera-" },
    async (d) => {
...
async function openScene(d) {
  ...
  if (hasRows === 0) {
    ...
    for (let attempt = 0; attempt < 6; attempt += 1) {
      const outlineVisible = await A.call(
        d,
        function (name) {
    ...
    for (let attempt = 0; attempt < 12 && !(hit && hit.reachable); attempt += 1) {
      await A.call(
        d,
        function (name) {
          const leaf = [...document.querySelectorAll("span, div")].find(
      ...
      hit = await A.call(
        ...
        function (name) {
          ...
          return {
            ...
            cy,
            reachable: Boolean(
              target && (target === leaf || leaf.contains(target) || target.contains(leaf))
            )
          };
        },
        SCENE
...
const FOCUS_SLOT = function () {
  ...
  const area =
    areas.find((e) => /narration|旁白/i.test(e.placeholder || "")) ||
...
async function commitCommand(d, line) {
  const before = await A.call(d, function () {
    return [...document.querySelectorAll("[data-story-row-block-id]")].map((e) =>
      e.getAttribute("data-story-row-block-id")
    );
  });
  ...
  if (typed !== line)
    throw new Error(
      `SETUP GUARD: the slot holds ${JSON.stringify(typed)}, not ${JSON.stringify(line)}`
    );
  ...
  if (added.length !== 1)
    throw new Error(`expected exactly one new row for "${line}", got ${added.length}`);
  return added[0];
}
...
const PROPERTIES_OPEN = function () {
  const heading = [...document.querySelectorAll("*")].find(
    (e) => e.children.length === 0 && /^(Properties|属性)$/.test((e.textContent || "").trim())
  );
...
const CAMERA_SECTION = function () {
  const heading = [...document.querySelectorAll("div")].find(
    (e) =>
      e.children.length === 0 &&
      /^(Camera · story-wide|镜头 · 跨场景保留)/.test((e.textContent || "").trim())
  );
  ...
  return {
    ...
    viewfinder: frame
      ? {
          ...
          barIsSibling: Boolean(
            inner &&
            [...frame.children].some(
              (c) => c !== inner && /rgba\(0, 0, 0/.test(getComputedStyle(c).backgroundColor)
            )
          )
        }
      : null,
  ...
  const hit = await A.call(
    ...
    function (wanted) {
      ...
      return {
        ...
        cy,
        reachable: Boolean(
          target && (target === button || button.contains(target) || target.contains(button))
        )
      };
    },
    label
...
const MOTION_PICKER = function () {
  const panel = [...document.querySelectorAll("div")].find(
    (e) => /max-h-\[420px\]/.test(e.className || "") && e.getBoundingClientRect().width > 200
  );
  if (!panel) return null;
  const buttons = [...panel.querySelectorAll("button")];
  const tabButtons = buttons.filter((b) =>
    /^(Project|Presets|工程|预设)$/.test((b.textContent || "").trim())
  );
  const cards = buttons
  ...
  return {
    ...
    activeTab:
      (tabButtons.find((b) => /text-primary/.test(b.className || "")) || {}).textContent?.trim() ||
      null,
    cards,
    headings: [...panel.querySelectorAll("section > div")]
...
async function main() {
  ...
  const blockId = await onWorkspace(async (d) => {
    ...
    record(
      "six-way operation picker is present",
      sixWay.length >= 6,
      `buttons=${JSON.stringify(initial.buttons)}`
    );
    record(
      "viewfinder is drawn at the stage aspect ratio",
      Boolean(initial.viewfinder) && /\d+\s*\/\s*\d+/.test(initial.viewfinder.aspectRatio || ""),
      initial.viewfinder
        ? `aspectRatio=${initial.viewfinder.aspectRatio} rect=${JSON.stringify(initial.viewfinder.rect)}`
        : "no viewfinder"
    );
    record(
      "dialogue bar sits outside the moving stage rect",
      Boolean(initial.viewfinder && initial.viewfinder.barIsSibling),
    ...
    record(
      "viewfinder shows the committed zoom",
      /scale\(1\.8\)/.test(initial.viewfinder?.stageTransform || ""),
    ...
    record(
      "dragging the zoom slider redraws the viewfinder",
      Boolean(slid) && /scale\(3\)/.test(afterSlide.viewfinder?.stageTransform || ""),
    ...
    const frameBox = await A.call(d, function () {
      ...
      const section = heading && heading.closest("section");
      const frame =
        section && [...section.querySelectorAll("div")].find((e) => e.style && e.style.aspectRatio);
      if (!frame) return null;
      const r = frame.getBoundingClientRect();
      return { x: r.x, y: r.y, w: r.width, h: r.height, cursor: getComputedStyle(frame).cursor };
    });
    record(
      "pan turns the viewfinder into a drag surface",
      frameBox && frameBox.cursor === "crosshair",
      `cursor=${frameBox && frameBox.cursor}`
    );
    // Drag to the right quarter, one quarter down from the top.
    await d.click(
      Math.round(frameBox.x + frameBox.w * 0.75),
      Math.round(frameBox.y + frameBox.h * 0.25)
    );
    await A.sleep(900);
    ...
    record(
      "a click on the viewfinder writes the align it landed on",
    ...
    record(
      "motion replaces the pose controls with the motion field",
      motionState.sliders === 0 &&
        !motionState.viewfinder &&
        /(Choose motion|选择动作|动作)/.test(motionState.text),
    ...
    record(
      "the picker offers both halves and opens on the useful one",
      Boolean(opened) && opened.tabs.length === 2 && Boolean(opened.activeTab),
      opened
        ? `tabs=${JSON.stringify(opened.tabs)} active=${opened.activeTab} projectCameraMotions=${cameraMotionsOnDisk()}`
        : "no picker"
    );
    ...
    const shown = picker
      ? cameraNames.filter((name) => picker.cards.some((card) => card.includes(name)))
      : [];
    const spriteNames = ["Heartbeat", "Nod", "Breathe", "心跳", "点头", "呼吸"];
    const leaked = picker
      ? spriteNames.filter((name) => picker.cards.some((card) => card.includes(name)))
      : [];
    record("camera presets are listed", shown.length >= 4, `shown=${JSON.stringify(shown)}`);
    record(
      "sprite presets are NOT offered to the camera",
      leaked.length === 0,
    ...
    record(
      "the cards are parked on distinguishable poses",
    ...
    if (!pickTarget)
      throw new Error(`no shake card to pick; cards=${JSON.stringify(picker.cards).slice(0, 300)}`);
    ...
    record(
      'the row names the bound motion, not just "Motion"',
      Boolean(rowText) && /(Shake|震动)/.test(rowText),
      `row="${rowText}"`
    );
    ...
    const spriteShown = sprite
      ? spriteWanted.filter((name) => sprite.cards.some((card) => card.includes(name)))
      : [];
    const cameraLeaked = sprite
      ? ["Dutch tilt", "Pan sweep", "荷兰角", "横摇"].filter((name) =>
          sprite.cards.some((card) => card.includes(name))
        )
      : [];
    record(
      "sprite presets are listed for a portrait",
      spriteShown.length >= 4,
      `shown=${JSON.stringify(spriteShown)}`
    );
    record(
      "camera shots are NOT offered to a portrait",
      cameraLeaked.length === 0,
      `leaked=${JSON.stringify(cameraLeaked)}`
    );
    record(
      "the categories are the displayable ones",
      Boolean(sprite) && sprite.headings.length >= 4 && !sprite.headings.includes("Camera"),
    ...
    record(
      "the presets audition on a portrait-shaped subject, legible at card scale",
  ...
  const storyIndex = JSON.parse(
    fs.readFileSync(path.join(PROJECT, "editor/story/index.json"), "utf8")
  );
  const doc = JSON.parse(
    fs.readFileSync(path.join(PROJECT, storyIndex.stories[0].documentPath), "utf8")
  );
  const block = Object.values(doc.scenes)
    .flatMap((scene) => Object.values(scene.blocks))
    .find((b) => b.id === blockId);
  record(
    "the row persisted as a camera motion payload",
    Boolean(block) &&
      block.payload.action === "camera" &&
      block.payload.operation === "motion" &&
      Boolean(block.payload.motion?.animationId),
    JSON.stringify(block && block.payload)
  );

  const animIndex = path.join(PROJECT, "editor/story/animations/index.json");
  const anims = fs.existsSync(animIndex)
    ? JSON.parse(fs.readFileSync(animIndex, "utf8"))
    : { animations: [] };
  const cameraAsset = (anims.animations || []).find((a) => a.targetKind === "camera");
  record(
    "the created asset is filed under the camera target kind",
    Boolean(cameraAsset),
    JSON.stringify(cameraAsset || (anims.animations || []).map((a) => a.targetKind))
  );
