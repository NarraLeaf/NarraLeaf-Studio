...
const ROW_STATE = function (blockId) {
  ...
  if (!row) return null;
  const host =
    row.querySelector('[contenteditable="true"]') || row.querySelector("[data-story-row-text]");
  if (!host) return null;
  const bold = Array.from(host.querySelectorAll("*")).some((el) => {
    const weight = getComputedStyle(el).fontWeight;
    return (
      el.tagName === "B" || el.tagName === "STRONG" || Number(weight) >= 600 || weight === "bold"
    );
  });
  return {
    editing: Boolean(row.querySelector('[contenteditable="true"]')),
...
const SNAPSHOT = function () {
  return Array.from(document.querySelectorAll("[data-story-row-block-id]")).map((row) => {
    ...
    return (
      row.getAttribute("data-story-row-block-id") +
      " @" +
      indent +
      " " +
...
async function openScene(d) {
  if (
    await A.call(d, function () {
      return document.querySelectorAll("[data-story-row-block-id]").length > 0;
    })
  )
    return;
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const visible = await A.call(
      d,
      function (name) {
  ...
  if (!hit || !hit.reachable)
    throw new Error(`SETUP GUARD: scene "${SCENE}" never became clickable`);
...
async function editRow(d, blockId) {
  ...
  if (!point || !point.reachable)
    throw new Error(`SETUP GUARD: row ${blockId} body is not reachable`);
...
async function main() {
  if (!process.env.NLS_VERIFY_PROJECT)
    throw new Error("set NLS_VERIFY_PROJECT to the project COPY this run may open");

  await withDriver({ target: "workspace", port: PORT }, async (d) => {
    await A.assertVisible(d, A.WINDOWS.workspace, PID);
    await openScene(d);
    record(
      "the scene starts with no row open for editing",
      await leaveEdit(d),
      "a previous run may have left one open"
    );
    ...
    const rows = await A.call(d, function () {
      return Array.from(document.querySelectorAll("[data-story-row-block-id]"))
        .map((row) => {
          ...
          return {
            id: row.getAttribute("data-story-row-block-id"),
    ...
    record(
      "the strip starts collapsed for this phase",
      collapsedBefore.present && !collapsedBefore.expanded,
      JSON.stringify(collapsedBefore)
    );
    record(
      "the caret is in the line before the first Tab",
      caretBefore.where === "editor",
      JSON.stringify(caretBefore)
    );
    ...
    record(
      "Tab opens the collapsed strip",
      Boolean(afterFirstTab.present && afterFirstTab.expanded),
      JSON.stringify({
        expanded: afterFirstTab.expanded,
        controls: (afterFirstTab.titles || []).length
      })
    );
    ...
    record(
      "opening the strip leaves the caret in the line",
      focusAfterFirstTab.where === "editor",
    ...
    record(
      ...
      forward.where === "toolbar" && forward.index === 2,
    ...
    record(
      ...
      back.where === "toolbar" && back.index === 1,
      JSON.stringify(back)
    );
    ...
    record(
      "Shift+Tab off the first control reaches collapse",
      wrappedToCollapse.index === 0,
    ...
    record(
      "and wraps round to the last control",
      wrappedToEnd.index === wrappedToEnd.count - 1,
    ...
    record(
      "Tab returns to bold with a selection held",
      onBoldAgain.where === "toolbar" && onBoldAgain.index === 1,
    ...
    record(
      "activating bold marks the text",
      Boolean(boldOn && boldOn.bold && !(beforeBold && beforeBold.bold)),
    ...
    record(
      "focus stays on the control after activating",
      focusAfterActivate.where === "toolbar" && focusAfterActivate.index === 1,
      JSON.stringify(focusAfterActivate)
    );
    const pressed = await A.call(d, STRIP);
    record(
      "the control reports itself pressed",
      pressed.pressed[1] === "true",
      `aria-pressed=${JSON.stringify(pressed.pressed.slice(0, 3))}`
    );
    ...
    record(
      "activating again takes the mark back off",
      Boolean(boldOff && !boldOff.bold),
    ...
    if (pauseIndex > 0) {
      ...
      record(
        "Tab walks to the insert-pause control",
        onPause.index === pauseIndex,
        JSON.stringify(onPause)
      );

      const beforePause = await A.call(
        d,
        function (id) {
          const row = document.querySelector('[data-story-row-block-id="' + id + '"]');
      ...
      record(
        "Space activates the control too",
        afterPause === beforePause + 1,
        `pauses ${beforePause} -> ${afterPause}`
      );
      record(
        "focus stays on it after an insertion",
        focusAfterPause.index === pauseIndex,
    ...
    if (paletteIndex > 0) {
      ...
      if (from.where === "toolbar") {
        ...
        record(
          "Escape closes the palette and stays in the strip",
          !afterFirst &&
            whereAfterFirst.where === "toolbar" &&
            whereAfterFirst.index === paletteIndex,
    ...
    record(
      "Escape hands the caret back to the line",
      backInLine.where === "editor",
      JSON.stringify(backInLine)
    );
    record(
      "and the row is still open for editing",
      Boolean(stillEditing && stillEditing.editing),
    ...
    record(
      "a second Escape leaves edit mode, as it always did",
    ...
    record(
      "the drive left every row exactly as it found it",
