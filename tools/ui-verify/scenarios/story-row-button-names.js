...
async function main() {
  ...
  await withDriver({ target: "workspace", port: PORT }, async (d) => {
    ...
    record(
      "the active row exposes its button cluster",
      controls.length >= 3,
    ...
    record(
      "every row control has an accessible name",
      nameless.length === 0,
    ...
    const weaker = controls.filter((control) => {
      ...
      const titleWithoutKeys = control.title
        .replace(/[\s([（【][^)\]）】]*[)\]）】]\s*$/u, "")
        .trim();
      return titleWithoutKeys.length > control.announced.trim().length;
    });
    record(
      "no control announces less than its tooltip shows",
      weaker.length === 0,
      weaker.length === 0
    ...
    record(
      "the insert button names what it inserts",
    ...
    const remove = controls.find((control) =>
      /^Delete this row|删除此行/i.test(control.title || "")
    );
    record(
      "the delete button names what it deletes",
    ...
    console.log(
      `\ndistinct named controls on the active row:\n${[...distinct.values()]
        .map(
          (c) =>
            `  announced=${JSON.stringify(c.announced)}  aria=${JSON.stringify(c.aria)}  title=${JSON.stringify(c.title)}`
        )
        .join("\n")}`
    );
  });
