...
const INSTALL_ERROR_SINK = function () {
  ...
  console.error = function (...args) {
    try {
      window.__nlsErr.push(args.map(String).join(" ").slice(0, 300));
    } catch (e) {
      /* ignore */
    }
  ...
  window.addEventListener("unhandledrejection", (e) =>
    window.__nlsErr.push(`unhandled: ${String(e.reason).slice(0, 300)}`)
  );
  return 0;
};
...
async function openAssetsPanel(d) {
  const rendered = () =>
    A.call(d, function () {
      return (
        document.querySelectorAll("[data-asset-category]").length ||
  ...
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    const made = await A.call(d, async function () {
      ...
      out.counts = {
        ...
        video: Object.keys(assets.video || {}).length,
        json: Object.keys(assets.json || {}).length,
        blueprint: Object.keys(assets.blueprint || {}).length
      };
      return out;
    });
    ...
    if (!made.counts || made.counts.audio < 1 || made.counts.video < 1) {
      throw new Error(
        `SETUP GUARD: seeding did not produce at least one audio and one video: ${JSON.stringify(made)}`
      );
    }
    await A.sleep(1200);
  });
...
async function phaseCategories() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    run.check(
      "C-1",
      "the sidebar shows exactly six category sections",
      headers.length === 6,
      JSON.stringify(headers.map((h) => h.category))
    );
    run.check(
      "C-2",
      "the six are image/media/data/font/model/other, in that order",
      JSON.stringify(headers.map((h) => h.category)) === JSON.stringify(EXPECTED_CATEGORIES),
      JSON.stringify(headers.map((h) => h.category))
    );
    run.check(
      "C-3",
      "no header still reads Audio / Videos / JSON Files / Blueprints",
    ...
    const counts = await A.call(d, function () {
      ...
      return {
        ...
        headerText: Array.from(document.querySelectorAll("[data-asset-category]")).reduce(
          (acc, el) => {
            acc[el.getAttribute("data-asset-category")] = (el.innerText || "")
              .replace(/\s+/g, " ")
              .trim();
            return acc;
          },
...
async function phaseGrouping() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    const result = await A.call(d, async function () {
      ...
      if (!audio || !video)
        return {
          guard: `need one audio and one video asset; have ${Object.keys(assets.audio || {}).length}/${Object.keys(assets.video || {}).length}`
        };
      const created = await svc.createGroup("media", "ACCEPTANCE-MIXED");
      const group = created && (created.data || created);
      if (!group || !group.id)
        return { guard: `createGroup('media') returned ${JSON.stringify(created).slice(0, 200)}` };
    ...
    run.check(
      "G-1",
      "a group can be created on the merged Media category",
      Boolean(result.groupId),
      JSON.stringify(result)
    );
    run.check(
      "G-2",
      "that group's domain is the category, not a single asset type",
      result.groupCategory === "media",
      String(result.groupCategory)
    );
    run.check("G-3", "an audio asset moves into it", result.audioOk, String(result.audioOk));
    run.check(
      "G-4",
      "a video asset moves into the SAME group",
      result.videoOk,
      String(result.videoOk)
    );
    ...
    const rows = await A.call(d, function () {
      const el = Array.from(document.querySelectorAll("*")).find(
        (e) => (e.textContent || "").trim() === "ACCEPTANCE-MIXED" && e.children.length === 0
      );
      return Boolean(el);
    });
...
async function phaseCreate() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    if (!header.found || !header.reachable)
      throw new Error(
        `SETUP GUARD: the Other category header is not reachable: ${JSON.stringify(header)}`
      );
    ...
    const menu = await A.call(d, function () {
      const items = Array.from(document.querySelectorAll('[data-context-menu="true"] > div')).map(
    ...
    run.check(
      "N-1",
      "the Other category header has a context menu carrying a new-text-file row",
      menu.some((m) => /New Text File|新建文本文件/i.test(m)),
      JSON.stringify(menu)
    );

    await A.clickNamed(d, '[data-context-menu="true"] > div', "New Text File|新建文本文件", {
      flags: "i"
    });
    await A.sleep(900);
    ...
    run.check(
      "N-2",
      "the project input dialog opens pre-filled with a localized *.txt name",
      Boolean(dialog) && /\.txt$/.test(dialog.value) && dialog.value.length > 4,
    ...
    const state = await A.call(d, function () {
      ...
      const tabs = Array.from(document.querySelectorAll("[data-editor-tab-id]")).map((e) =>
        e.getAttribute("data-editor-tab-id")
      );
      return {
    ...
    run.check(
      "N-3",
      "the asset is created under Other and keeps the extension the author typed",
      Boolean(state.assetId) && /acceptance-plan\.md$/.test(state.assetName || ""),
      JSON.stringify({ id: state.assetId, name: state.assetName, ext: state.assetExt })
    );
    run.check(
      "N-4",
      "creating it also opens an editor tab for it",
      state.tabs.some((t) => /text-editor/.test(t || "")),
      JSON.stringify(state.tabs)
    );
    run.check(
      "N-5",
      "that tab really is a mounted Monaco instance, not a textarea",
      state.monaco > 0,
      String(state.monaco)
    );
    run.check(
      "N-6",
      "opening it raised no page errors",
      state.errors.length === 0,
      JSON.stringify(state.errors.slice(0, 5))
    );

    if (state.assetId)
      fs.writeFileSync(path.join(__dirname, "..", "out", "acceptance-asset-id.txt"), state.assetId);
  });
}
...
async function phaseEdit() {
  ...
  if (!fs.existsSync(idFile))
    throw new Error("SETUP GUARD: run --phase create first (no asset id recorded)");
  ...
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    if (!rect)
      throw new Error("SETUP GUARD: no Monaco view-lines on screen — the text tab is not open");
    ...
    if (!landed)
      throw new Error(
        "SETUP GUARD: the text never reached the editor — driver problem, not a product one"
      );
  ...
  run.check(
    ...
    Boolean(bytes && bytes.length),
    bytes ? `${bytes.length} bytes` : "no file found"
  );
  ...
  run.check(
    "E-2",
    "the bytes decode as UTF-8 to exactly what was typed (EOL-insensitive)",
    onDisk.split("\r\n").join("\n") === TEXT,
    bytes ? JSON.stringify(onDisk.slice(0, 80)) : "n/a"
  );
  run.note(
    `EOL written for a new file on this platform: ${onDisk.indexOf("\r\n") >= 0 ? "CRLF" : "LF"}`
  );
  run.check(
    "E-3",
    "UTF-8 is the default: no BOM was written",
  ...
  run.check(
    "E-4",
    "the asset record hash was recomputed after the write (stale hash = silent stale reads downstream)",
    Boolean(meta && meta.hash),
    meta ? String(meta.hash).slice(0, 16) : "no record"
  );
  ...
  const policy = fs.readFileSync(
    path.join(__dirname, "..", "..", "..", "src", "shared", "vcs", "workingSet.ts"),
    "utf8"
  );
  ...
  run.check(
    "E-5",
    "nothing on the new file's path is on the VCS exclusion list",
...
async function phaseEncoding() {
  const id = fs
    .readFileSync(path.join(__dirname, "..", "out", "acceptance-asset-id.txt"), "utf8")
    .trim();
  ...
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    const token = await A.call(d, function () {
      const el = Array.from(document.querySelectorAll("*")).find(
        (e) => e.children.length === 0 && /^(UTF-8|UTF8)$/i.test((e.textContent || "").trim())
      );
      if (!el) return { found: false };
      const r = el.getBoundingClientRect();
    ...
    const items = await A.call(d, function () {
      return Array.from(document.querySelectorAll('[data-context-menu="true"] > div')).map((e) =>
        (e.textContent || "").replace(/\s+/g, " ").trim()
      );
    });
    run.check(
      "X-1",
      "the encoding token offers both reopen-with and save-with",
      items.filter((i) => /reopen|重新打开|save|保存/i.test(i)).length >= 2,
      JSON.stringify(items)
    );
  });

  run.note(
    `GBK save asserted by hand after driving the menu; pre-change bytes were ${before ? before.length : 0}`
  );
}
...
async function phaseAccordion() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    const heights = () =>
      A.call(d, function () {
        ...
        document.querySelectorAll("[data-asset-category]").forEach((h) => {
          ...
          out[h.getAttribute("data-asset-category")] = body
    ...
    if (withBody.length < 2)
      throw new Error(
        `SETUP GUARD: need two non-empty categories to test isolation, saw ${JSON.stringify(start)}`
      );

    for (const category of withBody) {
      const before = await heights();
      if (!(await toggle(category)))
        throw new Error(`SETUP GUARD: no toggle button on the ${category} header`);
      await A.sleep(700);
      ...
      run.check(
        `A-${category}`,
        `${category} responds to its own toggle`,
      ...
      run.check(
        `A-${category}-iso`,
        `toggling ${category} leaves every other section alone`,
        moved.length === 0,
        moved.length
          ? JSON.stringify(moved.map((c) => `${c}:${before[c]}->${after[c]}`))
          : "none moved"
      );
...
async function phaseFrozen() {
  await D.onWindow("workspace", A.WINDOWS.workspace, async (d) => {
    ...
    if (!before || !before.len)
      throw new Error(
        "SETUP GUARD: no text editor with content on screen — freeze phase would be vacuous"
      );

    run.check(
      "F-1",
      "frozen: the text is still visible at full opacity (freezing must not hide what it protects)",
      before.opacity === "1",
      JSON.stringify({ opacity: before.opacity, len: before.len })
    );
    ...
    run.check(
      "F-2",
      "frozen: typing does not change the buffer",
...
(async () => {
  ...
  const order =
    phase === "all" ? ["seed", "accordion", "categories", "grouping", "create", "edit"] : [phase];
