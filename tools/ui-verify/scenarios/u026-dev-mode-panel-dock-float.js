...
const READ_TOGGLE = function (dockedRe, floatingRe) {
  ...
  if (!hit) {
    return {
      ...
      found: false,
      nameless,
      names: controls.map(nameOf).filter(Boolean)
    };
  }
...
const HEADER_GRAB = function (titleText) {
  ...
  for (let f = 0.05; f <= 0.95; f += 0.05) {
    ...
    if (
      el.closest(
        'button, [role="button"], [role="switch"], select, input, [role="combobox"], [role="listbox"]'
      )
    )
      continue;
  ...
  return {
    ...
    why: "every sampled point on the header is a control"
  };
};

// --- helpers -------------------------------------------------------------------------------------

const inWindow = (b, win) =>
  b.x >= -1 && b.y >= -1 && b.x + b.w <= win.w + 1 && b.y + b.h <= win.h + 1;
const fits = (box, area) => box.w <= area.w + 1 && box.h <= area.h + 1;
const round = (o) =>
  o
    ? Object.fromEntries(
        Object.entries(o).map(([k, v]) => [k, typeof v === "number" ? Math.round(v) : v])
      )
    : o;
...
async function waitForStoryOnStage(d, tries = 44) {
  ...
  throw new Error(
    `SETUP GUARD: the boot menu is still on stage after ${Math.round(tries * 0.75)}s — the story never started, so nothing here would be measuring a running scene; stage="${last.slice(0, 160)}"`
  );
}
...
async function toggleMode(d, want) {
  ...
  if (!before.reachable)
    throw new Error(`SETUP GUARD: the mode toggle "${before.name}" has a rect but is covered`);
  await d.click(before.cx, before.cy);
  for (let i = 0; i < 20; i += 1) {
    await A.sleep(250);
    const now = await A.call(d, READ_TOGGLE, DOCKED_NAME, FLOATING_NAME);
    if (now.found && now.mode === want) {
      await A.sleep(500);
      return now;
    }
  }
...
(async () => {
  ...
  await D.onWindow("dev-mode", A.WINDOWS.devmode, async (d) => {
    ...
    if (!first.stage)
      throw new Error(
        "SETUP GUARD: no StageViewportFrame box in the DOM — the stage never laid out"
      );
    if (!(first.stage.w > 0 && first.stage.h > 0))
      throw new Error(`SETUP GUARD: stage box has no size: ${JSON.stringify(first.stage)}`);
    if (!first.panel)
      throw new Error(
        'SETUP GUARD: the runtime panel opened but has no [role="complementary"] box'
      );
    if (first.panel.w <= 0)
      throw new Error(`SETUP GUARD: the panel box is 0 wide: ${JSON.stringify(first.panel)}`);

    const toggle0 = await A.call(d, READ_TOGGLE, DOCKED_NAME, FLOATING_NAME);
    run.check(
      "D-5a",
      'the panel header carries a mode toggle with an accessible name ("Float panel" / "Dock panel")',
      Boolean(toggle0.found),
      toggle0.found
        ? `${toggle0.name} (mode=${toggle0.mode}, reachable=${toggle0.reachable})`
        : `no match; panel controls named: ${JSON.stringify(toggle0.names || [])}`
    );
    run.check(
      "D-5c",
      "no visible control inside the panel lacks an accessible name",
      Array.isArray(toggle0.nameless) && toggle0.nameless.length === 0,
      toggle0.nameless || "panel absent"
    );
    ...
    if (toggle0.found) {
      ...
      if (dockedToggle.mode !== "dock")
        throw new Error(
          `SETUP GUARD: could not reach docked mode; toggle reads "${dockedToggle.name}"`
        );
      await A.sleep(700);
    }
    ...
    run.check(
      "D-1",
      "docked: stage fit area + panel width == window width (the panel really takes space)",
    ...
    run.check(
      "D-3a",
      "docked: stage box fits its area and sits inside the window",
    ...
    run.check(
      "D-8a",
      "docked: the panel is an opaque reading surface",
      A.alphaOf(docked.panel.surfaceBg) === 1,
      `background=${docked.panel.surfaceBg}`
    );
    ...
    if (!toggle0.found) {
      for (const [id, text] of [
        ...
        [
          "D-3c",
          "the design aspect ratio is identical in both modes (a refit, not a crop or stretch)"
        ],
    ...
    run.check(
      "D-5b",
      "the toggle flips both ways (docked -> floating -> docked)",
      floatToggle.mode === "float",
    ...
    run.check(
      "D-2",
      "floating: the stage gets the whole window back, and the panel is still visible",
    ...
    run.check(
      "D-3b",
      "floating: stage box fits its area and sits inside the window",
    ...
    run.check(
      "D-3c",
      "the design aspect ratio is identical in both modes (a refit, not a crop or a stretch)",
      Math.abs(aDock - aFloat) < 0.01,
      `docked=${aDock.toFixed(4)} floating=${aFloat.toFixed(4)}`
    );
    ...
    if (widthBound) {
      run.check(
        "D-3d",
        "floating gives the stage back the width the docked panel took",
        floating.stage.w > docked.stage.w + 1,
        `docked=${Math.round(docked.stage.w)}px floating=${Math.round(floating.stage.w)}px`
      );
    } else {
      run.note(
        `D-3d skipped: the docked stage is height-bound at this window size (area ${Math.round(docked.area.w)}x${Math.round(docked.area.h)}), so reclaiming width cannot change it`
      );
    }

    run.check(
      "D-7",
      "floating: the panel does not cover the centre of the stage it is debugging",
    ...
    run.check(
      "D-8b",
      "floating: the panel is still an opaque reading surface",
      A.alphaOf(floating.panel.surfaceBg) === 1,
      `background=${floating.panel.surfaceBg}`
    );
    ...
    if (!grab || grab.cx === null) {
      ...
      run.check(
        "D-4a",
        "floating: dragging the header moves the panel by the drag delta",
        false,
        `no grab point: ${why}`
      );
      run.check(
        "D-4b",
        "floating: a drag past the edge leaves the panel fully inside the window",
        false,
        `no grab point: ${why}`
      );
    } else {
    ...
    await d.keys("Escape");
    await A.sleep(900);
    const closed = await A.call(d, function () {
      return Boolean(document.querySelector('[role="tablist"]'));
    });
    if (closed) {
    ...
    run.check(
      "D-6a",
      "float mode survives closing and reopening the panel",
      reopened.found &&
        reopened.mode === "float" &&
    ...
    const row = await A.call(d, function () {
      ...
      const el =
        rows.find((e) => Number((e.innerText || "").trim().match(/^(\d+)/)[1]) === 10) || rows[0];
      if (!el) return null;
    ...
    if (!row || !row.reachable) {
      run.check(
        "D-6b",
        "float mode survives a timeline jump (game-session remount)",
        false,
      ...
      if (landed !== jumpLine) {
        throw new Error(
          `SETUP GUARD: the timeline jump to row ${jumpLine} never took (play head reads ${landed}) — D-6b would have been green without a session remount ever happening`
        );
      }
