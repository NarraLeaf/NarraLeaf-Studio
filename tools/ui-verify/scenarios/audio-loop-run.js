...
async function readFresh(expression, label) {
  const d = await withTimeout(
    connect({ target: "dev-mode", port: PORT }),
    15000,
    `connect (${label})`
  );
  try {
...
(async () => {
  ...
  try {
    ...
    if (!String(wsPath).includes("workspace"))
      throw new Error(`ws target is not workspace: ${wsPath}`);
    ...
    for (let i = 0; i < 30; i += 1) {
      await sleep(2000);
      const seen = await readFresh('typeof window.__audioProbe !== "undefined"', "arm-poll").catch(
        () => false
      );
      if (seen === true) {
        armed = true;
    ...
    const clicker = await withTimeout(
      connect({ target: "dev-mode", port: PORT }),
      15000,
      "connect (click)"
    );
    try {
      await withTimeout(clicker.click(700, 450), 15000, "stage click");
    } finally {
      clicker.close();
    }
    await sleep(2500);
