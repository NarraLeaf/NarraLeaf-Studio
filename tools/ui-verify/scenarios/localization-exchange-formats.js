...
const OPEN_PANEL = function () {
  ...
  const collapsed = Array.from(document.querySelectorAll("button")).find(
    (b) => b.getAttribute("aria-label") === "Collapsed panels"
  );
...
const CLICK_MENU_ITEM = function (text) {
  const rows = Array.from(document.querySelectorAll('[data-context-menu="true"] *')).filter(
    (e) => e.children.length === 0 && e.textContent.includes(text)
  );
...
const OPEN_LOCALE_MENU = function (display) {
  const row = Array.from(document.querySelectorAll("[data-panel-id] .group")).find((r) =>
    r.textContent.includes(display)
  );
  if (!row) return false;
  const more = Array.from(row.querySelectorAll("button")).find(
    (b) => (b.getAttribute("data-tip") || b.getAttribute("title")) === "More"
  );
  ...
  const menu = Array.from(document.querySelectorAll("body > div"))
    .filter(
      (d) =>
        ...
        (d.className || "").includes("bg-surface-raised")
    )
    .pop();
  if (!menu) return [];
...
const CLICK_BUTTON = function (pattern) {
  const button = Array.from(document.querySelectorAll("button")).find((b) =>
    new RegExp(pattern).test(b.textContent.trim())
  );
...
const CLICK_SELECT_ROW = function (pattern) {
  ...
  const row = Array.from(menu.querySelectorAll("*")).find(
    (e) => new RegExp(pattern).test(e.textContent.trim()) && e.querySelectorAll("*").length <= 3
  );
  if (!row) return false;
  row.click();
  return true;
...
const LOCALE_ROWS = function () {
  return Array.from(document.querySelectorAll("[data-panel-id] .group")).map((r) =>
    r.textContent.trim()
  );
};
...
async function phaseMenu(d) {
  ...
  const opened = await d.evaluate(
    `(${OPEN_LOCALE_MENU.toString()})(${JSON.stringify(LOCALE.display)})`
  );
  ...
  run.check(
    "X1",
    "the locale menu offers one export and one import, format-agnostic",
    items.some((i) => /^Export translations/.test(i)) &&
      items.some((i) => /^Import translations/.test(i)),
    items
  );
  run.check(
    "X2",
    "no format is named in the menu itself",
    !items.some((i) => /CSV|XLIFF|PO|JSON/.test(i)),
    items
  );
  await d.keys("Escape");
}
...
async function phaseDialog(d) {
  ...
  const title = await d.evaluate(
    'document.body.innerText.includes("Export ' + LOCALE.display + ' translations")'
  );
  ...
  run.check(
    "X4",
    "all four formats are offered, each naming the tools that open it",
    ["CSV", "XLIFF 1.2", "gettext PO", "JSON"].every((name) =>
      formats.some((f) => f.startsWith(name))
    ) && formats.some((f) => /Poedit/.test(f)),
    formats
  );
  ...
  await A.sleep(400);
  const scopes = (await A.call(d, SELECT_OPTIONS)).filter((s) =>
    /^(Everything|Untranslated)/.test(s)
  );
  run.check(
    "X5",
    "both scopes carry their own counts",
    scopes.length >= 2 && scopes.every((s) => /\(\d+\)$/.test(s)),
    scopes
  );
...
async function phaseRoundTrip(d) {
  ...
  if (!PROJECT)
    throw new Error("NLS_VERIFY_PROJECT is required to read the written translations back");
  ...
  run.check(
    "X6",
    `the ${format} export lands on disk`,
    fs.existsSync(file),
    `${file} :: ${saved.trim()}`
  );
  ...
  const patched = {
    csv: () =>
      text.replace(
        /^([^",\r\n]+),([^",\r\n]*),([^",\r\n]*),,,\r?$/m,
        `$1,$2,$3,${marker},translated,`
      ),
    xliff: () =>
      text.replace('<target state="new"/>', `<target state="translated">${marker}</target>`),
  ...
  const stored = JSON.parse(
    fs.readFileSync(path.join(PROJECT, "editor", "localization", `${LOCALE.code}.json`), "utf8")
  );
