...
const STRUCT_RE =
  /const\s+([A-Za-z0-9_]+)\s*=\s*koffi\.struct\(\s*"([A-Za-z0-9_]+)"\s*,\s*\{([^}]*)\}\s*\)/g;
...
const ALIAS_RE =
  /const\s+([A-Za-z0-9_]+)\s*=\s*koffi\.alias\(\s*"([A-Za-z0-9_]+)"\s*,\s*"([A-Za-z0-9_ *[\]]+)"\s*\)/g;
/** `const X = koffi.proto("LoreEventCallbackFunction", "void", [...])` */
const PROTO_RE =
  /const\s+([A-Za-z0-9_]+)\s*=\s*koffi\.proto\(\s*"([A-Za-z0-9_]+)"\s*,\s*"([a-z0-9_]+)"\s*,\s*\[([^\]]*)\]\s*\)/g;
...
function normalizeType(expression, variables, context) {
  ...
  const array = /^koffi\.array\(\s*"([^"]+)"\s*,\s*koffi\.sizeof\(([A-Za-z0-9_]+)\)\s*\)$/.exec(
    text
  );
