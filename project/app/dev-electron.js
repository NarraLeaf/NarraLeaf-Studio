...
function ignoreDotSegmentsBelow(roots) {
  ...
  return (target) => {
    ...
    const base = bases.find(
      (candidate) => resolved === candidate || resolved.startsWith(candidate + path.sep)
    );
...
(async () => {
  try {
    ...
    if (error && error.code === "EADDRINUSE") {
      console.error(
        `[dev] port ${DEV_RELOAD_PORT} is already in use — another \`yarn dev\` session owns it.`
      );
      console.error(
        "[dev] dist was left untouched. Stop that session first, or run `yarn build:dev` to rebuild in place."
      );
    } else {
  ...
  function tryStartElectronOnce() {
    if (
      ...
      appStarted = true;
      console.log(
        `[dev] all initial builds completed in ${((Date.now() - startedAt) / 1000).toFixed(1)}s. starting electron...`
      );
      restartElectron();
    }
  }
  ...
  function startElectron() {
    ...
    electronProcess = spawn(
      electronBinary,
      [mainEntry, "--dev", `--dev-reload-port=${DEV_RELOAD_PORT}`, ...forwardedElectronArgs],
      {
        stdio: "inherit"
      }
    );
    ...
    electronProcess.on("exit", (code, signal) => {
      ...
      if (code === 0) {
        ...
        console.log(
          "[dev] Studio quit; shutting down the dev session and releasing the reload port."
        );
        wss.close();
        process.exit(0);
      }
      ...
      console.log(
        `[dev] electron process exited with code ${code}${signal ? ` (${signal})` : ""}; waiting for the next change.`
      );
    });
  }
   ...
   */
  const buildArtifactCompileWorker = () =>
    watchBuild(
      {
  ...
  const buildPreloadScript = async () => {
    if (!fs.existsSync(preloadEntry)) {
      console.warn(
        '[preload] Entry "src/main/preload/preload.ts" not found. Skipping preload build.'
      );
