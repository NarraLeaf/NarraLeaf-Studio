...
function printDevtools(result) {
  ...
  for (const entry of entries) {
    ...
    console.log(
      `${time}  ${pad(entry.level, 7)} ${pad(entry.windowType, 10)}  ${entry.message}${origin}`
    );
  }
}
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--json":
        ...
        break;
      case "--host":
        options.host = takeValue(i, arg);
        i += 1;
        break;
      case "--port":
        options.port = Number(takeValue(i, arg));
        i += 1;
        break;
      case "--channel":
        params.channel = takeValue(i, arg);
        i += 1;
        break;
      case "--window":
        params.window = takeValue(i, arg);
        i += 1;
        break;
      case "--level":
        params.level = takeValue(i, arg);
        i += 1;
        break;
      case "--source":
        params.source = takeValue(i, arg);
        i += 1;
        break;
      case "--since":
        params.since = Number(takeValue(i, arg));
        i += 1;
        break;
      case "--after-seq":
        params.afterSeq = Number(takeValue(i, arg));
        i += 1;
        break;
      case "--limit":
        params.limit = Number(takeValue(i, arg));
        i += 1;
        break;
      default:
        if (arg.startsWith("--")) {
