...
function listCandidateProcesses() {
  if (IS_WINDOWS) {
    ...
    const raw = runCommand("powershell.exe", [
      "-NoProfile",
      "-NonInteractive",
      "-Command",
      script
...
async function runCli(argv = process.argv.slice(2)) {
  ...
  for (const { port, pid, command } of foreignPorts) {
    ...
    if (force) {
      ...
      console.warn(
        `[stop] port ${port} is held by PID ${pid}${label}, which is not from this checkout.`
      );
  ...
  const targets = force
    ? [...own, ...foreignPorts.map((f) => ({ ...f, role: `port ${f.port} holder` }))]
    : own;
  ...
  const ordered = [...targets].sort(
    (a, b) => Number(b.role === "dev server") - Number(a.role === "dev server")
  );
