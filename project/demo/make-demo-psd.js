...
function decodePng(buf) {
    let off = 8, idat = [], width = 0, height = 0, colorType = 0, bitDepth = 8;
    ...
    if (bitDepth !== 8 || (colorType !== 2 && colorType !== 6)) {
        throw new Error(`unsupported PNG (depth ${bitDepth}, colour type ${colorType}) — needs 8-bit RGB or RGBA`);
    }
...
const layer = (width, height) => ({ width, height, data: new Uint8ClampedArray(width * height * 4) });
...
function readProjectAsset(projectDir, assetName) {
    ...
    if (!asset) {
        throw new Error(`no image asset named "${assetName}" in ${projectDir}\n  available: ${assets.map(a => a.name).join(", ")}`);
    }
    const id = asset.id.replace(/-/g, "");
    return fs.readFileSync(path.join(projectDir, "assets", "content", id.slice(0, 2), id.slice(2, 4), id.slice(4)));
}
...
const cheekL = face.x - face.w * 0.30;
const cheekR = face.x + face.w * 0.30;
...
blob(calm, cheekL - moodBox.left, cheekY - moodBox.top, cheekR2, [255, 138, 150], 0.30);
blob(calm, cheekR - moodBox.left, cheekY - moodBox.top, cheekR2, [255, 138, 150], 0.30);
...
verticalWash(warm, [255, 186, 140], 0.10, 0.26);
...
const psd = {
    ...
    height: H,
    children: [
        { name: "Body", left: 0, top: 0, right: W, bottom: H, opacity: 1, blendMode: "normal", imageData: base },
        {
            name: "Warm tint", left: 0, top: 0, right: W, bottom: H,
            opacity: 1, blendMode: "normal", clipping: true, imageData: warm,
        },
        {
            name: "Rim light", left: 0, top: 0, right: W, bottom: H,
            opacity: 1, blendMode: "color", clipping: true, imageData: rim,
        },
        {
            name: "Mood", opacity: 1, blendMode: "pass through",
            children: [
                {
                    name: "Calm", left: moodBox.left, top: moodBox.top, right: moodBox.right, bottom: moodBox.bottom,
                    opacity: 1, blendMode: "normal", imageData: calm,
                },
                {
                    name: "Angry", left: moodBox.left, top: moodBox.top, right: moodBox.right, bottom: moodBox.bottom,
                    opacity: 1, blendMode: "normal", imageData: angry,
                },
            ],
        },
        {
            name: "Shade", left: 0, top: shadeTop, right: W, bottom: H,
            opacity: 1, blendMode: "multiply", imageData: shade,
        },
        {
            name: "Grain", left: 0, top: 0, right: grain.width, bottom: grain.height,
            opacity: 1, blendMode: "dissolve", imageData: grain,
        },
        {
            name: "Scratch (WIP)", left: 40, top: Math.round(H * 0.55), right: 40 + scratch.width,
            bottom: Math.round(H * 0.55) + scratch.height,
            opacity: 1, blendMode: "normal", hidden: true, imageData: scratch,
        },
    ],
};
