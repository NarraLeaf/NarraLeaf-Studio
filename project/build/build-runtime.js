...
function runtimeAliasPlugin() {
  ...
  const exactAliases = new Map([
    ["@/lib/i18n", shim("i18n.ts")],
    [
      "@/lib/ui-editor/hooks/useEditorAppearanceInspectorVariant",
      shim("useEditorAppearanceInspectorVariant.ts")
    ],
    ["@/lib/workspace/hooks/useAssetObjectUrl", shim("useAssetObjectUrl.ts")],
    ["@/lib/workspace/hooks/useEditorFontFamily", shim("useEditorFontFamily.ts")],
    ["@/lib/workspace/hooks/useSurfacePuppetSession", shim("useSurfacePuppetSession.ts")],
    ["@/apps/workspace/modules/properties/framework/utils/colorUtils", shim("colorUtils.ts")],
    ["@/lib/workspace/services/ui-editor/UIEditorStateService", shim("UIEditorStateService.ts")],
    ["@/lib/workspace/services/ui-editor/UIDocumentService", shim("UIDocumentService.ts")],
    ["@/lib/workspace/services/ui/UIStore", shim("UIStore.ts")],
    ["@/lib/ui-editor/interaction/inlineTextEdit", shim("inlineTextEdit.ts")],
    ["@/lib/ui-editor/interaction/containerDrillSelection", shim("containerDrillSelection.ts")],
    [
      "@/lib/ui-editor/interaction/surfaceInlineTextEditActivation",
      shim("surfaceInlineTextEditActivation.ts")
    ],
    ["@/lib/ui-editor/interaction/doubleClickDebug", shim("doubleClickDebug.ts")]
  ]);
  ...
  return {
    ...
    setup(build) {
      build.onResolve({ filter: /^@\/(?:apps|lib)\/.*$/ }, (args) => {
        ...
        if (
          allowedPrefixes.some((prefix) => args.path.startsWith(prefix)) ||
          allowedExact.has(args.path)
        ) {
        ...
        return {
          errors: [
            {
              text:
                `Runtime bundle must not import "${args.path}" (imported by ${args.importer}). ` +
