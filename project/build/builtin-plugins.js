...
function readManifest(pluginDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error(
      `Built-in plugin is missing manifest.json: ${path.relative(rootDir, pluginDir)}`
    );
  }
...
function resolveManifestEntries(pluginDir, manifest) {
  ...
  if (!entries || typeof entries !== "object" || Array.isArray(entries)) {
    throw new Error(
      `Built-in plugin manifest must declare "entries" (manifestVersion 2): ${path.relative(rootDir, pluginDir)}`
    );
  }
  const declared = ["studio", "runtime"]
    .map((target) => ({
      target,
      entry: typeof entries[target] === "string" ? entries[target].trim() : ""
    }))
    .filter((item) => item.entry);
  if (declared.length === 0) {
    throw new Error(
      `Built-in plugin manifest declares no entries: ${path.relative(rootDir, pluginDir)}`
    );
  }
  return declared;
}
...
function resolveSourceEntry(pluginDir, manifest, entry) {
  ...
  if (!sourceEntry) {
    throw new Error(
      `Built-in plugin source entry not found for ${manifest.id || pluginDir}: ${entry}`
    );
  }
  return sourceEntry;
}
...
async function buildBuiltInPlugin(pluginDir, options = {}) {
  ...
  if (typeof manifest.icon === "string" && manifest.icon.trim()) {
    ...
    if (!fs.existsSync(source)) {
      throw new Error(
        `Built-in plugin ${manifest.id || pluginName} declares a missing icon: ${manifest.icon}`
      );
    }
