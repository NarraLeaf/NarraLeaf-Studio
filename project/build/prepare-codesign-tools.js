...
(async () => {
    if (process.env.NLS_SKIP_CODESIGN_TOOLS === '1') {
        console.log('[codesign-tools] NLS_SKIP_CODESIGN_TOOLS=1, skipping; iOS signing will report as unavailable');
        return;
    }
    ...
    try {
        ...
        fs.rmSync(stagingDir, { recursive: true, force: true });
        console.error(`[codesign-tools] Failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
})();
