...
由绑定的 `nl.image` Element Literal / Element Flush 派生时，节点类型为 `blueprint.element.image.getImageAsset`，并带有 `element:nl.image` 输入。
...
由绑定的 `nl.image` Element Literal / Element Flush 派生时，节点类型为 `blueprint.element.image.setImageAsset`，并带有 `element:nl.image` 输入。
