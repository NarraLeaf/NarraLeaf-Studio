...
List Element 节点使用 `blueprint.element.list.*`，带 `list: element:nl.list` 输入，创建浮窗中归入 `Element` 分类。它们只有在当前图中已有绑定到 `nl.list` 的 Element Literal 或 Element Flush 时才会显示；同一节点类型只显示一项。若兼容来源唯一，放置时会自动连接 `list` 输入；若有多个兼容来源，则保留 `list` 输入由作者手动选择/连接。条目上下文读取节点不需要目标 List，但只在 List item template 后代元素的蓝图中出现。
