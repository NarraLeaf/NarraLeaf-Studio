...
Text Element 节点使用同一属性目录下的 `blueprint.element.text.*`，带顶部 `element:nl.text` 输入，创建浮窗中归入 `Element` 分类。它们只有在当前图中已有绑定到 `nl.text` 的 Element Literal 或 Element Flush 时才会显示；同一节点类型只显示一项。若兼容来源唯一，放置时会自动连接 `element` 输入；若有多个兼容来源，则保留 `element` 输入由作者手动选择/连接。读取节点是 pure，可用于 Blueprint Value；写入节点只用于 event/macro。
