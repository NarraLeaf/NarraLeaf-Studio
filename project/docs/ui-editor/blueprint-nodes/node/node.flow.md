...
一次执行在两次真正的等待之间最多连续运行 512 个节点。经过真正等待的节点（时长大于 0 的 `Delay`、动画、读写文件等）的循环不受此限，会一直运行到所在界面关闭；时长为 0 的 `Delay` 不等待，不重置计数。超过上限的执行会被中止，Dev Mode 的问题面板与发布版游戏的日志会写明它从哪个事件头开始、在哪个节点处中止。
