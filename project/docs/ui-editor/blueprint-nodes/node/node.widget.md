...
Element 版使用 `blueprint.element.<widget>.*`，带 typed Element 输入，创建浮窗中归入 `Element` 分类。它们只有在当前图中已有兼容的 Element Literal 或 Element Flush 时才会显示；同一节点类型只显示一项。若兼容来源唯一，放置时会自动连接目标输入；若有多个兼容来源，则保留目标输入由作者手动选择/连接。
...
- `blueprint.frameWidget.setTargetPage` / `blueprint.element.frame.setTargetPage` - `Set Frame Page`，设置目标 Page surface id；卡片内的 `Page` 参数使用 Page 下拉选择，空选项清除目标。Self 形态只在 `nl.frame` 私有蓝图中出现；Element 形态可从绑定到 `nl.frame` 的 Element Literal / Element Flush 派生，调用目标就是该 Frame 引用。切换后由 Frame 控件走现有进场/出场动画。
