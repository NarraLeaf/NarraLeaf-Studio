...
- **Global 蓝图**收到词表里的**全部**操作，不需要在任何地方开启：按键在游戏收得到按键的任何时候（与 Global 的 `On Key Down` 同一道门——文本框持有焦点时不触发，事件已被停止传播时不触发），不论持有键盘的是页面还是模态层；指针手势在它落到的第一条 lane 上，每次物理输入只触发一次，落在可操作控件上时与 Surface 同样让位。
