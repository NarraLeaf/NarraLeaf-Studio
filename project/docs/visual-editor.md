...
推荐结构：`surfaces[]` + `nodesById{}` + `roots{surfaceId -> rootNodeId}`。
...
export type UIDocument = {
  ...
  name: string;
  // Surfaces are first-class. Each surface has a root node.
  surfaces: UISurface[];
  // Normalized node store (across all surfaces)
...
- `surface.rootNodeId` 必须存在于 `nodes`
- 每个 `UINode` 的 `childrenIds` 中的 id 必须存在，且其 `parentId` 必须等于当前 node id
- 跨 surface 的 node 可以共用同一 `nodes` 表，但必须可以从某个 surface root 追溯到（否则视为悬挂节点，需要清理或隔离到回收站）
...
目标：将 `UIDocument` 的 `UISurface.rootNodeId` 渲染为 **React DOM 组件树**，并且这棵树尽可能与 NarraLeaf React 运行时一致；同时对接 NarraLeaf React 的 **Player Stage / Page Router** 概念，以保证“编辑/生产同构”。
...
- 显示当前 surface 的 node 树（root -> children）
- 单选/多选（Shift/Ctrl）
...
- 【待实现】定义 `UIDocument` 最小 schema（surface + node）
