...
> ✅ 已实现：图结构由 `[src/shared/types/ui-editor/graph.ts](src/shared/types/ui-editor/graph.ts)` 定义，`UIGraphService` 负责读写 `editor/ui/uigraphs.json`、管理 dirty/auto-save，并通过 `behavior-graph` 目录提供的 `BehaviorNodeRegistry` 与 `executeGraph` 执行器运行内置节点（`sequence`, `if`, `delay`, `effect.run`）并调用 `UIHostAdapter.effects.runEffect` 落地副作用。该实现为后续蓝图可视化编辑器提供运行时基础。
...
> ✅ `ProjectNameConvention.EditorUIGraphs` 已经指向 `editor/ui/uigraphs.json`，`UIGraphService` (位于 `src/renderer/lib/workspace/services/ui-editor/UIGraphService.ts`) 负责创建目录/文件、管理 dirty/auto-save。图的结构定义放在 `@shared/types/ui-editor/graph.ts`，与 UI 元素上的行为绑定直接对齐。
...
> ✅ 目前的 `PropertyEditor`（`src/renderer/apps/workspace/modules/properties/framework/PropertyEditor.tsx`）已经支持通过 `tabs` 配置将字段拆成多个标签页，例如“外观 / 行为”，以后可据此扩展更复杂的 Inspector 结构。
...
#### 4.6.5 预览合成与渲染管线（必须修正）

现状问题：编辑器预览未正确合成，不显示元素。必须明确渲染管线与数据流，否则后续交互与属性都无法闭环。

最小可落地的渲染管线（【待实现】）：

1. `UIDocumentService` 提供 `getCurrentSurface()` 与 `getElementTree(surfaceId)`（或等效接口）。
2. `UIRuntimeBridge` 将 surface root 渲染为 React DOM 树：
   - 通过 `ElementTypeRegistry.render(...)` 渲染实际元素视图
   - 每个节点外包 `EditorNodeWrapper`
3. `EditorNodeWrapper` 负责：
   - 输出 `data-ui-element-id`
   - 统一命中与 measurement
   - 屏蔽运行时交互（例如 click 事件不触发行为执行）
4. 画布容器有固定可预测的层级：
   - `Viewport`（负责缩放/平移）
   - `SurfaceRoot`（负责设计分辨率与背景）
   - `NodeTree`（真实 DOM）
   - `InteractionOverlay`（选中框/对齐线/命中辅助）

**必须保证：**

- 没有元素时仍渲染空 surface（可显示空态提示）
- 渲染顺序可控（以 `childrenIds` 顺序为准）
- 元素缺失 registry 时用 `UnknownElement` 兜底
- 画布与 overlay 的坐标系一致（同一 viewport transform）

#### 4.6.6 创建模式与右键菜单（必须落地）

现状问题：创建逻辑不明确，依赖模式选择器内的下拉框。需要改为“创建模式 + 右键菜单选择类型”的原型工具式交互。

交互规范（【待实现】）：

- 工具栏仅负责模式切换（Select / Pan / Insert）。
- 当 `Insert` 模式激活时：
  - 右键弹出“元素类型菜单”（从 `ElementTypeRegistry` 动态生成）
  - 菜单项支持搜索与分组（可选）
  - 选择类型后，在鼠标位置创建元素（或拖拽创建区域）
- 退出 Insert 的方式：
  - 创建完成后自动切回 Select（建议）
  - 或按 Esc 退出 Insert

> 模式选择器内的下拉框只作为“最后一次选择类型”的展示（可选），不作为主要入口。

#### 4.6.7 选中样式与 Outline 规则（必须一致）

现状问题：元素 Outline 选中样式不正确。必须统一在交互叠层绘制，不允许由元素自身样式产生不可控描边。

规范（【待实现】）：

- 选中边框、hover 高亮、对齐线全部在 `InteractionOverlay` 绘制
- `EditorNodeWrapper` 只负责提供 DOM 位置与命中信息
- 多选时只对 primary element 显示实线边框，其余使用虚线
- 选中与 hover 必须能清晰区分（颜色/线型）

#### 4.6.8 Surface 管理器的删除能力（必须可用）

现状问题：UI 的 Surface 管理器无法删除元素。这里需要明确“删除”是 Surface 级还是 Element 级，以及入口与行为。

约定（【待实现】）：

- Surface 管理器负责“Surface 的 CRUD”，并且必须支持删除 Surface
- 删除 Surface 时：
  - 若 surface 下无元素，直接删除
  - 若有元素，弹出确认（说明将删除该 surface 的所有元素）
  - 删除后自动切到同类列表的下一个 surface（若存在）
- Element 的删除必须在 Layers/Canvas 层级操作（不是 Surface 管理器的职责）

#### 4.6.9 可用性验收清单（必须满足）

用于衡量“编辑器完全可用”的最低标准（【待实现】）：

- 能创建/重命名/复制/删除 Surface
- 打开 Surface 后能稳定渲染元素树（空态也可见）
- 画布点击与 Layers 点击能互相同步 selection
- Insert 模式右键菜单可创建元素并立即可见
- 选中样式正确（outline 与 hover 不冲突）
- Delete/Backspace 可删除选中元素（走 command）
- 保存后重新进入仍能还原 UI 文档
