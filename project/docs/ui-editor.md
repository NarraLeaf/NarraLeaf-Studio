...
- On-Stage 只在游戏舞台可见时渲染（`NlrStageLayer` 的 `renderOnStage={gameStageVisible}`），避免 boot preload 阶段或游戏外 Page 过渡时露出 on-stage 元素。它的 surface shell 用 inline `pointer-events: none` 保证点击穿透——NLR RootLayout 用通配符 CSS 规则 `.pointer-events-auto-rest *` 把所有后代设成 `pointer-events: auto`，inline 样式优先于该规则（`GameSurfaceRenderer` 的 `surfacePointerEvents` prop 负责这点）；空白区域点击穿透到舞台，元素自身经 node wrapper 恢复 pointer events。全屏交互容器仍会挡住舞台点击。
