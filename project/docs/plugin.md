...
```json
{
  ...
  "contributes": {
    ...
    "widgets": ["publisher.plugin-name.widget-type"]
  },
  "permissions": []
}
```
...
独立游戏 runtime 的 `index.html` 映射 runtime API 与 React host externals（无 `react-dom/client`）：
...
React shim 从任一插件宿主全局（workspace 的 `__NLS_PLUGIN_MODULE__` 或游戏环境的 `__NLS_RUNTIME_PLUGIN_MODULE__`）解析 externals，同一份 shim 源码（`src/shared/utils/pluginRuntimeApiModule.ts`）服务所有窗口——Dev Mode 窗口的插件 widget 渲染器因此也能解析宿主 React。
