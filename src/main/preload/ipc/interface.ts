...
import {
  WindowAppType,
  WindowControlAbility,
  WindowProps,
  WindowCloseResults,
  WorkspaceViewRequest
} from "@shared/types/window";
import type {
  DevModeBlueprintDebugEventPayload,
  DevModeEntry,
  DevModeStatus,
  DevModeBundle,
  DevModeConsoleLogPayload,
  DevModeStoryRowHighlight,
  DevModeStoryRowOpenPayload,
  DevModeStoryRowOpenRequest,
  DevModeStoryRowPayload
} from "@shared/types/devMode";
import type { GameRuntimeLaunchEntry, PreviewStatus } from "@shared/types/gameRuntime";
import type {
  GameTestEventPayload,
  ...
  return {
    fs: {
      ...
      writeFileNoFollow: (
        ...
        invoke(IPCEventType.privilegedFsCall, {
          actor,
          operation: "writeFileNoFollow",
          path,
          data,
          encoding
        }),
      recoverCorruptedJsonFile: (
        actor: PrivilegedActor,
        path: string,
        replacement: string,
        encoding: BufferEncoding = "utf-8"
        ...
        invoke(IPCEventType.privilegedFsCall, {
          actor,
          operation: "recoverCorruptedJsonFile",
          path,
          replacement,
          encoding
        }),
...
export const IPCInterface: Window[typeof RendererInterfaceKey] = {
  ...
  getWindowProps: <T extends WindowAppType>(): Promise<RequestStatus<WindowProps[T]>> =>
    ipcClient.invoke(IPCEventType.appWindowProps, {}) as Promise<RequestStatus<WindowProps[T]>>,
  terminate: async (err?: string) =>
    ipcClient.send(IPCEventType.appTerminate, { err: err ?? null }),
  reportError: (report: RendererErrorReport) =>
    ipcClient.send(IPCEventType.appReportRendererError, report),
  window: {
    ready: () => ipcClient.send(IPCEventType.appWindowReady, {}),
    close: () => ipcClient.send(IPCEventType.appWindowClose, {}),
    closeWith: <T extends WindowAppType = WindowAppType>(result: WindowCloseResults[T]) =>
      ipcClient.send(IPCEventType.appWindowCloseWith, { result }),
    editCommand: (command: EditMenuRole) =>
      ipcClient.send(IPCEventType.appWindowEditCommand, { command }),
    control: {
      ...
      ability: () =>
        ipcClient.invoke(IPCEventType.appWindowControlAbility, {}) as Promise<
          RequestStatus<WindowControlAbility>
        >,
  ...
  fs: {
    ...
    requestRead: (path: string, encoding: FsTextEncoding) =>
      ipcClient.invoke(IPCEventType.fsRequestRead, { path, encoding, raw: false }),
    requestReadRaw: (path: string) =>
      ipcClient.invoke(IPCEventType.fsRequestRead, { path, raw: true }),
    requestReadDir: (path: string) => ipcClient.invoke(IPCEventType.fsRequestReadDir, { path }),
    requestWrite: (path: string, encoding: FsTextEncoding) =>
      ipcClient.invoke(IPCEventType.fsRequestWrite, { path, encoding, raw: false }),
    requestWriteRaw: (path: string) =>
      ipcClient.invoke(IPCEventType.fsRequestWrite, { path, raw: true }),
    ensureRegularFile: (path: string, data: string, encoding: BufferEncoding = "utf-8") =>
      ipcClient.invoke(IPCEventType.fsEnsureRegularFile, { path, data, encoding }),
    writeFileNoFollow: (path: string, data: string, encoding: BufferEncoding = "utf-8") =>
      ipcClient.invoke(IPCEventType.fsWriteFileNoFollow, { path, data, encoding }),
    recoverCorruptedJsonFile: (
      path: string,
      replacement: string,
      encoding: BufferEncoding = "utf-8"
    ) => ipcClient.invoke(IPCEventType.fsRecoverCorruptedJsonFile, { path, replacement, encoding }),
    ...
    rename: (oldPath: string, newName: string, isDir: boolean) =>
      ipcClient.invoke(IPCEventType.fsRename, { oldPath, newName, isDir }),
    copyFile: (src: string, dest: string) =>
      ipcClient.invoke(IPCEventType.fsCopyFile, { src, dest }),
    copyDir: (src: string, dest: string) => ipcClient.invoke(IPCEventType.fsCopyDir, { src, dest }),
    moveFile: (src: string, dest: string) =>
      ipcClient.invoke(IPCEventType.fsMoveFile, { src, dest }),
    ...
    selectFile: (filters: string[], multiple: boolean) =>
      ipcClient.invoke(IPCEventType.fsSelectFile, { filters, multiple }),
    selectDirectory: (multiple: boolean) =>
      ipcClient.invoke(IPCEventType.fsSelectDirectory, { multiple }),
  ...
  mediaConvert: {
    start: (request: MediaConvertRequest) =>
      ipcClient.invoke(IPCEventType.mediaConvertStart, { request }) as Promise<
        RequestStatus<{ state: MediaConvertStateSnapshot }>
      >,
    cancel: (jobId: string) =>
      ipcClient.invoke(IPCEventType.mediaConvertCancel, { jobId }) as Promise<
        RequestStatus<{ state: MediaConvertStateSnapshot }>
      >,
    getStatus: (jobId: string) =>
      ipcClient.invoke(IPCEventType.mediaConvertGetStatus, { jobId }) as Promise<
        RequestStatus<{ state: MediaConvertStateSnapshot }>
      >
  },
  workspace: {
    getDefaultProjectDirectory: () =>
      ipcClient.invoke(IPCEventType.projectWizardGetDefaultDirectory, {}),
    ...
    openProjectFolder: () => ipcClient.invoke(IPCEventType.workspaceOpenProjectFolder, {}),
    ...
    onResolveImageAssetUrl: (
      handler: (payload: { assetId: string }) => Promise<RequestStatus<{ url: string }>>
    ) => ipcClient.onRequest(IPCEventType.workspaceResolveImageAssetUrl, handler),
    onBlueprintNavigateFromPreview: (
      handler: (payload: PreviewStudioBlueprintOpenPayload) => void
  ...
  app: {
    launchSettings: (props: WindowProps[WindowAppType.Settings]) =>
      ipcClient.invoke(IPCEventType.appLaunchSettings, { props }),
    ...
    requestWorkspaceView: (view: WorkspaceViewRequest) =>
      ipcClient.invoke(IPCEventType.appRequestWorkspaceView, { view }),
    ...
    readBackgroundImage: (file: string) =>
      ipcClient.invoke(IPCEventType.appReadBackgroundImage, { file }),
    launchProjectWizard: () =>
      ipcClient.invoke(IPCEventType.projectWizardLaunch, {}) as Promise<
        RequestStatus<{ created: boolean; projectPath: string } | null>
      >,
    promptServerTrust: (props: ServerTrustPromptProps) =>
      ipcClient.invoke(IPCEventType.serverTrustPrompt, { props }),
    state: {
      getGlobalState: <K extends GlobalStateKeys>(key: K) =>
        ipcClient.invoke(IPCEventType.appGlobalStateGet, { key }) as Promise<
          RequestStatus<{ value: GlobalStateValue<K> }>
        >,
      setGlobalState: <K extends GlobalStateKeys>(key: K, value: GlobalStateValue<K>) =>
        ipcClient.invoke(IPCEventType.appGlobalStateSet, { key, value }) as Promise<
          RequestStatus<void>
        >,
      getAllGlobalState: () =>
        ipcClient.invoke(IPCEventType.appGlobalStateGetAll, {}) as Promise<
          RequestStatus<{ settings: Record<string, any> }>
        >,
      deleteGlobalState: (keys: string[]) =>
        ipcClient.invoke(IPCEventType.appGlobalStateDelete, { keys }) as Promise<
          RequestStatus<{ deleted: string[]; refused: string[] }>
        >,
    ...
    addRecentProject: (name: string, path: string) =>
      ipcClient.invoke(IPCEventType.appAddRecentProject, { name, path }) as Promise<
        RequestStatus<void>
      >,
    removeRecentProject: (path: string) =>
      ipcClient.invoke(IPCEventType.appRemoveRecentProject, { path }) as Promise<
        RequestStatus<void>
      >,
    revealRecentProject: (path: string) =>
      ipcClient.invoke(IPCEventType.appRevealRecentProject, { path }) as Promise<
        RequestStatus<void>
      >,
    checkRecentProjects: () =>
      ipcClient.invoke(IPCEventType.appCheckRecentProjects, {}) as Promise<
        RequestStatus<{ missing: MissingRecentProject[] }>
      >,
    getSystemPath: (name: "desktop" | "home") =>
      ipcClient.invoke(IPCEventType.appSystemPath, { name }) as Promise<
    ...
    getCacheInventory: () => ipcClient.invoke(IPCEventType.appCacheInventory, {}),
    clearCaches: (ids: string[]) => ipcClient.invoke(IPCEventType.appCacheClear, { ids }),
    ...
    importSettings: () => ipcClient.invoke(IPCEventType.appImportSettings, {}),
    update: {
  ...
  devMode: {
    launch: (projectPath: string, entry: DevModeEntry) =>
      ipcClient.invoke(IPCEventType.devModeLaunch, { projectPath, entry }) as Promise<
        RequestStatus<{ status: DevModeStatus }>
      >,
    stop: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.devModeStop, { projectPath }) as Promise<
        RequestStatus<{ status: DevModeStatus }>
      >,
    reload: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.devModeReload, { projectPath }) as Promise<
        RequestStatus<{ status: DevModeStatus }>
      >,
    getStatus: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.devModeGetStatus, { projectPath }) as Promise<
        RequestStatus<{ status: DevModeStatus }>
      >,
    getFullscreen: () =>
      ipcClient.invoke(IPCEventType.devModeFullscreenGet, {}) as Promise<
        RequestStatus<{ isFullscreen: boolean }>
      >,
    setFullscreen: (fullscreen: boolean) =>
      ipcClient.invoke(IPCEventType.devModeFullscreenSet, { fullscreen }) as Promise<
        RequestStatus<void>
      >,
    onFullscreenChanged: (handler: (payload: { isFullscreen: boolean }) => void) =>
    ...
    openStoryRowInWorkspace: (payload: DevModeStoryRowOpenPayload) =>
      ipcClient.invoke(IPCEventType.devModeOpenStoryRowInWorkspace, payload) as Promise<
        RequestStatus<void>
      >,
    ...
    resolveAssetUrl: (assetId: string, assetType?: string) =>
      ipcClient.invoke(IPCEventType.devModeResolveAssetUrl, { assetId, assetType }) as Promise<
        RequestStatus<{ url: string }>
      >,
    resolveImageAssetUrl: (assetId: string) =>
      ipcClient.invoke(IPCEventType.devModeResolveImageAssetUrl, { assetId }) as Promise<
        RequestStatus<{ url: string }>
      >,
    openBlueprintInWorkspace: (
      payload: PreviewStudioBlueprintOpenPayload & { projectPath: string }
    ) =>
      ipcClient.invoke(IPCEventType.devModeOpenBlueprintInWorkspace, payload) as Promise<
        RequestStatus<void>
      >,
    save: {
      write: (
      ...
      read: (projectRef: DevModeSaveProjectRef, id: string) =>
        ipcClient.invoke(IPCEventType.devModeSaveRead, { projectRef, id }) as Promise<
          RequestStatus<{ record: DevModeSaveRecord | null }>
        >,
      listIds: (projectRef: DevModeSaveProjectRef) =>
        ipcClient.invoke(IPCEventType.devModeSaveListIds, { projectRef }) as Promise<
          RequestStatus<{ ids: string[] }>
        >,
      readPreview: (projectRef: DevModeSaveProjectRef, id: string) =>
        ipcClient.invoke(IPCEventType.devModeSaveReadPreview, { projectRef, id }) as Promise<
          RequestStatus<{ capture: string | null }>
        >,
      delete: (projectRef: DevModeSaveProjectRef, id: string) =>
        ipcClient.invoke(IPCEventType.devModeSaveDelete, { projectRef, id }) as Promise<
          RequestStatus<{ deleted: boolean }>
        >
    }
  },
  ...
  preview: {
    launch: (projectPath: string, entry: GameRuntimeLaunchEntry) =>
      ipcClient.invoke(IPCEventType.previewLaunch, { projectPath, entry }) as Promise<
        RequestStatus<{ status: PreviewStatus }>
      >,
    stop: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.previewStop, { projectPath }) as Promise<
        RequestStatus<{ status: PreviewStatus }>
      >,
    getStatus: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.previewGetStatus, { projectPath }) as Promise<
        RequestStatus<{ status: PreviewStatus }>
      >
  },
  ...
  gameTest: {
    launch: (request: GameTestLaunchRequest) =>
      ipcClient.invoke(IPCEventType.gameTestLaunch, request) as Promise<
        RequestStatus<GameTestLaunchResult>
      >,
    stop: (projectPath: string, sessionId: string) =>
      ipcClient.invoke(IPCEventType.gameTestStop, { projectPath, sessionId }) as Promise<
        RequestStatus<void>
      >,
  ...
  vcs: {
    ...
    getAvailability: () =>
      ipcClient.invoke(IPCEventType.vcsGetAvailability, {}) as Promise<
        RequestStatus<VcsAvailability>
      >,
    isRepository: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsIsRepository, { projectPath }) as Promise<
        RequestStatus<{ isRepository: boolean }>
      >,
    getInfo: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsGetInfo, { projectPath }) as Promise<
        RequestStatus<VcsRepositoryInfo>
      >,
    ...
    initRepository: (projectPath: string, options?: VcsInitOptions) =>
      ipcClient.invoke(IPCEventType.vcsInitRepository, { projectPath, options }) as Promise<
        RequestStatus<VcsRepositoryInfo>
      >,
    /**
     ...
     */
    getStatus: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsGetStatus, { projectPath }) as Promise<
        RequestStatus<VcsStatus>
      >,
    /**
    ...
    commit: (projectPath: string, options?: VcsCommitOptions) =>
      ipcClient.invoke(IPCEventType.vcsCommit, { projectPath, options }) as Promise<
        RequestStatus<VcsCommitResult>
      >,
    ...
    checkpoint: (projectPath: string, reason: VcsCheckpointReason) =>
      ipcClient.invoke(IPCEventType.vcsCheckpoint, { projectPath, reason }) as Promise<
        RequestStatus<{ revision: VcsCommitResult | null }>
      >,
    /**
    ...
    restoreRevision: (projectPath: string, revision: RevisionId, options?: VcsRestoreOptions) =>
      ipcClient.invoke(IPCEventType.vcsRestoreRevision, {
        projectPath,
        revision,
        options
      }) as Promise<RequestStatus<VcsRestoreResult>>,
    /**
    ...
    getHistory: (projectPath: string, limit?: number, includeDetails?: boolean) =>
      ipcClient.invoke(IPCEventType.vcsGetHistory, {
        projectPath,
        limit,
        includeDetails
      }) as Promise<RequestStatus<{ entries: VcsHistoryEntry[] }>>,
    readBlob: (projectPath: string, revision: RevisionId, path: string) =>
      ipcClient.invoke(IPCEventType.vcsReadBlob, { projectPath, revision, path }) as Promise<
        RequestStatus<{ contentBase64: string }>
      >,
    /** The same file on disk now. `refusal` = it is real and too large to hand over. */
    readWorkingFile: (projectPath: string, path: string) =>
      ipcClient.invoke(IPCEventType.vcsReadWorkingFile, { projectPath, path }) as Promise<
        RequestStatus<VcsWorkingFileRead>
      >,
    ...
    readRevisionDocuments: (projectPath: string, revision: RevisionId, paths?: string[]) =>
      ipcClient.invoke(IPCEventType.vcsReadRevisionDocuments, {
        projectPath,
        revision,
        paths
      }) as Promise<RequestStatus<{ documents: { path: string; contentBase64: string | null }[] }>>,
    getChangedPaths: (projectPath: string, from: RevisionId, to: RevisionId) =>
      ipcClient.invoke(IPCEventType.vcsGetChangedPaths, { projectPath, from, to }) as Promise<
        RequestStatus<{ paths: string[] }>
      >,
    ...
    diffRevisions: (projectPath: string, from: RevisionId, to: RevisionId) =>
      ipcClient.invoke(IPCEventType.vcsDiffRevisions, { projectPath, from, to }) as Promise<
        RequestStatus<VcsRevisionDiffResult>
      >,
    /** Changes since the last version. Never cached, and never on a timer - it scans (docs §4.17). */
    diffWorkingTree: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsDiffWorkingTree, { projectPath }) as Promise<
        RequestStatus<VcsWorkingTreeDiffResult>
      >,
    getThreeWay: (projectPath: string, mine: RevisionId, theirs: RevisionId, path: string) =>
      ipcClient.invoke(IPCEventType.vcsGetThreeWay, { projectPath, mine, theirs, path }) as Promise<
        RequestStatus<VcsThreeWayResult>
      >,
    getMergeBase: (projectPath: string, a: RevisionId, b: RevisionId) =>
      ipcClient.invoke(IPCEventType.vcsGetMergeBase, { projectPath, a, b }) as Promise<
        RequestStatus<{ base?: RevisionId }>
      >,
    /** Is a merge open here? Repository state, so ask on project open, not only after a sync. */
    getMergeState: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsGetMergeState, { projectPath }) as Promise<
        RequestStatus<VcsMergeState>
      >,
    /**
    ...
    getMergeDocument: (projectPath: string, path: string) =>
      ipcClient.invoke(IPCEventType.vcsGetMergeDocument, { projectPath, path }) as Promise<
        RequestStatus<VcsMergeDocument>
      >,
    ...
    resolveConflicts: (projectPath: string, paths: string[], choice: VcsConflictChoice) =>
      ipcClient.invoke(IPCEventType.vcsResolveConflicts, { projectPath, paths, choice }) as Promise<
        RequestStatus<VcsMergeResolveResult>
      >,
    /** Takes one side per path AND commits, as one act. Writes files: re-read afterwards. */
    completeMerge: (
      projectPath: string,
      decisions: VcsMergeDecision[],
      options?: VcsCommitOptions
    ) =>
      ipcClient.invoke(IPCEventType.vcsCompleteMerge, {
        projectPath,
        decisions,
        options
      }) as Promise<RequestStatus<VcsMergeCompletion>>,
    unresolveConflicts: (projectPath: string, paths: string[]) =>
      ipcClient.invoke(IPCEventType.vcsUnresolveConflicts, { projectPath, paths }) as Promise<
        RequestStatus<VcsMergeResolveResult>
      >,
    /** Merges these paths again, discarding the working-tree bytes for them. */
    restartConflicts: (projectPath: string, paths: string[]) =>
      ipcClient.invoke(IPCEventType.vcsRestartConflicts, { projectPath, paths }) as Promise<
        RequestStatus<VcsMergeState>
      >,
    /** Full rollback to before the merge (measured). Re-read every document afterwards. */
    abortMerge: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsAbortMerge, { projectPath }) as Promise<
        RequestStatus<VcsMergeState>
      >,
    /** Local read - no socket. Null means the project has no server. */
    getRemote: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsGetRemote, { projectPath }) as Promise<
        RequestStatus<{ url: string | null }>
      >,
    /** Local write - does NOT contact the server. `null` disconnects. */
    setRemote: (projectPath: string, url: string | null) =>
      ipcClient.invoke(IPCEventType.vcsSetRemote, { projectPath, url }) as Promise<
        RequestStatus<{ url: string | null }>
      >,
    /** Goes to the network; ~2s when nothing answers. On demand only, never on a timer. */
    getSyncState: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsGetSyncState, { projectPath }) as Promise<
        RequestStatus<VcsSyncState>
      >,
    /** Local read - no socket. Null means nobody has signed in to this project's server. */
    getServerSession: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsGetServerSession, { projectPath }) as Promise<
        RequestStatus<{ session: VcsServerSession | null }>
      >,
    ...
    signIn: (projectPath: string, authUrl: string, token: string) =>
      ipcClient.invoke(IPCEventType.vcsSignIn, { projectPath, authUrl, token }) as Promise<
        RequestStatus<VcsSignInOutcome>
      >,
    ...
    trustAuthority: (projectPath: string, certificatePath: string) =>
      ipcClient.invoke(IPCEventType.vcsTrustAuthority, { projectPath, certificatePath }) as Promise<
        RequestStatus<{ installed: boolean; output: string }>
      >,
    signOut: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsSignOut, { projectPath }) as Promise<
        RequestStatus<{ session: null }>
      >,
    /** Goes to the network. Reads one address and says what is behind it. */
    probeServer: (address: string) =>
      ipcClient.invoke(IPCEventType.vcsProbeServer, { address }) as Promise<
        RequestStatus<VcsServerProbe>
      >,
    /** Local read, and no project: servers belong to the machine. */
    listServers: () =>
      ipcClient.invoke(IPCEventType.vcsListServers, {}) as Promise<
        RequestStatus<{ servers: VcsServerSession[] }>
      >,
    ...
    addServer: (authUrl: string, remoteUrl: string, token: string) =>
      ipcClient.invoke(IPCEventType.vcsAddServer, { authUrl, remoteUrl, token }) as Promise<
        RequestStatus<VcsAddServerOutcome>
      >,
    forgetServer: (remoteOrigin: string) =>
      ipcClient.invoke(IPCEventType.vcsForgetServer, { remoteOrigin }) as Promise<
        RequestStatus<{ servers: VcsServerSession[] }>
      >,
    push: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsPush, { projectPath }) as Promise<
        RequestStatus<VcsPushResult>
      >,
    /** Writes the working tree: re-read every document once this resolves. */
    sync: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.vcsSync, { projectPath }) as Promise<
        RequestStatus<VcsSyncResult>
      >,
    /** Destination must be an empty (or missing) folder. */
    clone: (url: string, destination: string) =>
      ipcClient.invoke(IPCEventType.vcsClone, { url, destination }) as Promise<
        RequestStatus<{ root: string; branch: string; fileCount: number }>
      >
  },

  gameBuild: {
    start: (projectPath: string, entry: GameRuntimeLaunchEntry, request: GameBuildRequest) =>
      ipcClient.invoke(IPCEventType.gameBuildStart, { projectPath, entry, request }) as Promise<
        RequestStatus<{ state: GameBuildStateSnapshot }>
      >,
    cancel: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.gameBuildCancel, { projectPath }) as Promise<
        RequestStatus<{ state: GameBuildStateSnapshot }>
      >,
    getStatus: (projectPath: string) =>
      ipcClient.invoke(IPCEventType.gameBuildGetStatus, { projectPath }) as Promise<
        RequestStatus<{ state: GameBuildStateSnapshot }>
      >,
    selectOutputDir: (defaultPath?: string) =>
      ipcClient.invoke(IPCEventType.gameBuildSelectOutputDir, { defaultPath }) as Promise<
        RequestStatus<{ path: string | null }>
      >,
    preflight: (projectPath: string, request: GameBuildRequest) =>
      ipcClient.invoke(IPCEventType.gameBuildPreflight, { projectPath, request }) as Promise<
        RequestStatus<{ findings: BuildPreflightFinding[] }>
      >
  },
  ...
  signing: {
    list: () =>
      ipcClient.invoke(IPCEventType.signingList, {}) as Promise<
        RequestStatus<{ credentials: SigningCredential[] }>
      >,
    /** `input` holds plain passwords. Do not log it or keep it after the call. */
    import: (input: SigningCredentialImport) =>
      ipcClient.invoke(IPCEventType.signingImport, { input }) as Promise<
        RequestStatus<{ credential: SigningCredential }>
      >,
    remove: (id: string) =>
      ipcClient.invoke(IPCEventType.signingRemove, { id }) as Promise<
        RequestStatus<{ removed: boolean }>
      >,
    inspect: (id: string) =>
      ipcClient.invoke(IPCEventType.signingInspect, { id }) as Promise<
        RequestStatus<SigningInspectResult>
      >,
    /** `storePassword` is plain text. Do not log it or keep it after the call. */
    keystoreAliases: (file: string, storePassword: string) =>
      ipcClient.invoke(IPCEventType.signingKeystoreAliases, { file, storePassword }) as Promise<
        RequestStatus<{ aliases: string[] }>
      >,
    /** The code-signing identities in this Mac's keychains; empty on other hosts. */
    macIdentities: () =>
      ipcClient.invoke(IPCEventType.signingMacIdentities, {}) as Promise<
        RequestStatus<{ identities: MacSigningIdentity[] }>
      >
  },
  ...
  blueprintPersistence: {
    getAll: (projectRef: BlueprintPersistenceProjectRef) =>
      ipcClient.invoke(IPCEventType.blueprintPersistenceGetAll, { projectRef }) as Promise<
        RequestStatus<{ values: Record<string, unknown> }>
      >,
    getValue: (projectRef: BlueprintPersistenceProjectRef, key: string) =>
      ipcClient.invoke(IPCEventType.blueprintPersistenceGetValue, { projectRef, key }) as Promise<
        RequestStatus<{ value: unknown }>
      >,
    setValue: (projectRef: BlueprintPersistenceProjectRef, key: string, value: unknown) =>
      ipcClient.invoke(IPCEventType.blueprintPersistenceSetValue, {
        projectRef,
        key,
        value
      }) as Promise<RequestStatus<void>>,
    removeValue: (projectRef: BlueprintPersistenceProjectRef, key: string) =>
      ipcClient.invoke(IPCEventType.blueprintPersistenceRemoveValue, {
        projectRef,
        key
      }) as Promise<RequestStatus<void>>
  },
  ...
  blueprintExternalLink: {
    ...
    openForPlugin: (pluginId: string, request: BlueprintOpenExternalRequest) =>
      ipcClient.invoke(IPCEventType.blueprintExternalLinkOpenForPlugin, {
        pluginId,
        request
      }) as Promise<RequestStatus<{ result: BlueprintOpenExternalResult }>>
  },
  ...
  plugins: {
    ...
    approve: (pluginId: string) => ipcClient.invoke(IPCEventType.pluginApprove, { pluginId }),
    uninstall: (pluginId: string) => ipcClient.invoke(IPCEventType.pluginUninstall, { pluginId }),
    revoke: (pluginId: string) => ipcClient.invoke(IPCEventType.pluginRevoke, { pluginId }),
    getWorkspacePlugins: () => ipcClient.invoke(IPCEventType.pluginWorkspaceList, {}),
    getRuntimePlugins: () => ipcClient.invoke(IPCEventType.pluginRuntimeList, {}),
    reportLoadError: (pluginId: string, error: string | null) =>
      ipcClient.invoke(IPCEventType.pluginReportLoadError, { pluginId, error }),
    getLocaleContributions: () => ipcClient.invoke(IPCEventType.pluginLocaleList, {}),
    onLocalesChanged: (handler: (change: { version: number }) => void) =>
      ipcClient.onMessage(IPCEventType.pluginLocalesChanged, handler),
    registryFetch: () => ipcClient.invoke(IPCEventType.pluginRegistryFetch, {}),
  ...
  uiTemplates: {
    registryFetch: () => ipcClient.invoke(IPCEventType.uiTemplateRegistryFetch, {}),
  ...
  projectTemplates: {
    list: () => ipcClient.invoke(IPCEventType.projectTemplateList, {}),
  ...
  puppetRuntimes: {
    installSdk: (runtimeId: string, projectPath: string, archivePath: string) =>
      ipcClient.invoke(IPCEventType.puppetRuntimeInstallSdk, {
        runtimeId,
        projectPath,
        archivePath
      })
