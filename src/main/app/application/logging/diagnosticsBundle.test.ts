...
describe("sanitizeBundleFileName", () => {
    it("keeps an ordinary name", () => {
        expect(sanitizeBundleFileName("narraleaf-studio-workspace-init-20260731-120000.log", "fallback.log"))
    ...
    it("reduces a path to its basename, so a renderer cannot steer the dialog", () => {
        ...
        expect(sanitizeBundleFileName("C:\\Windows\\System32\\report.log", "fallback.log")).toBe("report.log");
    });
...
describe("composeDiagnosticsBundle", () => {
    ...
    it("puts the report before the log tail", () => {
        ...
        expect(bundle.indexOf("=== NarraLeaf Studio diagnostics ==="))
            .toBeLessThan(bundle.indexOf("REPORT-MARKER"));
    ...
    it("marks a development build", () => {
        ...
        expect(composeDiagnosticsBundle({ ...ENVIRONMENT, packaged: true }, "", ""))
            .not.toContain("development build");
    });
});
