...
export function resolveStartupProject(
  ...
  const exact = projects.filter(
    (project) => foldName(recentProjectDisplayName(project)) === folded
  );
  if (exact.length === 1) {
  ...
  if (exact.length > 1) {
    return {
      ok: false,
      reason:
        `--project "${wanted}" matches ${exact.length} recent projects ` +
        `(${exact.map((project) => project.path).join(", ")}). Pass the path instead.`
    };
  }

  const partial = projects.filter((project) =>
    foldName(recentProjectDisplayName(project)).includes(folded)
  );
  if (partial.length === 1) {
  ...
  if (partial.length > 1) {
    return {
      ok: false,
      reason:
        `--project "${wanted}" matches ${partial.length} recent projects ` +
