...
describe("parseMainCommandLine", () => {
  ...
  it("parses inline CDP port values", () => {
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--cdp", "--cdp-port=9333"]).cdp
    ).toEqual({
      enabled: true,
      port: 9333,
      portSource: "argument",
  ...
  it("parses split CDP port values", () => {
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--cdp", "--cdp-port", "9334"]).cdp
    ).toEqual({
      enabled: true,
      port: 9334,
      portSource: "argument",
  ...
  it("reports invalid CDP port values", () => {
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--cdp", "--cdp-port=abc"]).cdp
    ).toEqual({
      enabled: true,
      port: DEFAULT_CDP_PORT,
      portSource: "default",
  ...
  it("parses inline dev reload port values", () => {
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--dev", "--dev-reload-port=5628"])
        .devReload
    ).toEqual({
      port: 5628,
      portSource: "argument",
  ...
  it("reports invalid dev reload port values", () => {
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--dev", "--dev-reload-port=abc"])
        .devReload
    ).toEqual({
      port: DEFAULT_DEV_RELOAD_PORT,
      portSource: "default",
  ...
  it("lets a later dev reload port override an earlier one", () => {
    ...
    const argv = [
      ...
      "dist/main/index.js",
      "--dev",
      "--dev-reload-port=5588",
      "--dev-reload-port=5628"
    ];
  ...
  it("reads the onboarding rerun flag", () => {
    expect(parseMainCommandLine(["electron", "dist/main/index.js", "--dev"]).onboarding).toBe(
      false
    );
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--dev", "--onboarding"]).onboarding
    ).toBe(true);
  });
  ...
  it("reads the onboarding skip flag", () => {
    expect(parseMainCommandLine(["electron", "dist/main/index.js", "--dev"]).skipOnboarding).toBe(
      false
    );
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--dev", "--skip-onboarding"])
        .skipOnboarding
    ).toBe(true);
  });
  ...
  it("parses inline and split --project values", () => {
    expect(
      parseMainCommandLine(["electron", "dist/main/index.js", "--dev", "--project=demo3"]).project
    ).toEqual({
      selector: "demo3",
      error: null
    });
