...
describe("PluginPermissionManager actor grants", () => {
  ...
  it("checks filesystem grants by plugin id, path, mode, and recursion", () => {
    ...
    expect(
      manager.isPluginFileSystemAllowed(
        plugin.id,
        plugin.version,
    ...
    expect(
      manager.isPluginFileSystemAllowed(
        plugin.id,
        plugin.version,
        path.join(tempDir, "other", "story.json"),
        "read"
      )
    ).toBe(false);
    expect(
  ...
  it("checks api and bash grants by plugin id", () => {
    ...
    expect(
      manager.isPluginCapabilityAllowed(plugin.id, plugin.version, ApiCapability.BashExecute)
    ).toBe(true);
    expect(
      manager.isPluginCapabilityAllowed("plugin.other", plugin.version, ApiCapability.BashExecute)
    ).toBe(false);
    expect(manager.isPluginCapabilityAllowed(plugin.id, plugin.version, "plugin.install")).toBe(
      false
    );
  });
  ...
  it("approves install prompts and grants declared manifest permissions for that plugin version", () => {
    ...
    expect(manager.isPluginCapabilityAllowed(plugin.id, "0.0.2", ApiCapability.BashExecute)).toBe(
      false
    );
  });
  ...
  it("revokes all persistent and temporary grants for an uninstalled plugin", () => {
    ...
    expect(manager.isPluginFileSystemAllowed(plugin.id, plugin.version, filePath, "read")).toBe(
      false
    );
    expect(manager.isPluginFileSystemAllowed(plugin.id, plugin.version, filePath, "write")).toBe(
      false
    );
    expect(
      manager.isPluginCapabilityAllowed(plugin.id, plugin.version, ApiCapability.BashExecute)
    ...
    expect(
      manager.getExistingGrantResult({
        kind: "filesystem",
        requestId: "fs-2",
        plugin,
        path: filePath,
