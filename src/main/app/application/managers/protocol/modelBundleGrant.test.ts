...
const HIYORI_FILES = [
  ...
  ...Array.from(
    { length: 10 },
    (_, index) => `motions/Hiyori_m${String(index + 1).padStart(2, "0")}.motion3.json`
  )
];
...
describe("model bundle directory grants", () => {
  ...
  it("reports the real MIME type per file, not one type for the whole bundle", async () => {
    ...
    const texture = await handler.handle(
      makeRequest(new URL("Hiyori.2048/texture_00.png", url).href)
    );
  ...
  it("404s a bundle path asked of an ordinary single-file grant", async () => {
    ...
    expect((await handler.handle(makeRequest(`app://fs/${fileHash}/Hiyori.moc3`))).statusCode).toBe(
      404
    );
  });
});
