 ...
 */
const TEST_LAUNCH_ENTRY: GameRuntimeLaunchEntry = {
  kind: "surface",
...
export function classifyGameTestExit(facts: GameTestExitFacts): GameTestExit {
  ...
  if (
    facts.sawMainRuntimeError ||
    facts.signal !== null ||
    (facts.code !== null && facts.code !== 0)
  ) {
...
export class GameTestManager {
  ...
  public launch(request: GameTestLaunchRequest): Promise<GameTestLaunchResult> {
    ...
    if (this.sessions.has(key)) {
      return Promise.resolve({
        ok: false,
        reason:
          "A test is already running a game for this project. Stop it before starting another."
      });
    }
  ...
  private async launchNow(session: GameTestSession, request: GameTestLaunchRequest): Promise<void> {
    try {
      ...
      this.emitConsole(
        session,
        "verbose",
        `artifact compile finished: ${artifact.copiedAssetCount} asset(s)`
      );
   ...
   */
  private dialControlSocket(
    session: GameTestSession,
    deadline: number
  ): Promise<ControlDialOutcome> {
      ...
      ws.on("message", (raw) => {
        ...
        if (!subscribed) {
          ...
          settle({
            outcome: "refused",
            error: new Error(
              typeof payload.error === "string" ? payload.error : "subscribe rejected"
            )
          });
          return;
        }
