...
export class MenuManager {
  ...
  public buildMenu(): Menu {
    ...
    this.app.logger.debug(
      `[Menu] Built: ${template.map((item) => item.label ?? item.role).join(" | ")}`
    );
  ...
  private buildWorkspaceTemplate(
    t: Translator["t"],
    window: AppWindow
  ): MenuItemConstructorOptions[] {
   ...
   * has no name for.
   */
  private buildDevMenu(
    t: Translator["t"],
    runtime: NativeMenuRuntimeStatus | undefined
  ): MenuItemConstructorOptions {
    ...
    const slotHeldByOther = (mine: boolean) =>
      (devModeActive || previewActive || testActive) && !mine;
  ...
  private openRecentProject(projectPath: string): void {
    ...
    void target
      .getApp()
      .openProject(target, projectPath, { replaceOpener: true })
      .catch((error) => {
  ...
  private buildSlottedItems(items: NativeMenuItem[]): MenuItemConstructorOptions[] {
    ...
    return [{ type: "separator" }, ...items.map((item) => this.buildSyncedMenuItem(item))];
  }
