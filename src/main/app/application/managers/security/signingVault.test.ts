...
function fakeSealer(available = true): SecretSealer & { available: boolean } {
  const sealer = {
    ...
    encryptString: (plainText: string) =>
      Buffer.concat([Buffer.from("sealed:"), Buffer.from(plainText, "utf8")]),
...
describe("SigningVault", () => {
  ...
  it("copies the key material in and leaves the original alone", async () => {
    ...
    await fs.rm(pfxPath);
    expect(await fs.readFile(vault.materialPath(credential, "file")!, "utf8")).toBe(
      "pretend pkcs12 bytes"
    );
  });
  ...
  it("refuses to persist the password when the keyring is unavailable, and says so", async () => {
    ...
    expect(await fs.readFile((resolved as { file: string }).file, "utf8")).toBe(
      "pretend pkcs12 bytes"
    );
  });
  ...
  it("degrades rather than throws when a sealed password cannot be opened", async () => {
    ...
    expect(await makeVault(wrongKeyring).resolveMaterial(credential.id)).toMatchObject({
      password: null
    });
  ...
  it("rejects an import that is missing a required field, leaving nothing behind", async () => {
    ...
    await expect(
      ...
      } as SigningCredentialImport)
    ).rejects.toThrow(/subject name or a thumbprint/);
    await expect(
      vault.import({ kind: "nonsense", label: "x" } as unknown as SigningCredentialImport)
  ...
  it("cleans up the material when a source file cannot be copied", async () => {
    const vault = makeVault();
    await expect(
      vault.import({ ...pfxImport(), file: path.join(sourceDir, "absent.pfx") })
    ).rejects.toThrow();
    expect(await fs.readdir(path.join(root, "material")).catch(() => [])).toEqual([]);
    expect(await vault.list()).toEqual([]);
  });
  ...
  it("refuses material names that would escape the credential's directory", async () => {
    ...
    const credential = await vault.import(pfxImport());
    const indexPath = path.join(root, "credentials.json");
    const file = JSON.parse(await fs.readFile(indexPath, "utf8")) as {
  ...
  it("does not let concurrent imports lose a credential", async () => {
    ...
    expect(new Set(listed.map((credential) => credential.id))).toEqual(
      new Set(imported.map((c) => c.id))
    );
  });

  it.skipIf(process.platform === "win32")(
    "stores the index and the key material owner-only",
    async () => {
  ...
  describe("plugin build secrets", () => {
    ...
    it("drops the previous ciphertext when a re-set cannot seal", async () => {
      ...
      await expect(vault.setPluginSecret("a different token", handle)).resolves.toEqual({
        handle,
        available: false
      });
