...
describe("MediaConvertManager logging", () => {
    ...
    function errorLines(): string {
        return entries.filter(entry => entry.level === "error").map(entry => entry.message).join("\n");
    }
    ...
    it("logs nothing at error level when the conversion succeeds", async () => {
        ...
        expect(entries.some(entry => entry.level === "info" && entry.message.includes(target))).toBe(true);
    });
    ...
    it("records a host with no ffmpeg as a warning rather than a failure", async () => {
        ...
        expect(errorLines()).toBe("");
        expect(entries.some(entry => entry.level === "warn" && entry.message.includes("No ffmpeg"))).toBe(true);
    });
});
