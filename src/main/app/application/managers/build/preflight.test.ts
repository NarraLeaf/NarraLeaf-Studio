...
describe("readMobileOrientation", () => {
  ...
  it("falls back rather than pass an unknown value to the shell", () => {
    ...
    expect(readMobileOrientation(config({ mobile: { orientation: "sideways" } }))).toBe(
      "landscape"
    );
...
describe("sidecar preflight", () => {
  ...
  it("blocks an executable sidecar cross-built from Windows for macOS or Linux", () => {
    ...
    expect(
      requirements
        .filter((requirement) => sidecarLosesExecBit(requirement, "windows"))
    ...
    expect(requirements.some((requirement) => sidecarLosesExecBit(requirement, "macos"))).toBe(
      false
    );
    expect(requirements.some((requirement) => sidecarLosesExecBit(requirement, "linux"))).toBe(
      false
    );
  });
...
describe("checkIcon", () => {
  ...
  it("reports a corrupt PNG as unusable", async () => {
    await fs.writeFile(
      path.join(projectPath, "resources/icons/derived/windows.png"),
  ...
  it("resolves a legacy five-slot project through its promoted master", async () => {
    ...
    expect(await checkIcon(projectPath, config, "windows")).toMatchObject({
      status: "ok",
      baked: false
    });
...
describe("the Google Play finding", () => {
  ...
  async function findingsFor(
    formats: GameBuildTarget["formats"]
...
describe("signingReachesNetwork", () => {
  ...
  it("flags a macOS credential only when it actually notarizes", () => {
    ...
    expect(
      signingReachesNetwork(
        credential("macos-keychain", { identity: "Developer ID Application: X" })
      )
    ).toBe(false);
    expect(signingReachesNetwork(credential("macos-apple", { p12File: "cert.p12" }))).toBe(false);
    expect(
      signingReachesNetwork(
        credential("macos-apple", {
          p12File: "cert.p12",
...
describe("findGpgBinary", () => {
  ...
  it("prefers the path recorded on the credential over PATH", async () => {
    ...
    expect(
      await findGpgBinary({ configuredPath: configured, env: { PATH: onPath }, platform: "linux" })
    ).toBe(configured);
  });
  ...
  it("looks for the .exe names and the Gpg4win install location on Windows", async () => {
    ...
    expect(
      await findGpgBinary({ env: { PATH: "", ProgramFiles: programFiles }, platform: "win32" })
    ).toBe(gpg);
  ...
  it("reports nothing when the host has no gpg", async () => {
    expect(
      await findGpgBinary({ env: { PATH: path.join(root, "empty") }, platform: "linux" })
    ).toBeNull();
    ...
    expect(findings).toEqual([
      {
        ...
        detail: {
          plugin: "Acme Storefront",
          field: "Storefront app id",
          platforms: "windows, macos"
        }
      }
    ]);
