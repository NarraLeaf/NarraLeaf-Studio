...
const { readIconSlotSizes, readPngSize, stripAlphaChannel, writeScaledIcons } =
  await import("./mobileIcons");
...
async function makeTemplateZip(entries: Record<string, Buffer>): Promise<Buffer> {
  ...
  await writeZip(
    ...
    Object.entries(entries).map(([name, data]) => ({
      name,
      source: { kind: "buffer" as const, data }
    })),
...
describe("readIconSlotSizes", () => {
  ...
  it("resolves slots under an entry prefix (the iOS .app dir)", async () => {
    ...
    expect(readIconSlotSizes(zip, ["AppIcon60x60@2x.png"], "Shell.app/")).toEqual([
      { slot: "AppIcon60x60@2x.png", width: 120, height: 120 }
    ]);
  });
  ...
  it("refuses a declared slot the template does not hold", async () => {
    ...
    expect(() => readIconSlotSizes(zip, ["res/mipmap-hdpi-v4/ic_launcher.png"])).toThrow(
  ...
  it("refuses a slot that is not a readable PNG", async () => {
    const zip = await makeTemplateZip({
      "res/mipmap-mdpi-v4/ic_launcher.png": Buffer.from("not a png")
    });
    expect(() => readIconSlotSizes(zip, ["res/mipmap-mdpi-v4/ic_launcher.png"])).toThrow(
      /not a readable PNG/
    );
  });
});
...
describe("writeScaledIcons", () => {
  it("scales the icon to every slot's size and maps slot to file", async () => {
    ...
    const written = await writeScaledIcons(
      "/project/icon.png",
      [
        { slot: "res/mipmap-mdpi-v4/ic_launcher.png", width: 48, height: 48 },
        { slot: "res/mipmap-xxhdpi-v4/ic_launcher.png", width: 144, height: 144 }
    ...
    expect(await fs.readFile(written["res/mipmap-mdpi-v4/ic_launcher.png"], "utf8")).toBe(
      "png:48x48"
    );
    expect(await fs.readFile(written["res/mipmap-xxhdpi-v4/ic_launcher.png"], "utf8")).toBe(
      "png:144x144"
    );
  });
  ...
  it("fails loudly when the icon cannot be read", async () => {
    ...
    await expect(writeScaledIcons("/project/empty.png", [], outputDir)).rejects.toThrow(
      /could not be read/
    );
  });
  ...
  it("strips the alpha channel an opaque target must not carry", async () => {
    ...
    const opaquePng = Buffer.from(
      await encodeOpaquePng(rgba, 4, 4, (bytes) => zlib.deflateSync(bytes))
    );
