...
describe("resolveMobileShellDirForApp", () => {
  it("uses the staged copy under resources when packaged", () => {
    ...
    expect(resolveMobileShellDirForApp(app)).toBe(
      path.join("/Studio.app/Contents/Resources", "mobile-shell")
    );
  });
  ...
  it("reads node_modules directly in development", () => {
    ...
    expect(resolveMobileShellDirForApp(app)).toBe(
      path.resolve("/repo/node_modules/@narraleaf/studio-shell")
    );
  });
});
...
describe("loadMobileShellTemplate", () => {
  ...
  it("reports an incompatible template package distinctly from a broken one", async () => {
    const dir = await makeTemplateDir({
      ...validManifest,
      schemaVersion: SUPPORTED_SHELL_MANIFEST_SCHEMA + 1
    });
    await expect(loadMobileShellTemplate(dir, "release")).rejects.toThrow(
  ...
  it("names the missing variant when a template file was not staged", async () => {
    ...
    await expect(loadMobileShellTemplate(dir, "debug")).rejects.toThrow(
      /Android debug shell template is missing/
    );
  });
...
describe("the installed @narraleaf/studio-shell package", () => {
  ...
  const packageDir = path.resolve(
    __dirname,
    "../../../../../../node_modules/@narraleaf/studio-shell"
  );
  ...
  it("satisfies the manifest contract for both variants", async () => {
    for (const variant of ["release", "debug"] as const) {
      ...
      expect(template.manifest.android.placeholders.applicationId).toBe(
        "com.narraleaf.shell.placeholder"
      );
