...
`;
    expect(
      await findMacSigningIdentities({ platform: "darwin", run: async () => duplicated })
    ).toHaveLength(1);
  });
...
`;
    expect(
      await findMacSigningIdentities({
        platform: "darwin",
        validOnly: false,
        run: async () => both
      })
...
describe.skipIf(process.platform !== "darwin")("the security oracle", () => {
  const runReal = (validOnly: boolean) => async (): Promise<string> =>
    (
...
describe("macIdentityPresent", () => {
  ...
  it("matches the full name and the thumbprint", () => {
    expect(
      macIdentityPresent(identities, "Developer ID Application: NarraLeaf Ltd (A1B2C3D4E5)")
    ).toBe(true);
  ...
  it("refuses a name that is not there, and an empty one", () => {
    ...
    expect(macIdentityPresent([], "Developer ID Application: NarraLeaf Ltd (A1B2C3D4E5)")).toBe(
      false
    );
  });
});
