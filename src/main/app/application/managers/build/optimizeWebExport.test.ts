...
async function readPack(): Promise<{
  assets: { items: Record<string, GameRuntimeAssetManifestEntry> };
}> {
...
describe("optimizeWebExportImages", () => {
  ...
  it("leaves a JPEG alone in lossless mode and takes it in lossy mode", async () => {
    ...
    const items = {
      [ASSET_B]: { relativePath: `assets/${ASSET_B}.jpg`, ext: "jpg", mimeType: "image/jpeg" }
    };
  ...
  it("never touches a model bundle's files", async () => {
    ...
    await expect(
      fs.stat(path.join(appDir, "assets", ASSET_B, "textures", "texture_00.png"))
    ).resolves.toBeTruthy();
  });
  ...
  it("stops when the build is cancelled, leaving the manifest consistent with the files", async () => {
    ...
    for (const entry of Object.values(items)) {
      await expect(
        fs.stat(path.join(appDir, ...entry.relativePath.split("/")))
      ).resolves.toBeTruthy();
    }
  });
