...
import {
  readProjectAppTagDocumentFromDir,
  readProjectAppTagsFromDir
} from "../../utils/appTagsFile";
import {
...
import {
  buildWebIndexHtml,
  WEB_APPLE_TOUCH_FILENAME,
  WEB_FAVICON_FILENAME
} from "../preview/compiler/webShell";
import { formatPreviewProcessOutput } from "../preview/PreviewManager";
import {
  selectRuntimePluginsForPack,
  type RuntimePluginPackSelection
} from "../preview/selectRuntimePlugins";
import type {
 ...
 * which of the two happened.
 */
export function toWorkerMacSigning(
  material: ResolvedSigningMaterial
): GameBuildWorkerMacSigning | null {
...
export function toWorkerGpgSigning(
  material: ResolvedSigningMaterial
): GameBuildWorkerGpgSigning | null {
...
export function toWorkerAndroidSigning(
  material: ResolvedSigningMaterial
): GameBuildWorkerAndroidSigning | null {
  if (
    material.kind !== "android-keystore" ||
...
export class GameBuildManager {
  ...
  public async preflight(
    ...
    const crossTargets = desktopTargets.filter(
      (target) =>
        target.platform !== hostPlatform && hostCanBuildTarget(hostPlatform, target.platform)
    );
    ...
    if (!version) {
      ...
      mobileTargets.some((target) => target.platform === "android") &&
    ...
    if (!variantsUnreadable) {
      findings.push(
        ...(await collectVariantContentFindings({
    ...
    findings.push(
      ...(await this.pluginConfigPreflight(
        (
    ...
    if (desktopTargets.length > 0 && this.encryptAssetsEnabled(projectConfig)) {
      const key = await this.resolveEncryptionKey(normalizedProjectPath, projectConfig).catch(
        () => undefined
      );
      if (!key) {
        findings.push({
    ...
    if (
      targets.some((target) => target.platform === "web") &&
      this.encryptAssetsEnabled(projectConfig)
    ) {
      findings.push({ code: "web-unprotected", severity: "warning", section: "content" });
    }
    findings.push(
      ...(await collectProgressCarryFindings({
        projectPath: normalizedProjectPath,
  ...
  private async run(
    ...
    if (unknownTargets.length > 0) {
      throw new Error(
        `Unknown build platform(s): ${unknownTargets.map((t) => String(t.platform)).join(", ")}`
      );
    }
    ...
    const unbuildable = desktopTargets.filter(
      (target) => !hostCanBuildTarget(hostPlatform, target.platform)
    );
    ...
    const encryptionKey =
      desktopTargets.length > 0 || mobileTargets.length > 0
    ...
    if (encryptionKey && desktopTargets.length > 0) {
      this.emit(session, {
        level: "info",
        source: "Build",
        message: "asset protection enabled; sealing pack"
      });
    }
    if (encryptionKey && mobileTargets.length > 0) {
      this.emit(session, {
        level: "info",
        source: "Build",
        message: "asset protection enabled; protecting the mobile payload"
      });
    }
    ...
    const sidecarPlatformKey =
      sidecarPlatformKeys.length === 1 ? sidecarPlatformKeys[0] : undefined;
    if (
      !sidecarPlatformKey &&
      ...
      this.emit(session, {
        ...
        source: "Build",
        message:
          `plugin sidecars ship for one platform per build, but this build targets ` +
    ...
    if (desktopTargets.length > 0) {
      ...
      desktopArtifact = await compileGameRuntimeArtifactInWorker(
        this.app,
        {
          projectPath,
          entry,
    ...
    if (webTarget || mobileTargets.length > 0) {
      webArtifact = await compileGameRuntimeArtifactInWorker(
        this.app,
        {
          projectPath,
          entry,
    ...
    const workerConfig: GameBuildWorkerConfig = {
      ...
      targets: await Promise.all(
        desktopTargets.map(async (target) => ({
          platform: target.platform,
    ...
    if (icon.status === "ok") {
      if (icon.lowResolution) {
        this.emit(session, {
          ...
          source: "Build",
          message:
            `the ${platform} icon is smaller than ${MIN_ICON_SIZE}×${MIN_ICON_SIZE}; ` +
            "it ships upscaled"
        });
      }
  ...
  private async resolveDesktopTargetSigning(
    ...
    if (platform === "windows" && signing.windows) {
      ...
      this.emit(session, {
        ...
        source: "Build",
        message:
          "the Windows build is code signed, so asar integrity validation is enabled; " +
    ...
    if (platform === "macos" && signing.macos) {
      ...
      this.emit(session, {
        ...
        source: "Build",
        message:
          "the macOS build is code signed, so asar integrity validation is enabled" +
  ...
  private async pluginConfigPreflight(
    ...
    for (const slot of pluginBuildConfigSlots(fields, platforms)) {
      ...
      if (
        field.type === "secret" &&
        vault &&
        !(await vault.pluginSecretAvailable(resolved.value))
      ) {
        findings.push({
  ...
  private async signingPreflight(
    ...
    const slots = new Set(
      targets
        .map((target) => signingPlatformForTarget(target.platform))
    ...
    for (const platform of slots) {
      ...
      if (
        SIGNING_CREDENTIAL_SECRET_FIELDS[credential.kind].length > 0 &&
      ...
      if (
        credential.kind === "linux-gpg" &&
        !(await findGpgBinary({
          ...(credential.gpgPath ? { configuredPath: credential.gpgPath } : {})
        }))
      ) {
        findings.push({
  ...
  private async signingExpiryPreflight(
    ...
    const inspected = await inspectCertificateFile(container.file, container.secrets).catch(
      () => null
    );
  ...
  private async buildMobileJob(
    ...
    const job: GameBuildWorkerMobileJob = {
      ...
      orientation,
      indexHtmlOverride: buildWebIndexHtml(site.pack, {
        hasFavicon,
        hasAppleTouchIcon,
        variant: "mobile"
      }),
    ...
    if (androidTarget) {
      const outputs = androidOutputNames(
        androidTarget.formats,
      ...
      if (applicationId !== identity.appId) {
        this.emit(session, {
          ...
          source: "Build",
          message:
            `the app id ${identity.appId} is not a valid Android package name; ` +
      ...
      const releaseKeystore = input.signing.android
        ? toWorkerAndroidSigning(input.signing.android)
        : null;
      if (releaseKeystore) {
        ...
        this.emit(session, {
          ...
          source: "Build",
          message:
            "the Android build is signed with your release keystore instead of the local debug " +
    ...
    if (input.targets.some((target) => target.platform === "ios")) {
      ...
      if (bundleId !== identity.appId) {
        this.emit(session, {
          ...
          source: "Build",
          message:
            `the app id ${identity.appId} is not a valid iOS bundle identifier; ` +
  ...
  private async resolveMobileIcons(
    ...
    const slots = readIconSlotSizes(
      await fs.readFile(input.templatePath),
      input.slots,
      input.entryPrefix
    );
        ...
        settle(() =>
          reject(
            new Error(
              session.cancelled
                ? "Build cancelled"
                : `Packaging worker exited unexpectedly (code ${code})`
            )
          )
        );
      });
  ...
  private async resolveBuildVariant(
    ...
    if (requested && !hasAppTag(appTags, requested)) {
      throw new Error(
        `The project has no build variant "${requested}". Pick one in the build dialog.`
      );
    }
    ...
    if (!isBuiltinAppTagId(tag.id)) {
      this.emit(session, {
        level: "info",
        source: "Build",
        message: `building the "${tag.name}" variant`
      });
    }
    return tag;
  }
  ...
  private resolveIdentity(
    ...
    const projectName =
      projectConfig?.name?.trim() || path.basename(projectPath) || "NarraLeaf Game";
    ...
    const copyright =
      typeof rawCopyright === "string" && rawCopyright.trim() ? rawCopyright.trim() : undefined;
    return {
      appId,
      productName,
  ...
  private encryptAssetsEnabled(projectConfig: ProjectConfigData | null): boolean {
    return (
      (projectConfig?.app as { security?: { encryptAssets?: unknown } } | undefined)?.security
        ?.encryptAssets === true
    );
  }

  private webOptimizationConfig(
    projectConfig: ProjectConfigData | null
  ...
  private async optimizeCompiledSite(
    ...
    try {
      ...
      this.emit(session, {
        ...
        source: "Build",
        message:
          `${config.lossyImages ? "recompressed" : "converted"} ${result.converted} image(s) to WebP, ` +
      ...
      this.emit(session, {
        ...
        source: "Build",
        message:
          "could not optimize the exported images, so they ship as they are: " +
