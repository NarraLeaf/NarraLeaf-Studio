...
describe("androidOutputNames", () => {
  it("names only the packages the target selected", () => {
    expect(androidOutputNames(["apk"], "MyGame", "1.2.0")).toEqual({
      apk: "MyGame-1.2.0-android.apk"
    });
    expect(androidOutputNames(["aab"], "MyGame", "1.2.0")).toEqual({
...
describe("gameFusesForPlatform", () => {
  ...
  it("keeps asar integrity off on unsigned builds (downside-only footgun)", () => {
    for (const platform of ["windows", "macos", "linux"] as const) {
      expect(gameFusesForPlatform(platform, false).enableEmbeddedAsarIntegrityValidation).toBe(
        false
      );
    }
  });
...
describe("resolveElectronDistDirForApp", () => {
  it("uses the embedded preview runner when packaged", () => {
    ...
    expect(resolveElectronDistDirForApp(app)).toBe(
      path.join("/resources", "preview-runner", "dist")
    );
  });
...
describe("hasSigningIdentityForPlatform", () => {
  ...
  it("earns the asar-integrity fuse only where a real signature seals it", () => {
    ...
    expect(
      gameFusesForPlatform(
        "windows",
        hasSigningIdentityForPlatform("windows", { windows: windowsPfx })
      )
  ...
  it("stays false on macOS and Linux even when other platforms are signed", () => {
    ...
    const signing = {
      windows: windowsPfx,
      linux: { kind: "linux-gpg", id: "cred-gpg", keyId: "ABCD" }
    } as const;
...
describe("signingSecretsResolved", () => {
  it("accepts material whose passwords all came back", () => {
    expect(
      signingSecretsResolved({ kind: "windows-pfx", id: "a", file: "/f.pfx", password: "pw" })
    ).toBe(true);
    expect(
      signingSecretsResolved({
  ...
  it("rejects material with an unsealed password", () => {
    ...
    expect(
      signingSecretsResolved({ kind: "windows-pfx", id: "a", file: "/f.pfx", password: null })
    ).toBe(false);
    expect(
      signingSecretsResolved({
        kind: "android-keystore",
...
describe("signing material mapped for the worker", () => {
  ...
  it("maps a certificate-store credential onto whichever selector it carries", () => {
    expect(
      toWorkerWindowsSigning({ kind: "windows-store", id: "a", subjectName: "Studio Ltd" })
    ).toEqual({ source: "certificate-store", certificateSubjectName: "Studio Ltd" });
    expect(toWorkerWindowsSigning({ kind: "windows-store", id: "a", sha1: "DEADBEEF" })).toEqual({
      source: "certificate-store",
      certificateSha1: "DEADBEEF"
    });
  });
  ...
  it("refuses a Windows mapping for a credential of another platform, or an unsealed one", () => {
    ...
    expect(
      toWorkerWindowsSigning({ kind: "windows-pfx", id: "a", file: "/c.pfx", password: null })
    ).toBeNull();
  });
  ...
  it("maps gpg to a key id and nothing more", () => {
    ...
    expect(toWorkerGpgSigning({ kind: "linux-gpg", id: "a", keyId: "ABCD1234" })).toEqual({
      keyId: "ABCD1234"
    });
    expect(
      toWorkerGpgSigning({ kind: "linux-gpg", id: "a", keyId: "ABCD", gpgPath: "/usr/bin/gpg" })
    ).toEqual({ keyId: "ABCD", gpgPath: "/usr/bin/gpg" });
    expect(
      toWorkerGpgSigning({ kind: "windows-pfx", id: "a", file: "/c.pfx", password: "pw" })
    ).toBeNull();
  });
  ...
  it("maps the Apple identity with the profile that gets embedded", () => {
    expect(
      toWorkerIosSigning(
        ...
        "/resources/codesign/win32/zsign.exe"
      )
    ).toEqual({
      p12File: "/vault/apple.p12",
      p12Password: "pw",
...
describe("iosSigningToolPathFrom", () => {
  it("hands over the path of a staged tool", () => {
    expect(
      iosSigningToolPathFrom({ available: true, path: "C:/app/resources/codesign/win32/zsign.exe" })
