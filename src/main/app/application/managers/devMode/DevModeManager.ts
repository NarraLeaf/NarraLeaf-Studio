...
export class DevModeManager {
  ...
  constructor(
    ...
    compiler?: INLangCompiler,
    bundleSource?: DevModeBundleSource
  ) {
  ...
  private async resolveLaunchSource(session: DevModeSession): Promise<void> {
    const source = await resolveDevModeLaunchSource({
      ...
      materialize: (revision) =>
        this.app.getVcsManager().materializeRevisionSnapshot(session.projectPath, revision, {
          onProgress: (message) =>
            this.emitWorkspaceConsoleLog(session, {
              level: "info",
              source: "Dev Mode",
    ...
    this.emitWorkspaceConsoleLog(session, {
      ...
      source: "Dev Mode",
      message:
        `running version ${source.revision.slice(0, 12)}, not your current files, because that is` +
  ...
  private async requestBlueprintCloseDecision(
    window: AppWindow<WindowAppType.DevMode>
  ): Promise<boolean> {
    try {
      ...
      if (!result.success) {
        this.app.logger.warn(
          `[DevMode] close decision failed, closing the window: ${result.error}`
        );
        return true;
      }
      ...
      this.app.logger.warn(
        `[DevMode] no answer to the close decision, closing the window: ${String(error)}`
      );
      return true;
    }
  }

  private async compileAndSendBundle(
    session: DevModeSession,
    status: DevModeStatus
  ): Promise<void> {
    ...
    try {
      ...
      if (!blueprintScripts.ok) {
        ...
        this.app.logger.error(
          "[DevMode] TypeScript blueprint compile failed",
          blueprintScripts.errors
        );
        this.emitWorkspaceConsoleLog(session, {
          level: "error",
          source: "Dev Mode",
  ...
  private tryFlushPendingToDevWindow(session: DevModeSession): void {
    ...
    if (session.pendingBundle) {
      ...
      window.sendIpcEvent(IPCEventType.devModeControlReload, {
        revision: session.pendingBundle.revision
      });
  ...
  private watchProjectFiles(session: DevModeSession): void {
    ...
    const characterStorePath = path.join(
      session.projectPath,
      "editor",
      "services",
      "character.json"
  ...
  private scheduleReload(session: DevModeSession, event: string, file: string): void {
    ...
    this.emitVerbose(
      session,
      `project file ${event}; scheduling reload: ${path.relative(session.projectPath, file)}`
    );
