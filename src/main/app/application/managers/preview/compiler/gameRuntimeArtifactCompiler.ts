...
export type GameRuntimeArtifactCompileInput = {
    ...
    /**
     * `build.zigMirror` as the author set it, for the toolchain a per-title build
...
/* A compile with no target named is a preview: it runs from this app dir on
     ...
     *
     * With one, each image is compiled with this title's binding in it, and a
     * published copy of the codec package opens nothing this build sealed -
     * which is the whole reason per-title compilation exists. Without one, the
