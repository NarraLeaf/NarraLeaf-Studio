...
export async function compileGameRuntimePreviewArtifact(
    ...
    await fs.writeFile(packPath, input.encryptionKey ? protectBuffer(packJson, input.encryptionKey) : packJson);
    await fs.writeFile(
...
async function copyMaybeEncrypt(sourcePath: string, targetPath: string, encryptionKey?: string): Promise<void> {
    ...
    await fs.writeFile(targetPath, protectBuffer(data, encryptionKey));
}
