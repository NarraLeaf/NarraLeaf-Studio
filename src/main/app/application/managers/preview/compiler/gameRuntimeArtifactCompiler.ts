 ...
 */
type PackTarget = { kind: "loose" } | { kind: "sealed"; writer: SealedBundleWriter };
...
export async function compileGameRuntimeArtifact(
  ...
  const stripping =
    Boolean(input.packaging) &&
    Boolean(input.appTag) &&
    !isBuiltinAppTagId(input.appTag?.id ?? APP_TAG_ID_RELEASE);
  const shipped = stripping
    ? await planShippedAssets(input.projectPath, assembled, input.runtimePlugins ?? [], (message) =>
  ...
  if (shipped && shipped.removedAssetCount > 0) {
    notices.push(
      `${shipped.removedAssetCount} assets are unreachable in this edition and do not ship`
    );
  }
  ...
  try {
    ...
    const pack: GameRuntimePackV1 = {
      ...
      network: {
        ...
        allowHttp:
          (projectConfig?.app as { network?: { allowHttp?: unknown } } | undefined)?.network
            ?.allowHttp === true,
...
async function assertRuntimeDistReady(
  ...
  try {
    manifest = await readJson<{ mode?: unknown }>(
...
export function planShippedCharacters(
  ...
  onNotice?.(
    `${characters.length - kept.length} characters are unreachable in this edition and do not ship`
  );
  ...
  if (localization && localization.removedUnitCount > 0) {
    onNotice?.(
      `${localization.removedUnitCount} character names belong to characters that do not ship`
    );
  }
  return {
    bundle: {
    ...
    for (const [assetId, rawAsset] of Object.entries(metadata)) {
      ...
      try {
        if (input.target.kind === "sealed") {
          ...
          relativePath = path
            .join("assets", `${normalized.id}.${normalized.ext}`)
            .replace(/\\/g, "/");
          await fs.copyFile(
            sourcePath,
            path.join(input.assetsDir, `${normalized.id}.${normalized.ext}`)
          );
        }
      } catch (error) {
        throw new Error(
...
async function copyAssetBundle(input: {
  ...
  if (files.length === 0) {
    throw new Error(
      `Model bundle "${normalized.name}" (${normalized.id}) is empty at ${sourceDir}`
    );
  }

  const authored = input.authoredEntry ? normalizeBundlePath(input.authoredEntry) : null;
  const entry =
    authored && files.includes(authored) ? authored : detectModelBundleEntry(files).entry;
  if (!entry) {
  ...
  const makeEntry = (
    ...
    originalRelativePath: path
      .relative(input.projectPath, path.join(sourceDir, filePath))
      .replace(/\\/g, "/"),
...
async function copyPuppetRuntimes(input: {
  ...
  for (const dirent of dirents
      ...
      try {
        if (input.target.kind === "sealed") {
          await input.target.writer.add(
            gameRuntimeBundleRuntimeEntry(relativePath),
    ...
    entries.push({
      name: dirent.name,
      entryRelativePath: path.posix.join(
        PUPPET_RUNTIMES_PACK_DIR,
        dirent.name,
        PUPPET_RUNTIME_ENTRY_FILE
      ),
  ...
  for (const plugin of input.runtimePlugins) {
    ...
    try {
      if (input.target.kind === "sealed") {
        await input.target.writer.add(
          gameRuntimeBundleRuntimeEntry(relativePath),
          await fs.readFile(plugin.entryPath)
        );
      } else {
        const targetPath = path.join(input.appDir, ...relativePath.split("/"));
        await fs.mkdir(path.dirname(targetPath), { recursive: true });
...
async function resolveSidecarInclude(input: {
  ...
  if (include.startsWith(SIDECAR_DEP_INCLUDE_PREFIX)) {
    ...
    const relativePath = normalizeSidecarPath(
      separator === -1 ? "" : reference.slice(separator + 1)
    );
    if (!relativePath) {
      throw new Error(`${where}: include "${include}" names no file inside the build dependency`);
    }
    const dependency = plugin.manifest.contributes.buildDependencies.find(
      (item) => item.id === dependencyId
    );
    if (!dependency) {
      throw new Error(
        `${where}: include "${include}" references undeclared build dependency "${dependencyId}"`
      );
    }
  ...
  const expected = (input.target.sha256[include] ?? input.target.sha256[relativePath] ?? "")
    .trim()
    .toLowerCase();
  if (!expected) {
...
async function copyProjectIcon(input: {
  ...
  const icon =
    ...
    (input.defaultIcon
      ? {
          path: input.defaultIcon.path,
          sourceName: path.basename(input.defaultIcon.path),
          mediaType: "image/png"
        }
      : null);
  if (!icon) {
  ...
  const extension = normalizeExtension(
    path.extname(icon.path).replace(".", ""),
    icon.path,
    "other"
  );
...
async function copyWebIcons(input: {
  ...
  const appleTouch =
    projectIcon("web-apple-touch") ?? input.defaultIcon?.opaquePath ?? input.defaultIcon?.path;
  return {
...
function normalizeAssetRecord(assetId: string, type: string, rawAsset: AssetMetadataRecord) {
  ...
  const name =
    typeof rawAsset?.name === "string" && rawAsset.name.trim() ? rawAsset.name.trim() : id;
 ...
 */
async function readEndingSurfaceId(
  projectPath: string,
...
function resolvePackNetworkAllowlist(
  ...
  const network = (
    projectConfig?.app as { network?: { policy?: unknown; allowlist?: unknown } } | undefined
  )?.network;
