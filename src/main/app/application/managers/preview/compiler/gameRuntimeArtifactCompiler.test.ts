...
describe("game runtime artifact compiler", () => {
  ...
  it("writes a real preview app with pack.json and flat copied assets", async () => {
    ...
    await expect(fs.readFile(path.join(result.appDir, "main.js"), "utf-8")).resolves.toBe(
      "// main"
    );
    await expect(fs.readFile(path.join(result.appDir, "preload.js"), "utf-8")).resolves.toBe(
      "// preload"
    );
    await expect(fs.readFile(path.join(result.appDir, "renderer.js"), "utf-8")).resolves.toBe(
      "// renderer"
    );
    await expect(fs.readFile(path.join(result.appDir, "renderer.css"), "utf-8")).resolves.toBe(
      "/* renderer css */"
    );
    await expect(fs.readFile(path.join(result.appDir, "renderer.css.map"), "utf-8")).resolves.toBe(
      "{}"
    );
    await expect(fs.readFile(path.join(result.appDir, "index.html"), "utf-8")).resolves.toBe(
      "<!doctype html>"
    );
    await expect(
      fs.readFile(path.join(result.appDir, "assets", `${ASSET_ID}.png`), "utf-8")
    ).resolves.toBe("local image bytes");
    await expect(
      fs.readFile(
  ...
  it("copies plugin runtime entries into the pack", async () => {
    ...
    await expect(
      fs.readFile(path.join(result.appDir, "plugins", "acme.sample-plugin", "runtime.js"), "utf-8")
  ...
  it("copies each installed puppet runtime directory whole and records it in the pack", async () => {
    ...
    await fs.mkdir(path.join(projectPath, "runtimes", "puppet", "half-installed"), {
      recursive: true
    });
    ...
    await expect(
      fs.readFile(path.join(result.appDir, "puppet", "demo-backend", "index.js"), "utf-8")
    ).resolves.toBe("export default {};");
  ...
  it("omits the puppet runtime list when the project installed none", async () => {
    ...
    const result = await compileGameRuntimeArtifact(
      previewCompileInput(projectPath, runtimeDistDir, 47332)
    );
  ...
  it("refuses to ship a sidecar file that does not match its declared digest", async () => {
    ...
    await expect(
      ...
      new RegExp(
        `Sidecar "${SIDECAR_ID}" of plugin "${SIDECAR_PLUGIN_ID}".+bin/tool\\.exe.+sha256`,
        "s"
      )
    );
  });
  ...
  it("resolves a dep: include through the build dependency cache", async () => {
    ...
    await expect(fs.readFile(path.join(sidecarDir, "bin", "redist.dll"), "utf-8")).resolves.toBe(
      dependencyBytes
    );
    await expect(fs.readFile(path.join(sidecarDir, "bin", "tool.exe"), "utf-8")).resolves.toBe(
      "MZ fake executable"
    );
  ...
  it("gives each plugin its own build config, resolved for the variant, with no secret in it", async () => {
    ...
    const compile = async (appTag?: { id: string; name: string }) =>
      (
        await compileGameRuntimeArtifact({
          projectPath,
          runtimeDistDir,
          runtimeVersion: "0.0.1-test",
    ...
    expect(release.plugins[1].buildConfig).toEqual({
  ...
  it("produces an empty plugin list when no runtime plugins are supplied", async () => {
    ...
    const result = await compileGameRuntimeArtifact(
      previewCompileInput(projectPath, runtimeDistDir, 47325)
    );
  ...
  it("packs a remote asset from its versioned snapshot, like any other asset", async () => {
    ...
    const result = await compileGameRuntimeArtifact(
      previewCompileInput(projectPath, runtimeDistDir, 47322)
    );

    const entry = result.pack.assets.items[REMOTE_ASSET_ID];
    expect(entry.source).toBe("remote");
    await expect(
      fs.readFile(path.join(result.appDir, "assets", `${REMOTE_ASSET_ID}.jpg`), "utf-8")
  ...
  it("fails with a clear diagnostic when a remote asset has never been fetched", async () => {
    ...
    await expect(
      compileGameRuntimeArtifact(previewCompileInput(projectPath, runtimeDistDir, 47323))
  ...
  it("preserves authored Animate opacity percent params in the preview pack", async () => {
    ...
    const result = await compileGameRuntimeArtifact(
      previewCompileInput(projectPath, runtimeDistDir, 47323)
    );
  ...
  it("consolidates the pack, assets and plugins into a single protected store", async () => {
    ...
    await expect(
      fs.access(path.join(result.appDir, RUNTIME_BUNDLE_FILENAME))
    ).resolves.toBeUndefined();
    await expect(
      fs.access(path.join(result.appDir, RUNTIME_SUPPORT_FILENAME))
    ).resolves.toBeUndefined();
    ...
    try {
      ...
      expect((await reader.read("plugins/acme.sample-plugin/runtime.js")).toString("utf-8")).toBe(
        "export default {};"
      );
    } finally {
      await reader.close();
    ...
    await expect(
      openSealedBundle(runtimeSupportPath(), path.join(result.appDir, RUNTIME_BUNDLE_FILENAME))
    ).rejects.toThrow();
  });
  ...
  it("writes a production app without a control channel or sibling userData", async () => {
    ...
    const manifest = JSON.parse(
      await fs.readFile(path.join(result.appDir, "package.json"), "utf-8")
    );
  ...
  it("marks preview app manifests with the preview mode", async () => {
    ...
    const result = await compileGameRuntimeArtifact(
      previewCompileInput(projectPath, runtimeDistDir, 47326)
    );
  ...
  it("rejects a runtime dist without a build manifest", async () => {
    ...
    await expect(
      compileGameRuntimeArtifact(previewCompileInput(projectPath, runtimeDistDir, 47328))
  ...
  it("rejects a runtime dist whose build manifest is not production", async () => {
    ...
    await expect(
      compileGameRuntimeArtifact(previewCompileInput(projectPath, runtimeDistDir, 47329))
  ...
  it("rejects mode/control-channel mismatches", async () => {
    ...
    await expect(compileGameRuntimeArtifact({ ...base, mode: "production" })).rejects.toThrow(
      /must not carry a preview control channel/
    );
    await expect(compileGameRuntimeArtifact({ ...base, preview: undefined })).rejects.toThrow(
      /requires a preview control channel/
    );
  });
});
