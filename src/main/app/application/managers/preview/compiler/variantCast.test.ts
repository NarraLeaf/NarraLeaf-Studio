...
describe("planShippedCharacters", () => {
    it("drops a character no shipped byte names", () => {
        ...
        const result = planShippedCharacters(
            bundle({ storyLibrary: { ...bundle().storyLibrary!, documents: { s1: { line: `speaks as ${REN}` } } } as never }),
            null,
    ...
    it("does not let the name table vote for its own character", () => {
        ...
        const result = planShippedCharacters(
            bundle({
                storyLibrary: { ...bundle().storyLibrary!, documents: { s1: { line: `speaks as ${REN}` } } } as never,
                localization: {
                    sourceLocale: "en",
                    locales: ["en"],
    ...
    it("changes nothing when the whole cast is reachable", () => {
        const input = bundle({
            storyLibrary: { ...bundle().storyLibrary!, documents: { s1: { a: REN, b: TRAITOR } } } as never,
        });
        const notice = vi.fn();
    ...
    it("says nothing about a project with no characters", () => {
        const result = planShippedCharacters({ storyLibrary: { characters: [] } } as unknown as DevModeBundle, null);
