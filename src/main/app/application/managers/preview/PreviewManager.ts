...
import { currentGameBuildPlatform, normalizeGameBuildArch } from "@shared/types/gameBuild";
...
export class PreviewManager {
  ...
  public getStatus(projectPath?: string): PreviewStatus {
    ...
    return (
      [...this.sessions.values()].find((session) => session.status !== "idle")?.status ?? "idle"
    );
  }
  ...
  public launch(projectPath: string, entry: GameRuntimeLaunchEntry): Promise<PreviewStatus> {
    ...
    if (frozen) {
      ...
      emitWorkspaceConsoleLog(this.app, projectPath, {
        level: "error",
        source: "Preview",
        message
      });
  ...
  private async launchNow(
    ...
    try {
      ...
      if (pluginSelection.fallbackAll && pluginSelection.selected.length > 0) {
        this.emitVerbose(
          session,
          "project has no plugin dependency table; packaging every enabled runtime plugin"
        );
      }
      if (pluginSelection.skippedPluginIds.length > 0) {
        this.emitVerbose(
          session,
          `runtime plugins not packaged (unused by this project): ${pluginSelection.skippedPluginIds.join(", ")}`
        );
      }
      if (pluginSelection.selected.length > 0) {
        this.emitVerbose(
          session,
          `packaging runtime plugin(s): ${pluginSelection.selected.map((source) => source.manifest.id).join(", ")}`
        );
      }
      ...
      const artifact = await compileGameRuntimeArtifactInWorker(
        this.app,
        {
          projectPath: normalizedProjectPath,
          entry,
  ...
  private async requestRuntimeShutdown(session: PreviewSession): Promise<void> {
    ...
    for (;;) {
      ...
      const attempt = await sendShutdownCommand(
        session.controlPort,
  ...
  private scheduleRelaunch(session: PreviewSession, event: string, file: string): void {
    ...
    this.emitVerbose(
      session,
      `project file ${event}; scheduling relaunch: ${path.relative(session.projectPath, file)}`
    );
  ...
  private enqueue(
    projectPath: string,
    operation: () => Promise<PreviewStatus>
  ): Promise<PreviewStatus> {
    const key = this.projectKey(projectPath);
    const previous = this.operations.get(key) ?? Promise.resolve("idle" as PreviewStatus);
    const next = previous.catch(() => "error" as PreviewStatus).then(operation);
  ...
  private emitWorkspaceConsoleLog(
    session: PreviewSession,
      ...
      settle(
        payload.ok === true
          ...
          : {
              outcome: "refused",
              error: new Error(
                typeof payload.error === "string" ? payload.error : "shutdown rejected"
              )
            }
      );
    });
