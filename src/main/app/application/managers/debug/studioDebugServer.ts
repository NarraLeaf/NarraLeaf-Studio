...
export class StudioDebugServer {
    ...
    switch (path) {
      ...
      case "/windows":
        this.sendJson(res, 200, {
          ...
          buffered: this.buffer.listWindows()
        });
        return;
      case "/devtools":
  ...
  private findWorkspaceWindow(): AppWindow | null {
    return (
      this.app.windowManager
        .getWindows()
        .find(
          (window) => !window.isClosed() && window.getWindowType() === WindowAppType.Workspace
        ) ?? null
    );
  }
  ...
  private async consoleSnapshot(url: URL): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      return {
        available: false,
        reason: `eval-failed: ${String((error as Error)?.message ?? error)}`
      };
    }
  }
  ...
  private parseDevtoolsQuery(url: URL): DevtoolsConsoleQuery {
    ...
    const level =
      levelParam && (CONSOLE_LEVELS as readonly string[]).includes(levelParam)
