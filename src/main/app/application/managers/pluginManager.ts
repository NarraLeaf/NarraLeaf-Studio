...
export class PluginManager {
   ...
   */
  public async listRuntimePluginPackSources(): Promise<
    Array<{
      manifest: NormalizedPluginManifestV2;
      entry: string;
      entryPath: string;
  ...
  > {
    ...
    return Object.values(this.getRecords())
      .filter(
        (record) => this.toListItem(record).status === "enabled" && record.manifest.entries.runtime
      )
      .map((record) => {
      ...
      for (const entry of locales) {
        try {
          ...
          if (!this.isSameOrChild(filePath, root)) {
            console.warn(
              `[PluginManager] locale "${entry.code}" for ${record.manifest.id} escapes the package; skipped`
            );
            continue;
          }
          ...
          console.warn(
            `[PluginManager] failed to read locale "${entry.code}" for ${record.manifest.id}:`,
            error
          );
        }
      }
  ...
  private async listTargetPlugins(
    target: "studio" | "runtime"
  ): Promise<WorkspacePluginDescriptor[]> {
  ...
  public async installFromDirectory(
    ...
    if (!samePath) {
      await this.swapPluginDirectory(installPath, (staged) =>
        fs.cp(sourceDir, staged, { recursive: true })
      );
    }
  ...
  public async setPluginEnabled(pluginId: string, enabled: boolean): Promise<PluginListItem> {
    ...
    const next = {
      ...
      enabled,
      updatedAt: Date.now(),
      lastError: enabled ? null : record.lastError
    };
    this.saveRecord(next);
    return this.toListItem(next);
  }

  public async approvePlugin(
    pluginId: string,
    grant: PluginPermissionGrantResult | null
  ): Promise<PluginApproveResult> {
    await this.initialize();
    const record = this.getRecord(pluginId);
    if (!grant?.approved) {
  ...
  public async resolvePluginEntryFile(url: URL): Promise<string | null> {
    ...
    if (
      !record ||
      this.toListItem(record).status !== "enabled" ||
  ...
  private async scanInstalledPlugins(): Promise<void> {
    ...
    for (const entry of entries) {
      if (
        ...
        LEGACY_STAGING_DIR.test(entry.name)
      ) {
        continue;
      }
      const installPath = path.join(this.pluginsDir, entry.name);
      const existing = Object.values(records).find(
        (record) => path.resolve(record.installPath) === path.resolve(installPath)
      );
      try {
        ...
        const inheritsGrant =
          priorGrant !== null && isPermissionSubset(manifest.permissions, priorGrant);
        ...
        nextRecords[manifest.id] = {
          ...
          grantedPermissions: builtIn ? manifest.permissions : inheritsGrant ? priorGrant : null,
  ...
  private async replacePluginDirectory(sourcePath: string, installPath: string): Promise<void> {
    await this.swapPluginDirectory(installPath, (staged) =>
      this.copyDirectoryFromAsar(sourcePath, staged)
    );
  }
  ...
  private async discard(target: string): Promise<void> {
    await fs
      ...
      .catch((error) =>
        console.warn(`[PluginManager] Failed to remove stale staging directory ${target}:`, error)
      );
  }
   ...
   * proves.
   */
  private grantedPermissionsOf(
    record: PluginInstallRecord | undefined
  ): PluginInstallPermission[] | null {
