...
import type {
  SpellcheckRegistryEntry,
  SpellcheckRegistryIndex
} from "@shared/types/spellcheckRegistry";
...
export function normalizeDictionaryEntry(raw: unknown): SpellcheckRegistryEntry | null {
  ...
  const bytes =
    typeof record.bytes === "number" && Number.isFinite(record.bytes) && record.bytes >= 0
      ...
      : 0;
  const words =
    typeof record.words === "number" && Number.isFinite(record.words) && record.words >= 0
...
export async function fetchDictionaryIndex(url: string): Promise<SpellcheckRegistryIndex> {
  ...
  if (!response.ok) {
    throw new Error(
      `Dictionary registry request failed (${response.status} ${response.statusText})`
    );
  }
  let parsed: unknown;
  try {
