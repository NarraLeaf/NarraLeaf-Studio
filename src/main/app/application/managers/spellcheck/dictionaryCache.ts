...
export class DictionaryCache {
    ...
    public async readWords(code: string): Promise<string | null> {
        ...
        const expanded = await gunzipAsync(compressed, { maxOutputLength: SPELLCHECK_MAX_EXPANDED_BYTES });
    ...
    public async remove(code: string): Promise<boolean> {
        ...
        const existed = await this.exists(this.manifestPath(code)) || await this.exists(this.wordsPath(code));
    ...
    public async readManifest(code: string): Promise<DictionaryManifest | null> {
        ...
        try {
            const raw = JSON.parse(await fs.readFile(this.manifestPath(code), "utf-8")) as Partial<DictionaryManifest>;
