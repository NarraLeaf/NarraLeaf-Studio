...
describe("extractWords", () => {
  ...
  it("passes over what is not prose", () => {
    ...
    expect(extractWords("see https://exampel.com/pge now").map((entry) => entry.word)).toEqual([
      "see",
      "now"
    ]);
    expect(extractWords("mail alise@exampel.com please").map((entry) => entry.word)).toEqual([
      "mail",
      "please"
    ]);
  });
