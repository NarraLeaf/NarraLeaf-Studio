...
export class VcsManager extends Manager {
  ...
  private async sessionFor(
    projectPath: string
  ): Promise<{ session: VcsSession; backend: VcsBackend }> {
  ...
  private async commitWithKind(
    ...
    return this.serialize(projectPath, async () => {
      ...
      return backend.commitWorkingTree(
        {
          ...session.globals,
          identity: this.resolveIdentity(options.identity, session.remoteOrigin)
        },
        { message, kind }
      );
    });
  ...
  public async restoreRevision(
    ...
    return this.serialize(projectPath, async () => {
      ...
      const applied = await applyRevisionRestore({
        ...
        source: {
          read: (entry) =>
            backend.readEntryBytes(globals, session.store, session.repositoryId, entry)
        }
      });
  ...
  public async readRevisionDocuments(
    ...
    return this.serialize(projectPath, async () => {
      ...
      return backend.documentsAt(session.globals, session.store, session.repositoryId, revision, {
        paths: options.paths,
        accept: (entry) =>
          entry.size <= maxBytes &&
    ...
    return this.serialize(projectPath, async () => {
      ...
      return materializeRevisionSnapshot({
        ...
        source: {
          ...
          list: () =>
            backend.listFilesAt(session.globals, session.store, session.repositoryId, revision),
          read: (entry) =>
            backend.readEntryBytes(session.globals, session.store, session.repositoryId, entry)
        }
      });
    });
  }
  ...
  public async signIn(
    ...
    return this.serialize(projectPath, async () => {
      ...
      const others = this.storedServerSessions().filter(
        (stored) => stored.remoteOrigin !== signedIn.remoteOrigin
      );
      this.writeStoredServerSessions([...others, signedIn]);
      this.app.logger.info(
        "[Vcs] Signed in",
        signedIn.remoteOrigin,
        "at",
        signedIn.authUrl,
        "as",
        signedIn.account.username || signedIn.account.displayName
      );
  ...
  public async trustAuthority(
    ...
    this.app.logger.info(
      "[Vcs]",
      outcome.installed ? "Trusted" : "Failed to trust",
      "a server authority from",
      target
    );
    return outcome;
  }
   ...
   */
  public async addServer(options: {
    authUrl: string;
    remoteUrl: string;
    token: string;
    ...
    const servers = [
      ...this.storedServerSessions().filter(
        (stored) => stored.remoteOrigin !== signedIn.remoteOrigin
      ),
      signedIn
    ];
      ...
      await backend
        ...
        .catch((error: unknown) => {
          this.app.logger.warn(
            "[Vcs] Removed",
            stored.remoteOrigin,
            "without clearing its token:",
            error instanceof Error ? error.message : String(error)
          );
        });
    }

    const servers = this.storedServerSessions().filter(
      (other) => other.remoteOrigin !== stored.remoteOrigin
    );
    this.writeStoredServerSessions(servers);
  ...
  private async confirmStoredSession(
    ...
    this.app.logger.info(
      "[Vcs] The stored sign-in for",
      stored.remoteOrigin,
      "is no longer held by the backend"
    );
    return null;
  }
  ...
  public async completeMerge(
    ...
    return this.serialize(projectPath, async () => {
      if (this.flushPendingSaves) {
        await this.flushPendingSaves(projectPath).catch((error) => {
          this.app.logger.warn(
            "[Vcs] Could not flush pending saves before completing a merge",
            error
          );
        });
      }
  ...
  public async restartConflicts(
    ...
    return this.serialize(projectPath, async () => {
      if (this.flushPendingSaves) {
        await this.flushPendingSaves(projectPath).catch((error) => {
          this.app.logger.warn(
            "[Vcs] Could not flush pending saves before restarting a merge",
            error
          );
        });
      }
  ...
  public async abortMerge(projectPath: string): Promise<VcsMergeState> {
    return this.serialize(projectPath, async () => {
      if (this.flushPendingSaves) {
        await this.flushPendingSaves(projectPath).catch((error) => {
          this.app.logger.warn(
            "[Vcs] Could not flush pending saves before abandoning a merge",
            error
          );
        });
      }
  ...
  public async dispose(): Promise<void> {
    ...
    await Promise.all(
      [...keys].map((key) =>
        this.closeProject(key).catch((error) => {
