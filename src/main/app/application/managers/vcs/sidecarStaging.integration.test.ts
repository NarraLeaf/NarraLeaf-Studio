...
describe.skipIf(!supported)("merge sidecars and staging", () => {
  it("does not carry the sidecars into a commit that staged the whole tree first", async () => {
    ...
    const sidecars = ["~base", "~mine", "~theirs"].filter((suffix) =>
      fs.existsSync(path.join(root, `${DOCUMENT}${suffix}`))
    );
    expect(sidecars, "the merge did not leave sidecars, so this test proves nothing").toHaveLength(
      3
    );
    ...
    const stagedSidecars = staged.files
      .map((file) => file.path)
    ...
    const committed = (
      await listFilesAt(globals, store, created.repository, revision.revision)
