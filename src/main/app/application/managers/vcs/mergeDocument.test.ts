...
function conflictedLocale(root: string, options: { base?: boolean } = {}): void {
  ...
  write(
    root,
    `${LOCALE_PATH}~mine`,
    locale({ greeting: unit("mine"), fromMine: unit("only mine") })
  );
  write(
    root,
    `${LOCALE_PATH}~theirs`,
    locale({ greeting: unit("theirs"), fromTheirs: unit("only theirs") })
  );
}
...
describe("readMergeDocument", () => {
  ...
  it("falls back to tier one rather than truncating a huge decision list", async () => {
    ...
    const many = (prefix: string) =>
      Object.fromEntries(
        Array.from({ length: MERGE_DECISION_LIMIT + 1 }, (_, index) => [
          `${prefix}${index}`,
          unit("x")
        ])
      );
