...
import { listAuthSessions, loginWithToken, logoutFromRemote, type LoreGlobals } from "./lore";
...
export class VcsSignInError extends Error {
  constructor(
    readonly problem: VcsSignInProblem,
    message: string
  ) {
    super(message);
...
export function readSignInToken(token: string): VcsSignInToken {
  ...
  const fingerprint =
    typeof claims.authority_sha256 === "string" ? claims.authority_sha256.trim() : "";
...
export async function diagnoseEndpoint(
  ...
  return new Promise<EndpointVerdict>((resolve) => {
    ...
    socket.setTimeout(timeoutMs, () =>
      done({ kind: "unreachable", detail: `${host}:${port} did not answer` })
    );
    socket.on("error", (error: NodeJS.ErrnoException) => {
      ...
      // question - the endpoint is there, it simply would not agree with us.
        ...
        error.code &&
          /ECONN|ENOTFOUND|EHOSTUNREACH|ENETUNREACH|ETIMEDOUT|EAI_AGAIN/.test(error.code)
