...
describe("reading one file out of the working tree", () => {
  ...
  it("accepts either separator, because a caller on Windows has both spellings", async () => {
    ...
    await expect(readWorkingSetFile(project, "editor\\story\\index.json")).resolves.toEqual(
      Buffer.from("{}")
    );
  });
});
...
describe("what it refuses", () => {
  it("refuses a path that leaves the project directory", async () => {
    ...
    expect((await refusalOf(path.join(path.dirname(project), "outside.png"))).refusal).toBe(
      "escapes"
    );
  });
  ...
  it("defaults the ceiling to the one the comparison surface budgets against", async () => {
    ...
    await expect(readWorkingSetFile(project, "assets/content/ab/cd/small")).resolves.toHaveLength(
      16
    );
  });
});
