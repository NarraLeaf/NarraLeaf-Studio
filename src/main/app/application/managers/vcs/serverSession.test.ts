...
describe("reading the addresses out of a token", () => {
  ...
  it("carries the fingerprint the server signs with, when the token names one", () => {
    const fingerprint =
      "3D:38:9F:E6:12:C3:14:F0:C5:28:53:41:39:06:DC:E9:B6:0A:7A:EA:F1:FF:D0:3C:B2:4C:F8:71:01:46:DC:48";
    const read = readSignInToken(
      tokenWith({ ...ACCOUNT, aud: AUDIENCE, authority_sha256: fingerprint })
    );
  ...
  it("reads a single-valued audience, which is legal even though a Team server writes an array", () => {
    expect(readSignInToken(tokenWith({ ...ACCOUNT, aud: "https://one.example.lan" })).authUrl).toBe(
      "https://one.example.lan"
    );
  });
...
describe("the install plan", () => {
  const certificate =
    process.platform === "win32" ? "C:\\Users\\a b\\team.crt" : "/home/a b/team.crt";
