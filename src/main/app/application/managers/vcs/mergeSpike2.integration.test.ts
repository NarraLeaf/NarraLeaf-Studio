...
describe.skipIf(!supported)("L0 - what a conflicted merge leaves behind", () => {
  it("describes the sidecars, every status form, and both branches' graphs", async () => {
    ...
    try {
      ...
      observations.mergeStart = await observe(() =>
        branchMergeStart(fixture.globals, { branch: "feature" })
      );
      ...
      observations.status = {
        ...
        scanCheckDirty: await observe(async () =>
          statusFlags(
            (await repositoryStatus(fixture.globals, { scan: true, checkDirty: true })).files
          )
        ),
        noScan: await observe(async () =>
          statusFlags((await repositoryStatus(fixture.globals, { scan: false })).files)
        ),
        paths: await observe(async () =>
          statusFlags(
            (
              await repositoryStatus(fixture.globals, {
                scan: true,
                checkDirty: true,
      ...
      observations.graphs = await observe(async () => {
        ...
        const describe = (nodes: ReadonlyMap<string, { number: number }>) =>
          [...nodes.entries()]
            .map(([revision, node]) => `#${node.number} ${revision.slice(0, 8)}`)
            .sort();
        return {
          ...
          baseFromCurrentGraph:
            mergeBase(current, fixture.mineRevision as string, fixture.theirsRevision as string) ??
            null,
          baseFromUnionGraph:
            mergeBase(union, fixture.mineRevision as string, fixture.theirsRevision as string) ??
            null,
          actualBaseRevision: fixture.baseRevision
        };
      });

      observations.threeWay = await observe(() =>
        withStore(fixture.globals, fixture.root, async (store) => {
...
async function resolveExperiment(
  ...
  observations.afterResolve = {
    ...
    sidecarsStillPresent: ["~base", "~mine", "~theirs"].filter((suffix) =>
      fs.existsSync(`${fixture.document}${suffix}`)
    )
  };
  ...
  observations.committedTree = await observe(() =>
    withStore(fixture.globals, fixture.root, async (store) => {
      const files = await listFilesAt(
        fixture.globals,
        store,
        fixture.repositoryId,
        committed.revision
      );
      const bytes = await blobAt(
        fixture.globals,
        store,
        fixture.repositoryId,
        committed.revision,
...
describe.skipIf(!remoteEnabled)("R1 - resolving a sync conflict", () => {
  it("names every event the sync emits, then resolves with hand-written bytes", async () => {
    ...
    try {
      ...
      observations.filesAddedBySync = Object.keys(after)
        .filter((c) => !(c in before))
        .sort();
      observations.sidecars = Object.fromEntries(
        ["", "~base", "~mine", "~theirs"].map((suffix) => [
          `${DOCUMENT}${suffix}`,
          describeFile(`${document}${suffix}`)
        ])
      );
      ...
      observations.status = {
        scanCheckDirty: await observe(async () =>
          statusFlags(
            (await repositoryStatus(authorOnline, { scan: true, checkDirty: true })).files
          )
        )
      };

      fs.writeFileSync(document, HAND_TEXT, "utf-8");
      observations.resolveResult = await observe(() =>
        branchMergeResolve(authorOnline, [document])
      );
      ...
      observations.readMergeRevisionOffline = await observe(() =>
        readWith(authorOffline, committed.revision)
      );
      observations.readMergeRevisionOnline = await observe(() =>
        readWith(authorOnline, committed.revision)
      );
...
describe.skipIf(!remoteEnabled)("R2 - blob reads on a connected repository", () => {
  it("isolates which step of connecting a server breaks blobAt", async () => {
    ...
    try {
        ...
        return observe(async () => {
          ...
          const store =
            open === "wrapper"
              ...
              : {
                  handleId: (
                    await invoke("storageOpen", globals, {
          ...
          try {
            ...
            return {
              read: "ok",
              byteLength: bytes.byteLength,
              matchesBase: bytes.equals(Buffer.from(BASE_TEXT, "utf-8"))
            };
          } finally {
            await flushRepository(globals).catch(() => undefined);
            await closeStore(globals, store).catch(() => undefined);
       ...
       */
      const syncThenRead = async (name: string, conflicting: boolean, storeRemote: boolean) =>
        observe(async () => {
          const root = tmp(`nl-m2-r2-${name}-`);
          const globals = offline(root);
          const onlineGlobals = online(root);
          ...
          try {
            ...
            const entries = await observe(() =>
              listFilesAt(globals, store, created.repository, mine)
            );
            ...
            return {
              ...
              walkRoute: {
                ...
                read: entry
                  ? await observe(async () => ({
                      byteLength: (await readEntryBytes(globals, store, created.repository, entry))
                        .byteLength
                    }))
                  : { error: "no entry" }
              },
      ...
      observations.f_afterSyncConflict = await observe(async () => {
        ...
        return withStore(globals, root, async (store) => {
          ...
          const atMerge = await observe(async () => ({
            byteLength: (
              await blobAt(globals, store, created.repository, merged.revision, DOCUMENT)
            ).byteLength
          }));
