...
import { commit, createRepository, flushRepository, releaseRepository, stage } from "./lore";
import {
  cloneInto,
  publishToRemote,
  pushToRemote,
...
describe.skipIf(!enabled)("remote", () => {
  ...
  it("clones a project and carries changes back and forth", async () => {
    const { root: authorRoot, repositoryId } = await studioProject(
      "nl-remote-author-",
      "hello.txt",
      "one"
    );
  ...
  it("refuses to clone into a folder that is not empty", async () => {
    ...
    await expect(cloneInto(online(occupied), { repositoryUrl: serverUrl("nope") })).rejects.toThrow(
      /not empty/i
    );
  ...
  it("refuses a diverged push, and syncing merges instead", async () => {
    const { root: aRoot, repositoryId } = await studioProject(
      "nl-remote-diva-",
      "base.txt",
      "base"
    );
  ...
  it("disconnects without disturbing the rest of the backend's config", async () => {
    ...
    expect(disconnected.replace(/^.*remote_url.*$/m, "")).toBe(
      connected.replace(/^.*remote_url.*$/m, "")
    );
