...
describe.skipIf(!supported)("revisionReader", () => {
  ...
  it("lists changed paths between two revisions", async () => {
    ...
    expect(paths.some((candidate) => candidate.replace(/\\/g, "/").endsWith("sprite.bin"))).toBe(
      true
    );
  }, 60_000);
  ...
  it("answers null for a path the revision does not contain, while the per-path read throws", async () => {
    const later = await commitBytes(
      "assets/added-in-a-later-revision.bin",
      Buffer.from("later"),
      "add"
    );
    ...
    await expect(
      blobAt(globals, store, repositoryId, rev1, "assets/added-in-a-later-revision.bin")
    ).rejects.toThrow();
  }, 180_000);
  ...
  it("reports an absent base rather than an empty one for a file added on one side", async () => {
    ...
    const older = await threeWay(globals, store, repositoryId, head, rev1, ONLY_IN_HEAD).catch(
      () => null
    );
...
describe.skipIf(!supported)("what a content address means", () => {
  ...
  async function walkFreshRepository(
    ...
    try {
      const entries = await listFilesAt(
        localGlobals,
        localStore,
        created.repository,
        revision.revision
