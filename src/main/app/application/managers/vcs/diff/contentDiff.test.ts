...
function rows(
  ...
  return diffDocumentContent({ path, contentClass: contentClassOf(path), base: a, head: b })
    .changes;
}
...
describe("bitmaps", () => {
  it("puts a resolution change in a row of its own", () => {
    ...
    const changes = rows(
      "assets/content/face.png",
      side(2048, png(512, 512)),
      side(4096, png(256, 256))
    );
...
describe("sound", () => {
  it("reports the length and the sample rate", () => {
    const changes = rows(
      "assets/content/bgm.mp3",
      side(3_000, mp3(0, 1000)),
      side(4_000, mp3(1, 2000))
    );
...
describe("fonts", () => {
  it("carries the family as the author's own word", () => {
    ...
    const changes = rows(
      "assets/content/ui.ttf",
      side(400, font("Inter")),
      side(420, font("Noto Sans"))
    );
...
describe("formats nobody here reads", () => {
  it("tells an unrecognised format from an uninspected one", () => {
    expect(keys(rows("a/b.qqq", side(10, undefined, "x"), side(20, undefined, "y")))).toEqual([
      "documentDiff.content.unrecognized",
      "documentDiff.content.size"
    ]);
    expect(keys(rows("a/b.txt", side(10), side(20)))).toEqual([
      "documentDiff.content.notInspected",
      "documentDiff.content.size"
    ]);
  });

  it("says a model binary changed and nothing else, ever", () => {
    expect(keys(rows("assets/content/hiyori/Hiyori.moc3", side(1000), side(2000)))).toEqual([
      "documentDiff.content.size"
    ]);
  });
});
...
describe("deciding two files hold the same bytes", () => {
  const probe = (size: number, hash?: string): ContentProbe => ({
    size,
    ...(hash ? { hash } : {})
  });
  ...
  it("pairs a removal with the addition holding the same bytes", () => {
    const pairs = pairMoves(
      new Map([
        ["old/a.png", probe(10, "h1")],
        ["old/b.png", probe(20, "h2")]
      ]),
      new Map([
        ["new/a.png", probe(10, "h1")],
        ["new/b.png", probe(20, "h2")]
      ])
    );
