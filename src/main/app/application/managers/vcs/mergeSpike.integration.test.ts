...
const EARLY_KEY_EDIT = edited((document) => {
  document.characters.alice.displayName = "Alicia";
});
const LATE_KEY_EDIT = edited((document) => {
  document.version = 8;
});

/** E1b, E5, E6: both sides on the same key, which is the case that must conflict. */
const SAME_KEY_MINE = edited((document) => {
  document.title = "Prologue (main)";
});
const SAME_KEY_THEIRS = edited((document) => {
  document.title = "Prologue (feature)";
});

/** E3, E6: an answer neither side wrote, which is what a per-change resolution produces. */
const HAND_RESOLVED = edited((document) => {
  document.title = "Prologue (resolved by hand)";
});
...
async function twoSidedRepository(prefix: string, mine: string, theirs: string): Promise<TwoSided> {
  ...
  try {
    ...
    fixture.branches = (await listBranches(globals)).map(
      (branch) => `${branch.name}${branch.isCurrent ? " (current)" : ""}`
    );
  } catch (error) {
...
describe.skipIf(!supported)("E2 - conflicted working-tree bytes", () => {
  it("compares the working tree against base, mine and theirs", async () => {
    ...
    if (!fixture || fixture.setupError || !fixture.mineRevision || !fixture.theirsRevision) {
      report("E2 WHOSE BYTES", {
        skipped: "E1b left no conflicted repository",
        setup: fixture?.setupError
      });
    ...
    observations.workingTree = {
      exists: Boolean(working),
      byteLength: working?.byteLength ?? 0,
      sha256: workingHash
    };
...
describe.skipIf(!remoteEnabled)("E6 - conflict through revisionSync", () => {
  ...
  it("reports what a diverged sync leaves behind, and whether the resolve verbs apply to it", async () => {
    ...
    observations.sync = failed(synced)
      ? synced
      : {
    ...
    observations.resolveTheirs = await observe(() =>
      branchMergeResolveTheirs(authorOnline, [document])
    );
    observations.resolveMine = await observe(() =>
      branchMergeResolveMine(authorOnline, [document])
    );

    const confirm = await observe(() =>
      repositoryStatus(authorOnline, { scan: false, revisionOnly: true })
    );
    observations.beforeCommit = failed(confirm)
      ? confirm
      : {
    ...
    if (committed) {
      ...
      observations.mergeRevision = failed(graph)
        ? graph
        : {
