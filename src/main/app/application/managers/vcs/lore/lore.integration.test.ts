...
describe.skipIf(!supported)("lore binding: library", () => {
  ...
  it("redirects an asar path to the unpacked copy", () => {
    ...
    expect(unpackAsarPath("/app/Resources/app.asar/node_modules/@lore-vcs/x/lib.dylib")).toBe(
      "/app/Resources/app.asar.unpacked/node_modules/@lore-vcs/x/lib.dylib"
    );
    expect(unpackAsarPath("/plain/node_modules/@lore-vcs/x/lib.dylib")).toBe(
      "/plain/node_modules/@lore-vcs/x/lib.dylib"
    );
  });
});
...
describe.skipIf(!supported)("lore binding: identifier encoding", () => {
  ...
  it("refuses a revision hash of the wrong width at the call boundary", async () => {
    await expect(loadTree(globals, store, repositoryId, "deadbeef")).rejects.toBeInstanceOf(
      LoreValueError
    );
  ...
  it("refuses a path that escapes the repository", () => {
    ...
    expect(() => repositoryPath(root, "editor/../../outside.txt")).toThrow(
