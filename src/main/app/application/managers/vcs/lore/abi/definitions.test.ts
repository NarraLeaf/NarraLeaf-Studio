...
const upstreamPrototypes = upstream.prototypes as Record<
  string,
  { returns: string; args: string[] }
>;
...
describe("Lore ABI", () => {
  ...
  it("binds only verbs the header declares, with the expected signature", () => {
    for (const [verb, { symbol, args }] of Object.entries(LORE_VERBS)) {
      ...
      expect(signature.args, symbol).toEqual([
        "LoreGlobalArgs*",
        `${args}*`,
  ...
  it("declares every struct its own fields reference", () => {
    ...
    for (const [structName, fields] of Object.entries(LORE_STRUCTS)) {
      for (const [field, type] of Object.entries(fields)) {
        ...
        expect(known.has(base), `${structName}.${field} references unknown type ${base}`).toBe(
          true
        );
      }
    }
