...
export interface LorePathIgnorePayload {
  path: string;
}
export interface LoreFilterExcludePayload {
  reason: number;
  path: string;
}
export interface LoreRepositoryCreatePayload {
  repository: LoreHex;
  name: string;
  path: string;
}
...
export interface LoreStatusSummaryPayload {
  adds: number;
  deletes: number;
  modifies: number;
  moves: number;
...
export interface LoreConfigPayload {
  key: string;
  value: string;
}
...
export interface LoreCloneBeginPayload {
  repository: LoreHex;
  branch: string;
  revision?: LoreHex;
  path: string;
}
export interface LoreCloneProgressPayload {
  count: LoreCloneCountPayload;
}
...
export interface LoreStageFilePayload {
  path: string;
  fromPath: string;
  action: number;
}
export interface LoreStageEndPayload {
  totalCount: number;
  fileAddCount: number;
...
export interface LoreHistoryPayload {
  repository: LoreHex;
  branch: LoreHex;
}
...
export interface LoreRevisionInfoPayload {
  repository: LoreHex;
  revision: LoreHex;
  revisionNumber: number;
  parents: LoreHex[];
}
export interface LoreDiffFilePayload {
  path: string;
  action: number;
  oldIsFile: boolean;
  newIsFile: boolean;
}
export interface LoreRestoreRevisionPayload {
  revision: LoreHex;
  revisionNumber: number;
}
export interface LoreRestoreFilePayload {
  path: string;
  action: number;
  size: number;
  isFile: boolean;
...
export interface LoreStorageHeaderPayload {
  id: number;
  hash: LoreHex;
  context: LoreHex;
  sizeContent: number;
...
export interface LoreTreeResolvePayload {
  id: number;
  nodeId: number;
  repository: LoreHex;
  revision: LoreHex;
...
export interface LoreTreeCloseCompletePayload {
  id: number;
  errorCode: number;
}
...
export interface LoreBranchCreatePayload {
  name: string;
  latest?: LoreHex;
  isCommit: boolean;
}
export interface LoreLogPayload {
  level: number;
  message: string;
  location: string;
}
...
export interface LoreMergeStartBeginPayload {
  branch: LoreHex;
  revision?: LoreHex;
  revisionNumber: number;
}
...
export interface LoreMergeConflictFilePayload {
  path: string;
}
export interface LoreMergeResolveFilePayload {
  path: string;
}
export interface LoreMergeResolveRevisionPayload {
  repository: LoreHex;
  revision?: LoreHex;
}
 ...
 */
export interface LoreMergeAbortBeginPayload {
  stateStagedRevision?: LoreHex;
  stateCurrentRevision?: LoreHex;
}

/** The abort finished. Carries no data; decoded so its arrival is observable at all. */
export interface LoreMergeAbortEndPayload {
  unused: number;
}

export interface LoreMergeUnresolveFilePayload {
  path: string;
}
...
function decodeParents(raw: unknown, field: string): LoreHex[] {
  const slots = ((raw as Record<string, unknown>)[field] ?? []) as Array<{
...
function decoderTable(library: LoreLibrary): Map<number, Decoder> {
  ...
  table.set(LoreTag.COMPLETE, (raw): LoreCompletePayload => {
    ...
    if (locations?.ptr && count > 0) {
      ...
      const entries = koffi.decode(locations.ptr, type("LoreTraceLocation"), count) as Array<
  ...
  table.set(LoreTag.METADATA, (raw, pointer): LoreMetadataPayload => {
    ...
    if (tag === LORE_METADATA_TAGS.NUMERIC) {
      return {
        key,
        tag,
        numeric: decodeCount(koffi.decode(pointer, metadataValueOffset, "uint64_t"))
      };
    }
    return { key, tag };
  });
