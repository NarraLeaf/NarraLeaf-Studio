...
describe.skipIf(!supported || PERSIST === "")("R3 - does a fresh process recover after a sync", () => {
    it("builds a synced repository, or reads one a previous process left behind", async () => {
        ...
        try {
            ...
            const readWith = async (globals: LoreGlobals) => observe(async () => {
                const store = await openStore(globals, root);
                try {
                    ...
                    return {
                        ...
                        bytes: await observe(async () => ({
                            byteLength: (await blobAt(globals, store, repositoryId, revision, DOCUMENT)).byteLength,
                        })),
                    };
                } finally {
                    await flushRepository(globals).catch(() => undefined);
            ...
            const readWithRemoteStore = await observe(async () => {
                ...
                const opened = await invoke("storageOpen", globals, {
                    ...
                    inMemory: 0,
                    remoteConfig: { remoteUrl: loreString(typeof remoteUrl === "string" ? remoteUrl : undefined) },
                    hasRemoteConfig: 1,
                    cacheTargetBytes: 0,
