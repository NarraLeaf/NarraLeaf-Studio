...
export class PluginPermissionManager {
  ...
  constructor(userDataDir: string) {
    const dbPath = path.join(
      userDataDir,
      UserDataNamespace.Authorization,
      "plugin-permissions.config"
    );
  ...
  public isPluginFileSystemAllowed(
    pluginId: string,
  ...
  public isPluginCapabilityAllowed(
    pluginId: string,
    version: string | undefined,
    capability: string
  ): boolean {
    return this.getApiGrants({ id: pluginId, version }).some(
  ...
  public getExistingGrantResult(
    request: PluginPermissionRequest
  ): PluginPermissionGrantResult | null {
    switch (request.kind) {
      case "trust": {
      ...
      case "api": {
        const grant = this.getApiGrants(request.plugin).find(
          (record) => record.capability === request.capability
        );
        return grant ? this.grantRecordToResult(request, grant.persistence, grant.grantedAt) : null;
      }
      case "install":
  ...
  private getMatchingFileSystemGrants(
    ...
    const target = path.resolve(fsPath);
    const grants = this.getFileSystemGrants(plugin).filter((grant) =>
      this.fileSystemPathAllows(grant, target, recursive)
    );
  ...
  private fileSystemModeAllows(
    grantMode: PluginFileSystemPermissionMode,
