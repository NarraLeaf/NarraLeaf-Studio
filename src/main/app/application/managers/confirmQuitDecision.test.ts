...
describe("decideQuitAction", () => {
    ...
    it("does not let a leaned-on ⌘Q become the second press", () => {
        expect(decideQuitAction(press({ key: "q", meta: true, isAutoRepeat: true }), PENDING)).toBe("swallow");
        expect(decideQuitAction(press({ key: "q", meta: true, isAutoRepeat: true }), IDLE)).toBe("swallow");
    });

    it("leaves ⌘Q alone when the preference is off, so the menu quits as it always did", () => {
        expect(decideQuitAction(press({ key: "q", meta: true }), { enabled: false, pending: false }))
            .toBe("ignore");
    });
     ...
     */
    it.each(["Meta", "Shift", "Alt", "Control", "CapsLock"])("survives %s between the two presses", (key) => {
