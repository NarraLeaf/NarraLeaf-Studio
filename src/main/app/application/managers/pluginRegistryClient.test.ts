...
} from "../../../buildWorker/mobile/zipWriter";
import { extractPluginZip, fetchRegistryIndex } from "./pluginRegistryClient";

const OPTIONS: ZipWriteOptions = {
  mtime: new Date(Date.UTC(2020, 0, 1, 12, 0, 0)),
  allowZip64: false
};
...
async function freshTempDir(): Promise<string> {
  const dir = path.join(
    os.tmpdir(),
    `nls-registry-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
  tempDirs.push(dir);
  return dir;
}
...
describe("extractPluginZip", () => {
  it("unpacks a nested plugin package and returns the manifest directory", async () => {
    ...
    expect(await fs.readFile(path.join(manifestDir, "manifest.json"), "utf-8")).toBe(
      '{"id":"acme.demo"}'
    );
  ...
  it("rejects an entry that escapes the extract directory (zip-slip)", async () => {
    ...
    const buffer = Buffer.from(
      built.toString("latin1").replaceAll("PH/escape.txt", "../escape.txt"),
      "latin1"
    );
    const dest = await freshTempDir();
