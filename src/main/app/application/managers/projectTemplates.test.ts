...
describe("listProjectTemplates", () => {
  ...
  it("reads a manifest, including its per-locale wording", async () => {
    await writeFile(
      path.join(templatesDir, "skeleton", "template.json"),
      JSON.stringify({
        name: "Skeleton",
  ...
  it("drops a malformed template rather than the whole list", async () => {
    ...
    await writeFile(
      path.join(templatesDir, "nameless", "template.json"),
      JSON.stringify({ version: "1" })
    );
    await writeFile(
      path.join(templatesDir, "good", "template.json"),
...
describe("scaffoldProjectFromTemplate", () => {
  it("copies the content tree over the project, nested directories included", async () => {
    await writeFile(
      path.join(templatesDir, "skeleton", "template.json"),
      JSON.stringify({ name: "Skeleton" })
    );
    await writeFile(
      path.join(templatesDir, "skeleton", "content", "editor", "ui", "uidoc.json"),
      '{"authored":true}'
    );
    await writeFile(
      path.join(templatesDir, "skeleton", "content", "assets", "content", "ab", "pic.png"),
      "PNG"
    );
    ...
    expect(await fs.readFile(path.join(projectDir, "editor", "ui", "uidoc.json"), "utf-8")).toBe(
      '{"authored":true}'
    );
    expect(
      await fs.readFile(path.join(projectDir, "assets", "content", "ab", "pic.png"), "utf-8")
    ).toBe("PNG");
  });
  ...
  it("never copies a project config, which is generated per project", async () => {
    ...
    await writeFile(
      path.join(templatesDir, "skeleton", "content", "Template.nlproj"),
      "template config"
    );
    ...
    expect(await fs.readFile(path.join(projectDir, "MyGame.nlproj"), "utf-8")).toBe(
      "the author's own"
    );
  });
  ...
  it("produces a plain project for a manifest-only template", async () => {
    await writeFile(
      path.join(templatesDir, "meta", "template.json"),
      JSON.stringify({ name: "Meta" })
    );

    expect(await scaffoldProjectFromTemplate(templatesDir, "meta", projectDir)).toEqual({
      filesCopied: 0
    });
  });
  ...
  it("refuses an id that would read outside the templates directory", async () => {
    ...
    await expect(
      scaffoldProjectFromTemplate(templatesDir, "../secrets", projectDir)
    ).rejects.toThrow(/Unsafe project template id/);
    await expect(scaffoldProjectFromTemplate(templatesDir, "..", projectDir)).rejects.toThrow(
      /Unsafe project template id/
    );
  });
});
