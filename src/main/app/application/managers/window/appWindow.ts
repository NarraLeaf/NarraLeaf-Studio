...
import {
  ...
  applyZoomFactorToWebContents,
  windowTypeUsesZoom
} from "@/app/application/zoom";
import {
...
export class AppWindow<T extends WindowAppType = any> extends WindowProxy {
  ...
  public onEvent<Request, Response>(
    event: string,
    fn: (payload: Request) => Promise<Response> | Response
  ) {
  ...
  public onKeyUp(
    ...
    fn: (event: Electron.Event, input: Electron.Input) => void
  ): AppEventToken {
  ...
  public applyStoredZoom(): void {
    ...
    try {
      ...
      this.getApp().logger.debug(
        `[Zoom] Failed to apply zoom to ${this.getWindowType()}: ${String(error)}`
      );
    }
  }
  ...
  private prepareEvents(): void {
    ...
    webContents.on("will-frame-navigate", (event) => {
      ...
      this.getApp().logger.warn(
        `[Window] Blocked navigation for ${this.getWindowType()}: ${event.url} (${decision.reason})`
      );
    });
    ...
    webContents.on("did-create-window", (child, details) => {
      ...
      if (!decision.allowed) {
        ...
        this.getApp().logger.error(
          `[Window] Detached window arrived unkeyed: ${details.frameName}`
        );
        child.destroy();
        return;
      }

      this.getApp().logger.info(
        `[Window] Detached window opened from ${this.getWindowType()}: ${details.frameName}`
      );
    ...
    webContents.on("will-attach-webview", (event, _webPreferences, params) => {
      event.preventDefault();
      this.getApp().logger.warn(
        `[Window] Blocked webview attachment for ${this.getWindowType()}: ${params.src ?? ""}`
      );
    });
  ...
  private async offerReloadForCrash(reason: string): Promise<void> {
    ...
    const subject = describeWindowSubject(
      (this.getProps() as { projectPath?: string } | undefined)?.projectPath
    );
    ...
    if (!looping && choice === 0) {
      this.getApp().logger.info(
        `[Crash] Reloading the ${this.getWindowType()} window after a renderer exit`
      );
  ...
  private async offerReloadForHang(): Promise<void> {
    ...
    try {
      ...
      if (answer.response === 1 && !win.isDestroyed()) {
        this.getApp().logger.info(
          `[Crash] Reloading the ${this.getWindowType()} window after a hang`
        );
