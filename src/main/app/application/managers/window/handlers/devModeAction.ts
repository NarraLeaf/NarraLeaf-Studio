...
export class DevModeLaunchHandler extends IPCHandler<IPCEventType.devModeLaunch> {
  ...
  public async handle(
    ...
    RequestStatus<{ status: IPCEvents[IPCEventType.devModeLaunch]["response"]["status"] }>
  > {
...
export class DevModeReloadHandler extends IPCHandler<IPCEventType.devModeReload> {
  ...
  public async handle(
    ...
    RequestStatus<{ status: IPCEvents[IPCEventType.devModeReload]["response"]["status"] }>
  > {
    return this.tryUse(async () => {
...
async function resolveDevModeAssetUrl(
  ...
  try {
    ...
    return {
      success: true,
      data: { url: await promoteDevModeAssetGrant(window, workspaceResult.data.url) }
    };
  } catch (error) {
 ...
 */
async function promoteDevModeAssetGrant(
  window: AppWindow<WindowAppType.DevMode>,
  url: string
): Promise<string> {
  let parsed: URL;
...
export class DevModeOpenBlueprintInWorkspaceHandler extends IPCHandler<IPCEventType.devModeOpenBlueprintInWorkspace> {
  ...
  public async handle(
    ...
    if (
      data.ownerKind !== "surfaceMain" &&
      data.ownerKind !== "widgetMain" &&
      data.ownerKind !== "widgetValue"
    ) {
