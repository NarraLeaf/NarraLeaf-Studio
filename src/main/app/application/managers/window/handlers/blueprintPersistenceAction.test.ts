...
class MemoryPersistentState {
  ...
  private ensureValidKey(key: string): void {
    ...
    if (!keyPattern.test(key) || key.length === 0) {
      throw new Error(
        `Invalid key: "${key}". Keys must contain only English letters, numbers, and dots.`
      );
    }
  }
}
...
describe("blueprint persistence IPC handlers", () => {
  it("isolates values by project identifier and removes saved values", () => {
    ...
    const projectA: BlueprintPersistenceProjectRef = {
      projectIdentifier: "project-a",
      projectPath: "/tmp/project"
    };
    const projectB: BlueprintPersistenceProjectRef = {
      projectIdentifier: "project-b",
      projectPath: "/tmp/project"
    };

    expect(
      set.handle(window, { projectRef: projectA, key: "settings.volume", value: 0.8 }).success
    ).toBe(true);
    ...
    expect(remove.handle(window, { projectRef: projectA, key: "settings.volume" }).success).toBe(
      true
    );
    expect(get.handle(window, { projectRef: projectA, key: "settings.volume" })).toEqual({
      success: true,
  ...
  it("uses project-path fallback namespaces and rejects invalid keys", () => {
    ...
    expect(set.handle(window, { projectRef: firstPath, key: "flag", value: true }).success).toBe(
      true
    );
    ...
    expect(
      set.handle(window, { projectRef: firstPath, key: "bad/key", value: true })
    ).toMatchObject({
      success: false
    });
  });
