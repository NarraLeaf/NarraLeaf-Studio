...
import {
  ...
  pruneBackgroundCache
} from "../../storage/backgroundCache";
...
export class AppReportRendererErrorHandler extends IPCHandler<IPCEventType.appReportRendererError> {
  ...
  public handle(window: AppWindow, data: IPCEvents[IPCEventType.appReportRendererError]["data"]) {
    ...
    const lines = [`[Renderer] ${window.getWindowType()} error (${where}): ${data.message}`];
    if (data.stack) {
 ...
 */
async function findRecentProjectMissingReason(
  projectPath: string
): Promise<RecentProjectMissingReason | null> {
  let entries: Dirent[];
  try {
...
export class AppCheckRecentProjectsHandler extends IPCHandler<IPCEventType.appCheckRecentProjects> {
  ...
  public async handle(
    window: AppWindow
  ): Promise<RequestStatus<{ missing: MissingRecentProject[] }>> {
    ...
    return this.success({
      missing: checked
        .filter(
          (entry): entry is typeof entry & { reason: RecentProjectMissingReason } =>
            entry.reason !== null
        )
...
export class AppRevealRecentProjectHandler extends IPCHandler<IPCEventType.appRevealRecentProject> {
  ...
  public async handle(
    ...
    const known = window.app.globalState.recentlyOpened
      .list()
      .find((project) => normalizeProjectPath(project.path) === wanted);
    if (!wanted || !known) {
      return this.failed(
        new Error("Refusing to reveal a folder that is not a remembered project.")
      );
    }
...
export class AppReadBackgroundImageHandler extends IPCHandler<IPCEventType.appReadBackgroundImage> {
  ...
  public async handle(
    _window: AppWindow,
    { file }: IPCEvents[IPCEventType.appReadBackgroundImage]["data"]
  ): Promise<RequestStatus<{ data: Uint8Array | null }>> {
    if (!file || file !== path.basename(file)) {
      return this.failed(new Error("Invalid background image name"));
    }
    try {
      const data = await fs.readFile(
        path.join(backgroundCacheDirectory(electronApp.getPath("userData")), file)
      );
...
export class AppOpenExternalHandler extends IPCHandler<IPCEventType.appOpenExternal> {
  ...
  public async handle(
    _window: AppWindow,
    { url }: IPCEvents[IPCEventType.appOpenExternal]["data"]
  ): Promise<RequestStatus<void>> {
    if (!/^https?:\/\//i.test(url)) {
