...
describe("dev mode save IPC handlers", () => {
  ...
  it("writes, overwrites, reads, lists, and reads preview captures by project", async () => {
    ...
    const projectA: DevModeSaveProjectRef = {
      projectIdentifier: "project-a",
      projectPath: "/tmp/project"
    };
    const projectB: DevModeSaveProjectRef = {
    ...
    await expect(
      read.handle(window, { projectRef: projectA, id: "slot 1" })
    ...
    const filesBeforeDelete = await listAllFiles(
      path.join(tempDir, UserDataNamespace.DevModeSaves)
    );
    expect(filesBeforeDelete).toHaveLength(1);
    expect(filesBeforeDelete[0]).not.toContain("slot 1");

    await expect(
      deleteSave.handle(window, { projectRef: projectA, id: "slot 1" })
    ).resolves.toEqual({
      success: true,
      data: { deleted: true }
    });
  ...
  it("rejects unsafe ids and skips corrupted files when listing", async () => {
    ...
    await expect(
      write.handle(window, { projectRef, id: "bad/id", savedGame: {} })
    ).resolves.toMatchObject({
      success: false
    });
    ...
    await expect(
      write.handle(window, { projectRef, id: "good", savedGame: { ok: true } })
    ).resolves.toMatchObject({
      success: true
    });
