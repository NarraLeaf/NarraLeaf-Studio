...
async function ensureActorPathAllowed<T>(
  ...
  return unauthorizedResult<T>(
    authorization.reason ?? `File system access is not allowed for path: ${fsPath}`
  );
}
    ...
    switch (data.operation) {
      case "selectFile": {
        if (data.actor.kind !== "facade" || data.actor.id !== "default") {
          return this.success(
            unauthorizedResult<string[]>("Only the default facade can open the file picker")
          );
        }
        const grantPolicy = getRuntimeGrantPolicy(window, "selectFile");
        if (!grantPolicy) {
          return this.success(
            unauthorizedResult<string[]>("File picker is not allowed for this window")
          );
        }
      ...
      case "selectSaveFile": {
        if (data.actor.kind !== "facade" || data.actor.id !== "default") {
          return this.success(
            unauthorizedResult<string | null>("Only the default facade can open the save dialog")
          );
        }
        const grantPolicy = getRuntimeGrantPolicy(window, "selectSaveFile");
        if (!grantPolicy) {
          return this.success(
            unauthorizedResult<string | null>("Save dialog is not allowed for this window")
          );
        }
      ...
      case "requestRead": {
        ...
        if (denied) return this.success(denied);
        return this.success(
          await this.allocateRead(
            window,
            data.path,
            data.raw,
            "encoding" in data ? data.encoding : undefined
          )
        );
      }
      case "requestWrite": {
      ...
      case "ensureRegularFile": {
        ...
        return this.success(
          denied ?? (await Fs.ensureRegularFile(data.path, data.data, data.encoding))
        );
      }
      case "writeFileNoFollow": {
        const denied = await ensureActorPathAllowed<void>(window, data, data.path, "write");
        return this.success(
          denied ?? (await Fs.writeFileNoFollow(data.path, data.data, data.encoding))
        );
      }
      case "recoverCorruptedJsonFile": {
        const denied = await ensureActorPathAllowed<void>(window, data, data.path, "write");
        return this.success(
          denied ?? (await Fs.recoverCorruptedJsonFile(data.path, data.replacement, data.encoding))
        );
      }
      case "createDir": {
        const denied = await ensureActorPathAllowed<void>(window, data, data.path, "write");
      ...
      case "rename": {
        ...
        const denied = await ensureActorPathsAllowed<void>(
          window,
          data,
          "write",
          data.oldPath,
      ...
      case "moveFile": {
        const readDenied = await ensureActorPathAllowed<void>(window, data, data.src, "read");
        if (readDenied) return this.success(readDenied);
        const writeDenied = await ensureActorPathsAllowed<void>(
          window,
          data,
          "write",
          data.src,
      ...
      case "dirExists": {
        const denied = await ensureActorPathAllowed<boolean>(window, data, data.path, "read");
        return this.success(
          denied ?? ((await Fs.isDirExists(data.path)) as FsRequestResult<boolean>)
        );
      }
      case "isFile": {
        const denied = await ensureActorPathAllowed<boolean>(window, data, data.path, "read");
      ...
      default:
        return this.success(
          unauthorizedResult(
            `Unsupported privileged file system operation: ${(data as any).operation}`
          )
        );
    }
  }
  ...
  private async allocateRead(
    ...
    try {
      ...
      if (!exists.ok || !exists.data) {
        ...
        return !exists.ok
          ? (exists as FsRequestResult<string>)
          : {
              ok: false,
              error: {
                code: FsRejectErrorCode.NOT_FOUND,
                message: "File does not exist: " + fsPath
              }
            };
      }
      ...
      if (!isFile.ok || !isFile.data) {
        window.app.storageManager.cleanup(hash);
        return !isFile.ok
          ? (isFile as FsRequestResult<string>)
          : {
              ok: false,
              error: {
                code: FsRejectErrorCode.NOT_A_FILE,
                message: "Path is not a file: " + fsPath
              }
            };
      }
  ...
  private async allocateWrite(
    ...
    try {
      ...
      if (!dirExists.ok || !dirExists.data) {
        window.app.storageManager.cleanup(hash);
        return !dirExists.ok
          ? (dirExists as FsRequestResult<string>)
          : {
              ok: false,
...
export class PrivilegedPermissionRequestHandler extends IPCHandler<IPCEventType.privilegedPermissionRequest> {
  ...
  public async handle(
    ...
    const promptWindow = await window
      .getApp()
      .launchPluginPermissionPrompt(window, { request: data.request });
...
function getRequiredPermissionRequestCapability(
