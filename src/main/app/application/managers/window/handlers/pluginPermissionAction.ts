  ...
  public async handle(
    ...
    return new Promise<RequestStatus<PluginPermissionPromptResult>>((resolve) => {
      promptWindow.setCloseResultResolver(
        (result: WindowCloseResults[WindowAppType.PluginPermissionPrompt]) => {
...
export class PluginPermissionGrantHandler extends IPCHandler<IPCEventType.pluginPermissionGrant> {
  ...
  public handle(window: AppWindow, data: IPCEvents[IPCEventType.pluginPermissionGrant]["data"]) {
    return this.tryUse(() =>
      window.app.pluginPermissionManager.grantPermission(data.request, data.decision)
    );
  }
}
