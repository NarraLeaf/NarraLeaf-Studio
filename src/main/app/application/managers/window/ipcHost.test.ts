...
describe("IPCHost.invoke", () => {
  ...
  it("throws when invoked on a destroyed window", () => {
    ...
    expect(() => host.invoke(win, "test-event" as never, {} as never)).toThrow(
      "Window is destroyed"
    );
  });
});
...
describe("IPCHost global registration", () => {
  ...
  it("registers a single ipcMain.handle per event and routes by sender", async () => {
    const host = new IPCHost(Namespace.NarraLeafStudio);
    const handler = vi.fn(async (_sender: Electron.WebContents, data: unknown) => ({
      success: true,
      data
    }));
    ...
    const [channel, registered] = ipcMainMock.handle.mock.calls[0] as [
      string,
      (event: any, data: any) => Promise<any>
    ];
