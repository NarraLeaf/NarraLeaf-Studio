...
export const windowPermissionDeclarations: { [T in WindowAppType]: WindowPermissionDeclaration } = {
  ...
  [WindowAppType.Settings]: {
    fs: noFileSystemAccess,
    api: noElevatedAccess,
    capabilities: noDefaultCapabilities
  },
  ...
  [WindowAppType.Workspace]: {
    fs: projectFileSystemAccess,
    api: noElevatedAccess,
    capabilities: workspaceDefaultCapabilities,
    runtimeGrants: workspaceImportGrants
  },
  ...
  [WindowAppType.PluginPermissionPrompt]: {
    fs: noFileSystemAccess,
    api: pluginPermissionElevatedAccess,
    capabilities: noDefaultCapabilities
  },
...
export function getDeclaredFileSystemGrants(
  window: AppWindow,
  mode: FileSystemAccessMode
): FileSystemGrant[] {
...
export function getRuntimeGrantPolicy(
  window: AppWindow,
  grantType: keyof RuntimeGrantPolicy
): RuntimeGrantPolicy[typeof grantType] {
...
export function canUsePluginFileSystemGrantsAsWindowPolicy(window: AppWindow): boolean {
  return (
    windowPermissionDeclarations[window.getWindowType()].pluginFileSystemGrantAuthority?.({
      window
    }) ?? noPluginFileSystemGrantAuthority()
  );
}

export function getDeniedApiCapability(
  window: AppWindow,
  required: readonly ApiCapability[] | undefined
): ApiCapability | null {
