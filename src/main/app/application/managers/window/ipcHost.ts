...
export class IPCHost extends IPC<IPCEvents, IPCType.Host> {
  ...
  invoke<K extends keyof OnlyRequest<IPCEvents, IPCType.Host>>(
    ...
    return new Promise((resolve, reject) => {
      ...
      const handler = (
        ...
        response: Exclude<IPCEvents[K]["response"], never>
      ) => {
        cleanup();
        resolve(response);
      };
  ...
  send<K extends keyof OnlyMessage<IPCEvents, IPCType.Host>>(
    ...
    if (win.isDestroyed()) {
      throw new Error(
        `Window is destroyed. Tried to send IPC event: ${this.getEventName(key)}. (send)`
      );
    }
  ...
  ): RequestStatus<T extends undefined ? void : T> {
    if (data !== undefined) {
      return {
        success: true,
        data
