...
export type NavigationDecision = { allowed: true } | { allowed: false; reason: string };
...
export function decideWindowNavigation(request: WindowNavigationRequest): NavigationDecision {
  ...
  if (!request.isMainFrame) {
    return isBlankDocument(target) ? { allowed: true } : deny("Subframe navigation is not allowed");
  }
