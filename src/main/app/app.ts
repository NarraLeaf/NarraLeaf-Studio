...
import {
  devModeNetworkPolicy,
  readProjectNetworkSettings
} from "./application/managers/devMode/devModeNetworkPolicy";
...
export class App extends BaseApp {
  ...
  private findLauncher(): AppWindow<WindowAppType.Launcher> | undefined {
    return this.windowManager
      .getWindows()
      .find((window) => !window.isClosed() && window.getWindowType() === WindowAppType.Launcher) as
      | AppWindow<WindowAppType.Launcher>
      | undefined;
  }
  ...
  public async revealSettings(
    ...
    const existing = this.windowManager
      .getWindows()
      .find(
        (candidate) => !candidate.isClosed() && candidate.getWindowType() === WindowAppType.Settings
      );
    if (existing) {
      if (props?.highlight) {
  ...
  public handleLastWindowClosed(): void {
    ...
    this.logger.warn(
      "[App] No window and no status-bar item; quitting rather than going headless."
    );
    this.quit();
  }
   ...
   */
  private async confirmWorkspaceClose(
    ...
    try {
      ...
      if (!result.success) {
        this.logger.warn(
          `[Workspace] Close confirmation failed, keeping the window open: ${result.error}`
        );
        return false;
      }
      ...
      this.logger.warn(
        `[Workspace] No answer to the close confirmation, keeping the window open: ${String(error)}`
      );
      return false;
    }
  }
   ...
   */
  public async flushWorkspacePendingSaves(
    window: AppWindow<WindowAppType.Workspace>
  ): Promise<void> {
    if (window.isClosed()) {
      return;
    ...
    try {
      ...
      if (!result.success) {
        ...
        this.logger.warn(
          "[Workspace] Some stores failed to flush; see the workspace's Storage console channel"
        );
      }
    } catch (error) {
      ...
      await Promise.race([
        ...
        new Promise<void>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(`the checkpoint did not finish within ${CLOSE_CHECKPOINT_TIMEOUT_MS}ms`)
              ),
  ...
  public async flushAllWorkspacesPendingSaves(): Promise<void> {
    const workspaces = this.windowManager
      .getWindows()
      .filter(
  ...
  private hasOtherOpenWorkspace(window: AppWindow<WindowAppType.Workspace>): boolean {
    return this.windowManager
      ...
      .some(
        (other) =>
          other !== window && !other.isClosed() && other.getWindowType() === WindowAppType.Workspace
      );
  }
   ...
   */
  private async handleWorkspaceCloseRequest(
    window: AppWindow<WindowAppType.Workspace>
  ): Promise<void> {
    ...
    if (
      this.globalState.get("workspace.returnToLauncherOnClose") &&
      !this.hasOtherOpenWorkspace(window)
    ) {
      ...
      try {
        ...
        this.logger.error(
          "[Workspace] Keeping the window open, the launcher failed to start:",
          error
        );
  ...
  public authorizeRecentProjectAccess(opener: AppWindow, projectPath: string): void {
    const recentProject = this.globalState.recentlyOpened
      .list()
      .find((project) => normalizeProjectPath(project.path) === normalizeProjectPath(projectPath));
  ...
  public findWorkspaceForProject(
    projectPath: string
  ...
  public async openProject(
    ...
    const replaceOpener =
      Boolean(options.replaceOpener) && opener.getWindowType() === WindowAppType.Workspace;
  ...
  private workspacePlacement(
    ...
    return fits ? { x, y, width: bounds.width, height: bounds.height, center: false } : fallback;
  }
