...
describe("text codec round-trip", () => {
  ...
  it("round-trips the remaining CJK and Latin encodings", () => {
    expect(decodeTextBytes(encodeTextBytes("中文 GB18030", "gb18030"), "gb18030")).toBe(
      "中文 GB18030"
    );
    ...
    expect(decodeTextBytes(encodeTextBytes("naïve — café", "windows1252"), "windows1252")).toBe(
      "naïve — café"
    );
    expect(decodeTextBytes(encodeTextBytes("naïve café", "iso88591"), "iso88591")).toBe(
      "naïve café"
    );
