...
describe("findGpg", () => {
  it("takes the credential's own path, as the file or as its directory", async () => {
    ...
    expect(
      await findGpg({ configuredPath: at(root, "vault/gpg.exe"), env: {}, platform: "win32" })
    ).toBe(at(root, "vault/gpg.exe"));
    expect(await findGpg({ configuredPath: at(root, "vault"), env: {}, platform: "win32" })).toBe(
      at(root, "vault/gpg.exe")
    );
  });
  ...
  it("derives Git for Windows' bundled gpg, which no PATH search can find", async () => {
    ...
    expect(await findGpg({ env: { PATH: at(root, "Program/Git/cmd") }, platform: "win32" })).toBe(
      at(root, "Program/Git/usr/bin/gpg.exe")
    );

    const mingw = await tempTree(["Git/mingw64/bin/git.exe", "Git/usr/bin/gpg.exe"]);
    expect(await findGpg({ env: { PATH: at(mingw, "Git/mingw64/bin") }, platform: "win32" })).toBe(
      at(mingw, "Git/usr/bin/gpg.exe")
    );
  });
  ...
  it("looks for unsuffixed names off Windows, and never for git's gpg", async () => {
    ...
    expect(await findGpg({ env: { PATH: at(root, "bin") }, platform: "linux" })).toBe(
      at(root, "bin/gpg2")
    );
...
describe.skipIf(!oracleKey || !gpgBinary)("the gpg oracle", () => {
  it("produces a signature gpg itself accepts, and rejects after one flipped byte", async () => {
    ...
    const verify = (): string =>
      execFileSync(gpgBinary!, ["--batch", "--verify", signature, artifact], {
        encoding: "utf8",
        stdio: "pipe"
      });
