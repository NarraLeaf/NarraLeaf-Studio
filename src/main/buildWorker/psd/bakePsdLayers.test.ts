...
function redSquare(): { width: number; height: number; data: Uint8Array } {
  return {
    width: 2,
    height: 2,
    data: new Uint8Array([255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 128, 255, 0, 0, 128])
  };
}
  ...
  it("keeps the tree's shape and gives every node its path", () => {
    const psd = {
      ...
      children: [
        ...
        {
          name: "Mood",
          children: [{ name: "Happy", left: 1, top: 1, right: 3, bottom: 3, blendMode: "multiply" }]
        }
      ]
    } as unknown as Psd;
    ...
    expect(described.layers[0]).toMatchObject({
      path: ["Body"],
    ...
    expect(described.layers[1].children?.[0]).toMatchObject({
      path: ["Mood", "Happy"],
      blendMode: "multiply"
    });
  });
...
describe("bakeLayer", () => {
  it("places a cropped layer at its document position on a transparent canvas", async () => {
    ...
    const at = (x: number, y: number) => [
      ...decoded.rgba.slice((y * 4 + x) * 4, (y * 4 + x) * 4 + 4)
    ];
  ...
  it("multiplies layer opacity into alpha, because the engine has nowhere else to keep it", async () => {
    ...
    const decoded = decodePngToRgba(
      await bakeLayer(layer, { width: 2, height: 2 }, deflate),
      inflate
    );
  ...
  it("clips a layer that hangs off the canvas rather than wrapping it", async () => {
    ...
    const decoded = decodePngToRgba(
      await bakeLayer(layer, { width: 4, height: 4 }, deflate),
      inflate
    );
    const at = (x: number, y: number) => [
      ...decoded.rgba.slice((y * 4 + x) * 4, (y * 4 + x) * 4 + 4)
...
describe("bakeLayers with a merge", () => {
      ...
      children: [
        {
          ...
          opacity: 1,
          blendMode: "normal",
          imageData: solid(200, 200, 200, 255)
        },
        {
          name: "Shade",
          left: 0,
  ...
  it("flattens the merged layer using its own blend mode, not as a plain stack", async () => {
    ...
    const write = async (name: string, png: Uint8Array) => {
      written.set(name, png);
      return name;
    };
  ...
  it("ignores a mask the author switched off", async () => {
    const layer = {
      ...
      mask: {
        ...
        bottom: 2,
        disabled: true,
        imageData: maskData(2, 2, [0, 0, 0, 0])
      }
    } as unknown as Layer;
    const decoded = decodePngToRgba(
      await bakeLayer(layer, { width: 2, height: 2 }, deflate),
