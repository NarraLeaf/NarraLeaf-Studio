...
describe("asarUnpackedPath", () => {
  it("maps a packaged worker onto its unpacked twin", () => {
    expect(
      asarUnpackedPath(
        "/Applications/NarraLeaf Studio.app/Contents/Resources/app.asar/dist/main/buildWorker.js",
        "/"
      )
    ).toBe(
      "/Applications/NarraLeaf Studio.app/Contents/Resources/app.asar.unpacked/dist/main/buildWorker.js"
    );
  });
  ...
  it("handles the Windows shape from any host", () => {
    expect(
      asarUnpackedPath(
        "C:\\Program Files\\NarraLeaf Studio\\resources\\app.asar\\dist\\main\\buildWorker.js",
        "\\"
      )
    ).toBe(
      "C:\\Program Files\\NarraLeaf Studio\\resources\\app.asar.unpacked\\dist\\main\\buildWorker.js"
    );
  });

  it("returns null outside an asar, which is every unpackaged run", () => {
    expect(
      asarUnpackedPath("/Users/dev/NarraLeaf-Studio/dist/main/buildWorker.js", "/")
    ).toBeNull();
  });
