...
function text(size = 20_000): Buffer {
  return Buffer.from(
    "the quick brown fox jumps over the lazy dog ".repeat(Math.ceil(size / 43)).slice(0, size)
  );
}
...
describe("precompressWebSite", () => {
  it("writes a .br and a .gz beside each text file, mirroring the layout", async () => {
    ...
    for (const relativePath of ["renderer.js", path.join("assets", "story.json")]) {
      for (const suffix of [".br", ".gz"]) {
        await expect(
          fs.stat(path.join(targetDir, `${relativePath}${suffix}`))
        ).resolves.toBeTruthy();
      }
    }
  });
  ...
  it("writes variants that decompress back to the original bytes", async () => {
    ...
    expect(
      zlib.brotliDecompressSync(await fs.readFile(path.join(targetDir, "pack.json.br")))
    ).toEqual(source);
    expect(zlib.gunzipSync(await fs.readFile(path.join(targetDir, "pack.json.gz")))).toEqual(
      source
    );
  });
