...
function builderConfiguration(
  ...
  return {
    ...
    ...(config.electronMirror ? { electronDownload: { mirror: config.electronMirror } } : {}),
    directories: {
...
export async function runGameBuild(
  config: GameBuildWorkerConfig,
  log: GameBuildLogger
): Promise<string[]> {
...
async function packageDesktopTargets(
  config: GameBuildWorkerConfig,
  log: GameBuildLogger
): Promise<string[]> {
