...
describe("packageWebSite", () => {
  it("copies the site for the dir format", async () => {
    const artifacts = await packageWebSite(
      {
        ...
        precompress: false,
        formats: ["dir"],
        dirName: "Game-1.0.0-web",
        zipName: "Game-1.0.0-web.zip"
      },
      outputDir,
      noopLog
    );
    ...
    await expect(
      fs.readFile(path.join(outputDir, "Game-1.0.0-web", "index.html"), "utf-8")
    ).resolves.toBe("<!doctype html>");
    await expect(
      fs.stat(path.join(outputDir, "Game-1.0.0-web", "assets", "a.png"))
    ).resolves.toBeTruthy();
  });
  ...
  describe("precompression", () => {
    ...
    it("never writes into the compiled site, which the mobile packages share", async () => {
      ...
      expect((await fs.readdir(sourceDir)).sort()).toEqual([
        "assets",
        "index.html",
        "pack.json",
        "renderer.js"
