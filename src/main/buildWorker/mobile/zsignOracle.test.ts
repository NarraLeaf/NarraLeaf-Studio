...
function readCodeSignature(macho: Buffer): { directory: CodeDirectory; hasCms: boolean } {
    ...
    const cd = superBlob.subarray(directoryOffset!, directoryOffset! + superBlob.readUInt32BE(directoryOffset! + 4));
...
async function buildSignedIpa(): Promise<{ ipaPath: string; templateManifest: MobileShellManifest }> {
    ...
    const job: GameBuildWorkerMobileJob = {
        ...
        ios: {
            ...
            bundleVersion: "1.2.3",
            signing: { p12File, p12Password: APPLE_IDENTITY_PASSWORD, provisioningProfileFile, toolPath: zsignPath },
        },
    };
...
async function readExecutable(ipaPath: string, templateManifest: MobileShellManifest): Promise<Buffer> {
...
describe("the zsign oracle's availability", () => {
    it("is present when CI demands it", () => {
        if (oracleRequired) {
            expect(
                zsign.available,
                "REQUIRE_ZSIGN_ORACLE is set but no staged zsign was found",
            ).toBe(true);
        }
    });
});
...
describe.skipIf(!zsign.available)("zsign on a Studio-built .ipa", () => {
    it("signs the package the repack produced", async () => {
        ...
        const { directory, hasCms } = readCodeSignature(await readExecutable(ipaPath, templateManifest));
    ...
    it("embeds the profile, and the identity it was signed with", async () => {
        ...
        const embedded = index.entries.find(entry =>
            entry.name === "Payload/OracleGame.app/embedded.mobileprovision");
        ...
        const codeResources = index.entries.find(entry =>
            entry.name === "Payload/OracleGame.app/_CodeSignature/CodeResources");
    ...
    it("refuses a profile issued for a different app", async () => {
        ...
        }, outputDir, () => undefined, MTIME)).rejects.toThrow(/does not cover the bundle id/);
