...
export async function buildAab(input: BuildAabInput): Promise<Buffer> {
  ...
  for (const entry of index.entries) {
    if ((entry.unixMode & 0o170000) === 0o120000) {
      throw new Error(
        `Template contains a symlink ("${entry.name}"), which the bundle build does not support`
      );
    }
    ...
    moduleEntries.push({
      modulePath: bundleModulePath(entry.name),
      source: rawPassthrough(templateApk, entry)
    });
  }
  ...
  const entries: ZipWriteEntry[] = [
    ...
    ...moduleEntries.map((entry) => ({
      name: `${BUNDLE_MODULE}/${entry.modulePath}`,
      source: entry.source
    }))
  ];
