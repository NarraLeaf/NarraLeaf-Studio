...
function plistProfile(overrides: {
  ...
  const devices = (overrides.devices ?? []).map((device) => `<string>${device}</string>`).join("");
  return Buffer.from(
...
describe("extractProvisioningProfilePlist", () => {
  ...
  it("refuses something that is neither", () => {
    expect(() => extractProvisioningProfilePlist(Buffer.from("not a profile at all"))).toThrow(
      /neither a signed document nor a property list/
    );
  });
...
describe("parseProvisioningProfile", () => {
  ...
  it("splits the team prefix off the bundle id, wildcard and all", () => {
    expect(
      parseProvisioningProfile(plistProfile({ applicationIdentifier: "TEAM123456.*" }))
        .bundleIdPattern
    ).toBe("*");
    expect(
      parseProvisioningProfile(plistProfile({ applicationIdentifier: "TEAM123456.com.a.b.c" }))
        .bundleIdPattern
    ).toBe("com.a.b.c");
  });

  it("names what is missing rather than returning a half-read profile", () => {
    expect(() => parseProvisioningProfile(Buffer.from("<plist><dict></dict></plist>"))).toThrow(
...
describe("profileCoversBundleId", () => {
  ...
  it("honours a team-wide wildcard", () => {
    const anything = parseProvisioningProfile(
      plistProfile({ applicationIdentifier: "TEAM123456.*" })
    );
