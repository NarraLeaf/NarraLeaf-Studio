...
describe("convertBinaryManifestToProto", () => {
      ...
      attributes: [
        {
          namespaceUri: "",
          name: "package",
          value: "com.narraleaf.shell.placeholder",
          resourceId: "0x00000000"
        },
        {
  ...
  it("writes a source position on compiled attributes only", () => {
    ...
    const element = messageAt(
      decodeMessage(convertBinaryManifestToProto(buildProtoManifestFixture())),
      1
    )!;
  ...
  it("converts the identity the APK patcher wrote, not the template's", () => {
    ...
    const root = summarizeXmlNode(convertBinaryManifestToProto(data)) as Extract<
      ReturnType<typeof summarize>,
      { name: string }
    >;
    const byName = new Map(root.attributes.map((attribute) => [attribute.name, attribute]));
  ...
  it("rejects a file that is not binary XML", () => {
    expect(() =>
      convertBinaryManifestToProto(Buffer.from([0x02, 0x00, 0x0c, 0x00, 8, 0, 0, 0]))
