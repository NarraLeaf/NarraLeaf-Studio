...
export function summarizeItem(item: ProtoMessage): string {
  ...
  if (reference) {
    ...
    return name
      ? `${kind}:@${name}`
      : `${kind}:0x${uintAt(reference, 2).toString(16).padStart(8, "0")}`;
  }
  if (has(item, 2)) {
...
export function summarizeCompound(compound: ProtoMessage): string {
  ...
  const referenceId = (message: ProtoMessage, field: number) =>
    `0x${uintAt(decodeMessage(bytesAt(message, field)), 2)
      .toString(16)
      .padStart(8, "0")}`;

  const attribute = messageAt(compound, 1);
  if (attribute) {
    const symbols = repeatedAt(attribute, 4).map(
      (symbol) => `${referenceId(symbol, 3)}=${uintAt(symbol, 4)}`
    );
    return (
      `attr(format=0x${uintAt(attribute, 1).toString(16)},min=${intAt(attribute, 2)},max=${intAt(attribute, 3)})` +
  ...
  if (style) {
    ...
    const entries = repeatedAt(style, 3).map(
      (entry) => `${referenceId(entry, 3)}=${summarizeItem(messageAt(entry, 4)!)}`
    );
    return `style(parent=${parent}){${entries.join(",")}}`;
  }
  const array = messageAt(compound, 4);
  if (array) {
    return `array[${repeatedAt(array, 1)
      .map((element) => summarizeItem(messageAt(element, 3)!))
      .join(",")}]`;
  }
  const plural = messageAt(compound, 5);
  if (plural) {
    const entries = repeatedAt(plural, 1).map(
      (entry) => `${PLURAL_ARITIES[uintAt(entry, 3)]}=${summarizeItem(messageAt(entry, 4)!)}`
    );
