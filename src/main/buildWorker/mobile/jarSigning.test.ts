  ...
  const entries: ZipWriteEntry[] = [
    {
      ...
      source: { kind: "buffer", data: Buffer.from([0x08, 0x01, 0x12, 0x02, 0x18, 0x1a]) }
    },
    { name: "base/", source: null },
    {
      name: "base/manifest/AndroidManifest.xml",
      source: { kind: "buffer", data: Buffer.from([0x03, 0x00, 0x08, 0x00, 1, 2, 3, 4]) }
    },
    {
      name: "base/dex/classes.dex",
      source: { kind: "buffer", data: Buffer.from("dex\n035\0placeholder") }
    },
    // Stored (.png): a payload a test can flip a byte in directly.
    {
      name: "base/res/drawable/icon.png",
      source: { kind: "buffer", data: Buffer.alloc(96, 0x37) }
    },
    { name: LONG_NAME, source: { kind: "buffer", data: Buffer.alloc(48, 0x5c) } },
    {
      name: "base/assets/www/index.html",
      source: { kind: "buffer", data: Buffer.from("<!doctype html><title>g</title>") }
    },
    // An existing META-INF entry: excluded from the manifest, kept in the zip.
    {
      name: "META-INF/com/android/build/app-metadata.properties",
      source: { kind: "buffer", data: Buffer.from("appMetadataVersion=1.1\n") }
    }
  ];
...
const sha256Base64 = (data: Buffer): string =>
...
describe("signJar + verifyJarSignature", () => {
  it("signs a bundle its verifier accepts, covering exactly the payload entries", async () => {
    ...
    expect(result.certificateChainDer.map((der) => der.toString("base64"))).toEqual([
      debugIdentity.certificateDerBase64
    ]);
  });
  ...
  it("preserves every original entry byte-identically", async () => {
    ...
    const prefixLength =
      signedIndex.entries[3].localHeaderOffset - unsignedIndex.entries[0].localHeaderOffset;
    expect(prefixLength).toBeGreaterThan(0);
    expect(
      signed
        .subarray(prefixLength, prefixLength + unsignedIndex.centralDirectoryOffset)
        .equals(unsigned.subarray(0, unsignedIndex.centralDirectoryOffset))
    ).toBe(true);
  ...
  it("is byte-identical across runs", async () => {
    const unsigned = await buildBundle();
    expect(
      signJar(unsigned, identity, { mtime: MTIME }).equals(
        signJar(unsigned, identity, { mtime: MTIME })
      )
    ).toBe(true);
  });
  ...
  it("refuses to double-sign", async () => {
    ...
    expect(() => signJar(signed, identity, { mtime: MTIME, signerName: "OTHER" })).toThrow(
      /already JAR-signed/
    );
  });
  ...
  it("rejects a signature file name the JAR spec cannot express", async () => {
    const unsigned = await buildBundle();
    expect(() => signJar(unsigned, identity, { signerName: "TOO-LONG-NAME" })).toThrow(
      /not a usable/
    );
  ...
  it("is not fooled by a tamper that repairs the zip's own CRC", async () => {
    ...
    const signed = signJar(await buildBundle(), identity, { mtime: MTIME });
    const tampered = flipByteIn(signed, "base/res/drawable/icon.png", 13);
    expect(entryBytes(tampered, "base/res/drawable/icon.png")[13]).toBe(
      entryBytes(signed, "base/res/drawable/icon.png")[13] ^ 0xff
    );
  });
  ...
  it("embeds a whole chain, leaf first, and still verifies", async () => {
    ...
    expect(result.verified).toBe(true);
    expect(result.certificateChainDer.map((der) => der.toString("base64"))).toEqual(
      chained.certificateChainDerBase64
    );
  });
});
