  ...
  const entries: ZipWriteEntry[] = [
    {
      ...
      source: { kind: "buffer", data: Buffer.from("vcs") }
    },
    {
      name: "AndroidManifest.xml",
      source: { kind: "buffer", data: buildProtoManifestFixture() },
      method: "deflate"
    },
    ...
    {
      name: "lib/arm64-v8a/libnlcrypto.so",
      source: { kind: "buffer", data: NATIVE_BYTES },
      method: "store"
    },
    {
      name: "lib/x86_64/libnlcrypto.so",
      source: { kind: "buffer", data: NATIVE_BYTES },
      method: "store"
    },
    ...
    {
      name: "resources.arsc",
      source: { kind: "buffer", data: buildProtoArscFixture() },
      method: "store",
      forceAlign: 4
    },
    ...extra
...
describe("bundleModulePath", () => {
  it("routes every APK entry to its bundle home", () => {
    ...
    expect(bundleModulePath("kotlin-tooling-metadata.json")).toBe(
      "root/kotlin-tooling-metadata.json"
    );
...
describe("targeting derivation", () => {
  ...
  it("declares only the directories that hold a file directly", () => {
    ...
    expect(assetDirectoriesOf(["assets/www/js/app.js", "assets/shell-config.json"])).toEqual([
      "assets/www/js",
      "assets"
    ]);
    expect(
...
describe("buildAab", () => {
  it("lays the template out as a base module", async () => {
    ...
    expect(names(aab)).toEqual([
      ...
      "base/lib/arm64-v8a/libnlcrypto.so",
      "base/lib/x86_64/libnlcrypto.so",
  ...
  it("converts the manifest to proto XML with the patched identity", async () => {
    ...
    const attributes = (root as { attributes: { name: string; value: string; item?: string }[] })
      .attributes;
  ...
  it("converts resources.arsc to proto with the package renamed in lockstep", async () => {
    ...
    expect(table.packages[0].types.map((type) => type.name)).toEqual([
      "drawable",
      "string",
      "style"
    ]);
  ...
  it("targets each native directory by its ABI", async () => {
    ...
    expect(directories.map((directory) => stringAt(directory, 1))).toEqual([
      "lib/arm64-v8a",
      "lib/x86_64"
    ]);
    ...
    expect(
      directories.map((directory) => uintAt(messageAt(messageAt(directory, 2)!, 1)!, 1))
    ).toEqual([3, 5]);
  });
  ...
  it("injects the game site and shell config under the manifest's roots", async () => {
    const aab = await build();
    expect(bytesOf(aab, "base/assets/www/index.html").toString("utf8")).toBe(
      "<!doctype html><title>g</title>"
    );
    expect(bytesOf(aab, "base/assets/www/js/app.js").toString("utf8")).toBe("console.log(1)");
    expect(bytesOf(aab, "base/assets/shell-config.json").toString("utf8")).toBe(
      BASE.shellConfigJson
    );
  });
  ...
  it("passes untouched entries through byte-identically", async () => {
    ...
    expect(bytesOf(aab, "base/lib/arm64-v8a/libnlcrypto.so").equals(NATIVE_BYTES)).toBe(true);
  });
  ...
  it("is deterministic for identical inputs", async () => {
    ...
    expect(
      (await build({ templateApk: template })).equals(await build({ templateApk: template }))
    ).toBe(true);
  });
  ...
  it("stamps entries with the injected mtime, not the wall clock", async () => {
    ...
    const late = await build({
      ...
      mtime: new Date(Date.UTC(2021, 5, 15, 10, 30))
    });
  ...
  it("fails loudly when an icon override matches no template slot", async () => {
    await expect(
      build({ iconPngBySlot: { "res/mipmap-nope/ic.png": Buffer.from([1]) } })
  ...
  it("rejects a template containing a symlink (would be silently flattened)", async () => {
    const template = await buildTemplateApk([
      {
        name: "lib/arm64-v8a/link.so",
        source: { kind: "buffer", data: Buffer.from("target") },
        unixMode: 0o120755
      }
    ]);
    ...
    const template = await buildTemplateApk([
      {
        ...
        source: { kind: "buffer", data: Buffer.from([0x03, 0x00, 8, 0, 1, 2, 3, 4]) }
      }
    ]);
  ...
  it("fails loudly when the template lacks AndroidManifest.xml or resources.arsc", async () => {
    ...
    await expect(build({ templateApk: other.toBuffer() })).rejects.toThrow(
