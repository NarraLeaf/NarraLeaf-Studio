...
function locateSections(apk: Buffer): ApkSections {
  ...
  for (let offset = apk.length - 22; offset >= lowest; offset--) {
    if (
      apk.readUInt32LE(offset) === EOCD_SIGNATURE &&
  ...
  if (
    centralDirOffset >= APK_SIGNING_BLOCK_MAGIC.length &&
    apk
      .subarray(centralDirOffset - APK_SIGNING_BLOCK_MAGIC.length, centralDirOffset)
...
function computeContentDigest(sections: ApkSections): Buffer {
  ...
  digestChunks(
    eocdWithCentralDirOffset(sections.endOfCentralDirectory, sections.centralDirOffset),
    chunkDigests
  );
...
function findSigningBlock(apk: Buffer): {
  block: Buffer;
  centralDirOffset: number;
