...
function parseSections(source: Buffer): ManifestSection[] {
  ...
  while (cursor < source.length) {
    ...
    while (
      ...
      source[contentEnd] !== 0x0d &&
      source[contentEnd] !== 0x0a
    ) {
      contentEnd++;
    }
    let next = contentEnd;
    ...
    if (contentEnd === cursor) {
      ...
      if (source[cursor] === 0x20) {
        ...
        headers.push({
          key,
          value: [source.subarray(Math.min(valueStart, contentEnd), contentEnd)]
        });
      }
    }
    cursor = next;
...
function prependEntries(zip: Buffer, newEntries: NewEntry[], mtime: Date): Buffer {
  ...
  if (eocdOffset + EOCD_SIZE > zip.length || zip.readUInt32LE(eocdOffset) !== EOCD_SIGNATURE) {
    throw new Error(
      "Malformed archive: unexpected bytes between the central directory and the EOCD"
    );
  }
 ...
 */
export function signJar(
  zip: Buffer,
  identity: ApkSigningIdentity,
  options: JarSignOptions = {}
): Buffer {
  ...
  const signatureFileParts: Buffer[] = [
    writeSection([
...
function parsePkcs7(block: Buffer): { certificates: Buffer[]; signer: ParsedSignerInfo } {
  ...
  for (const part of parts.slice(3)) {
    if (part.tag === 0xa0) {
      certificates = asn1Children(block, part).map((node) =>
        Buffer.from(block.subarray(node.start, node.end))
      );
  ...
  if (signerNodes.length !== 1) {
    throw new Error(
      `the SignedData carries ${signerNodes.length} SignerInfos, expected exactly one`
    );
  }
  ...
  if (
    !algorithmOid.equals(encodeOid(OID_RSA_ENCRYPTION)) &&
    !algorithmOid.equals(encodeOid(OID_SHA256_WITH_RSA))
  ) {
...
export function verifyJarSignature(zip: Buffer): JarVerifyResult {
  ...
  try {
    ...
    const signatureEntries = index.entries.filter((entry) =>
      SIGNATURE_FILE_PATTERN.test(entry.name)
    );
    ...
    const manifestDigest = requireAttribute(
      signatureMain,
    ...
    if (
      sha256(manifest.subarray(mainSection.start, mainSection.end)).toString("base64") !==
      mainDigest
    ) {
    ...
    for (const section of signatureSections.slice(1)) {
      ...
      const actual = sha256(manifest.subarray(manifestSection.start, manifestSection.end)).toString(
        "base64"
      );
      if (actual !== expected) {
