...
describe("buildSelfSignedCertificate", () => {
  ...
  it("encodes validity per RFC 5280: UTCTime through 2049, GeneralizedTime from 2050", () => {
    ...
    const { der } = build();
    expect(
      der.includes(Buffer.concat([Buffer.from([0x17, 0x0d]), Buffer.from("200101000000Z")]))
    ).toBe(true);
    expect(
      der.includes(Buffer.concat([Buffer.from([0x18, 0x0f]), Buffer.from("20500101000000Z")]))
    ).toBe(true);
  });
  ...
  it("is byte-deterministic for fixed inputs", () => {
    ...
    expect(buildSelfSignedCertificate(params).equals(buildSelfSignedCertificate(params))).toBe(
      true
    );
  });
});
