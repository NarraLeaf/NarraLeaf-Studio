...
function parseAttributes(
  ...
  for (let i = 0; i < attributeCount; i++) {
    ...
    const value =
      dataType === RES_VALUE_TYPE_STRING && rawValueIndex === NO_ENTRY
    ...
    attributes.push({
      ...
      ...(dataType === RES_VALUE_TYPE_STRING
        ? {}
        : { compiledItem: encodePrimitiveItem({ dataType, data }) })
    });
  }
  return attributes;
}
...
function parseTree(axml: Buffer): ProtoNode {
  ...
  for (const chunk of chunks) {
    const ext = chunk.start + chunk.headerSize;
    const line =
      chunk.type === RES_STRING_POOL_TYPE || chunk.type === RES_XML_RESOURCE_MAP_TYPE
        ? 0
