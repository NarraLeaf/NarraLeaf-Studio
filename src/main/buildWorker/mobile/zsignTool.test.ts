...
describe("zsignHostTarget", () => {
  it("covers the three hosts upstream publishes a binary for", () => {
    expect(zsignHostTarget("win32", "x64")).toEqual({
      platformKey: "win32",
      binaryName: "zsign.exe"
    });
    expect(zsignHostTarget("linux", "x64")).toEqual({ platformKey: "linux", binaryName: "zsign" });
    expect(zsignHostTarget("darwin", "arm64")).toEqual({
      platformKey: "darwin",
      binaryName: "zsign"
    });
  });
...
describe("resolveZsignTool", () => {
  it("finds the staged binary in a packaged Studio", async () => {
    const { app, root } = await makeApp(true);
    const staged = await writeFileAt(
      path.join(root, "resources", "codesign", "win32", "zsign.exe")
    );
    await expect(
      resolveZsignTool(app, { platform: "win32", arch: "x64", env: {} })
    ).resolves.toEqual({
      available: true,
      path: staged
    });
  ...
  it("falls back to the dev cache when the tree was never staged", async () => {
    const { app, root } = await makeApp(false);
    const cached = await writeFileAt(
      path.join(root, ".dev", "cache", "codesign", "darwin", "zsign")
    );
    await expect(
      resolveZsignTool(app, { platform: "darwin", arch: "arm64", env: {} })
    ).resolves.toEqual({
      available: true,
      path: cached
    });
  ...
  it("prefers the staged tree over the dev cache", async () => {
    ...
    await expect(
      resolveZsignTool(app, { platform: "linux", arch: "x64", env: {} })
    ).resolves.toEqual({
      available: true,
      path: staged
    });
  ...
  it("ignores a directory that happens to sit where the binary should be", async () => {
    const { app, root } = await makeApp(true);
    await fs.mkdir(path.join(root, "resources", "codesign", "win32", "zsign.exe"), {
      recursive: true
    });
    const result = await resolveZsignTool(app, { platform: "win32", arch: "x64", env: {} });
    expect(result.available).toBe(false);
