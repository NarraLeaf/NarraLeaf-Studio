...
async function fixtureFiles(): Promise<{ dir: string; signing: GameBuildWorkerIosSigning }> {
    ...
    return { dir, signing: { p12File, p12Password: APPLE_IDENTITY_PASSWORD, provisioningProfileFile, toolPath: "C:/nowhere/zsign.exe" } };
}
...
describe("signIpa", () => {
    ...
    it("deletes the ephemeral container after a successful run", async () => {
        const zsign = succeeds();
        await signIpa({ ...await options(), run: zsign.run });
        await expect(fs.access(zsign.calls[0].args[zsign.calls[0].args.indexOf("-k") + 1])).rejects.toThrow();
    });
    ...
    it("refuses to run at all when the tool is unavailable", async () => {
        const zsign = succeeds();
        const error = await expectFailure(signIpa({
            ...await options(),
            tool: { available: false, reason: "not-staged", detail: "the tool was never staged", searched: [] },
            run: zsign.run,
        }), "tool-unavailable");
    ...
    it("names a leaf-only identity rather than letting zsign fail on the issuer", async () => {
        ...
        await fs.writeFile(p12File, writePasswordlessPkcs12({
            privateKeyDer: crypto.createPrivateKey(original.privateKeyPem)
        ...
        const error = await expectFailure(signIpa({
            ...await options(),
            signing: { p12File, p12Password: "", provisioningProfileFile: profileFile, toolPath: "C:/nowhere/zsign.exe" },
            run: zsign.run,
    ...
    it("fails when zsign claims success but writes nothing", async () => {
        const error = await expectFailure(
            signIpa({ ...await options(), run: recorder(() => ({ exitCode: 0, output: ">>> Done." })).run }),
            "signing-failed",
        );
    ...
    it("carries zsign's own words into the failure, rather than guessing", async () => {
        const error = await expectFailure(signIpa({
            ...await options(),
            run: recorder(() => ({ exitCode: 255, output: ">>> Unknown issuer hash 0xdeadbeef!\n>>> Signed Failed!" })).run,
        }), "signing-failed");
    ...
    it("refuses to write its output over its input", async () => {
        const opts = await options();
        await expect(signIpa({ ...opts, signedIpaPath: opts.unsignedIpaPath, run: succeeds().run }))
