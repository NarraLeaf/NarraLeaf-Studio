...
import { parseStringPool, readChunk, RES_STRING_POOL_TYPE, type Chunk, type StringPool } from "./axml";
...
export function encodePrimitiveItem(value: ResValue): Buffer | undefined {
    ...
    switch (dataType) {
        ...
        case RES_VALUE_TYPE_DYNAMIC_ATTRIBUTE: {
            const isAttribute = dataType === RES_VALUE_TYPE_ATTRIBUTE
                || dataType === RES_VALUE_TYPE_DYNAMIC_ATTRIBUTE;
            const isDynamic = dataType === RES_VALUE_TYPE_DYNAMIC_REFERENCE
                || dataType === RES_VALUE_TYPE_DYNAMIC_ATTRIBUTE;
            return encodeMessage(item => item.message(1, reference => {
        ...
        default:
            return encodeMessage(item => item.message(7, prim => encodePrimitiveBody(prim, dataType, data)));
    }
}
...
function encodePrimitiveBody(prim: ProtoWriter, dataType: number, data: number): void {
    switch (dataType) {
        ...
        case RES_VALUE_TYPE_INT_COLOR_RGB4:
            ...
            return;
        default:
            throw new Error(`Unsupported Res_value dataType 0x${dataType.toString(16)} in the resource table`);
    }
}
...
function parsePackage(buffer: Buffer, chunk: Chunk): ArscPackage {
    ...
    while (cursor < chunk.start + chunk.size) {
        ...
        if (sub.type !== RES_TABLE_TYPE_TYPE && sub.type !== RES_TABLE_TYPE_SPEC_TYPE
...
function encodeStringItem(typeName: string, text: string, resourceName: string): Buffer {
    if (typeName !== "string" && text.startsWith("res/")) {
        ...
        return encodeMessage(item => item.message(5, file => {
            file.string(1, text);
            if (typeName === "raw") {
                return;
            }
...
function encodeTableItem(
    ...
    if (value.dataType === RES_VALUE_TYPE_STRING) {
        ...
        if (text === undefined) {
            throw new Error(`Resource "${typeName}/${resourceName}" references string ${value.data}, which is not in the pool`);
        }
...
function encodeCompoundValue(
    ...
    switch (typeName) {
        case "style":
            return encodeMessage(compound => compound.message(2, style => {
                if (map.parent !== 0) {
        ...
        case "array":
            return encodeMessage(compound => compound.message(4, array => {
                for (const child of map.entries) {
                    array.message(1, element => element.messageBytes(3, item(child.value)));
                }
            }));
        case "plurals":
            return encodeMessage(compound => compound.message(5, plural => {
                for (const child of map.entries) {
        ...
        case "attr":
        case "^attr-private":
            return encodeMessage(compound => compound.message(1, attribute => {
                let formatFlags = 0;
                let minInt = 0;
                let maxInt = 0;
                ...
                for (const child of map.entries) {
                    if (child.name === ATTR_TYPE) {
                        ...
                        symbols.push({ name: child.name, value: child.value.data, type: child.value.dataType });
                    }
                }
