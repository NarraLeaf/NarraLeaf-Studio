...
describe("writeZip round trip", () => {
  ...
  it("streams a chunked source with a patched CRC", async () => {
    ...
    const buffer = await writeToBuffer(
      [
        {
          name: "big.bin",
          source: { kind: "stream", size: whole.length, open: openStream },
          method: "store"
        },
        {
          name: "big.txt",
  ...
  it("passes raw entries through byte-identically without re-encoding", async () => {
    ...
    for (const entry of parseZipIndex(repacked).entries) {
      ...
      expect(
        repacked
          .subarray(repackedSpan.start, repackedSpan.end)
          .equals(original.subarray(sourceSpan.start, sourceSpan.end))
      ).toBe(true);
    }
...
describe("alignment", () => {
  ...
  it("honors forceAlign for entries that must sit aligned regardless of policy", async () => {
    ...
    const entry = parseZipIndex(buffer).entries.find(
      (candidate) => candidate.name === "resources.arsc"
    )!;
...
describe("entry validation", () => {
  ...
  it("rejects unsafe and duplicate names", async () => {
    await expect(
      writeToBuffer([{ name: "../escape", source: payload }], APK_OPTIONS)
    ).rejects.toThrow(/Unsafe/);
    await expect(
      writeToBuffer([{ name: "/rooted", source: payload }], APK_OPTIONS)
    ).rejects.toThrow(/Unsafe/);
    await expect(writeToBuffer([{ name: "a\\b", source: payload }], APK_OPTIONS)).rejects.toThrow(
      /Unsafe/
    );
    await expect(
      writeToBuffer(
  ...
  it("rejects a directory with data and a file without", async () => {
    await expect(writeToBuffer([{ name: "dir/", source: payload }], APK_OPTIONS)).rejects.toThrow(
      /must not/
    );
    await expect(writeToBuffer([{ name: "file.txt", source: null }], APK_OPTIONS)).rejects.toThrow(
      /must/
    );
  });
});
...
describe("zip64", () => {
  ...
  it(
    "switches to zip64 records at exactly 65535 entries (the EOCD sentinel) and parses back",
    async () => {
  ...
  it(
    "stays plain zip below the sentinel",
    async () => {
      const output = new BufferZipOutput();
  ...
  it(
    "fails loudly instead of writing zip64 when the target forbids it",
    async () => {
