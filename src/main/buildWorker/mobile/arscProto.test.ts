...
describe("convertArscToProto", () => {
                  ...
                  values: [
                    {
                      config: "(default)",
                      value:
                        "style(parent=0x01030000){0x01010098=ref:0x7f010000,0x010100d4=bool:true}"
                    }
                  ]
                }
              ]
  ...
  it("carries the package rename the APK patcher wrote", () => {
    ...
    expect(summarizeResourceTable(convertArscToProto(data)).packages[0].name).toBe(
      "com.acme.mygame"
    );
  });
  ...
  it("rejects a file that is not a resource table", () => {
    expect(() => convertArscToProto(Buffer.from([0x03, 0x00, 0x08, 0x00, 8, 0, 0, 0]))).toThrow(
      /Not a resources\.arsc file/
    );
  });
});
