...
describe("writePasswordlessPkcs12", () => {
  ...
  it("keeps the leaf first, which is what makes it the signing certificate", () => {
    ...
    const leaf = new crypto.X509Certificate(
      Buffer.from(reread.certificateChainDerBase64[0], "base64")
    );
  ...
  it("takes a freshly generated identity too, not only a re-packed one", () => {
    ...
    const bytes = writePasswordlessPkcs12({
      privateKeyDer: crypto
        .createPrivateKey(debug.privateKeyPem)
        .export({ type: "pkcs8", format: "der" }),
      certificateChainDer: [Buffer.from(debug.certificateDerBase64, "base64")],
      macSalt: FIXED_SALT
    });
    expect(readKeystore(bytes, { storePassword: "" }).certificateDerBase64).toBe(
      debug.certificateDerBase64
    );
  });

  it("refuses to write a container with no certificate", () => {
    expect(() =>
      writePasswordlessPkcs12({ privateKeyDer: Buffer.alloc(4), certificateChainDer: [] })
  ...
  it("does not open with a non-empty password", () => {
    ...
    const { bytes } = repack();
    expect(() => readKeystore(bytes, { storePassword: "anything" })).toThrow(
