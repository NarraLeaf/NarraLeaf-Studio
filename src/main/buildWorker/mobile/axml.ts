...
function encodePoolString(value: string, utf8: boolean): Buffer {
  if (utf8) {
    ...
    return Buffer.concat([
      encodeVarint8(utf16Length),
      encodeVarint8(bytes.length),
      bytes,
      Buffer.from([0])
    ]);
  }
...
export function parseBinaryManifest(axml: Buffer): BinaryManifestIdentity {
  ...
  const versionCodeAttribute = findAttribute(parsed, "manifest", {
    resourceId: RES_ID_VERSION_CODE
  });
  const versionNameAttribute = findAttribute(parsed, "manifest", {
    resourceId: RES_ID_VERSION_NAME
  });
