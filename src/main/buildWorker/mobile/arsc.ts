...
function readPackageName(arsc: Buffer, chunk: Chunk): string {
  const slot = arsc.subarray(
    chunk.start + PACKAGE_NAME_OFFSET,
    chunk.start + PACKAGE_NAME_OFFSET + PACKAGE_NAME_SLOT_BYTES
  );
  let end = 0;
 ...
 */
export function patchArscPackageName(
  arsc: Buffer,
  packageName: string
): { data: Buffer; previous: string } {
  if (packageName.length > PACKAGE_NAME_MAX_UNITS) {
    throw new Error(
      `Package name exceeds ${PACKAGE_NAME_MAX_UNITS} UTF-16 units: "${packageName}"`
    );
  }
