...
const chainIdentity = {
  ...
  certificateChainDerBase64: [
    ...
    otherIdentity.certificateDerBase64
  ]
};
...
async function buildUnsignedApk(): Promise<Buffer> {
  const entries: ZipWriteEntry[] = [
    ...
    {
      name: "resources.arsc",
      source: { kind: "buffer", data: Buffer.alloc(64, 7) },
      method: "store",
      forceAlign: 4
    },
...
describe("signApkV2 + verifyApkV2", () => {
  ...
  it("embeds the identity's certificate", async () => {
    const signed = signApkV2(await buildUnsignedApk(), identity);
    const result = verifyApkV2(signed);
    expect(result.certificatesDer?.map((der) => der.toString("base64"))).toEqual([
      debugIdentity.certificateDerBase64
    ]);
  });
  ...
  it("embeds a whole chain, leaf first, and still verifies", async () => {
    ...
    expect(result.verified).toBe(true);
    expect(result.certificatesDer?.map((der) => der.toString("base64"))).toEqual(
      chainIdentity.certificateChainDerBase64
    );
  });
  ...
  it("grows the signing block by exactly the extra certificate", async () => {
    ...
    expect(signApkV2(unsigned, chainIdentity).length - signApkV2(unsigned, identity).length).toBe(
      extra + 4
    );
  });
  ...
  it("preserves the original entry bytes ahead of the signing block", async () => {
    ...
    expect(
      signed.subarray(0, centralDirOffset).equals(unsigned.subarray(0, centralDirOffset))
    ).toBe(true);
