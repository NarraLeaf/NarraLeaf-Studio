...
const identity = toApkSigningIdentity(
  ...
  const entries: ZipWriteEntry[] = [
    {
      ...
      source: { kind: "buffer", data: buildBinaryManifestFixture() },
      method: "deflate"
    },
    ...
    {
      name: "resources.arsc",
      source: { kind: "buffer", data: buildArscFixture() },
      method: "store",
      forceAlign: 4
    }
  ];
...
describe("repackApk", () => {
  ...
  it("injects the game site and shell config under the manifest's roots", async () => {
    ...
    expect(readEntryBytes(apk, entryByName(apk, "assets/www/index.html")).toString("utf8")).toBe(
      "<!doctype html><title>g</title>"
    );
    expect(readEntryBytes(apk, entryByName(apk, "assets/www/pack.json")).toString("utf8")).toBe(
      "{}"
    );
    expect(readEntryBytes(apk, entryByName(apk, "assets/shell-config.json")).toString("utf8")).toBe(
  ...
  it("replaces icon slots that were overridden and keeps the rest", async () => {
    ...
    expect(readEntryBytes(apk, entryByName(apk, ICON_SLOT_HDPI)).equals(PLACEHOLDER_ICON)).toBe(
      true
    );
  });
  ...
  it("fails loudly when an icon override matches no template slot", async () => {
    await expect(
      repack({ iconPngBySlot: { "res/mipmap-nope/ic.png": Buffer.from([1]) } })
  ...
  it("only replaces entries the manifest declares as icon slots", async () => {
    ...
    await expect(
      repack({ iconPngBySlot: { "classes.dex": Buffer.from("PWNED") } })
    ).rejects.toThrow(/not present in the template/);
  });
  ...
  it("fails loudly when the template lacks AndroidManifest.xml", async () => {
    ...
    await expect(repack({ templateApk: output.toBuffer() })).rejects.toThrow(
  ...
  it("renames authority-shaped derived strings with the package", async () => {
    ...
    expect(manifest.includes(Buffer.from("com.narraleaf.shell.placeholder", "utf16le"))).toBe(
      false
    );
  });
});
