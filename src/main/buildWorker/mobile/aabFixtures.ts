...
type FixtureAttribute = { ns: number; name: number; rawValue: number; dataType: number; data: number };
...
export function buildProtoManifestFixture(options: ProtoManifestFixtureOptions = {}): Buffer {
    ...
    [0x01010000, 0x01010001, 0x01010003, 0x01010010, 0x0101021b, 0x0101021c]
        .forEach((id, index) => resourceIds.writeUInt32LE(id, index * 4));
    ...
    const body = Buffer.concat([
        ...
        startElement(2, 9, [
            ...
            { ns: 6, name: 4, rawValue: NO_ENTRY, dataType: TYPE_INT_DEC, data: options.versionCode ?? 1 },
...
function mapEntry(key: number, parent: number, values: { name: number; dataType: number; data: number }[]): Buffer {
...
export function buildProtoArscFixture(packageName = "com.narraleaf.shell.placeholder"): Buffer {
    ...
    const drawableMdpi = typeChunk(1, config(buffer => buffer.writeUInt16LE(160, 14)), [
        simpleEntry(0, TYPE_STRING, 0),
    ]);
    const drawableXhdpi = typeChunk(1, config(buffer => buffer.writeUInt16LE(320, 14)), [
        simpleEntry(0, TYPE_STRING, 1),
    ]);
    const stringEnUs = typeChunk(2, config(buffer => buffer.write("enUS", 8, "latin1")), [
        simpleEntry(1, TYPE_STRING, 2),
    ]);
    const styleDefault = typeChunk(3, config(() => undefined), [
    ...
    const packageBody = Buffer.concat([
        typePool, keyPool,
        typeSpec(1, 1), drawableMdpi, drawableXhdpi,
        typeSpec(2, 1), stringEnUs,
        typeSpec(3, 1), styleDefault,
    ]);
