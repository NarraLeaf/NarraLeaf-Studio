...
describe("patchInfoPlist", () => {
  ...
  it("throws when a targeted key is absent (template drift)", () => {
    ...
    expect(() => patchInfoPlist(withoutVersion, { bundleVersion: "2" })).toThrow(
      /no top-level <string> value for CFBundleVersion/
    );
  });

  it("throws when no orientation array exists", () => {
    const withoutOrientation = TEMPLATE.replace(
      /\t<key>UISupportedInterfaceOrientations<\/key>\n\t<array>[\s\S]*?<\/array>\n/,
      ""
    ).replace(
      /\t<key>UISupportedInterfaceOrientations~ipad<\/key>\n\t<array>[\s\S]*?<\/array>\n/,
    ...
    expect(() => patchInfoPlist(withoutOrientation, { orientation: "landscape" })).toThrow(
      /no UISupportedInterfaceOrientations array/
    );
  });
});
...
${body}
  ...
  it("reads every scalar type Apple writes", () => {
    const value = parsePlistDictionary(
      document(`<dict>
  ...
  it("unescapes entities in strings", () => {
    const value = parsePlistDictionary(
      document("<dict><key>k</key><string>a &amp; b &lt;c&gt; &quot;d&quot; &#65;</string></dict>")
    );
  ...
  it("refuses a document whose root is not a dict when a dict is required", () => {
    expect(() => parsePlistDictionary(document("<array><string>x</string></array>"))).toThrow(
      /not a dictionary/
    );
  });
  ...
  it("reports malformed structure instead of guessing", () => {
    ...
    expect(() => parsePlist(document("<dict><string>no key</string></dict>"))).toThrow(
      /Expected a <key>/
    );
    expect(() => parsePlist(document("<dict><key>k</key><integer>oops</integer></dict>"))).toThrow(
      /Malformed <integer>/
    );
    expect(() => parsePlist(document("<dict><key>k</key><date>never</date></dict>"))).toThrow(
      /Malformed <date>/
    );
    expect(() => parsePlist("<plist><dict><key>k</key><string>x</string>")).toThrow(
      /Unclosed <dict>/
    );
  });
});
