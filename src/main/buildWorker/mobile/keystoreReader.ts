...
function readAlgorithmIdentifier(node: Asn1): { oid: string; parameters?: Asn1 } {
  ...
  return {
    oid: decodeOid(expectTag(parts[0], TAG_OID, "an algorithm name")),
    parameters: parts[1]
  };
}
...
function decryptPbes2(
  ...
  const kdf = readAlgorithmIdentifier(
    expectTag(parts[0], TAG_SEQUENCE, "the key derivation settings")
  );
  ...
  const iterations = decodeInteger(
    expectTag(kdfParts[1], TAG_INTEGER, "the password iteration count")
  );
...
function collectSafeBags(
  ...
  for (const bag of items(
    expectTag(readOnlyAsn1(safeContentsDer), TAG_SEQUENCE, "a keystore section")
  )) {
    ...
    if (bagId === OID_SHROUDED_KEY_BAG) {
      ...
      if (
        decodeOid(expectTag(certParts[0], TAG_OID, "a certificate type")) !== OID_X509_CERTIFICATE
      ) {
...
function decryptJksKey(encryptedKey: Buffer, keyPassword: string): Buffer | null {
  ...
  const algorithm = readAlgorithmIdentifier(
    expectTag(outer[0], TAG_SEQUENCE, "a key entry algorithm")
  );
...
export function readKeystore(file: Buffer, options: ReadKeystoreOptions): KeystoreIdentity {
  ...
  if (format === "jks") {
    ...
    const entry =
      entries[
        pickAlias(
          entries.map((candidate) => candidate.alias),
          options.alias
        )
      ];
