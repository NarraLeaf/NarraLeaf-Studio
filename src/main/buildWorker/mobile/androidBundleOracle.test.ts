...
const DEFAULT_WWW: BuildAabInput["www"] = [
    { relativePath: "index.html", source: { kind: "buffer", data: Buffer.from("<!doctype html><title>mobile</title>") } },
...
describe("the App Bundle oracle's availability", () => {
    it("is present when CI demands it", () => {
        ...
        // exists to prevent.
        if (oracleRequired) {
            expect(bundletoolJar, "REQUIRE_ANDROID_BUNDLE_ORACLE is set but BUNDLETOOL_JAR names no jar").not.toBeNull();
            expect(aapt2Path, "REQUIRE_ANDROID_BUNDLE_ORACLE is set but no aapt2 was found").not.toBeNull();
        }
    });
});
...
describe.skipIf(!oracleAvailable)("Google's tools on a Studio-built App Bundle", () => {
    ...
    it("builds installable APKs whose payload and identity survive", async () => {
        ...
        bundletool([
            "build-apks", `--bundle=${await buildBundle()}`, `--output=${apks}`, "--mode=universal", "--overwrite",
        ]);
        ...
        expect(entryBytes(universal, "assets/www/index.html").toString("utf8"))
            .toBe("<!doctype html><title>mobile</title>");
        expect(entryBytes(universal, "assets/www/js/app.js").toString("utf8")).toBe("/* bridge */");
        expect(entryBytes(universal, "assets/shell-config.json").toString("utf8")).toContain("\"orientation\":\"landscape\"");
    ...
    it("reads back the manifest Studio wrote", async () => {
        ...
        expect(manifest).toContain("<uses-sdk android:minSdkVersion=\"26\" android:targetSdkVersion=\"34\"/>");
    ...
    it("reads back every resource Studio wrote, with its values", async () => {
        const resources = bundletool(["dump", "resources", `--bundle=${await buildBundle()}`, "--values"]);
        ...
        for (const [density, directory] of [
            [160, "mdpi"], [240, "hdpi"], [320, "xhdpi"], [480, "xxhdpi"], [640, "xxxhdpi"],
        ] as const) {
            expect(resources).toContain(`density: ${density} - [FILE] res/mipmap-${directory}-v4/ic_launcher.png`);
        }
    ...
    it("converts the manifest to the same thing aapt2 does", async () => {
        ...
        expect(summarizeXmlNode(convertBinaryManifestToProto(entryBytes(template, "AndroidManifest.xml"))))
    ...
    it("converts the resource table to the same thing aapt2 does", async () => {
        ...
        const template = await fs.readFile(templateApk);
        const theirs = await fs.readFile(protoApk);
        expect(summarizeResourceTable(convertArscToProto(entryBytes(template, "resources.arsc"))))
