  ...
  const walk = async (dir: string, prefix: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        ...
        throw new Error(
          `The compiled site contains a symlink ("${relativePath}"), which cannot be packaged`
        );
      }
      if (!entry.isFile()) {
        continue;
...
async function siteEntries(
  ...
  const overrideEntry: SiteEntry = {
    ...
    source: {
      kind: "buffer",
      data: contentKey ? encryptBuffer(overrideBytes, contentKey) : overrideBytes
    }
  };
