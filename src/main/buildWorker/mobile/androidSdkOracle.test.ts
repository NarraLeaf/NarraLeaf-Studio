...
function tool(name: string, args: string[]): string {
  return execFileSync(path.join(buildTools!, name), args, {
    encoding: "utf8",
...
describe("the Android SDK oracle's availability", () => {
  it("is present when CI demands it", () => {
    ...
    if (oracleRequired) {
      expect(
        buildTools,
        "REQUIRE_ANDROID_SDK_ORACLE is set but no Android build-tools were found"
      ).not.toBeNull();
    }
  });
});
...
describe.skipIf(!buildTools)("Google's tools on a Studio-built APK", () => {
  ...
  it("rejects the same APK with one payload byte flipped", async () => {
    ...
    expect(() =>
      tool("apksigner", ["verify", "--min-sdk-version", String(MIN_SDK), tampered])
    ).toThrow();
  });
  ...
  it("attributes a release-signed APK to the author's own certificate", async () => {
    ...
    expect(output.toLowerCase()).toContain(
      `certificate sha-256 digest: ${ANDROID_RELEASE_P12_SHA256.replace(/:/g, "").toLowerCase()}`
    );
