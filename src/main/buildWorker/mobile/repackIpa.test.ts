...
async function buildTemplateAppZip(): Promise<Buffer> {
  const entries: ZipWriteEntry[] = [
    { name: "Shell.app/", source: null },
    {
      ...
      source: { kind: "buffer", data: EXECUTABLE_BYTES },
      unixMode: 0o755,
      method: "store"
    },
    {
      name: "Shell.app/Info.plist",
      source: { kind: "buffer", data: Buffer.from(TEMPLATE_INFO_PLIST) }
    },
    {
      name: "Shell.app/AppIcon60x60@2x.png",
      source: { kind: "buffer", data: PLACEHOLDER_ICON },
      method: "store"
    },
    { name: "Shell.app/LaunchScreen.storyboardc/", source: null },
    {
      name: "Shell.app/LaunchScreen.storyboardc/Info.plist",
      source: { kind: "buffer", data: Buffer.from("<plist/>") }
    }
  ];
...
describe("repackIpa", () => {
  ...
  it("patches the Info.plist identity, display name and orientation", async () => {
    const ipa = await repack();
    const plist = readEntryBytes(ipa, entryByName(ipa, "Payload/My Game.app/Info.plist")).toString(
      "utf8"
    );
  ...
  it("fails loudly when an icon override matches no template slot (manifest drift)", async () => {
    await expect(
      repack({ iconPngBySlot: { "does-not-exist.png": Buffer.from([1]) } })
  ...
  it("rejects a template containing a symlink (would be silently corrupted)", async () => {
    ...
    await writeZip(
      ...
      [
        ...
        {
          name: "Shell.app/link",
          source: { kind: "buffer", data: Buffer.from("Shell") },
          unixMode: 0o120755
        }
      ],
...
describe("repackIpa 7za validation", () => {
  ...
  it("produces an archive 7za verifies and extracts with the expected layout", async () => {
    ...
    const indexHtml = await fs.readFile(
      path.join(outDir, "Payload", "My Game.app", "www", "index.html"),
      "utf8"
    );
    expect(indexHtml).toContain("<!doctype html>");
    const info = await fs.readFile(
      path.join(outDir, "Payload", "My Game.app", "Info.plist"),
      "utf8"
    );
