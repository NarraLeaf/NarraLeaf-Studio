...
const FORBIDDEN_IMPORTS = [
  ...
  },
  {
    pattern: /^@\/apps(?:\/|$)/,
    reason:
      "Runtime renderer must use shared renderer runtime modules instead of app-specific UI code."
  }
];
  ...
  it("does not import Studio workspace, bridge, layout, or plugin modules", async () => {
    ...
    for (const filePath of await listRuntimeSourceFiles(RUNTIME_ROOT)) {
      ...
      for (const match of source.matchAll(STATIC_IMPORT_PATTERN)) {
        ...
        if (forbidden) {
          violations.push(
            `${path.relative(RUNTIME_ROOT, filePath)} imports ${specifier}: ${forbidden.reason}`
          );
        }
      }
    }
