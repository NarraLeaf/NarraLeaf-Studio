...
type SidecarUnavailableListener = (info: {
  pluginId: string;
  sidecarId: string;
  reason: string;
}) => void;
...
function subscribeSidecar<T>(registry: Map<string, Set<T>>, key: string, listener: T): () => void {
...
ipcRenderer.on(
  GAME_RUNTIME_SIDECAR_MESSAGE_CHANNEL,
  (_event, message: GameRuntimeSidecarMessage) => {
    if (!message || typeof message !== "object") {
      return;
    }
    const key = sidecarKey(message.pluginId, message.sidecarId);
    if (message.kind === "event") {
      dispatchSidecar(sidecarEventListeners.get(key), (listener) =>
    ...
    dispatchSidecar(sidecarUnavailableListeners, (listener) =>
      listener({
...
const bridge: GameRuntimePreloadBridge & GameRuntimeTestSignalBridge = {
  ...
  assetUrl: (assetId: string) =>
    `${GAME_RUNTIME_PROTOCOL}://asset/${String(assetId ?? "")
      .split("/")
      .map(encodeURIComponent)
      .join("/")}` + `?v=${encodeURIComponent(assetVersion)}`,
