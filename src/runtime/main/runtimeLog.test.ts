...
describe("formatRuntimeLogLine", () => {
    it("writes a sortable timestamp, the level, and the message", () => {
        const line = formatRuntimeLogLine("warning", "Asset preload timed out", new Date(Date.UTC(2026, 7, 11, 9, 30, 0)));
...
describe("installRuntimeLogSink", () => {
    ...
    it("writes to a file that outlives the session", () => {
        ...
        expect(fs.readFileSync(runtimeLogPath(dir), "utf-8")).toContain("[ERROR] Blueprint execution failed");
    });
