...
const HANG_DETAIL =
  "Restarting reopens the game at its title screen. Progress since the last save is lost.";
...
export function installWindowCrashHandling(win: BrowserWindow, host: WindowCrashHost): void {
  ...
  const recoverDeadRenderer = async (reason: string, exitCode: number): Promise<void> => {
    ...
    if (isCrashLooping(crashHistory)) {
      host.log(
        "error",
        "[Crash] The game window has stopped working repeatedly; not reloading it again."
      );
  ...
  const offerHangReload = async (): Promise<void> => {
    ...
    try {
      ...
      host.log(
        "warning",
        `[Crash] Could not ask about restarting the hung window: ${describe(error)}`
      );
    } finally {
