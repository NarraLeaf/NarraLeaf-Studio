...
import { RuntimePersistenceStore, RuntimeSaveStore } from "./runtimeStorage";
...
function readShellManifest(): { mode: "preview" | "production"; userDataDirName: string | null } {
  try {
    const manifest = JSON.parse(
      fsSync.readFileSync(path.join(appDir, "package.json"), "utf-8")
    ) as {
    ...
    return {
      ...
      userDataDirName:
        typeof userDataDir === "string" && userDataDir.trim() ? userDataDir.trim() : null
    };
  } catch {
...
const userDataDir = useSiblingUserData
  ...
  : resolveRuntimeUserDataDir(shellManifest.userDataDirName, {
      ...
      warn: (message) => {
        console.warn(`[GameRuntime] ${message}`);
      }
    });
...
app.on("before-quit", (event) => {
  ...
  if (!runtimeQuitDrained) {
    const pendingWrites =
      saveStore?.hasPendingWrites() === true || persistenceStore?.hasPendingWrites() === true;
      ...
      if (url.hostname === "runtime") {
        ...
        if (isIndexDocument(pathname)) {
          return serveIndexDocument(
            resolveRuntimeStaticPath(appDir, pathname),
            allowHttp,
            allowlist
          );
        }
...
function registerRuntimeIpc(): void {
  ...
  ipcMain.on(
    GAME_RUNTIME_CLOSE_DECISION_CHANNEL,
    (_event, payload: { requestId?: number; allow?: boolean }) => {
  ...
  ipcMain.handle(
    "runtime:save:write",
    (_event, data: { id: string; savedGame: unknown; capture?: string; metadata?: unknown }) =>
  ...
  ipcMain.handle("runtime:persistence:getValue", (_event, key: string) =>
    persistence.getValue(key)
  );
  ipcMain.handle("runtime:persistence:setValue", (_event, key: string, value: unknown) =>
    persistence.setValue(key, value)
  );
  ipcMain.handle("runtime:persistence:removeValue", (_event, key: string) =>
    persistence.removeValue(key)
  );
  ...
  ipcMain.handle("runtime:progress:read", async () => {
    const pack = await readPack();
    const result = await readGameProgressFile(
      progressEnvironment(),
      String(pack.progressKey ?? "")
    );
    if (result.outcome === "failed") {
