  ...
  it("treats a pack with no sidecar fields as a pack with no sidecars", () => {
    const pack = {
      plugins: [
        ...
        {
          manifest: { id: "b" },
          entryRelativePath: "plugins/b/runtime.js",
          sidecars: [{ id: "b.one" }]
        }
      ]
    } as never;
    expect(collectPackSidecars(pack).map((entry) => `${entry.pluginId}/${entry.id}`)).toEqual([
      "b/b.one"
    ]);
  });
});
...
describe("SidecarHost", () => {
  ...
  it("hand-shakes with hello/ready before the start resolves", async () => {
    ...
    expect(spawned[0]!.command).toBe(
      path.resolve("/app", `sidecars/${PLUGIN_ID}/${SIDECAR_ID}/bin/tool`)
    );
  ...
  it("routes replies to their own requests, whatever order they arrive in", async () => {
    ...
    const idOf = (method: string) =>
      requests.find((frame) => frame.method === method)!.id as number;
  ...
  it("rejects a request the sidecar answers with an error frame", async () => {
    ...
    children[0]!.sendFrame({
      t: "res",
      id,
      error: { message: "no such achievement", code: "ENOENT" }
    });
  ...
  it("forwards stderr as log lines, dropping chatter in production", async () => {
    ...
    expect(
      preview.logs.some(
        (entry) => entry.level === "info" && entry.message.includes("just chatting")
      )
    ).toBe(true);
    expect(
      preview.logs.some((entry) => entry.level === "warning" && entry.message.includes("slow disk"))
    ).toBe(true);
    ...
    expect(
      production.logs.some((entry) => entry.level === "error" && entry.message.includes("broke"))
    ).toBe(true);
  });
  ...
  it("rejects every in-flight request when the process dies", async () => {
    ...
    const first = expect(host.request(PLUGIN_ID, SIDECAR_ID, "first")).rejects.toThrow(
      /exited \(code=3/
    );
    const second = expect(host.request(PLUGIN_ID, SIDECAR_ID, "second")).rejects.toThrow(
      /exited \(code=3/
    );
    children[0]!.sendFrame({ t: "ready" });
    await vi.advanceTimersByTimeAsync(0);
  ...
  it("refuses a sidecar this build never shipped", async () => {
    ...
    await expect(host.start(PLUGIN_ID, "acme.tools.other")).rejects.toThrow(
      /never shipped|No sidecar/
    );
