...
describe("runtime protocol path resolution", () => {
  ...
  it("rejects static and generic paths that escape the app directory", () => {
    ...
    expect(() => resolveRuntimeStaticPath(appDir, "/../../outside.js")).toThrow(
  ...
  it("resolves manifest assets and rejects missing or escaping entries", () => {
    ...
    expect(resolveRuntimeAssetPath(appDir, pack, "image")).toBe(
      path.join(appDir, "assets", "image.png")
    );
    expect(() => resolveRuntimeAssetPath(appDir, pack, "missing")).toThrow(
