...
class SidecarInstance {
  ...
  public start(): Promise<void> {
    if (this.unavailableReason) {
      return Promise.reject(
        new Error(`Sidecar "${this.label}" is unavailable: ${this.unavailableReason}`)
      );
    }
    if (this.state === "ready") {
  ...
  private spawnAndHandshake(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      try {
        ...
        reject(
          new Error(
            `Sidecar "${this.label}": could not create its working directory: ${describeError(error)}`
          )
        );
        return;
      }
  ...
  private wireStreams(child: SidecarChildProcess): void {
    ...
    const stdoutFramer = new NdjsonDecoder(MAX_FRAME_LENGTH, (dropped) => {
      this.deps.log(
        "warning",
        `sidecar ${this.label}: dropped ${dropped} bytes of an over-long protocol line`
      );
    });
  ...
  private handleFrame(line: string): void {
    ...
    try {
      ...
      this.deps.log(
        "warning",
        `sidecar ${this.label}: ignored a non-JSON stdout line: ${truncate(line)}`
      );
      return;
    }
    if (!frame || typeof frame !== "object") {
      this.deps.log(
        "warning",
        `sidecar ${this.label}: ignored a non-object frame: ${truncate(line)}`
      );
      return;
    }
  ...
  private handleReady(record: Record<string, unknown>): void {
    ...
    if (typeof protocol === "number" && protocol !== SIDECAR_PROTOCOL_VERSION) {
      this.failStart(
        `speaks protocol ${protocol}, but this game hosts protocol ${SIDECAR_PROTOCOL_VERSION}`
      );
      return;
    }
    ...
    this.deps.log(
      "info",
      `sidecar ${this.label}: ready${caps.length > 0 ? ` (caps: ${caps.join(", ")})` : ""}`
    );
  ...
  public notify(method: string, params?: unknown): void {
    ...
    void this.start()
      ...
      .catch((error) => {
        this.deps.log(
          "warning",
          `sidecar ${this.label}: dropped notify "${method}": ${describeError(error)}`
        );
      });
  }
  ...
  private chargeFailure(reason: string, options: { scheduleRestart: boolean }): void {
    ...
    const delay = Math.min(
      Math.max(0, backoffMs) * 2 ** (this.restartAttempts - 1),
      MAX_BACKOFF_MS
    );
    this.deps.log(
      "info",
  ...
  private markUnavailable(reason: string): void {
    ...
    this.deps.log(
      "error",
      `sidecar ${this.label}: giving up after ${this.restartAttempts} restart(s) (${reason})`
    );
    this.deps.send({
      kind: "unavailable",
      pluginId: this.declaration.pluginId,
  ...
  private resolveEntryPath(): string | null {
    ...
    try {
      ...
      if (!resolved) {
        this.deps.log(
          "warning",
          `sidecar ${this.label}: entry is missing from this build (${this.declaration.entry})`
        );
      }
    } catch (error) {
      this.deps.log(
        "error",
        `sidecar ${this.label}: entry path is not inside the app dir: ${describeError(error)}`
      );
    }
  ...
  private ensureExecutable(entryPath: string): void {
    ...
    const repaired =
      mode | 0o100 | ((mode & 0o040) === 0 ? 0 : 0o010) | ((mode & 0o004) === 0 ? 0 : 0o001);
...
export class SidecarHost {
  ...
  public notify(pluginId: string, sidecarId: string, method: string, params?: unknown): void {
    ...
    if (!instance) {
      this.deps.log(
        "warning",
        `sidecar ${pluginId}/${sidecarId}: notify dropped, no such sidecar in this build`
      );
      return;
    }
