...
export function collectRuntimeSurfaceAssetIds(
...
export function collectRuntimePackAssetIds(
  pack: GameRuntimePackV1,
...
export async function preloadRuntimeSurfaceAssets(input: {
  ...
  await Promise.race([
    preloadAll,
    new Promise((resolve) =>
      setTimeout(resolve, input.timeoutMs ?? RUNTIME_SURFACE_PRELOAD_TIMEOUT_MS)
    )
  ]);
...
export async function preloadRuntimePackAssets(input: {
  ...
  const { firstSurfaceAssetIds, assetIds } = collectRuntimePackAssetIds(
    input.pack,
    input.firstSurface
  );
