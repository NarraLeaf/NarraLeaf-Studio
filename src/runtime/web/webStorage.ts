...
export class WebGameStorage {
  ...
  public async deleteSave(id: string): Promise<{ deleted: boolean }> {
    const normalizedId = normalizeRuntimeSaveId(id);
    const existingKey = await this.run(SAVES_STORE, "readonly", (store) =>
      store.getKey(normalizedId)
    );
  ...
  private openDb(): Promise<IDBDatabase> {
    this.dbPromise ??= new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(request.error ?? new Error("Failed to open the game save database"));
    });
