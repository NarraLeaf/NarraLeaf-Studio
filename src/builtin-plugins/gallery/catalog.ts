...
export function toImageAssetValue(
  assetId: string | null | undefined
): GalleryImageAssetValue | null {
...
function normalizeSettings(value: unknown): GallerySettings {
  ...
  return {
    ...
    lockedNameMask:
      typeof record.lockedNameMask === "string" ? record.lockedNameMask : DEFAULT_LOCKED_NAME_MASK
  };
}
...
function lockedPlaceholderAssetId(
...
export function projectGalleryEntries(
  ...
  for (const artwork of store.items) {
    ...
    views.push({
      ...
      id: artwork.id,
      name:
        isUnlocked || !store.settings.lockedNameMask ? artwork.name : store.settings.lockedNameMask,
...
export function projectGalleryVariants(
  ...
  for (const variant of artwork.variants) {
    ...
    views.push({
      ...
      name:
        isUnlocked || !store.settings.lockedNameMask ? variant.name : store.settings.lockedNameMask,
      unlocked: isUnlocked,
      locked: !isUnlocked,
      image: toImageAssetValue(assetId),
      assetId: assetId ?? "",
