...
async function writeStoredUnlocked(ctx: ExecuteCtx, variantIds: string[]): Promise<void> {
  if (!ctx.game.store) {
    ctx.game.log(
      "warning",
      "gallery unlocks are not persisted here: plugin storage is unavailable"
    );
    return;
  }
...
export function createGalleryBlueprintNodes(
  readCatalog: GalleryCatalogReader
  ...
  return [
    ...
    // ---------------------------------------------------------------
    {
      ...
      isPure: false,
      isLatent: true,
      pins: [execIn, groupIdIn, onlyUnlockedIn, execNext, entriesOut, countOut, unlockedCountOut],
    ...
    },
    {
      ...
      isPure: false,
      isLatent: true,
      pins: [execIn, artworkIdIn, onlyUnlockedIn, execNext, entriesOut, countOut, unlockedCountOut],
    ...
    },
    {
      ...
      execute: async (ctx) => {
        ...
        return {
          nextPort: "next",
          outputValues: {
            unlocked: variantId ? unlocked.has(variantId) : isArtworkUnlocked(artwork, unlocked)
          }
        };
      }
    },
