...
describe("unlock / lock nodes", () => {
  ...
  it("rejects an artwork that is not picked or not found", async () => {
    ...
    await expect(run(V2_CATALOG, `${P}.add`, { galleryItemId: "nope" })).rejects.toThrow(
      /not found/
    );
  });
});
...
describe("degradation", () => {
  ...
  it("reads as locked and swallows writes when plugin storage is unavailable", async () => {
    ...
    const read = (await defs
      .get(`${P}.isUnlocked`)!
      .execute(ctxWithoutStore({ galleryItemId: "art.a" }))) as {
      outputValues?: Record<string, unknown>;
    };
    expect(read.outputValues?.unlocked).toBe(false);

    await expect(
      defs.get(`${P}.add`)!.execute(ctxWithoutStore({ galleryItemId: "art.a" }))
...
const V3_STORE = {
  ...
  items: [
    ...
    },
    {
      id: "art.b",
      name: "Beach",
      variants: [{ id: "art.b.v.1", name: "Only", imageAssetId: "asset-beach" }]
    },
    {
      id: "art.secret",
      name: "True End",
