...
describe("resolveCoverVariant", () => {
  ...
  it("prefers the explicit cover", () => {
    const result = resolveCoverVariant(
      artwork({ variants: [first, second], coverVariantId: "v2" })
    );
  ...
  it("falls back to the first variant when no cover is set", () => {
    const result = resolveCoverVariant(
      artwork({ variants: [first, second], coverVariantId: null })
    );
...
describe("normalizeGalleryStore", () => {
  ...
  it("keeps an explicitly empty mask, so a gallery can show real titles while locked", () => {
    ...
    expect(
      normalizeGalleryStore({ settings: { lockedNameMask: "" } }).settings.lockedNameMask
    ).toBe("");
    expect(normalizeGalleryStore({ settings: {} }).settings.lockedNameMask).toBe(
      DEFAULT_LOCKED_NAME_MASK
    );
  });
});
...
describe("projectGalleryEntries", () => {
  ...
  it("falls back to the catalog placeholder when the artwork has none", () => {
    const bare = artwork({
      id: "bare",
      variants: [{ id: "bare.v.1", name: "A", imageAssetId: "real" }]
    });
    const store = storeOf({
      items: [bare],
    ...
    const store = storeOf({
      ...
      items: [
        artwork({
          id: "a",
          groupId: "g1",
          variants: [{ id: "a.v", name: "A", imageAssetId: "x" }]
        }),
        artwork({ id: "b", variants: [{ id: "b.v", name: "B", imageAssetId: "y" }] })
      ]
    });

    expect(projectGalleryEntries(store, new Set(), { groupId: "g1" }).map((r) => r.id)).toEqual([
      "a"
    ]);
    expect(
      projectGalleryEntries(store, new Set(["b.v"]), { onlyUnlocked: true }).map((r) => r.id)
    ).toEqual(["b"]);
  });
...
describe("projectGalleryVariants", () => {
  ...
  it("marks the cover and can drop locked differentials", () => {
    ...
    expect(projectGalleryVariants(store, target, new Set()).map((r) => r.isCover)).toEqual([
      false,
      true
    ]);
    expect(
      projectGalleryVariants(store, target, new Set(["art.v.1"]), { onlyUnlocked: true })
    ).toHaveLength(1);
  });
});
...
describe("v4 kinds", () => {
  ...
  it("carries the line text onto an unlocked voice member and withholds it when locked", () => {
    ...
    expect(projectGalleryVariants(store, line, new Set(["vo.v.1"]))[0]).toMatchObject({
      lineText: "Good morning!",
      voiceUnitId: "text-uuid-1"
    });
  ...
  it("filters entries and progress by kind", () => {
    ...
    expect(projectGalleryEntries(store, new Set(), { kind: "music" }).map((r) => r.id)).toEqual([
      "album"
    ]);
    expect(projectGalleryEntries(store, new Set(), { kind: "cg" }).map((r) => r.id)).toEqual([
      "cg"
    ]);
    expect(computeGalleryStats(store, new Set(["album.v.1"]), { kind: "music" })).toMatchObject({
      total: 1,
      unlocked: 1,
      percent: 100
    });
    expect(computeGalleryStats(store, new Set(["album.v.1"]), { kind: "voice" })).toMatchObject({
      total: 1,
      unlocked: 0,
      percent: 0
    });
