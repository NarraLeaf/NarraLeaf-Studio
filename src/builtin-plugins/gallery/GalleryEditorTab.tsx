...
export function GalleryEditorTab({ app, store }: { app: PluginApp; store: GalleryStore }) {
  ...
  const selected =
    selection.length === 1 ? (data.items.find((item) => item.id === selection[0]) ?? null) : null;

  const itemCount = data.items.reduce((total, item) => total + item.variants.length, 0);
  const groupForNew =
    groupFilter === GROUP_ALL || groupFilter === GROUP_UNGROUPED ? null : groupFilter;
  ...
  const onPickerConfirm = (assets: Asset[]) => {
    ...
    switch (target.kind) {
      case "importCgs":
        void run(async () =>
          setSelectedIds((await store.importArtworks(assets, groupForNew)).slice(-1))
        );
        return;
      case "importTracks":
      ...
      case "variantImage":
        void run(() =>
          store.patchVariant(target.artworkId, target.variantId, {
      ...
      case "lockedImage":
        void run(() =>
          store.patchArtworkFields(target.artworkId, {
  ...
  const pickerWantsMany =
    picker?.kind === "importCgs" ||
    ...
    <div className="flex h-full min-h-0 flex-col bg-surface text-fg">
      <header className="shrink-0 border-b border-edge">
        <div className="flex items-center gap-2 px-3 pt-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            />
            <ui.Button size="sm" variant="primary" onClick={primaryAction} {...freeze.writes(busy)}>
              <Plus size={13} />
              {meta.createLabel}
            </ui.Button>
      ...
      <GroupChips
        ...
        onDropEntry={(entryId, groupId) =>
          void run(() => store.patchArtworkFields(entryId, { groupId }))
        }
      />
              ...
              {visible.map((item) => (
                <EntryCard
                  ...
                  onDropBefore={(draggedId) =>
                    void run(() => store.moveArtwork(draggedId, item.id))
                  }
                />
              ))}
            </div>
  ...
  const chip = (
    ...
    >
      <button
        ...
        onDoubleClick={
          options.groupId ? freeze.gesture(() => setRenamingId(options.groupId!)) : undefined
        }
      >
        {label}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 px-1.5 py-1">
        <span className="min-w-0 flex-1 truncate text-2xs" data-tip={entry.name}>
          {entry.name}
        </span>
      ...
      {entry.kind === "scene" && (
        <div
          className={`truncate border-t border-edge px-1.5 py-0.5 text-2xs ${
...
function EntryRow({
  ...
  const totalSeconds = entry.variants.reduce(
    (total, variant) => total + (variant.durationSec ?? 0),
    0
  );
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-2xs" data-tip={entry.name}>
          {entry.name}
        </div>
  ...
  return (
    <div className="flex flex-col gap-3 p-3">
        ...
        <ui.TextArea
          ...
          onBlur={(event) => {
            if (!freeze.frozen && event.target.value !== entry.description) {
              void onRun(() =>
                store.patchArtworkFields(entry.id, {
      ...
      <button
        ...
        onClick={() =>
          void onRun(() => store.patchArtworkFields(entry.id, { hidden: !entry.hidden }))
        }
        {...freeze.writes(busy)}
      >
...
function MemberList({
  ...
  const label =
    entry.kind === "music"
      ? many
        ? `Tracks (${entry.variants.length})`
        : "Track"
      : entry.kind === "voice"
        ? many
          ? `Lines (${entry.variants.length})`
          : "Line"
        : many
          ? `Differentials (${entry.variants.length})`
          : "Image";
        ...
        <div className="space-y-1">
          {entry.variants.map((variant) => (
            <MemberRow
              ...
              onRename={(name) =>
                void onRun(() => store.patchVariant(entry.id, variant.id, { name }))
              }
              onPickImage={() =>
                onPick({
                  kind: "variantImage",
...
function VoiceUnitPicker({
  ...
  const taken = useMemo(
    () =>
      new Set(
        existing.map((variant) => variant.voiceUnitId).filter((id): id is string => Boolean(id))
      ),
    [existing]
  );
  ...
  return (
    <ui.Modal isOpen title="Add voice lines" onClose={onClose}>
      ...
      <ui.ModalFooter>
        ...
        <ui.Button size="sm" variant="secondary" onClick={onClose}>
          Cancel
        </ui.Button>
        <ui.Button
          size="sm"
...
function LockedLookModal({
  ...
  return (
    ...
    <ui.Modal isOpen title="How locked entries look in game" onClose={onClose}>
        ...
        <div className="space-y-4">
          ...
          <Field label="Locked title">
            <InlineNameInput
              ...
              onCommit={(lockedNameMask) =>
                void onRun(() => store.patchSettings({ lockedNameMask }))
              }
            />
          </Field>
      ...
      <ui.ModalFooter>
        <ui.Button size="sm" variant="primary" onClick={onClose}>
          Done
        </ui.Button>
      </ui.ModalFooter>
    </ui.Modal>
