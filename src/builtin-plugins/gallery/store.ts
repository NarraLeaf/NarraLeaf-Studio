...
export function createGalleryStore(app: PluginApp) {
  ...
  const patchArtwork = (artworkId: string, patch: (artwork: GalleryArtwork) => GalleryArtwork) =>
    commitItems(
      data.items.map((artwork) =>
        artwork.id === artworkId ? { ...patch(artwork), updatedAt: Date.now() } : artwork
      )
    );
  ...
  return {
    ...
    async patchArtworkFields(
      artworkId: string,
      patch: Partial<Omit<GalleryArtwork, "id" | "variants">>
    ) {
      ...
      await patchArtwork(artworkId, (artwork) => ({
        ...
        variants: [
          ...
          ...assets.map((asset, index) =>
            variantFromAsset(artwork.id, asset, `Variant ${artwork.variants.length + index + 1}`)
          )
        ]
      }));
    },
    ...
    async addAudioVariants(artworkId: string, assets: Asset[]) {
      await patchArtwork(artworkId, (artwork) => ({
        ...
        variants: [
          ...artwork.variants,
          ...assets.map((asset, index) =>
            variantFromAudio(artwork.id, asset, `Track ${artwork.variants.length + index + 1}`)
          )
        ]
      }));
    },
    ...
    async importTracks(assets: Asset[], groupId: string | null = null): Promise<string> {
      ...
      artwork.variants = assets.map((asset, index) =>
        variantFromAudio(artwork.id, asset, `Track ${index + 1}`)
      );
