                ...
                <ul className="flex flex-col gap-2">
                  {view.rows.map((row) => (
                    <li key={row.key} className={rowClass}>
                      {row.kind === "page" ? <PageRow row={row} /> : <LayerRow row={row} />}
                    </li>
                  ))}
                </ul>
              </section>
...
function LayerRow(props: { row: CompositeStackLayerRow }): ReactNode {
  ...
  return (
    <>
      ...
      {row.onScreen ? null : <div className="text-warning">{t("devMode.layers.offScreen")}</div>}
      <InputFacts interactive={row.interactive} keyboardOwner={row.keyboardOwner} />
      {row.owner ? (
  ...
  return (
    <>
      ...
      <RowFacts>
        ...
        <span>
          {row.group ? t("devMode.layers.group", { group: row.group }) : t("devMode.layers.queued")}
        </span>
      </RowFacts>
      {row.owner ? (
        <div className="truncate text-fg-subtle">
