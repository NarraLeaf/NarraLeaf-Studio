...
function DevModeDebugOverlay(props: {
  ...
  useEffect(() => {
    ...
    const flush = (): void => {
      ...
      const blockId = blockIdForActionId(
  ...
  const handleTitleBarPointerDown = useCallback(
    (event: ReactPointerEvent<HTMLElement>) => {
      ...
      if (
        target?.closest(
          'button, [role="button"], [role="switch"], select, input, textarea, [role="combobox"], [role="listbox"]'
        )
      ) {
        return;
      }
        ...
        <div className="pointer-events-auto absolute bottom-3 left-3">
          <div className="relative flex w-11 flex-col items-start">
              ...
              >
                <button
                  ...
                  className={`flex w-full cursor-default items-center gap-2 px-3 py-2 text-left text-xs transition-colors ${
                    fastForwarding ? "text-fg-subtle" : "text-fg-muted hover:bg-fill hover:text-fg"
                  }`}
                  onClick={() => {
                    void handleFastForward();
                  }}
            ...
            <button
              ...
              aria-label={
                devtoolsMenuOpen ? t("devMode.devtools.closeMenu") : t("devMode.devtools.openMenu")
              }
...
export function DevModeContent(props: DevModeContentProps) {
  ...
  const projectRef = useMemo<
    (BlueprintPersistenceProjectRef & DevModeSaveProjectRef) | null
  >(() => {
    if (!projectPath) {
      return null;
    }
   ...
   */
  const [acknowledgedKeys, setAcknowledgedKeys] =
    useState<ReadonlySet<string>>(NO_ACKNOWLEDGED_KEYS);
  ...
  const pendingSessionError =
    sessionError !== null && sessionError !== acknowledgedSessionError ? sessionError : null;
  ...
  const resolveStoryAssetUrl = useCallback<GameAppHost["resolveStoryAssetUrl"]>(
   ...
   * one's grants.
   */
  useEffect(() => registerDevModePuppetHost(createDevModePuppetHost(projectPath)), [projectPath]);

  const requireProjectRef = useCallback(
    (operation: string): DevModeSaveProjectRef => {
      if (!projectRef) {
  ...
  const subscribeFullscreenChanged = useCallback(
    (listener: (isFullscreen: boolean) => void): (() => void) => {
      const token = getInterface().devMode.onFullscreenChanged(({ isFullscreen }) =>
  ...
  const subscribeCloseRequested = useCallback(
    (listener: () => Promise<boolean> | boolean): (() => void) => {
      const listeners = closeListenersRef.current;
  ...
  const pluginHost = useMemo(
    () =>
      new RuntimePluginHostController({
        persistence: {
          getAll: async () => {
            const result = await getInterface().blueprintPersistence.getAll(
              await awaitProjectRef()
            );
            if (!result.success) {
          ...
          getValue: async (key) => {
            const result = await getInterface().blueprintPersistence.getValue(
              await awaitProjectRef(),
              key
            );
            if (!result.success) {
          ...
          setValue: async (key, value) => {
            const result = await getInterface().blueprintPersistence.setValue(
              await awaitProjectRef(),
              key,
              value
            );
          ...
          removeValue: async (key) => {
            const result = await getInterface().blueprintPersistence.removeValue(
              await awaitProjectRef(),
              key
            );
            if (!result.success) {
        ...
        navigation: {
          openExternal: async (ownerPluginId, request) => {
            const result = await getInterface().blueprintExternalLink.openForPlugin(
              ownerPluginId,
              request
            );
            if (!result.success) {
