 ...
 */
function refusalAsFailure(
  outcome: Extract<SaveLoadOutcome, { status: "refused" }>
): SaveLoadFailure {
  return {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const entries = await Promise.all(
        ids.map(async (id): Promise<SaveSlotEntry> => {
          try {
    ...
    async (entry: SaveSlotEntry) => {
      ...
      try {
        ...
        const knownActionIds = new Set(
          storyRuntime.getActionIdBindings().map((binding) => binding.staticId)
        );
...
function LoadResultBlock(props: { result: SaveLoadResult }): ReactNode {
  ...
  if (result.kind === "ok") {
    ...
    if (losses.droppedBacklog > 0) {
      lines.push(
        t("devMode.saves.droppedBacklog", {
    ...
    return (
      <div className="mt-1 rounded-md border border-warning/30 bg-warning/10 px-1.5 py-1">
        ...
        {lines.map((line) => (
          <p key={line} className="text-2xs text-fg-subtle">
            {line}
          </p>
        ))}
      </div>
...
function SaveContents(props: { decoded: DecodedSave | null; hasRunningStory: boolean }): ReactNode {
  ...
  return (
    <div className="space-y-2">
      ...
      {decoded.visited &&
      (decoded.visited.scenes.length > 0 || decoded.visited.options.length > 0) ? (
        <div>
        ...
        <ul className="space-y-0.5">
          {view.declared.map((row) => (
            <li key={row.storageKey} className="flex gap-2">
              ...
              <span
                className={cn(
                  "min-w-0 flex-1 break-all text-fg-muted",
                  row.live ? "" : "opacity-60"
                )}
              >
                {formatDebugValue(row.value)}
