 ...
 */
function useStoryRowLookups(
  bundle: DevModeBundle,
  document: StoryDocument,
  scene: StoryScene | undefined
): StoryRowLookups {
  return useMemo(() => buildStoryRowLookups(bundle, document, scene), [bundle, document, scene]);
}
...
function useStoryRunTrail(
  ...
  useEffect(() => {
    ...
    let next =
      trailByRun.get(bindings) ??
      seedStoryRunTrail(storyRuntime.getStoryContext()?.sceneId ?? entrySceneId);
  ...
  return (
    <div className={rootClass}>
      ...
      <div
        ...
        role="tablist"
        aria-label={t("devMode.runtime.panelsAria")}
      >
        {tabs.map(([id, label]) => {
          const active = tab === id;
          return (
...
function VariablesTab(props: {
  ...
  const rows = useMemo(() => {
    ...
    const savedValues = namespaces.saved
      ? storyRuntime.readStorableNamespace(namespaces.saved)
      : null;
  ...
  const writeValue = useCallback(
    (variable: DeclaredStoryVariable, value: StoryLiteralValue) => {
      if (variable.scope === "persistent") {
        ...
        const namespaces = storyRuntime.getVariableNamespaces();
        const name =
          variable.scope === "scene" ? (namespaces.sceneLocal[sceneId] ?? null) : namespaces.saved;
        if (name) {
            ...
            <ul className="space-y-1">
              {scopeRows.map((row) => (
                <li key={`${scope}:${row.variable.id}`} className="flex items-center gap-2">
                  <span
                    className="w-[42%] shrink-0 truncate text-fg-muted"
                    data-tip={row.variable.name}
                  >
                    {row.variable.name}
                  </span>
                  <VariableValueEditor
...
function VariableValueEditor(props: {
  ...
  return <VariableTextEditor valueType={valueType} value={value} live={live} onCommit={onCommit} />;
}
...
function ExecutionContextTab(props: {
  ...
  return (
    <div className="min-h-0 flex-1 space-y-3 overflow-auto p-2">
      <div>
        <h3 className="mb-1 text-2xs font-medium tracking-wide text-fg-subtle">
          {t("devMode.runtime.contextScene")}
        </h3>
        <p className="truncate text-fg" data-tip={view.sceneName}>
          {view.sceneName}
        </p>
      </div>
      ...
      {view.chain.length > 0 || view.orphanRound ? (
        <div>
          <h3 className="mb-1 text-2xs font-medium tracking-wide text-fg-subtle">
            {t("devMode.runtime.contextInside")}
          </h3>
          <ul className="space-y-0.5">
            ...
            {view.orphanRound ? (
              <li
                className="flex items-baseline gap-1.5"
                style={{ paddingLeft: view.chain.length * 10 }}
              >
      ...
      {view.branches.length > 0 ? (
        <div>
          <h3 className="mb-1 text-2xs font-medium tracking-wide text-fg-subtle">
            {t("devMode.runtime.contextRunning")}
          </h3>
          <ul className="space-y-0.5">
...
function readLiveNumericValue(
  ...
  registry: {
    saved: readonly VariableRegistryEntry[];
    persistent: readonly VariableRegistryEntry[];
  }
): number | null {
  ...
  const declared = listDeclaredStoryVariables(
    ...
    registry.saved,
    registry.persistent
  ).find((candidate) => candidate.scope === variable.scope && candidate.id === variable.variableId);
  if (!declared) {
    return null;
  }
  let raw: unknown;
  if (declared.scope === "persistent") {
    ...
    const namespaces = storyRuntime.getVariableNamespaces();
    const name =
      declared.scope === "scene" ? (namespaces.sceneLocal[sceneId] ?? null) : namespaces.saved;
...
function SceneTab(props: {
  ...
  const sceneChips = useMemo(() => {
    ...
    for (const [sceneId, range] of computeVariableRanges(
      graph,
      document,
      focused.key,
      registryVariables
