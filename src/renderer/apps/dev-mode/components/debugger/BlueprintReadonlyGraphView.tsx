  ...
  const nodes = useMemo<Node<BlueprintReadonlyGraphNodeData>[]>(() => {
    return Object.values(ir?.nodes ?? {}).map((node) => {
      ...
      return {
        ...
        connectable: false,
        data: {
          title: catalog ? resolveBlueprintNodeTitle(catalog.displayName, t) : node.type,
          inputs: pins
          ...
          breakpoint: breakpointsByKey.get(
            blueprintBreakpointKey({ blueprintId, graphId, nodeId: node.id })
          ),
