...
export function resolveBlueprintGraphName(
  ...
  const entry =
    blueprint.program.graphs.events?.[graphId] ?? blueprint.program.graphs.functions?.[graphId];
...
export function resolveBlueprintNodeType(
  ...
  return resolveBlueprintGraphIr(document, breakpoint.blueprintId, breakpoint.graphId)?.nodes?.[
    breakpoint.nodeId
  ]?.type;
}
