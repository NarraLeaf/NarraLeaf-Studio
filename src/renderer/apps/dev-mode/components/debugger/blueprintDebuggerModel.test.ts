...
const withNodes = (id: string, name: string, nodeIds: string[]) => ({
    id,
    name,
    graph: { nodes: Object.fromEntries(nodeIds.map(node => [node, { id: node, type: "test.node" }])) },
});

describe("run graph ids", () => {
    it("round-trips kind, blueprint and graph", () => {
        const id = buildBlueprintRunGraphId("fnCall", "bp-1", "graph-9");
        expect(parseBlueprintRunGraphId(id)).toEqual({ kind: "fnCall", blueprintId: "bp-1", graphId: "graph-9" });
    });
...
describe("breakpoint table", () => {
    it("drops entries it cannot trust and de-duplicates by node", () => {
        ...
        expect(table.breakpoints).toEqual([{ blueprintId: "a", graphId: "g", nodeId: "n1", enabled: true }]);
    });
    ...
    it("keeps a condition only when every part of it is valid", () => {
        const table = parseBlueprintBreakpointTable({
            version: 1,
            breakpoints: [
                { blueprintId: "a", graphId: "g", nodeId: "ok", enabled: true, condition: { variableId: "hp", op: "<", value: 5 } },
                { blueprintId: "a", graphId: "g", nodeId: "badOp", enabled: true, condition: { variableId: "hp", op: "~=", value: 5 } },
                { blueprintId: "a", graphId: "g", nodeId: "badValue", enabled: true, condition: { variableId: "hp", op: "<", value: {} } },
            ],
        });
...
describe("breakpoint conditions", () => {
    it("compares what the author sees rather than what the runtime stores", () => {
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: "==", value: 3 }, "3")).toBe(true);
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: "!=", value: 3 }, 4)).toBe(true);
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: ">=", value: 3 }, "10")).toBe(true);
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: "<", value: 3 }, true)).toBe(true);
    });

    it("reads undecidable as false rather than throwing", () => {
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: ">", value: 3 }, undefined)).toBe(false);
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: ">", value: 3 }, "abc")).toBe(false);
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: "==", value: 3 }, { a: 1 })).toBe(false);
    });

    it("handles contains for both lists and text", () => {
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: "contains", value: "b" }, ["a", "b"])).toBe(true);
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: "contains", value: "ell" }, "hello")).toBe(true);
        expect(evaluateBlueprintBreakpointCondition({ variableId: "v", op: "contains", value: "z" }, 42)).toBe(false);
    });
});
...
describe("debuggable blueprints", () => {
    ...
    it("resolves a graph's IR from either table", () => {
        ...
        expect(Object.keys(resolveBlueprintGraphIr(document, "bp-a", "e1")?.nodes ?? {})).toEqual(["n1"]);
        expect(Object.keys(resolveBlueprintGraphIr(document, "bp-a", "f1")?.nodes ?? {})).toEqual(["n2"]);
