...
} from "@/lib/ui-editor/blueprint-runtime/BlueprintDebugSession";
import {
  ...
  const viewIr = useMemo(
    () =>
      resolveBlueprintGraphIr(
        blueprintDocument,
        viewBlueprintId ?? undefined,
        viewGraphId ?? undefined
      ),
    [blueprintDocument, viewBlueprintId, viewGraphId]
  );

  const viewBlueprint = viewBlueprintId
    ? blueprintDocument?.blueprints[viewBlueprintId]
    : undefined;
  ...
  const target = useCallback(
    (nodeId: string): BlueprintBreakpointTarget | null =>
      viewBlueprintId && viewGraphId
        ? { blueprintId: viewBlueprintId, graphId: viewGraphId, nodeId }
        : null,
    [viewBlueprintId, viewGraphId]
  );
