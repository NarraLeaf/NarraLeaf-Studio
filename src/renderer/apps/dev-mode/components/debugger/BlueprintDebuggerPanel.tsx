...
export function BlueprintDebuggerPanel(props: {
    ...
    const scope = paused && snapshot.pausedFrameId ? session?.readFrameScope(snapshot.pausedFrameId) ?? null : null;
        ...
        <ul className="space-y-0.5">
            {props.entries.map(entry => (
                <li key={entry.name} className="flex gap-1.5">
                    ...
                    <span className="min-w-0 flex-1 truncate text-fg-muted">{formatDebugValue(entry.value)}</span>
                </li>
            ))}
        </ul>
    );
