...
describe("listDevModeBlueprints — workspace", () => {
  ...
  it("scopes the blueprints tab to global and the current interface", () => {
    ...
    expect(
      listForWorkspace(blueprints, { document, activeSurfaceId: "app" }).map((bp) => bp.id)
    ).toEqual(["app-surface", "app-widget", "global"]);
  });
  ...
  it("scopes a Game UI to its own nodes", () => {
    ...
    const blueprints = {
      ...
      linkedAppWidgetInStage: authoredBlueprint(
        "linked-app-widget-in-stage",
        "Linked App Widget In Stage",
        {
          kind: "widgetMain",
          surfaceId: "mounted-stage",
    ...
    expect(
      listForWorkspace(blueprints, { document, activeSurfaceId: "mounted-stage" }).map(
        (bp) => bp.id
      )
...
describe("blueprintWidgetElementId", () => {
  ...
  it("answers nothing for the owners that are not a widget on the stage", () => {
    const surface = authoredBlueprint("surface", "Surface", {
      kind: "surfaceMain",
      surfaceId: "app"
    });
