...
export function BlueprintRuntimeDebugPanel(props: BlueprintRuntimeDebugPanelProps) {
    ...
    return (
        <div className="flex w-[280px] shrink-0 flex-col border-l border-white/10 bg-[#0d0f11] text-[11px] text-gray-300">
            <button
                type="button"
            ...
            {open ? (
                <div className="min-h-0 flex-1 overflow-auto p-2 font-mono leading-snug">
                    {recent.length === 0 ? (
                        <p className="text-gray-500">No events yet. Click a wired button in the preview.</p>
                    ) : (
                        <ul className="space-y-1">
                            {recent.map((ev, i) => (
                                <li key={`${i}-${ev.type}`} className="break-all text-[10px] text-gray-400">
