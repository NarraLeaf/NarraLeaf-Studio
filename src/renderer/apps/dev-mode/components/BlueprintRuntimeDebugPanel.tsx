...
export const DEFAULT_OUTPUT_LOG_LEVELS: ReadonlySet<BlueprintOutputLogLevel> =
  new Set<BlueprintOutputLogLevel>(["error", "warning", "log"]);
...
export function BlueprintRuntimeDebugPanel(props: BlueprintRuntimeDebugPanelProps) {
  ...
  return (
    <div className={rootClass}>
            ...
            {safeAreaMenuOpen ? (
              ...
              >
                ...
                {SAFE_AREA_FAMILY_LABELS.map(([family, familyLabel]) => (
                  <div key={family}>
                    <div className="px-1.5 pb-0.5 pt-1.5 text-2xs text-fg-subtle">
                      {familyLabel}
                    </div>
      ...
      <div
        className="flex shrink-0 border-b border-edge bg-surface-sunken"
        role="tablist"
        aria-label={t("devMode.devtools.panelsAria")}
      >
        {(
          [
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden font-mono leading-snug">
                ...
                <ul className="space-y-1">
                  {outputLines.map((ev, i) => (
                    <li key={`${i}-${ev.type}`} className="break-all text-2xs text-fg-subtle">
                      <span className={outputLogLevelClassName(getBlueprintDebugEventLogLevel(ev))}>
                        {ev.type}
                      </span>
                      {" · "}
                      {formatEvent(ev)}
        ...
        {tab === "uiState" ? (
          <div className="min-h-0 flex-1 space-y-3 overflow-auto p-2">
            <KeyValueBlock
              title={t("devMode.uiState.surface")}
              entries={surfaceSnap}
            ...
            <div>
              ...
              <ul className="space-y-0.5 text-2xs text-fg-muted">
                ...
                <li>
                  {t("devMode.uiState.active")} · {widgetSnap.activePointerId ?? "-"}
                </li>
                <li>
                  {t("devMode.uiState.focus")} · {widgetSnap.focusedId ?? "-"}
                </li>
...
function studioPayloadSupported(bp: Blueprint): boolean {
  return (
    bp.owner.kind === "surfaceMain" ||
    bp.owner.kind === "widgetMain" ||
    bp.owner.kind === "widgetValue"
  );
}
          ...
          {keys.map((k) => (
            <li key={k} className="flex gap-2 text-2xs text-fg-muted">
              ...
              <span className="min-w-0 flex-1 break-all text-fg-muted">
                {formatDebugValue(entries.get(k))}
              </span>
            </li>
          ))}
        </ul>
...
function formatExecutionError(
  ev: Extract<BlueprintDebugEvent, { type: "execution.error" }>
): string {
...
function formatExecutionCancelled(
  ev: Extract<BlueprintDebugEvent, { type: "execution.cancelled" }>
): string {
