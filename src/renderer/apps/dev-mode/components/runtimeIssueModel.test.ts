...
function dialogue(id: StoryBlockId, characterId: string, text: string): StoryBlock {
  return {
    ...
    payload: {
      action: "dialogue",
      characterId,
      text: { textId: `t-${id}`, value: text, role: "dialogue" }
    }
  };
}
...
describe("locateStoryBlock", () => {
  const storyA = document("story-a", "Story A", [
    scene(
      "scene-1",
      "Opening",
      [narration("a", "first"), narration("b", "second"), narration("c", "third")],
      ["a", "b", "c"]
    )
  ]);
  ...
  it("still names the scene for an orphan block that occupies no line", () => {
    ...
    const orphaned = document("story-d", "Story D", [
      scene(
        "scene-1",
        "Opening",
        [narration("a", "first"), narration("ghost", "unreachable")],
        ["a"]
      )
    ]);
...
describe("appendRuntimeIssue", () => {
  ...
  it("puts the newest first", () => {
    const list = appendRuntimeIssue(
      appendRuntimeIssue([], issue("1", "first")),
      issue("2", "second")
    );
  ...
  it("keys a repeat the same as the report it repeats, and two rows apart", () => {
    ...
    expect(runtimeIssueKey(issue("1", "boom", "row-1"))).toBe(
      runtimeIssueKey(issue("2", "boom", "row-1"))
    );
    expect(runtimeIssueKey(issue("1", "boom", "row-1"))).not.toBe(
      runtimeIssueKey(issue("2", "boom", "row-2"))
    );
  });
