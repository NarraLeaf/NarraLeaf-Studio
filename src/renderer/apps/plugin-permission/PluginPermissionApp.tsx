...
function buildPermissionCopy(
  props: PluginPermissionPromptProps,
  t: Translator["t"]
): PermissionCopy {
  ...
  switch (request.kind) {
    case "install": {
      ...
      return {
        ...
        body: [t("pluginPermission.install.body1"), t("pluginPermission.install.body2")],
    ...
    case "api":
      return {
        ...
        body: [t("pluginPermission.api.body1"), t("pluginPermission.api.body2")],
    ...
    case "trust":
      return {
        ...
        body: [t("pluginPermission.trust.body1"), t("pluginPermission.trust.body2")],
...
export function PluginPermissionApp() {
  ...
  const showPersistentChoices = Boolean(
    request && isPrivilegedRequest(request) && request.persistence === "permanent"
  );
  ...
  return (
    ...
    >
      <div className="flex h-full min-h-0 flex-col bg-surface text-fg">
        ...
        {copy ? (
          <>
            <div className="border-b border-edge bg-surface-sunken px-4 py-2">
              <div className="text-2xs font-medium tracking-normal text-fg-muted">{copy.type}</div>
              <div className="mt-1 text-sm font-medium text-fg">{copy.title}</div>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-auto px-4 py-3">
              ...
              {copy.detail ? (
                <div className="mt-3 truncate font-mono text-2xs text-fg-subtle">{copy.detail}</div>
              ) : null}
        ...
        {request ? (
          <div
            className={`grid gap-2 border-t border-edge bg-surface-sunken p-3 ${showPersistentChoices ? "grid-cols-[1fr_1fr_1.15fr]" : "grid-cols-2"}`}
          >
            <button
              type="button"
              onClick={handleDeny}
            ...
            >
              ...
              <span className="whitespace-nowrap">
                {request.kind === "install"
                  ? t("pluginPermission.button.dontAllow")
                  : t("pluginPermission.button.deny")}
              </span>
            </button>
            ...
            >
              ...
              <span className="whitespace-nowrap">
                {busy
                  ? t("pluginPermission.button.granting")
                  : showPersistentChoices
                    ? t("pluginPermission.button.alwaysAllow")
                    : t("pluginPermission.button.allow")}
              </span>
            </button>
          </div>
        ) : null}
