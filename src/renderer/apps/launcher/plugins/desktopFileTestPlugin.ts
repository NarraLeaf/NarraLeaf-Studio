...
export async function runDesktopFileTestPlugin(): Promise<DesktopFileTestResult> {
  ...
  try {
    ...
    const ensureResult = unwrapFs(
      await runtime.app.fs.ensureRegularFile(targetPath, body, "utf-8")
    );
    if (!ensureResult.ok) {
...
function unwrapFs<T>(
  result: { success: true; data: FsRequestResult<T> } | { success: false; error?: string }
): FsRequestResult<T> {
  if (result.success) {
    return result.data;
  }
