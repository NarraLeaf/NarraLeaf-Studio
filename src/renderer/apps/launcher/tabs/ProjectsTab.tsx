...
import {
  useMissingRecentProjects,
  useRecentProjects,
  useRemoveRecentProject,
  useRevealRecentProject
} from "@/lib/app/hooks/useRecentProjects";
import {
    ...
    <div className="h-full w-full flex flex-col pt-4 px-6 pb-6 text-fg">
              ...
              >
                {project.icon && !missingEntry ? (
                  <img
                    src={project.icon}
                    alt=""
                    className="flex-shrink-0 w-10 h-10 rounded-lg object-contain"
                  />
                ) : (
                  <span
                    aria-hidden
                ...
                <span className="flex-1 min-w-0">
                  <span
                    className={cn(
                      ...
                      missingEntry ? "text-fg-muted" : "text-fg"
                    )}
                  >
                    {project.name}
                  </span>
      ...
      {missingTarget && (
        <MissingProjectDialog
          ...
          reason={
            missingByPath.get(normalizeProjectPath(missingTarget.path))?.reason ?? "folder-missing"
          }
          error={missingError}
