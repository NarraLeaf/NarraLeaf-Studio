...
interface RegistryContextValue {
  ...
  updateEditorTabPayload: <TPayload = any>(
    tabId: string,
    payload: TPayload,
    groupId?: string
  ) => void;
...
export function RegistryProvider({ children }: RegistryProviderProps) {
  ...
  const registerPanel = useCallback(
    <TPayload = any,>(panel: PanelDefinition<TPayload>) => {
  ...
  const reorderPanels = useCallback(
    (position: PanelPosition, orderedIds: string[]) => {
      uiService.getStore().setPanelOrder(position, orderedIds);
    },
    [uiService]
  );

  const setCollapsedPanels = useCallback(
    (position: PanelPosition, panelIds: string[]) => {
      uiService.getStore().setCollapsedPanels(position, panelIds);
    },
    [uiService]
  );

  const updatePanelPayload = useCallback(
    <TPayload = any,>(panelId: string, payload: TPayload) => {
  ...
  const registerActionGroup = useCallback(
    (group: ActionGroup) => {
  ...
  const unregisterActionGroup = useCallback(
    (id: string) => {
  ...
  const openEditorTab = useCallback(
    <TPayload = any,>(tab: EditorTabDefinition<TPayload>, groupId?: string) => {
  ...
  const closeEditorTab = useCallback(
    (tabId: string, groupId?: string) => {
      ...
      if (currentFocus.area === FocusArea.EditorTabs) {
        ...
        if (group && currentFocus.targetId === group.id && group.tabs.length === 0) {
          uiService.focus.clearFocus();
        }
      }
    },
  ...
  const closeEditorTabs = useCallback(
    (tabIds: string[], groupId?: string) => {
      ...
      if (currentFocus.area === FocusArea.EditorTabs && currentFocus.targetId === resolvedGroupId) {
  ...
  const updateEditorTabPayload = useCallback(
    <TPayload = any,>(tabId: string, payload: TPayload, groupId?: string) => {
