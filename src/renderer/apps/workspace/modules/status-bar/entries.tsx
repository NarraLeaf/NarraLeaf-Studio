...
const THEME_META: Record<ThemeMode, { icon: React.ReactNode; labelKey: TranslationKey }> = {
  auto: {
    ...
    labelKey: "settings.items.themeMode.options.auto" as TranslationKey
  },
  light: {
    icon: <Sun className="h-3 w-3" />,
    labelKey: "settings.items.themeMode.options.light" as TranslationKey
  },
  dark: {
    icon: <Moon className="h-3 w-3" />,
    labelKey: "settings.items.themeMode.options.dark" as TranslationKey
  }
};
...
export function RunStatusEntry() {
  ...
  return (
    <StatusEntry
      emphasis
      onClick={openConsole}
      tooltip={t("workspace.shell.statusBar.openConsole")}
    >
...
export function SaveStatusEntry() {
  ...
  return (
    <StatusEntry
      emphasis={failed}
      tooltip={t(
        failed ? "workspace.shell.statusBar.retrySave" : "workspace.shell.statusBar.saveNow"
      )}
      onClick={() => {
    ...
    >
      ...
      <span>
        {t(
          failed
            ? "workspace.shell.statusBar.saveFailed"
            : "workspace.shell.statusBar.unsavedChanges"
        )}
      </span>
    </StatusEntry>
  );
...
export function WordCountEntry() {
  ...
  useEffect(() => {
    ...
    const computeFor = (target: SceneRef | null) => {
      ...
      setStats({
        name: getSceneName(doc.scenes, target.sceneId),
        words: counts.words,
        lines: counts.lines
      });
    };
  ...
  return (
    <StatusEntry
      tooltip={t("workspace.shell.statusBar.openCurrentScene")}
      onClick={openCurrentScene}
    >
