...
import {
  formatVoiceDuration,
  readAudioDuration
} from "@/lib/workspace/services/voice/audioDuration";
...
export function VoiceEditorTab({
  payload,
  active
}: EditorComponentProps<VoiceEditorTabPayload | undefined>) {
  ...
  const localizationService = useMemo(
    () =>
      context && isInitialized
        ? context.services.get<LocalizationService>(Services.Localization)
        : null,
    [context, isInitialized]
  );
  ...
  useEffect(() => {
    ...
    const extract = () => {
      try {
        ...
        setRows(
          withSceneIndices(voiceService.extractRows(document)).map((row) => {
    ...
    void Promise.all([storyService.loadStory(storyId), voiceService.loadLineTexts(locale)])
      .then(() => {
        if (!disposed) {
          extract();
        }
  ...
  const resolveAsset = useCallback(
    (assetId: string | undefined): Asset | null => {
  ...
  const togglePlay = useCallback(
    async (unitId: string, asset: Asset | null) => {
      ...
      try {
        ...
        if (!result.success) {
          uiService?.showNotification(
            result.error || t("workspace.voice.table.clipMissing"),
            "warning"
          );
          return;
        }
  ...
  const assignAsset = useCallback(
    (unitId: string, sourceText: string, assetId: string) => {
  ...
  const setNote = useCallback(
    (unitId: string, sourceText: string, note: string) => {
  ...
  const assetNameKeys = useMemo(() => {
    ...
    return buildAssetNameKeyMap(
      Object.values(audio).map((asset) => ({ id: asset.id, name: asset.name }))
    );
  }, [assetsService, assetsRev]);
   ...
   */
  const matchingAssetIdFor = useCallback(
    (row: TableRow): string | undefined => {
      if (!config) {
        return undefined;
      }
  ...
  const visibleRows = useMemo(() => {
    if (mode === "audition") {
      const withClips = rows.filter(
        (row) => (rowStates.get(row.unitId) ?? "missing") !== "missing"
      );
  ...
  const groups = useMemo(() => {
    ...
    for (const row of visibleRows) {
      const key =
        row.role === "narration" ? NARRATION_GROUP_KEY : (row.characterId ?? `name:${row.speaker}`);
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: (index) =>
      flatItems[index]?.kind === "group" ? GROUP_ROW_HEIGHT_PX : VOICE_ROW_HEIGHT_PX,
    overscan: 16,
  ...
  const filterOptions: SelectOption[] = useMemo(
    () => [
      ...
      {
        value: "missing",
        label: `${t("workspace.voice.table.filterMissing")} (${counts.missing})`
      },
      {
        value: "outdated",
        label: `${t("workspace.voice.table.filterOutdated")} (${counts.outdated})`
      },
      { value: "voiced", label: `${t("workspace.voice.table.filterVoiced")} (${counts.voiced})` },
      {
        value: "approved",
        label: `${t("workspace.voice.table.filterApproved")} (${counts.approved})`
      }
    ],
    [counts, t]
  );
  ...
  const auditionFilterOptions: SelectOption[] = useMemo(
    () => [
      ...
      {
        value: "pending",
        label: `${t("workspace.voice.table.auditionFilterPending")} (${counts.voiced})`
      },
      {
        value: "approved",
        label: `${t("workspace.voice.table.auditionFilterApproved")} (${counts.approved})`
      }
    ],
    [counts, t]
  );
  ...
  const groupAxisOptions: { key: GroupAxis; label: string; icon: React.ReactNode }[] = [
    {
      key: "scene",
      label: t("workspace.voice.table.groupByScene"),
      icon: <ListMusic className="h-3.5 w-3.5" />
    },
    {
      key: "character",
      label: t("workspace.voice.table.groupByCharacter"),
  ...
  const modeOptions: { key: EditorMode; label: string; icon: React.ReactNode }[] = [
    {
      key: "assign",
      label: t("workspace.voice.table.modeAssign"),
      icon: <PenLine className="h-3.5 w-3.5" />
    },
    {
      key: "audition",
      label: t("workspace.voice.table.modeAudition"),
      icon: <AudioLines className="h-3.5 w-3.5" />
    }
  ];
  ...
  const auditionQueueEmpty =
    mode === "audition" &&
    auditionFilter === "pending" &&
    counts.clips > 0 &&
    ...
    <div className="flex h-full min-h-0 flex-col bg-surface" data-help-topic="voice">
      <div className="flex items-center gap-3 border-b border-edge px-4 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="rounded-md border border-edge px-1.5 py-0.5 text-2xs text-fg-subtle">
            {locale}
          </span>
        </div>
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto">
        {stories.length === 0 ? (
          <EmptyMessage
            ...
            text={t("workspace.voice.table.noStories")}
          />
        ) : rows.length === 0 ? (
          <EmptyMessage
            icon={<AudioLines className="h-5 w-5" />}
            text={t("workspace.voice.table.emptyStory")}
          />
        ) : auditionQueueEmpty ? (
          <EmptyMessage
            icon={<CheckCircle2 className="h-5 w-5 text-success" />}
            text={t("workspace.voice.table.auditionAllClear")}
          />
        ) : visibleRows.length === 0 ? (
          <EmptyMessage
            icon={<ListMusic className="h-5 w-5" />}
            text={t("workspace.voice.table.emptyFilter")}
          />
        ) : (
                    ...
                    ? (() => {
                        ...
                        return (
                          <VoiceRow
                            ...
                            onRemove={() =>
                              voiceService?.updateUnit(locale, row.unitId, row.sourceText, {
                                assetId: ""
                              })
                            }
                            onApprove={() =>
                            ...
                            onDropAsset={(assetId) =>
                              assignAsset(row.unitId, row.sourceText, assetId)
                            }
                          />
                        );
                      })()
