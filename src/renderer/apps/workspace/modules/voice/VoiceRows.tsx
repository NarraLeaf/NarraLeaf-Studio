...
export function VoiceRow(props: VoiceRowProps) {
  ...
  const statusLabel =
    state === "approved"
      ...
      : state === "stale"
        ? strings.statusOutdated
        : state === "linked"
          ? strings.statusVoiced
          : "";
  ...
  return (
    ...
    >
      ...
      <span
        ...
        data-tip={
          row.authoredText && row.authoredText !== row.sourceText
            ? `${row.sourceText}\n${row.authoredText}`
            : row.sourceText
        }
      >
      ...
      {hasClip ? (
        <>
          <button
            ...
            data-tip={isPlaying ? strings.stop : strings.play}
          ...
          <span
            ...
            data-tip={
              state === "stale" ? strings.outdatedHint : (asset?.name ?? strings.clipMissing)
            }
          >
...
function readAudioAssetId(dataTransfer: DataTransfer | null, peek = false): string | null {
  ...
  try {
    ...
    const audio = parsed.i?.find(
      (item) => item?.t === AUDIO_ASSET_TYPE && typeof item.id === "string"
    );
