...
export function TestReportTab({ payload }: EditorTabComponentProps<TestReportPayload>) {
  ...
  const rerunBlocked =
    !record || running || !testRun || testRun.getAvailability(record.testId).available === false;
...
function TestFindingRow({
  ...
  if (!finding.target) {
    return (
      <div className="flex w-full items-baseline gap-2 rounded-md px-2 py-0.5 text-left">
        {body}
      </div>
    );
  }
