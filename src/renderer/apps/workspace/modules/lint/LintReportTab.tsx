...
export function LintReportTab() {
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: (index) =>
      rows[index]?.kind === "group" ? GROUP_ROW_HEIGHT_PX : ENTRY_ROW_HEIGHT_PX,
    overscan: 16,
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-surface" data-help-topic="lint">
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-2 py-1.5">
        {rows.length === 0 ? (
          placeholder ? (
            <p className="px-1 py-1 text-xs text-fg-subtle">{placeholder}</p>
          ) : null
        ) : (
...
function LintGroupRow({
  ...
  return (
    ...
    >
      ...
      <span
        className={cn("min-w-0 truncate text-2xs font-semibold", SEVERITY_TEXT_CLASS[severity])}
      >
        {title}
      </span>
...
function LintEntryRow({
  ...
  const body = (
    <>
        ...
        <span className="flex min-w-0 max-w-[45%] shrink-0 items-baseline text-2xs text-fg-subtle">
          ...
          {line === null ? null : (
            <span
              ...
              aria-label={t("lint.report.lineAria", { line })}
            >
              {label ? `:${line}` : line}
            </span>
          )}
      ...
      {secondary ? (
        <span
          className={cn(
            "max-w-[30%] shrink-0 truncate text-2xs",
            SEVERITY_TEXT_CLASS[entry.severity]
          )}
        >
          {secondary}
        </span>
  ...
  if (!entry.target) {
    return (
      <div
        className="flex w-full items-baseline gap-2 rounded-md px-2 py-0.5 text-left"
        data-tip={hint}
      >
        {body}
      </div>
