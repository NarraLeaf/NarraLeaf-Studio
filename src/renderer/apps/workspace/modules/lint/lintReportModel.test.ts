...
describe("lintLocationLabel", () => {
  ...
  it("falls back to the id when a name is missing", () => {
    ...
    expect(lintLocationLabel({ kind: "character", characterId: "c1", characterName: "" }, "")).toBe(
      "c1"
    );
  });
});
...
describe("lintEntryLocator", () => {
  ...
  it("grouped by location, names only the row - the heading already named the place", () => {
    expect(lintEntryLocator(row(12), "location", labels.locationLabel, "Jumps to ending")).toEqual({
      label: "",
      line: 12
    });
  });
  ...
  it("says nothing the message already said", () => {
    ...
    expect(
      lintEntryLocator(asset, "rule", labels.locationLabel, "dialog.png is not used anywhere")
    ).toEqual({ label: "", line: null });
    // Only the repeated half goes: the story name is still the reader's only way to that story.
    expect(
      lintEntryLocator(row(12), "rule", labels.locationLabel, "Opening references a missing asset")
...
describe("filterLintEntries", () => {
  ...
  it("keeps only the chosen severity", () => {
    expect(filterLintEntries(entries, "error").map((e) => e.ruleId)).toEqual([
      "story/goto-missing"
    ]);
...
describe("groupLintEntries", () => {
  ...
  it("puts a location's findings back into row order, scene-wide ones first", () => {
    ...
    expect(
      groups[0].entries.map((e) => (e.location.kind === "story" ? e.location.line : null))
  ...
  it("keeps the report's own order inside a group", () => {
    ...
    expect(gotoGroup.entries.map((e) => e.location.kind === "story" && e.location.sceneId)).toEqual(
      ["sc1", "sc2"]
    );
  });
