...
import {
  UITemplateCard,
  UITemplateCardSkeleton,
  type UITemplatePreviewState
} from "./UITemplateCard";
...
export function UITemplateStoreModal({
  ...
  const loadPreviews = useCallback(
    async (loaded: UITemplateRegistryEntry[]) => {
      if (loaded.length === 0) {
        return;
      }
      setPreviews(
        Object.fromEntries(loaded.map((entry) => [entry.id, { status: "loading" } as const]))
      );
      const result = await getInterface().uiTemplates.fetchPreviews(
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (loadedThemes.length > 0) {
        const posterResult = await getInterface().uiTemplates.fetchThemePreviews(
          loadedThemes.map((theme) => theme.id)
        );
        if (posterResult.success) {
          setPosters(Object.fromEntries(posterResult.data.map((p) => [p.id, p.dataUrl])));
        }
      } else {
  ...
  const placementLabel = useCallback(
    (entry: UITemplateRegistryEntry): string => {
  ...
  const blockedReason = useCallback(
    (entry: UITemplateRegistryEntry): string | undefined => {
      if (entry.surface.kind !== "stageSurface") {
        return undefined;
      }
  ...
  const tabs = useMemo(() => {
    ...
    const shelfCount = (target: UISurfaceKind) =>
      entries ? <span className="text-fg-subtle tabular-nums">{count(target)}</span> : undefined;
    return [
      { id: "appSurface", label: t("uiEditor.surfaceKind.page"), badge: shelfCount("appSurface") },
      {
        id: "stageSurface",
        label: t("uiEditor.surfaceKind.gameUi"),
        badge: shelfCount("stageSurface")
      }
    ];
  }, [entries, themed, t]);
  ...
  const body = () => {
    ...
    if (entries === null) {
      return (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(15rem,1fr))] gap-3">
          {[0, 1, 2, 3].map((index) => (
            <UITemplateCardSkeleton key={index} />
          ))}
        </div>
      );
    }
    if (browsingThemes) {
      ...
      const shown = needle
        ? themes.filter((theme) =>
            `${theme.name} ${theme.description} ${theme.publisher}`.toLowerCase().includes(needle)
          )
        : themes;
      if (shown.length === 0) {
        return (
          <EmptyState
            ...
            title={
              needle ? t("uiEditor.templateStore.emptyFiltered") : t("uiEditor.templateStore.empty")
            }
          />
        );
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t("uiEditor.templateStore.title")} size="xl">
