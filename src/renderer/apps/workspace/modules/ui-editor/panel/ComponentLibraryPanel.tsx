...
function getComponentPreviewSize(component: UIComponentDefinition): {
  width: number;
  height: number;
} {
...
export function ComponentLibraryPanel({
  ...
  const usageCounts = useMemo(
    () => countComponentUsage(documentService),
    [components, documentService]
  );
  ...
  const handleRename = useCallback(
    async (component: UIComponentDefinition) => {
    ...
    (
      ...
      const items: ContextMenuDef = [
        ...
        },
        {
          ...
          label:
            activeIds.length > 1
              ? t("uiEditor.componentLibrary.duplicateSelected")
              : t("common.duplicate"),
          ...freeze.menuRow(),
          onClick: () => {
            hideMenu();
        ...
        { id: "sep", separator: true },
        {
          ...
          label:
            activeIds.length > 1
              ? t("uiEditor.componentLibrary.deleteSelected")
              : t("common.delete"),
          ...freeze.menuRow(),
          onClick: () => {
            hideMenu();
        ...
        <div className="space-y-2 border-t border-edge p-2">
          ...
          {selectedCount > 0 ? (
            <div className="flex items-center gap-1 rounded-md border border-edge bg-fill-subtle p-1">
              <span className="min-w-0 flex-1 px-1 text-2xs text-fg-muted">
                {t("uiEditor.componentLibrary.selectedCount", { count: selectedCount })}
              </span>
              <button
                type="button"
          ...
          <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
            {filteredComponents.length === 0 ? (
              <div className="rounded-md border border-dashed border-edge px-3 py-4 text-center text-xs text-fg-subtle">
                {components.length === 0
                  ? t("uiEditor.componentLibrary.emptyCreate")
                  : t("uiEditor.componentLibrary.noMatches")}
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        aria-label={t("uiEditor.componentLibrary.selectComponent", {
                          name: component.name
                        })}
                      />
                      <div
                      ...
                      <button
                        ...
                        onClick={(event) =>
                          openContextMenu(event, component, { selectComponent: false })
                        }
                    ...
                    <div className="mt-1 text-2xs text-fg-subtle">
                      ...
                      {documentService ? (
                        <span className="ml-2">
                          {t("uiEditor.componentLibrary.refs", {
                            count: usageCounts[component.id] ?? 0
                          })}
                        </span>
                      ) : null}
                    </div>
