...
function collectSurfaceOwnedEditorTabs(
  layout: EditorLayout,
  surfaceId: string
): { groupId: string; tabIds: string[] }[] {
...
export function UISurfacesPanel({ panelId }: PanelComponentProps) {
  ...
  const currentKindOption = useMemo(
    () => SURFACE_KIND_OPTIONS.find((option) => option.kind === kind),
    [kind]
  );
  ...
  const defaultStageSlotId = useMemo<UIStageSlotId>(() => {
    return (
      GAME_UI_SLOT_IDS.find((slotId) => !occupiedStageSlotIds.has(slotId)) ?? DEFAULT_STAGE_SLOT_ID
    );
  }, [occupiedStageSlotIds]);
   ...
   */
  const focusSurfaceProperties = useCallback(
    (subjectId: string) => {
      if (!context) {
        return;
      }
  ...
  const renderSurfacePreview = useCallback(
    (surface: UISurface) => {
      if (!runtimeBridge) {
        return null;
      }
  ...
  const handleDeleteSurface = useCallback(
    async (surface: UISurface) => {
      ...
      const remaining = documentService
        .getDocument()
        .surfaces.filter((next) => next.kind === surface.kind);
  ...
  const handleRenameSurface = useCallback(
    async (surface: UISurface) => {
      ...
      const updatedSurface = documentService
        .getDocument()
        .surfaces.find((next) => next.id === surface.id);
      if (!updatedSurface) {
        return;
      }
      ...
      if (groupId) {
        uiService
          .getStore()
          .openEditorTabInGroup(createSurfaceEditorTab(updatedSurface), groupId, false);
      }
    },
  ...
  const handleOpenMenu = useCallback(
    (event: MouseEvent<HTMLDivElement | HTMLButtonElement>, surface: UISurface) => {
      ...
      setMenuItems(
        appendDeveloperIdSection(items, [{ kind: "surface", value: surface.id, label }], {
          hideMenu,
