...
export const getComponentEditorSurfaceId = (componentId: string) =>
  `${COMPONENT_EDITOR_SURFACE_PREFIX}${componentId}`;
export const getComponentEditorRootId = (componentId: string) =>
  `${COMPONENT_EDITOR_ROOT_PREFIX}${componentId}`;
...
function resolveDefaultComponentEditorDesignSize(
  baseDocument: UIDocument
): UISurface["designSize"] {
  const appSurface =
...
function isComponentEditorWrapperRoot(element: UIElement, componentRootId: string): boolean {
  return (
    element.id === componentRootId &&
    element.type === "nl.container" &&
    (element.name ?? "").trim() === "Root"
  );
}
...
export class ComponentDocumentServiceAdapter {
  ...
  public createElement(
    ...
    const created = this.base.createComponentElement(
      this.componentId,
      actualParentId,
      type,
      layoutPatch
  ...
  public pasteClipboardPayload(
    ...
    return this.base.pasteComponentClipboardPayload(
      this.componentId,
      actualParentId,
      beforeChildId,
      payload
