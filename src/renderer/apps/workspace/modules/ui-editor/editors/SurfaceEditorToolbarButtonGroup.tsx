...
export function SurfaceEditorToolbarButtonGroup({
  ...
  return (
    <TooltipGroup
      role="group"
      aria-label={ariaLabel}
      className={`${GROUP_SHELL} ${className}`.trim()}
    >
      {children}
    </TooltipGroup>
  );
...
export const SurfaceEditorToolbarSegButton = forwardRef<
  HTMLButtonElement,
  SurfaceEditorToolbarSegButtonProps
>(function SurfaceEditorToolbarSegButton(
  { active, className = "", children, type = "button", disabled, ...rest },
  ref
) {
  const state = active ? "bg-primary/20 text-fg" : "text-fg-muted hover:bg-fill hover:text-fg";
  return (
    <button
      ref={ref}
      type={type}
 ...
 */
export function SurfaceEditorToolbarSegSlot({
  children,
  className = ""
}: {
  children: ReactNode;
