...
function findEditorGroupIdByTabId(layout: EditorLayout, tabId: string): string | null {
  ...
  return (
    findEditorGroupIdByTabId(layout.first, tabId) ?? findEditorGroupIdByTabId(layout.second, tabId)
  );
}

export function UISurfaceEditorTab({
  tabId,
  payload,
  active
}: EditorComponentProps<{ surfaceId?: string; componentId?: string }>) {
  ...
  const {
    runtimeBridge,
    stateService,
    documentService: baseDocumentService,
    uiService
  } = useUISurfaceEditorServices();
  ...
  const graphService = useMemo(
    () => context?.services.get<UIGraphService>(Services.UIGraph) ?? null,
    [context]
  );
  ...
  useEffect(
    () => subscribeElementBindingSession(() => setBindingSession(readElementBindingSession())),
    []
  );
  useEffect(() => {
    if (!stateService) {
  ...
  useEffect(() => {
    ...
    if (current.type === "scene") {
      const currentSceneId =
        typeof current.data === "string" ? current.data : (current.data?.id ?? null);
  ...
  const surfaceDiagnostics = useMemo(() => {
    ...
    return collectSurfaceDiagnostics(documentService.getDocument(), surface.id, {
  ...
  const activeBindingSession =
    !isComponentEdit && bindingSession && surface && bindingSession.surfaceId === surface.id
      ? bindingSession
      : null;
  ...
  const requestRenamePrimary = useCallback(() => {
    ...
    void inputDialog
      .showRenameDialog(el.name ?? el.type ?? t("uiEditor.editor.layerFallback"), "layer")
      .then((name) => {
        if (name) {
  ...
  const handleOpenSurfaceEditor = useCallback(
    (targetSurfaceId: string) => {
      const targetSurface = baseDocumentService
        ?.getDocument()
        .surfaces.find((next) => next.id === targetSurfaceId);
      if (!targetSurface) {
        return;
      }
      openEditorTab({
  ...
  if (!surface || !stateService) {
    return (
      <div className="h-full flex items-center justify-center text-sm text-fg-subtle">
        {isComponentEdit
          ? t("uiEditor.editor.componentNotFound")
          : t("uiEditor.editor.interfaceNotFound")}
      </div>
    );
  }
        ...
        >
          ...
          <div className="absolute top-3 right-3 z-20 flex items-center gap-2 rounded-md border border-edge-strong bg-surface-canvas/80 px-2 py-1">
            <button
              ...
              data-tip={t("uiEditor.editor.selectTool")}
              aria-label={t("uiEditor.editor.selectTool")}
            >
            ...
            <button
              ...
              data-tip={t("uiEditor.editor.panTool")}
              aria-label={t("uiEditor.editor.panTool")}
            >
            ...
            <button
              ...
              data-tip={
                isComponentEdit
                  ...
                  : t("uiEditor.editor.openInDevMode")
              }
              aria-label={
                isComponentEdit
                  ? t("uiEditor.editor.componentDefinitionHint")
            ...
            {surfaceLevelDiagnosticMessages.length > 0 ? (
              <div className="absolute left-64 right-36 top-14 z-20 rounded-md border border-warning/35 bg-warning/10 px-3 py-2 text-2xs text-warning">
                <span className="font-medium text-warning">
                  {t("uiEditor.editor.staticChecks")}
                </span>
                <span className="text-warning/85">
                  {surfaceLevelDiagnosticMessages.join(" · ")}
                </span>
