  ...
  const handleCanvasContextMenu = useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (shouldApplyCanvasContextRetarget(surface.id, lastContextHitElementId.current, curSel)) {
        const nextSel = resolveCanvasContextSelection(
          surface.id,
          lastContextHitElementId.current,
          curSel
        );
      ...
      const items = buildCanvasContextMenu({
        ...
        actions: {
          ...
          renamePrimary: () => {
            ...
            void inputDialog
              .showRenameDialog(el.name ?? el.type ?? "Layer", "layer")
              .then((name) => {
                if (name) {
          ...
          addSelectionToComponentLibrary: () => {
            ...
            if (component) {
              uiService?.showNotification(
                `Added "${component.name}" to Component Library`,
                "success"
              );
            }
          },
      ...
      setMenuItems(
        freezeContextMenuRows(
          withDeveloperRows,
          freeze.frozen,
          FREEZE_READ_ONLY_CANVAS_MENU_IDS,
          freeze.reason
        )
      );
      showMenu(event);
