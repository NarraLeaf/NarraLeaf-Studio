...
export function useSurfaceImageDrop(params: {
  ...
  const { viewportRef, viewport, documentService, surface, stateService, workspaceContext } =
    params;
    ...
    (dropCtx: AssetDropContext) => {
      ...
      void (async () => {
        ...
        const dimList = await Promise.all(
          resolved.map(async (asset) => {
            ...
            return {
              ...
              height:
                Number.isFinite(height) && height > 0 ? height : DEFAULT_IMAGE_PLACEHOLDER_SIZE
            };
          })
        );
        ...
        for (let i = 0; i < resolved.length; i++) {
          ...
          const layoutPatch = buildLayoutPatchForNewElementFromSurfaceRect(
            freshDoc,
            target.parentId,
            {
  ...
  const {
    dropTargetProps: surfaceImageDropTargetProps,
    overlayClassName: surfaceImageDropOverlayClass
  } = useAssetDropTarget({
    ...
    canDrop: ({ resolved }) =>
      !freeze.frozen && resolved.length > 0 && resolved.every((a) => a.type === AssetType.Image),
