...
function LocaleProgressRow({ stat }: { stat: LocaleProgressStat }) {
  ...
  const translated = Math.min(
    Math.max(0, stat.completed - reviewed),
    Math.max(0, total - reviewed)
  );
...
export function DashboardTab({ active }: EditorTabComponentProps) {
  ...
  const projectKey = useMemo(() => {
    ...
    try {
      return getDashboardOpenProjectKey({
        ...
        projectIdentifier: context.services.get<ProjectService>(Services.Project).getProjectConfig()
          .identifier
      });
    } catch {
      return null;
  ...
  const hasCurve = windowPoints.some(
    (point) => point.status === "tracked" && (point.delta ?? 0) > 0
  );
  ...
  const variableCount = scale
    ? scale.variables.scene + scale.variables.saved + scale.variables.persistent
    : 0;

  return (
    <div
      className="h-full overflow-y-auto overflow-x-hidden bg-surface"
      data-help-topic="dashboard"
    >
        ...
        <DashboardSection title={t("dashboard.scale.title")}>
          {scale ? (
            <StatList>
              ...
              />
              <StatRow
                label={t("dashboard.scale.totalWords")}
                value={formatNumber(scale.totalWords)}
              />
              <StatRow
                label={t("dashboard.scale.characters")}
                value={formatNumber(scale.characters)}
              />
              ...
              />
              <StatRow
                label={t("dashboard.scale.uiSurfaces")}
                value={formatNumber(scale.uiSurfaces)}
              />
              <StatRow label={t("dashboard.scale.variables")} value={formatNumber(variableCount)} />
            </StatList>
          ) : (
            <p className="text-xs text-fg-subtle">
              {failed ? t("dashboard.failed") : t("dashboard.loading")}
            </p>
          )}
        </DashboardSection>
        ...
        {structure && (
          <DashboardSection title={t("dashboard.structure.title")}>
            <StatList>
              ...
              />
              <StatRow
                label={t("dashboard.structure.branches")}
                value={formatNumber(structure.branches)}
              />
            </StatList>
          </DashboardSection>
        )}
        ...
        <footer className="flex flex-col gap-3 border-t border-edge pt-5">
          <label className="flex items-center justify-between gap-4">
            <span className="text-xs text-fg-muted">
              {t("dashboard.footer.openOnWorkspaceOpen")}
            </span>
            <Switch
              size="sm"
