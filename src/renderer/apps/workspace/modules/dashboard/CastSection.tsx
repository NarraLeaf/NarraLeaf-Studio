 ...
 */
function SpeakerRow({ label, lines, words, muted }: { label: string; lines: string; words: string; muted?: boolean }) {
    return (
        <div className="flex items-baseline justify-between gap-3 py-1">
            <span className={cn("min-w-0 truncate text-xs", muted ? "text-fg-subtle" : "text-fg-muted")}>{label}</span>
            <span
                className={cn(
...
export function CastSection({ cast }: { cast: CastStats }) {
    ...
    const rest = useMemo(() => {
        const total = { speakers: cast.overflow?.speakers ?? 0, lines: cast.overflow?.lines ?? 0, words: cast.overflow?.words ?? 0 };
            ...
            <div className="rounded-md border border-edge bg-fill-subtle px-3 py-2">
                ...
                <div className="pt-0.5">
                    ...
                    {rest && (
                        <SpeakerRow
                            muted
                            label={tn("dashboard.cast.others", rest.speakers, { count: formatNumber(rest.speakers) })}
