...
export function openAssetPreviewTabsInEditor(
  ...
  const targetGroupId =
    splitInto && groupId
      ? (uiService
          .getStore()
          .splitEditorGroupForDrop(groupId, splitInto.direction, splitInto.side) ?? groupId)
      : groupId;
  ...
  if (returnFocusToAssetsPanel) {
    uiService.focus.setFocus(returnFocusToAssetsPanel.focusArea, returnFocusToAssetsPanel.panelId, {
      silent: true
    });
  }
}
 ...
 */
export function setWorkspaceSelectionToPrimaryAsset(
  context: WorkspaceContext,
  primary: Asset
): void {
  const uiService = context.services.get<UIService>(Services.UI);
