...
export function MagicTagDialog({
  visible,
  assets,
  template,
  onClose,
  ...
  return createPortal(
    <div className="nl-window-content-layer z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Content */}
        <div
          className="px-6 py-6 overflow-y-auto space-y-6"
        ...
        >
          ...
          {template.mode === "auto" && template.delimiters && template.delimiters.length > 0 && (
            <div className="space-y-3">
              <label className="text-sm font-medium text-fg-muted">
                {t("assets.magicTag.detectedDelimiters")}
              </label>
          ...
          {template.mode === "regex" && template.regex && (
            <div className="space-y-3">
              <label className="text-sm font-medium text-fg-muted">
                {t("assets.magicTag.regexPattern")}
              </label>
              <div className="bg-fill rounded-md p-3 font-mono text-sm text-fg-muted">
                {template.regex.pattern}
              </div>
              <p className="text-xs text-fg-muted">
                {t("assets.magicTag.captureGroups", {
                  groups: template.regex.captureGroups.join(", ")
                })}
              </p>
            </div>
          )}
          ...
          <div className="space-y-4">
            <label className="text-sm font-medium text-fg-muted">
              {t("assets.magicTag.categoryMapping")}
            </label>
            <div className="bg-fill rounded-lg p-4 space-y-4">
              ...
              <div className="pb-4 border-b border-edge">
                <div className="text-xs text-fg-muted mb-3">
                  {t("assets.magicTag.exampleFilename", { filename: template.example })}
                </div>
                ...
                {template.exampleSegments.map((segment, index) => (
                  <div key={index} className="flex items-center gap-4">
                    <div className="flex items-center gap-3 flex-1">
                      <span className="text-sm text-fg-muted w-12 text-center font-mono">
                        {index}
                      </span>
          ...
          {Object.keys(categoryMapping).length > 0 && (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-sm text-fg-muted">
                ...
                <span>
                  {t("assets.magicTag.summary", { tags: totalTagsToAdd, files: preview.length })}
                </span>
              </div>
            </div>
          )}
