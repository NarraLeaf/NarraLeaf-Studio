...
export function AssetSelector({
  ...
  const virtualAssetCount = useMemo(
    () => virtualGroups?.reduce((n, g) => n + g.assets.length, 0) ?? 0,
    [virtualGroups]
  );
    ...
    const updatePosition = () => {
      ...
      if (anchorRef?.current) {
        ...
        const top = clamp(
          96,
          viewportTop,
          Math.max(viewportTop, viewportHeight - viewportMargin - maxPanelHeight)
        );
        ...
        {visibleGroups.map((group) => {
          ...
          return (
            <div key={group.id} className="mb-1">
              ...
              {isExpanded && <div className="space-y-1">{renderGroup(group.id, level + 1)}</div>}
            </div>
          );
        })}

        {groupId !== null && assets.length > 0 && (
          <div className="space-y-1">{assets.map((asset) => renderAssetRow(asset, level))}</div>
        )}
  ...
  const headerLabel =
    title ?? t("assets.selector.selectType", { type: t(`assets.types.${assetType}`) });
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-fg-muted">
                {virtualAssetCount > 0
                  ? tn("assets.itemCount", typeAssets.length + virtualAssetCount)
                  : tn("assets.itemCount", typeAssets.length)}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2">
