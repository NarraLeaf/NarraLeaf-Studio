...
export function ImageCropper({
  ...
  const buildDefaultSelection = useCallback(
    (size: Size): CropRect => {
      const targetWidth =
        lockedSize?.width ?? Math.min(size.width * 0.6, maxSize?.width ?? size.width);
      const targetHeight =
        lockedSize?.height ?? Math.min(size.height * 0.6, maxSize?.height ?? size.height);
  ...
  const handleImageLoad = () => {
    ...
    const nextSelection = initialSelection
      ? sanitizeRect(initialSelection)
      : buildDefaultSelection(size);
    ...
    selectionDisplay && metrics
      ? [
          ...
          },
          {
            style: {
              left: 0,
              top: selectionDisplay.top + selectionDisplay.height,
              right: 0,
              bottom: 0
            }
          }
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-edge">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={loading}
              data-tip={t("assets.cropper.reload")}
              aria-label={t("assets.cropper.reload")}
            >
        ...
        <div className="px-4 py-3 rounded-xl flex items-center justify-between bg-surface-sunken">
          <div className="text-xs text-fg-muted">
            {selection
              ? t("assets.cropper.selection", {
                  width: Math.round(selection.width),
                  height: Math.round(selection.height)
                })
              : t("assets.cropper.waiting")}
          </div>
