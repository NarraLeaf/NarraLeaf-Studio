...
export function ModelImportWizard(props: {
  ...
  const scan = useCallback(
    async (family: ModelFamily, root: string) => {
      if (!context) return;
  ...
  const describeProblem = useCallback(
    (problem: ModelScanProblem): string => {
      switch (problem.kind) {
        case "missing":
          ...
          <div className="grid grid-cols-2 gap-2">
            {MODEL_FAMILIES.map((family) => (
              ...
              >
                ...
                <span className="text-2xs leading-relaxed text-fg-subtle">
                  {t(FAMILY_HINT[family])}
                </span>
              </button>
            ))}
          </div>
      ...
      <div className="space-y-3 py-1">
        <div className="flex items-center gap-2">
          ...
          {"root" in phase && phase.root ? (
            <span className="truncate text-2xs text-fg-subtle" data-tip={phase.root}>
              {phase.root}
            </span>
          ) : (
        ...
        {phase.step === "results" && phase.models.length === 0 && (
          <div className="space-y-1.5 py-6">
            ...
            <p className="text-2xs leading-relaxed text-fg-subtle">
              {t("assets.modelImport.noneFoundHint")}
            </p>
          </div>
        )}
          ...
          <>
            <div className="flex items-center justify-between">
              ...
              <button className="text-2xs text-primary hover:underline" onClick={toggleAll}>
                {t(
                  selected.length === phase.models.length
            ...
            <div className="max-h-[46vh] space-y-1.5 overflow-y-auto pr-1">
              {phase.models.map((model) => {
                ...
                const advisory = model.problems.filter(
                  (problem) => !isBlockingModelProblem(problem)
                );
                return (
                  <div
                    key={model.rootPath}
                    className="rounded-md border border-edge bg-fill-subtle px-2.5 py-2"
                  >
                    ...
                    {model.entryChoices.length > 1 && (
                      <div className="mt-1.5 flex items-center gap-2 pl-6">
                        ...
                        <Select
                          ...
                          options={model.entryChoices.map((choice) => ({
                            label: choice,
                            value: choice
                          }))}
