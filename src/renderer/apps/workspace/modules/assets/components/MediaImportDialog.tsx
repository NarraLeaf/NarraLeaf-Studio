...
export function MediaImportDialog({
  ...
  const runRef = useRef<{ stopped: boolean; jobId: string | null }>({
    stopped: false,
    jobId: null
  });
  ...
  const convertAndImport = useCallback(async () => {
    ...
    setStates(
      Object.fromEntries(
        convertible.map((problem) => [problem.path, { status: "waiting" as const, fraction: null }])
      )
    );
  ...
  const importAnyway = useCallback(
    () =>
      resolveWith(
        convertible.map((problem) => problem.path),
        [],
        []
      ),
    ...
    >
      <div className="space-y-3 py-1">
        <p className="text-xs text-fg-muted">
          {converting ? t("assets.mediaConvert.convertingIntro") : t("assets.mediaConvert.intro")}
        </p>
          ...
          <div className="max-h-[46vh] space-y-3 overflow-y-auto pr-1">
            {grouped.map(({ group, rows }) => (
              <section key={group} className="space-y-1.5">
                ...
                {group !== "refused" && (
                  <p className="text-2xs leading-relaxed text-fg-muted">{t(GROUP_HINT[group])}</p>
                )}
 ...
 */
function ConvertingRow({
  problem,
  state
}: {
  problem: MediaImportProblem;
      ...
      <div className="mt-1">
        {state.status === "converting" ? (
          ...
          <Progress
            ...
            size="sm"
            variant={
              state.status === "done" ? "success" : state.status === "failed" ? "error" : "default"
            }
            animated={false}
          />
        )}
