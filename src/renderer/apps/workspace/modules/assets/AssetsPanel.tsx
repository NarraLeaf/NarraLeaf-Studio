...
function resolveAssetGroupPathIds(
  pathIds: string[],
  ...
  Object.values(groups)
    .flat()
    .forEach((group) => groupById.set(group.id, group));
...
export function AssetsPanel({ panelId, payload }: PanelComponentProps<AssetsPanelPayload>) {
  ...
  const [assetsIconToolbarCenter, setAssetsIconToolbarCenter] =
    useState<AssetsIconViewToolbarCenter | null>(null);
  ...
  const [categoryOpenItems, setCategoryOpenItems] = useState<string[]>(
  ...
  const { assets, groups, loading, hasLoaded, error, loadAssets } = useAssetData({
    context,
    isInitialized
  });

  const { focusedItemId, setFocusedItemId, handleAssetClick, handleGroupFocus, setFocusToPanel } =
    useAssetFocus({ context, panelId, focusArea });

  const { selectedItems, isMultiSelectMode, handleItemSelect, handleClearSelection } =
    useMultiSelection({
      assets,
      groups,
  ...
  const {
    ...
    context,
    viewMode === "overview" || filtersNeedLibrarySnapshot(activeFilters)
  );
  ...
  const handleIconGroupPathChange = useCallback(
    (next: string[]) => {
  ...
  const handleRetryFailedImports = useCallback(() => {
    ...
    void handleRetryImport(
      run.category,
      importState.failures.map((failure) => failure.path),
      run.groupId
    );
  ...
  const handleMagicTagsApply = useCallback(
    async (categoryMapping: Record<number, string>) => {
  ...
  const { menuState, contextMenu, showContextMenu, closeContextMenu } = useAssetsContextMenu({
    ...
    handleCreateTextFile,
    handleImportToGroup,
    handleCreateMagicTags: handleMagicTagsClick,
  ...
  const handleRootDrop = useCallback(
    async (
      ...
      contextualGroup?: AssetGroup | null
    ) => {
  ...
  useEffect(() => {
    ...
    const hasClipboardContent =
      !!clipboard && (clipboard.assets.length > 0 || clipboard.groups.length > 0);
    const when = (focus: { area: FocusArea; targetId?: string }) =>
      focus.area === focusArea && focus.targetId === panelId;
  ...
  if (loading && !hasLoaded && Object.values(assets).every((arr) => arr.length === 0)) {
    return (
      <div className="p-4 flex items-center gap-2 text-fg-muted">
        <RefreshCw className="w-4 h-4 animate-spin" /> <span>{t("assets.loading")}</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-4 text-danger flex items-start gap-2">
        <AlertCircle className="w-4 h-4" />{" "}
        <div>
          <p>{t("assets.loadError")}</p>
  ...
  const effectiveOpenItems = isNarrowed
    ? ASSET_CATEGORY_ORDER.filter(
        (category) => filteredAssets[category].length > 0 || filteredGroups[category].length > 0
      )
    : categoryOpenItems;
              ...
              {isSearchActive ? (
                <>
                  ...
                  <button
                    ...
                    data-tip={t("assets.closeSearch")}
                    aria-label={t("assets.closeSearch")}
                  >
                    <X className="w-4 h-4" />
                  </button>
                </>
                ...
                <>
                  ...
                  <button
                    ...
                    data-tip={t("assets.searchTooltip")}
                    aria-label={t("assets.searchTooltip")}
                  >
