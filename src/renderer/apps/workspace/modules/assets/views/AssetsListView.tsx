...
export function AssetsListView({
  ...
  const hasAnyItems = useMemo(
    () =>
      Object.values(filteredAssets).some((list) => list.length > 0) ||
      Object.values(filteredGroups).some((list) => list.length > 0),
    [filteredAssets, filteredGroups]
  );
            ...
            title={
              <span className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-fg-subtle">
                  {tn("assets.itemCount", categoryAssets.length)}
                </span>
              </span>
            }
            actions={
              ...
              {(categoryAssets.length > 0 || categoryGroups.length > 0) && (
                <div className="py-1">
                  {categoryGroups
                    ...
                    .map((group) => (
                      <GroupItem key={group.id} group={group} category={category} level={0} />
                    ))}
                  {categoryAssets
                    .filter((a) => !a.groupId)
                    .map((asset) => (
                      <AssetItem key={asset.id} asset={asset} category={category} level={0} />
                    ))}
                </div>
              )}
            </div>
      ...
      {isOpen && (
        <div>
          {childGroups.map((child) => (
            <GroupItem key={child.id} group={child} category={category} level={level + 1} />
          ))}
          {groupAssets.map((asset) => (
            <AssetItem key={asset.id} asset={asset} category={category} level={level + 1} />
          ))}
        </div>
      )}
    </div>
...
function AssetItem({
  ...
  return (
    ...
    >
      ...
      {asset.tags.length > 0 && (
        <span className="text-xs text-fg-subtle">+{asset.tags.length}</span>
      )}
    </div>
  );
}
