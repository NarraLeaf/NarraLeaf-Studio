...
export function AssetsIconView({
  ...
  const parentPredicate = useCallback(
    (parentId?: string) =>
      isNarrowed ? true : activeGroup ? parentId === activeGroup.group.id : !parentId,
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
        {displayCategories.map((category) => {
          ...
          const categoryAssets = filteredAssets[category].filter((asset) =>
            parentPredicate(asset.groupId)
          );
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-fg-subtle">
                      {tn("assets.iconView.assetCount", scopedAssets.length)}
                    </p>
                  </div>
                </div>
              ...
              {hasItems && (
                ...
                >
                  {categoryGroups.map((group) => {
                    const childGroups = filteredGroups[category].filter(
                      (g) => g.parentGroupId === group.id
                    );
                    const childAssets = filteredAssets[category].filter(
  ...
  return (
    ...
    >
      <div className="aspect-square w-full overflow-hidden rounded-md bg-surface-sunken">
        {preview.length > 0 ? (
          <div
            className={`grid h-full w-full gap-px ${preview.length > 1 ? "grid-cols-2 grid-rows-2" : ""}`}
          >
            {preview.map((asset) => (
              <AssetThumbnail
                key={asset.id}
      ...
      <div className="flex min-w-0 items-center gap-1.5">
        ...
        <span
          ...
          data-tip={tn("assets.itemCount", childCount)}
        >
          {childCount}
        </span>
      </div>
...
function AssetIconTile({ asset, category }: { asset: Asset; category: AssetCategory }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        {support && (
          <AssetSupportBadge
            record={support}
            onConvert={
              asset.source === AssetSource.Local ? () => handleConvertMedia(asset) : undefined
            }
      ...
      <div className="flex min-w-0 items-center gap-1.5">
        <span className="truncate text-xs font-medium" data-tip={asset.name}>
          {asset.name}
        </span>
        {asset.tags.length > 0 && (
          <span className="ml-auto shrink-0 text-2xs text-fg-subtle">
            {tn("assets.iconView.tagCount", asset.tags.length)}
          </span>
        )}
      </div>
    </div>
