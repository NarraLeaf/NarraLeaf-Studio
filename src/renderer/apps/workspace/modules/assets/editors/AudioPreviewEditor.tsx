...
export function AudioPreviewEditor({
  ...
  payload,
  active
}: EditorComponentProps<AudioPreviewPayload>) {
  ...
  const [committedLoop, setCommittedLoop] = useState<LoopPoints>(() =>
    fromAssetExtras(payload?.asset.extras)
  );
  ...
  useEffect(() => {
    ...
    setSelection((current) => {
      ...
      return current.start >= length
        ? null
        : { start: current.start, end: Math.min(current.end, length) };
    });
  }, [clip]);
  ...
  const auditionRange = useMemo<PlayRange | null>(() => {
    ...
    const toSample = (ms: number) =>
      Math.min(totalSamples, Math.round((ms / 1000) * clip.sampleRate));
  ...
  const togglePlay = useCallback(() => {
    ...
    play(
      resolvePlayStart({ position, selection: auditionRange, totalSamples, finished }),
      auditionRange
    );
    ...
    (factor: number) => {
      setView((current) =>
        zoomAt(
          ...
          totalSamples,
          factor,
          playing ? position : (current.start + current.end) / 2
        )
      );
    },
  ...
  const keybindings = useMemo<KeybindingDefinition[]>(
    () => [
      ...
      { id: "to-end", key: "end", description: "Go to end", handler: () => seekTo(totalSamples) },
      {
        id: "nudge-back",
        key: "arrowleft",
        description: "Nudge back",
        handler: () => nudge(-NUDGE_SECONDS)
      },
      {
        id: "nudge-forward",
        key: "arrowright",
        description: "Nudge forward",
        handler: () => nudge(NUDGE_SECONDS)
      },
      {
      ...
      // control to grey out, which is what `freeze.run` is for.
      {
        ...
        handler: () => setLoop((value) => !value)
      },
      {
        id: "mark-in",
        key: "i",
        description: "Set in point",
        handler: freeze.run(() => markLoopPoint("in"))
      },
      {
        id: "mark-loop",
        key: "l",
        description: "Set loop point",
        handler: freeze.run(() => markLoopPoint("loop"))
      },
      {
        id: "mark-out",
        key: "o",
        description: "Set out point",
        handler: freeze.run(() => markLoopPoint("out"))
      },
      {
        id: "go-to-in",
        key: "shift+i",
        description: "Go to in point",
        handler: () => goToLoopPoint("in")
      },
      {
        id: "go-to-loop",
        key: "shift+l",
        description: "Go to loop point",
        handler: () => goToLoopPoint("loop")
      },
      {
        id: "go-to-out",
        key: "shift+o",
        description: "Go to out point",
        handler: () => goToLoopPoint("out")
      },
      // Clearing a marker is as much a write as setting one.
      {
        ...
        handler: freeze.run(() => clearLoopPoint("in"))
      },
      {
        id: "clear-loop",
        key: "mod+shift+l",
      ...
      },
      {
        ...
        handler: freeze.run(() => clearLoopPoint("out"))
      },
      ...
      // record without ever going through `commitLoop` - which is why they need the guard too.
      {
        id: "undo",
        key: "mod+z",
        description: "Undo marker change",
        handler: freeze.run(() => void loopHistory.undo())
      },
      {
        id: "redo",
        key: "mod+shift+z",
        description: "Redo marker change",
        handler: freeze.run(() => void loopHistory.redo())
      },
      ...
      { id: "zoom-out", key: "-", description: "Zoom out", handler: () => zoomBy(1 / 1.4) },
      {
        id: "zoom-fit",
        key: "0",
        description: "Fit whole clip",
        handler: () => setView(fitAll(totalSamples))
      }
    ],
    [
      togglePlay,
    ...
    <div className="flex h-full flex-col bg-surface" data-help-topic="audioClips">
      ...
      <TooltipGroup className="flex shrink-0 flex-wrap items-center gap-1 border-b border-edge bg-surface-raised px-2 py-1.5">
        ...
        <button
          ...
          className={ICON_BUTTON_CLASS}
          data-tip={t("assets.audio.editor.zoomOut")}
          aria-label={t("assets.audio.editor.zoomOut")}
        >
        ...
        <button
          ...
          onClick={() => zoomBy(1.4)}
          className={ICON_BUTTON_CLASS}
          data-tip={t("assets.audio.editor.zoomIn")}
          aria-label={t("assets.audio.editor.zoomIn")}
        >
        ...
        <button
          ...
          className={ICON_BUTTON_CLASS}
          data-tip={t("assets.audio.editor.zoomFit")}
          aria-label={t("assets.audio.editor.zoomFit")}
        >
        ...
        <button
          ...
          className={ICON_BUTTON_CLASS}
          data-tip={t("assets.audio.editor.zoomSelection")}
          aria-label={t("assets.audio.editor.zoomSelection")}
        >
        ...
        <button
          ...
          data-tip={t("assets.audio.editor.markLoop")}
          aria-label={t("assets.audio.editor.markLoop")}
        >
      ...
      <div className="flex shrink-0 items-center gap-3 border-t border-edge px-3 py-1 text-2xs tabular-nums text-fg-subtle">
        {hasSelection && selection && (
          <span className="text-fg-muted">
            {formatTime(selection.start / clip.sampleRate)} –{" "}
            {formatTime(selection.end / clip.sampleRate)}
            {" ("}
            {formatTime(selectionSeconds)}
            {")"}
          </span>
        )}
        {(loopPoints.inMs !== null ||
          loopPoints.loopStartMs !== null ||
          loopPoints.outMs !== null) && (
