...
describe("textPreferencePatch", () => {
    it("writes nothing when a file is merely opened", () => {
        ...
        expect(textPreferencePatch("open", undefined, { encoding: "gbk", lineEnding: "CRLF" })).toBeNull();
        expect(
    ...
    it("writes an explicit choice", () => {
        ...
        expect(textPreferencePatch("save-with", { textEncoding: "utf8" }, { encoding: "shiftjis" })).toEqual({
    ...
    it("writes nothing when the record already says this", () => {
        expect(
            textPreferencePatch("reopen-with", { textEncoding: "gbk", textEol: "lf" }, { encoding: "gbk" }),
        ).toBeNull();
