  ...
  return (
    <div className="flex items-center justify-between px-4 py-2 border-b border-edge bg-surface-raised">
      <div className="flex items-center gap-4">
        ...
        <span className="text-sm text-fg-muted">{imageData.metadata.format.toUpperCase()}</span>
        <span className="text-sm text-fg-muted">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          data-tip={t("assets.image.zoomOut")}
          aria-label={t("assets.image.zoomOut")}
        >
          <ZoomOut className="w-4 h-4" />
        </button>
        <span className="text-sm text-fg-muted min-w-16 text-center">{controls.zoomLabel}</span>
        <button
          onClick={controls.zoomIn}
          className="p-1 rounded-md hover:bg-fill text-fg-muted hover:text-fg transition-colors cursor-default"
          data-tip={t("assets.image.zoomIn")}
          aria-label={t("assets.image.zoomIn")}
        >
        ...
        <button
          ...
          data-tip={t("assets.image.resetView")}
          aria-label={t("assets.image.resetView")}
        >
...
export function ImagePreviewEditor({ tabId, payload }: EditorComponentProps<ImagePreviewPayload>) {
  ...
  const handleZoomIn = useCallback(
    () => withFocusedPreview((controls) => controls.zoomIn()),
    [withFocusedPreview]
  );
  const handleZoomOut = useCallback(
    () => withFocusedPreview((controls) => controls.zoomOut()),
    [withFocusedPreview]
  );
  const handleResetView = useCallback(
    () => withFocusedPreview((controls) => controls.resetView()),
    [withFocusedPreview]
  );
  ...
  useEffect(() => {
    ...
    const focusWhen = (ctx: any) =>
      ...
      typeof ctx?.targetId === "string" &&
      mountedPreviews.has(ctx.targetId);
