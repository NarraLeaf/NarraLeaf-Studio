...
function chooseTickSeconds(secondsPerPixel: number): number {
  ...
  return (
    candidates.find((candidate) => candidate >= targetSeconds) ?? candidates[candidates.length - 1]
  );
}
...
export function WaveformView({
  ...
  const draw = useCallback(() => {
    ...
    if (loop.inMs !== null && loop.outMs !== null) {
      ...
      const split =
        loop.loopStartMs !== null && loop.loopStartMs > loop.inMs && loop.loopStartMs < loop.outMs
      ...
      context.fillRect(
        from,
        RULER_HEIGHT + 2,
        Math.max(1, (split ?? to) - from),
        MARKER_STRIP_HEIGHT - 4
      );
      if (split !== null) {
    ...
    const hoveredEnd =
      hover?.inStrip && gestureRef.current === null
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      ...
      if (
        !rect ||
        (Math.round(rect.width) === lastWidth && Math.round(rect.height) === lastHeight)
      ) {
        return;
      }
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (!gesture) {
      ...
      event.currentTarget.style.cursor =
        overPoint || overEdge ? "col-resize" : inStrip ? "default" : "text";
      draw();
      return;
    }
    ...
    if (gesture.kind === "loop") {
      ...
      if (
        !gesture.moved &&
        Math.abs(sample - gesture.sample) / samplesPerPixel < DRAG_THRESHOLD_PX
      ) {
        return;
      }
      gesture.moved = true;
    ...
    if (gesture.kind === "resize") {
      onSelectionChange({
        start: Math.min(gesture.anchor, sample),
        end: Math.max(gesture.anchor, sample)
      });
      return;
    }
    ...
    gesture.moved = true;
    onSelectionChange({
      start: Math.min(gesture.originSample, sample),
      end: Math.max(gesture.originSample, sample)
    });
  };
