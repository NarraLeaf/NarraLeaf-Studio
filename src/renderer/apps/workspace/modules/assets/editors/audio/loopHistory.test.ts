...
describe("fromAssetExtras", () => {
  it("prefers the current shape", () => {
    expect(
      fromAssetExtras({ audioLoop: { inMs: 1, outMs: 2 }, cuePoints: [{ timeMs: 999 }] })
    ).toEqual(loop(1, 2));
  });
  ...
  it("drops a stored loop point the shared normalizer rejects", () => {
    ...
    expect(fromAssetExtras({ audioLoop: { inMs: 0, outMs: 84_000, loopStartMs: 90_000 } })).toEqual(
      loop(0, 84_000)
    );
  });

  it("reads the superseded cue list as in/out, earliest first", () => {
    expect(fromAssetExtras({ cuePoints: [{ timeMs: 800 }, { timeMs: 200 }] })).toEqual(
      loop(200, 800)
    );
  });
  ...
  it("ignores cue points beyond the first two", () => {
    expect(fromAssetExtras({ cuePoints: [{ timeMs: 1 }, { timeMs: 2 }, { timeMs: 3 }] })).toEqual(
      loop(1, 2)
    );
  });
