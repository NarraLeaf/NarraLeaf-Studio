...
describe("resolvePlaybackGeometry", () => {
  it("is the whole clip when no range is given", () => {
    expect(
      resolvePlaybackGeometry({ from: 300, range: null, totalSamples: 1000, looping: true })
    ).toEqual({
      start: 300,
      end: 1000,
      loopStart: 0,
  ...
  it("clamps a loop point that sits outside the range, so it can never describe an empty loop", () => {
    const outside = (loopStart: number) =>
      resolvePlaybackGeometry({
        from: 0,
  ...
  it("keeps the start inside the clip", () => {
    expect(
      resolvePlaybackGeometry({ from: 5000, range: null, totalSamples: 1000, looping: false }).start
    ).toBe(999);
    expect(
      resolvePlaybackGeometry({ from: -5, range: null, totalSamples: 1000, looping: false }).start
    ).toBe(0);
    expect(
      resolvePlaybackGeometry({ from: 0, range: null, totalSamples: 0, looping: false }).start
    ).toBe(0);
  });
});
...
describe("playbackPosition", () => {
  ...
  it("holds at the loop point rather than dividing by zero on a degenerate loop", () => {
    expect(
      playbackPosition(geometry({ start: 0, end: 500, loopStart: 500, looping: true }), 900)
    ).toBe(500);
  });
