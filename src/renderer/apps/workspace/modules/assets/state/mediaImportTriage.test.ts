...
function probed(
  ...
  durationUs: number | null = 5_000_000
): MediaProbeOutcome {
...
describe("planMediaImport", () => {
  ...
  it("leaves a playable file alone", async () => {
    const plan = await planMediaImport(["C:/media/intro.mp4"], async () =>
      probed(verdict({ tier: "accept" }))
    );
  ...
  it("carries the probe's duration into a container swap", async () => {
    const plan = await planMediaImport(["C:/media/clip.avi"], async () =>
      probed(
        verdict({
          tier: "remux",
  ...
  it("calls a re-encode partially usable only when something in it already plays", async () => {
    const hevcWithSound = await planMediaImport(["C:/media/phone.mp4"], async () =>
      probed(
        verdict({
          tier: "reencode",
    ...
    const everythingDead = await planMediaImport(["C:/media/old.mpg"], async () =>
      probed(
        verdict({
          tier: "reencode",
          reason: "codec-unsupported",
  ...
  it("separates the two reasons there is nothing to offer", async () => {
    ...
    expect(probe).not.toHaveBeenCalled();
    expect(playlist.problems[0]).toMatchObject({
      group: "refused",
      refusal: "notMedia",
      target: null
    });

    const empty = await planMediaImport(["C:/media/subs.mkv"], async () =>
      probed(verdict({ tier: "refuse", reason: "no-streams" }))
    );
