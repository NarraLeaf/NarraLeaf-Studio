...
describe("runMediaConversion", () => {
  ...
  it("ends on the status, not on the fraction reaching 1", async () => {
    ...
    expect(outcome).toEqual({
      status: "done",
      outputPath: "C:/project/.nlstudio/convert/run/0/clip.mp4"
    });
  ...
  it("passes a missing duration through as null rather than zero", async () => {
    const { spy, fractions } = hooks();
    await runMediaConversion(
      { ...REQUEST, durationUs: null },
      bridgeOver([
        { jobId: "j", status: "converting" },
        {
          jobId: "j",
          status: "converting",
          progress: { outTimeUs: 1, durationUs: null, fraction: null }
        },
  ...
  it("distinguishes a stop, a missing converter and a failure", async () => {
    ...
    await expect(
      runMediaConversion(
        REQUEST,
        bridgeOver([{ jobId: "j", status: "unavailable", error: "no ffmpeg" }]),
        spy
      )
    ).resolves.toEqual({ status: "unavailable", detail: "no ffmpeg" });
    ...
    ).resolves.toEqual({ status: "failed", detail: "ffmpeg exited with 1" });
  });
  ...
  it("refuses to report success without a file", async () => {
    const { spy } = hooks();
    await expect(
      runMediaConversion(REQUEST, bridgeOver([{ jobId: "j", status: "done" }]), spy)
    ).resolves.toEqual({ status: "failed", detail: undefined });
  });
