...
export function filtersNeedLibrarySnapshot(activeFilters: readonly ActiveFilter[]): boolean {
  return activeFilters.some(
    (filter) => filter.filterId === "size" || filter.filterId === "referenced"
  );
}
...
export function useAssetFilters({
  ...
  const filteredData = useMemo(() => {
    ...
    const extensionFilters = activeFilters
      .filter((f) => f.filterId === "file-extensions")
      .map((f) => f.optionId);
    const typeFilters = new Set(
      activeFilters.filter((f) => f.filterId === "type").map((f) => f.optionId)
    );
    ...
    ASSET_CATEGORY_ORDER.forEach((category) => {
      ...
      filteredAssets[category] = categoryAssets.filter((asset) => {
        ...
        if (tagFilters.length > 0 && !tagFilters.every((tag) => asset.tags.includes(tag)))
          return false;
        ...
        if (
          ...
          bytesByAssetId &&
          !matchesSizeBand(bytesByAssetId.get(asset.id), sizeFilters)
        ) {
          return false;
        }
