  ...
  const unreferenced = useMemo(
    () =>
      (snapshot?.entries ?? [])
        ...
        .sort(
          (a, b) => (b.bytes ?? 0) - (a.bytes ?? 0) || a.asset.name.localeCompare(b.asset.name)
        ),
    [snapshot]
  );
            ...
            <li key={bucket.category} className="flex flex-col gap-1">
              <div className="flex items-baseline justify-between gap-3">
                <span className="min-w-0 truncate text-xs text-fg-muted">
                  {t(
                    `assets.categories.${bucket.category}` as `assets.categories.${AssetCategory}`
                  )}
                </span>
...
function AssetDetail({ entry }: { entry: AssetOverviewEntry }) {
  ...
  const previewUrl = useBadgeImageUrl(
    asset.type === AssetType.Image
      ? { kind: "project", asset: asset as Asset<AssetType.Image> }
      : null
  );
  ...
  const sourceUrl =
    asset.source === AssetSource.Remote
  ...
  return (
    <div className="mb-2 ml-2 flex flex-col gap-2.5 border-l border-edge pl-3 pt-2">
      ...
      <dl className="flex flex-col gap-1.5">
        ...
        <DetailRow
          label={t("assets.overview.stat.referenced")}
          value={tn("assets.overview.uses", entry.referenceCount)}
        />
      </dl>
      ...
      <div>
        ...
        >
          <ChevronDown
            className={cn("h-3 w-3 transition-transform", !showStorage && "-rotate-90")}
          />
        ...
        {showStorage && (
          <dl className="mt-1.5 flex flex-col gap-1.5">
            ...
            {sourceUrl && <CopyRow label={t("properties.asset.remote.url")} value={sourceUrl} />}
          </dl>
        )}
      </div>
