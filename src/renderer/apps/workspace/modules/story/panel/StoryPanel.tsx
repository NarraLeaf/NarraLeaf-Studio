...
export function StoryPanel({ panelId }: PanelComponentProps) {
  ...
  const {
    beginExport: beginScriptExport,
    beginImport: beginScriptImport,
    dialogs: scriptDialogs
  } = useStoryScriptIo();
  ...
  const [chapterOpenItemsByStoryId, setChapterOpenItemsByStoryId] = useState<
  ...
  useEffect(() => {
    ...
    setChapterOpenItemsByStoryId((prev) => {
      ...
      const current = hasSavedForStory
  ...
  const handleSetDefaultStory = useCallback(
  ...
  const handleOpenSceneFlow = useCallback(
    (entry: StoryLibraryEntry) => {
      if (!context) {
        return;
      }
  ...
  const buildStoryContextMenu = useCallback(
    (entry: StoryLibraryEntry): ContextMenuDef => {
   ...
   */
  const withDeveloperRows = useCallback(
    (items: ContextMenuDef, entries: DeveloperIdEntry[]) =>
  ...
  const handleOpenStoryMenu = useCallback(
    (event: React.MouseEvent, entry: StoryLibraryEntry) => {
      event.stopPropagation();
      setMenuItems(
        withDeveloperRows(buildStoryContextMenu(entry), [{ kind: "story", value: entry.id }])
      );
      showMenu(event);
    },
  ...
  const handleSetEntryScene = useCallback(
    (scene: StoryScene) => {
      if (!storyService || !selectedStoryId) {
        return;
      }
  ...
  const buildSceneContextMenu = useCallback(
    (scene: StoryScene): ContextMenuDef => {
  ...
  const buildChapterContextMenu = useCallback(
    (chapter: StoryChapter): ContextMenuDef => [
      {
  ...
  const handleOpenChapterMenu = useCallback(
    (event: React.MouseEvent, chapter: StoryChapter) => {
      event.preventDefault();
      // Without this the accordion header treats the click as a toggle, so the menu opens and the
      // chapter collapses under it.
      event.stopPropagation();
      setMenuItems(
        withDeveloperRows(buildChapterContextMenu(chapter), [
          { kind: "chapter", value: chapter.id }
        ])
      );
      showMenu(event);
    },
  ...
  const handleOpenSceneMenu = useCallback(
    (event: React.MouseEvent, scene: StoryScene) => {
      event.preventDefault();
      event.stopPropagation();
      setMenuItems(
        withDeveloperRows(buildSceneContextMenu(scene), [{ kind: "scene", value: scene.id }])
      );
      showMenu(event);
    },
  ...
  const chapterOpenItems = selectedStoryId
    ? (chapterOpenItemsByStoryId[selectedStoryId] ?? [])
    : [];
  ...
  const handleChapterOpenChange = useCallback(
    (nextOpenItems: string[]) => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-panel-id={panelId}>
      <div className="min-h-0 flex-1 overflow-y-auto">
        <Accordion
          openItems={getRenderedStoryRootOpenItems(
            filterStoryRootOpenItems(rootOpenItems),
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        data-tip={t("story.panel.storyActions")}
                        aria-label={t("story.panel.storyActions")}
            ...
            >
              {loadingDocument ? (
                ...
                >
                  {document.chapters.map((chapter) => (
                    <AccordionItem
                      ...
                      id={chapter.id}
                      level={1}
                      title={t("story.panel.chapterTitle", {
                        name: chapter.name,
                        count: chapter.sceneIds.length
                      })}
                      className="!border-b-0"
                      headerProps={{
                    ...
                    >
                      {chapter.sceneIds.length === 0 ? (
                        <div className="px-8 py-2 text-xs text-fg-subtle">
                            ...
                            >
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="truncate text-2xs text-fg-subtle">
                                  {tn("story.panel.lineCount", lineCount)}
                                </div>
                              </div>
                              <button
                                type="button"
                                className="rounded-md p-1 text-fg-muted opacity-0 hover:bg-fill hover:text-fg group-hover/scene:opacity-100"
                                data-tip={t("story.panel.sceneActions")}
                                aria-label={t("story.panel.sceneActions")}
                ...
                <div className="px-3 py-3 text-sm text-fg-muted">
                  {t("story.panel.documentUnavailable")}
                </div>
              )}
            </AccordionItem>
          ) : null}
