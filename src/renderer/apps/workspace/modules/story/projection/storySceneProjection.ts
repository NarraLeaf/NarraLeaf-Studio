...
export function updateBlockTextValue(
  block: StoryBlock,
  value: string
...
function projectBlockLine(
  ...
  if (block.kind === "action") {
    return {
      text: `${indent}${formatAction(block.payload, scene, document, variableNames)}`,
      editable: false,
      prefix: ""
    };
  }
  if (block.kind === "control") {
    if (block.payload.control === "conditionBranch") {
      const label =
        block.payload.branch === "else"
          ? "else"
          : `${block.payload.branch} ${formatCondition(block.payload.condition, scene, document, variableNames)}`;
      return { text: `${indent}/${label}`, editable: false, prefix: "" };
    }
    if (block.payload.control === "label") {
      return {
        text: `${indent}/label ${block.payload.name}`.trimEnd(),
        editable: false,
        prefix: ""
      };
    }
    if (block.payload.control === "goto") {
      return {
        text: `${indent}/goto ${block.payload.targetLabel}`.trimEnd(),
        editable: false,
        prefix: ""
      };
    }
    ...
    if (block.payload.control === "cut") {
      ...
      return {
        text: `${indent}/cut ${block.payload.appTagId}`.trimEnd(),
        editable: false,
        prefix: ""
      };
    }
    ...
    if (block.payload.control === "repeat") {
      return block.payload.until
        ? {
            text: `${indent}/until ${formatCondition(block.payload.until, scene, document, variableNames)}`,
            editable: false,
            prefix: ""
          }
  ...
  if (block.kind === "jump") {
    return {
      text: `${indent}/jump ${getSceneName(document?.scenes, block.payload.targetSceneId)}`,
      editable: false,
      prefix: ""
    };
  }
  ...
  if (block.kind === "declaration") {
    ...
    const token =
      block.payload.scope === "scene"
        ? "local"
        : block.payload.scope === "saved"
          ? "var"
          : "persis";
    const suffix =
      block.payload.defaultValue !== undefined
        ? ` ${formatStoryLiteral(block.payload.defaultValue)}`
        : "";
    return {
      text: `${indent}/${token} ${block.payload.name}${suffix}`,
      editable: false,
      prefix: ""
    };
  }
...
function formatAction(
  ...
  if (payload.action === "character") {
    const subject = payload.characterId
      ? ` ${payload.characterId}`
      : payload.objectName
        ? ` ${payload.objectName}`
        : "";
    ...
    const suffix =
      payload.operation === "setName" && payload.displayName
        ? ` ${payload.displayName}`
        : payload.puppetName
          ? ` ${payload.puppetName}`
  ...
  if (payload.action === "wait") {
    return payload.mode === "duration"
      ? `/wait ${formatStorySecondsLabel(payload.durationMs ?? 0)}`
      : "/wait click";
  }
...
function describeAssignment(
  ...
  const readsTarget = (node: typeof ast): boolean =>
  ...
  if (ast.kind === "binary" && (ast.op === "+" || ast.op === "-") && readsTarget(ast.left)) {
    ...
    const step =
      ast.right.kind === "literal" && ast.right.value === 1 ? "" : ` ${formatExpr(ast.right)}`;
...
function describeVariableRef(
  ...
  if (ref.scope === "saved") {
    return (
      (document ? savedVariableDefs(document) : {})[ref.variableId]?.name ??
