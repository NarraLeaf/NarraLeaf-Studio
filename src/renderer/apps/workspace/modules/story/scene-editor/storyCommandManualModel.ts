...
function describeType(type: StoryCommandParamType, t: ManualTranslate): string {
  switch (type.kind) {
    ...
    case "character":
      return t(
        type.allowTemp ? "story.manual.type.characterOrName" : "story.manual.type.character"
      );
    case "characterForm":
    ...
    case "target":
      return type.accepts
        .map((kind) => t(`story.manual.target.${kind}` as TranslationKey))
        .join(" / ");
    case "content":
    ...
    case "expression":
      return t(
        type.expects === "boolean"
          ? "story.manual.type.expressionBoolean"
          : "story.manual.type.expression"
      );
  }
}
...
function manualParams(spec: AnyStoryCommandSpec, t: ManualTranslate): StoryCommandManualParam[] {
  return Object.entries(spec.params).map(([name, param]) => ({
    ...
    accepts: paramTypes({ ...param, name })
      .map((type) => describeType(type, t))
      .join(" | "),
 ...
 * order.
 */
export function filterStoryCommandManual(
  entries: StoryCommandManualEntry[],
  rawQuery: string
): StoryCommandManualEntry[] {
