...
const anchor = (blockId: string | null): StoryEditorScrollAnchor => ({
  blockId,
  offset: 0,
  scrollTop: 0
});
...
describe("storyEditorSessionStore view state", () => {
  ...
  it("merges focus and scroll patches without clobbering each other", () => {
    ...
    patchStoryEditorViewState(panelState, sceneId, {
  ...
  it("keeps per-scene view states independent", () => {
    const panelState = fakePanelState();
    patchStoryEditorViewState(panelState, "scene-a", {
      activeBlockId: "a",
      selectedBlockIds: ["a"]
    });
    patchStoryEditorViewState(panelState, "scene-b", {
      activeBlockId: "b",
      selectedBlockIds: ["b"]
    });
...
describe("storyEditorSessionStore view prefs", () => {
  it("opens an untouched project unfiltered, at the default density", () => {
    expect(getStoryEditorViewPrefs(fakePanelState())).toEqual({
      density: "compact",
      selectedRowFacets: [],
      selectedRowSpeakers: []
    });
  ...
  it("carries a pre-filter 'narrative only' over as the preset it always meant", () => {
    const panelState = fakePanelState();
    panelState.setPanelState("story:editor:view-prefs", {
      density: "standard",
      narrativeOnly: true
    });
  ...
  it("round-trips both axes and drops a facet this build no longer knows", () => {
    const panelState = fakePanelState();
    patchStoryEditorViewPrefs(panelState, {
      selectedRowFacets: ["sound", "gone"] as never,
      selectedRowSpeakers: ["id:c1", "oops"]
    });
