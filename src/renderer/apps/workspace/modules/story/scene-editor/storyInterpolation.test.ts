...
function documentWithDeclarations(
  declarations: { id: string; scope: string; name: string }[]
): StoryDocument {
  ...
  return {
    ...
    scenes: {
      "scene-1": { id: "scene-1", name: "Scene", runtimeName: "scene", rootBlockIds, blocks }
    }
  } as StoryDocument;
}
...
describe("resolveVariableRefName", () => {
  ...
  it("falls back when neither surface declares the ref", () => {
    ...
    expect(
      resolveVariableRefName(document, "scene-1", [], { scope: "saved", variableId: "gone" })
    ).toBe("variable");
  });
});
