...
describe("getQuickParams", () => {
  it("exposes a wait's duration (with presets) only in duration mode", () => {
    ...
    expect((params[0].value as { presetsMs: number[] }).presetsMs).toEqual([
  ...
  it("exposes audio volume + loop and applies edits without touching other fields", () => {
    const params = getQuickParams(
      action({ action: "audio", operation: "setBgm", assetId: "a1", volume: 0.8, loop: true })
    );
    expect(params.map((p) => p.id)).toEqual(["vol", "loop"]);
    expect(params[0].apply({ kind: "percent", ratio: 0.5 })).toMatchObject({
      assetId: "a1",
      volume: 0.5,
      loop: true
    });
    expect(params[1].apply({ kind: "toggle", on: false })).toMatchObject({
      volume: 0.8,
      loop: false
    });
  });
  ...
  it("exposes a transition duration only when a transition already exists", () => {
    ...
    const withTransition = getQuickParams(
      action({
    ...
    expect(withTransition[0].apply({ kind: "duration", ms: 700 })).toMatchObject({
      transition: { kind: "dissolve", durationMs: 700 }
    });
  });
  ...
  it("exposes the enter/exit duration off the TRANSFORM - the field `d=` writes", () => {
    ...
    expect(enter[0].apply({ kind: "duration", ms: 400 })).toMatchObject({
      transform: { preset: "center", durationMs: 400 }
    });

    const exit = getQuickParams(
      action({
        action: "character",
        operation: "exit",
        characterId: "c1",
    ...
    expect(
      getQuickParams(
        action({
          ...
          operation: "enter",
          characterId: "c1",
          transform: { mode: "animation", animationId: "anim1" }
        })
      )
    ).toEqual([]);
    expect(
      getQuickParams(action({ action: "character", operation: "enter", characterId: "c1" }))
    ).toEqual([]);
    ...
    expect(
      getQuickParams(
        action({
          ...
          characterId: "c1",
          transition: { kind: "fadeIn", durationMs: 200 }
        })
      )
    ).toEqual([]);
    expect(
...
describe("blockOverview (structured projection)", () => {
  it("puts wait/jump values only in the token, keeping the bare verb as the base", () => {
    const wait = blockOverview(
      action({ action: "wait", mode: "duration", durationMs: 500 }),
      [],
      undefined,
      undefined,
      label
  ...
  it("splices quick-param tokens in as fragments after the base, not as a second layer", () => {
    ...
    expect(fragments.slice(1).map((f) => (f.kind === "quick" ? f.param.id : f.kind))).toEqual([
      "vol",
      "loop"
    ]);
  });
