...
import {
  ...
  type StoryCommandNamedRef,
  type StoryCommandStageObjectKind,
  type StoryPuppetChannel,
  type StoryCommandValue
...
function refCandidates(
  ...
  const rest = query
    ? entries.filter(
        (entry) => !startsWithFold(entry.name, query) && containsFold(entry.name, query)
      )
    : [];
  return [...prefix, ...rest].map((entry) => ({
    value: entry.name,
    label: entry.name,
    ...(mark ? { mark: mark(entry) } : {})
  }));
}
...
function nameCandidates(
  ...
  const rest = query
    ? names.filter((name) => !startsWithFold(name, query) && containsFold(name, query))
    : [];
  return [...prefix, ...rest].map((name) => ({
    value: name,
    label: name,
    ...(mark ? { mark } : {})
  }));
...
function stageObjectCandidates(
  ...
  return refCandidates(
    ...
    query,
    () => ({ kind: "stageObject", objectKind: kind })
...
function targetCandidates(
  ...
  if (type.accepts.includes("character")) {
    candidates.push(
      ...refCandidates(context.characters, query, (entry) => ({
        kind: "character",
  ...
  if (
    freeTargetKind(type) &&
    typed &&
    !candidates.some((candidate) => candidate.value.trim().toLowerCase() === typed.toLowerCase())
  ) {
...
function contentCandidates(
  ...
  if (target.objectKind === "image") {
    return refCandidates(context.images, query, (entry) => ({
      kind: "asset",
      assetType: "image",
      assetId: entry.id
    }));
  ...
  if (target.objectKind === "video") {
    return refCandidates(context.videos, query, (entry) => ({
      kind: "asset",
      assetType: "video",
      assetId: entry.id
    }));
...
function candidatesForType(
  ...
  switch (type.kind) {
    case "asset":
      return refCandidates(assetsOfType(context, type.assetType), query, (entry) => ({
        kind: "asset",
        assetType: type.assetType,
        assetId: entry.id
      }));
    case "character": {
      const found = refCandidates(context.characters, query, (entry) => ({
        kind: "character",
        characterId: entry.id
      }));
      if (!type.allowTemp) {
    ...
    case "characterForm": {
      ...
      return (
        (context.appearanceByCharacterId[characterId] ?? [])
          ...
          .map((ref) => ({
            value: ref.name,
            label: ref.name,
            mark: { kind: "appearance" as const, characterId, refId: ref.id, axisId: ref.axisId }
          }))
      );
    }
    case "puppetName": {
      ...
      const names = puppetChannelNames(
        context,
        ownerCharacterId(resolved[type.dependsOn]) ?? undefined,
        type.channel
      );
    ...
    case "label":
      return refCandidates(
        context.labels.map((name) => ({ id: name, name })),
        query,
        () => ({ kind: "label" })
      );
    case "appTag":
    ...
    case "variable":
      return context.variables
        ...
        .map((entry) => ({
          value: entry.name,
          label: entry.name,
          detail: entry.valueType,
          mark: { kind: "variable" as const, valueType: entry.valueType }
        }));
    case "target":
      ...
      return (
        type.options
          .filter(
            (option) =>
              !query ||
              matchEnumOptionLocalized(type, query) === option ||
              containsFold(option.value, query) ||
          ...
          .map((option) => ({
            value: localizedEnumValue(type, option),
            label: localizedEnumValue(type, option),
            detail: option.aliases?.[0],
            mark: { kind: "word" as const, value: option.value }
          }))
      );
    case "keyword":
      return !query || startsWithFold(type.value, query)
        ? [{ value: type.value, label: type.value, mark: { kind: "word", value: type.value } }]
        : [];
    case "boolean":
      return ["true", "false"]
        .filter((value) => !query || value.startsWith(query.toLowerCase()))
        .map((value) => ({
          value,
          label: value,
          mark: { kind: "boolean" as const, value: value === "true" }
        }));
    case "expression": {
      ...
      const booleans =
        target?.kind === "variable" && target.valueType === "boolean"
          ? ["true", "false"]
              .filter((value) => !query || value.startsWith(query.toLowerCase()))
              .map((value) => ({
                value,
      ...
      return [
        ...
        ...context.valueBlueprints
          .filter((entry) => !query || containsFold(entry.name, query))
          .map((entry) => ({
            value: `${formatStoryExpressionName(entry.name)}()`,
            label: entry.name,
            detail: "bp",
            mark: { kind: "blueprint" as const }
          })),
        ...STORY_EXPR_FUNCTIONS.filter((fn) => query !== "" && startsWithFold(fn, query)).map(
          (fn) => ({
            value: `${fn}(`,
            label: fn,
            detail: "fn",
            mark: { kind: "function" as const }
          })
        ),
        ...
        ...STORY_VISITED_CALLS.filter((name) => !query || startsWithFold(name, query)).map(
          (name) => ({
            value: `${name}(`,
            label: name,
            detail: "fn",
...
function candidatesForParam(
  ...
  return paramTypes(param).flatMap((type) =>
    candidatesForType(type, query, context, resolved, call)
  );
}
