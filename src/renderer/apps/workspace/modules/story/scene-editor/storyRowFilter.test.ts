...
const otherDialogue = block({
  kind: "nodeAction",
  payload: {
    action: "dialogue",
    characterId: "c2",
    text: { textId: "t", role: "dialogue", value: "ho" }
  }
});
const tempDialogue = block({
  kind: "nodeAction",
  payload: {
    action: "dialogue",
    speakerName: "Guard",
    text: { textId: "t", role: "dialogue", value: "halt" }
  }
});
const unassignedDialogue = block({
  kind: "nodeAction",
  payload: { action: "dialogue", text: { textId: "t", role: "dialogue", value: "?" } }
});
const narration = block({
  kind: "nodeAction",
  payload: { action: "narration", text: { textId: "t", role: "narration", value: "..." } }
});
const choice = block({ kind: "nodeAction", payload: { action: "choice" } });
const choiceOption = block({
  kind: "nodeAction",
  payload: { action: "choiceOption", text: { textId: "t", role: "choiceText", value: "" } }
});
const note = block({ kind: "note", payload: { text: { textId: "t", role: "note", value: "" } } });
const expression = block({
  kind: "action",
  payload: { action: "character", operation: "expression", characterId: "c1" }
});
const enter = block({
  kind: "action",
...
const image = block({
  kind: "action",
  payload: { action: "image", operation: "show", imageId: "i1" } as never
});
...
function filter(
  partial: Partial<{ facets: StoryRowFacetId[]; speakers: string[] }>
): StoryRowFilter {
...
describe("dialogue-only preset", () => {
  ...
  it("recognizes itself, and nothing near it", () => {
    ...
    expect(
      isDialogueOnlyStoryRowFilter(filter({ facets: ["dialogue"], speakers: ["id:c1"] }))
    ).toBe(false);
...
describe("the two axes", () => {
  ...
  it("counts both axes for the toolbar badge", () => {
    ...
    expect(storyRowFilterSize(filter({ facets: ["sound", "camera"], speakers: ["id:c1"] }))).toBe(
      3
    );
  });
});
...
describe("revealRowInStoryRowFilter", () => {
  it("widens each constrained axis by exactly what the row needs", () => {
    ...
    expect([...after.speakers]).toEqual([
      speakerKeyForCharacter("c1"),
      speakerKeyForCharacter("c2")
    ]);
  });
  ...
  it("leaves a filter that already passed the row exactly as it was", () => {
    ...
    expect(revealRowInStoryRowFilter(narration, EMPTY_STORY_ROW_FILTER, byName)).toBe(
      EMPTY_STORY_ROW_FILTER
    );
  });
  ...
  it("makes the row pass — which is the whole contract", () => {
    for (const block of [
      ...
      expect(
        storyRowPassesFilter(block, revealRowInStoryRowFilter(block, before, byName), byName)
      ).toBe(true);
    }
  });
...
describe("facet table", () => {
  it("is partitioned into the two sections with nothing missing or counted twice", () => {
    expect([...STORY_ROW_NARRATIVE_FACETS, ...STORY_ROW_STAGING_FACETS]).toEqual([
      ...STORY_ROW_FACETS
    ]);
...
describe("tallyStoryRows", () => {
  ...
  it("still lists a hidden speaker with no rows on this page, at zero", () => {
    ...
    expect(speakers).toEqual([
      { key: speakerKeyForCharacter("c9"), characterId: "c9", name: "", count: 0 }
    ]);
  });
});
...
describe("normalize", () => {
  ...
  it("keeps any well-formed speaker key — the cast is the project's, not a closed vocabulary", () => {
    expect(normalizeStoryRowSpeakers(["id:c1", "name:Guard", "id:c1", "c1", 7])).toEqual([
      "id:c1",
      "name:Guard"
    ]);
