...
describe("findRangesInText", () => {
  ...
  it("ignores case unless asked", () => {
    ...
    expect(findRangesInText("Alice and alice", "alice", { caseSensitive: true })).toEqual([
      { start: 10, end: 15 }
    ]);
  });
  ...
  it("carries the shared matcher's options through", () => {
    expect(
      findRangesInText("cat catalog", "cat", { caseSensitive: true, wholeWord: true })
...
describe("replaceInSegment", () => {
  ...
  it("keeps the marks the match sat in", () => {
    ...
    expect(next.rich).toEqual([
      { text: "Hello " },
      { text: "Zoe", marks: { bold: true } },
      { text: "!" }
    ]);
  });
...
describe("replaceRangesInSegment", () => {
  it("gives each hit its own replacement, which is what `$1` needs", () => {
    const matcher = compileMatcher("(\\w)(\\d)", {
      caseSensitive: true,
      wholeWord: false,
      regex: true
    });
