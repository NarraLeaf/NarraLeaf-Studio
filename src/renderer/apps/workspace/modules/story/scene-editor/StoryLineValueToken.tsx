...
const TOKEN_CLASS =
  "cursor-pointer rounded-md px-0.5 underline decoration-dotted decoration-fg-subtle/60 underline-offset-2 transition-colors hover:bg-fill";
...
function ValuePopover(props: {
  ...
  return createPortal(
    ...
    >
      {control.kind === "number" ? (
        <NumberControl control={control} value={edit.value} onCommit={apply} onPick={pick} />
      ) : null}
      {control.kind === "enum" ? (
