...
function CharacterMark({
  ...
  const { url, frame, showingSprite } = useCharacterAvatar(character, {
    ...
    tags:
      appearance?.axisId && appearance.refId
        ? { [appearance.axisId]: appearance.refId }
        : undefined,
    resolveVariant: true,
...
export function StoryCandidateSpeakerMark({ character }: { character: Character | null }) {
  return character ? (
    <CharacterMark
      mark={{ kind: "character", characterId: character.profile.getId() }}
      character={character}
    />
  ) : (
...
export function StoryCandidateMarkView({
  mark,
  characters
}: {
  ...
  if ((mark.kind === "character" || mark.kind === "appearance") && mark.characterId) {
    ...
    return (
      <CharacterMark
        mark={mark}
        character={characters.find((entry) => entry.profile.getId() === characterId)}
      />
    );
  }
  return <MarkGlyph mark={mark} />;
