...
export function exportBlockPlainText(
  block: StoryBlock,
...
export function parseDialogueLine(
  ...
  const character = characters.find((item) => {
    ...
    return (
      profile.getName().toLowerCase() === speaker ||
      profile.getNicknames().some((name) => name.toLowerCase() === speaker)
    );
  });
...
export function isStoryClipboardPayload(
  payload: StoryClipboardPayload
): payload is StoryClipboardPayload {
