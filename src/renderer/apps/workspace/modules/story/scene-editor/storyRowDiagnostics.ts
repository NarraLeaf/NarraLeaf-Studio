...
function assetExists(context: StoryCommandContext, assetId: string): boolean {
  return (
    context.images.some((asset) => asset.id === assetId) ||
 ...
 */
function requestedPuppetChannel(
  block: StoryBlock
): { characterId: string; channel: StoryPuppetChannel; name: string } | null {
...
function requestedPuppetParams(block: StoryBlock): { characterId: string; ids: string[] } | null {
  if (
    ...
    block.payload.operation !== "setParams"
  ) {
    return null;
  }
  const characterId = block.payload.characterId;
  const ids = Object.keys(block.payload.params ?? {})
    .map((id) => id.trim())
    .filter((id) => id !== "");
...
export function diagnoseRow(input: StoryRowDiagnosticInput): StoryRowDiagnostic | null {
  ...
  if (requestedParams) {
    ...
    if (
      known.length > 0 &&
      requestedParams.ids.some((id) => !known.some((spec) => spec.id === id))
    ) {
