...
const PAUSE_CHIP_CLASS =
  "story-rt-pause mx-0.5 inline-flex select-none items-center rounded-md bg-primary/20 px-1 py-0.5 align-middle text-2xs font-medium text-primary";
const PAUSE_CHIP_INTERACTIVE_CLASS = "cursor-pointer hover:bg-primary/30";
const PAUSE_ICON_SVG =
  '<svg width="9" height="9" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><rect x="14" y="4" width="4" height="16" rx="1"></rect><rect x="6" y="4" width="4" height="16" rx="1"></rect></svg>';

/** Interpolation chip class (variable / blueprint value). */
const INTERP_CHIP_CLASS =
  "story-rt-interp mx-0.5 inline-flex select-none items-center rounded-md bg-success/20 px-1 py-0.5 align-middle text-2xs font-medium text-success";
const INTERP_CHIP_INTERACTIVE_CLASS = "cursor-pointer hover:bg-success/30";
const INTERP_ICON_SVG =
  '<svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M8 3H7a2 2 0 0 0-2 2v4a2 2 0 0 1-2 2 2 2 0 0 1 2 2v4a2 2 0 0 0 2 2h1"></path><path d="M16 3h1a2 2 0 0 1 2 2v4a2 2 0 0 0 2 2 2 2 0 0 0-2 2v4a2 2 0 0 1-2 2h-1"></path></svg>';
...
const EVENT_CHIP_CLASS =
  "story-rt-event mx-0.5 inline-flex select-none items-center rounded-md bg-warning/20 px-1 py-0.5 align-middle text-2xs font-medium text-warning";
const EVENT_CHIP_INTERACTIVE_CLASS = "cursor-pointer hover:bg-warning/30";
/** lucide "smile" — the expression-switch token. */
const EVENT_FACE_ICON_SVG =
  '<svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><circle cx="12" cy="12" r="10"></circle><path d="M8 14s1.5 2 4 2 4-2 4-2"></path><line x1="9" y1="9" x2="9.01" y2="9"></line><line x1="15" y1="9" x2="15.01" y2="9"></line></svg>';
/** lucide "music" — the sound-only token. */
const EVENT_SOUND_ICON_SVG =
  '<svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M9 18V5l12-2v13"></path><circle cx="6" cy="18" r="3"></circle><circle cx="18" cy="16" r="3"></circle></svg>';
...
export function isInterpolationRun(
  run: StoryRichRun
): run is { interpolation: StoryInterpolationRef; marks?: StoryTextMarks } {
...
function isChipElement(el: HTMLElement): boolean {
  return (
    el.dataset.pause !== undefined ||
    el.dataset.interp !== undefined ||
    el.dataset.event !== undefined
  );
}
  ...
  for (const run of runs) {
    if (isTextRun(run)) {
      ...
      if (prev && isTextRun(prev) && marksKey(prev.marks) === marksKey(marks)) {
        out[out.length - 1] = marks
          ? { text: prev.text + run.text, marks }
      ...
      const marks = cleanMarks(run.marks);
      out.push(
        marks ? { interpolation: run.interpolation, marks } : { interpolation: run.interpolation }
      );
...
function mergeMarks(
  a: StoryTextMarks | undefined,
  b: StoryTextMarks | undefined
): StoryTextMarks | undefined {
  if (!a && !b) {
    return undefined;
  }
...
function createPauseChip(pause: number | true, options: RichRenderOptions): HTMLSpanElement {
  ...
  span.className = options.interactive
    ? `${PAUSE_CHIP_CLASS} ${PAUSE_CHIP_INTERACTIVE_CLASS}`
    : PAUSE_CHIP_CLASS;
  ...
  span.dataset.tip =
    pause === true
      ? options.titles.pauseClick
      : options.titles.pauseSeconds(storyMsToSeconds(pause));
  const label =
    pause === true ? "" : `<span class="ml-0.5">${formatStorySecondsLabel(pause)}</span>`;
...
function createInterpolationChip(
  interp: StoryInterpolationRef,
  label: string,
  marks: StoryTextMarks | undefined,
  ...
  span.contentEditable = "false";
  span.className = options.interactive
    ? `${INTERP_CHIP_CLASS} ${INTERP_CHIP_INTERACTIVE_CLASS}`
    : INTERP_CHIP_CLASS;
  if (options.interactive) {
    span.setAttribute("role", "button");
  }
...
function createEventChip(event: StoryInlineEvent, options: RichRenderOptions): HTMLSpanElement {
  ...
  span.contentEditable = "false";
  span.className = options.interactive
    ? `${EVENT_CHIP_CLASS} ${EVENT_CHIP_INTERACTIVE_CLASS}`
    : EVENT_CHIP_CLASS;
  if (options.interactive) {
    span.setAttribute("role", "button");
  }
  ...
  if (event.expression) {
    ...
    span.dataset.tip = form
      ? `${options.titles.expressionEvent}: ${form}`
      : options.titles.expressionEvent;
 ...
 */
export function renderRunsToElement(
  root: HTMLElement,
  runs: StoryRichRun[],
  options: RichRenderOptions
): void {
  ...
  for (const run of runs) {
    if (isTextRun(run)) {
      const marks = cleanMarks(run.marks);
      root.appendChild(
        marks ? createMarkSpan(run.text, marks) : globalThis.document.createTextNode(run.text)
      );
    } else if (isInterpolationRun(run)) {
      root.appendChild(
        createInterpolationChip(
          run.interpolation,
          options.resolveLabel?.(run.interpolation) ?? options.titles.valueFallback,
          run.marks,
          options
        )
      );
    ...
    node.childNodes.forEach((child) => {
      ...
      if (el.dataset.interp !== undefined) {
        ...
        if (interp) {
          ...
          runs.push(
            chipMarks ? { interpolation: interp, marks: chipMarks } : { interpolation: interp }
          );
        }
        return;
      }
...
export function spliceRuns(
  ...
  return normalizeRuns([...sliceRuns(runs, 0, start), ...insert, ...sliceRuns(runs, end, total)]);
}
...
export function rangeMarkColor(
  runs: StoryRichRun[],
  start: number,
  end: number
): string | undefined {
 ...
 */
export function rubyRunAt(
  runs: StoryRichRun[],
  unit: number
): { start: number; end: number; ruby: string } | null {
  let pos = 0;
...
export function applyMarkToRange(
  ...
  for (const run of runs) {
    ...
    if (localStart > 0) {
      out.push(
        run.marks
          ? { text: text.slice(0, localStart), marks: run.marks }
    ...
    if (localEnd < len) {
      out.push(
        run.marks
          ? { text: text.slice(localEnd), marks: run.marks }
 ...
 */
export function markSelectedChips(
  root: HTMLElement,
  range: { start: number; end: number } | null
): void {
  const active =
    Boolean(range) &&
    (range as { start: number; end: number }).end > (range as { start: number; end: number }).start;
  ...
  root.querySelectorAll<HTMLElement>("[data-interp],[data-pause],[data-event]").forEach((chip) => {
    ...
    if (
      active &&
      start >= (range as { start: number; end: number }).start &&
      start + 1 <= (range as { start: number; end: number }).end
    ) {
