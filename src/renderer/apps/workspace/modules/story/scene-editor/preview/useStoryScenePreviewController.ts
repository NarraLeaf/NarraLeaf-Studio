...
import {
  createWorkspaceBlobUrlResolver,
  type WorkspaceBlobUrlResolver
...
export function useStoryScenePreviewController(input: {
  ...
  const logStoryConsole = useCallback(
    (level: "warning" | "error", message: string, source: string) => {
      consoleService?.append(STORY_CONSOLE_CHANNEL_ID, { level, message, source });
    },
    [consoleService]
  );

  const pushIssue = useCallback(
    (issue: StoryPreviewIssue) => {
  ...
  const audioClips = useMemo(
    () =>
      context && open
        ? collectAudioClipRegions(context.services.get<AssetsService>(Services.Assets))
        : undefined,
    [context, open]
  );
  ...
  const variableTables = useMemo(() => {
    ...
    const registry = context.services
  ...
  const disposeRunObject = useCallback(
    (run: PreviewRun | null) => {
      if (!run) {
        return;
      }
  ...
  const handleStagePosed = useCallback(
    (runId: number) => {
      if (runId !== runIdRef.current) {
        return;
      }
  ...
  const handleBeforeTarget = useCallback(
    (runId: number) => {
      if (runId !== runIdRef.current) {
        return;
      }
      const run =
        displayRunRef.current?.runId === runId
          ? displayRunRef.current
          : pendingRunRef.current?.runId === runId
  ...
  const handleLiveGameReady = useCallback(
    (sessionId: string, liveGame: LiveGame) => {
...
async function loadReferencedAnimations(
