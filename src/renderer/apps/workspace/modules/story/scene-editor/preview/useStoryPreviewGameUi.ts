...
export function useStoryPreviewGameUi(input: {
  ...
  const characters = useMemo(() => {
    ...
    return mapCharacterStoreEntriesToSummaries(
      characterService.listCharacter().map((character) => character.toJSON())
    );
  }, [context, enabled]);
  ...
  const bundle = useMemo((): DevModeBundle | null => {
    ...
    const localBlueprintService = context.services.get<LocalBlueprintService>(
  ...
  const createPreviewGame = useCallback(
    (gameInput: {
      sessionId: string;
      requireLiveGame: (operation: string) => LiveGame;
      getLiveGame: () => LiveGame | null;
      ...
      if (context) {
        const defaults = context.services
