...
function clampRect(rect: StoryScenePreviewFloatRect, bounds: Bounds): StoryScenePreviewFloatRect {
  const width = clamp(
    rect.width,
    STORY_PREVIEW_FLOAT_MIN_WIDTH,
    Math.max(STORY_PREVIEW_FLOAT_MIN_WIDTH, bounds.width)
  );
  const height = clamp(
    rect.height,
    STORY_PREVIEW_FLOAT_MIN_HEIGHT,
    Math.max(STORY_PREVIEW_FLOAT_MIN_HEIGHT, bounds.height)
  );
  return {
    width,
    height,
...
export function StoryScenePreviewFloat(props: {
  ...
  const beginInteraction = useCallback(
    (event: ReactPointerEvent, interaction: Interaction) => {
      if (event.button !== 0) {
        return;
      }
  ...
  const startMove = useCallback(
    (event: ReactPointerEvent) => beginInteraction(event, { kind: "move" }),
    [beginInteraction]
  );
  const startResize = useCallback(
    (corner: Corner) => (event: ReactPointerEvent) =>
      beginInteraction(event, { kind: "resize", corner }),
    [beginInteraction]
  );
  ...
  return (
    ...
    <div className="pointer-events-none absolute inset-0 z-30 overflow-hidden">
      ...
      >
        ...
        <div
          className={`${edgeClass} inset-x-3 top-0 h-1.5`}
          style={{ touchAction: "none" }}
          onPointerDown={startMove}
        />
        <div
          className={`${edgeClass} inset-x-3 bottom-0 h-1.5`}
          style={{ touchAction: "none" }}
          onPointerDown={startMove}
        />
        <div
          className={`${edgeClass} inset-y-3 left-0 w-1.5`}
          style={{ touchAction: "none" }}
          onPointerDown={startMove}
        />
        <div
          className={`${edgeClass} inset-y-3 right-0 w-1.5`}
          style={{ touchAction: "none" }}
          onPointerDown={startMove}
        />
        ...
        />
        <div
          className={`${cornerClass} left-0 bottom-0 cursor-nesw-resize`}
          style={{ touchAction: "none" }}
          onPointerDown={startResize("sw")}
        />
        <div
          className={`${cornerClass} right-0 bottom-0 cursor-nwse-resize`}
          style={{ touchAction: "none" }}
