...
export function ConditionEditor(props: {
  ...
  const variableValue: VariableCondition =
    props.value?.kind === "variable" ? props.value : DEFAULT_VARIABLE_CONDITION;
    ...
    () =>
      createStoryExpressionScope(
        allVariables.map((option) => ({
          name: option.name,
          ref: makeVariableRef(option.scope, option.id)
        })),
        {
          scenes: Object.values(props.document.scenes ?? {}).map((scene) => ({
            id: scene.id,
            name: scene.name
          })),
  ...
  const setExpressionSource = (source: string) => {
    ...
    props.onChange({
      kind: "expression",
      expression: parseStoryExpression(source, expressionScope).expression
    });
  };
  ...
  const openEditor = (options?: BlueprintOpenOptions) => {
    ...
    openBlueprint(
      { blueprintId: id, ownerKind: "storyAction", title: t("story.condition.title") },
      options
    );
  };
  ...
  const variableSelectOptions: SelectOption[] = allVariables.length
    ? allVariables.map((option) => ({
        value: option.key,
        label: option.name,
        secondaryLabel: option.valueType
      }))
    : [{ value: "", label: t("story.interpolation.noVariables") }];

  const showValueField =
    variableValue.operator === "equals" || variableValue.operator === "notEquals";
    ...
    <div className="flex flex-col gap-2" data-help-topic="storyExpressions">
      ...
      {kind === "expression" ? (
        ...
        <StoryActionBlueprintPreviewCard
          ...
          variant="detailed"
          ariaLabel={
            blueprintId ? t("story.condition.openGraphAria") : t("story.condition.createGraphAria")
          }
        />
      )}
    </div>
...
function ConditionExpressionField(props: {
  ...
  const parsed = useMemo(
    () => parseStoryExpression(props.source, props.scope),
      ...
      {problem ? (
        ...
        <p className="truncate text-2xs text-fg-subtle">
          {props.variableNames.length
            ? t("story.condition.expressionVariables", {
                names: props.variableNames.slice(0, 6).join(", ")
              })
  ...
  if (props.valueType === "boolean") {
    return (
      <label className="flex items-center gap-2 text-sm text-fg-muted">
        ...
        />
        <span>
          {props.value === true ? t("story.condition.valueTrue") : t("story.condition.valueFalse")}
        </span>
      </label>
    );
  }
