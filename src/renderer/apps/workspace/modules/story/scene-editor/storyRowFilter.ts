...
export const STORY_ROW_FACETS: readonly StoryRowFacetId[] = [...STORY_ROW_NARRATIVE_FACETS, ...STORY_ROW_STAGING_FACETS];
 ...
 */
const STORY_ROW_FACET_STYLE: Record<StoryRowFacetId, { icon: LucideIcon; category: StoryCommandCategoryId | null }> = {
 ...
 */
export function storyRowSpeakerKeyOf(block: StoryBlock, resolveCharacterIdByName: StoryCharacterNameLookup): StoryRowSpeakerKey | null {
    ...
    if (payload.action === "displayable" && payload.target.kind === "character") {
        ...
        return characterId ? speakerKeyForCharacter(characterId) : speakerKeyForName(payload.target.name);
    }
    return null;
}
...
export function revealRowInStoryRowFilter(
    ...
    const facets = filter.facets.size === 0 ? filter.facets : new Set(filter.facets).add(storyRowFacet(block));
...
export function normalizeStoryRowFacets(value: unknown): StoryRowFacetId[] {
    ...
    return [...new Set(value.filter((entry): entry is StoryRowFacetId => typeof entry === "string" && known.has(entry)))];
}
...
export function normalizeStoryRowSpeakers(value: unknown): StoryRowSpeakerKey[] {
    ...
    return [...new Set(value.filter((entry): entry is string => typeof entry === "string" && (entry.startsWith("id:") || entry.startsWith("name:"))))];
}
...
export function tallyStoryRows(
    ...
    const facets = Object.fromEntries(STORY_ROW_FACETS.map(facet => [facet, 0])) as Record<StoryRowFacetId, number>;
