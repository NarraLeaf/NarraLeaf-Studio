...
export function useStorySceneClipboardHandlers(params: {
   ...
   */
  const insertPastedBlocks = useCallback(
    (blocks: StoryBlock[], target: StoryBlockTarget) => {
  ...
  const pasteBlocks = useCallback(
    (roots: SerializedStoryBlock[], target: StoryBlockTarget) => {
   ...
   */
  const pasteSingleLine = useCallback(
    (text: string, target: StoryBlockTarget) => {
  ...
  const pastePlain = useCallback(
    async (text: string, anchor: StoryPasteAnchor) => {
  ...
  const resolveAnchor = useCallback((): StoryPasteAnchor | null => {
    ...
    if (editorMode.kind === "insert") {
      return {
        target:
          editorMode.slot.target ?? getInsertionTargetAfter(scene, editorMode.slot.afterBlockId),
        plain: { kind: "none" }
      };
    }
    const afterBlockId =
  ...
  const routePaste = useCallback(
    (data: DataTransfer, plainRequested: boolean): boolean => {
  ...
  const copySelectionToClipboard = useCallback(
    (event: ClipboardEvent<HTMLDivElement>) => {
      ...
      event.clipboardData.setData(
        "text/plain",
        roots.map((id) => exportBlockPlainText(scene.blocks[id], characters, scenes)).join("\n")
      );
    },
    [params]
  );
   ...
   */
  const takePlainRequest = useCallback(
  ...
  const handlePaste = useCallback(
    (event: ClipboardEvent<HTMLDivElement>) => {
      ...
      const intoInsertSlot =
        Boolean(params.insertInputRef.current) && event.target === params.insertInputRef.current;
   ...
   */
  const handleRowTextPaste = useCallback(
