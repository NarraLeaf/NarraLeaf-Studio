...
export function InterpolationPopover(props: {
  ...
  const allVariables = useMemo(
    () => [
      ...
      ...props.options.persistent.map((v) => ({
        key: `persistent:${v.id}`,
  ...
  const currentVariableKey =
    props.value.kind === "variable"
  ...
  const setKind = (nextKind: string) => {
    ...
    if (nextKind === "blueprint") {
      ...
      props.onChange(
        props.value.kind === "blueprint" ? props.value : { kind: "blueprint", blueprintId: "" }
      );
    } else {
    ...
    openBlueprint(
      {
        ...
        title: t("story.interpolation.storyValueTitle")
      },
      options
    );
  };
