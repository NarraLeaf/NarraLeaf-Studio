...
export {
  ...
  EMPTY_STORY_COMMAND_STAGE_OBJECTS
} from "./storyCommandValues";
...
function findByName(
  ...
  raw: string
): StoryCommandNamedRef | "ambiguous" | null {
...
function resolveTarget(
  ...
  if (freeKind && value.trim() !== "") {
    return {
      value: {
        kind: "target",
        target: { type: "stageObject", objectKind: freeKind, name: value.trim(), known: false }
      }
    };
  }
  ...
  switch (type.kind) {
    ...
    case "characterForm": {
      ...
      const match = refs.find(
        (ref) => ref.name.trim().toLowerCase() === value.trim().toLowerCase()
      );
      if (match) {
        // The row stores the id, so renaming a pose or a tag later leaves it resolving.
        return {
          value: { kind: "characterForm", refId: match.id, label: match.name, axisId: match.axisId }
        };
      }
      const characterName =
        context.characters.find((entry) => entry.id === characterId)?.name ?? "";
    ...
    case "scene": {
      ...
      return found
        ? { value: { kind: "scene", sceneId: found.id } }
        : { issue: { code: "unknownScene", span, value } };
    }
    case "audioTrack": {
    ...
    case "label": {
      ...
      return found
        ? { value: { kind: "label", name: found } }
        : { issue: { code: "unknownLabel", span, value } };
    }
    case "appTag": {
    ...
    case "variable": {
      const needle = value.trim().toLowerCase();
      const matches = context.variables.filter(
      ...
      return {
        value: {
          ...
          name: matches[0].name,
          defaultValue: matches[0].defaultValue
        }
      };
    }
    case "target":
...
function resolveExpression(
  ...
  if (type.expects === "boolean") {
    const inferred = inferStoryExpressionType(expression.ast, (ref) =>
      variableTypeOf(context, ref)
    );
...
function matchCompoundAssignment(
  value: string
): { op: "+" | "-" | "*" | "/"; rest: string } | null {
  const match = /^\s*([+\-*/])=\s*(.*)$/.exec(value);
  return match && match[2].trim() !== ""
    ? { op: match[1] as "+" | "-" | "*" | "/", rest: match[2] }
    : null;
}
...
): StoryVariableValueType | undefined {
