...
export type StoryCommandIssue =
  ...
  | {
      ...
      span: StoryCommandSpan;
      value: string;
      expected: readonly StoryCommandParamType[];
    }
...
export type StoryCommandLine =
  ...
  | {
      kind: "character";
      query: string;
      querySpan: StoryCommandSpan;
      text: string;
      textSpan: StoryCommandSpan;
    }
  | {
      kind: "command";
...
export function tokenizeCommandLine(
  source: string,
  from = 0
): { tokens: StoryCommandToken[]; unterminatedQuote: boolean } {
...
function parseCommand(source: string): Extract<StoryCommandLine, { kind: "command" }> {
  ...
  if (!def) {
    ...
    return {
      kind: "command",
      token: nameToken.text,
      tokenSpan: nameToken.span,
      def: null,
      args: [],
      issues
    };
  ...
  for (let index = 1; index < tokens.length; index += 1) {
    ...
    const namesRealParam =
      equals > 0 && findParamLocalized(def, token.raw.slice(0, equals)) !== null;
    ...
    if (
      flagParam &&
      isFlagParam(flagParam) &&
      !seen.has(flagParam.name) &&
      (positionalIndex > 0 || positionals.length === 0)
    ) {
    ...
    if (param.skippable) {
      ...
      if (
        next &&
        !seen.has(next.name) &&
        strictlyMatches(next, token.text) &&
        !strictlyMatches(param, token.text)
      ) {
        positionalIndex += 1;
        param = next;
      }
