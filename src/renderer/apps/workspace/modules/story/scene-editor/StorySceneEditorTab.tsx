...
const SCENE_TEXT_FIELD_CLASS =
  "w-full rounded-md border border-edge bg-surface-raised px-3 py-2 text-sm text-fg outline-none transition-colors placeholder:text-fg-subtle focus:border-primary/50";
...
function StorySceneOverviewBlock(props: {
  ...
  const [collapsed, setCollapsed] = useState<boolean>(() => {
    const stored = panelStateService
      ? getStoryEditorViewState(panelStateService, scene.id)?.overviewCollapsed
      : undefined;
        ...
        <div className="border-t border-edge p-3">
          ...
          >
            <button
              ...
              data-tip={
                backgroundAssetId
                  ? t("story.sceneEditor.changeBackgroundTitle")
                  : t("story.sceneEditor.selectBackgroundTitle")
              }
            >
              {url ? (
                <img
              ...
              <div className="absolute inset-x-0 bottom-0 flex min-h-9 items-center justify-between gap-2 bg-black/55 px-3 py-2 text-xs text-white backdrop-blur-sm">
                ...
                <span className="shrink-0 text-primary opacity-0 transition-opacity group-hover:opacity-100">
                  {backgroundAssetId
                    ? t("story.sceneEditor.change")
                    : t("story.sceneEditor.select")}
                </span>
              </div>
            </button>
            ...
            <div className="grid min-w-0 gap-3">
              <div>
                <label className={SCENE_FIELD_LABEL_CLASS}>
                  {t("story.sceneEditor.sceneName")}
                </label>
                <input
              ...
              <div>
                <label className={SCENE_FIELD_LABEL_CLASS}>
                  {t("story.sceneEditor.defaultBackground")}
                </label>
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    <span
                      className={["truncate", backgroundAsset ? "" : "italic text-fg-subtle"].join(
                        " "
                      )}
                    >
                      {backgroundLabel}
                  ...
                  <button
                    ...
                    data-tip={t("story.sceneEditor.clearBackground")}
...
export function StorySceneEditorTab({
  tabId,
  payload,
  active
}: EditorComponentProps<StorySceneEditorTabPayload | undefined>) {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    ...
    return () => {
      const current =
        uiService.panels.getPayload<StoryVariablesPanelPayload>(STORY_VARIABLES_PANEL_ID);
  ...
  const sortableRowIds = useMemo(
    () => editor.visibleRows.map((row) => row.block.id),
    [editor.visibleRows]
  );
   ...
   * moves nothing.
   */
  const insertSlotHostId: StoryBlockId | null =
    editor.editorMode.kind === "insert"
  ...
  const scrollRowIntoView = useCallback(
    (blockId: StoryBlockId, align: "center" | "auto" = "auto") => {
      const index = editor.visibleRows.findIndex((row) => row.block.id === blockId);
      if (index < 0) {
        return false;
      }
  ...
  useEffect(() => {
    ...
    const blockId =
      editor.activeBlockId && scene.blocks[editor.activeBlockId] ? editor.activeBlockId : null;
    ...
    const sceneList = Object.values(storyDocument.scenes)
    ...
    if (
      ...
      publishStoryInspectorState(tabId, {
        ...
        onCreateLayer: (nextBeforeBlockId) =>
          editorRef.current.createLayerBeforeBlock(nextBeforeBlockId),
    ...
    const isOurs =
      current.type === STORY_BLOCK_SELECTION_TYPE &&
  ...
  const panelStateService = useMemo(
    () =>
      editor.context && editor.isInitialized
        ? editor.context.services.get<PanelStateService>(Services.PanelState)
        : null,
    [editor.context, editor.isInitialized]
  );
  ...
  scrollContextRef.current = () => {
    ...
    setScrollContext(
      describeScrollContext(
        editorRef.current.visibleRows,
    ...
    scrollSaveRafRef.current = window.requestAnimationFrame(() => {
      ...
      if (el && sceneId && panelStateService) {
        patchStoryEditorViewState(panelStateService, sceneId, {
          scroll: captureStoryEditorScrollAnchor(el)
        });
      }
    });
  ...
  useLayoutEffect(() => {
    ...
    if (
      ...
      Math.abs(el.scrollTop - liveScrollTopRef.current) > 1
    ) {
  ...
  useEffect(() => {
    ...
    return subscribeStoryRowHighlight((highlight) => {
      if (
        !active ||
        highlight.storyId !== payload?.storyId ||
        highlight.sceneId !== payload?.sceneId
      ) {
        return;
      }
  ...
  const commitPreviewFloat = useCallback(
    (float: StoryScenePreviewFloatRect) => {
      setPreviewPane((current) => {
        if (!current) {
          return current;
        }
  ...
  const handlePreviewResize = useCallback(
    (delta: number): number => {
      ...
      const maxWidth = Math.max(
        STORY_PREVIEW_PANE_MIN_WIDTH,
        containerWidth * STORY_PREVIEW_PANE_MAX_FRACTION
      );
      const nextWidth = Math.round(
        Math.min(maxWidth, Math.max(STORY_PREVIEW_PANE_MIN_WIDTH, width - delta))
      );
      if (nextWidth !== width) {
        previewWidthRef.current = nextWidth;
        setPreviewPane((current) => ({
          ...(current ?? DEFAULT_STORY_SCENE_PREVIEW_PANE_STATE),
          width: nextWidth
        }));
        if (panelStateService) {
  ...
  const playFromRow = useCallback(
    (blockId: StoryBlockId) => {
          ...
          [
            {
              ...
              primary: true,
              onClick: () => {
                const created = storyService.createSceneSnapshot(
                  storyId,
                  sceneId,
                  `${t("storySnapshot.defaultName")} 1`
                );
      ...
      const saved = panelStateService
        ? getSelectedSnapshotId(panelStateService, storyId, sceneId)
        : undefined;
      const snapshotId =
        saved && snapshots.some((snapshot) => snapshot.id === saved) ? saved : snapshots[0].id;
  ...
  const openRowContextMenu = useCallback(
    (event: ReactMouseEvent, blockId: StoryBlockId) => {
  ...
  const replaceCurrentMatch = useCallback(() => {
    ...
    const next = replaceInSegment(
      slot.segment,
      match,
      findMatcher.expand(plain, match, findReplacement)
    );
    editor.updateBlockPayloads([
      { blockId: match.blockId, payload: slot.withSegment(next).payload }
    ]);
  ...
  const isInsertingAfterLastRow =
    editor.editorMode.kind === "insert" &&
    !editor.editorMode.slot.replaceBlockId &&
    editor.editorMode.slot.afterBlockId === lastVisibleRowId;
  ...
  const dragGroupSizeFor = (blockId: StoryBlockId) =>
    editor.selectedBlockIds.has(blockId) ? selectionDragSize : 1;
  ...
  const menuAllDisabled =
    menuRoots.length > 0 && menuRoots.every((id) => scene.blocks[id]?.disabled);
  const rowMenuItems: ContextMenuDef = menuTarget
    ? [
        {
          id: "insert-above",
          label: t("story.rowMenu.insertAbove"),
          ...freeze.menuRow(),
          onClick: () => editor.startInsertBefore(menuTarget)
        },
        {
          id: "insert-below",
          label: t("story.rowMenu.insertBelow"),
          ...freeze.menuRow(),
          onClick: () => editor.startInsertAfter(menuTarget, true)
        },
        { id: "sep-insert", separator: true },
        {
          id: "duplicate",
          label: t("story.rowMenu.duplicate"),
          ...freeze.menuRow(),
          onClick: () => editor.duplicateSelection()
        },
        {
          id: "disable",
          label: menuAllDisabled ? t("story.rowMenu.enable") : t("story.rowMenu.disable"),
          ...freeze.menuRow(),
          onClick: () => editor.toggleDisableSelection()
        },
        { id: "sep-op", separator: true },
        {
          id: "play",
          label: t("story.rowMenu.playFromHere"),
          onClick: () => playFromRow(menuTarget)
        },
        {
          id: "inspector",
          label: t("story.rowMenu.openInspector"),
          onClick: () => editor.activateBlockForInspectorOrOp(menuTarget)
        },
        { id: "sep-del", separator: true },
        {
          id: "delete",
          label: t("story.rowMenu.delete"),
          ...freeze.menuRow(),
          onClick: () =>
            void editor.deleteRows(
  ...
  const rowMenuItemsWithDeveloperRows = menuTarget
    ? appendDeveloperIdSection(
        rowMenuItems,
        [
          { kind: "storyRow", value: menuTarget },
          { kind: "scene", value: scene.id }
        ],
        {
            ...
            <div className="flex min-h-[44px] items-center gap-3 border-b border-edge px-3">
              ...
              <div className="ml-auto flex shrink-0 items-center gap-1">
                ...
                >
                  ...
                  {rowFilterSize > 0 ? (
                    <span className="text-2xs tabular-nums">{rowFilterSize}</span>
                  ) : null}
                </button>
                ...
                <button
                  ...
                  data-tip={
                    previewOpen ? t("story.preview.closePreview") : t("story.preview.openPreview")
                  }
                  aria-pressed={previewOpen}
                  className={[
                    "flex min-h-7 cursor-default items-center gap-1.5 rounded-md px-2 py-1 text-xs transition-colors",
                    previewOpen
                      ? "bg-primary/15 text-primary"
                      : "text-fg-muted hover:bg-fill hover:text-fg"
                  ].join(" ")}
            ...
            <div ref={editorBodyRef} className="relative flex min-h-0 flex-1 flex-row">
              <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
                ...
                >
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                left: 0,
                                width: "100%",
                                transform: `translateY(${virtualRow.start - rowListMargin}px)`
                              }}
                            >
                              ...
                              {editor.editorMode.kind === "insert" &&
                              !editor.editorMode.slot.replaceBlockId &&
                              editor.editorMode.slot.afterBlockId === null &&
                              editor.editorMode.slot.target?.beforeBlockId === row.block.id ? (
                                <InsertRow
                                  ...
                                  onCommitNarration={(focusNext) =>
                                    editor.commitNarrationFromInsert(focusNext)
                                  }
                              ...
                              {editor.editorMode.kind === "insert" &&
                              editor.editorMode.slot.replaceBlockId === row.block.id ? (
                                <InsertRow
                                  mode={editor.editorMode}
                                  depth={row.depth}
                                  characters={editor.characters}
                                ...
                                <StoryBlockRow
                                  ...
                                  commandContext={editor.commandContext}
                                  selected={
                                    !insertActive && editor.selectedBlockIds.has(row.block.id)
                                  }
                                  active={!insertActive && editor.activeBlockId === row.block.id}
                                  collapsed={editor.collapsedBlockIds.has(row.block.id)}
                                  editing={
                                    editor.editorMode.kind === "text" &&
                                    editor.editorMode.blockId === row.block.id
                                  }
                                  editInitialCaret={
                                    editor.editorMode.kind === "text" &&
                                    editor.editorMode.blockId === row.block.id
                                      ? (editor.editorMode.caret ?? "end")
                                      : undefined
                                  }
                                  ...
                                  coDragging={
                                    draggingGroup.size > 1 && draggingGroup.has(row.block.id)
                                  }
                                />
                              )}
                              {editor.shouldRenderActiveInsertSlot &&
                              editor.editorMode.kind === "insert" &&
                              !editor.editorMode.slot.replaceBlockId &&
                              editor.editorMode.slot.afterBlockId === row.block.id ? (
                                <InsertRow
                                  mode={editor.editorMode}
                                  // Inserting *inside* this row (its `+ Add action`) nests one level
                                  // deeper; a sibling-after slot keeps the row's own depth.
                                  depth={
                                    editor.editorMode.slot.target?.parentId === row.block.id
                                      ? row.depth + 1
                                      : row.depth
                                  }
                                  characters={editor.characters}
                  ...
                  {editor.visibleRows.length === 0 &&
                  editor.editorMode.kind !== "insert" &&
                  editor.unfilteredRowCount > 0 ? (
                  ...
                  editor.unfilteredRowCount === 0 ? (
                    <div className="mx-auto mt-6 flex max-w-md flex-col gap-2 px-6 text-xs text-fg-subtle">
                      <p>
                        {t("story.sceneEditor.emptyHint", {
                          trigger: editor.slashAtAlias ? "@" : "/"
                        })}
                      </p>
                      <ul className="flex flex-col gap-1">
                        {EMPTY_SCENE_EXAMPLES.map((example) => (
                          <li
                            key={example.commandId}
              ...
              {previewOpen && previewMode === "dock" ? (
                <>
                  ...
                  <div
                    ...
                    className="min-h-0 shrink-0 border-l border-edge"
                  >
