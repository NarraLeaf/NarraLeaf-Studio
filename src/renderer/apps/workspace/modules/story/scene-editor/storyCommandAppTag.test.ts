...
describe("app tag command slot - resolution", () => {
  ...
  it("matches the name case-insensitively, the way a scene name is matched", () => {
    expect(resolveCommandLine(commandLine("demo"), CONTEXT).args.tag).toEqual({
      kind: "appTag",
      appTagId: "tag-demo"
    });
  });
  ...
  it("reports two variants of one name rather than picking one", () => {
    ...
    expect(
      resolveCommandLine(commandLine("Demo"), twins).issues.map((issue) => issue.code)
  ...
  it("still takes the release variant's name, which is what a stranded row reads as", () => {
    ...
    expect(resolveCommandLine(commandLine("main"), CONTEXT).args.tag).toEqual({
      kind: "appTag",
      appTagId: APP_TAG_ID_RELEASE
    });
  });
