...
const CONTEXT: StoryCommandContext = {
  images: [
    ...
    { id: "i2", name: "forest_night" },
    { id: "i3", name: "city rain" }
  ],
...
describe("getCommandCursor", () => {
  ...
  it("offers param names once every positional is given", () => {
    ...
    expect(
      (cursor as Extract<StoryCommandCursor, { kind: "paramName" }>).params.map((p) => p.name)
    ).toEqual(["t", "d"]);
  });
  ...
  it("widens the caret inside a single-quoted name to the whole quoted region", () => {
    ...
    expect(at("/set gold 'Comp|")).toMatchObject({
      kind: "expression",
      query: "Comp",
  ...
  it("knows when the caret is inside a visited / picked argument", () => {
    ...
    expect(at("/set flag visited(Chap|)")).toMatchObject({
      kind: "expression",
      query: "Chap",
      call: "visited"
    });
    expect(at("/set flag picked( Ref|")).toMatchObject({
      kind: "expression",
      query: "Ref",
      call: "picked"
    });
    // Quoted argument: the region rule still applies, and the enclosing call is still known.
    expect(at("/set flag visited('Chap|ter 2')")).toMatchObject({
      kind: "expression",
      query: "Chap",
      call: "visited"
    });
...
describe("defaultHighlights", () => {
  ...
  it("does not highlight when the typed text already IS the top candidate", () => {
    ...
    expect(
      defaultHighlights(at("/local met true|"), [{ value: "true" }, { value: "false" }] as never)
    ).toBe(false);
...
describe("getCommandCandidates", () => {
  ...
  it("offers a speaker's forms only once the speaker resolves", () => {
    ...
    const resolvedTarget = {
      target: {
        ...
        target: { type: "character" as const, characterId: "c1", name: "Alice" }
      }
    };
  ...
  it("says which world a name lives in when the slot spans several", () => {
    ...
    const details = (marked: string) =>
      getCommandCandidates(at(marked), CONTEXT, {}).map((c) => `${c.value}:${c.detailKind ?? ""}`);
  ...
  it("offers Alice's forms as the positional after her name once she resolves", () => {
    ...
    expect(
      values("/face Alice |", { character: { kind: "character", characterId: "c1" } })
    ).toEqual(["smile", "angry"]);
  });
  ...
  it("offers the variables in scope inside an expression, leading with true/false for a boolean target", () => {
    const variable = (valueType: "number" | "boolean") => ({
      variable: {
        ...
        ref: { scope: "scene" as const, variableId: "v1" },
        valueType,
        name: "gold"
      }
    });
...
describe("hasCandidateSource", () => {
  ...
  it("asks the model, not the grammar, whether a puppet name has a source", () => {
    ...
    expect(
      hasCandidateSource(motion, CONTEXT, { character: { kind: "character", characterId: "c3" } })
    ).toBe(true);
    // Described, but with nothing on this channel: no list, so no empty state either.
    expect(
      hasCandidateSource(param("skin", "name"), CONTEXT, {
        character: { kind: "character", characterId: "c3" }
      })
    ).toBe(false);
    // A puppet nobody could ask, and a line that has not named anyone yet.
    expect(
      hasCandidateSource(motion, CONTEXT, { character: { kind: "character", characterId: "c4" } })
    ).toBe(false);
...
describe("completionFor", () => {
  ...
  it("replaces the whole token being typed, not just what follows the caret", () => {
    expect(completionFor(at("/bg fo|"), "forest_day")).toEqual({
      text: "forest_day ",
      replace: { start: 4, end: 6 }
    });
  });
