...
describe("richText", () => {
  ...
  it("merges adjacent runs with equal marks and drops empty runs", () => {
    ...
    ).toEqual([{ text: "ab", marks: { bold: true } }, { text: "c" }]);
  });
  ...
  it("collapses effectively-plain content to undefined", () => {
    ...
    expect(richIfMeaningful([{ text: "x", marks: { italic: true } }])).toEqual([
      { text: "x", marks: { italic: true } }
    ]);
    expect(richIfMeaningful([{ text: "x" }, { pause: true }])).toEqual([
      { text: "x" },
      { pause: true }
    ]);
  });
  ...
  it("toggles a mark off when the whole range already carries it", () => {
    ...
    expect(
      applyMarkToRange(runs, 0, 3, (marks) => ({ ...marks, bold: active ? undefined : true }))
    ).toEqual([{ text: "abc" }]);
  });
  ...
  it("styles an inline value chip like a word (marks apply to the atomic unit)", () => {
    const interp = {
      ...
      target: { scope: "scene" as const, variableId: "gold" }
    };
  ...
  it("splices a pause into the run stream and counts units", () => {
    expect(spliceRuns([{ text: "abcd" }], 2, 2, [{ pause: 300 }])).toEqual([
      { text: "ab" },
      { pause: 300 },
      { text: "cd" }
    ]);
  ...
  describe("ruby", () => {
    it("annotates the selected characters and splits the run at its edges", () => {
      const runs = applyMarkToRange(
        [{ text: "漢字を読む" }],
        0,
        2,
        (marks) => ({ ...marks, ruby: "かんじ" }),
        { textOnly: true }
      );
      expect(runs).toEqual([{ text: "漢字", marks: { ruby: "かんじ" } }, { text: "を読む" }]);
    });
    ...
    it("leaves an inline value chip in the range unannotated", () => {
      ...
      const interp = {
        kind: "variable" as const,
        target: { scope: "scene" as const, variableId: "name" }
      };
      const runs: StoryRichRun[] = [{ text: "御名" }, { interpolation: interp }];
      expect(
        applyMarkToRange(runs, 0, 3, (marks) => ({ ...marks, ruby: "みな" }), { textOnly: true })
      ).toEqual([{ text: "御名", marks: { ruby: "みな" } }, { interpolation: interp }]);
    ...
    it("reports one reading for a range only when every character shares it", () => {
      const runs: StoryRichRun[] = [{ text: "漢字", marks: { ruby: "かんじ" } }, { text: "を" }];
    ...
    it("ignores chips when reading a range, so one beside the words does not deny the reading", () => {
      const interp = {
        kind: "variable" as const,
        target: { scope: "scene" as const, variableId: "n" }
      };
      expect(
        rangeMarkRuby([{ text: "漢", marks: { ruby: "かん" } }, { interpolation: interp }], 0, 2)
      ).toBe("かん");
    });
    ...
    it("removes the reading without disturbing the marks beside it", () => {
      ...
      expect(
        applyMarkToRange(runs, 0, 2, (marks) => ({ ...marks, ruby: undefined }), { textOnly: true })
      ).toEqual([{ text: "漢字", marks: { bold: true } }]);
    });

    it("keeps an annotated run out of the plain-text collapse", () => {
      expect(richIfMeaningful([{ text: "漢字", marks: { ruby: "かんじ" } }])).toEqual([
        { text: "漢字", marks: { ruby: "かんじ" } }
      ]);
    });
  });
  ...
  describe("inline event runs", () => {
    ...
    it("survives normalizeRuns intact — never mis-routed to a pause run", () => {
      ...
      expect(normalizeRuns([{ text: "a" }, event, { text: "b" }])).toEqual([
        { text: "a" },
        event,
        { text: "b" }
      ]);
    ...
    it("counts as one atomic unit and splices like a chip", () => {
      ...
      expect(spliceRuns([{ text: "abcd" }], 2, 2, [event])).toEqual([
        { text: "ab" },
        event,
        { text: "cd" }
      ]);
      // Deleting the event's single unit rejoins the surrounding text.
      expect(spliceRuns([{ text: "a" }, event, { text: "b" }], 1, 2, [])).toEqual([{ text: "ab" }]);
    });
