...
export function findParam(def: StoryCommandDef, key: string): StoryCommandParam | null {
  ...
  return (
    def.params.find((param) => param.name === normalized) ??
...
export function numberValueOf(
  ...
  const body =
    unit && trimmed.length > unit.length && trimmed.toLowerCase().endsWith(unit.toLowerCase())
...
export function matchEnumOption(
  type: Extract<StoryCommandParamType, { kind: "enum" }>,
  raw: string
): StoryCommandEnumOption | null {
  const normalized = raw.trim().toLowerCase();
  return (
    type.options.find((option) => option.value.toLowerCase() === normalized) ??
 ...
 */
export function freeTargetKind(
  type: Extract<StoryCommandParamType, { kind: "target" }>
): StoryCommandStageObjectKind | null {
  const stageKinds = type.accepts.filter(
    (kind): kind is StoryCommandStageObjectKind => kind !== "character"
  );
...
export function dependsOnParam(type: StoryCommandParamType): string | null {
  if (
    type.kind === "characterForm" ||
    type.kind === "content" ||
    type.kind === "puppetName" ||
    type.kind === "puppetParam"
  ) {
