...
import type {
  StorySceneEditorDraftJump,
  StorySceneEditorTabPayload
} from "./storySceneEditorTabId";
...
import {
  useStorySceneClipboardHandlers,
  type StoryPasteWizardRequest
} from "./useStorySceneClipboardHandlers";
...
import {
  forgetStoryPasteSeparator,
  getStoryPasteMemory,
  rememberStoryPasteSpeakers,
  saveStoryPasteSeparator
} from "./storyPasteMemory";
...
import {
  ...
  ALT_ACTION_TRIGGER,
  isActionCommandLine,
  toCanonicalCommandLine,
  toDisplayedCommandLine
} from "./commandTrigger";
...
export function useStorySceneEditorController(
  tabId: string,
  payload: StorySceneEditorTabPayload | undefined
) {
  ...
  const uuidService = useMemo(
    () => (context && isInitialized ? context.services.get<UuidService>(Services.Uuid) : null),
    [context, isInitialized]
  );
  ...
  const puppetDescriptionService = useMemo(
    () =>
      context && isInitialized
        ? context.services.get<PuppetDescriptionService>(Services.PuppetDescription)
        : null,
    [context, isInitialized]
  );
  ...
  const [activeBlockId, setActiveBlockId] = useState<StoryBlockId | null>(() => {
    ...
    return (
      (panelStateService && sceneId
        ? getStoryEditorViewState(panelStateService, sceneId)?.activeBlockId
        : null) ?? null
    );
  });
  const [selectedBlockIds, setSelectedBlockIds] = useState<Set<StoryBlockId>>(() => {
    ...
    const saved =
      panelStateService && sceneId
        ? getStoryEditorViewState(panelStateService, sceneId)?.selectedBlockIds
        : undefined;
  ...
  const [rowFilter, setRowFilterState] = useState<StoryRowFilter>(() => {
    ...
    return prefs
      ? { facets: new Set(prefs.selectedRowFacets), speakers: new Set(prefs.selectedRowSpeakers) }
      : EMPTY_STORY_ROW_FILTER;
  });
  const [density, setDensityState] = useState<StoryEditorDensity>(() =>
    panelStateService ? getStoryEditorViewPrefs(panelStateService).density : "compact"
  );
  const setRowFilter = useCallback(
    (value: StoryRowFilter) => {
      setRowFilterState(value);
      if (panelStateService) {
        patchStoryEditorViewPrefs(panelStateService, {
          selectedRowFacets: [...value.facets],
          selectedRowSpeakers: [...value.speakers]
        });
      }
    },
    [panelStateService]
  ...
  const updateTextDraft = useCallback(
    (blockId: StoryBlockId, value: string, rich: StoryRichRun[]) => {
      const draft = textDraftRef.current;
  ...
  useEffect(() => {
    ...
    setEditorMode((current) =>
      current.kind === "text" || current.kind === "insert" ? { kind: "idle" } : current
    );
  }, [frozen]);
  ...
  useEffect(() => {
    ...
    patchStoryEditorViewState(panelStateService, sceneId, {
      activeBlockId,
  ...
  const scene = useMemo(
    () => (document && sceneId ? (document.scenes[sceneId] ?? null) : null),
    [document, sceneId]
  );
  /**
  ...
  const characters = useMemo(
    ...
    [characterRevision, characterService]
  );
  ...
  const puppetCharacters = useMemo(
    () =>
      characters
        .filter((character) => character.profile.appearance.getKind() === "puppet")
        .map((character) => character.profile.getId()),
    [characters]
  );
  ...
  const dialogueAppearances = useMemo(
    () => (scene ? buildDialogueAppearances(scene) : null),
    [document, scene]
  );
  /**
   ...
   */
  const revealRowInFilter = useCallback(
    (block: StoryBlock) => {
  ...
  const unfilteredRows = useMemo(
    () => (scene ? buildVisibleRows(scene, collapsedBlockIds) : []),
    [collapsedBlockIds, document, scene]
  );
  const rowFilterTallies = useMemo(
    () =>
      tallyStoryRows(
        unfilteredRows.map((row) => row.block),
        characterIdByName,
        [...rowFilter.speakers]
      ),
  ...
  const shouldRenderActiveInsertSlot =
    editorMode.kind === "insert" && editorMode.slot.afterBlockId !== null;
  const editorFocusKey =
    editorMode.kind === "insert"
  ...
  const extendDragSelectionAtPoint = useCallback(
    (x: number, y: number) => {
   ...
   */
  const finishTextSelectGesture = useCallback(
    (pending: { blockId: StoryBlockId; textEl: HTMLElement }) => {
   ...
   */
  const escalateTextSelectToRowDrag = useCallback(
    (blockId: StoryBlockId, x: number, y: number) => {
  ...
  useEffect(() => {
    ...
    escalateTextSelectToRowDrag,
    extendDragSelectionAtPoint,
    finishTextSelectGesture,
    setRowTextSelectable,
    startDragSelectAutoScroll,
    stopDragSelection
  ]);
  ...
  const updateBlockPayloadFor = useCallback(
    (blockId: StoryBlockId, payload: StoryBlock["payload"]) => {
      if (storyService && storyId && sceneId) {
   ...
   */
  const updateBlockPayloads = useCallback(
    (edits: readonly { blockId: StoryBlockId; payload: StoryBlock["payload"] }[]) => {
  ...
  const updateSceneMetadata = useCallback(
    (patch: StorySceneUpdate): boolean => {
      ...
      const nextName =
        patch.name !== undefined
          ? patch.name.trim() || scene.name || translate("story.sceneEditor.defaultSceneName")
          : scene.name;
      const nextDescription =
        patch.description !== undefined ? patch.description.trim() : (scene.description ?? "");
      const nextBackgroundAssetId =
      ...
      const hasDescriptionChange =
        patch.description !== undefined && nextDescription !== (scene.description ?? "");
      const hasBackgroundChange =
        patch.defaultBackgroundAssetId !== undefined &&
        nextBackgroundAssetId !== (scene.defaultBackgroundAssetId ?? undefined);
      ...
      const changed = storyService.updateScene(storyId, sceneId, {
        ...
        defaultBackgroundAssetId:
          patch.defaultBackgroundAssetId !== undefined ? (nextBackgroundAssetId ?? null) : undefined
      });
      if (changed) {
        if (hasNameChange) {
  ...
  const createBlock = useCallback(
    (kind: ActionCommandId, initialText = "", characterId?: string): StoryBlock | null => {
      if (!uuidService) {
        return null;
      }
  ...
  const insertBlock = useCallback(
    (
      ...
      storyService.insertBlock(
        storyId,
        sceneId,
        block,
        options?.target ?? getInsertionTargetAfter(scene, afterBlockId)
      );
  ...
  const createLayerBeforeBlock = useCallback(
    (beforeBlockId: StoryBlockId): StoryBlockId | null => {
      ...
      recordHistory();
      storyService.insertBlock(storyId, sceneId, block, {
  ...
  const startInsertAfter = useCallback(
    (afterBlockId: StoryBlockId | null, focus = true, confirmation?: string) => {
      slotDiscardedRef.current = false;
      insertDraftRef.current = "";
      setEditorMode({
        kind: "insert",
        slot: { afterBlockId, focusToken: Date.now() },
        initialValue: "",
        confirmation
      });
      if (afterBlockId) {
   ...
   */
  const startInsertBefore = useCallback(
    (blockId: StoryBlockId, focus = true) => {
      const block = scene?.blocks[blockId];
      if (!block) {
        return;
      }
  ...
  const startLineEdit = useCallback(
    (block: StoryBlock) => {
      ...
      slotDiscardedRef.current = false;
      insertDraftRef.current =
        block.kind === "invalid" ? block.payload.source : (getTextSegment(block)?.value ?? "");
      setEditorMode({
        kind: "insert",
        slot: {
   ...
   */
  const startCharacterActionSlot = useCallback(
    (blockId: StoryBlockId, value: string): boolean => {
      ...
      setEditorMode({
        ...
        slot: {
          ...
          characterScope: {
            characterId: character.profile.getId(),
  ...
  const startInsertInside = useCallback(
    (parentId: StoryBlockId) => {
      if (!scene) {
        return;
      }
      ...
      setEditorMode({
        kind: "insert",
        slot: {
          afterBlockId: lastChildId ?? parentId,
          focusToken: Date.now(),
          target: { parentId, beforeBlockId: null }
        },
        initialValue: ""
      });
   ...
   */
  const startJumpDraft = useCallback(
    (draft: StorySceneEditorDraftJump): boolean => {
      if (frozen || !scene || !document) {
        return false;
      }
      const line = writeStoryJumpLine(
        draft.targetSceneId,
        document.scenes,
        slashAtAlias ? ALT_ACTION_TRIGGER : ACTION_TRIGGER
      );
      if (!line) {
        return false;
      }
  ...
  const addConditionBranch = useCallback(
    (conditionId: StoryBlockId, branch: "if" | "elseIf" | "else") => {
      if (!scene) {
        return;
      }
      ...
      const elseBranch = branchBlocks.find(
        (child) => child.payload.control === "conditionBranch" && child.payload.branch === "else"
      );
  ...
  const addInsideContainer = useCallback(
    (parentId: StoryBlockId) => {
  ...
  const scaffoldContainer = useCallback(
    (block: StoryBlock, condition?: StoryExpression) => {
      ...
      if (block.kind === "control" && block.payload.control === "condition") {
        ...
        const seeded =
          condition && branch.kind === "control" && branch.payload.control === "conditionBranch"
            ? {
                ...branch,
                payload: {
                  ...branch.payload,
                  condition: { kind: "expression" as const, expression: condition }
                }
              }
            : branch;
        storyService.insertBlock(storyId, sceneId, seeded, {
          parentId: block.id,
          beforeBlockId: null
        });
        ...
        storyService.insertBlock(storyId, sceneId, option, {
          parentId: block.id,
          beforeBlockId: null
        });
      }
  ...
  const insertContinuationAfterCurrentTextEdit = useCallback(() => {
    ...
    const characterId =
      currentBlock.kind === "nodeAction" && currentBlock.payload.action === "dialogue"
  ...
  const navigateFromTextEdit = useCallback(
    (direction: "up" | "down" | "left" | "right", caretX?: number | null) => {
      if (editorMode.kind !== "text") {
        return;
      }
   ...
   */
  const activateBlockForInspectorOrOp = useCallback(
    (blockId: StoryBlockId) => {
      const block = scene?.blocks[blockId];
      if (!block) {
        return;
  ...
  const commitNarrationFromInsert = useCallback(
    (focusNext: boolean) => {
      ...
      if (
        isActionCommandLine(insertDraftRef.current, slashAtAlias) ||
        insertDraftRef.current.startsWith("#")
      ) {
        return;
      }
      ...
      insertBlock(block, editorMode.slot.afterBlockId, false, {
        target: editorMode.slot.target,
        replaceBlockId: editorMode.slot.replaceBlockId
      });
      if (focusNext) {
  ...
  const dismissInsertChooser = useCallback(() => {
    setEditorMode((current) =>
      current.kind !== "insert" ? current : { ...current, chooserDismissed: true }
    );
  }, []);
  ...
  useEffect(() => {
    ...
    const anchorId =
      editorMode.slot.replaceBlockId ??
      editorMode.slot.target?.beforeBlockId ??
      editorMode.slot.afterBlockId;
   ...
   */
  const createPluginActionBlock = useCallback(
    (actionId: string, initialText?: string): StoryBlock | null => {
  ...
  const chooseCharacterForInsert = useCallback(
    (characterId: string) => {
      if (editorMode.kind !== "insert") {
        return;
      }
      ...
      if (block) {
        ...
        setEditorMode({
          kind: "text",
          blockId: block.id,
          value: getTextSegment(block)?.value ?? ""
        });
      }
    },
  ...
  const commitCommandFromInsert = useCallback(
    (value: string): boolean => {
      ...
      const replacing = editorMode.slot.replaceBlockId
        ? scene?.blocks[editorMode.slot.replaceBlockId]
        : null;
      const followTarget = replacing
        ? (() => {
            const siblings =
              (replacing.parentId
                ? scene?.blocks[replacing.parentId]?.childrenIds
                : scene?.rootBlockIds) ?? [];
            const index = siblings.indexOf(replacing.id);
            return {
              parentId: replacing.parentId,
              beforeBlockId: index === -1 ? null : (siblings[index + 1] ?? null)
            };
          })()
        : null;
      const block = spec.build(args, {
        generateId: () => uuidService.generate(),
        context: commandContext
      });
      const openInspector = opensInspectorAfterCommit(spec, block);
      insertBlock(block, editorMode.slot.afterBlockId, openInspector, {
        target: editorMode.slot.target,
        replaceBlockId: editorMode.slot.replaceBlockId
      });
  ...
  const chooseCommand = useCallback(
    (commandId: string) => {
      ...
      const initialText = toCanonicalCommandLine(insertDraftRef.current, slashAtAlias).replace(
        /^\/\S*\s?/,
        ""
      );
  ...
  const resolveInsertLine = useCallback(() => {
    ...
    commitCommandFromInsert,
    commitInvalidFromInsert,
    commitNarrationFromInsert,
    editorMode,
    slashAtAlias
  ]);
   ...
   */
  const chooseTempSpeakerForInsert = useCallback(
    (speakerName: string) => {
      if (editorMode.kind !== "insert") {
        return;
      }
   ...
   */
  const createActionFromSidebar = useCallback(
    (commandId: string) => {
      const spec = getCommandSpec(commandId);
      if (!spec?.build || !uuidService) {
      ...
      const block = spec.build(
        {},
        { generateId: () => uuidService.generate(), context: commandContext }
      );
   ...
   */
  const setDialogueSpeaker = useCallback(
    (block: StoryBlock, speaker: { characterId: string } | { speakerName: string } | null) => {
      if (block.kind !== "nodeAction" || block.payload.action !== "dialogue") {
        return;
      }
   ...
   */
  const setDialogueGroupPosition = useCallback(
    (head: StoryBlock, position: StoryStagePlacement, positionSourceId: StoryBlockId | null) => {
      if (!storyService || !storyId || !sceneId || !scene || !uuidService) {
        return;
      }
      if (
        head.kind !== "nodeAction" ||
        head.payload.action !== "dialogue" ||
        !head.payload.characterId
      ) {
        return;
      }
      ...
      insertBlock(move, null, false, {
        target: { parentId: head.parentId, beforeBlockId: head.id }
      });
    },
   ...
   */
  const createCharacterFromSpeaker = useCallback(
    (block: StoryBlock, name: string) => {
  ...
  const selectRow = useCallback(
    (blockId: StoryBlockId, event?: MouseEvent) => {
  ...
  const { copySelectionToClipboard, handlePaste, handleRowTextPaste, insertPastedBlocks } =
    useStorySceneClipboardHandlers({
      storyService,
      uuidService,
      uiService,
   ...
   */
  const confirmPasteWizard = useCallback(
    (plan: PastePlan, authoredMappings: Record<string, SpeakerMappingTarget>) => {
      ...
      try {
        ...
        rememberStoryPasteSpeakers(
          panelStateService,
          resolveMappingsForMemory(authoredMappings, createdCharacterIds)
        );
  ...
  const savePasteSeparator = useCallback(
    (name: string, choice: PasteSeparatorChoice) => {
  ...
  const forgetPasteSeparator = useCallback(
    (name: string) => {
  ...
  const handlePasteInEditor = useCallback(
    (event: ClipboardEvent<HTMLDivElement>) => {
      if (!isStoryEditorFocusActive()) {
        return;
      }
  ...
  const deleteRows = useCallback(
    async (ids: string[], options?: { confirmMultiple?: boolean }) => {
      if (!storyService || !storyId || !sceneId || !scene) {
        return;
      }
      if (ids.length === 0) {
  ...
  const deleteSelection = useCallback(
    async (options?: { confirmMultiple?: boolean }) => {
      const ids =
        selectedBlockIds.size > 0 ? [...selectedBlockIds] : activeBlockId ? [activeBlockId] : [];
    ...
    (direction: "in" | "out") => {
      ...
      for (const id of roots) {
        ...
        if (direction === "in") {
          ...
          const parentSiblings = grandParentId
            ? scene.blocks[grandParentId]?.childrenIds
            : scene.rootBlockIds;
          const beforeBlockId =
            parentSiblings?.[parentSiblings.indexOf(block.parentId) + 1] ?? null;
          recordOnce();
  ...
  const moveActiveRowSelection = useCallback(
    (direction: "up" | "down") => {
  ...
  const jumpRowSelection = useCallback(
    (edge: "first" | "last") => {
  ...
  const pageRowSelection = useCallback(
    (direction: "up" | "down") => {
      if (visibleRows.length === 0) {
        return;
      }
      const currentIndex = activeBlockId ? (rowIndexById.get(activeBlockId) ?? -1) : -1;
      const base =
        currentIndex === -1 ? (direction === "down" ? -1 : visibleRows.length) : currentIndex;
      const nextIndex =
        direction === "down"
  ...
  const duplicateSelection = useCallback(() => {
    ...
    const orderedRoots = visibleRows
      .map((row) => row.block.id)
      .filter((blockId) => rootSet.has(blockId));
    ...
    for (const rootId of orderedRoots) {
      const cloned = cloneSerializedBlock(serializeBlockSubtree(scene, rootId), () =>
        uuidService.generate()
      );
  ...
  const beginBlockDrag = useCallback(
    (blockId: StoryBlockId): StoryBlockId[] => {
   ...
   */
  const moveDraggedBlocksToSortablePosition = useCallback(
    (draggedBlockId: StoryBlockId, targetBlockId: StoryBlockId) => {
  ...
  return {
    ...
    revealBlock,
    handleKeyDown,
    copySelectionToClipboard: handleCopy,
    handlePaste: handlePasteInEditor,
    handleRowTextPaste,
    pasteWizard,
    pasteMemory,
    cancelPasteWizard,
    confirmPasteWizard,
    savePasteSeparator,
    forgetPasteSeparator,
    deleteRows,
    deleteSelection,
    replaceRowWithBlankLine,
    startInsertAfter,
    startInsertBefore,
    startJumpDraft,
    selectRow,
    ...
    updateSceneMetadata,
    setDialogueSpeaker,
    setDialogueGroupPosition,
    createCharacterFromSpeaker,
    commitTextEdit,
    ...
    undoEdit,
    redoEdit,
    startInsertAfterSelection,
    indentSelection,
    selectAllRows,
    moveActiveRowSelection,
    insertContinuationAfterCurrentTextEdit,
    commitNarrationFromInsert,
    handleInsertBackspaceEmpty,
    chooseCommand,
    chooseCharacterForInsert,
    dismissInsertChooser,
    discardInsertSlot,
    resolveInsertLine,
    commitInvalidFromInsert,
    chooseTempSpeakerForInsert,
    tempSpeakers,
    createActionFromSidebar,
    addInsideContainer,
    addConditionBranch,
    navigateFromTextEdit,
    resetGoalColumn,
    handleBackspaceAtEmptyStart,
    startCharacterActionSlot,
    enterEditOrInspectorForActive,
    activateBlockForInspectorOrOp,
    closeInspector,
    ...
    beginBlockDrag,
    endBlockDrag,
    moveDraggedBlocksToSortablePosition,
...
function resolveMappingsForMemory(
  ...
  for (const [key, target] of Object.entries(authoredMappings)) {
    ...
    const created = Object.entries(createdCharacterIds).find(
...
function nextLayerName(scene: StoryScene): string {
  ...
  for (const block of Object.values(scene.blocks)) {
    if (
      ...
      block.payload.operation === "create"
    ) {
