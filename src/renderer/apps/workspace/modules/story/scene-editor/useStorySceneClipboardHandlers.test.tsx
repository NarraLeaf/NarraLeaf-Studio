...
function uuidFactory(): { generate: () => string } {
  let issued = 0;
  return {
    generate: () => `00000000-0000-4000-8000-${(issued += 1).toString(16).padStart(12, "0")}`
  };
}
...
describe("plain paste over the confirm threshold", () => {
  const BULK = Array.from(
    { length: STORY_PASTE_CONFIRM_THRESHOLD + 10 },
    (_, index) => `line ${index}`
  ).join("\n");
  ...
  it("inserts the rows when the confirm comes back yes", async () => {
    ...
    expect(handlers.storyService.insertBlock).toHaveBeenCalledTimes(
      STORY_PASTE_CONFIRM_THRESHOLD + 10
    );
  });
