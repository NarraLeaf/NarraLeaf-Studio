...
import {
  addRecentColor,
  useRecentColors
} from "@/apps/workspace/modules/properties/framework/fields/recentColors";
 ...
 */
export const RichTextToolbar = forwardRef<
...
>(function RichTextToolbar(props, ref) {
  ...
  const controls = () =>
    Array.from(stripRef.current?.querySelectorAll<HTMLButtonElement>("button") ?? []);
  ...
  const strip = expanded ? (
    ...
    >
      <button
        ...
        className={BTN}
        onClick={collapse}
        data-tip={t("story.richText.collapse")}
        aria-label={t("story.richText.collapse")}
      >
      ...
      <button
        type="button"
        className={active.bold ? BTN_ACTIVE : BTN}
        aria-pressed={active.bold}
        onClick={() => keepKeyboard(() => props.editor.current?.toggleMark("bold"))}
        data-tip={t("story.richText.bold")}
        aria-label={t("story.richText.bold")}
      >
        <Bold className="h-3.5 w-3.5" />
      </button>
      <button
        type="button"
        className={active.italic ? BTN_ACTIVE : BTN}
        aria-pressed={active.italic}
        onClick={() => keepKeyboard(() => props.editor.current?.toggleMark("italic"))}
        data-tip={t("story.richText.italic")}
        aria-label={t("story.richText.italic")}
      >
      ...
      <button
        ...
        data-tip={active.canRuby ? t("story.richText.ruby") : t("story.richText.rubyHint")}
        aria-label={active.canRuby ? t("story.richText.ruby") : t("story.richText.rubyHint")}
      >
        ...
        return (
          <button
            ...
            className={`h-4 w-4 rounded-full border transition-transform hover:scale-110 ${FOCUS_RING} ${
              isActive
                ? "scale-110 border-fg ring-2 ring-fg/80 ring-offset-1 ring-offset-surface-raised"
                : "border-edge-strong"
            }`}
            style={{ backgroundColor: color }}
            aria-pressed={isActive}
            onClick={() => keepKeyboard(() => applyColor(color))}
            data-tip={t("story.richText.textColor", { color })}
            aria-label={t("story.richText.textColor", { color })}
          />
        );
      })}
      <button
        ...
        onClick={() =>
          palette
            ? // Closing it from the keyboard leaves the author on this button — they are still
              ...
              closePalette(
                stripRef.current?.contains(globalThis.document.activeElement) ? "trigger" : "editor"
              )
            : openPalette()
        }
      ...
      <button
        type="button"
        className={BTN}
        onClick={() => keepKeyboard(() => props.editor.current?.insertPause(true))}
        data-tip={t("story.richText.insertPause")}
        aria-label={t("story.richText.insertPause")}
      >
      ...
      <button
        ...
        onClick={() =>
          keepKeyboard(() =>
            props.editor.current?.insertInterpolation(
              defaultInterpolationForKind(getLastInterpolationKind())
            )
          )
        }
        data-tip={
          props.hasVariables ? t("story.richText.insertValue") : t("story.richText.insertValueHint")
        }
        aria-label={
          props.hasVariables ? t("story.richText.insertValue") : t("story.richText.insertValueHint")
        }
      ...
      {props.canInsertEvent ? (
        ...
        <button
          type="button"
          className={BTN}
          onClick={() => props.onInsertEvent?.()}
          data-tip={t("story.richText.insertExpression")}
          aria-label={t("story.richText.insertExpression")}
        >
    ...
    <button
      ...
      data-tip={t("story.richText.tools")}
      aria-label={t("story.richText.tools")}
    >
