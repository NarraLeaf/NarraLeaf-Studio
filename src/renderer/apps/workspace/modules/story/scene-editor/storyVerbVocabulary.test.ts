...
describe("storyVerbVocabulary", () => {
  it("names only commands that actually exist", () => {
    ...
    const operations: Array<[StoryActionPayload["action"], readonly string[]]> = [
      [
        "character",
        ["enter", "exit", "move", "expression", "setName", "setMotion", "setSkin", "setParams"]
      ],
  ...
  it("splits mute from unmute on the flag, since one payload carries both verbs", () => {
    expect(
      storyVerbCommandId(payload({ action: "audio", operation: "muteSound", muted: true }))
    ).toBe("mute");
    expect(
      storyVerbCommandId(payload({ action: "audio", operation: "muteSound", muted: false }))
    ).toBe("unmute");
  ...
  it("says nothing for the inspector-only displayable operations", () => {
    for (const operation of ["mask", "clearMask", "clip", "filter", "backdrop", "blend"]) {
      expect(
        storyVerbCommandId(payload({ action: "displayable", operation })),
        operation
      ).toBeNull();
    }
  });
  ...
  it("builds the same key namespace the action creator reads", () => {
    ...
    expect(storyVerbLabelKey(payload({ action: "character", operation: "exit" }))).toBe(
      "story.command.hide.label"
    );
  });
});
