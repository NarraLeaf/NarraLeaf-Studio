...
describe("tokenizeCommandLine", () => {
  ...
  it("groups single-quoted entity references exactly like double quotes, recording the kind", () => {
    ...
    expect(tokens[1]).toMatchObject({
      text: "Complex Var Name",
      raw: "'Complex Var Name'",
      quoted: true,
      quote: "single"
    });
  ...
  it("treats the other quote kind as data inside a quoted token", () => {
    ...
    expect(tokenizeCommandLine('/bg "Bob\'s Bar"', 1).tokens[1]).toMatchObject({
      text: "Bob's Bar",
      quote: "double"
    });
    expect(tokenizeCommandLine("/bg 'say \"hi\"'", 1).tokens[1]).toMatchObject({
      text: 'say "hi"',
      quote: "single"
    });
  });
  ...
  it("collapses runs of spaces", () => {
    expect(tokenizeCommandLine("/bg   forest", 1).tokens.map((t) => t.text)).toEqual([
      "bg",
      "forest"
    ]);
  });
...
describe("parseCommandLine - classification", () => {
  it("classifies empty, narration, character and command lines", () => {
    ...
    expect(parseCommandLine("#alice 你好")).toMatchObject({
      kind: "character",
      query: "alice",
      text: "你好"
    });
  ...
  it("treats a bare slash as a command with no token yet", () => {
    expect(parseCommandLine("/")).toMatchObject({
      kind: "command",
      token: null,
      def: null,
      issues: []
...
describe("parseCommandLine - command resolution", () => {
  ...
  it("reports an unknown command and stops", () => {
    ...
    expect(line.issues).toEqual([
      { code: "unknownCommand", span: { start: 1, end: 4 }, token: "bgg" }
    ]);
  });
});
...
describe("parseCommandLine - a command named in the active locale", () => {
  ...
  it("parses a Chinese token's arguments exactly as the English token would - English keys keep working", () => {
    ...
    expect(localized.args.map((arg) => [arg.param?.name, arg.value])).toEqual(
      english.args.map((arg) => [arg.param?.name, arg.value])
    );
...
describe("parseCommandLine - args", () => {
  it("fills positional params in declaration order", () => {
    ...
    expect(line.args.map((arg) => [arg.param?.name, arg.value])).toEqual([
      ["variable", "gold"],
      ["value", "100"]
    ]);
  });
...
describe("canCommit", () => {
  ...
  it("blocks a command whose required core is unfilled, and names the missing slot", () => {
    ...
    expect(missingCoreParams(parseCommandLine("/bg")).map((param) => param.name)).toEqual([
      "image"
    ]);
