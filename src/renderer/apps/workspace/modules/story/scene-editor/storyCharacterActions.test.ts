...
describe("storyCharacterActions — which verbs a speaker's line may insert", () => {
    it("offers every verb that acts on a character already on stage, and only those", () => {
        ...
        // `/transform` and `/reset` are here because their target `accepts` includes a character —
        // the same rule the sidebar files them by. Hard-coding a list of "character commands" instead
        // is exactly the object-type × verb matrix the taxonomy exists to refuse. M2 shrank the list
        // by three (`/fx`, `/mirror`, `/move` are all `/transform` with a prop) and grew it by one.
        expect(scoped).toEqual(["face", "motion", "param", "rename", "reset", "say", "skin", "transform"].sort());
    });

    it("leaves the staging pair out — the speaker of the line is already on stage", () => {
        const scoped = new Set(characterScopedActions(specPaletteCommands()).map(command => command.id));
    ...
    it("leaves out the verbs that never name a character", () => {
        const scoped = new Set(characterScopedActions(specPaletteCommands()).map(command => command.id));
        for (const id of ["background", "bgm", "sound", "image", "video", "set", "jump", "menu", "note"]) {
...
describe("storyCharacterActions — filling the speaker in", () => {
    ...
    it("has a lead for every scoped verb it offers", () => {
        ...
        for (const spec of listCommandSpecs()) {
            if (!characterScopedActions(specPaletteCommands()).some(command => command.id === spec.id)) {
                continue;
            }
...
describe("storyCharacterActions — whose line it is", () => {
    ...
    it("finds the speaker a dialogue row points at", () => {
        expect(dialogueActionCharacter(dialogue({ characterId: "alice" }), cast)?.profile.getName()).toBe("Alice");
    });
    ...
    it("declines a row that is not dialogue at all", () => {
        const narration = { id: "row", parentId: null, childrenIds: [], kind: "narration", payload: { text: { value: "" } } } as unknown as StoryBlock;
...
describe("storyCharacterActions — what a speaker's paragraph absorbs", () => {
    ...
    const action = (payload: Record<string, unknown>): StoryBlock =>
        ({ id: "row", parentId: null, childrenIds: [], kind: "action", payload }) as unknown as StoryBlock;

    it("absorbs every verb done to the speaker, not just the expression it started with", () => {
        for (const operation of ["expression", "move", "setMotion", "setSkin", "setParams", "setName"]) {
            expect(paragraphActionCharacterId(action({ action: "character", operation, characterId: "alice" }), cast)).toBe("alice");
        }
    });

    it("refuses the staging pair — a run must not draw its rule through the row that ends it", () => {
        for (const operation of ["enter", "exit"]) {
            expect(paragraphActionCharacterId(action({ action: "character", operation, characterId: "alice" }), cast)).toBeNull();
        }
    });

    it("resolves a displayable target by name, which is all /fx and /transform ever store", () => {
        const fx = action({ action: "displayable", operation: "filter", target: { kind: "character", name: "Alice" } });
        expect(paragraphActionCharacterId(fx, cast)).toBe("alice");
        // A layer or an image of the same name is a different subject entirely.
        const layer = action({ action: "displayable", operation: "transform", target: { kind: "layer", name: "Alice" } });
        expect(paragraphActionCharacterId(layer, cast)).toBeNull();
    });

    it("stays silent about a row with no speaker to borrow", () => {
        expect(paragraphActionCharacterId(action({ action: "character", operation: "move" }), cast)).toBeNull();
        expect(paragraphActionCharacterId(action({ action: "setBackground", color: "#000" }), cast)).toBeNull();
