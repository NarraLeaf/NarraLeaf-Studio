...
describe("speaker identities", () => {
  ...
  it("gives a character the same colour with and without artwork", () => {
    expect(characterSpeakerIdentity("Anyo", { hasPortrait: true }).paint).toEqual(
      characterSpeakerIdentity("Anyo", { hasPortrait: false }).paint
    );
  });

  it("takes the project's colour when there is one, and the name hash when there is not", () => {
    expect(
      characterSpeakerIdentity("Anyo", { hasPortrait: false, color: "#c94f7c" }).paint
    ).toEqual({ source: "author", hex: "#c94f7c" });
    expect(characterSpeakerIdentity("Anyo", { hasPortrait: false }).paint).toEqual({
      source: "name",
      hue: storySpeakerHash("Anyo")
    });
  });
