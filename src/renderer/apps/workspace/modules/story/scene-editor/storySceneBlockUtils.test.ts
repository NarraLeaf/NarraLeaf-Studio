...
function dialogue(
  ...
  return {
    ...
    payload: {
      action: "dialogue",
      ...speaker,
      text: { textId: `${id}-t`, role: "dialogue", value: id }
    }
  };
}
...
function characterAction(
  id: string,
  payload: Extract<StoryBlock, { kind: "action" }>["payload"]
): StoryBlock {
...
function narration(
  ...
  return {
    ...
    parentId,
    childrenIds,
    payload: { action: "narration", text: { textId: `${id}-t`, role: "narration", value: id } }
  };
}
...
describe("isContainerBlock", () => {
  it("is true for control, choice, choiceOption, and nvl", () => {
    ...
    expect(
      isContainerBlock(
        nodeAction({ action: "choiceOption", text: { textId: "t", role: "choiceText", value: "" } })
      )
    ).toBe(true);
    expect(
      isContainerBlock({
  ...
  it("is false for plain actions and narration", () => {
    expect(
      isContainerBlock(
        nodeAction({ action: "narration", text: { textId: "t", role: "narration", value: "hi" } })
      )
    ).toBe(false);
...
describe("getContainerHeaderInfo", () => {
  it("labels condition branches in plain language and flags which carry a condition", () => {
    expect(
      getContainerHeaderInfo(control({ control: "conditionBranch", branch: "if" }))
    ).toMatchObject({ pill: "If", role: "branch", hasCondition: true });
    expect(
      getContainerHeaderInfo(control({ control: "conditionBranch", branch: "elseIf" }))
    ).toMatchObject({ pill: "Else if", hasCondition: true });
    expect(
      getContainerHeaderInfo(control({ control: "conditionBranch", branch: "else" }))
    ).toMatchObject({ pill: "Otherwise", hasCondition: false });
  });

  it("exposes the repeat count on repeat groups", () => {
    expect(getContainerHeaderInfo(control({ control: "repeat", times: 3 }))).toMatchObject({
      pill: "Repeat",
      role: "group",
      repeatTimes: 3
    });
  ...
  it("labels the remaining group kinds and menu containers", () => {
    expect(getContainerHeaderInfo(control({ control: "parallel", mode: "all" }))?.role).toBe(
      "group"
    );
    expect(getContainerHeaderInfo(control({ control: "race", mode: "any" }))?.role).toBe("group");
    expect(getContainerHeaderInfo(control({ control: "sequence", mode: "do" }))?.role).toBe(
      "group"
    );
    expect(getContainerHeaderInfo(nodeAction({ action: "choice" }))).toMatchObject({
      pill: "Menu",
      role: "menu"
    });
  });
...
describe("annotateDialogueGroups", () => {
  ...
  const continuesOf = (blocks: StoryBlock[]) =>
    ...
    ).map((row) => row.groupContinues ?? false);
  ...
  it("starts a new group when the speaker changes", () => {
    expect(
      rolesOf([dialogue("a", { characterId: "c1" }), dialogue("b", { characterId: "c2" })])
    ).toEqual(["head", "head"]);
  });
  ...
  it("leaves a run of one, and the row after a run, with nothing to continue", () => {
    expect(continuesOf([dialogue("a", { characterId: "c1" }), narration("n")])).toEqual([
      false,
      false
    ]);
  });
  ...
  it("groups a run of narration the same way it groups one speaker", () => {
    expect(rolesOf([narration("a"), narration("b"), narration("c")])).toEqual([
      "head",
      "member",
      "member"
    ]);
    expect(continuesOf([narration("a"), narration("b"), narration("c")])).toEqual([
      true,
      true,
      false
    ]);
  ...
  it("still breaks a dialogue run at narration, and a narration run at dialogue", () => {
    ...
    expect(rolesOf([narration("a"), dialogue("d", { characterId: "c1" }), narration("b")])).toEqual(
      ["head", "head", "head"]
    );
  });
  ...
  it("groups bare speakers by exact name, but never two unnamed rows", () => {
    expect(
      rolesOf([dialogue("a", { speakerName: "Guard" }), dialogue("b", { speakerName: "Guard" })])
    ).toEqual(["head", "member"]);
    expect(
      rolesOf([dialogue("a", { speakerName: "Guard" }), dialogue("b", { speakerName: "Maid" })])
    ).toEqual(["head", "head"]);
  ...
  it("never groups a real character with a bare name, even when the names would print the same", () => {
    ...
    expect(
      rolesOf([dialogue("a", { characterId: "c1" }), dialogue("b", { speakerName: "c1" })])
    ).toEqual(["head", "head"]);
    expect(
      rolesOf([dialogue("a", { speakerName: "c1" }), dialogue("b", { characterId: "c1" })])
    ).toEqual(["head", "head"]);
  });
  ...
  it("does not group across a container boundary — an option body's last line vs a same-speaker line outside", () => {
    ...
    const opt: StoryBlock = {
      ...
      payload: {
        action: "choiceOption",
        text: { textId: "opt-t", role: "choiceText", value: "pick" }
      }
    };
    const inside: StoryBlock = {
      id: "inside",
      kind: "nodeAction",
      parentId: "opt",
      childrenIds: [],
      payload: {
        action: "dialogue",
        characterId: "c1",
        text: { textId: "inside-t", role: "dialogue", value: "inside" }
      }
    };
    const outside: StoryBlock = {
      id: "outside",
      kind: "nodeAction",
      parentId: null,
      childrenIds: [],
      payload: {
        action: "dialogue",
        characterId: "c1",
        text: { textId: "outside-t", role: "dialogue", value: "outside" }
      }
    };
    const rows = annotateDialogueGroups(
      buildVisibleRows(scene([opt, inside, outside], ["opt", "outside"]), new Set())
    );
...
describe("buildVisibleRows line numbers", () => {
  ...
  it("numbers every row of the scene in document order", () => {
    ...
    expect(rows.map((row) => `${row.block.id}:${row.lineNumber}`)).toEqual([
      "n1:1",
      "g:2",
      "c1:3",
      "c2:4",
...
describe("filter then group", () => {
  it("keeps original line numbers and groups the survivors that filtering made adjacent", () => {
    ...
    const filtered = annotateDialogueGroups(
      visible.filter((row) => storyRowPassesFilter(row.block, dialogueOnly, () => null))
    );
...
describe("buildDialogueAppearances", () => {
  it("gives a dialogue its speaker's most recent enter/expression, resetting on exit", () => {
    ...
    const map = buildDialogueAppearances(
      scene(
        blocks,
        blocks.map((b) => b.id)
      )
  ...
  it("a move relocates the placement and becomes the new source, keeping the form", () => {
    ...
    expect(map.get("d2")).toMatchObject({
      position: "right",
      positionSourceId: "m",
      pose: "casual"
    });
  ...
  it("reads back a placement move on a never-shown speaker without inventing a shown avatar", () => {
    ...
    const appearance = buildDialogueAppearances(
      scene(
        blocks,
        blocks.map((b) => b.id)
      )
...
describe("planRowBackspaceReplacement", () => {
  ...
  function showRow(id: string, parentId: string | null = null): StoryBlock {
    return {
      ...
      parentId,
      childrenIds: [],
      payload: { action: "character", operation: "enter", characterId: "c1" }
    };
  }

  it("plans an in-place replacement for a single selected leaf action row", () => {
    const blocks = [narration("n1"), showRow("a1"), narration("n2")];
    expect(planRowBackspaceReplacement(scene(blocks, ["n1", "a1", "n2"]), ["a1"])).toEqual({
      replaceBlockId: "a1",
      target: { parentId: null, beforeBlockId: "a1" }
    });
  });
  ...
  it("keeps the row's parent and position, so the replacement lands where the row was", () => {
    ...
    expect(planRowBackspaceReplacement(scene(blocks, ["g"]), ["a1"])).toEqual({
      replaceBlockId: "a1",
      target: { parentId: "g", beforeBlockId: "a1" }
    });
  });
  ...
  it("treats a childless container as a leaf", () => {
    ...
    expect(planRowBackspaceReplacement(scene(blocks, ["g"]), ["g"])).toMatchObject({
      replaceBlockId: "g"
    });
  });
...
describe("planBlockGroupMove", () => {
  ...
  const flat = () =>
    scene([narration("a"), narration("b"), narration("c"), narration("d")], ["a", "b", "c", "d"]);
  ...
  it("carries the group into the container it is dropped on", () => {
    const live = scene(
      [narration("a"), narration("b"), group("g", ["g1"]), narration("g1", "g")],
      ["a", "b", "g"]
    );
  ...
  it("declines a drop inside a moving container - a block cannot be moved into itself", () => {
    const live = scene([narration("a"), group("g", ["g1"]), narration("g1", "g")], ["a", "g"]);
