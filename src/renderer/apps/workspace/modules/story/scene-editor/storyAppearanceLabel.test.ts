...
describe("storyAppearanceLabel — what an inline expression switch says", () => {
  it("names a preset character's pose, never its id", () => {
    expect(
      storyAppearanceLabel(selection({ characterId: "alice", pose: "pro5swd" }), appearanceName)
    ).toBe("平常");
  });
  ...
  it("reads a puppet's expression back as itself — there is no table to look it up in", () => {
    expect(
      storyAppearanceLabel(
        selection({ characterId: "puppet", puppetName: "angry_02" }),
        appearanceName
      )
    ).toBe("angry_02");
  });
  ...
  it("answers null rather than an id when the reference resolves to nothing", () => {
    ...
    expect(
      storyAppearanceLabel(selection({ characterId: "alice", pose: "pos-deleted" }), appearanceName)
    ).toBeNull();
    expect(
      storyAppearanceLabel(selection({ characterId: "nobody", pose: "pro5swd" }), appearanceName)
    ).toBeNull();
    expect(
      storyAppearanceLabel(
        selection({ characterId: "inko", tags: { mood: "gone" } }),
        appearanceName
      )
    ).toBeNull();
  });
  ...
  it("answers null — not an id — for a surface that has no lookup to resolve against", () => {
    expect(
      storyAppearanceLabel(selection({ characterId: "alice", pose: "pro5swd" }), undefined)
    ).toBeNull();
    // A puppet still reads back: its name never needed the lookup.
    expect(
      storyAppearanceLabel(selection({ characterId: "doll", puppetName: "angry_02" }), undefined)
    ).toBe("angry_02");
  });
