...
const LOOKUPS: StoryCommandLineLookups = {
  ...
  assetName: (id) =>
    [...CONTEXT.images, ...CONTEXT.audio, ...CONTEXT.videos].find((entry) => entry.id === id)
      ?.name ?? null,
  audioTrackName: (id) => CONTEXT.audioTracks.find((entry) => entry.id === id)?.name ?? null,
  appearanceName: (characterId, refId) =>
    CONTEXT.appearanceByCharacterId[characterId]?.find((ref) => ref.id === refId)?.name ?? null,
...
describe("projectStoryCommandLine", () => {
  ...
  it("names the appearance a row asks for, never its id", () => {
    ...
    expect(
      projectStoryCommandLine(build("/face Alice smile"), {
        ...LOOKUPS,
        appearanceName: () => null
      })?.source
    ).toBe("/face Alice");
  ...
  it("never prints an id", () => {
    ...
    const orphan = projectStoryCommandLine(build("/bg night"), {
      ...LOOKUPS,
      assetName: () => null
    });
  ...
  it("marks the editable values by where they sit", () => {
    ...
    const at = (edit: { span: { start: number; end: number } }) =>
      line!.source.slice(edit.span.start, edit.span.end);
  ...
  it("marks the character's face onto the name, and onto nothing else", () => {
    ...
    expect(line.ornaments).toEqual([
      { at: line.source.indexOf("Alice"), kind: "character", id: "c1" }
    ]);
  ...
  it("keeps the whole value clickable, unit and all", () => {
    ...
    expect(editable.map((part) => part.pieces.map((piece) => piece.text).join(""))).toEqual([
      "Alice",
      "fade",
      "1s"
    ]);
  ...
  it("prints the values alone when the author asked for it, and only the keys go", () => {
    ...
    const text = (hide: boolean) =>
      storyCommandLineParts(line.source, line.edits, hide)
        .flatMap((part) => part.pieces)
        .map((piece) => piece.text)
        .join("");
  ...
  it("keeps every value clickable with the keys hidden", () => {
    ...
    const line = projectStoryCommandLine(build("/hide Alice d=1"), LOOKUPS)!;
    const editable = storyCommandLineParts(line.source, line.edits, true).filter(
      (part) => part.edit
    );
    expect(editable.map((part) => part.pieces.map((piece) => piece.text).join(""))).toEqual([
      "Alice",
  ...
  it("keeps a bare flag when the keys are hidden — there is no value behind it", () => {
    ...
    const text = storyCommandLineParts(line.source, line.edits, true)
      .flatMap((part) => part.pieces)
      .map((piece) => piece.text)
      .join("");
  ...
  it("declares in the line that declares it, type and description and all", () => {
    ...
    expect(project("/local hp 100 desc='Player health'")).toBe(
      "/local hp default=100 type=number desc='Player health'"
    );
    ...
    i18nStore.setLocale("zh");
    expect(project("/local 早到分钟 7 desc=测试")).toBe(
      "/场景变量 早到分钟 初始值=7 类型=数字 说明=测试"
    );
    ...
    expect(comparable(build("/场景变量 早到分钟 初始值=7"))).toEqual(
      comparable(build("/local 早到分钟 7"))
    );
  });
  ...
  it("prints a retired scope's row with the verb that would have declared it, never its command id", () => {
    const saved = declarationRow({
      scope: "saved",
      name: "chapter",
      valueType: "number",
      defaultValue: 1
    });
    const global = declarationRow({
      scope: "persistent",
      name: "seenIntro",
      valueType: "boolean",
      defaultValue: false
    });
    expect(projectStoryCommandLine(saved, LOOKUPS)!.source).toBe(
      "/save chapter default=1 type=number"
    );
    expect(projectStoryCommandLine(global, LOOKUPS)!.source).toBe(
      "/global seenIntro default=false type=boolean"
    );
  ...
  it("names a registry-declared saved or persistent variable on a /set row", () => {
    const setSaved: StoryBlock = {
      ...
      payload: {
        action: "setVariable",
        target: { scope: "saved", variableId: "reg-affection" },
        value: 5
      }
    };
    const setPersistent: StoryBlock = {
      ...
      payload: {
        action: "setVariable",
        target: { scope: "persistent", variableId: "key_seen" },
        value: true
      }
    };
    const withRegistry: StoryCommandLineLookups = {
      ...
      projectVariableName: (scope, variableId) =>
        scope === "saved" && variableId === "reg-affection"
          ? "Affection"
          : scope === "persistent" && variableId === "key_seen"
            ? "Seen Intro"
            : null
    };
    expect(projectStoryCommandLine(setSaved, withRegistry)!.source).toBe("/set Affection 5");
    expect(projectStoryCommandLine(setPersistent, withRegistry)!.source).toBe(
      "/set 'Seen Intro' true"
    );
  ...
  it("retypes from the row the way the inspector retypes", () => {
    ...
    expect(
      projectStoryCommandLine({ ...block, payload: edit.apply("boolean") } as StoryBlock, LOOKUPS)!
        .source
  ...
  it("writes an edited value back where the line reads it from", () => {
    ...
    const edited = (source: string, current: string, next: string): string => {
      ...
      if (!edit) {
        throw new Error(
          `${source} has no editable value "${current}" (has ${line.edits.map((e) => e.value).join(", ")})`
        );
      }
      return projectStoryCommandLine(
        { ...block, payload: edit.apply(next) } as StoryBlock,
        LOOKUPS
      )!.source;
    };
    ...
    expect(edited("/font title color=#ffcc00", "#ffcc00", "#102030")).toBe(
      "/font title color=#102030"
    );
    ...
    expect(edited("/vignette opacity=0.45", "0.45", "0.9")).toBe(
      "/vignette d=0.3s hold=0.6s color=#000000 opacity=0.9"
    );
  });

  it("offers exactly the options the grammar declares, and no writer where nothing can be edited", () => {
    const controls = (source: string) =>
      projectStoryCommandLine(build(source), LOOKUPS)!.edits.map((edit) => edit.control);
    ...
    expect(
      transition.kind === "enum" && transition.options.map((option) => option.value)
    ).toContain("circle");
    ...
    expect(
      created.edits.filter((edit) => edit.control.kind === "choice").map((edit) => edit.value)
    ).toEqual(["i2"]);
  ...
  it("round-trips: what the row prints rebuilds the row", () => {
    for (const source of [
      ...
      expect(comparable(build(line!.source)), `${source} → ${line!.source}`).toEqual(
        comparable(first)
      );
    }
  });
