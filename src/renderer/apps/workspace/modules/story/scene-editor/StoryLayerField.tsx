...
import type { StoryDocument, StoryLayerRef, StorySceneId } from "@shared/types/story";
...
export function StoryLayerField(props: {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      >
        <TriggerIcon
          className={["h-3.5 w-3.5 shrink-0", unresolved ? "text-warning" : "text-fg-muted"].join(
            " "
          )}
        />
        <span className="truncate text-fg">
          {resolved.name || t("story.layerField.defaultName")}
        </span>
        {unresolved ? (
          <span
      ...
      {open ? (
        ...
        >
          <div className="max-h-72 overflow-auto p-1">
            ...
            {customLayers.length > 0 ? <div className="my-1 border-t border-edge-subtle" /> : null}
...
function LayerRow(props: { label: string; hint: string; active: boolean; onChoose: () => void }) {
  return (
    <button
      type="button"
      role="option"
