...
function choicesOf(
  entries: readonly { id: string; name: string }[] | readonly string[]
...
function positional(
  param: string,
  value: string | undefined | null,
  extra: Omit<Arg, "param" | "value" | "positional"> = {}
): Arg | null {
...
function writeLine(
  ...
  for (const entry of args) {
    ...
    source += quoteValue(
      entry.enum && param ? enumWord(param, entry.value) : entry.value,
      param?.greedy === true
    );
    ...
    if (control && entry.apply) {
      edits.push({
        ...
        control,
        value: entry.editValue ?? entry.value,
        apply: entry.apply
      });
    }
  }
 ...
 */
function patchTransform<P extends { transform?: StoryTransformRef }>(
  payload: P,
  patch: Partial<StoryTransformRef>
): P {
  return { ...payload, transform: { ...(payload.transform ?? {}), ...patch } };
}

function patchTransition<P extends { transition?: StoryTransitionRef }>(
  payload: P,
  patch: Partial<StoryTransitionRef>
): P {
...
function pickAsset(
  ...
  const assets =
    kind === "image" ? context?.images : kind === "audio" ? context?.audio : context?.videos;
...
const PLACEMENTS: ReadonlySet<StoryTransformPreset> = new Set<StoryTransformPreset>([
  "left",
  "center",
  "right"
]);
...
function transitionWord(
  kind: StoryTransitionRef["kind"] | undefined,
...
function appearanceWord(
  ...
  return tags.length === 1
    ? (lookups.appearanceName(characterId, tags[0]) ?? undefined)
    : undefined;
}
...
function appearanceChoice(
  ...
  const current =
    payload.pose ??
    (Object.values(payload.tags ?? {}).length === 1
      ? Object.values(payload.tags ?? {})[0]
      : undefined);
  return {
...
function characterSentence(
  ...
  const who = {
    ...
    ...(payload.characterId
      ? { ornament: { kind: "character" as const, id: payload.characterId } }
      : {})
  };
  ...
  const reveal = (direction: "reveal" | "conceal") =>
  ...
  switch (payload.operation) {
    ...
    case "setSkin":
      ...
      return {
        commandId,
        args: [positional("character", name, who), positional("name", payload.puppetName?.trim())]
      };
    case "setName":
      return {
        commandId,
        args: [positional("character", name, who), positional("name", payload.displayName)]
      };
    case "setParams": {
...
function audioSentence(
  ...
  const track =
    payload.audioTrackId && lookups.audioTrackName
      ? lookups.audioTrackName(payload.audioTrackId)
      : null;
  const fade = arg("fade", seconds(payload.fadeMs), {
    apply: (next) => ({ ...payload, fadeMs: msOf(next) })
  });
  const clip =
    pickAsset(payload, lookups, "audio", (next) => ({ ...payload, assetId: next })) ?? {};
  // The handle a control verb addresses: any sound already on stage, plus the reserved `bgm`.
  const handle =
    pickStageObject(lookups, "audio", name, (next) => ({ ...payload, objectName: next })) ?? {};
  ...
  const loop = arg("loop", booleanValue(payload.loop), {
    apply: (next) => ({ ...payload, loop: next === "true" })
  });
  switch (payload.operation) {
    case "setBgm":
      return {
    ...
    case "setVolume":
      return {
        commandId,
        args: [
          positional("target", name, handle),
          positional("volume", numberValue(payload.volume), volume),
          fade
        ]
      };
    case "setRate":
      return {
        ...
        args: [
          positional("target", name, handle),
          positional("rate", numberValue(payload.rate), {
            apply: (next) => ({ ...payload, rate: Number(next) })
          })
        ]
      };
    case "seekSound":
      return {
        ...
        args: [
          positional("target", name, handle),
          positional("time", seconds(payload.timeMs), {
            apply: (next) => ({ ...payload, timeMs: msOf(next) })
          })
        ]
      };
    case "stopSound":
...
function imageSentence(
  ...
  const swapAsset =
    pickAsset(payload, lookups, "image", (next) => ({
      ...payload,
      assetId: next,
      color: undefined
    })) ?? {};
  const object =
    pickStageObject(lookups, "image", name, (next) => ({ ...payload, objectName: next })) ?? {};
  if (payload.operation === "create") {
    // The NAME here is the one later rows address, so it is not offered; the asset is.
    return {
      commandId,
      args: [
        positional("image", asset ?? payload.color, swapAsset),
        arg("name", name),
        placement,
        reveal("reveal"),
        duration
  ...
  return {
    ...
    args: [
      positional("target", name, object),
      reveal(payload.operation === "show" ? "reveal" : "conceal"),
      duration
    ]
  };
}
...
function textSentence(
  ...
  const name = payload.objectName?.trim() || undefined;
  const object =
    pickStageObject(lookups, "text", name, (next) => ({ ...payload, objectName: next })) ?? {};
  switch (payload.operation) {
    case "create":
    ...
    case "setText":
      return {
        commandId,
        args: [positional("target", name, object), positional("content", payload.text)]
      };
    case "setFontSize":
      return {
        commandId,
        args: [
          positional("target", name, object),
          positional("size", numberValue(payload.fontSize), {
            apply: (next) => ({ ...payload, fontSize: Number(next) })
          })
        ]
      };
    case "setFontColor":
      return {
        ...
        args: [
          positional("target", name, object),
          arg("color", payload.fontColor, { apply: (next) => ({ ...payload, fontColor: next }) })
        ]
      };
    case "show":
    case "hide": {
      ...
      return {
        ...
        args: [
          ...
          arg("t", revealWord(payload.transform, direction), {
            enum: true,
            apply: (next) =>
              patchTransform(payload, { preset: transformPresetFor(direction, next) })
          }),
...
function layerSentence(
  ...
  const name =
    payload.operation === "create"
      ...
      : resolveStoryLayerRef(
          lookups.scene,
          layerActionTargetRef(payload.target, payload.objectName)
        ).name || undefined;
  if (payload.operation === "create" || payload.operation === "setZIndex") {
    return {
      ...
      args: [
        ...
        arg("z", numberValue(payload.zIndex), {
          apply: (next) => ({ ...payload, zIndex: Number(next) })
        })
      ]
    };
  }
...
function videoSentence(
  ...
  const name = payload.objectName?.trim() || undefined;
  const object =
    pickStageObject(lookups, "video", name, (next) => ({ ...payload, objectName: next })) ?? {};
  if (payload.operation === "create") {
    return {
      commandId,
      args: [
        positional(
          "video",
          assetWord(lookups, payload.assetId),
          pickAsset(payload, lookups, "video", (next) => ({ ...payload, assetId: next })) ?? {}
        ),
        arg("name", name),
        arg("muted", booleanValue(payload.muted), {
          apply: (next) => ({ ...payload, muted: next === "true" })
        })
      ]
    };
  }
...
function vfxSentence(
  ...
  const name = payload.objectName?.trim() || undefined;
  const duration = arg("d", seconds(payload.durationMs), {
    apply: (next) => ({ ...payload, durationMs: msOf(next) })
  });
  if (payload.operation === "create") {
    return {
      commandId,
      args: [
        ...
        arg("name", name),
        arg("opacity", numberValue(payload.opacity), {
          apply: (next) => ({ ...payload, opacity: Number(next) })
        }),
        duration
      ]
    };
  ...
  const object =
    pickStageObject(lookups, "vfx", name, (next) => ({ ...payload, objectName: next })) ?? {};
...
function cameraSentence(
  ...
  const amount = (): string | undefined => {
    switch (payload.operation) {
      case "pan":
        return storyCameraPanPlacement(payload.position) ?? undefined;
      case "zoom":
        return numberValue(payload.zoom);
      case "rotate":
        return numberValue(payload.rotation);
      case "darken":
        return numberValue(payload.darkness);
  ...
  const applyAmount = (next: string): StoryBlock["payload"] => {
    switch (payload.operation) {
      case "pan":
        return { ...payload, position: getPresetPosition(next, {}) ?? payload.position };
      case "zoom":
        return { ...payload, zoom: Number(next) };
      case "rotate":
        return { ...payload, rotation: Number(next) };
      case "darken":
        return { ...payload, darkness: Number(next) };
      case "motion":
      case "reset":
        return payload;
    }
  ...
  return {
    ...
    args: [
      ...
      payload.operation === "motion"
        ? null
        : arg("d", seconds(payload.durationMs), {
            apply: (next) => ({ ...payload, durationMs: msOf(next) })
          })
    ]
  };
    ...
    case "setBackground":
      return {
        ...
        args: [
          positional(
            "image",
            assetWord(lookups, payload.assetId) ?? payload.color,
          ...
          arg("t", transitionWord(payload.transition?.kind, "scene"), {
            enum: true,
            apply: (next) =>
              patchTransition(payload, { kind: transitionKindFor("scene", next) ?? "dissolve" })
          }),
    ...
    case "screenEffect":
      return {
        ...
        args: [
          ...
          arg("hold", seconds(payload.holdMs), {
      ...
      return {
        ...
        args: [
          arg("t", revealWord(payload.transition, "nvl"), {
            ...
            apply: (next) => ({
              ...payload,
              transition: { ...payload.transition, preset: transformPresetFor("nvl", next) }
            })
          }),
          arg("d", seconds(payload.transition?.durationMs), {
            apply: (next) => ({
              ...payload,
              transition: { ...payload.transition, durationMs: msOf(next) }
            })
          })
        ]
      };
...
function blockSentence(block: StoryBlock, lookups: StoryCommandLineLookups): Sentence | null {
  ...
  if (block.kind === "control") {
    ...
    if (block.payload.control === "cut") {
      ...
      const tags = (lookups.commandContext?.appTags ?? []).filter(
        (tag) => tag.id !== APP_TAG_ID_RELEASE
      );
 ...
 */
export function projectStoryCommandLine(
  block: StoryBlock,
  lookups: StoryCommandLineLookups
  ...
  const verb = def
    ? localizedCommandToken(def)
    : (retiredCommandToken(sentence.commandId) ?? sentence.commandId);
