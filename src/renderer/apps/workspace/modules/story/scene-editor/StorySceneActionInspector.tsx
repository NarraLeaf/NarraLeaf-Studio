...
const TEXTAREA_CLASS =
  "w-full resize-none rounded-md border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-muted outline-none transition-colors focus:border-primary/50 disabled:cursor-not-allowed disabled:opacity-50";
 ...
 */
function useStoryVariableOptions(
  document: StoryDocument,
  sceneId: StorySceneId
): StoryVariableOptions {
...
function VariableValueField(props: {
  ...
  if (props.valueType === "json") {
    const text =
      typeof props.value === "string" ? props.value : JSON.stringify(props.value ?? null);
    return (
      <LabeledTextarea
          ...
          <Section title={t("storyInspector.section.timing")}>
            <FieldGrid cols={2}>
              <CheckboxField
                ...
                onChange={(checked) =>
                  props.onUpdatePayload({ ...payload, pauseAfter: checked ? true : undefined })
                }
              />
              {pauseEnabled ? (
                <SecondsField
                  ...
                  value={pauseMs}
                  onChange={(ms) =>
                    props.onUpdatePayload({ ...payload, pauseAfter: ms === undefined ? true : ms })
                  }
                />
              ) : null}
            </FieldGrid>
            ...
            <div className="grid grid-cols-1 gap-2">
              <div>
                <div className={FIELD_LABEL_CLASS}>
                  {t("storyInspector.choiceOption.hiddenWhen")}
                </div>
                <ConditionEditor
                  document={props.document}
                  sceneId={props.sceneId}
              ...
              <div>
                <div className={FIELD_LABEL_CLASS}>
                  {t("storyInspector.choiceOption.disabledWhen")}
                </div>
                <ConditionEditor
                  document={props.document}
                  sceneId={props.sceneId}
      ...
      <div className="grid grid-cols-1 gap-3">
        <div className="max-w-sm">
          <SelectField
            ...
            onChange={(targetSceneId) =>
              props.onUpdatePayload({ ...payload, targetSceneId: String(targetSceneId) })
            }
          />
        </div>
        <TransitionEditor
    ...
    <div className="nl-field-grid">
      ...
      <SelectField
        ...
        onChange={(value) => {
          ...
          props.onChange({
            ...payload,
            valueType,
            defaultValue: declarationDefaultForType(valueType)
          });
        }}
      />
      <VariableValueField
      ...
      <TextField
        ...
        onChange={(description) =>
          props.onChange({ ...payload, description: description || undefined })
        }
      />
    </div>
  );
      ...
      openBlueprint(
        {
          ...
          title: t("storyInspector.blueprint.storyActionTitle")
        },
        options
      );
    },
...
function ActionPayloadFields(props: {
  ...
  if (payload.action === "setBackground") {
    return <BackgroundActionEditor payload={payload} onChange={props.onChange} />;
  }
    ...
    return (
      <div className="nl-field-grid nl-field-grid-2">
        ...
        <SecondsField
          ...
          onChange={(durationMs) => props.onChange({ ...payload, durationMs })}
        />
      </div>
    );
  }
    ...
    return (
      <div className="grid grid-cols-1 gap-3">
        <div className="nl-field-grid">
          ...
          />
          <TextField
            label={t("storyInspector.image.imageName")}
          ...
          />
          <CheckboxField
            label={t("storyInspector.image.autoFit")}
            checked={Boolean(payload.autoFit)}
            onChange={(autoFit) => props.onChange({ ...payload, autoFit })}
          />
        </div>
  ...
  if (payload.action === "displayable") {
    ...
    const resolvedTarget = resolveDisplayableTargetRef(
      props.document.scenes[props.sceneId],
      payload.target
    );
    return (
        ...
        <div className="nl-field-grid">
          <SelectField
            ...
            onChange={(operation) =>
              props.onChange({
                ...payload,
                operation: operation as Extract<StoryActionPayload, { action: "text" }>["operation"]
              })
            }
          />
          <TextField
            label={t("storyInspector.text.textName")}
            value={payload.objectName}
            onChange={(objectName) => props.onChange({ ...payload, objectName })}
          />
          <StoryLayerField
          ...
          />
          <NumberField
            label={t("storyInspector.text.fontSize")}
            value={payload.fontSize}
            onChange={(fontSize) => props.onChange({ ...payload, fontSize })}
          />
          <ColorTextField
            label={t("storyInspector.text.fontColor")}
            value={payload.fontColor ?? "#ffffff"}
            onChange={(fontColor) => props.onChange({ ...payload, fontColor })}
          />
        </div>
        {payload.operation === "create" || payload.operation === "setText" ? (
          <LabeledTextarea
            label={t("storyInspector.text.text")}
            className="min-h-16"
            value={payload.text ?? ""}
            onChange={(text) => props.onChange({ ...payload, text })}
          />
        ) : null}
        <TransformPresetEditor
          value={payload.transform}
  ...
  if (payload.action === "layer") {
    ...
    const layerName = isCreate
      ...
      : resolveStoryLayerRef(props.document.scenes[props.sceneId], layerRefValue).name ||
        t("storyInspector.motionTarget.layer");
    return (
      <div className="grid grid-cols-1 gap-3">
        <div className="nl-field-grid">
          <SelectField
          ...
          {isCreate ? (
            <TextField
              label={t("storyInspector.layer.layerName")}
              value={payload.objectName}
              onChange={(objectName) => props.onChange({ ...payload, objectName })}
            />
          ) : (
          ...
          {showZIndex ? (
            <NumberField
              label={t("storyInspector.layer.zIndex")}
      ...
      <div className="nl-field-grid">
        <SelectField
          ...
          onChange={(operation) =>
            props.onChange({
              ...payload,
              operation: operation as Extract<StoryActionPayload, { action: "video" }>["operation"]
            })
          }
        />
        <TextField
          label={t("storyInspector.video.videoName")}
          value={payload.objectName}
          onChange={(objectName) => props.onChange({ ...payload, objectName })}
        />
        <AssetField
        ...
        {payload.operation === "seek" ? (
          <SecondsField
            ...
            onChange={(timeMs) =>
              props.onChange({
                ...payload,
                timeMs: timeMs === undefined ? undefined : Math.max(0, timeMs)
              })
            }
          />
        ) : null}
      ...
      <div className="nl-field-grid">
        <SelectField
          ...
          onChange={(effect) =>
            props.onChange({
              ...payload,
              effect: effect as Extract<StoryActionPayload, { action: "screenEffect" }>["effect"]
            })
          }
        />
        <SecondsField
        ...
        <SecondsField
          ...
          onChange={(holdMs) => props.onChange({ ...payload, holdMs })}
        />
        <ColorTextField
          label={t("storyInspector.field.color")}
          value={payload.color ?? "#000000"}
          onChange={(color) => props.onChange({ ...payload, color })}
        />
        <NumberField
          label={t("storyInspector.field.opacity")}
          value={payload.opacity}
          onChange={(opacity) => props.onChange({ ...payload, opacity })}
        />
        <SelectField
...
function* sceneBlocksInOrder(
  scene: StoryScene,
  blockIds: readonly StoryBlockId[]
): Generator<StoryBlock> {
  ...
  if (scene) {
    ...
    for (const block of sceneBlocksInOrder(scene, scene.rootBlockIds)) {
      ...
      return block.id === blockId
        ...
        : resolveAudioTrack(
            tracks,
            creator.audioTrackId,
            audioRowFallbackChannel(creator.operation)
          );
    }
  }
...
function AudioActionEditor(props: {
  ...
  const status = audioBusStatusLine(
    t,
    tracks,
    track.id,
    audioRowFallbackChannel(payload.operation)
  );
    ...
    <div className="grid grid-cols-1 gap-3">
      <div className="nl-field-grid">
        <SelectField
          ...
          value={payload.operation}
          onChange={(operation) =>
            props.onChange({ ...payload, operation: operation as AudioActionPayload["operation"] })
          }
        />
        {has("track") ? (
          <SelectField
            ...
            onChange={(value) =>
              props.onChange({ ...payload, audioTrackId: String(value) || undefined })
            }
          />
        ) : null}
        {has("name") ? (
        ...
        {has("asset") ? (
          <AssetField
            label={
              payload.operation === "setBgm"
                ? t("storyInspector.audio.bgmAsset")
                : t("storyInspector.audio.soundAsset")
            }
        ...
        {has("fade") ? (
          <SecondsField
            ...
            onChange={(fadeMs) =>
              props.onChange({
                ...payload,
                fadeMs: fadeMs === undefined ? undefined : Math.max(0, fadeMs)
              })
            }
          />
        ) : null}
        ...
        {has("rate") ? (
          <NumberField
            ...
            onChange={(rate) =>
              props.onChange({
                ...payload,
                rate: rate === undefined ? undefined : Math.max(0, rate)
              })
            }
          />
        ) : null}
 ...
 *    pipeline to offer.
 */
function VfxActionEditor(props: {
  payload: VfxActionPayload;
  onChange: (payload: StoryBlock["payload"]) => void;
}) {
  const { t } = useTranslation();
  const payload = props.payload;
    ...
    <Section title={t("storyInspector.section.vfx")}>
      <FieldGrid cols={2}>
        <SelectField
          ...
          value={payload.operation}
          onChange={(operation) =>
            props.onChange({ ...payload, operation: operation as VfxActionPayload["operation"] })
          }
        />
        <TextField
          ...
          <>
            ...
            <SelectField
              ...
              onChange={(blendMode) =>
                props.onChange({
                  ...payload,
                  blendMode: blendMode as VfxActionPayload["blendMode"]
                })
              }
            />
            <NumberField
            ...
            <SelectField
              ...
              onChange={(fit) =>
                props.onChange({ ...payload, fit: fit as VfxActionPayload["fit"] })
              }
            />
            <NumberField
        ...
        {fades ? (
          <SelectField
            ...
            value={payload.easing ?? ""}
            onChange={(easing) =>
              props.onChange({ ...payload, easing: String(easing) || undefined })
            }
          />
        ) : null}
      </FieldGrid>
        ...
        const options: SelectOption[] | undefined =
          specs.length > 0
            ? [
                ...
                ...specs
                  .filter((entry) => entry.id === id || !(entry.id in props.params))
...
function CharacterActionEditor(props: {
  ...
  const updateCharacter = useCallback(
    (characterIdValue: string | number) => {
      ...
      const autofill =
        !payload.objectName ||
        payload.objectName === previousName ||
        payload.objectName === payload.characterId;
      const objectName = autofill
        ? (nextCharacter?.profile.getName() ?? payload.objectName)
        : payload.objectName;
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        {selectedCharacter && selectedCharacter.profile.appearance.getKind() === "puppet" ? (
          ...
          <div className="text-xs text-fg-subtle">
            {t("storyInspector.character.notPuppetHint")}
          </div>
        ) : (
  ...
  return (
    <div className="grid grid-cols-1 gap-3">
      ...
      {selectedCharacter &&
      isPuppetAppearanceKind(selectedCharacter.profile.appearance.getKind()) ? (
...
function getTransformNumberProp(
...
export function AssetField(props: {
  ...
  const selectedAsset = props.assetId
    ? ((assetsService?.getAssets()[props.assetType] as Record<string, Asset> | undefined)?.[
        props.assetId
      ] ?? null)
    : null;
    ...
    <div>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          data-tip={t("storyInspector.asset.clear")}
          aria-label={t("storyInspector.asset.clear")}
  ...
  return (
    <Section title={t("storyInspector.section.effect")}>
      <FieldGrid cols={3}>
        ...
        {op === "mask" ? (
          <AssetField
            label={t("storyInspector.displayableEffect.maskImage")}
            assetType={AssetType.Image}
            assetId={payload.maskAssetId}
            onChange={(maskAssetId) => props.onChange({ ...payload, maskAssetId })}
          />
        ) : null}
        {op === "clip" ? (
          <TextField
            label={t("storyInspector.displayableEffect.clipPath")}
            value={payload.clipPath ?? ""}
            onChange={(clipPath) => props.onChange({ ...payload, clipPath: clipPath || undefined })}
          />
        ) : null}
        {op === "filter" ? (
          <TextField
            ...
            onChange={(filter) => props.onChange({ ...payload, filter: filter || undefined })}
          />
        ) : null}
        {op === "backdrop" ? (
          ...
          <TextField
            ...
            onChange={(backdropFilter) =>
              props.onChange({ ...payload, backdropFilter: backdropFilter || undefined })
            }
          />
        ) : null}
        {op === "blend" ? (
          ...
          <SelectField
            ...
            onChange={(mixBlendMode) =>
              props.onChange({ ...payload, mixBlendMode: mixBlendMode as StoryVfxBlendMode })
            }
          />
        ) : null}
        {op === "darken" ? (
          <NumberField
            ...
            onChange={(darkness) => props.onChange({ ...payload, darkness })}
          />
        ) : null}
        {op === "circleReveal" || op === "circleClose" ? (
          <>
            <TextField
              label={t("storyInspector.field.center")}
              value={paramString(payload.effectProps, "center", "50% 50%")}
              onChange={(center) => setEffectParam({ center: center || undefined })}
            />
            <NumberField
              label={t("storyInspector.field.fromRadius")}
              value={paramNumber(payload.effectProps, "from")}
              onChange={(from) => setEffectParam({ from })}
            />
            <NumberField
              label={t("storyInspector.field.toRadius")}
              value={paramNumber(payload.effectProps, "to")}
              onChange={(to) => setEffectParam({ to })}
            />
          </>
        ) : null}
        {op === "wipe" ? (
          <>
            ...
            <CheckboxField
              ...
              checked={paramBool(payload.effectProps, "reverse")}
              onChange={(reverse) => setEffectParam({ reverse: reverse || undefined })}
            />
          </>
        ) : null}
      </FieldGrid>
        ...
        <div className="grid grid-cols-1 gap-2">
          <FieldGrid cols={3}>
            <SelectField
              ...
              onChange={(preset) =>
                props.onChange({ ...value, mode: "preset", preset: preset as StoryTransformPreset })
              }
            />
            <SecondsField
              label={t("storyInspector.field.duration")}
            ...
            <SelectField
              ...
              value={value.easing ?? ""}
              onChange={(easing) =>
                props.onChange({ ...value, easing: String(easing) || undefined })
              }
            />
          </FieldGrid>
          <FieldGrid cols={3}>
            <NumberField
              ...
              onChange={(zoom) =>
                props.onChange(
                  setTransformNumberProp(value, "zoom", zoom, { preset: value.preset ?? "none" })
                )
              }
            />
            <NumberField
              ...
              onChange={(xoffset) =>
                props.onChange(
                  setTransformNumberProp(value, "xoffset", xoffset, {
                    preset: value.preset ?? "none"
                  })
                )
              }
            ...
            <NumberField
              ...
              onChange={(yoffset) =>
                props.onChange(
                  setTransformNumberProp(value, "yoffset", yoffset, {
                    preset: value.preset ?? "none"
                  })
                )
              }
          ...
          <Disclosure title={t("storyInspector.advancedParams")}>
            <TextField
              ...
              value={propsText}
              onChange={(nextProps) =>
                props.onChange({ ...value, props: parsePropsText(nextProps) })
              }
            />
          </Disclosure>
        </div>
...
function TransitionEditor(props: {
  ...
  const setBase = (patch: Partial<StoryTransitionRef>) =>
    props.onChange({ ...value, kind: realKind, ...patch });
    ...
    <Section title={t("storyInspector.section.transition")}>
      <FieldGrid cols={4}>
        ...
        {kind === "none" ? null : (
          <>
            <SecondsField
              label={t("storyInspector.field.duration")}
              value={value.durationMs}
              onChange={(durationMs) => setBase({ durationMs })}
            />
            <SelectField
              label={t("storyInspector.field.easing")}
              options={easingOptions(t)}
        ...
        {kind === "fadeIn" ? (
          <>
            <NumberField
              label={t("storyInspector.transition.startX")}
              value={paramNumber(value.props, "x")}
              onChange={(x) => setParam({ x })}
            />
            <NumberField
              label={t("storyInspector.transition.startY")}
              value={paramNumber(value.props, "y")}
              onChange={(y) => setParam({ y })}
            />
          </>
        ) : null}
        {kind === "maskCircle" ? (
          ...
          <TextField
            label={t("storyInspector.field.center")}
            value={paramString(value.props, "center", "50% 50%")}
            onChange={(center) => setParam({ center: center || undefined })}
          />
        ) : null}
        {kind === "maskWipe" ? (
          <SelectField
        ...
        {kind === "softWipe" ? (
          <>
            ...
            />
            <NumberField
              label={t("storyInspector.field.feather")}
              value={paramNumber(value.props, "feather")}
              onChange={(feather) => setParam({ feather })}
            />
          </>
        ) : null}
        {kind === "blinds" ? (
          <>
            ...
            />
            <NumberField
              label={t("storyInspector.field.slats")}
              value={paramNumber(value.props, "slats")}
              onChange={(slats) => setParam({ slats })}
            />
          </>
        ) : null}
        {kind === "slide" ? (
        ...
        {kind === "clock" ? (
          <>
            ...
            />
            <NumberField
              label={t("storyInspector.field.fromAngle")}
              value={paramNumber(value.props, "from")}
              onChange={(from) => setParam({ from })}
            />
            <NumberField
              label={t("storyInspector.field.feather")}
              value={paramNumber(value.props, "feather")}
        ...
        {kind === "fan" ? (
          <>
            <NumberField
              label={t("storyInspector.field.blades")}
              value={paramNumber(value.props, "blades")}
              onChange={(blades) => setParam({ blades })}
            />
            <TextField
              label={t("storyInspector.field.center")}
              value={paramString(value.props, "center", "50% 50%")}
        ...
        {kind === "dots" ? (
          <>
            <NumberField
              label={t("storyInspector.field.rows")}
              value={paramNumber(value.props, "rows")}
              onChange={(rows) => setParam({ rows })}
            />
            <NumberField
              label={t("storyInspector.field.cols")}
              value={paramNumber(value.props, "cols")}
              onChange={(cols) => setParam({ cols })}
            />
            <NumberField
              label={t("storyInspector.field.feather")}
              value={paramNumber(value.props, "feather")}
            ...
            />
            <NumberField
              label={t("storyInspector.field.stagger")}
              value={paramNumber(value.props, "stagger")}
              onChange={(stagger) => setParam({ stagger })}
            />
          </>
        ) : null}
        {kind === "blurDissolve" ? (
          <NumberField
            label={t("storyInspector.transition.blurPx")}
            value={paramNumber(value.props, "blur")}
            onChange={(blur) => setParam({ blur })}
          />
        ) : null}
        {kind === "throughColor" ? (
          <>
            ...
            <ColorTextField
              label={t("storyInspector.field.color")}
              value={paramString(value.props, "color", "#000000")}
              onChange={(color) => setParam({ color })}
            />
            <NumberField
              label={t("storyInspector.transition.holdPct")}
              value={paramNumber(value.props, "hold")}
        ...
        {kind === "darkness" ? (
          ...
          <>
            <NumberField
              label={t("storyInspector.transition.darknessFrom")}
              value={paramNumber(value.props, "from")}
              onChange={(from) => setParam({ from })}
            />
            <NumberField
              label={t("storyInspector.transition.darknessTo")}
              value={paramNumber(value.props, "to")}
              onChange={(to) => setParam({ to })}
            />
          </>
        ) : null}
      </FieldGrid>
...
function BackgroundActionEditor(props: {
  ...
  const [mode, setMode] = useState<"image" | "color">(() =>
    props.payload.assetId ? "image" : "color"
  );
  const [selectorOpen, setSelectorOpen] = useState(false);
  ...
  const handleSelectImage = useCallback((assets: Asset[]) => {
    const selected = assets[0];
    if (!selected) {
      return;
    }
  ...
  const handleColorChange = useCallback((colorValue: ColorValue) => {
    latestOnChangeRef.current({
      ...latestPayloadRef.current,
      assetId: undefined,
  ...
  const imageLabel =
    selectedAsset?.name ??
    (props.payload.assetId
      ? t("storyInspector.background.missing")
      : t("storyInspector.background.none"));
          ...
          <div className="flex min-w-0 flex-col gap-2">
            <div>
              ...
              <div className="flex h-9 min-h-[34px] min-w-0 items-center rounded-md border border-edge bg-surface-raised px-3 text-sm text-fg-muted">
                <span
                  className={["truncate", selectedAsset ? "" : "italic text-fg-subtle"].join(" ")}
                >
                  {imageLabel}
                </span>
              </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                data-tip={t("storyInspector.background.clearImage")}
                aria-label={t("storyInspector.background.clearImage")}
              >
                <Trash2 className="h-3.5 w-3.5" />
              </button>
            </div>
...
function ControlPayloadFields(props: {
  document: StoryDocument;
  sceneId: StorySceneId;
  payload: StoryControlPayload;
  onChange: (payload: StoryBlock["payload"]) => void;
}) {
  const { t } = useTranslation();
  if (props.payload.control === "condition") {
    return (
      <div className="text-sm text-fg-muted">{t("storyInspector.control.conditionContainer")}</div>
    );
  }
      ...
      <div className="max-w-sm">
        <SelectField
          ...
          options={
            options.length > 0
              ? options
              : [{ value: "", label: t("storyInspector.control.noLabels") }]
          }
          value={gotoPayload.targetLabel}
          onChange={(targetLabel) =>
            props.onChange({ ...gotoPayload, targetLabel: String(targetLabel) })
          }
        />
      </div>
    );
  ...
  if (props.payload.control !== "conditionBranch") {
    const groupPayload = props.payload as Extract<
      StoryControlPayload,
        ...
        <div className="nl-field-grid">
          <SelectField
            ...
            onChange={(control) => {
              ...
              props.onChange(
                next === "repeat"
                  ? { ...groupPayload, control: next }
                  : { ...groupPayload, control: next, until: undefined }
              );
            }}
          />
          <SelectField
            ...
            onChange={(mode) =>
              props.onChange({
                ...groupPayload,
                mode: mode as "do" | "doAsync" | "all" | "allAsync" | "any"
              })
            }
          />
          {isRepeat ? (
                ...
                props.onChange(
                  kind === "until"
                    ? {
                        ...groupPayload,
                        times: undefined,
                        until: groupPayload.until ?? EMPTY_EXPRESSION_CONDITION
                      }
          ...
          {isRepeat && !conditional ? (
            <NumberField
              label={t("storyInspector.control.times")}
              value={groupPayload.times}
              onChange={(times) => props.onChange({ ...groupPayload, times })}
            />
          ) : null}
        </div>
        {conditional ? (
          <div className="flex flex-col gap-2">
            ...
            <ConditionEditor
              ...
              onChange={(until) =>
                props.onChange({ ...groupPayload, until: until ?? EMPTY_EXPRESSION_CONDITION })
              }
            />
  ...
  return (
    <div className="grid grid-cols-1 gap-3">
      <SelectField
        ...
        onChange={(branch) =>
          props.onChange({ ...branchPayload, branch: branch as "if" | "elseIf" | "else" })
        }
      />
      {branchPayload.branch !== "else" ? (
        <div className="flex flex-col gap-2">
          ...
          !isStoryExpressionEvaluable(branchPayload.condition.expression.ast) ? (
    ...
    <div className="flex max-w-sm flex-col gap-2">
      <SelectField
        ...
        options={
          options.length > 0
            ? options
            : [{ value: "", label: t("storyInspector.control.cutNoVariants") }]
        }
        value={props.payload.appTagId}
    ...
    <div>
      ...
      {open && (
        <div className="mt-1 flex items-center gap-2 rounded-md border border-edge bg-surface-raised px-2.5 py-1.5">
          <span
            className="min-w-0 flex-1 truncate font-mono text-2xs text-fg-muted"
            data-tip={props.text.textId}
          >
            {props.text.textId}
          </span>
          <button
...
function paramNumber(
  props: Record<string, StoryLiteralValue> | undefined,
  key: string
): number | undefined {
