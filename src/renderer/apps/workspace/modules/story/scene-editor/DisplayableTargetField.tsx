...
import {
  ...
  type SceneDisplayableRef
} from "../../story-motion/storyMotionPreviewTarget";
...
const BUILTIN_ORDER: StoryDisplayableBuiltin[] = [
  "background",
  "backgroundLayer",
  "displayableLayer"
];
...
function matchesTarget(
  ...
  target: { name: string; kind?: string; sourceBlockId?: string }
): boolean {
  ...
  return (
    sameName(option.name, target.name ?? "") &&
    (target.kind === undefined || option.kind === target.kind)
  );
}
          ...
          <>
            <TriggerIcon
              className={[
                ...
                unresolved ? "text-warning" : "text-fg-muted"
              ].join(" ")}
            />
            ...
            {unresolved ? (
              ...
              <span className="shrink-0 text-2xs text-fg-subtle">
                {target.builtin
                  ? t("story.stage.builtin")
                  : t(`story.targetField.kind.${displayKind}`)}
              </span>
            )}
          </>
        ) : (
          <span className="truncate italic text-fg-subtle">
            {t("story.targetField.placeholder")}
          </span>
        )}
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate text-2xs text-fg-subtle">
          {t(`story.targetField.kind.${props.option.kind}`)}
          {preview ? ` · ${preview}` : ""}
        </span>
      </span>
      {props.active ? <Check className="h-3.5 w-3.5 shrink-0 text-primary" /> : null}
...
export function useAutoMenuPlacement(
