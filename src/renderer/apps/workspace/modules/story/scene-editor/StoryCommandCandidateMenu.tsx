  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => {
      const active = listRef.current?.querySelector(
        `[data-candidate-key="${CSS.escape(props.activeKey ?? "")}"]`
      );
      ...
      {props.items.length === 0 ? (
        ...
        props.items.map((item) => {
          ...
          return (
            ...
            >
              {item.mark ? (
                <StoryCandidateMarkView mark={item.mark} characters={props.characters} />
              ) : null}
              <span className="truncate text-sm text-fg">{item.label}</span>
              {item.detail ? (
                <span className="shrink-0 text-2xs text-fg-subtle">{item.detail}</span>
              ) : null}
              {item.tag ? (
                <span className="ml-auto shrink-0 text-2xs text-fg-subtle">{item.tag}</span>
              ) : null}
            </button>
          );
        })
