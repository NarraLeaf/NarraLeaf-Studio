...
export const bg = defineStoryCommand({
  ...
  params: {
    ...
    t: {
      aliases: ["transition"],
      hint: "transition",
      type: { kind: "enum", options: transitionOptions("scene") }
    },
    d: secondsParam()
  },
  build(args, ctx) {
...
export const wait = defineStoryCommand({
  ...
  params: {
    ...
    seconds: {
      hint: "waitFor",
      type: [{ kind: "keyword", value: "click" }, SECONDS_TYPE],
      positional: true
    }
  },
  build(args, ctx) {
    ...
    if (seconds !== undefined) {
      return {
        ...block,
        payload: { action: "wait", mode: "duration", durationMs: storySecondsToMs(seconds) }
      };
    }
...
export const nvl = defineStoryCommand({
  ...
  params: {
    t: {
      aliases: ["transition"],
      hint: "transition",
      type: { kind: "enum", options: transitionOptions("nvl") }
    },
    d: secondsParam()
  },
  build(args, ctx) {
