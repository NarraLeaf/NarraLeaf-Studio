...
function audioControlBlock(
  ...
  generateId: () => string,
  write?: (
    payload: Extract<Extract<StoryBlock, { kind: "action" }>["payload"], { action: "audio" }>
  ) => void
): StoryBlock {
...
function mediaControlBlock(
  ...
  ops: {
    video?: Extract<
      Extract<StoryBlock, { kind: "action" }>["payload"],
      { action: "video" }
    >["operation"];
    vfx?: Exclude<
      Extract<Extract<StoryBlock, { kind: "action" }>["payload"], { action: "vfx" }>["operation"],
      "create"
    >;
  },
  args: { readonly target?: StoryCommandValue; readonly fade?: StoryCommandValue },
...
export const bgm = defineStoryCommand({
  ...
  params: {
    audio: {
      aliases: ["src"],
      hint: "audioAsset",
      type: { kind: "asset", assetType: "audio" },
      positional: true,
      core: true
    },
...
export const vol = defineStoryCommand({
  ...
  build: (args, ctx) =>
    audioControlBlock("soundVolume", args, ctx.generateId, (payload) => {
...
export const resume = defineStoryCommand({
  ...
  build: (args, ctx) =>
    mediaControlBlock("resumeSound", { video: "resume", vfx: "resume" }, args, ctx)
});
...
export const mute = defineStoryCommand({
  ...
  build: (args, ctx) =>
    audioControlBlock("muteSound", args, ctx.generateId, (payload) => {
      payload.muted = true;
    })
});
