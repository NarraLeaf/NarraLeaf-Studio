...
const VARIABLE: StoryCommandParamSpec = {
  hint: "variable",
  type: { kind: "variable" },
  positional: true,
  core: true
...
type SetVariableBlock = Extract<StoryBlock, { kind: "action" }> & {
  payload: Extract<StoryActionPayload, { action: "setVariable" }>;
};

function setVariableBase(
  generateId: () => string,
  variable: StoryCommandValue | undefined
): {
  block: StoryBlock;
  ...
  if (
    block.kind !== "action" ||
    block.payload.action !== "setVariable" ||
...
export const set = defineStoryCommand({
  ...
  params: {
    variable: VARIABLE,
    value: {
      hint: "expressionValue",
      type: { kind: "expression", assignTo: "variable" },
      positional: true,
      greedy: true,
      core: true
    }
...
function buildIncDec(
  op: "+" | "-",
  args: { readonly variable?: StoryCommandValue; readonly by?: StoryCommandValue },
  generateId: () => string
): StoryBlock {
  ...
  const step: StoryExpr =
    args.by?.kind === "expression" ? args.by.expression.ast : { kind: "literal", value: 1 };
    ...
    return {
      ...
      payload: {
        ...
        expression: {
          ...
          ast: { kind: "unary", op: "!", operand: self }
        }
      }
    };
  }
...
export const reset = defineStoryCommand({
  ...
  build(args, ctx) {
    ...
    return {
      ...
      payload: {
        ...payload,
        value: args.variable.defaultValue ?? defaultForType(args.variable.valueType),
 ...
 * registry.
 */
function validateSceneDeclaration(
  args: { readonly name?: StoryCommandValue },
  ctx: StoryCommandValidateContext
): StoryCommandResolutionIssue[] {
  ...
  if (
    ctx.context.variables.some(
      (entry) => entry.ref.scope === "scene" && entry.name.trim().toLowerCase() === needle
    )
  ) {
 ...
 */
export function declarationFromArgs(
  ...
  const declaredType =
    args.type?.kind === "enum" ? (args.type.value as StoryVariableValueType) : undefined;
  ...
  return {
    ...
    defaultValue,
    description:
      args.desc?.kind === "text" && args.desc.value.trim() ? args.desc.value.trim() : undefined
  };
}
 ...
 */
function buildSceneDeclaration(
  args: ResolvedArgsOf<ReturnType<typeof declarationParams>>,
