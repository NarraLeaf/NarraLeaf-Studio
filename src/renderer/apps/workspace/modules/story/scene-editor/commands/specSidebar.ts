...
export function browseMenuStops(
  groups: readonly StoryCommandSidebarGroup[]
): readonly StoryCommandMenuStop[] {
...
export function buildSpecSidebarGroups(
  ...
  return STORY_COMMAND_GROUPS.map((group) => ({
    group,
    commands: byGroup.get(group.id) ?? []
...
export function dedupeToPrimarySubject(
  ...
  for (const entry of groups) {
    for (const command of entry.commands) {
      ...
      if (
        current === undefined ||
        (current !== command.group && entry.group.id === command.group)
      ) {
  ...
  return groups
    .map((entry) => ({
      ...entry,
      commands: entry.commands.filter((command) => home.get(command.id) === entry.group.id)
    }))
    .filter((entry) => entry.commands.length > 0);
}
...
export function filterSidebarGroups(
  ...
  return categoryId === null
    ? groups
    : groups.filter((entry) => entry.group.category === categoryId);
}
