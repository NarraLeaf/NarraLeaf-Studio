...
function validateTransitionForTarget(
  ...
  if (target.type === "character") {
    if (transformPresetFor(context, word) === undefined) {
      return [
        { code: "unsupportedOption", span, value: word, allowed: supportedTransitionWords(context) }
      ];
    }
    return [];
  }
  ...
  if (
    ...
    transformPresetFor(context, word) === undefined
  ) {
    return [
      { code: "unsupportedOption", span, value: word, allowed: supportedTransitionWords(context) }
    ];
  ...
  return [
    {
      code: "unknownForm",
      span,
      value: args.form.kind === "characterForm" ? args.form.label : "",
...
function stageObjectBlockId(
  objectKind: Exclude<
    Extract<StoryCommandTargetValue, { type: "stageObject" }>["objectKind"],
    "vfx"
  >,
  direction: "show" | "hide"
): ActionCommandId {
...
function buildShowHide<P extends StoryCommandParamsShape>(
  ...
  if (!target || target.type === "character") {
    const block = createBlockForCommand(
      direction === "show" ? "characterEnter" : "characterExit",
      ctx.generateId
    );
  ...
  const block = createBlockForCommand(
    stageObjectBlockId(target.objectKind, direction),
    ctx.generateId
  );
  ...
  if (block.payload.action === "image" || block.payload.action === "text") {
    const transform = withRevealTransform(
      block.payload.transform,
      direction === "show" ? "reveal" : "conceal",
      args.t,
      args.d
    );
    return {
      ...block,
      payload: { ...block.payload, objectName: target.name, ...(transform ? { transform } : {}) }
    };
  }
  ...
  if (block.payload.action === "displayable") {
    ...
    return {
      ...
      payload: {
        ...block.payload,
        target: { kind: "layer", name: target.name },
        ...(transform ? { transform } : {})
      }
    };
  }
...
export const show = defineStoryCommand({
  ...
  params: {
    ...
    t: {
      aliases: ["transition"],
      hint: "transition",
      type: { kind: "enum", options: transitionOptions("reveal") }
    },
    d: secondsParam()
  },
...
export const hide = defineStoryCommand({
  ...
  params: {
    ...
    t: {
      aliases: ["transition"],
      hint: "transition",
      type: { kind: "enum", options: transitionOptions("conceal") }
    },
    d: secondsParam()
  },
...
function validatePuppetCharacter(
  ...
  if (
    ...
    ctx.context.puppetCharacterIds.includes(character.characterId)
  ) {
    return [];
  }
  ...
  const name =
    ctx.context.characters.find((entry) => entry.id === character.characterId)?.name ?? "";
...
export const param = defineStoryCommand({
  ...
  params: {
    ...
    id: {
      hint: "puppetParam",
      type: { kind: "puppetParam", dependsOn: "character" },
      positional: true,
      core: true
    },
  ...
  build(args, ctx): StoryBlock {
    ...
    return {
      ...
      payload: {
        action: "character",
        operation: "setParams",
        ...(args.character?.kind === "character"
          ? { characterId: args.character.characterId }
          : {}),
...
export const say = defineStoryCommand({
  ...
  params: {
    character: {
      hint: "speaker",
      type: { kind: "character", allowTemp: true },
      positional: true,
      core: true
    },
