...
function enumType(
  commandId: string,
  paramName: string
): Extract<StoryCommandParamType, { kind: "enum" }> {
...
describe("localizedEnumValue", () => {
        ...
        for (const param of def.params) {
          for (const type of paramTypes(param)) {
            ...
            for (const option of type.options) {
              ...
              expect(
                /\s/.test(spelling),
                `${def.token}.${param.name} offers "${spelling}", not one token`
              ).toBe(false);
              expect(
  ...
  it("falls back to the canonical value where a locale adds no word of its own", () => {
    ...
    i18nStore.setLocale("zh");
    // `json` is a proper noun: zh spells it "JSON", which folds onto the canonical value and is
    // therefore dropped by the "already a canonical spelling" rule, leaving the canonical word.
    const declaredType = enumType("local", "type");
    const json = declaredType.options.find((option) => option.value === "json")!;
    expect(localizedEnumValue(declaredType, json)).toBe("json");
  });
});
