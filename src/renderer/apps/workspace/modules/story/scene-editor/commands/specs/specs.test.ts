...
const CONTEXT: StoryCommandContext = {
  ...
  choiceOptions: [
    { id: "o1", name: "Refuse her" },
    { id: "o2", name: "Say yes" }
  ],
  valueBlueprints: [
    { id: "bp1", name: "Bonus" },
    { id: "bp2", name: "Story Value" }
  ],
  ...
  appTags: [
    { id: "release", name: "main" },
    { id: "demo", name: "Demo" }
  ],
  variables: [
    {
      name: "gold",
      ref: { scope: "scene", variableId: "var_gold" },
      valueType: "number",
      defaultValue: 10
    },
    ...
    {
      name: "boss hp",
      ref: { scope: "saved", variableId: "var_boss" },
      valueType: "number",
      defaultValue: 3
    }
  ],
...
describe("scene commands", () => {
  it("/bg writes assetId XOR color, and maps the unified fade to a crossfade", () => {
    ...
    expect(build("/bg #1a1a1a")).toMatchObject({
      payload: { color: "#1a1a1a", assetId: undefined }
    });
  });
  ...
  it("/bg rejects a word its context does not support", () => {
    ...
    expect(parseCommandLine("/bg forest_day t=zoom")).toMatchObject({
      issues: [{ code: "badValue" }]
    });
  });
...
describe("generic verbs", () => {
  ...
  it("/show dispatches on the target: an image reveals through its transform preset", () => {
    expect(build("/show hero t=fade d=0.2")).toMatchObject({
      payload: {
        ...
        operation: "show",
        objectName: "hero",
        transform: { preset: "fadeIn", durationMs: 200 }
      }
    });
  });
  ...
  it("/hide is direction-aware: the same word fades OUT", () => {
    expect(build("/hide hero t=fade")).toMatchObject({
      payload: {
        ...
        operation: "hide",
        objectName: "hero",
        transform: { preset: "fadeOut" }
      }
    });
    expect(build("/hide Alice")).toMatchObject({
      payload: { action: "character", operation: "exit" }
    });
  });
  ...
  it("the row token reads back the `d=` the author typed, for a character and an image alike", () => {
    ...
    const ms = (source: string) =>
      getQuickParams(build(source)).find((param) => param.id === "d")?.value;
  ...
  it("/show reaches text, video and layer targets too", () => {
    expect(build("/show title")).toMatchObject({
      payload: { action: "text", operation: "show", objectName: "title" }
    });
    expect(build("/show clip")).toMatchObject({
      payload: { action: "video", operation: "show", objectName: "clip" }
    });
    expect(build("/show overlay")).toMatchObject({
      payload: {
        action: "displayable",
        operation: "show",
        target: { kind: "layer", name: "overlay" }
      }
    });
  });
  ...
  it("rejects a target nothing answers to, and an unsupported word for the resolved context", () => {
    ...
    expect(parseCommandLine("/show Alice t=blinds")).toMatchObject({
      issues: [{ code: "badValue" }]
    });
  ...
  it("/play plays a video by name", () => {
    expect(build("/play clip")).toMatchObject({
      payload: { action: "video", operation: "play", objectName: "clip" }
    });
  });
});
  ...
  it("/rename writes the new speaker label and nothing else", () => {
    ...
    expect(build("/rename Alice the man in grey")).toMatchObject({
      ...
      payload: {
        ...
        operation: "setName",
        characterId: "c1",
        displayName: "the man in grey"
      }
    });
    expect(build("/rename Alice ？？？")).toMatchObject({ payload: { displayName: "？？？" } });
    expect(build("/setname Alice X")).toMatchObject({
      payload: { operation: "setName", displayName: "X" }
    });
...
describe("puppet state channels", () => {
  ...
  it("/motion and /skin write their own operation and nothing else", () => {
    ...
    expect(build("/anim Doll walk")).toMatchObject({
      payload: { operation: "setMotion", puppetName: "walk" }
    });
    expect(build("/costume Doll summer")).toMatchObject({
      payload: { operation: "setSkin", puppetName: "summer" }
    });
  ...
  it("naming nothing is the request to clear - the engine's null, not an unfilled slot", () => {
    expect(build("/motion Doll")).toMatchObject({
      payload: { operation: "setMotion", characterId: "c2" }
    });
  ...
  it("writes a parameter row as a map, because one gesture is several parameters", () => {
    ...
    expect(build("/param Doll ParamAngleX -7.5")).toMatchObject({
      payload: { params: { ParamAngleX: -7.5 } }
    });
...
describe("media objects", () => {
  it("/image auto-names from the asset filename", () => {
    expect(build("/image forest_day at=left")).toMatchObject({
      payload: {
        action: "image",
        operation: "create",
        objectName: "forest_day",
        assetId: "i1",
        transform: { preset: "left" }
      }
    });
  });
  ...
  it("/font sets one thing per block and faults on both", () => {
    expect(build("/font title 48")).toMatchObject({
      payload: { operation: "setFontSize", fontSize: 48, objectName: "title" }
    });
    expect(build("/font title color=#ff0000")).toMatchObject({
      payload: { operation: "setFontColor", fontColor: "#ff0000" }
    });
...
describe("sound (target defaults to bgm)", () => {
  ...
  it("the id a line stores resolves to a bus, and a v1 id still resolves through the alias", () => {
    ...
    const trackId =
      stored.kind === "action" && stored.payload.action === "audio"
    ...
    const withOwnMusic = [
      ...tracks,
      { id: "music", name: "My Music", parentId: null, volume: 1, loop: false }
    ];
  ...
  it("/vol with no target turns down the music channel", () => {
    expect(build("/vol 0.5")).toMatchObject({
      payload: { action: "audio", operation: "setVolume", objectName: "bgm", volume: 0.5 }
    });
    expect(build("/vol music 0.2 fade=0.5")).toMatchObject({
      payload: { objectName: "music", volume: 0.2, fadeMs: 500 }
    });
  });

  it("the whole control family defaults to bgm", () => {
    expect(build("/stop")).toMatchObject({
      payload: { operation: "stopSound", objectName: "bgm" }
    });
    expect(build("/pause")).toMatchObject({
      payload: { operation: "pauseSound", objectName: "bgm" }
    });
    expect(build("/resume music")).toMatchObject({
      payload: { operation: "resumeSound", objectName: "music" }
    });
    expect(build("/mute")).toMatchObject({
      payload: { operation: "muteSound", muted: true, objectName: "bgm" }
    });
    expect(build("/unmute")).toMatchObject({
      payload: { operation: "muteSound", muted: false, objectName: "bgm" }
    });
    expect(build("/rate 1.5")).toMatchObject({
      payload: { operation: "setRate", rate: 1.5, objectName: "bgm" }
    });
  });
  ...
  it("/stop /pause /resume dispatch on what the target turned out to be", () => {
    ...
    expect(build("/pause")).toMatchObject({
      payload: { action: "audio", operation: "pauseSound", objectName: "bgm" }
    });
    expect(build("/pause music")).toMatchObject({
      payload: { action: "audio", operation: "pauseSound", objectName: "music" }
    });
    expect(build("/pause clip")).toMatchObject({
      payload: { action: "video", operation: "pause", objectName: "clip" }
    });
    expect(build("/resume clip")).toMatchObject({
      payload: { action: "video", operation: "resume", objectName: "clip" }
    });
    expect(build("/stop clip")).toMatchObject({
      payload: { action: "video", operation: "stop", objectName: "clip" }
    });
  });
...
describe("variables", () => {
  ...
  it("/set folds a constant back into value and keeps an expression as a tree", () => {
    expect(build("/set gold 100")).toMatchObject({
      payload: {
        action: "setVariable",
        target: { scope: "scene", variableId: "var_gold" },
        value: 100,
  ...
  it("/set addresses a name with spaces through single quotes, on either side of the assignment", () => {
    expect(build("/set 'boss hp' 5")).toMatchObject({
      payload: {
        action: "setVariable",
        target: { scope: "saved", variableId: "var_boss" },
        value: 5,
        expression: undefined
      }
    });
    expect(build("/set gold 'boss hp' + 1")).toMatchObject({
      payload: {
        target: { variableId: "var_gold" },
        expression: { source: "'boss hp' + 1", ast: { kind: "binary", op: "+" } }
      }
    });
  });
  ...
  it("/inc defaults its step to 1; /toggle negates; /reset snapshots the declared default", () => {
    ...
    expect(build("/dec gold 2")).toMatchObject({
      payload: { expression: { source: "gold - (2)" } }
    });
    expect(build("/toggle met")).toMatchObject({
      payload: { target: { variableId: "var_met" }, expression: { source: "!met" } }
    });
  ...
  it("declarationFromArgs pins the whole line to the declaration it produces", () => {
    ...
    expect(declare("/local hp 100")).toEqual({
      name: "hp",
      valueType: "number",
      defaultValue: 100,
      description: undefined
    });
    expect(declare("/local seen")).toEqual({
      name: "seen",
      valueType: "boolean",
    ...
    expect(declare("/local flag 1 type=bool")).toMatchObject({
      valueType: "boolean",
      defaultValue: 1
    });
  ...
  it("reads a bracketed default as the list or object it spells", () => {
    expect(declare('/local inv "[1, 2]" type=json')).toEqual({
      name: "inv",
      valueType: "json",
      defaultValue: [1, 2],
      description: undefined
    ...
    expect(declare("/local cfg '{\"hp\": 3}' type=json")).toMatchObject({
      defaultValue: { hp: 3 }
    });
    expect(declare('/local inv \'["sword", "potion"]\'')).toMatchObject({
      valueType: "json",
      defaultValue: ["sword", "potion"]
    });
    ...
    expect(declare('/local inv "[1, 2]"')).toMatchObject({
      valueType: "json",
      defaultValue: [1, 2]
    });
  ...
  it("leaves words that merely open with a bracket as the string they read as", () => {
    ...
    expect(declare('/local note "[draft]"')).toMatchObject({
      valueType: "string",
      defaultValue: "[draft]"
    });
    expect(declare('/local note "[1, 2"')).toMatchObject({
      valueType: "string",
      defaultValue: "[1, 2"
    });
    expect(declare('/local note "{unclosed"')).toMatchObject({
      valueType: "string",
      defaultValue: "{unclosed"
    });
    expect(declare('/local note "[TODO] rewrite this"')).toMatchObject({
      defaultValue: "[TODO] rewrite this"
    });
  });
  ...
  it("lets an explicit type=string outrank how the value happens to read", () => {
    ...
    expect(declare('/local x "[1, 2]" type=string')).toMatchObject({
      valueType: "string",
      defaultValue: "[1, 2]"
    });
    expect(declare('/local x "[1,2]" type=str')).toMatchObject({
      valueType: "string",
      defaultValue: "[1,2]"
    });
  ...
  it("still refuses to read or compute anything in a constant slot", () => {
    ...
    expect(parseCommandLine("/local hp min(1, 2)")).toMatchObject({
      issues: [{ code: "extraPositional" }]
    });
    expect(declare('/local hp "min(1, 2)"')).toMatchObject({
      valueType: "string",
      defaultValue: "min(1, 2)"
    });
    expect(declare('/local hp "gold + 1"')).toMatchObject({
      valueType: "string",
      defaultValue: "gold + 1"
    });
  });
  ...
  it("a token is a spelling, never a thing a document records", () => {
    ...
    expect(declaration("/local chapter 10 type=number")).toEqual(
      declaration("/scenevar chapter 10 type=number")
    );
    ...
    for (const source of ["/local chapter", "/scenevar chapter"]) {
      expect(Object.keys(build(source).payload).sort()).toEqual([
        "defaultValue",
        "description",
        "name",
        "scope",
    ...
    expect(build("/src hero night")).toMatchObject({
      payload: { operation: "setSource", assetId: "i2" }
    });
  });
  ...
  it("a declaration builds a ROW whose id is the variable and whose key is its own id (v6)", () => {
    ...
    expect(build("/local nickname type=string")).toMatchObject({
      kind: "declaration",
      payload: { scope: "scene", valueType: "string" }
    });
  });
});
...
describe("logic and effects", () => {
  it("paramless containers build their control blocks", () => {
    expect(build("/parallel")).toMatchObject({
      kind: "control",
  ...
  it("/until builds a conditional loop with no quotes and no count", () => {
    ...
    expect(block).toMatchObject({
      ...
      payload: {
        control: "repeat",
        until: { kind: "expression", expression: { source: "gold >= 100" } }
      }
    });
  ...
  it("/repeat until builds a conditional loop, and carries no count beside it", () => {
    ...
    expect(build("/repeat until=met")).toMatchObject({
      payload: { until: { kind: "expression" } }
    });
  ...
  it("/if builds the bare condition container - the expression rides to the scaffolded branch", () => {
    expect(build("/if gold >= 100")).toMatchObject({
      kind: "control",
  ...
  it("/label marks a place and /goto only accepts one that exists", () => {
    ...
    expect(build("/goto intro")).toMatchObject({
      kind: "control",
      payload: { control: "goto", targetLabel: "intro" }
    });
  ...
  it("/blink and /vignette read their knobs in seconds", () => {
    ...
    expect(build("/vignette opacity=0.5")).toMatchObject({
      payload: { effect: "vignette", opacity: 0.5 }
    });
  });
  ...
  it("/camera reads its operation as the first positional and the knob as the second", () => {
    ...
    expect(build("/camera darken 0.6")).toMatchObject({
      payload: { operation: "darken", darkness: 0.6 }
    });
    expect(build("/camera rotate -15")).toMatchObject({
      payload: { operation: "rotate", rotation: -15 }
    });
    expect(build("/camera reset d=0.6")).toMatchObject({
      payload: { operation: "reset", durationMs: 600 }
    });
    // `/cam` is the same command, and an alias resolves to the canonical operation.
    expect(build("/cam dim 0.4")).toMatchObject({
      payload: { action: "camera", operation: "darken", darkness: 0.4 }
    });
  });
  ...
  it("/camera leaves a knob it was not given at its neutral value", () => {
    ...
    expect(build("/camera pan")).toMatchObject({
      payload: { operation: "pan", position: { xalign: 0.5 } }
    });
  ...
  it("/camera motion commits an unbound Story Motion ref and routes only that line to the inspector", () => {
    ...
    expect(build("/cam shot")).toMatchObject({
  ...
  it("the generic verbs reach a vfx with its own payload", () => {
    ...
    expect(build("/hide petals")).toMatchObject({
      payload: { action: "vfx", operation: "hide", objectName: "petals" }
    });
    expect(build("/pause petals")).toMatchObject({
      payload: { action: "vfx", operation: "pause" }
    });
    expect(build("/resume petals")).toMatchObject({
      payload: { action: "vfx", operation: "resume" }
    });
    expect(build("/rate petals 0.5")).toMatchObject({
      payload: { action: "vfx", operation: "setRate", rate: 0.5 }
    });
    ...
    expect(build("/stop petals")).toMatchObject({
      payload: { action: "audio", operation: "stopSound", objectName: "petals" }
    });
  });
  ...
  it("/fx and /transform bind their displayable target and defer the rest to the inspector", () => {
    ...
    expect(build("/fx hero")).toMatchObject({
      payload: { action: "displayable", target: { kind: "image", name: "hero" } }
    });
...
describe("manual examples", () => {
  ...
  it("gives every command at least one worked line", () => {
    const missing = specs
      .filter((spec) => !spec.examples || spec.examples.length === 0)
      .map((spec) => spec.token);
  ...
  it("names the command it documents", () => {
    for (const spec of specs) {
      for (const example of spec.examples ?? []) {
        ...
        expect(
          tokens.some((token) => example === `/${token}` || example.startsWith(`/${token} `)),
          example
        ).toBe(true);
      }
    }
