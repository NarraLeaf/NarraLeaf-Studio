...
describe("diagnoseRow", () => {
  ...
  it("marks a row pointing at an asset the project no longer has", () => {
    expect(diagnoseRow({ block: background("gone"), context: CONTEXT })).toEqual({
      code: "missingAsset"
    });
  });
  ...
  describe("a puppet name the model does not have", () => {
    it("marks the row on every channel that carries one", () => {
      expect(
        diagnoseRow({ block: puppetRow("setMotion", "c2", "runn"), context: CONTEXT })
      ).toEqual({ code: "unknownPuppetName" });
      expect(
        diagnoseRow({ block: puppetRow("expression", "c2", "grin"), context: CONTEXT })
      ).toEqual({ code: "unknownPuppetName" });
    });

    it("says nothing about a name the model listed", () => {
      expect(
        diagnoseRow({ block: puppetRow("setMotion", "c2", "walk"), context: CONTEXT })
      ).toBeNull();
    });
    ...
    it("says nothing about a blank name - that is the request to clear, not an unfilled slot", () => {
      ...
      expect(
        diagnoseRow({ block: puppetRow("setMotion", "c2", "   "), context: CONTEXT })
      ).toBeNull();
    });
    ...
    it("stays quiet on a channel the model said nothing about", () => {
      ...
      expect(
        diagnoseRow({ block: puppetRow("setSkin", "c2", "winter"), context: CONTEXT })
      ).toBeNull();
    });
    ...
    it("stays quiet about a model nobody could ask", () => {
      ...
      expect(
        diagnoseRow({ block: puppetRow("setMotion", "c3", "anything"), context: CONTEXT })
      ).toBeNull();
    });
    ...
    it("checks every id on a parameter row, not just the first", () => {
      ...
      expect(
        diagnoseRow({ block: row({ ParamAngleX: 12, ParamAngleY: 3 }), context: CONTEXT })
      ).toEqual({ code: "unknownPuppetName" });
    ...
    it("leaves a character Studio draws itself alone", () => {
      ...
      expect(
        diagnoseRow({ block: puppetRow("expression", "c1", "smile"), context: CONTEXT })
      ).toBeNull();
    });
  });
});
