...
function FilterRow(props: {
  ...
  return (
    ...
    >
      ...
      <span className="flex h-5 w-5 shrink-0 items-center justify-center">{props.mark}</span>
      <span
        className={[
          "min-w-0 truncate",
          empty ? "text-fg-subtle" : props.selected ? "text-primary" : "text-fg"
        ].join(" ")}
      >
        {props.label}
      </span>
      ...
      <span
        className={["shrink-0 tabular-nums", empty ? "text-fg-subtle/60" : "text-fg-subtle"].join(
          " "
        )}
      >
        {props.count}
...
function CastMark(props: { identity: StorySpeakerIdentity; character: Character | null }) {
  ...
  const { url, frame, showingSprite } = useCharacterAvatar(props.character, {
    resolveVariant: true,
...
export function StoryRowFilterMenu(props: {
  ...
  const cast = useMemo(
    () =>
      props.tallies.speakers.map((tally) => ({
        key: tally.key,
        count: tally.count,
        ...
        character: tally.characterId
          ? (props.characters.find(
              (candidate) => candidate.profile.getId() === tally.characterId
            ) ?? null)
          : null,
  ...
  const left = Math.max(
    VIEWPORT_MARGIN,
    Math.min(props.anchor.right - PANEL_WIDTH, window.innerWidth - PANEL_WIDTH - VIEWPORT_MARGIN)
  );
  const top = Math.min(
    props.anchor.bottom + 6,
    Math.max(VIEWPORT_MARGIN, window.innerHeight - PANEL_MAX_HEIGHT)
  );
  ...
  const sections: { key: string; label: string; facets: readonly StoryRowFacetId[] }[] = [
    {
      key: "script",
      label: t("story.view.filter.sectionScript"),
      facets: STORY_ROW_NARRATIVE_FACETS
    },
    {
      key: "staging",
      label: t("story.view.filter.sectionStaging"),
      facets: STORY_ROW_STAGING_FACETS
    }
  ];
    ...
    >
      ...
      {sections.map((section) => (
        ...
        <div key={section.key} className="contents">
          ...
          <div className="shrink-0 px-2 pb-0.5 text-2xs font-medium text-fg-subtle">
            {section.label}
          </div>
      ...
      {cast.length > 0 ? (
        <>
          <div className="my-0.5 h-px shrink-0 bg-edge" />
          <div className="shrink-0 px-2 pb-0.5 text-2xs font-medium text-fg-subtle">
            {t("story.view.filter.sectionCast")}
          </div>
          {cast.map((member) => (
            <FilterRow
              key={member.key}
