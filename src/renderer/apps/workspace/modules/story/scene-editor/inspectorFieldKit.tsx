...
export function Section(props: {
  ...
  return (
    <section
      className={["rounded-lg border border-edge bg-fill-subtle p-2.5", props.className ?? ""].join(
        " "
      )}
    >
      {props.title || props.right ? (
        <div className="mb-2 flex items-center justify-between gap-2">
          {props.title ? (
            <div className="min-w-0 truncate text-2xs font-medium tracking-wide text-fg-muted">
              {props.title}
            </div>
          ) : (
            <span />
...
export function FieldGrid(props: { cols?: 2 | 3 | 4; className?: string; children: ReactNode }) {
  ...
  return (
    <div className={["nl-field-grid", colClass, props.className ?? ""].join(" ")}>
      {props.children}
    </div>
  );
}
 ...
 */
export function SecondsField(props: {
  label: string;
  value: number | undefined;
