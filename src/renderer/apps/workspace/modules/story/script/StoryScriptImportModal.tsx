    ...
    return (
        <Note kind={diagnostic.severity === "error" ? "error" : "warning"}>
            <span data-story-script-diag={diagnostic.code}>
                {diagnostic.line !== undefined ? `${t("story.script.line", { line: diagnostic.line })} · ` : ""}
            ...
            {plan && (
                <div className="flex flex-col gap-3">
                    ...
                    {plan.scenes.map(scene => (
                        <div key={scene.sceneId} className="flex flex-col gap-1" data-story-script-scene={scene.sceneId}>
                            <div className={ROW}>
                                <span className="min-w-0 flex-1 truncate font-medium">{scene.sceneName}</span>
                                <span className="shrink-0 text-2xs text-fg-subtle">
                                    {STAT_KEYS
                                        .filter(key => scene.stats[key] > 0)
                                        .map(key => t(`story.script.stat.${key}` as TranslationKey, { count: scene.stats[key] }))
                                        .join(" · ")}
                                </span>
                            </div>
                            {scene.missing && <Note kind="warning">{t("story.script.sceneMissing")}</Note>}
                            {scene.stale && !scene.missing && <Note kind="warning">{t("story.script.stale")}</Note>}
