...
} from "@/lib/story/script/storyScriptCodec";
import type {
  StoryScriptExportMode,
...
export function useStoryScriptIo(): StoryScriptIo {
  ...
  const runExport = useCallback(
    async (mode: StoryScriptExportMode) => {
      ...
      try {
        ...
        const blueprintService = context.services.get<LocalBlueprintService>(
        ...
        const motions = new Map(
          storyService.listAnimationAssets().map((entry) => [entry.id, entry.name])
        );
        ...
        if (!selection.success || !selection.data.ok) {
          throw new Error(
            selection.success && !selection.data.ok
  ...
  const beginExport = useCallback(
    (target: StoryScriptExportTarget) => {
      if (ready) {
