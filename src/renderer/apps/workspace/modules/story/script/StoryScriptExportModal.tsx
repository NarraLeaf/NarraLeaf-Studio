        ...
        >
            <div className="flex flex-col gap-1.5">
                {MODES.map(candidate => (
                    ...
                    >
                        <div className="text-xs font-medium text-fg">{t(`story.script.mode.${candidate}` as TranslationKey)}</div>
                        <div className="text-2xs text-fg-subtle">{t(`story.script.mode.${candidate}Detail` as TranslationKey)}</div>
                    </button>
                ))}
            </div>
        </Modal>
