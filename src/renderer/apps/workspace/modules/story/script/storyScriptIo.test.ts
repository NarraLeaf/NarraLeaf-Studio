...
function scenePlan(sceneId: string): StoryScriptScenePlan {
    const scene: StoryScene = { id: sceneId, name: sceneId, runtimeName: sceneId, rootBlockIds: [], blocks: {} };
    return {
        sceneId,
        sceneName: sceneId,
        scene,
...
describe("story script undo coverage", () => {
    it("is partial when only some of the scenes have an editor that can undo them", () => {
        expect(storyScriptUndoCoverage(SCENES, sceneId => sceneId === "a")).toEqual({ coverage: "partial", unundoable: 2 });
        expect(storyScriptUndoCoverage(SCENES, sceneId => sceneId !== "a")).toEqual({ coverage: "partial", unundoable: 1 });
    });

    it("is all or none only when it really is", () => {
        expect(storyScriptUndoCoverage(SCENES, () => true)).toEqual({ coverage: "all", unundoable: 0 });
        expect(storyScriptUndoCoverage(SCENES, () => false)).toEqual({ coverage: "none", unundoable: 3 });
