...
const BLUEPRINT_VARIABLE_SELECT_OPTIONS: SelectOption[] = BLUEPRINT_VARIABLE_TYPE_OPTIONS.map(
  (option) => ({
    value: option.value,
    label: option.label
  })
  ...
  const nameError =
    trimmedName.length === 0
      ...
      : normalizedExistingNames.has(trimmedName.toLowerCase())
        ...
        : undefined;
  const valid =
    !nameError && BLUEPRINT_VARIABLE_TYPE_OPTIONS.some((option) => option.value === valueType);
