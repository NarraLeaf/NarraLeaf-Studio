...
function JsonEditorSelect(props: SelectProps) {
  ...
  const scopedMenuAttributes = scopeId ? { [JSON_EDITOR_SCOPE_ATTRIBUTE]: scopeId } : undefined;
    ...
    <div>
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1">
          {objectKeyEditor ?? (
            <span
              className="min-w-[4.5rem] truncate text-2xs text-fg-muted"
...
function JsonEditorPortal({
  ...
  const reactId = useId();
  const scopeId = useMemo(() => `json-editor-${reactId.replace(/[^a-zA-Z0-9_-]/g, "")}`, [reactId]);
      ...
      <JsonEditorScopeContext.Provider value={scopeId}>
        <div className="mb-2 flex min-w-0 items-center gap-2 border-b border-edge pb-2">
          ...
          <Button
            ...
            aria-label={
              mode === "raw" ? t("blueprint.json.treeEditor") : t("blueprint.json.rawJson")
            }
            variant="ghost"
            size="sm"
    ...
    >
      <div className="min-w-0 flex-1 rounded-md border border-edge bg-surface px-1.5 py-1">
        ...
        <div
          ...
          data-tip={summarizeJsonValue(root)}
        >
          {summarizeJsonValue(root)}
        </div>
      </div>
