...
import { resolveBlueprintCategoryLabel, resolveBlueprintNodeTitle } from "../blueprintNodeI18n";
...
export function BlueprintAddNodeMenu({
  ...
  const layout = useMemo(() => {
    ...
    const maxHeight = Math.min(
      MENU_MAX_H,
      Math.max(280, hostWindow.innerHeight - viewportTop - pad)
    );
    const left = Math.max(pad, Math.min(anchor.x, hostWindow.innerWidth - MENU_W - pad));
    const top = Math.max(
      viewportTop,
      Math.min(anchor.y, Math.max(viewportTop, hostWindow.innerHeight - maxHeight - pad))
    );
  ...
  const listMaxHeight = Math.max(
    120,
    Math.min(MENU_MAX_H - MENU_CHROME_H, layout.maxHeight - MENU_CHROME_H)
  );
  ...
  const selectRelativeCategory = useCallback((offset: number) => {
    ...
    requestAnimationFrame(() => {
      const el = categoryListRef.current?.querySelector(
        `[data-bp-add-node-category-idx="${nextIndex}"]`
      );
...
function BlueprintAddNodeRow(props: {
  ...
  const subtitle = magicRef ? `${categoryLabel} -> ${magicRef.label}` : categoryLabel;
  const title = [
    nodeTitle,
    props.entry.type,
    magicRef
      ? t("blueprint.addNode.targetTooltip", { label: magicRef.label, type: magicRef.elementType })
      : "",
