...
describe("BlueprintAddNodeMenuModel", () => {
  it("builds an All category without dropping registry-filtered entries", () => {
    ...
    expect(categories[0]).toEqual({
      id: BLUEPRINT_ADD_NODE_ALL_CATEGORY_ID,
      label: "All",
      count: 3
    });
  ...
  it("matches display name, type, category, and keywords", () => {
    expect(
      filterBlueprintAddNodeEntries(entries, "all", "literal").map((item) => item.type)
    ).toEqual(["nl.data.string"]);
    expect(
      filterBlueprintAddNodeEntries(entries, "all", "math.add").map((item) => item.type)
    ).toEqual(["nl.math.add"]);
    expect(
      filterBlueprintAddNodeEntries(entries, "all", "events").map((item) => item.type)
    ).toEqual(["nl.event.init"]);
    expect(
      filterBlueprintAddNodeEntries(entries, "all", "startup").map((item) => item.type)
    ).toEqual(["nl.event.init"]);
  });

  it("supports fuzzy token and compact type matches", () => {
    expect(
      filterBlueprintAddNodeEntries(entries, "all", "str lit").map((item) => item.type)
    ).toEqual(["nl.data.string"]);
    expect(filterBlueprintAddNodeEntries(entries, "all", "madd").map((item) => item.type)).toEqual([
      "nl.math.add"
    ]);
    expect(
      filterBlueprintAddNodeEntries(entries, "all", "oninit").map((item) => item.type)
    ).toEqual(["nl.event.init"]);
  });
  ...
  it("applies category and query filters together", () => {
    expect(
      filterBlueprintAddNodeEntries(entries, "Data", "literal").map((item) => item.type)
    ).toEqual(["nl.data.string"]);
  ...
  it("searches unambiguous element-derived entries by target label", () => {
    ...
    expect(
      filterBlueprintAddNodeEntries(derived, "Element", "icon").map(
        (item) => item.magicElementRef?.label
      )
    ).toEqual(["Icon"]);
  });
});

function elementDerivedEntry(
  sourceNodeId: string,
  elementId: string,
  label: string
