...
import {
  ...
  type BlueprintVariableDialogValue
} from "./BlueprintVariableDialogContent";
...
function BlueprintPersistentVariableRow({
  ...
  return (
    <div className="group rounded-md border border-edge bg-surface-sunken px-2 py-1.5 space-y-1.5">
      <div className="flex items-center justify-between gap-1">
        <label
          htmlFor={`persistent-variable-name-${v.id}`}
...
function sortedVariables(blueprint: Blueprint): BlueprintVariable[] {
  return Object.values(blueprint.members?.variables ?? {}).sort((a, b) =>
...
export function BlueprintMemberTree({
        ...
        const handleConfirm = () => {
          if (!selection.valid) {
            uiService.showNotification(
              t("blueprint.memberTree.createPersistentInvalid"),
              "warning"
            );
            return;
          }
  ...
  const globalBlueprintId =
    blueprintDocument.ownerRecords[GLOBAL_MAIN_OWNER_KEY]?.activeBlueprintId;
  const globalBlueprint = globalBlueprintId
    ? blueprintDocument.blueprints[globalBlueprintId]
    : undefined;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 text-xs text-fg-muted">
                  ...
                  >
                    ...
                    {errors > 0 ? (
                      <span className="ml-1 text-danger">
                        {t("blueprint.memberTree.errorBadge", { count: errors })}
                      </span>
                    ) : warnings > 0 ? (
                      <span className="ml-1 text-warning">
                        {t("blueprint.memberTree.warningBadge", { count: warnings })}
                      </span>
                    ) : null}
                  </button>
                </li>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto border-t border-edge-subtle pt-2">
        ...
        >
          <div className="space-y-2">
            {sortedPersistentList.length === 0 ? (
              <p className="text-fg-subtle">{t("blueprint.memberTree.persistentEmpty")}</p>
            ) : null}
