...
describe("blueprintElementPreviewsSignature", () => {
  it("changes when an Element preview revision changes", () => {
    ...
    expect(blueprintElementPreviewsSignature(first)).not.toBe(
      blueprintElementPreviewsSignature(second)
    );
  });
  ...
  it("is stable across object entry order", () => {
    ...
    expect(blueprintElementPreviewsSignature({ a, b })).toBe(
      blueprintElementPreviewsSignature({ b, a })
    );
  });
});
