...
type BlueprintFlowCanvasInnerProps = {
  ...
  onCommitIr: (
    ...
    history?: { mergeKey?: string; mergeWindowMs?: number }
  ) => void;
  /**
  ...
  displayableTargetVariantsByNodeId?: Record<
    string,
    BlueprintFlowNodeData["displayableTargetVariants"]
  >;
...
function nodeDiagnosticsSignature(
  map: ReadonlyMap<string, readonly BlueprintGraphEditorDiagnostic[]>
): string {
  return [...map.entries()]
    .map(
      ([nodeId, items]) =>
 ...
 */
function syncSameGraphFnCallSnapshots(
  ir: BlueprintGraphIr,
...
function BlueprintFlowCanvasInner({
  ...
  const { getNodes, screenToFlowPosition, fitView, getViewport, setViewport, setCenter } =
    useReactFlow();
  ...
  const patchNodeParam = useCallback(
    (nodeId: string, key: string, value: unknown, history?: BlueprintNodeParamHistoryOptions) => {
      ...
      if (n.type === BLUEPRINT_NODE_TYPE_LOCAL_DECLARE_VAR && key === "valueType") {
        next.defaultValue = resolveBlueprintVariableDefaultValue(
        ...
        for (const other of Object.values(snap.nodes ?? {})) {
          if (
            variableId &&
            (other.type === BLUEPRINT_NODE_TYPE_LOCAL_GET ||
              other.type === BLUEPRINT_NODE_TYPE_LOCAL_SET) &&
  ...
  const removeDynamicInputPin = useCallback(
    (nodeId: string, pinId: string) => {
      ...
      const params = { ...(n.params ?? {}) };
      const list = readDynamicInputPinIds(params, d.storageKey).filter(
        (id) => !removalIdSet.has(id)
      );
      if (list.length > 0) {
        params[d.storageKey] = list;
      } else {
  ...
  const [pendingPlacementEntry, setPendingPlacementEntry] =
    useState<BlueprintNodeEditorCatalogEntry | null>(null);
  ...
  focusPendingRef.current =
    focusRequestKey !== null && focusRequestKey !== appliedFocusKeyRef.current;
  ...
  const onEdgeDoubleClick = useCallback(
    (_e: ReactMouseEvent, edge: Edge) => {
      ...
      const filtered = before.filter(
        (e) =>
          !(e.from.nodeId === src && e.from.port === sh && e.to.nodeId === tgt && e.to.port === th)
      );
