...
import {
  ...
  type LiteralEditMode
} from "../../components/BlueprintLiteralValueControl";
...
const CARD_INPUT = "rounded-md border-edge bg-surface px-1.5 py-1 font-mono text-2xs";
...
function AudioAssetPickerRow({
  ...
  const asset = useMemo(() => {
    ...
    return (
      context.services.get<AssetsService>(Services.Assets).getAssets()[AssetType.Audio]?.[
        assetId
      ] ?? null
    );
  }, [assetId, context]);
    ...
    >
      ...
      >
        ...
        <span className={`min-w-0 flex-1 truncate ${assetId ? "text-fg" : "text-fg-subtle"}`}>
          {assetId ? (assetName ?? t("blueprint.image.missing")) : t("blueprint.image.select")}
        </span>
        {marked && (
...
function ImageAssetPickerCard({
  ...
  const anchorRef = useRef<HTMLButtonElement | null>(null);
  const label = assetId
    ? (assetName ?? t("blueprint.image.fallback"))
  ...
  const detail = assetId
    ? error
      ? t("blueprint.image.missing")
      : "ImageAsset"
    : t("blueprint.image.none");
...
function readDynamicPinLabelValues(
  params: Record<string, unknown>,
...
function InputPinRow({
  ...
  const canInlineLiteral =
    semantic === "data" && Boolean(pin.allowInlineLiteral) && Boolean(onPatchNodeParam) && !isWired;
  ...
  return (
    <div ref={rootRef} className="nodrag relative min-w-0">
      ...
      <button
        ...
        data-tip={
          displayValue
            ? t("blueprint.keyboard.boundTo", { key: displayValue })
            : t("blueprint.keyboard.bind")
        }
      ...
      >
        ...
        <span
          className={`min-w-0 flex-1 truncate font-mono text-2xs ${displayValue ? "" : "text-fg-subtle"}`}
        >
...
function InspectorParamOnCard({
  ...
  const rawSelectOptions: BlueprintInspectorParamSelectOption[] | undefined =
    spec.kind === "select"
      ? (spec.options ??
        (spec.dynamicOptionsSource ? dynamicSelectOptions?.[spec.dynamicOptionsSource] : undefined))
      : undefined;
  const selectOptions =
    rawSelectOptions && spec.dynamicOptionsFilter
      ? rawSelectOptions.filter(
          (option) =>
            option.meta?.[spec.dynamicOptionsFilter!.optionMetaKey] ===
            String(params[spec.dynamicOptionsFilter!.paramKey] ?? "")
        )
      : rawSelectOptions;
  const selectComponentOptions: SelectOption[] | undefined = selectOptions
    ? [
        ...
        ...selectOptions.map((opt) => ({
          value: opt.value,
          label: resolveBlueprintLabel(opt.label, t)
        }))
      ]
    : undefined;
  ...
  const isVarDefaultValueParam =
    ...
    nodeType === BLUEPRINT_NODE_TYPE_LOCAL_DECLARE_VAR;
  ...
  return (
    ...
    >
      <div className="mb-0.5 text-2xs tracking-wide text-fg-subtle">
        {resolveBlueprintLabel(spec.label, t)}
      </div>
      {spec.kind === "select" && selectComponentOptions ? (
        ...
        <Input
          className={
            spec.kind === "number" ? `${CARD_INPUT} ${INPUT_NUMBER_NO_SPINNER}` : CARD_INPUT
          }
          ...
          fullWidth
          onChange={(e) => {
            const v = spec.kind === "number" ? Number(e.target.value) : e.target.value;
...
function DisplayableSetVariantCard({
  ...
  const waitForTransition =
    params.waitForTransition === "wait" || params.waitForTransition === true ? "wait" : "continue";
    ...
    >
      ...
      <div className="mt-1.5">
        ...
        <Select
          ...
          onChange={(value) =>
            onPatchNodeParam(nodeId, "waitForTransition", String(value) || "continue")
          }
          portalMenu
          menuPlacement="below"
        />
    ...
    >
      ...
      <div className="mt-1.5 grid grid-cols-2 gap-1.5">
        <div>
          ...
          <CardNumberInput
            value={isLegacyAbsolutePositionFromDefault ? "" : optionalNumericParam(params, "from")}
            unit={valueUnit}
...
function BlueprintCommentNodeCard({
  ...
  const colorKey =
    typeof params.color === "string" && COMMENT_COLORS[params.color] ? params.color : "amber";
        ...
        >
          ...
          <button
            ...
            className={`relative h-4 w-4 rounded-full border border-dashed ${
              backgroundEnabled ? "border-edge-strong bg-fill-strong" : "border-edge bg-transparent"
            }`}
            data-tip={
              backgroundEnabled
                ? t("blueprint.comment.backgroundOn")
                : t("blueprint.comment.backgroundOff")
            }
            aria-label={
              backgroundEnabled
                ? t("blueprint.comment.sendBehind")
                : t("blueprint.comment.restoreLayer")
            }
...
function BlueprintElementLiteralNodeCard({
  ...
  const boundLabel =
    elementPreview?.name || (elementId ? elementId : t("blueprint.element.select"));
    ...
    >
      <div className="border-b border-edge px-2 py-1.5">
        <div className="text-2xs text-fg-subtle">
          {resolveBlueprintCategoryLabel(catalog.category, t)}
        </div>
      ...
      <div className="mx-2 my-1.5">
        <button
          ...
          aria-label={
            elementPreview
              ? t("blueprint.element.selectNamed", { name: boundLabel })
...
function BlueprintImageAssetLiteralNodeCard({
  ...
  return (
    ...
    >
      <div className="border-b border-edge px-2 py-1.5">
        <div className="text-2xs tracking-wide text-fg-subtle">
          {resolveBlueprintCategoryLabel(catalog.category, t)}
        </div>
        <div className="font-medium leading-tight text-fg">
          {resolveBlueprintNodeTitle(catalog.displayName, t)}
...
export function BlueprintFlowNode(props: NodeProps) {
  ...
  return (
    <BlueprintFlowNodeBreakpointMarker nodeId={readNodeId(props)}>
      {body}
    </BlueprintFlowNodeBreakpointMarker>
  );
}
...
function BlueprintFlowNodeBreakpointMarker(props: {
  nodeId: string;
  children: ReactNode;
}): ReactNode {
...
function BlueprintFlowNodeCard({ data, selected }: NodeProps) {
  ...
  const showAddPinRow = Boolean(catalog.supportsDynamicInputPins) && Boolean(onAddDynamicInputPin);
  ...
  const showCardInspector =
    Boolean(onPatchNodeParam) && inspectorParams.length > 0 && !showCustomCardInspector;
