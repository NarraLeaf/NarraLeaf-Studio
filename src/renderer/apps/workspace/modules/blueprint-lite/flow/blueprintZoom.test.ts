...
describe("computeBlueprintZoomViewport", () => {
    it("shows the whole graph in contain, with the breathing room the canvas opens at", () => {
        const v = computeBlueprintZoomViewport({ mode: "contain", bounds: BOUNDS, container: CONTAINER, range: OPEN })!;
    ...
    it("leaves no empty side in cover, and no padding either", () => {
        const v = computeBlueprintZoomViewport({ mode: "cover", bounds: BOUNDS, container: CONTAINER, range: OPEN })!;
    ...
    it("spans the pane horizontally in width, whatever that does to the height", () => {
        const v = computeBlueprintZoomViewport({ mode: "width", bounds: BOUNDS, container: CONTAINER, range: OPEN })!;

        expect(v.zoom).toBeCloseTo(1000 / ((1 + BLUEPRINT_FIT_PADDING) * 800), 5);
    });

    it("draws one graph pixel per screen pixel in actual", () => {
        const v = computeBlueprintZoomViewport({ mode: "actual", bounds: BOUNDS, container: CONTAINER, range: OPEN })!;
    ...
    it("centres the graph in every mode, wherever its nodes happen to sit", () => {
        for (const mode of ["contain", "cover", "width", "actual"] as const) {
            const v = computeBlueprintZoomViewport({ mode, bounds: BOUNDS, container: CONTAINER, range: OPEN })!;
    ...
    it("refuses an empty graph and a pane that has not been laid out", () => {
        ...
        expect(computeBlueprintZoomViewport({ mode: "contain", bounds: empty, container: CONTAINER, range: RANGE }))
            .toBeNull();
        expect(
            computeBlueprintZoomViewport({
                mode: "contain",
