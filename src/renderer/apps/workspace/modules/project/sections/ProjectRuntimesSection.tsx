...
export function ProjectRuntimesSection({ uiService }: ProjectSectionProps) {
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            const names = [...new Set([...listKnownPuppetRuntimes().map(runtime => runtime.backend), ...onDisk])];
            const rows = await Promise.all(names.map(async backend => ({
                backend,
                known: knownPuppetRuntimeFor(backend),
                state: await readPuppetRuntimeInstallState(context.project, backend)
                    .catch((): PuppetRuntimeInstallState => ({ status: "absent" })),
            })));
            if (active) {
    ...
    const remove = useCallback(async (row: RuntimeRow) => {
                    ...
                    >
                        ...
                        <button
                            ...
                            onClick={() => setInstalling(
                                row.known ? { kind: "known", id: row.known.id } : { kind: "custom", suggestedName: row.backend },
                            )}
                            {...freeze.writes(false, row.state.status === "installed"
