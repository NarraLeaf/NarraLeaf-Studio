...
export function ProjectIconsSection({
  projectService,
  uiService,
  onConfigChange
}: ProjectSectionProps) {
  ...
  const refresh = useCallback(
    async (next?: ProjectIconSet) => {
      ...
      try {
        ...
        if (persisted.master) {
          const url = await toObjectUrl(
            projectService,
            persisted.master.path,
            persisted.master.mediaType
          );
          if (url) {
            loaded.master = url;
          }
  ...
  const importInto = useCallback(
    async (slot: "master" | ProjectIconTarget) => {
      setBusy(true);
      try {
  ...
  const editSpec = useCallback(
    (target: ProjectIconTarget, patch: Partial<ProjectIconSpec>) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <Slider
              ...
              onValueCommit={(value) => {
                ...
                setInsetDraft(value);
                void editSpec(selected, { inset: value / 100 }).finally(() => setInsetDraft(null));
              }}
            ...
            <div className="flex items-center gap-2">
              ...
              <ColorPickerTrigger
                ...
                onCommit={(value) => {
                  ...
                  void editSpec(selected, { background: hex }).finally(() =>
                    setBackgroundDraft(null)
                  );
                }}
              />
      ...
      >
        {url ? (
          <img
            src={url}
            alt=""
            style={{ width: artwork, height: artwork }}
