...
export function ProjectLintingSection({
  projectService,
  uiService,
  config,
  onConfigChange
  ...
  const commit = useCallback(
    async (field: string, patch: Partial<LintingConfiguration>) => {
      if (saving) {
        return;
      }
  ...
  const setRuleSeverity = useCallback(
    (rule: LintRule, next: LintRuleSeverity) => {
  ...
  const setRuleOption = useCallback(
    (rule: LintRule, key: string, spec: LintRuleOptionSpec, next: string | number) => {
  ...
  const renderRule = (rule: LintRule) => {
    ...
    const optionValues =
      optionSpecs.length > 0 ? resolveRuleOptions(rule, linting.options[rule.id]) : {};
