...
function DependencyRow({ entry }: { entry: DependencyResolutionEntry }) {
  ...
  const meta = [
    ...
    t("project.dependencies.meta.installed", {
      version: installedVersion ?? t("project.dependencies.meta.notInstalled")
    }),
  ...
  return (
    <section className="rounded-md border border-edge bg-fill-subtle p-3">
      <div className="flex items-center justify-between gap-3">
        ...
        {needsAttention ? (
          <span
            className={`shrink-0 text-2xs font-medium ${DEPENDENCY_STATUS_TEXT_STYLES[status]}`}
          >
            {suppressed
              ? t("project.dependencies.status.disabled")
              : t(DEPENDENCY_STATUS_LABEL_KEYS[status])}
          </span>
        ) : null}
      </div>
