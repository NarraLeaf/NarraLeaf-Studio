...
export function ProjectDetailsSection({
  ...
  const commitMetadata = useCallback(
    async (key: MetadataTextKey, value: string) => {
      const next = await projectService.updateProjectMetadata({
        [key]: value
  ...
  return (
    <label className="grid gap-1.5" data-tip={frozen["data-tip"]}>
      <div className="flex items-center gap-1.5">
        ...
        {required ? (
          <span className="text-2xs text-fg-subtle">{t("project.details.required")}</span>
        ) : null}
