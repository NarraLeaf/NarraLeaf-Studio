 ...
 */
export async function bakeProjectIcons(
  io: IconBakeIO,
  set: ProjectIconSet
): Promise<IconBakeReport> {
  ...
  for (const output of PROJECT_ICON_OUTPUTS) {
    ...
    const fingerprint = projectIconFingerprint({
      sourceHash: fnv1a64BytesHex(bytes),
      spec,
      output
    });
    ...
    if (!decoded.has(source.path)) {
      decoded.set(
        source.path,
        await decodeIcon(bytes, source.mediaType, source.path).catch(() => null)
      );
    }
