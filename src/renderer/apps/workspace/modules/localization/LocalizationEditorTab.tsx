...
export function LocalizationEditorTab({
  payload,
  active
}: EditorComponentProps<LocalizationEditorTabPayload | undefined>) {
  ...
  useEffect(() => {
    ...
    const extract = () => {
      try {
        const characterRows: TableRow[] = extractCharacterTranslationRows(characters).map(
          (row) => ({
            unitId: row.unitId,
            sourceText: row.sourceText,
            interpolationCount: 0,
        ...
        const storyRows: TableRow[] = localizationService
          .extractRows(storyService.getStoryDocument(storyId))
          .map((row) => ({
            unitId: row.unitId,
            sourceText: row.sourceText,
  ...
  const handleTargetChange = useCallback(
    (row: TranslationTableRow, target: string) => {
  ...
  const handleKeySourceChange = useCallback(
    (row: TranslationTableRow, sourceText: string) => {
  ...
  const handleAddKey = useCallback(
    (name: string, sourceText: string): boolean => {
  ...
  const modeOptions: { key: EditorMode; label: string; icon: React.ReactNode }[] = [
    {
      key: "translate",
      label: t("workspace.localization.table.modeTranslate"),
      icon: <PenLine className="h-3.5 w-3.5" />
    },
    {
      key: "review",
      label: t("workspace.localization.table.modeReview"),
  ...
  const filterOptions: SelectOption[] = useMemo(
    () => [
      ...
      {
        value: "untranslated",
        label: `${t("workspace.localization.table.filterUntranslated")} (${counts.untranslated})`
      },
      {
        value: "stale",
        label: `${t("workspace.localization.table.filterStale")} (${counts.stale})`
      },
      {
        value: "completed",
        label: `${t("workspace.localization.table.filterCompleted")} (${counts.completed})`
      }
    ],
    [counts, t]
  );
  ...
  const reviewFilterOptions: SelectOption[] = useMemo(
    () => [
      ...
      {
        value: "reviewed",
        label: `${t("workspace.localization.table.reviewFilterReviewed")} (${counts.reviewed})`
      },
      {
        value: "unreviewed",
        label: `${t("workspace.localization.table.reviewFilterUnreviewed")} (${counts.all - counts.reviewed})`
      }
    ],
    [counts, t]
  );
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="text-2xs text-fg-subtle">
              {t("workspace.localization.table.storyLabel")}
            </span>
            <Select
              ...
              <button
                ...
                data-tip={
                  option.key === "review" && counts.pending > 0
                    ? t("workspace.localization.table.reviewPendingCount", {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {stories.length === 0 && sourceValue !== UI_SOURCE_VALUE ? (
          <EmptyMessage
            ...
            text={t("workspace.localization.table.noStories")}
          />
          ...
          <EmptyMessage
            ...
            text={t("workspace.localization.table.reviewAllClear")}
          />
        ) : visibleRows.length === 0 && !showKeysExtras ? (
          <EmptyMessage
            icon={<SplitSquareVertical className="h-5 w-5" />}
            text={t("workspace.localization.table.emptyFilter")}
          />
        ) : (
