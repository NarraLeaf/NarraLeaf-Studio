 ...
 */
const FREEZE_READ_ONLY_LOCALIZATION_MENU_IDS: ReadonlySet<string> = new Set([
  "export-translations"
]);
...
export function LocalizationPanel({ panelId }: PanelComponentProps) {
  ...
  const projectName = useMemo(() => {
    ...
    return (
      context.services.get<ProjectService>(Services.Project).getProjectConfig().name?.trim() ?? ""
    );
  }, [context, isInitialized]);
  ...
  const handleAddLocale = useCallback(async () => {
    ...
    try {
      await localizationService.addLocale({
        code,
        displayName: nameDraft.trim() || autonymFor(code)
      });
      cancelAddLocale();
    } catch (error) {
  ...
  const writeExport = useCallback(
    async (code: string, format: TranslationExchangeFormat, scope: TranslationExportScope) => {
      ...
      try {
        ...
        const selection = await appPrivilegedFacade.fs.selectSaveFile(`${code}.${extension}`, [
          extension
        ]);
        ...
        uiService?.showNotification(
          t("workspace.localization.exchange.exportDone", {
  ...
  const handleExport = useCallback(
    async (code: string, displayName: string) => {
      ...
      let scope: TranslationExportScope =
        pendingCount > 0 && pendingCount < rows.length ? "pending" : "all";
      const dialogId = uiService.dialogs.show({
  ...
  const handleImport = useCallback(
    async (code: string, displayName: string) => {
      ...
      try {
        ...
        if (!content.ok) {
          throw new Error(
            content.error.message || t("workspace.localization.exchange.importFailed")
          );
        }
        ...
        const summary = localizationService.applyImportedRows(
          code,
          parsed.rows,
          currentSourceByUnit
        );
        uiService.showNotification(
          t("workspace.localization.panel.importSummary", { ...summary }),
          "success"
        );
              ...
              {locales.map((locale) => {
                ...
                return (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    data-tip={t("workspace.localization.panel.openTable")}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        data-tip={t("workspace.localization.panel.more")}
                        aria-label={t("workspace.localization.panel.more")}
                        ...
                        <span className="flex items-center gap-2 text-2xs text-fg-subtle">
                          ...
                          {progress && progress.stale > 0 ? (
                            <span className="text-warning">
                              {t("workspace.localization.panel.staleCount", {
          ...
          {!addingLocale ? (
            ...
            >
              ...
              <button
                ...
                data-tip={t("workspace.localization.panel.confirm")}
