...
function channelLabel(
  ...
  channel: ConsoleChannelDefinition
): string {
...
): Record<ConsoleChannelId, ConsoleEntry[]> {
...
export function ConsolePanel({ panelId }: PanelComponentProps) {
  ...
  const [channels, setChannels] = useState<readonly ConsoleChannelDefinition[]>(
    () => consoleService?.getChannels() ?? []
  );
  const [activeChannel, setActiveChannel] = useState<ConsoleChannelId>("build");
  const [visibleLevels, setVisibleLevels] = useState<Set<ConsoleLogLevel>>(
    () => new Set(DEFAULT_VISIBLE_LEVELS)
  );
  const [filterMenuOpen, setFilterMenuOpen] = useState(false);
  const [entriesByChannel, setEntriesByChannel] = useState<
    Record<ConsoleChannelId, ConsoleEntry[]>
  >(() => readServiceEntries(consoleService));
  const [progressByChannel, setProgressByChannel] = useState<
    Record<ConsoleChannelId, ConsoleProgress | null>
  >({});
  ...
  const handleExport = () => {
    ...
    const label = activeChannelDef
      ? channelLabel(t, activeChannelDef)
      : t("console.outputFallback");
    if (entries.length === 0) {
    ...
    void (async () => {
      ...
      if (!result.success) {
        uiService?.showNotification(
          t("console.exportFailed", { error: result.error ?? "" }),
          "error"
        );
        return;
      }
          ...
          {channels.map((channel) => {
            ...
            return (
              <button
                ...
                data-tip={channelDescription(t, channel)}
                aria-label={channelDescription(t, channel)}
                className={`relative flex min-w-28 cursor-default items-center justify-center gap-2 px-4 text-xs transition-colors ${
                  active ? "bg-surface text-fg" : "text-fg-muted hover:bg-fill-subtle hover:text-fg"
                }`}
                onClick={() => {
              ...
              >
                ...
                {active ? (
                  <span
                    className="pointer-events-none absolute inset-x-0 bottom-0 h-0.5 bg-primary/70"
                    aria-hidden
                  />
                ) : null}
              </button>
  ...
  return (
    ...
    >
      {progress.indeterminate ? (
        ...
        <>
          <div
            className={`nl-motion-keep absolute inset-y-0 animate-progress-indeterminate-1 rounded-full ${barColor}`}
          />
          <div
            className={`nl-motion-keep absolute inset-y-0 animate-progress-indeterminate-2 rounded-full ${barColor}`}
          />
        </>
      ) : (
        <div
    ...
    >
      ...
      {entries.map((entry, index) => (
        ...
        >
          ...
          {entryText(entry).length === 0 ? (
            <span className="text-fg-subtle">{t("console.entryEmpty")}</span>
          ) : null}
        </div>
      ))}
    </div>
