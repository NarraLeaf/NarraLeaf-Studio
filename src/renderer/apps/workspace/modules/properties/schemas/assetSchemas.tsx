 ...
 */
function createCommonAssetFields<T extends AssetType>(
  t: TranslateFn
): FieldDefinition<AssetEditorContext<T>>[] {
  return [
    {
      id: "replaceContent",
      type: "custom",
    ...
    },
    {
      ...
      component: ({ data }) => (
        <AssetReferencesSection assetId={data.asset.id} assetType={data.asset.type} />
      ),
      order: 400
    }
  ];
  ...
  createPropertyEditorSchema<AssetEditorContext<AssetType.Model>>({
    ...
    fields: [
      ...
      },
      {
        ...
        options: (ctx) => {
          const meta = ctx.metadata?.metadata as ModelAssetMetadata | undefined;
          return listModelEntryChoices(meta?.files ?? []).map((path) => ({
            label: path,
            value: path
          }));
        },
    ...
    },
    {
      ...
      getValue: (ctx) => {
        ...
        return parts.length > 1
          ...
          : t("properties.asset.other.unknown");
      }
    },
    {
      label: t("properties.asset.info.size"),
