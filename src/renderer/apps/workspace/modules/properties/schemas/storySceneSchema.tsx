...
function SceneDefaultBackgroundField({ data }: CustomFieldProps<StorySceneEditorContext>) {
  ...
  const label =
    asset?.name ?? (assetId ? t("story.background.missingImage") : t("story.background.none"));
    ...
    <div>
      ...
      <div className="mt-2 flex gap-2">
        <button
          ...
          onClick={() => setSelectorOpen(true)}
        >
          <span className={["truncate", asset ? "" : "italic text-fg-subtle"].join(" ")}>
            {label}
          </span>
        </button>
        <button
...
function SceneBackgroundMusicField({ data }: CustomFieldProps<StorySceneEditorContext>) {
  ...
  const asset = bgm?.assetId
    ? (assetsService?.getAssets()[AssetType.Audio]?.[bgm.assetId] ?? null)
    : null;
  const label =
    asset?.name ?? (bgm?.assetId ? t("story.music.missingAudio") : t("story.music.none"));
  ...
  const trackOptions = [
    {
      ...
      label: t("storyInspector.audio.trackDefault", {
        name: resolveAudioTrack(tracks, undefined, "bgm").name
      })
    },
    ...
    <div>
      <div className="flex gap-2">
        ...
        <button
          ...
          className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-edge bg-fill-subtle text-fg-muted hover:border-danger/40 hover:text-danger disabled:cursor-not-allowed disabled:opacity-40"
          disabled={!bgm}
          data-tip={t("story.sceneEditor.clearSceneMusic")}
          aria-label={t("story.sceneEditor.clearSceneMusic")}
        ...
        <div className="mt-2 space-y-2">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              step={0.05}
              value={bgm.volume ?? 1}
              data-tip={t("story.sceneEditor.sceneMusicVolume")}
              aria-label={t("story.sceneEditor.sceneMusicVolume")}
          ...
          <div className="flex items-center gap-3">
            ...
            <label className="flex min-w-0 flex-1 items-center justify-end gap-1.5 text-xs text-fg-muted">
              ...
              <input
                ...
                onChange={(event) =>
                  patch({ fadeMs: Math.max(0, Number(event.target.value) || 0) * 1000 })
                }
              />
            </label>
          </div>
...
function buildRegionHint(
  ...
  if (region.loopStartMs !== undefined && region.loopStartMs !== (region.inMs ?? 0)) {
    return t("story.sceneEditor.sceneMusicIntroLoop", {
      from,
      loop: formatSeconds(region.loopStartMs),
      to
    });
  }
