...
function createLayoutInspectorSchema(
  ...
  const readElementSurfaceTopLeft = (element: UIElement) => {
    ...
    return document.elements[element.id]
      ? getElementSurfaceTopLeft(document, element.id)
      : element.layout;
  };
        ...
        render: ({ data, onSaving }: InlineRowItemContext<UIInspectorData>) => {
          ...
          return (
            <button
              ...
              aria-label={
                pressed ? t("properties.layout.unlockAspect") : t("properties.layout.lockAspect")
              }
              data-tip={
                pressed ? t("properties.layout.unlockAspect") : t("properties.layout.lockAspect")
              }
 ...
 */
function createElementAnimationField(
  element: UIElement,
  t: TranslateFn
...
function mergeInspectorWithLayoutSchema(
  ...
  if (inspectorSchema.tabs && inspectorSchema.tabs.length > 0) {
    const targetTabId = inspectorSchema.defaultTabId ?? inspectorSchema.tabs[0]?.id ?? null;
...
function LinkedComponentParamsSection({ data }: { data: UIInspectorData }) {
  return (
    <LinkedComponentParamsField element={data.element} documentService={data.documentService} />
  );
}
...
export function PropertiesPanel({ panelId, payload }: PanelComponentProps) {
  ...
  const [storyMotionSelection, setStoryMotionSelection] =
    useState<StoryMotionKeyframeSelection | null>(null);
  ...
  const activeComponentDefinition = useMemo(() => {
    ...
    return (
      documentService.getDocument().components?.find((item) => item.id === componentId) ?? null
    );
  ...
  const activeSceneSurface = useMemo(() => {
    ...
    return (
      documentService.getDocument().surfaces.find((surface) => surface.id === activeSceneId) ?? null
    );
  }, [activeSceneId, documentService, documentVersion]);
  ...
  useEffect(() => {
    ...
    const setSelectionState = (selection: SelectionState) => {
      ...
      setActiveAsset(
        !motionSelection && !story && selection.type === "asset" ? (selection.data as Asset) : null
      );
      setActiveCharacter(
        !motionSelection && !story && selection.type === "character"
          ? (selection.data as Character)
          : null
      );
      setAssetMetadata(null);
      setUISelection(
        !motionSelection && !story && isUIElementSelection(selection)
          ? (selection.data as UIElementSelection)
          : null
      );
      const sceneId =
        ...
        return (
          <PropertyEditor
            ...
            data={{
              element,
              elements,
              documentService: inspectorDocumentService,
              surfaceId: deferredUiSelection.surfaceId
            }}
          />
        );
      }
  ...
  const uiSelectionDiagnosticStrip = useMemo(() => {
    ...
    const diagnosticDocumentService = componentId
      ? createComponentDocumentServiceAdapter(documentService, componentId)
      : documentService;
    const all = collectSurfaceDiagnostics(
      diagnosticDocumentService.getDocument(),
      deferredUiSelection.surfaceId,
      {
              ...
              {d.elementId ? (
                ...
                >
                  ...
                  <span className="ml-1 text-2xs text-primary">
                    {t("properties.diagnostics.selectOnCanvas")}
                  </span>
                </button>
              ) : (
  ...
  const characterContext = useMemo<CharacterEditorContext | null>(() => {
    ...
    return {
      ...
      thumbnailUrl,
      poses: activeCharacter.profile.appearance
        .getPoses()
        .map((pose) => ({ id: pose.id, name: pose.name }))
    };
  ...
  const storyContent = useMemo(() => {
    ...
    return storySceneContext ? (
      <PropertyEditor schema={storySceneSchema} data={storySceneContext} />
    ) : null;
  ...
  return (
    <div className="nl-editor-surface h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-edge">
        ...
        {panelSubtitle && <span className="text-xs text-fg-subtle">{panelSubtitle}</span>}
      </div>
