...
export function ColorPickerTrigger({
    ...
    setColorState((prev) => {
      ...
      if (incomingHex === prev.hex) {
        if (
          ...
          Math.abs((resolvedValue.alpha ?? 1) - prev.alpha) > 1e-6
        ) {
  ...
  useLayoutEffect(() => {
    ...
    const topLimit = Math.max(
      PANEL_EDGE_PADDING,
      viewportHeight - rect.height - PANEL_EDGE_PADDING
    );
    const leftLimit = Math.max(PANEL_EDGE_PADDING, viewportWidth - rect.width - PANEL_EDGE_PADDING);
    const clampTop = (value: number) => Math.min(Math.max(value, PANEL_EDGE_PADDING), topLimit);
    const clampLeft = (value: number) => Math.min(Math.max(value, PANEL_EDGE_PADDING), leftLimit);
  ...
  const displayColor = useMemo(
    () =>
      colorValueToCss({
        ...
        link: activeBrandLink ?? undefined
      }),
  ...
  const mapCoordinates = useMemo(() => {
    const { s, v } = rgbToHsv(currentRgb.r, currentRgb.g, currentRgb.b);
    return {
      saturation: s * 100,
      value: v * 100
    };
  ...
  const handleHslChange = useCallback(
    (channel: "h" | "s" | "l", raw: string) => {
      ...
      if (!Number.isFinite(parsed)) return;
      const nextH = channel === "h" ? clamp(parsed, 0, 360) : colorState.hue;
      const nextS = channel === "s" ? clamp(parsed, 0, 100) : colorState.saturation;
      const nextL = channel === "l" ? clamp(parsed, 0, 100) : colorState.lightness;
      applyColorState(() => ({
        hue: nextH,
        saturation: nextS,
        lightness: nextL
      ...
      <div className="grid grid-cols-3 gap-2 mt-3">
        {[
          ...
          <div key={channel} className="space-y-1">
            ...
            <EnhancedInput
              value={String(value)}
              onChange={(next) => handleHslChange(channel as "h" | "s" | "l", next)}
              inputMode="decimal"
              readOnly={readOnly}
            />
          </div>
    ...
    >
      ...
      <div className="mt-3 space-y-2">
        ...
        <input
          ...
          disabled={readOnly}
          onChange={(event) => applyColorState(() => ({ hue: Number(event.target.value) }))}
          ...
          <div className="flex flex-wrap gap-1">
            {brandColors.map((color) => (
              <button
                ...
                data-tip={brandColorLabel(color)}
                aria-label={brandColorLabel(color)}
  ...
  const triggerContent = (
    ...
    >
      ...
      {displayMode === "icon-hex" && (
        <span className="text-xs text-fg font-mono tracking-wide">{colorState.hex}</span>
      )}
    </button>
  );
