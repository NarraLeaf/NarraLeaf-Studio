...
const secondaryGhostButtonClass = `${HEADER_ACTION_BASE} border border-primary/30 bg-primary/5 text-primary hover:bg-primary/15 hover:border-primary/50`;
...
function ThumbnailFieldInner<TData>({ field, data, onSaving }: ThumbnailFieldProps<TData>) {
  ...
  const [cropSourceSize, setCropSourceSize] = useState<{ width: number; height: number } | null>(
    null
  );
  ...
  const serviceAssets =
    context && isInitialized
      ? context.services.get<ServiceAssetsService>(Services.ServiceAssets)
      : null;
  ...
  useEffect(() => {
    ...
    const loadThumb = async () => {
      try {
        ...
        const externalUrl = await Promise.resolve(
          fieldRef.current.getThumbnailUrl(dataRef.current)
        );
  ...
  return (
    <div className={field.className}>
      {field.label && (
        <label className="block text-xs font-medium text-fg-muted mb-1">{field.label}</label>
      )}
