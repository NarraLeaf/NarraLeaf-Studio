...
export function InputGroupField<TData>({ field, data, onSaving }: InputGroupFieldProps<TData>) {
  ...
  const containerClass = ["flex", shouldWrap ? "flex-wrap" : "flex-nowrap", "items-stretch"].join(
    " "
  );
    ...
    <FieldLayout field={field} className={field.className}>
      ...
      >
        {field.inputs.map((item) => {
          ...
          const itemClassNames = ["flex flex-col flex-1 min-w-0 space-y-1", item.className]
            .filter(Boolean)
            .join(" ");
          ...
          return (
            <div key={item.id} className={itemClassNames}>
              {item.label && (
                <span className="text-2xs tracking-wider text-fg-subtle">{item.label}</span>
              )}
              <EnhancedInput
                value={value}
