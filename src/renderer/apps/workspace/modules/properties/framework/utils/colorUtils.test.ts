...
import { colorValueToCss, normalizeHex, normalizeHexInputDraft, parseColorValue, serializeColorValue } from "./colorUtils";
...
describe("colorUtils resolves brand links", () => {
    ...
    it("stores an ordinary colour exactly as it always did", () => {
        expect(serializeColorValue({ hex: "#40A8C4", alpha: 1 })).toBe(colorValueToCss({ hex: "#40A8C4", alpha: 1 }));
        expect(serializeColorValue({ hex: "#40A8C4", alpha: 0.5 })).toBe(colorValueToCss({ hex: "#40A8C4", alpha: 0.5 }));
