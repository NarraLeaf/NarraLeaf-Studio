...
export function BindablePropertyField<TData>({ field, data, onSaving, children }: Props<TData>) {
  ...
  const child = isValidElement(children)
    ? cloneElement(
        children as ReactElement<{ field: typeof field; data: TData; onSaving: typeof onSaving }>,
        {
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {bp.status === "bound" || bp.status === "broken" ? (
          <span className="text-2xs text-fg-muted">
            {bp.fieldLabel ? (
              <>
                {t("properties.binding.fieldLabel")}{" "}
                <span className="text-fg">{bp.fieldLabel}</span>
              </>
            ) : (
        ...
        {bp.status === "literal" ? (
          <>
            <button
              ...
              onClick={() => openPicker()}
              data-tip={!bp.canBind ? t("properties.binding.notReady") : undefined}
            >
              ...
              <div className="absolute left-0 top-full z-50 mt-1 w-[min(100%,20rem)] rounded-lg border border-edge bg-surface-overlay p-3 shadow-xl">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <span className="text-2xs font-medium text-fg">
                    {t("properties.binding.bindProperty")}
                  </span>
                  <button
                    type="button"
                ...
                <div className="mb-2 max-h-32 overflow-auto rounded-md border border-edge">
                  {filteredCandidates.length === 0 ? (
                    <p className="px-2 py-2 text-2xs text-fg-subtle">
                      {t("properties.binding.noMatches")}
                    </p>
                  ) : (
