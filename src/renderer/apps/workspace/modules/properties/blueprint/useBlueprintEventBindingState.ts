...
export function useBlueprintEventBindingState(data: UIInspectorData): {
  ...
  const snapshot = useMemo(() => {
    ...
    const blueprint = blueprintId
      ? localBp.getBlueprintDocument().blueprints[blueprintId]
      : undefined;
    ...
    (uiEventName: string) => {
      ...
      if (snapshot.blueprint?.program.kind === "graph") {
        ...
        localBp?.ensureEventGraph(
          ...
          uiEventName,
          defs.find((def) => def.id === uiEventName)?.displayName
        );
      }
      openBlueprint({
  ...
  const rows: BlueprintEventBindingRow[] = useMemo(() => {
    return defs.map((def) => ({
      ...
      legacyGraphEventId:
        getWiredBlueprintEventRef(snapshot.element, def.id, snapshot.blueprintId)?.eventId ?? null,
      openEventGraph: () => openWiredEventGraphTab(def.id)
    }));
  }, [
    defs,
    openWiredEventGraphTab,
    snapshot.blueprint?.program.kind,
    snapshot.blueprintId,
    snapshot.element,
