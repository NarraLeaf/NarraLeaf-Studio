...
import {
  ...
  hasUpdate,
  isCompatible
} from "@/lib/plugins/ui/pluginPresentation";
...
export function PluginsPanel({
  panelId,
  payload
}: PanelComponentProps<PluginsPanelPayload | undefined>) {
  ...
  const uninstall = useCallback(
    (pluginId: string) => {
      ...
      setDetailId((current) =>
        current && registry?.some((entry) => entry.id === current) ? current : null
      );
    },
    [catalog, registry]
  );
  ...
  const openRowMenu = useCallback(
    (event: React.MouseEvent, plugin: PluginListItem) => {
      ...
      if (plugin.status === "needsAuthorization") {
        items.push({
          id: "authorize",
          label: t("plugins.authorize"),
          onClick: () => catalog.approve(plugin.pluginId)
        });
      ...
      if (hasUpdate(plugin, entry) && isCompatible(entry)) {
        items.push({
          id: "update",
          label: t("plugins.store.update"),
          onClick: () => catalog.installFromStore(plugin.pluginId)
        });
      }
      ...
      if (live && activity.activityOf(plugin) !== "off" && plugin.manifest.entries.studio) {
        items.push({
          id: "reload",
          label: t("plugins.workspace.reload"),
          onClick: () => reload(plugin.pluginId)
        });
      }
      ...
      const disabled = busy
        ? items.map((item) => ("separator" in item ? item : { ...item, disabled: true }))
        : items;
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {tab === "installed" ? (
          ...
          <PanelEmpty
            message={t("plugins.store.offline")}
            detail={registryError}
            onRetry={catalog.refreshAll}
            retryLabel={t("plugins.store.retry")}
          />
        ) : registry === null ? (
        ...
        {detailId && (detailInstalled || detailEntry) ? (
          ...
          >
            <PluginDetailsPage
              ...
              loadError={
                detailInstalled ? (activity.session.failed[detailInstalled.pluginId] ?? null) : null
              }
              busy={busy}
              canReload={live}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate text-2xs text-fg-subtle">
          {entry.description || entry.publisher}
        </div>
      </div>
      {!compatible ? (
        <Badge tone="warning">
