...
import { workspacePluginSession, type WorkspacePluginActivity } from "@/lib/plugins/workspacePluginSession";
 ...
 * a panel body.
 */
function serviceOrNull<T extends Service>(ctx: WorkspaceContext | null, service: Services): T | null {
    if (!ctx) {
        return null;
    }
    try {
...
export function useWorkspacePluginActivity(): WorkspacePluginActivityState {
    ...
    const session = useSyncExternalStore(
        useCallback(
            (listener: () => void) => (context ? workspacePluginSession(context).subscribe(listener) : () => {}),
            [context],
        ),
        useCallback(
    ...
    const suppressed = useMemo<ReadonlySet<string>>(
        () => (resolution?.suppressedPluginIds.length ? new Set(resolution.suppressedPluginIds) : NO_SUPPRESSION),
        [resolution],
    );

    const running = useMemo(() => new Set(session.running), [session.running]);

    const activityOf = useCallback((plugin: PluginListItem): PluginActivity => {
