...
describe("storyMotionPresets", () => {
  ...
  it("keeps ids and generated keyframe ids unique, and builds value-stably", () => {
    ...
    for (const preset of STORY_MOTION_PRESETS) {
      ...
      for (const track of timeline.tracks) {
        ...
        expect(new Set(keyframeIds).size, `${preset.id}/${track.property}`).toBe(
          keyframeIds.length
        );
        expect(
          keyframeIds.every((id) => id.length > 0),
          preset.id
        ).toBe(true);
      }
    ...
    for (const preset of STORY_MOTION_PRESETS) {
      for (const track of preset.build().tracks) {
        ...
        for (const keyframe of track.keyframes) {
          ...
          expect(
            Object.keys(keyframe.value as object)
              .sort()
              .join(",")
          ).toMatch(/^(xoffset|yoffset|xoffset,yoffset)$/);
        }
      }
    }
  });
  ...
  it("names every category, and files every preset under a named one", () => {
    ...
    for (const category of STORY_MOTION_PRESET_CATEGORIES) {
      expect(
        STORY_MOTION_PRESETS.some((preset) => preset.category === category),
        category
      ).toBe(true);
    }
  });
  ...
  it("keeps the catalogues and the table in exact correspondence", () => {
    ...
    expect(Object.keys(motionEn.presetCategory).sort()).toEqual(
      [...STORY_MOTION_PRESET_CATEGORIES].sort()
    );
    expect(Object.keys(motionZh.presetCategory).sort()).toEqual(
      [...STORY_MOTION_PRESET_CATEGORIES].sort()
    );
  });

  it("falls back to the first preset for an unknown id", () => {
    expect(createStoryMotionPresetTimeline("not-a-preset")).toEqual(
      STORY_MOTION_PRESETS[0].build()
    );
  ...
  it("parks every preset on a pose that is both off-neutral and visible", () => {
    ...
    for (const preset of STORY_MOTION_PRESETS) {
      ...
      expect(
        parked.opacity,
        `${preset.id} parks at an invisible frame (t=${signature})`
