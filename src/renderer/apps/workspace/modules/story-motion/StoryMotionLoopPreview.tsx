 ...
 */
export function useStoryMotionLoopTimeMs(
  durationMs: number,
  active: boolean,
  restTimeMs = 0
): number {
...
export function StoryMotionLoopPreview(props: {
  ...
  const preview = useMemo(
    () => sampleStoryMotionPreview(props.timeline, timeMs),
    [props.timeline, timeMs]
  );
  const scale = Math.min(
    props.box.width / props.stageSize.width,
    props.box.height / props.stageSize.height
  );
