...
import {
  ...
  type StoryMotionPreviewDragMode
} from "./StoryMotionStagePreview";
...
export function createStoryMotionEditorTab(
  payload: StoryMotionEditorPayload
): EditorTabDefinition<StoryMotionEditorPayload> {
  return {
...
export function StoryMotionEditorTab({
  tabId,
  payload,
  active
}: EditorTabComponentProps<StoryMotionEditorPayload>) {
  ...
  const [keyframeDrag, setKeyframeDrag] = useState<{ keyframeId: string; timeMs: number } | null>(
    null
  );
  ...
  const [selectedAddProperty, setSelectedAddProperty] =
    useState<StoryAnimationTrackProperty>("position");
  const [previewOverride, setPreviewOverride] = useState<Partial<
    ReturnType<typeof sampleStoryMotionPreview>
  > | null>(null);
  const timelineScrollRef = useRef<HTMLDivElement | null>(null);
  const timelineFitInitializedRef = useRef<string | null>(null);
  const playheadRef = useRef(0);
  const durationRef = useRef(0);
  const lastObservedTimelineRef = useRef<{ json: string; timeline: StoryAnimationTimeline } | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    setLoadError(null);
    void storyService
      .loadAnimationAsset(payload.animationId)
      .then((next) => {
        if (!disposed) {
          setAsset(next);
    ...
    const unsubscribe = storyService.onAnimationsChanged((index) => {
      ...
      void storyService
        ...
        .catch(() => {
          if (!disposed) {
            setAsset((current) =>
              current ? { ...current, name: entry.name, targetKind: entry.targetKind } : current
            );
          }
        });
    });
  ...
  useEffect(() => {
    ...
    let disposed = false;
    void storyService
      .loadStory(payload.actionContext.storyId)
      .then((next) => {
        if (!disposed) {
          setDocument(next);
        }
  ...
  const timelineWidth = Math.max(
    ...
    MIN_TIMELINE_WIDTH,
    durationMs * pxPerMs +
      Math.max(TIMELINE_SCROLL_HEADROOM_PX, (timelineViewport.width - TIMELINE_LEFT_COL_PX) * 0.5)
  );
  const tracks = useMemo(() => orderTracks(timeline.tracks), [timeline.tracks]);
  const selected = useMemo(
    () => findKeyframe(timeline, selectedKeyframeId),
    [selectedKeyframeId, timeline]
  );
  const addPropertyOptions = useMemo<SelectOption[]>(() => {
    const existing = new Set(tracks.map((track) => track.property));
    return STORY_MOTION_PROPERTIES.filter((item) => !existing.has(item.property)).map((item) => ({
      value: item.property,
  ...
  const previewTimeline = useMemo(() => {
    ...
    return {
      ...
      tracks: timeline.tracks.map((track) =>
        track.keyframes.some((keyframe) => keyframe.id === keyframeDrag.keyframeId)
          ? {
              ...track,
              keyframes: track.keyframes.map((keyframe) =>
                keyframe.id === keyframeDrag.keyframeId
  ...
  const previewTarget = useMemo(
    ...
    [
      ...
      asset?.targetKind,
      document,
      payload?.actionContext?.blockId,
      payload?.actionContext?.sceneId
    ]
  );
  ...
  const readEditorPanelState = useCallback(
    () =>
      panelStateService
        ? normalizeStoryMotionEditorPanelState(
            panelStateService.getPanelState<StoryMotionEditorPanelState>(editorStatePanelId)
          )
        : {},
    [editorStatePanelId, panelStateService]
  );

  const persistEditorPanelState = useCallback(
    (patch: Partial<StoryMotionEditorPanelState>) => {
  ...
  useEffect(() => {
    ...
    setSelectedKeyframeId(
      saved.selectedKeyframeId && findKeyframe(timeline, saved.selectedKeyframeId)
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setTimelineViewport((current) =>
        current.width === width && current.scrollLeft === scrollLeft
          ? current
          : { width, scrollLeft }
      );
    };
  ...
  const updateAsset = useCallback(
    (updater: (asset: StoryAnimationAsset) => StoryAnimationAsset) => {
  ...
  const updateTimeline = useCallback(
    (updater: (timeline: StoryAnimationTimeline) => StoryAnimationTimeline) => {
      updateAsset((current) => ({
        ...current,
  ...
  const scrubToClientX = useCallback(
    (clientX: number, rect: DOMRect, snap: boolean) => {
      const raw = (clientX - rect.left) / pxPerMs;
      const value = snap
        ? snapStoryMotionTimeToFrame(raw, STORY_MOTION_FPS)
        : clampStoryMotionTimeMs(raw);
  ...
  const startPlayheadDrag = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const onMove = (moveEvent: PointerEvent) =>
        scrubToClientX(moveEvent.clientX, rect, !moveEvent.altKey);
      const onUp = () => {
  ...
  const selectKeyframe = useCallback(
    (track: StoryAnimationTrack, keyframe: StoryAnimationKeyframe) => {
      ...
      const onUp = () => {
        ...
        if (lastTime !== startTime) {
          updateTimeline((current) =>
            updateStoryMotionKeyframe(current, keyframe.id, (currentKeyframe) => ({
              ...currentKeyframe,
              timeMs: lastTime
            }))
          );
  ...
  const addKeyframeAtTime = useCallback(
    (track: StoryAnimationTrack, timeMs: number) => {
  ...
  const handleLaneDoubleClick = useCallback(
    (event: ReactMouseEvent<HTMLDivElement>, track: StoryAnimationTrack) => {
      if ((event.target as HTMLElement | null)?.closest("button")) {
        return;
      }
      const raw = clampStoryMotionTimeMs(
        (event.clientX - event.currentTarget.getBoundingClientRect().left) / pxPerMs
      );
      const timeMs =
        Math.abs(raw - playheadMs) * pxPerMs <= TIMELINE_PLAYHEAD_SNAP_PX
          ? playheadMs
          : event.altKey
            ? raw
            : snapStoryMotionTimeToFrame(raw, STORY_MOTION_FPS);
  ...
  const restoreTimeline = useCallback(
    (snapshot: StoryAnimationTimeline) => {
      if (!storyService || !asset) {
        return;
      }
      ...
      lastObservedTimelineRef.current = {
        json,
        timeline: JSON.parse(json) as StoryAnimationTimeline
      };
      setAsset(nextAsset);
    },
    [asset, storyService]
  ...
  const stepPlayhead = useCallback((frames: number) => {
    setPlayheadMs((current) =>
      Math.min(durationRef.current, stepStoryMotionTimeByFrames(current, frames, STORY_MOTION_FPS))
    );
  }, []);
  ...
  const deleteTrack = useCallback(
    (track: StoryAnimationTrack) => {
      if (
        selectedKeyframeId &&
        track.keyframes.some((keyframe) => keyframe.id === selectedKeyframeId)
      ) {
        setSelectedKeyframeId(null);
      }
      const selection = uiService?.getStore().getSelection();
  ...
  const startPreviewDrag = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>, mode: StoryMotionPreviewDragMode) => {
      if (event.button !== 0) {
        return;
      }
      event.preventDefault();
      ...
      const onUp = () => {
        if (autoKey && latestProperty && latestValue !== null) {
          updateTimeline((current) =>
            upsertStoryMotionKeyframe(current, latestProperty!, playheadMs, latestValue!)
          );
        }
  ...
  const stopPreviewPan = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      const pan = previewPanRef.current;
  ...
  return (
    ...
    >
      <div className="flex h-12 shrink-0 items-center gap-2 border-b border-edge px-3">
        <div
          className="min-w-0 flex-[0_1_320px] truncate text-sm font-medium text-fg"
          data-tip={asset.name}
        >
          {asset.name}
        </div>
        ...
        <input
          ...
          onChange={(event) =>
            setPlayheadMs(clampNumber(Number(event.target.value), 0, durationMs, 0))
          }
        />
        ...
        <div className="flex min-h-0 flex-col">
          ...
          <div className="h-64 shrink-0 border-t border-edge bg-surface">
            <div className="flex h-12 items-center gap-3 border-b border-edge px-3">
              <div className="w-[168px] text-xs font-medium text-fg-muted">
                {t("motion.editor.animatedProperties")}
              </div>
              <Select
                className="w-44"
                size="md"
              ...
              <Button
                ...
                onClick={() =>
                  updateTimeline((current) =>
                    ensureStoryMotionTrack(current, selectedAddProperty, playheadMs)
                  )
                }
            ...
            <div
              ...
              className="h-[calc(100%-48px)] overflow-auto overscroll-contain"
            ...
            >
              ...
              >
                ...
                <div
                  className="sticky top-0 z-30 h-8 border-b border-edge bg-surface-sunken"
                  onPointerDown={startPlayheadDrag}
                >
                  {buildTicks(pxPerMs, timelineWidth, timelineViewport, STORY_MOTION_FPS).map(
                    (tick) => (
                      <div
                        key={tick.timeMs}
                        className="absolute top-0 h-full border-l border-edge"
                        style={{ left: tick.timeMs * pxPerMs }}
                      >
                  ...
                  <div
                    className="absolute top-0 z-20 h-full w-px bg-orange-400"
                  ...
                  <Fragment key={track.id}>
                    <div className="group sticky left-0 z-20 flex h-[34px] items-center gap-2 border-r border-b border-edge-subtle bg-surface-sunken px-3 text-xs text-fg-muted">
                      ...
                      <button
                        ...
                        aria-label={t("motion.editor.addKeyframeAria", {
                          property: getStoryMotionPropertyMeta(track.property).label
                        })}
                      >
                      ...
                      <button
                        ...
                        aria-label={t("motion.editor.deleteTrackAria", {
                          property: getStoryMotionPropertyMeta(track.property).label
                        })}
                      >
                    ...
                    <div
                      ...
                      onDoubleClick={freeze.gesture((event: ReactMouseEvent<HTMLDivElement>) =>
                        handleLaneDoubleClick(event, track)
                      )}
                    >
                      <div
                        className="absolute top-0 z-20 h-full w-px bg-orange-400/90"
                        style={{ left: playheadMs * pxPerMs }}
                      />
                      {track.keyframes.map((keyframe) => (
                        <button
                          ...
                          style={{
                            left:
                              (keyframeDrag?.keyframeId === keyframe.id
                                ? keyframeDrag.timeMs
                                : keyframe.timeMs) * pxPerMs
                          }}
                          onClick={() => selectKeyframe(track, keyframe)}
                          onPointerDown={freeze.gesture(
                            (event: ReactPointerEvent<HTMLButtonElement>) =>
                              startKeyframeDrag(event, track, keyframe)
                          )}
                          data-tip={`${getStoryMotionPropertyMeta(track.property).label} ${formatStoryMotionTime(keyframe.timeMs, STORY_MOTION_FPS)}`}
                          aria-label={`${getStoryMotionPropertyMeta(track.property).label} ${formatStoryMotionTime(keyframe.timeMs, STORY_MOTION_FPS)}`}
                        />
                      ))}
...
): { track: StoryAnimationTrack; keyframe: StoryAnimationKeyframe } | null {
  if (!keyframeId) {
    return null;
  }
...
function isStoryMotionTrackPropertyValue(value: unknown): value is StoryAnimationTrackProperty {
  return (
    typeof value === "string" && STORY_MOTION_PROPERTIES.some((item) => item.property === value)
  );
}
...
function centerPreviewViewport(
  ...
  viewport.scrollLeft =
    PREVIEW_CANVAS_PADDING + (stageSize.width * zoom) / 2 - viewport.clientWidth / 2;
  viewport.scrollTop =
    PREVIEW_CANVAS_PADDING + (stageSize.height * zoom) / 2 - viewport.clientHeight / 2;
}
...
function buildTicks(
  ...
  const step =
    TIMELINE_TICK_STEPS.find((candidate) => candidate * pxPerMs >= TIMELINE_TICK_MIN_PX) ??
  ...
  const endMs =
    viewport.width > 0
      ? Math.min(
          maxTimeMs,
          (viewport.scrollLeft + viewport.width - TIMELINE_LEFT_COL_PX + bufferPx) / pxPerMs
        )
      : maxTimeMs;
...
export function resolveStoryMotionStageSize(projectService: ProjectService | null): {
  width: number;
  height: number;
} {
  try {
