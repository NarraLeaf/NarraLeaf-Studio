...
export function getStoryMotionPropertyMeta(
  property: StoryAnimationTrackProperty
): StoryMotionPropertyMeta {
  return (
    STORY_MOTION_PROPERTIES.find((item) => item.property === property) ?? STORY_MOTION_PROPERTIES[0]
  );
}
...
export function getStoryMotionDurationMs(timeline: StoryAnimationTimeline | undefined): number {
  const keyframeTimes = (timeline?.tracks ?? []).flatMap((track) =>
...
export function storyMotionSignatureTimeMs(timeline: StoryAnimationTimeline | undefined): number {
  ...
  const sampled = Array.from({ length: SIGNATURE_SAMPLE_COUNT + 1 }, (_, index) =>
    Math.round((durationMs * index) / SIGNATURE_SAMPLE_COUNT)
  );
...
export function stepStoryMotionTimeByFrames(
  timeMs: number,
  frames: number,
...
export function getStoryMotionTimeline(
  asset: StoryAnimationAsset | null | undefined
): StoryAnimationTimeline {
...
export function sampleStoryMotionPreview(
  timeline: StoryAnimationTimeline | undefined,
  timeMs: number
): StoryMotionPreviewState {
  ...
  for (const track of timeline?.tracks ?? []) {
    ...
    if (track.property === "position" && typeof value === "object") {
      ...
      typeof value === "string" &&
      value &&
      getStoryMotionPropertyMeta(track.property).valueKind === "text"
    ) {
...
export function deleteStoryMotionKeyframe(
  timeline: StoryAnimationTimeline,
  keyframeId: string
): StoryAnimationTimeline {
  const next = cloneTimeline(timeline);
...
export function deleteStoryMotionTrack(
  timeline: StoryAnimationTimeline,
  trackId: string
): StoryAnimationTimeline {
  const next = cloneTimeline(timeline);
...
export function sampleStoryMotionTrackValue(
  track: StoryAnimationTrack,
  timeMs: number
): StoryAnimationKeyframeValue | undefined {
  ...
  const rawProgress = Math.max(
    0,
    Math.min(1, (timeMs - previous.timeMs) / Math.max(1, next.timeMs - previous.timeMs))
  );
...
function interpolateValue(
  ...
  if (typeof from === "object" && typeof to === "object") {
    return {
      xalign: lerp(
        from.xalign ?? DEFAULT_PREVIEW_STATE.position.xalign,
        to.xalign ?? DEFAULT_PREVIEW_STATE.position.xalign,
        easedProgress
      ),
      yalign: lerp(
        from.yalign ?? DEFAULT_PREVIEW_STATE.position.yalign,
        to.yalign ?? DEFAULT_PREVIEW_STATE.position.yalign,
        easedProgress
      ),
