...
describe("listSceneDisplayableTargets", () => {
    ...
    const document = documentWith(
      {
        ...
        heroReskin: action("heroReskin", {
          action: "image",
          operation: "setSource",
          objectName: "hero",
          assetId: "asset-hero-2"
        }),
  ...
  it("lists only creator actions, never displayable references (which introduce nothing)", () => {
    const document = documentWith(
      {
        ghostShow: action("ghostShow", {
          action: "displayable",
          operation: "show",
          target: { name: "ghost" }
        }),
        ghostMove: action("ghostMove", {
          action: "displayable",
          operation: "transform",
          target: { name: "ghost", kind: "image" }
        }),
        probe: action("probe", {
          action: "displayable",
          operation: "hide",
...
describe("resolveDisplayableTargetRef", () => {
  ...
  it("follows the source block's current name when the stage name is renamed", () => {
    ...
    expect(
      resolveDisplayableTargetRef(scene, { name: "hero", kind: "image", sourceBlockId: "hero" })
  ...
  it("falls back to the stored name when the source block is gone", () => {
    ...
    expect(
      resolveDisplayableTargetRef(scene, { name: "ghost", kind: "image", sourceBlockId: "deleted" })
    ).toEqual({ name: "ghost", kind: "image", label: "ghost" });
  });

  it("uses the stored name/kind for legacy targets without a source block", () => {
    const scene = documentWith({}, []).scenes["scene-1"];
    expect(resolveDisplayableTargetRef(scene, { name: "hero", kind: "character" })).toEqual({
      name: "hero",
      kind: "character",
      label: "hero"
    });
...
describe("resolved target name matches the stage name the compiler registers", () => {
  ...
  const CHARACTER_CASES: Array<{
    label: string;
    payload: Extract<StoryActionPayload, { action: "character" }>;
  }> = [
    { label: "nothing configured", payload: { action: "character", operation: "enter" } },
    {
      label: "characterId, no stage name",
      payload: { action: "character", operation: "enter", characterId: "c-uuid" }
    },
    {
      label: "stage name auto-filled from the profile",
      payload: {
        action: "character",
        operation: "enter",
        characterId: "c-uuid",
    ...
    {
      label: "stage name cleared by the author",
      payload: { action: "character", operation: "enter", characterId: "c-uuid", objectName: "" }
    },
    {
      label: "stage name whitespace-only",
      payload: { action: "character", operation: "enter", characterId: "c-uuid", objectName: "   " }
    },
    {
      label: "stage name literally 'character'",
      payload: {
        action: "character",
        operation: "enter",
        characterId: "c-uuid",
  ...
  it("image / text / layer fall back to the compiler's stage name when unnamed", () => {
    ...
    expect(resolvedNameFor({ action: "image", operation: "create", objectName: "" }, "Image")).toBe(
      "object"
    );
    expect(
      resolvedNameFor({ action: "text", operation: "create", objectName: "  ", text: "hi" }, "Text")
    ).toBe("object");
    expect(resolvedNameFor({ action: "layer", operation: "create", objectName: "" }, "Layer")).toBe(
      "object"
    );
  });
});
...
describe("displayableSourceIdentity keeps author-facing labels free of internal ids", () => {
  ...
  it("labels an unnamed character 'Character' even though it keys on the characterId UUID", () => {
    const payload: StoryActionPayload = {
      action: "character",
      operation: "enter",
      characterId: "9f8c1e2a-uuid"
    };
