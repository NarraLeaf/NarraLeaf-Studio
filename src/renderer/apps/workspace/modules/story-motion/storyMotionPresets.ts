...
export const STORY_MOTION_PRESETS: readonly StoryMotionPreset[] = [
    ...
    // ── Emphasis ────────────────────────────────────────────────────────────────────────────────
    {
        ...
        scope: "displayable",
        build: () => timeline([
            track("opacity", [key(0, 0, "linear"), key(80, 1, "linear"), key(150, 0.2, "linear"), key(280, 1, "easeOut")]),
        ]),
    },
    {
        id: "pulse",
        ...
        scope: "displayable",
        build: () => timeline([
            track("zoom", [key(0, 1), key(140, 1.12, "easeOut"), key(320, 1, "easeOut")]),
        ]),
    },
    {
        id: "heartbeat",
    ...
    },
    {
        ...
        build: () => timeline([
            ...
            track("scaleY", [key(0, 1), key(360, 1, "linear"), key(430, 0.94, "easeOut"), key(520, 1, "easeOut")]),
        ]),
    },
    {
        id: "zoomPunch",
    ...
    },
    {
        ...
        scope: "camera",
        build: () => timeline([
            track("zoom", [key(0, 1), key(1600, 1.35, "easeInOut")]),
        ]),
    },
    {
        id: "cameraPullBack",
    ...
    },
    {
        ...
        scope: "camera",
        build: () => timeline([
            track("rotation", [key(0, 0), key(900, -4.5, "easeInOut")]),
        ]),
    },
    {
        id: "cameraZoomPunch",
...
export function storyMotionPresetsForTargetKind(targetKind: StoryMotionTargetKind): StoryMotionPreset[] {
...
export function storyMotionPresetCategoriesForTargetKind(targetKind: StoryMotionTargetKind): StoryMotionPresetCategory[] {
    const presets = storyMotionPresetsForTargetKind(targetKind);
    return STORY_MOTION_PRESET_CATEGORIES.filter(category => presets.some(preset => preset.category === category));
}
...
function track(property: StoryAnimationTrackProperty, keyframes: StoryAnimationKeyframe[]): StoryAnimationTrack {
    return {
        id: `track-${property}`,
        property,
        keyframes: keyframes.map(keyframe => ({ ...keyframe, id: `kf-${property}-${keyframe.timeMs}` })),
    };
}

/** `id` is filled in by {@link track}, which is the only place that knows the property. */
function key(timeMs: number, value: StoryAnimationKeyframeValue, easing = "easeOut"): StoryAnimationKeyframe {
...
function zoomPunch(peak: number): StoryAnimationTimeline {
    return timeline([
        track("zoom", [key(0, 1), key(90, peak, "easeOut"), key(320, 1, "easeInOut")]),
    ]);
}
