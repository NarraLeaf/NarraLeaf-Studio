...
const TOOL_BUTTON_CLASS =
  "inline-flex h-9 cursor-default items-center gap-1.5 rounded-md border border-edge bg-fill-subtle px-2 text-xs text-fg hover:border-primary/40 hover:text-primary disabled:cursor-not-allowed disabled:opacity-40";
...
export function MotionSelector(props: {
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = () => {
      ...
      const panelHeight = Math.min(
        panelRef.current?.offsetHeight ?? maxPanelHeight,
        maxPanelHeight
      );
      ...
      top = clamp(
        top,
        viewportTop,
        Math.max(viewportTop, viewportHeight - viewportMargin - panelHeight)
      );
  ...
  const openEditor = useCallback(
    (assetId: string) => {
      openEditorTab(
        createStoryMotionEditorTab({ animationId: assetId, actionContext: props.actionContext })
      );
    },
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="block truncate text-2xs text-fg-subtle">
                            {t(`motion.targetKind.${asset.targetKind}`)}
                          </span>
                        </span>
                      </button>
                      <button
...
function MotionHoverPreview(props: {
  ...
  useEffect(() => {
    let disposed = false;
    setAsset(null);
    void props.storyService
      .loadAnimationAsset(props.animationId)
      .then((loaded) => {
        if (!disposed) setAsset(loaded);
      })
      .catch(() => {
        if (!disposed) setAsset(null);
      });
    return () => {
      disposed = true;
    };
  ...
  const stageSize = useMemo(
    () => resolveStoryMotionStageSize(props.projectService),
    ...
    >
      {asset ? (
        ...
        <div className="grid h-full w-full place-items-center text-xs text-white/70">
          {t("motion.selector.loadingPreview")}
        </div>
      )}
    </div>
  );
...
export function MotionField(props: {
  ...
  useEffect(() => {
    ...
    let disposed = false;
    void storyService
      .loadAnimationAsset(animationId)
      .then((loaded) => {
        if (!disposed) setAsset(loaded);
      })
      .catch(() => {
  ...
  const openEditor = useCallback(() => {
    if (animationId) {
      openEditorTab(
        createStoryMotionEditorTab({ animationId, actionContext: props.actionContext })
      );
    }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        <span
          className={["min-w-0 truncate", animationId ? "" : "italic text-fg-subtle"].join(" ")}
        >
          {asset?.name ??
            (animationId ? t("motion.field.motionAsset") : t("motion.field.choosePlaceholder"))}
        </span>
      </button>
      {animationId ? (
        <>
          ...
          <button
            ...
            onClick={clear}
            data-tip={t("motion.clearMotion")}
            aria-label={t("motion.clearMotion")}
          >
            <X className="h-4 w-4" />
          </button>
        </>
