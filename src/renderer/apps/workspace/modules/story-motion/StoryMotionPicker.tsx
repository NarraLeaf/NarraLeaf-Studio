...
export function StoryMotionPicker(props: {
  ...
  const presetOptions = useMemo(
    () =>
      storyMotionPresetsForTargetKind(props.targetKind).map((preset) => ({
        value: preset.id,
        ...
        <div className="flex min-w-0 items-center gap-2 rounded-md border border-primary/25 bg-primary/10 p-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="truncate text-xs font-medium text-primary">
              {selectedAsset?.name ?? props.motionLabel}
            </div>
            <div className="truncate text-2xs text-fg-muted">
              {selectedAsset
                ? motionSummary(selectedAsset, t)
                : t("motion.picker.assetFallback", { id: animationId ?? "" })}
            </div>
          </div>
          <button
            className={ICON_BUTTON_CLASS}
            type="button"
            onClick={() => openEditor(animationId)}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate text-2xs text-fg-subtle">
                      {t("motion.picker.previewKind", {
                        kind: t(`motion.targetKind.${asset.targetKind}`)
                      })}
                    </span>
                  </span>
