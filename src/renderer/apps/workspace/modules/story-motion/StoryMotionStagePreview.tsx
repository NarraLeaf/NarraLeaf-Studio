...
export function StoryMotionStagePreview(props: {
  ...
  onPointerDrag: (
    ...
    mode: StoryMotionPreviewDragMode
  ) => void;
  interactive?: boolean;
  ...
  const handleInvX =
    1 / Math.max(0.05, Math.abs(canvasScale * props.preview.zoom * props.preview.scaleX));
  const handleInvY =
    1 / Math.max(0.05, Math.abs(canvasScale * props.preview.zoom * props.preview.scaleY));
  ...
  return (
    <div
      ...
      style={
        fixedStage ? { width: props.stageSize!.width, height: props.stageSize!.height } : undefined
      }
    >
      {props.backgroundUrl ? (
        <img
      ...
      <div
        className={`${fixedStage ? "absolute inset-0" : "absolute inset-6"} rounded-md border border-white/10 bg-[linear-gradient(90deg,rgba(255,255,255,.04)_1px,transparent_1px),linear-gradient(rgba(255,255,255,.04)_1px,transparent_1px)] bg-[length:32px_32px]`}
      />
      {showLabel ? (
        <div
          className={`${fixedStage ? "left-2 top-2" : "left-6 top-6"} absolute rounded-md border border-white/10 bg-black/30 px-2 py-1 text-2xs text-white/70`}
        >
        ...
        {interactive ? (
          <>
            <div
              ...
              style={{
                transform: `scale(${handleInvX}, ${handleInvY})`,
            ...
            <div
              ...
              style={{
                transform: `translateY(-50%) scale(${handleInvX}, ${handleInvY})`,
                transformOrigin: "center"
              }}
            ...
            <div
              ...
              style={{
                transform: `translateX(-50%) scale(${handleInvX}, ${handleInvY})`,
                transformOrigin: "center"
              }}
