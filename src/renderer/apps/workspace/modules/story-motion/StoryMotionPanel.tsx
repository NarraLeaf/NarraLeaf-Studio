...
import {
  ...
  type StoryMotionPreviewTarget
} from "./storyMotionPreviewTarget";

const ICON_BUTTON_CLASS = controlButtonClass();
const PREVIEW_LOOP_GAP_MS = 2000;
const PREVIEW_FRAME_MS = 1000 / STORY_MOTION_FPS;

export function StoryMotionPanel({
  payload
}: PanelComponentProps<StoryMotionPanelPayload | undefined>) {
  ...
  useEffect(() => {
    ...
    let disposed = false;
    void storyService
      .loadStory(actionContext.storyId)
      .then((next) => {
        if (!disposed) {
          setDocument(next);
        }
  ...
  const actionAnimationId =
    descriptor?.transform?.mode === "animation" ? descriptor.transform.animationId : undefined;
  ...
  const storyMotionPresetCategories = useMemo(
    () =>
      STORY_MOTION_PRESET_CATEGORIES.filter((category) =>
        presetsForTarget.some((preset) => preset.category === category)
      ),
    [presetsForTarget]
  );
  ...
  const bindToAction = useCallback(() => {
    ...
    storyService.updateBlock(
      actionContext.storyId,
      actionContext.sceneId,
      block.id,
      descriptor.setTransform(transform)
    );
  }, [actionContext, block, descriptor, selectedAsset, storyService]);

  const setPreviewAsset = useCallback(
    (field: "previewAssetId" | "previewBackgroundAssetId", assetId: string | undefined) => {
      if (!storyService || !selectedAsset) {
        return;
      }
      const next = storyService.updateAnimationAsset(selectedAsset.id, (asset) => ({
  ...
  const handlePreviewAssetConfirm = useCallback(
    (assets: Asset[]) => {
      if (assetPickerFor) {
        ...
        freeze.run(setPreviewAsset)(
          assetPickerFor === "target" ? "previewAssetId" : "previewBackgroundAssetId",
          assets[0]?.id
        );
      }
  ...
  const setConfig = useCallback(
    (
      patch: { repeat?: number; repeatDelayMs?: number },
      clear: ("repeat" | "repeatDelayMs")[] = []
    ) => {
      if (!storyService || !selectedAsset) {
        return;
      }
  ...
  const imageAssetName = useCallback(
  ...
  const previewDurationMs = useMemo(
    () => getStoryMotionDurationMs(previewTimeline),
    [previewTimeline]
  );
  useEffect(() => {
    if (!selectedAsset) {
  ...
  const { url: previewBackgroundUrl } = useAssetObjectUrl(
    selectedAsset?.previewBackgroundAssetId ?? null
  );
    ...
    <div className="flex h-full min-h-0 bg-surface text-fg">
      <aside className="flex w-64 shrink-0 flex-col border-r border-edge">
        <div className="flex items-center gap-2 border-b border-edge p-2">
          ...
          <button
            ...
            {...freeze.writes(false, t("motion.panel.createMotion"))}
            aria-label={t("motion.panel.createMotion")}
          >
            ...
            <div className="min-h-0 overflow-auto">
              <section className="rounded-lg border border-edge bg-fill-subtle p-4">
                <div className="grid gap-4">
                  ...
                  <SurfaceEditorToolbarButtonGroup
                    aria-label={t("motion.panel.motionActions")}
                    className="w-full"
                  >
                    <SurfaceEditorToolbarSegButton
                    ...
                    <SurfaceEditorToolbarSegButton
                      type="button"
                      onClick={duplicateMotion}
                      {...freeze.writes(false, t("common.duplicate"))}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <label className="grid min-w-0 gap-1.5">
                      <span className="text-xs font-medium text-fg-subtle">
                        {t("motion.panel.repeat")}
                      </span>
                      <NumericDraftEnhancedInput
                        committedDisplay={
                          selectedAsset.config?.repeat ? String(selectedAsset.config.repeat) : ""
                        }
                    ...
                    <label className="grid min-w-0 gap-1.5">
                      <span className="text-xs font-medium text-fg-subtle">
                        {t("motion.panel.repeatDelaySeconds")}
                      </span>
                      <NumericDraftEnhancedInput
                        committedDisplay={
                          selectedAsset.config?.repeatDelayMs
                            ? formatStorySecondsValue(selectedAsset.config.repeatDelayMs)
                            : ""
                        }
                        draftResetKey={selectedAsset.id}
                        placeholder="0"
                  ...
                  <div className="grid gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 truncate text-2xs text-fg-muted">
                        {actionAnimationId
                          ? t("motion.panel.actionUses", { id: actionAnimationId })
                          : t("motion.panel.actionNoMotion")}
                      </div>
                    </div>
                    <Button
                      variant="secondary"
            ...
            <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-edge">
              ...
              <div className="flex h-10 shrink-0 items-center gap-2 border-t border-edge bg-surface px-2">
                ...
                <PreviewAssetSlot
                  ...
                  label={
                    imageAssetName(selectedAsset.previewBackgroundAssetId) ??
                    t("motion.panel.background")
                  }
      ...
      {selectedAsset ? (
        <AssetSelector
          ...
          anchorRef={
            assetPickerFor === "background" ? backgroundPickerButtonRef : targetPickerButtonRef
          }
          selectedIds={(() => {
            const current =
              assetPickerFor === "background"
                ? selectedAsset.previewBackgroundAssetId
                : selectedAsset.previewAssetId;
            return current ? [current] : [];
          })()}
          title={
            assetPickerFor === "background"
              ? t("motion.panel.previewBackground")
              : t("motion.panel.previewTarget")
          }
...
function normalizeActionContext(
  payload: StoryMotionPanelPayload | undefined
): StoryMotionActionContext | null {
...
function collectStoryMotionEditorTabs(
  ...
  if ("tabs" in layout) {
    for (const tab of layout.tabs) {
      ...
      const related =
        tab.id === `story-motion:${animationId}` ||
...
function StoryMotionFittedStage(props: {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const next = Math.min(
        rect.width / props.stageSize.width,
        rect.height / props.stageSize.height
      );
  ...
  return (
    ...
    >
      {scale > 0 ? (
        <div
          style={{ width: props.stageSize.width * scale, height: props.stageSize.height * scale }}
        >
          <div
            style={{
