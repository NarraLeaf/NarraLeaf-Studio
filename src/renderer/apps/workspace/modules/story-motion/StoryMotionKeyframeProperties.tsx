...
export function StoryMotionKeyframeProperties(props: {
  ...
  const loadSelectedAsset = useCallback(() => {
    let disposed = false;
    void storyService
      .loadAnimationAsset(selection.animationId)
      .then((next) => {
        if (!disposed) {
          setAsset(next);
        }
    ...
    (
      ...
      setAsset((current) => {
        ...
        const next = storyService.updateAnimationAsset(selection.animationId, (motion) => ({
          ...motion,
          timeline: updateStoryMotionKeyframe(
            getStoryMotionTimeline(motion),
  ...
  const deleteKeyframe = useCallback(
    (data: StoryMotionKeyframeInspectorData) => {
      setAsset((current) => {
        if (!current) {
          return current;
        }
...
function createStoryMotionKeyframeSchema(
  ...
  return createPropertyEditorSchema<StoryMotionKeyframeInspectorData>({
    ...
    fields: [
      defineField<
        StoryMotionKeyframeInspectorData,
        FieldDefinition<StoryMotionKeyframeInspectorData>
      >({
        id: "summary",
        type: "info",
        items: (data) => {
          ...
          return [
            ...
            {
              label: t("motion.keyframe.time"),
              getValue: () => formatStoryMotionTime(data.keyframe.timeMs)
            }
          ];
        }
      }),
      ...
      >({
        ...
        setValue: (data, value) => {
          ...
          const easing =
            raw === CUSTOM_EASING_OPTION
              ? isStoryMotionBezierEasing(data.keyframe.easing)
      ...
      >({
        ...
        fields: [
          ...
          >({
            ...
            type: "inputGroup",
            hidden: (data) =>
              getStoryMotionPropertyMeta(data.track.property).valueKind !== "position",
            wrap: true,
            inputs: [
          ...
          >({
            ...
            step: 0.01,
            hidden: (data) =>
              getStoryMotionPropertyMeta(data.track.property).valueKind !== "number",
          ...
          >({
            ...
            getValue: (data) =>
              typeof data.keyframe.value === "string" ? data.keyframe.value : "",
            setValue: (_data, value) => {
              updateKeyframe((keyframe) => ({
                ...keyframe,
                value
      ...
      >({
        ...
        component: ({ data }) => (
          <SurfaceEditorToolbarButtonGroup
            aria-label={t("motion.keyframe.keyframeActions")}
            className="w-full"
          >
            <SurfaceEditorToolbarSegButton
              type="button"
...
function positionInput(
  ...
  return {
    ...
    getValue: (data: StoryMotionKeyframeInspectorData) =>
      String(readPositionValue(data.keyframe.value, key, fallback)),
...
function StoryMotionBezierEditor(props: { easing: string; onChange: (easing: string) => void }) {
  ...
  const toY = (value: number) =>
    ((BEZIER_Y_MAX - value) / (BEZIER_Y_MAX - BEZIER_Y_MIN)) * BEZIER_VIEW_SIZE;
  ...
  const startHandleDrag = (event: ReactPointerEvent<SVGCircleElement>, handle: 0 | 1) => {
    ...
    const onMove = (moveEvent: PointerEvent) => {
      ...
      const y =
        BEZIER_Y_MAX -
        ((moveEvent.clientY - rect.top) / rect.height) * (BEZIER_Y_MAX - BEZIER_Y_MIN);
      const clampedY = Math.min(BEZIER_Y_MAX, Math.max(BEZIER_Y_MIN, y));
      const next: [number, number, number, number] =
        handle === 0 ? [x, clampedY, points[2], points[3]] : [points[0], points[1], x, clampedY];
    ...
    <div className="grid gap-1.5">
      ...
      >
        ...
        <line
          ...
          x2={toX(points[0])}
          y2={toY(points[1])}
          style={{ stroke: "rgb(var(--nl-fg-muted) / 0.5)" }}
          strokeWidth={1}
        />
        <line
          x1={toX(1)}
        ...
        <circle
          ...
          onPointerDown={freeze.gesture((event: ReactPointerEvent<SVGCircleElement>) =>
            startHandleDrag(event, 0)
          )}
        />
        <circle
          cx={toX(points[2])}
          ...
          className={freeze.frozen ? undefined : "cursor-grab"}
          onPointerDown={freeze.gesture((event: ReactPointerEvent<SVGCircleElement>) =>
            startHandleDrag(event, 1)
          )}
        />
      </svg>
      <div className="text-center text-2xs tabular-nums text-fg-subtle">
        {formatStoryBezierEasing(points)}
      </div>
    </div>
  );
}

function clearStoryMotionKeyframeSelection(
  uiService: UIService,
  selection: StoryMotionKeyframeSelection
): void {
