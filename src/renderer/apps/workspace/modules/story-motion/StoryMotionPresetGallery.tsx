...
export function StoryMotionPresetGallery(props: {
  ...
  const presets = useMemo(
    () => storyMotionPresetsForTargetKind(props.targetKind),
    [props.targetKind]
  );
  const categories = useMemo(
    () => storyMotionPresetCategoriesForTargetKind(props.targetKind),
    [props.targetKind]
  );
    ...
    >
      ...
      <span className="block min-w-0 border-t border-edge bg-surface-raised px-2 py-1">
        <span className="flex min-w-0 items-center gap-1">
          <span className="min-w-0 flex-1 truncate text-2xs font-medium text-fg">
            {props.label}
          </span>
          ...
          {repeat ? (
            <span
              ...
              data-tip={t("motion.panel.repeat")}
            >
        ...
        <span className="block truncate text-2xs text-fg-subtle">
          {storyMotionTimelineSummary(timeline, t)}
        </span>
      </span>
    </button>
  );
