...
describe("importFieldsFor", () => {
  it("asks for every field the vault will demand", () => {
    ...
    for (const kind of SIGNING_CREDENTIAL_KINDS) {
      expect(importFieldNames(kind), kind).toEqual(
        expect.arrayContaining(requiredVaultFields(kind))
      );
    }
  });
...
describe("isImportComplete", () => {
  ...
  it("refuses a keystore with no key chosen", () => {
    expect(isImportComplete("android-keystore", { ...FULL["android-keystore"], alias: "" })).toBe(
      false
    );
    expect(isImportComplete("android-keystore", { ...FULL["android-keystore"], file: "" })).toBe(
      false
    );
  });
  ...
  it("lets the key password stand in for the store password", () => {
    expect(
      isImportComplete("android-keystore", { ...FULL["android-keystore"], keyPassword: "" })
    ).toBe(true);
  });

  it("takes either half of a Windows certificate-store credential, but not neither", () => {
    expect(
      isImportComplete("windows-store", { label: "Token", subjectName: "NarraLeaf Ltd" })
    ).toBe(true);
  ...
  it("accepts a macOS credential that does not notarize", () => {
    ...
    expect(
      isImportComplete("macos-keychain", { ...FULL["macos-keychain"], ...NO_NOTARIZATION })
    ).toBe(true);
    expect(isImportComplete("macos-apple", { ...FULL["macos-apple"], ...NO_NOTARIZATION })).toBe(
      true
    );
  });
  ...
  it("refuses a half-filled notary set, which the vault would reject anyway", () => {
    ...
    for (const field of ["notaryKeyFile", "notaryKeyId", "notaryIssuerId"] as const) {
      expect(isImportComplete("macos-apple", { ...FULL["macos-apple"], [field]: "" }), field).toBe(
        false
      );
    }
  });
  ...
  it("still requires the parts a macOS credential cannot do without", () => {
    expect(isImportComplete("macos-keychain", { ...FULL["macos-keychain"], identity: "" })).toBe(
      false
    );
    expect(isImportComplete("macos-apple", { ...FULL["macos-apple"], p12File: "" })).toBe(false);
    expect(isImportComplete("macos-apple", { ...FULL["macos-apple"], p12Password: "" })).toBe(
      false
    );
  });
});
...
describe("buildSigningImport", () => {
  it("builds the payload each kind declares", () => {
    ...
    expect(
      buildSigningImport("macos-apple", { ...FULL["macos-apple"], ...NO_NOTARIZATION })
    ).toEqual({
      kind: "macos-apple",
      label: "Mac file",
      p12File: "/keys/developer-id.p12",
  ...
  it("reads an empty key password as 'the same as the keystore password'", () => {
    ...
    expect(
      buildSigningImport("android-keystore", { ...FULL["android-keystore"], keyPassword: "" })
  ...
  it("leaves passwords exactly as typed", () => {
    ...
    expect(
      buildSigningImport("windows-pfx", { ...FULL["windows-pfx"], password: "  pad  " })
    ).toMatchObject({ password: "  pad  " });
  });

  it("omits optional fields left blank rather than storing empty strings", () => {
    expect(buildSigningImport("linux-gpg", { label: "GPG", keyId: "0xDEADBEEF" })).toEqual({
      kind: "linux-gpg",
      label: "GPG",
      keyId: "0xDEADBEEF"
    });
    expect(buildSigningImport("windows-store", { label: "Token", sha1: "AABB" })).toEqual({
      kind: "windows-store",
      label: "Token",
      sha1: "AABB"
    });
  ...
  it("carries nothing over from a kind the author switched away from", () => {
    ...
    const contaminated: SigningImportDraft = {
      ...FULL["linux-gpg"],
      ...FULL["windows-pfx"],
      label: "GPG"
    };

    expect(buildSigningImport("linux-gpg", contaminated)).toEqual({
      kind: "linux-gpg",
      label: "GPG",
      keyId: "0xDEADBEEF",
      gpgPath: "C:/gpg/gpg.exe"
    });
  });
});
