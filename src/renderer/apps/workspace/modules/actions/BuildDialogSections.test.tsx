  ...
  it("has a label for every format any platform offers", () => {
    for (const [platform, formats] of Object.entries(OFFERED_FORMATS)) {
      for (const format of formats) {
        expect(
          enBuild.format[format],
          `build.format.${format} is missing (offered by ${platform})`
        ).toBeTruthy();
      }
    }
  });
...
describe("ContentSection", () => {
  ...
  const render = (overrides: Partial<Parameters<typeof ContentSection>[0]> = {}) =>
  ...
  it("names the chosen position on the chooser, and its consequence underneath", () => {
    ...
    const listed = render({
      content: { encryptAssets: false, networkPolicy: "allowlist" as const }
    });
    expect(listed).toContain("Allowlist");
    expect(listed).toContain(
      "The packaged game reaches only the addresses on the project allowlist."
    );
...
describe("the variant page", () => {
  const render = (dialogInfo: BuildDialogInfo, appTagId: string) =>
    renderToStaticMarkup(
      <BuildDialogContent
        info={dialogInfo}
  ...
  it("lists every variant, release first, and marks the selected one", () => {
    ...
    expect(markup.indexOf('data-build-app-tag-option="release"')).toBeLessThan(
      markup.indexOf('data-build-app-tag-option="tag-demo"')
    );
  });
...
describe("the plugins page", () => {
  const declaring = (
    fields: PluginBuildConfigFieldContribution[]
  ...
  const stubService = (stated?: string) =>
    ({
      resolvePluginConfigValue: (id: string | null | undefined) =>
        stated !== undefined && id
  ...
  it("never shows a secret, and says which of the three states it is in", () => {
    ...
    const empty = {
      resolvePluginConfigValue: () => ({ value: "", overridden: false })
    } as unknown as AppTagService;
...
describe("OutputSection's artifact prediction", () => {
  ...
  const render = (variant: ProjectAppTag) =>
    renderToStaticMarkup(
      <OutputSection
        info={info}
        state={windowsZip}
