...
export function BuildDialogContent({
  ...
  const signablePlatforms = useMemo(() => {
    ...
    return SIGNING_PLATFORMS.filter((platform) =>
      platform === "linux" ? producesSomething : state.formats[platform].size > 0
    );
  }, [state.formats]);
  ...
  const severityBySection = useMemo(() => {
    ...
    for (const finding of findings) {
      if (map[finding.section] !== "error") {
        map[finding.section] =
          finding.severity === "error" ? "error" : (map[finding.section] ?? "warning");
      }
    }
    return map;
  }, [findings]);
    ...
    <div className="-mx-6 -my-4 flex flex-col text-sm">
      <div className="flex h-96 items-stretch">
        ...
        <div
          ...
          data-help-topic={PAGE_HELP_TOPICS[page]}
        >
          {page === "variant" && (
            <VariantPage
              info={info}
          ...
          {page === "content" && (
            <ContentSection
              ...
              onContentChange={(field, value) => {
                void commitContent(field, value);
              }}
              onRescanPlugins={() => {
                void rescanPlugins();
              }}
...
function IdentitySection({
  ...
  const appId = deriveGameAppId(
    ...
    identity.displayName.value || info.productName
  );
  return (
    <div className="grid gap-3">
...
function Field({
  ...
  return (
    <div className={cn("flex gap-3", align === "start" ? "items-start" : "items-center")}>
      ...
      <span className={cn("w-24 shrink-0 text-xs text-fg-muted", align === "start" && "pt-2.5")}>
        {label}
      </span>
  ...
  return (
    <div className="grid gap-3">
      <Toggled
        ...
        value={
          content.encryptAssets ? t("build.content.protectionOn") : t("build.content.protectionOff")
        }
        ...
        value={
          info.locales.length > 0
            ? info.locales
                .map((locale) =>
                  locale.source
                    ? t("build.content.localeSource", { name: locale.name })
                    : locale.name
                )
                .join(" · ")
...
function PluginStatus({ plugin }: { plugin: BuildPluginEntry }) {
  ...
  return (
    <span
      className={cn("shrink-0 text-2xs font-medium", DEPENDENCY_STATUS_TEXT_STYLES[plugin.status])}
    >
      {plugin.suppressed
...
export function OutputSection({
  ...
  const browse = async () => {
    const result = await getInterface().gameBuild.selectOutputDir(
        ...
        {artifacts.length === 0 ? (
          ...
          <div className="grid gap-1 rounded-md bg-fill-subtle px-2.5 py-2">
            {artifacts.map((artifact) => (
              <span
                key={`${artifact.platform}-${artifact.name}`}
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <Select
          ...
          options={BUILD_COMPRESSIONS.map((level) => ({
            value: level,
            label: t(
              `build.output.compression${capitalize(level)}` as "build.output.compressionStore"
            )
          }))}
        />
      </div>
...
export async function openBuildDialog(workspace: Workspace): Promise<void> {
  ...
  let _request: GameBuildRequest = stateToRequest(initialState);
  ...
  const dialogId = uiService.dialogs.show({
    ...
    content: (
      <BuildDialogContent
        ...
        initialPage={page}
        copyright={
          typeof projectConfig.metadata?.copyright === "string"
            ? projectConfig.metadata.copyright
            : ""
        }
        ...
        onPersistContent={async (patch) => {
          ...
          try {
            if (patch.encryptAssets !== undefined) {
              await projectService.updateSecurityConfiguration({
                encryptAssets: patch.encryptAssets
              });
            }
...
async function startBuild(workspace: Workspace, request: GameBuildRequest): Promise<void> {
  ...
  void projectService
    .updateBuildConfiguration(requestToBuildConfiguration(request))
    .catch(() => undefined);
