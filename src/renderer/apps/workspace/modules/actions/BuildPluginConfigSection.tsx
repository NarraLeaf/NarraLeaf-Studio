...
function SecretValue({
    ...
    useEffect(() => {
        ...
        let active = true;
        void getInterface().pluginBuildSecret.available(storedHandle).then(result => {
            if (active) {
    ...
    return (
        <div className="grid min-w-0 gap-1">
            ...
            {failed ? <span className="text-2xs text-danger">{t("build.pluginConfig.secretFailed")}</span> : null}
        </div>
    );
}
