...
function assets(stored: unknown, ok = true) {
    const writeStore = vi.fn(async (_namespace: string, _data: MergeDecisionDraft) => (
        { ok: true as const, data: { path: "x" } }
    ));
    const readStore = vi.fn(async (_namespace: string) => (ok
...
describe("mergeFingerprint", () => {
    ...
    it("changes when a different set of files is in conflict", () => {
        ...
        expect(mergeFingerprint(merge({ conflicts: ["editor/story/index.json"] })))
            .not.toBe(mergeFingerprint(merge()));
    });

    it("changes when the incoming revision does", () => {
        expect(mergeFingerprint(merge({ incoming: "bbb222" })))
            .not.toBe(mergeFingerprint(merge()));
    });
    ...
    it("has an answer for a merge the backend named no incoming revision for", () => {
        ...
        expect(mergeFingerprint(merge({ incoming: undefined })))
            .toBe(mergeFingerprint(merge({ incoming: undefined })));
    });
});
...
describe("readMergeDecisionDraft", () => {
    it("restores a draft written for this merge", async () => {
        ...
        await expect(readMergeDecisionDraft(service, mergeFingerprint(merge())))
            .resolves.toMatchObject({ decisions: { "editor/characters.json": "mine" } });
    });
