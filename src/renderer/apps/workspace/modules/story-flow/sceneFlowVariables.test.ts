...
const payloadOf = (block: StoryBlock): Extract<StoryActionPayload, { action: "setVariable" }> =>
  (block as Extract<StoryBlock, { kind: "action" }>).payload as Extract<
    StoryActionPayload,
    { action: "setVariable" }
  >;
...
const stepAst = (op: "+" | "-", step: number, variableId: string = AFFECTION): StoryExpr => ({
  kind: "binary",
  op,
  left: readVariable(variableId, "好感"),
  right: { kind: "literal", value: step }
});
...
function rangeTuples(
  ranges: Map<string, { kind: string; min?: number; max?: number }>
): Record<string, string> {
  return Object.fromEntries(
    Array.from(ranges, ([sceneId, range]) => [
      sceneId,
      range.kind === "known" ? `${range.min}..${range.max}` : "?"
    ])
  );
}
...
describe("readSetVariableDelta", () => {
  it("reads /inc and the longhand assignment that means the same thing identically", () => {
    ...
    expect(readSetVariableDelta(payloadOf(longhand))).toEqual(
      readSetVariableDelta(payloadOf(sugar))
    );
  });

  it("reads /dec as a negative amount and a literal /set as a set", () => {
    expect(
      readSetVariableDelta(payloadOf(setExpressionBlock("w1", stepAst("-", 3), "好感 - (3)")))
    ).toEqual({ op: "add", amount: -3 });
    expect(readSetVariableDelta(payloadOf(setLiteralBlock("w2", 5)))).toEqual({
      op: "set",
      value: 5
    });
    // A literal that arrived as an expression tree rather than folded into `value` is the same row.
    expect(
      readSetVariableDelta(payloadOf(setExpressionBlock("w3", { kind: "literal", value: 5 }, "5")))
  ...
  it("refuses to derive a number from anything it cannot read", () => {
    ...
    const otherVariable = setExpressionBlock(
      "w1",
      {
        kind: "binary",
        op: "+",
        left: readVariable(GOLD, "gold"),
        right: { kind: "literal", value: 1 }
      },
      "gold + 1"
    );
    const call = setExpressionBlock(
      "w2",
      {
        kind: "call",
        fn: "min",
        args: [readVariable(AFFECTION, "好感"), { kind: "literal", value: 10 }]
      },
      "min(好感, 10)"
    );
  ...
  it("does not read the target off the right-hand side", () => {
    ...
    const flipped = setExpressionBlock(
      "w1",
      {
        kind: "binary",
        op: "+",
        left: { kind: "literal", value: 2 },
        right: readVariable(AFFECTION, "好感")
      },
      "2 + 好感"
    );
...
describe("collectSceneEffects", () => {
  ...
  it("does not count a write the author disabled", () => {
    ...
    const story = document(
      [scene("a", "Opening", [affectionDecl, disabled, setLiteralBlock("w2", 5)])],
      "a"
    );
...
describe("collectBranchEffects", () => {
  ...
  it("omits an arm that touches nothing rather than listing it empty", () => {
    ...
    expect(Array.from(collectBranchEffects(buildSceneFlowGraph(story), story).keys())).toEqual([
      "scene-flow:branch:o1"
    ]);
  });
  ...
  it("nets an arm's movement for the chip, and admits when it cannot", () => {
    ...
    expect(branchDeltaFor(effects.get("scene-flow:branch:o1") ?? [], AFFECTION_KEY)).toEqual({
      op: "unknown"
    });
    expect(branchDeltaFor(effects.get("scene-flow:branch:if1") ?? [], AFFECTION_KEY)).toEqual({
      op: "add",
      amount: 3
    });
...
describe("computeVariableRanges", () => {
  function rangesOf(
    story: StoryDocument,
    variableKey: string = AFFECTION_KEY
  ): Record<string, string> {
  ...
  it("poisons every scene downstream of a write it cannot read", () => {
    const story = document(
      [
        ...
        scene("b", "Hallway", [
          setExpressionBlock("w2", stepAst("+", 2), "好感 + (2)"),
          jumpBlock("j2", "c")
        ]),
        scene("c", "Street", [])
      ],
  ...
  it("reports a loop that moves the counter as unbounded instead of iterating it", () => {
    const story = document(
      [
        ...
        scene("b", "Grind", [
          setExpressionBlock("w1", stepAst("+", 1), "好感 + (1)"),
          jumpBlock("j2", "a")
        ])
      ],
      "a"
  ...
  it("answers only for its own scene when the variable is a scene-local", () => {
    ...
    const story = document(
      [scene("a", "Opening", [jumpBlock("j1", "b")]), scene("b", "Hallway", [local])],
      "a"
    );
...
describe("foldRouteVariableValue", () => {
  ...
  it("agrees with the ending scene's range when one route reaches it", () => {
    ...
    expect(foldRouteVariableValue(graph, story, AFFECTION_KEY, route)).toEqual({
      kind: "known",
      min: 5,
      max: 5
    });
    ...
    expect(computeVariableRanges(graph, story, AFFECTION_KEY).get("b")).toEqual({
      kind: "known",
      min: 5,
      max: 5
    });
  ...
  it("refuses a route it cannot fold rather than reporting what it managed", () => {
    ...
    expect(
      foldRouteVariableValue(buildSceneFlowGraph(noDefault), noDefault, AFFECTION_KEY, {
        sceneIds: ["a", "b"],
        branchIds: []
      })
    ...
    expect(
      foldRouteVariableValue(graph, story, AFFECTION_KEY, {
        sceneIds: ["a", "b"],
        branchIds: ["ghost"]
      })
    ).toEqual({ kind: "unknown" });
    expect(
      foldRouteVariableValue(graph, story, AFFECTION_KEY, {
        sceneIds: ["b"],
        branchIds: ["scene-flow:branch:if1"]
      })
    ).toEqual({ kind: "unknown" });
  });
  ...
  it("will not name a single value for a write a deeper fork decides", () => {
    ...
    expect(
      foldRouteVariableValue(graph, story, AFFECTION_KEY, {
        sceneIds: ["a", "b"],
        branchIds: ["scene-flow:branch:o1"]
      })
    ).toEqual({ kind: "unknown" });
  ...
  it("counts an arm the route ended on, which leaves no scene", () => {
    ...
    expect(
      foldRouteVariableValue(buildSceneFlowGraph(story), story, AFFECTION_KEY, {
        sceneIds: ["a"],
        branchIds: ["scene-flow:branch:o1"]
      })
...
describe("listNumericStoryVariables", () => {
  it("lists numeric declarations only, saved and persistent before scene", () => {
    ...
    expect(listNumericStoryVariables(story)).toEqual([
      ...
      {
        key: "persistent:var_total",
        scope: "persistent",
        variableId: "var_total",
        name: "累计好感",
        defaultValue: null
      },
  ...
  it("lists registry saved variables, keyed the same way a row would be", () => {
    ...
    expect(
      listNumericStoryVariables(story, [
        {
          id: AFFECTION,
          name: "好感",
          scope: "saved",
  ...
  it("seeds a range from a registry saved variable's default", () => {
    const story = document([scene("a", "Opening", [setLiteralBlock("w1", 9)])], "a");
