...
function choiceScene(id: string, name: string, count: number, target: (index: number) => string | null): StoryScene {
    const blocks: StoryBlock[] = [choiceBlock("c1", Array.from({ length: count }, (_, index) => `o${index}`))];
    for (let index = 0; index < count; index++) {
        const targetSceneId = target(index);
        blocks.push(choiceOptionBlock(`o${index}`, targetSceneId === null ? [] : [`j${index}`], `Option ${index}`, "c1"));
...
describe("buildSceneFlowGraph branch topology", () => {
    it("gives five options into five scenes five arms leaving five distinct points", () => {
        ...
        expect(graph.branches.map(branch => branch.label))
            .toEqual(["Option 0", "Option 1", "Option 2", "Option 3", "Option 4"]);
        expect(graph.branches.map(branch => branch.targets)).toEqual([["e0"], ["e1"], ["e2"], ["e3"], ["e4"]]);
        expect(graph.branches.every(branch => branch.forkId === "c1" && branch.forkKind === "choice")).toBe(true);
    ...
    it("keeps an option that only continues, as a fall-through arm with no edge", () => {
        const graph = buildSceneFlowGraph(document([
            choiceScene("a", "Fork", 2, index => (index === 0 ? null : "a")),
        ], "a"));
    ...
    it("groups if / else-if / else into one fork and a second if into another", () => {
        ...
        expect(graph.branches.map(branch => branch.forkId)).toEqual(["if1", "if1", "if1", "if2", "if2"]);
        ...
        expect(graph.branches.map(branch => branch.kind))
            .toEqual(["condition", "conditionElseIf", "conditionElse", "condition", "conditionElse"]);
        expect(graph.branches.map(branch => branch.label))
            .toEqual(["gold >= 100", "gold >= 10", "", "mood > 3", ""]);
    ...
    it("stands an else-if with no if above it up as its own fork", () => {
        ...
        expect(graph.branches.map(branch => ({ forkId: branch.forkId, order: branch.order })))
    ...
    it("terminates on a corrupted childrenIds cycle", () => {
        ...
        expect(graph.branches.map(branch => branch.label)).toEqual(["gold >= 1", "gold >= 1 › gold >= 2"]);
    });
});
...
describe("buildSceneFlowGraph layout", () => {
    ...
    function wrappedColumnDocument(): StoryDocument {
        return document([
            scene("hub", "Hub", Array.from({ length: 8 }, (_, index) => jumpBlock(`j${index}`, `s${index}`))),
    ...
    it("lays a collapsed map out exactly where it laid it before branch rows existed", () => {
        ...
        expect(buildSceneFlowGraph(wrappedColumnDocument(), { expandedSceneIds: new Set() }).positions)
            .toEqual(expected);
        expect(Object.values(graph.nodeSizes).every(size =>
            size.width === SCENE_FLOW_NODE_WIDTH && size.height === SCENE_FLOW_NODE_HEIGHT)).toBe(true);
    });
    ...
    it("packs a column by real heights so an expanded scene does not sit under its neighbour", () => {
        ...
        expect(graph.nodeSizes.b.height)
            .toBe(SCENE_FLOW_NODE_HEIGHT + SCENE_FLOW_BRANCH_HEADER_HEIGHT + 3 * SCENE_FLOW_BRANCH_ROW_HEIGHT);
        expect(graph.nodeSizes.c.height).toBe(SCENE_FLOW_NODE_HEIGHT);
        expect(graph.positions.c.y).toBeGreaterThanOrEqual(graph.positions.b.y + graph.nodeSizes.b.height);
