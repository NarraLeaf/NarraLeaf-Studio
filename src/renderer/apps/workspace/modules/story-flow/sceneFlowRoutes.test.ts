...
function choiceScene(id: string, name: string, count: number, target: (index: number) => string | null): StoryScene {
    const blocks: StoryBlock[] = [
        choiceBlock(`${id}-c`, Array.from({ length: count }, (_, index) => `${id}-o${index}`)),
    ];
    for (let index = 0; index < count; index++) {
        const targetSceneId = target(index);
        blocks.push(choiceOptionBlock(
            `${id}-o${index}`,
...
describe("buildSceneFlowRouteMap routes", () => {
    it("gives two options two routes, each carrying its own arm", () => {
        ...
        expect(map.routes.map(route => route.branchIds))
            .toEqual([["scene-flow:branch:a-o0"], ["scene-flow:branch:a-o1"]]);
        expect(map.routes.map(route => route.sceneIds)).toEqual([["a", "e0"], ["a", "e1"]]);
        expect(map.deadBranchIds).toEqual([]);
    });
    ...
    it("still offers an unguarded exit written before the menu as a path of its own", () => {
        ...
        expect(standalone?.steps).toEqual([{ sceneId: "a", branchId: null, edgeId: "scene-flow:a->c" }]);
    ...
    it("does not let a menu nested inside a condition arm gate the rest of the scene", () => {
        ...
        expect(map.routes.some(route => route.branchIds.length === 0 && route.endingSceneId === "c")).toBe(true);
    });
    ...
    it("gives the five-option crossroads exactly the routes a player can walk", () => {
        ...
        expect(map.routes.map(route => route.endingSceneId))
            .toEqual(["two-of-us", "summer-ends", "alone", "summer-ends", "stranger", "nothing"]);
        ...
        expect(map.endings.map(ending => ending.sceneId))
            .toEqual(["two-of-us", "alone", "summer-ends", "stranger", "nothing"]);
        expect(map.deadBranchIds).toEqual([]);
        expect(map.unreachableEndings).toEqual([]);
    });
    ...
    it("still enumerates the paths a loop hangs off", () => {
        ...
        expect(map.routes.map(route => ({ end: route.endingSceneId, cut: route.truncatedByCycle })))
            .toEqual([{ end: "b", cut: true }, { end: "c", cut: false }]);
        expect(map.unreachableEndings).toEqual([]);
    });
    ...
    it("reports arms no route walks as dead options", () => {
        ...
        expect(map.endings.filter(ending => !ending.reachable).map(ending => ending.sceneId))
            .toEqual(["p", "q"]);
    });
    ...
    it("does not claim truncation when the story has exactly as many routes as the cap", () => {
        ...
        const map = routeMap(document([
            ...
            ...Array.from({ length: MAX_ROUTES }, (_, index) => scene(`e${index}`, `Ending ${index}`, [])),
        ], "a"));
