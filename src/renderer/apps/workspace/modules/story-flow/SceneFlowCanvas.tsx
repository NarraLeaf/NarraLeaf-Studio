...
const sceneFlowNodeTypes = {
  scene: SceneFlowNode,
  branch: SceneFlowBranchNode
} satisfies NodeTypes;
...
function SceneFlowCanvasInner({
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState<
    Node<SceneFlowNodeData | SceneFlowBranchNodeData>
  >([]);
  ...
  const projectedNodes = useMemo<Node<SceneFlowNodeData | SceneFlowBranchNodeData>[]>(() => {
    ...
    for (const sceneId of expandedScenes) {
      ...
      for (const branch of branchesBySceneId.get(sceneId) ?? []) {
        ...
        const height =
          (showForkHeader ? SCENE_FLOW_BRANCH_HEADER_HEIGHT : 0) + SCENE_FLOW_BRANCH_ROW_HEIGHT;
        branchNodes.push({
          id: branch.id,
          type: "branch",
          parentId: sceneId,
  ...
  const drawnLines = useMemo(
    () => buildSceneFlowLines(graph, expandedScenes),
    [graph, expandedScenes]
  );
  ...
  useEffect(() => {
    ...
    setEdges((current) => {
      ...
      return projectedEdges.map((edge) =>
        selection.has(edge.id) ? { ...edge, selected: true } : edge
      );
    });
  ...
  const typeScale = minTitleRenderedPx
    ? Math.min(
        MAX_NODE_TYPE_SCALE,
        Math.max(1, minTitleRenderedPx / (NODE_TITLE_BASE_PX * (zoom || 1)))
      )
    : 1;
  ...
  const handleBeforeDelete = useCallback(
    async ({ edges: deleted }: { edges: Edge[] }) => {
      if (editing) {
  ...
  return (
    <>
      ...
      <ReactFlow
        ...
        zoomOnPinch
        className={cn(
          "narraleaf-scene-flow bg-surface",
          connecting && "narraleaf-scene-flow-connecting"
        )}
        ...
        style={
          typeScale === 1
            ? undefined
            : ({ "--nl-scene-flow-type-scale": String(typeScale) } as CSSProperties)
        }
