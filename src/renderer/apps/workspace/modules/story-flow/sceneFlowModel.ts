...
function collectSceneArms(
    ...
    const childBlocks = (ids: readonly StoryBlockId[]): StoryBlock[] =>
        ids.map(childId => scene.blocks[childId]).filter((child): child is StoryBlock => Boolean(child));
    ...
    const record = (
        ...
        const label = branch.label && ownerLabel
    ...
    const visit = (ids: readonly StoryBlockId[], ownerLabel: string): void => {
        let openFork: { id: StoryBlockId; kind: SceneFlowForkKind; order: number; arms: number } | null = null;
        for (const block of childBlocks(ids)) {
            ...
            if (block.kind === "nodeAction" && block.payload.action === "choice") {
                const options = childBlocks(block.childrenIds)
                    .filter(child => child.kind === "nodeAction" && child.payload.action === "choiceOption");
                if (options.length > 0) {
    ...
    for (const layer of Array.from(columns.keys()).sort((a, b) => a - b)) {
        ...
        for (let subColumn = 0; subColumn * rows < members.length; subColumn++) {
            ...
            for (const sceneId of stack) {
                positions[sceneId] = { x: x + subColumn * (SCENE_FLOW_NODE_WIDTH + COLUMN_GAP), y: Math.round(y) };
...
export function buildSceneFlowGraph(document: StoryDocument, options?: SceneFlowGraphOptions): SceneFlowGraph {
