...
describe("avatarBakeFingerprint", () => {
    ...
    it("moves when the framing moves", () => {
        ...
        const b = avatarBakeFingerprint({ layerHashes: ["h1"], crop: { x: 0, y: 0, w: 0.5, h: 0.5 }, maxSize: 256 });
        expect(a).not.toBe(b);
    });
    ...
    it("holds still when nothing that affects the picture changed", () => {
        ...
        expect(avatarBakeFingerprint({ layerHashes: ["h1", null], crop, maxSize: 256 }))
            .toBe(avatarBakeFingerprint({ layerHashes: ["h1", null], crop, maxSize: 256 }));
    });
});
...
describe("per-differential framing", () => {
    ...
    function cropSensitiveRenderer(): AvatarRenderer {
        return vi.fn<AvatarRenderer>(async ({ crop }) => new Uint8Array([crop ? Math.round(crop.x * 100) : 0]));
    }
    ...
    it("re-bakes exactly the differential that was reframed", async () => {
        ...
        const reframed = { ...first.avatars, "p-angry": { ...first.avatars["p-angry"], portrait: ENTRY_CROP } };
        const second = await bakeCharacterAvatars(io, render, {
            characterId: "alice",
    ...
    it("keeps the crop of a differential whose art is missing", async () => {
        ...
        const report = await bakeCharacterAvatars(io, vi.fn<AvatarRenderer>(async () => null), {
            characterId: "alice",
