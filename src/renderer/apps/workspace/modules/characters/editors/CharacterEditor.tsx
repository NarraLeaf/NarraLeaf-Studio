...
} from "@/lib/workspace/services/character/characterCombinations";
...
export function CharacterEditor({ payload }: EditorComponentProps<CharacterEditorPayload>) {
  ...
  const tags = useMemo(
    () => appearance?.resolveTagSelection(previewTags) ?? {},
    [appearance, previewTags, version]
  );
  ...
  const overriddenAvatarKeys = useMemo(
    () =>
      new Set(
        Object.entries(appearance?.getAvatars() ?? {})
  ...
  const drawList =
    appearance && kind === "layered" ? appearance.resolveDrawList({ tags: previewTags }) : [];
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void getSpriteCompositor(assetsService)
      .occlusion(drawList)
      .then((flags) => {
        if (cancelled || !appearance) return;
        const ids = appearance.getLayers();
        setOccluded(
          Object.fromEntries(flags.map((covered, index) => [ids[index]?.id ?? "", covered]))
        );
      })
      .catch(() => {
        if (!cancelled) setOccluded({});
      });
    return () => {
      cancelled = true;
    };
  ...
  const toggleAvatarAxis = useCallback(
    (axisId: string) => {
  ...
  const nameCombination = useCallback(
  ...
  const confirmDelete = useCallback(
    async (name: string, noun: keyof typeof DELETE_DETAIL): Promise<boolean> => {
  ...
  const handleInspectorResize = useCallback(
    (delta: number): number => {
      ...
      const maxWidth = Math.max(
        CHARACTER_INSPECTOR_MIN_WIDTH,
        containerWidth * CHARACTER_INSPECTOR_MAX_FRACTION
      );
      const nextWidth = Math.round(
        Math.min(maxWidth, Math.max(CHARACTER_INSPECTOR_MIN_WIDTH, width - delta))
      );
      if (nextWidth !== width) {
  ...
  const visibleLayers: PreviewLayer[] =
    kind === "preset"
      ? [{ id: presetPreviewId, assetId: previewPose?.assetId ?? null }]
      : layers
          .filter((layer) => !hidden[layer.id])
          .map((layer) => ({ id: layer.id, assetId: draw(layer.id, tags) }));
  ...
  const onionLayers: PreviewLayer[] | null = (() => {
    ...
    return layers
      .filter((layer) => !hidden[layer.id])
      .map((layer) => ({ id: layer.id, assetId: draw(layer.id, selection) }));
  })();
  ...
  const measured = Object.fromEntries(
    layers
      .filter((layer) => draw(layer.id, tags))
      .map((layer) => [layer.id, sizes[layer.id]])
      .filter(([, size]) => size)
  ...
  const setCanvasFromLargest = () => {
    const largest = Object.values(measured).sort(
  ...
  const scopedCrop =
    kind === "preset"
      ...
      : avatarKey
        ? (appearance.getAvatarPortrait(avatarKey) ?? undefined)
        : undefined;
  const cropScoped =
    scopeIntent && scopeIntent.key === avatarKey ? scopeIntent.on : scopedCrop !== undefined;
  ...
  const bakeReceipt =
    bakeSummary && (bakeSummary.written || bakeSummary.unresolved || bakeSummary.removed)
  ...
  const gridToggle = (
    <button
      ...
      data-tip={t("characters.editor.combinations.title")}
  ...
  return (
    ...
    <div className="h-full bg-surface text-fg flex flex-col" data-help-topic="appearances">
      <div className="px-4 py-2 border-b border-edge flex items-center gap-2">
        ...
        <span className="text-xs text-fg-subtle">{characterKindLabel(kind, t)}</span>
      </div>
          ...
          <div className="flex min-h-0 min-w-0 flex-1 flex-col">
            {grid ? (
              <CombinationGrid
                ...
                onPick={(combination) => {
                  setPreviewTags(combination.tags);
                  setGrid(false);
                }}
                  ...
                  kind === "layered" ? (
                    <>
                      ...
                      <button
                        ...
                        onClick={() =>
                          patchView({
                            onionAxisId: onionAxisId
                              ? null
                              : (axes.find((axis) => axis.tags.length > 1)?.id ?? null)
                          })
                        }
                      >
            ...
            <>
              ...
              {kind === "preset" ? (
                <Section
                  ...
                  onAdd={() =>
                    appearance.createPose(
                      nextAutoName(
                        (n) => t("characters.editor.defaultPoseName", { n: String(n) }),
                        poses
                      )
                    )
                  }
                  ...
                  {poses.map((pose) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={(event) => {
                          event.stopPropagation();
                          openSlot({ kind: "pose", poseId: pose.id }, event.currentTarget);
                        }}
                      ...
                      <button
                        ...
                        onClick={async (event) => {
                          event.stopPropagation();
                          if (await confirmDelete(pose.name, "pose"))
                ...
                <>
                  <Section
                    ...
                    onAdd={() =>
                      appearance.createAxis(
                        nextAutoName(
                          (n) => t("characters.editor.defaultAxisName", { n: String(n) }),
                          axes
                        )
                      )
                    }
                  >
                    {axes.map((axis) => (
                      <div
                        ...
                        className={cn(
                          CARD,
                          focus?.kind === "axis" && focus.id === axis.id ? FOCUSED : "border-edge"
                        )}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={async (event) => {
                              event.stopPropagation();
                              if (await confirmDelete(axis.name, "axis"))
                                appearance.removeAxis(axis.id);
                            }}
                          ...
                          {axis.tags.map((tag) => (
                            ...
                            >
                              ...
                              <DefaultStar
                                ...
                                onSet={(event) => {
                                  event.stopPropagation();
                                  appearance.setAxisDefaultTag(axis.id, tag.id);
                                }}
                                writes={freeze.writes(false, t("characters.editor.makeDefault"))}
                              ...
                              <button
                                className={cn(
                                  ICON_BTN,
                                  "shrink-0 opacity-0 focus-visible:opacity-100 group-hover/tag:opacity-100"
                                )}
                                onClick={async (event) => {
                                  event.stopPropagation();
                                  if (await confirmDelete(tag.name, "tag"))
                                    appearance.removeTag(axis.id, tag.id);
                                }}
                  ...
                  <Section
                    ...
                    onAdd={() =>
                      appearance.createLayer(
                        nextAutoName(
                          (n) => t("characters.editor.defaultLayerName", { n: String(n) }),
                          layers
                        )
                      )
                    }
                  >
                    ...
                    {[...layers].reverse().map((layer) => {
                      const isFocused =
                        (focus?.kind === "layer" && focus.id === layer.id) ||
                      ...
                      return (
                        <div
                          ...
                          onDrop={(event) => {
                            ...
                            if (freeze.frozen) return;
                            const dragged =
                              dragRef.current || event.dataTransfer.getData("text/plain");
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <InlineName
                              ...
                              onEditingChange={(editing) =>
                                setRenamingId(editing ? layer.id : null)
                              }
                            ...
                            <button
                              ...
                              data-tip={t(
                                hidden[layer.id]
                                  ? "characters.editor.showLayer"
                                  : "characters.editor.hideLayer"
                              )}
                              aria-label={t(
                                hidden[layer.id]
                                  ? "characters.editor.showLayer"
                            ...
                            <button
                              ...
                              data-tip={t(
                                locked[layer.id]
                                  ? "characters.editor.unlockLayer"
                                  : "characters.editor.lockLayer"
                              )}
                              aria-label={t(
                                locked[layer.id]
                                  ? "characters.editor.unlockLayer"
                            ...
                            <Select
                              ...
                              value={layer.axisId ?? ""}
                              onChange={(value) =>
                                appearance.setLayerAxis(layer.id, String(value) || null)
                              }
                            ...
                            <button
                              ...
                              onClick={async (event) => {
                                event.stopPropagation();
                                if (await confirmDelete(layer.name, "layer"))
                                  appearance.removeLayer(layer.id);
                              }}
                              aria-label={t("characters.editor.removeLayer")}
                              {...freeze.writes(
                                locked[layer.id],
                                t("characters.editor.removeLayer")
                              )}
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                                ...
                                <div key={tag.id} className="flex items-center gap-2 text-2xs">
                                  ...
                                  <button
                                    ...
                                    onClick={(event) => {
                                      event.stopPropagation();
                                      openSlot(
                                        { kind: "option", layerId: layer.id, tagId: tag.id },
                                        event.currentTarget
                                      );
                                    }}
                                    aria-label={t("characters.editor.changeImage")}
                    ...
                    {appearance.getSnapshots().map((snapshot) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={async (event) => {
                            event.stopPropagation();
                            if (await confirmDelete(snapshot.name, "snapshot"))
                              appearance.removeSnapshot(snapshot.id);
                          }}
