...
export function PuppetEditor(props: { appearance: CharacterAppearance }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void readPuppetRuntimeInstallState(context.project, puppet.backend)
      .then((state) => {
  ...
  const runtimeRow = (
    <Field label={t("characters.editor.puppet.stepRuntime")}>
      ...
      <span
        className={
          runtimeReady ? "shrink-0 text-2xs text-primary" : "shrink-0 text-2xs text-fg-subtle"
        }
      >
        {runtimeStatusLabel}
      </span>
  ...
  const modelRow = (
    <Field label={t("characters.editor.puppet.stepModel")}>
      <span className="min-w-0 flex-1 truncate">
        {modelName ?? (
          <span className="text-fg-subtle">{t("characters.editor.puppet.noModel")}</span>
        )}
      </span>
