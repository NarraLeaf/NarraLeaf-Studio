...
export function PsdImportWizard(props: {
  ...
  const runImport = useCallback(async () => {
    ...
    try {
      ...
      const slots = applyPsdPlan(
        props.appearance,
        plan,
        (path) => assetIds.get(joinPath(path)) ?? null
      );
  ...
  const attachedTo = (leaf: PsdLeaf) =>
    (plan.attachments[joinPath(leaf.path)] ?? []).map((entry) => (
    ...
    <Modal
      ...
      footer={
        <div className="flex w-full items-center gap-2">
          ...
          >
            {busy === "importing"
              ? t("characters.editor.psd.importing")
              : t("characters.editor.psd.import")}
          </button>
        </div>
      }
    >
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-xs">
            ...
            {cost && (
              ...
              >
                {t("characters.editor.psd.cost", {
                  layers: cost.layers,
                  megabytes: cost.megabytes
                })}
              </span>
            )}
            <span className="text-fg-subtle" aria-label={t("characters.editor.psd.canvas")}>
                  ...
                  <div key={key} className={ROW} data-psd-blend={key}>
                    ...
                    <button
                      ...
                      data-tip={
                        mergeable
                          ? undefined
                          : t("characters.editor.psd.mergeUnavailable", { mode: leaf.blendMode })
                      }
                      disabled={!mergeable}
                      className={[
