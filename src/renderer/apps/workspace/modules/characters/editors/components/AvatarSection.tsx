...
} from "@/lib/workspace/hooks/useCompositedSprite";
  ...
  return (
    ...
    >
      <div className="mx-auto grid aspect-square w-full max-w-[256px] place-items-center overflow-hidden rounded-md border border-edge bg-fill">
        ...
        {source === "none" && <EmptyState size="sm" icon={<UserRound className="h-6 w-6" />} />}
      </div>
        ...
        props.onToggleScope && (
          ...
          <div className="flex items-center gap-2">
            ...
            <FieldLabel as="span" className="mb-0">
              {t("characters.preview.portraitScoped")}
            </FieldLabel>
          </div>
        )}

      {props.receipt && <div className="truncate text-2xs text-fg-subtle">{props.receipt}</div>}
    </SectionCard>
  );
}
