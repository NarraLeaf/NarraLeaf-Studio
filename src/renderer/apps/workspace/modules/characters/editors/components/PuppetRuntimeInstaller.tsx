...
const ROW =
  "flex items-center gap-2 rounded-md border border-edge bg-fill-subtle px-2 py-1.5 text-xs";
...
export function PuppetRuntimeInstaller(props: {
  ...
  const runtime: KnownPuppetRuntime | null =
    props.target.kind === "known" ? knownPuppetRuntimeFor(props.target.id) : null;

  // A custom runtime has no terms of ours to show, so it starts at the picker.
  const firstStep = useCallback(
    (): Phase => (runtime ? { step: "notice" } : { step: "pick" }),
    [runtime]
  );
  ...
  const openFor = props.visible
    ? props.target.kind === "known"
      ? props.target.id
      : "custom"
    : null;
  ...
  const run = useCallback(
    async (
      ...
      work: () => Promise<{ backend: string; summary: string; note?: string }>
    ) => {
  ...
  const installPrebuilt = useCallback(
    async (pick: () => Promise<string | null>, kind: "directory" | "file") => {
      if (!context) return;
      ...
      await run("characters.editor.runtime.copying", async () => {
        const installed = await installPrebuiltPuppetRuntime(context.project, name, {
          kind,
          path
        } as never);
        return {
    ...
    if (phase.step === "failed") {
      return (
        <div className="space-y-2 py-2">
          ...
          <p className="whitespace-pre-wrap break-words px-1 text-2xs text-fg-muted">
            {phase.message}
          </p>
        </div>
      );
        ...
        <div className="space-y-3 py-1">
          ...
          <div className="space-y-1.5">
            ...
            <button
              className="flex items-center gap-1.5 text-xs text-primary hover:underline"
              onClick={() =>
                void getInterface().app.openExternal(puppetRuntimeDocsUrl(runtime, locale))
              }
            >
        ...
        {!runtime && (
          <>
            <p className="px-1 text-2xs text-fg-subtle">
              {t("characters.editor.runtime.customNameHint")}
            </p>
            ...
            <button
              ...
              onClick={() =>
                void getInterface().app.openExternal(customPuppetRuntimeDocsUrl(locale))
              }
            >
              <BookOpen className="h-3.5 w-3.5" />
