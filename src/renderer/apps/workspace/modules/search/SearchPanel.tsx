...
import type {
  SearchGroup,
  SearchGroupResult
} from "@/lib/workspace/services/search/searchIndexModel";
...
export function SearchPanel() {
  ...
  const runQuery = useCallback(() => {
    ...
    setResults(
      searchService.search(queryRef.current, {
  ...
  const visibleResults = useMemo(
    () =>
      activeGroups.length === 0
        ? results
        : results.filter((group) => activeGroups.includes(group.group)),
  ...
  const replaceTitle =
    plan && !plan.applicable && plan.failures.length > 0
