...
function restoredLayout(store: {
  restoreEditorLayout: { mock: { calls: unknown[][] } };
}): EditorLayout | null {
...
describe("workspace editor session", () => {
  ...
  it("restores a split tree with its ratios and per-pane focus", () => {
    ...
    const blueprintTab = {
      ...
      payload: {
        blueprintId: "blueprint-1",
        ownerKind: "surfaceMain" as const,
        surfaceId: "surface-1"
      }
    };
    const session: WorkspaceEditorSessionV2 = {
      ...
      layout: {
        ...
        second: {
          ...
          tabs: [
            { kind: "storyMotion", title: "Story Motion", payload: { animationId: "motion-1" } }
          ]
        }
    ...
    const session: WorkspaceEditorSessionV2 = {
      ...
      layout: {
        ...
        first: {
          ...
          id: "main",
          focus: null,
          tabs: [{ kind: "surface", surfaceId: "gone" }]
        },
        second: {
          kind: "group",
          id: "group-1",
...
describe("workspace editor session serialization", () => {
  ...
  it("round-trips a split layout through serialize and parse", () => {
    ...
    const parsed = parseWorkspaceEditorSession(
      JSON.parse(JSON.stringify(serializeEditorSession(layout, "group-1")))
    );
  ...
  it("round-trips blank new-tab pages, keeping each one's identity", () => {
    ...
    const parsed = parseWorkspaceEditorSession(
      JSON.parse(JSON.stringify(serializeEditorSession(layout)))
    );
  ...
  it("round-trips a surface-less blueprint tab through serialize and parse", () => {
    ...
    const parsed = parseWorkspaceEditorSession(
      JSON.parse(JSON.stringify(serializeEditorSession(layout, "main")))
    );
