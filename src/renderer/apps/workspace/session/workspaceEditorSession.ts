...
import {
  TEXT_EDITOR_TAB_PREFIX,
  getTextEditorTabId
} from "@/apps/workspace/modules/assets/editors/text/textEditorTabId";
...
export function getWorkspaceEditorSessionSettingsKey(
  projectRef: WorkspaceEditorSessionProjectRef
): string {
...
function isSerializedTab(value: unknown): value is SerializedTab {
  ...
  if (
    kind === "storyScene" &&
    typeof o.title === "string" &&
    isStorySceneEditorPayload(o.payload)
  ) {
    return true;
  }
  if (
    kind === "storyMotion" &&
    typeof o.title === "string" &&
    isStoryMotionEditorPayload(o.payload)
  ) {
    return true;
  }
  return false;
  ...
  if (o.kind === "split") {
    ...
    return {
      ...
      ratio: Math.min(
        ...
        Math.max(EDITOR_SPLIT_RATIO_EPSILON, o.ratio)
      ),
      first,
      second
    };
 ...
 */
export function buildTabDefinition(
  ctx: WorkspaceContext,
  entry: SerializedTab
): EditorTabDefinition | null {
  ...
  if (entry.kind === "blueprint") {
    ...
    if (
      (p.ownerKind === "surfaceMain" ||
        p.ownerKind === "widgetMain" ||
        p.ownerKind === "widgetValue") &&
  ...
  if (entry.kind === "assetImage") {
    ...
    const asset = assetsService.getAssets()[AssetType.Image][entry.assetId] as
  ...
  if (entry.kind === "assetAudio") {
    const assetsService = ctx.services.get<AssetsService>(Services.Assets);
    const asset = assetsService.getAssets()[AssetType.Audio][entry.assetId] as
  ...
  if (entry.kind === "assetText") {
    const assetsService = ctx.services.get<AssetsService>(Services.Assets);
    const asset = assetsService.getAssets()[AssetType.Other][entry.assetId] as
  ...
  if (entry.kind === "storyMotion") {
    ...
    if (
      !storyService
        .listAnimationAssets()
        .some((animation) => animation.id === entry.payload.animationId)
    ) {
      return null;
    }
    return {
...
function buildLayoutNode(ctx: WorkspaceContext, node: SerializedEditorNode): EditorLayout | null {
  if (node.kind === "group") {
    ...
    const focus =
      node.focus && tabs.some((tab) => tab.id === node.focus)
        ? node.focus
