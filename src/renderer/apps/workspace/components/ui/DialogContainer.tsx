...
function DialogComponent({ dialog, onClose }: { dialog: Dialog; onClose: () => void }) {
  ...
  const defaultButtonIndex =
    dialog.buttons?.findIndex((button) => button.primary && !button.disabled && !button.danger) ??
    -1;
  ...
  const fallbackButtonIndex =
    dialog.buttons?.findIndex((button) => !button.disabled && !button.danger) ?? -1;
    ...
    <div className="nl-window-content-layer z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div
          ...
          style={{
            maxHeight: dialog.height ? `calc(${dialog.height}px - 140px)` : "calc(90vh - 140px)"
          }}
        >
          {dialog.message && (
            <p className="text-sm text-fg whitespace-pre-wrap">{dialog.message}</p>
          )}
          {dialog.content && <div className="text-fg">{dialog.content}</div>}
        </div>
