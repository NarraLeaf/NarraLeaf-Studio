...
export function useEditorGroupDrop(group: EditorGroup) {
  ...
  const resolveTarget = useCallback(
    (
      ...
      | { kind: "strip"; insertion: EditorTabInsertion }
      | { kind: "zone"; zone: EditorDropZone }
      | null => {
      const strip = stripRef.current;
      const overStrip =
        strip !== null && eventTarget instanceof Node && strip.contains(eventTarget);
      ...
      if (overStrip && strip) {
        ...
        const edge =
          index < rects.length
            ? rects[index].left
            : (rects[rects.length - 1]?.left ?? stripRect.left);
  ...
  const onDrop = useCallback(
    (e: DragEvent<Element>) => {
      ...
      const primary =
        assetDrop.resolved.find((a) => a.id === assetDrop.wire.p) ?? assetDrop.resolved[0];
      ...
      openAssetPreviewTabsInEditor(context, assetDrop.resolved, {
        ...
        splitInto:
          target.kind === "zone" ? (editorDropZoneToSplit(target.zone) ?? undefined) : undefined
      });
    },
