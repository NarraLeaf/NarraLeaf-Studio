...
import {
  collectQuickOpenEntries,
  QUICK_OPEN_KIND_LABEL_KEYS,
  type QuickOpenEntry
} from "./quickOpenModel";
...
export function QuickOpenPicker() {
  ...
  const ranked = useMemo(
    () =>
      rankFuzzyList(entries, query, (entry) => [entry.title, entry.detail ?? ""]).slice(
        0,
        MAX_ROWS
      ),
    [entries, query]
  ...
  return (
    <>
      ...
      <div
        className="nl-window-content-layer z-[45] bg-black/15 animate-fade-in"
        onMouseDown={close}
      />
      <QuickSwitchOverlay
        zClassName="z-[46]"
        placementClassName={
          anchorLeft === null ? "items-start justify-center pt-1" : "items-start pt-1"
        }
        ...
        emptyText={
          query.trim()
            ? t("workspace.shell.quickOpen.empty")
            : t("workspace.shell.quickOpen.placeholder")
        }
        search={{
          value: query,
