...
describe("the version rail in the dock account", () => {
  ...
  it("takes the rail out of a sidebar's ceiling, px for px", () => {
    ...
    for (const versionRailWidth of RAIL_WIDTHS) {
      ...
      expect(maxSidebarWidth("left", withRail, 320)).toBe(
        maxSidebarWidth("left", bare, 320) - versionRailWidth
      );
      expect(maxSidebarWidth("right", withRail, 320)).toBe(
        maxSidebarWidth("right", bare, 320) - versionRailWidth
      );
    }
  });
    ...
    for (const windowWidth of [900, 1000, 1376, 1600, 1920, 2560]) {
      for (const versionRailWidth of RAIL_WIDTHS) {
        for (const intent of [
          ...
          expect(residualEditorWidth(withRail, resolveDock(intent, withRail)), where).toBe(
            residualEditorWidth(bare, resolveDock(intent, bare))
          );
        }
      }
    }
  ...
  it("keeps the editor at or above its floor at every rail width, in the cases the clamp policy covers", () => {
    ...
    const snugFor = (versionRailWidth: number) =>
      2 * RAIL_SELECTOR_WIDTH +
      versionRailWidth +
      DOCK_REGIONS.left.default +
      DOCK_REGIONS.right.default +
      EDITOR_FLOOR.width;
  ...
  it("gives the space back when the rail collapses (nothing mutates the stored intent)", () => {
    const intent = { left: 700, right: 320, bottom: 256 };
    const expanded = resolveDock(
      intent,
      env({ windowWidth: 1500, versionRailWidth: VERSION_RAIL_EXPANDED_WIDTH })
    );
    const collapsed = resolveDock(
      intent,
      env({ windowWidth: 1500, versionRailWidth: VERSION_RAIL_COLLAPSED_WIDTH })
    );
  ...
  it("stops a drag at the rail-aware ceiling", () => {
    const e = env({
      windowWidth: 1500,
      rightVisible: false,
      versionRailWidth: VERSION_RAIL_EXPANDED_WIDTH
    });
    const { next } = applyResize("left", maxSidebarWidth("left", e, 0), 200, e, 0);
    expect(next).toBe(maxSidebarWidth("left", e, 0));
    expect(
      residualEditorWidth(e, resolveDock({ left: next, right: 320, bottom: 256 }, e))
    ).toBeGreaterThanOrEqual(EDITOR_FLOOR.width);
  });
});
...
describe("residualEditorWidth", () => {
  it("counts only the sidebars that are actually on screen", () => {
    const e = env({
      windowWidth: 1600,
      rightVisible: false,
      versionRailWidth: VERSION_RAIL_COLLAPSED_WIDTH
    });
  ...
  it("goes negative rather than lying when the minima no longer fit", () => {
    ...
    expect(
      residualEditorWidth(e, resolveDock({ left: 320, right: 320, bottom: 256 }, e))
    ).toBeLessThan(0);
  });
});
