...
export function ProjectSwitcher({ versionSurface }: { versionSurface: VersionSurface }) {
  ...
  const others = recentProjects.filter(
    (project) => normalizeProjectPath(project.path) !== currentIdentity
  );
      ...
      {open && (
        ...
        >
          ...
          <SwitcherAction
            ...
            label={t("workspace.shell.projectSwitcher.openProject")}
            onClick={handleOpenFolder}
          />
          <SwitcherAction
            icon={<Plus className="w-4 h-4" />}
            label={t("workspace.shell.projectSwitcher.newProject")}
  ...
  return (
    <>
      ...
      >
        ...
        <span
          className={cn("truncate text-sm tabular-nums", onRevision ? "text-primary" : "text-fg")}
        >
          {face.text}
        </span>
