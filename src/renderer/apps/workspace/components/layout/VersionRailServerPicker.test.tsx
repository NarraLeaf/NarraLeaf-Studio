...
function rows(): string[] {
  return [...document.querySelectorAll("[data-server-choice], [data-vcs-seam='picker-add']")].map(
    (node) => node.getAttribute("data-server-choice") ?? "add"
  );
}
...
describe("the server picker", () => {
  ...
  it("offers it where nothing has been added, which is the case it is most needed in", async () => {
    ...
    expect(document.querySelector("[data-vcs-seam='server-picker']")?.textContent).toContain(
      "workspace.shell.versionControl.server.picker.empty"
    );
  });
  ...
  it("opens Settings at the servers panel and closes, rather than asking for a token here", async () => {
    ...
    await waitFor(() =>
      expect(document.querySelector("[data-vcs-seam='picker-add']")).not.toBeNull()
    );
  ...
  it("opens with nothing chosen for a project that uses no server", async () => {
    ...
    await waitFor(() =>
      expect(document.querySelector(`[data-server-choice='${ONE}']`)).not.toBeNull()
    );
  ...
  it("opens on the server the project already uses, with the name it has there", async () => {
    ...
    await waitFor(() =>
      expect(
        document.querySelector(`[data-server-choice='${TWO}']`)?.getAttribute("aria-pressed")
      ).toBe("true")
    );
  ...
  it("keeps the address field, below the add row, for a server nobody signs in to", async () => {
    ...
    const nodes = [
      ...body.querySelectorAll("[data-vcs-seam='picker-add'], [data-vcs-seam='picker-address']")
    ];
    expect(nodes.map((node) => node.getAttribute("data-vcs-seam"))).toEqual([
      "picker-add",
      "picker-address"
    ]);
    ...
    expect(document.querySelector<HTMLInputElement>("input")?.value).toBe(
      "lore://plain.example.lan:41337/my-game"
    );
  });
});
