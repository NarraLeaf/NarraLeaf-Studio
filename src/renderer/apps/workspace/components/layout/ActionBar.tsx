 ...
 */
const ACTIONS_OWNED_BY_RUN_CONTROL: ReadonlySet<string> = new Set<string>([
  WorkspaceMenuAction.Build
]);
...
export function ActionBar({ hideAllGroups = false }: ActionBarProps) {
  ...
  const visibleActionGroups = hideAllGroups
    ? []
    : actionGroups.filter(
        (group) => getVisibleActionMenuItems(getActionGroupItems(group), focusContext).length > 0
      );
