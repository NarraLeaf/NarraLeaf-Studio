...
): VcsHistoryEntry => ({ revision, number, parents, kind });
...
describe("resolveVersionRailPresence", () => {
  ...
  it.each(table)("%s / expanded %s / frozen %s -> %s", (stateKey, expanded, frozen, presence) => {
    expect(resolveVersionRailPresence({ state: states[stateKey], expanded, frozen })).toBe(
      presence
    );
  });
  ...
  it("contributes nothing to the dock account wherever it is absent", () => {
    for (const [stateKey, expanded, frozen, presence] of table) {
      ...
      expect(
        versionRailWidth(resolveVersionRailPresence({ state: states[stateKey], expanded, frozen }))
      ).toBe(0);
    }
  });
  ...
  it("is never dismissible into nothing while the workspace is frozen", () => {
    ...
    for (const stateKey of [
      ...
      expect(
        resolveVersionRailPresence({ state: states[stateKey], expanded: false, frozen: true })
      ).toBe("strip");
    }
  });
});
...
describe("commit availability", () => {
  ...
  it.each([
    ...
    expect(canCommit({ state: states.current, frozen: false, busy: false, changedFiles })).toBe(
      pressable
    );
  });
...
describe("resolveVersionSurfaceState", () => {
  ...
  it("reports unavailability with its reason, ahead of every other question", () => {
    const state = resolveVersionSurfaceState(
      inputs({
  ...
  it("offers to enable version control when the backend works but the project has no repository", () => {
    expect(resolveVersionSurfaceState(inputs({ isRepository: false }))).toEqual({
  ...
  it("reports the working tree with the head's number", () => {
    expect(resolveVersionSurfaceState(inputs())).toEqual({
      kind: "current",
      head: "aaaa1111",
      number: 4
    });
  ...
  it("still reports the working tree when info has not answered yet", () => {
    expect(resolveVersionSurfaceState(inputs({ headNumber: null }))).toEqual({
      kind: "current",
      head: "aaaa1111",
      number: null
    });
...
describe("unavailableReasonKey", () => {
  it("blames the machine for an unsupported platform", () => {
    expect(unavailableReasonKey("unsupported-platform")).toBe(
  ...
  it("blames the installation for both backend failures", () => {
    ...
    expect(unavailableReasonKey("backend-missing")).toBe(
      "workspace.shell.versionControl.unavailable.installation"
    );
    expect(unavailableReasonKey("backend-load-failed")).toBe(
      "workspace.shell.versionControl.unavailable.installation"
    );
  });
});
...
describe("collapseCheckpoints", () => {
  ...
  it("shows everything when asked", () => {
    expect(collapseCheckpoints(flat, { showCheckpoints: true }).map((row) => row.revision)).toEqual(
      ["d", "c", "b", "a"]
    );
  });
...
describe("splitChangePath", () => {
  ...
  it("splits a deep path at its LAST separator, keeping the file name whole", () => {
    expect(splitChangePath("editor/story/chapter-01.json")).toEqual({
      directory: "editor/story",
      name: "chapter-01.json"
    });
  });
  ...
  it("reads a backslash path, because the type does not say which separator a scan produced", () => {
    ...
    expect(splitChangePath("assets\\images/bg.png")).toEqual({
  ...
  it("keeps a dotfile's name intact instead of reading the leading dot as an extension boundary", () => {
    ...
    expect(splitChangePath("project/.gitignore")).toEqual({
      directory: "project",
      name: ".gitignore"
    });
  });
  ...
  it("does not care what alphabet the path is in", () => {
    expect(splitChangePath("剧本/第一章/开场.json")).toEqual({
      directory: "剧本/第一章",
      name: "开场.json"
    });
  });
...
describe("buildChangeList", () => {
  ...
  it("caps the list and says how many rows it did not show", () => {
    const files = Array.from({ length: 130 }, (_, index) =>
      change(`f${String(index).padStart(3, "0")}.json`)
    );
  ...
  it("sorts before capping, so the conflict can never be the row that got cut", () => {
    ...
    const files = [
      ...Array.from({ length: 60 }, (_, index) =>
        change(`a${String(index).padStart(3, "0")}.json`)
      ),
...
describe("historyRowHeadline", () => {
  ...
  const t = ((key: string, params?: Record<string, string>) =>
    params ? `${key}(${Object.values(params).join(",")})` : key) as unknown as Parameters<
    typeof historyRowHeadline
  >[1];

  it("leads with the version's message", () => {
    expect(
      historyRowHeadline({ revision: "a91f3c8d2e4b6", message: "Chapter 2 opening" }, t)
  ...
  it("names itself with its hash when the revision carries no message", () => {
    ...
    expect(historyRowHeadline({ revision: "a91f3c8d2e4b6" }, t)).toEqual({
      text: "a91f3c8",
      isIdentity: true,
      original: null
    });
  ...
  it("keeps the version a restore went back to, which is a number rather than language", () => {
    expect(
      historyRowHeadline({ revision: "a91f3c8d2e4b6", message: composeRestoreMessage("#19") }, t)
    ).toEqual({
  ...
  it("leaves an author's own words alone, even when they start like one of ours", () => {
    ...
    expect(historyRowHeadline({ revision: "a91f3c8d2e4b6", message: own }, t)).toEqual({
      text: own,
      isIdentity: false,
      original: null
    });
...
describe("filterHistoryRows", () => {
  ...
  const rows: FlatHistoryEntry[] = [
    { revision: "aaa1111", number: 25, message: "Chapter 2 opening", author: "Aria", merge: false },
    {
      revision: "bbb2222",
      number: 24,
      message: VCS_CHECKPOINT_MESSAGES.interval,
      author: "Aria",
      merge: false
    },
    {
      revision: "ccc3333",
      number: 3,
      message: "Rewrote the confession",
      author: "Kai",
      merge: false
    }
  ];
  ...
  it("matches what the row SHOWS, not only what it stores", () => {
    ...
    expect(filterHistoryRows(rows, "checkpoint", t).map((row) => row.revision)).toEqual([
      "bbb2222"
    ]);
  });
...
describe("history day separators", () => {
  ...
  it("names the two nearest days and dates the rest", () => {
    ...
    expect(historyDayLabel(at(2026, 7, 12, 9), "en", t, now)).toBe(
      "workspace.shell.versionControl.today"
    );
    expect(historyDayLabel(at(2026, 7, 11, 9), "en", t, now)).toBe(
      "workspace.shell.versionControl.yesterday"
    );
  ...
  it("is right across a month boundary, where subtracting a day is the easy thing to get wrong", () => {
    ...
    expect(historyDayLabel(at(2026, 7, 31, 20), "en", t, now)).toBe(
      "workspace.shell.versionControl.yesterday"
    );
  });
});
...
describe("versionFace", () => {
  ...
  it("ignores surrounding whitespace rather than rendering an empty branch", () => {
    ...
    expect(versionFace({ state: states.current, branch: ` ${VCS_DEFAULT_BRANCH} ` }, t).text).toBe(
      "#12"
    );
  });
});
...
describe("which server the connect dialog opens on", () => {
  const servers = [
    { remoteOrigin: "lore://one.example.lan:41337" },
    { remoteOrigin: "lore://two.example.lan:41337" }
  ];
  ...
  it("opens on the server this project already uses", () => {
    ...
    expect(initialServerChoice(servers, "lore://two.example.lan:41337/my-game")).toBe(
      "lore://two.example.lan:41337"
    );
  });

  it("opens on that server for a remote with no name on it", () => {
    expect(initialServerChoice(servers, "lore://two.example.lan:41337")).toBe(
      "lore://two.example.lan:41337"
    );
  });
  ...
  it("opens on the address field where the project's server is not one of these", () => {
    ...
    expect(initialServerChoice(servers, "lore://plain.example.lan:41337/my-game")).toBe(
      MANUAL_SERVER
    );
  });
