...
export function WorkspaceLayout({ title, iconSrc }: WorkspaceLayoutProps) {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        const rightVisible = await settingsService.get<boolean>(
          SETTINGS_KEYS.RIGHT_SIDEBAR_VISIBLE
        );
        const bottomVisible = await settingsService.get<boolean>(
          SETTINGS_KEYS.BOTTOM_PANEL_VISIBLE
        );
        ...
        // Load active panels
        const leftPanel = normalizeStoredPanelId(
          await settingsService.get<string | null>(SETTINGS_KEYS.LEFT_SIDEBAR_ACTIVE_PANEL)
        );
        const rightPanel = normalizeStoredPanelId(
          await settingsService.get<string | null>(SETTINGS_KEYS.RIGHT_SIDEBAR_ACTIVE_PANEL)
        );
        const bottomPanel = normalizeStoredPanelId(
          await settingsService.get<string | null>(SETTINGS_KEYS.BOTTOM_PANEL_ACTIVE_PANEL)
        );

        // Only update if values exist in settings
        if (leftVisible !== undefined)
          setLeftSidebarVisible(Boolean(leftVisible && leftPanel !== null));
        if (rightVisible !== undefined)
          setRightSidebarVisible(Boolean(rightVisible && rightPanel !== null));
        if (bottomVisible !== undefined)
          setBottomPanelVisible(Boolean(bottomVisible && bottomPanel !== null));
        ...
        const railExpanded = await settingsService.get<boolean>(
          SETTINGS_KEYS.VERSION_RAIL_EXPANDED
        );
        ...
        for (const position of [PanelPosition.Left, PanelPosition.Right, PanelPosition.Bottom]) {
          const savedOrder = await settingsService.get<string[]>(
            ORDER_SETTINGS_KEY_BY_POSITION[position]
          );
        ...
        const savedCollapsed = await settingsService.get<string[]>(
          SETTINGS_KEYS.LEFT_SIDEBAR_COLLAPSED
        );
  ...
  }, [settingsService, settingsLoaded, debouncedSaveSettings]);
  ...
  useEffect(() => {
    ...
    const handlePanelVisibilityChanged = ({
      panelId,
      visible
    }: {
      panelId: string;
    ...
    const handleCollapsedPanelsChanged = ({
      position,
      collapsed
    }: {
      position: string;
    ...
    const unsubscribeVisibility = uiService
      .getEvents()
      .on("panelVisibilityChanged", handlePanelVisibilityChanged);
    const unsubscribeUnregistered = uiService
      .getEvents()
      .on("panelUnregistered", handlePanelUnregistered);
    const unsubscribeOrder = uiService.getEvents().on("panelOrderChanged", handlePanelOrderChanged);
    const unsubscribeCollapsed = uiService
      .getEvents()
      .on("collapsedPanelsChanged", handleCollapsedPanelsChanged);
    return () => {
    ...
    >
      ...
      {backgroundUrl && (
        ...
        >
          <div
            className="absolute"
            style={backgroundLayerStyle(backgroundSettings, backgroundUrl)}
          />
        </div>
      )}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={leftSidebarVisible && activeLeftPanelId ? "flex" : "hidden"}>
          <LeftSidebar
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
          ...
          <div
            ...
            style={{
              height: bottomPanelVisible && activeBottomPanelId ? `${effective.bottom}px` : 0
            }}
          >
        ...
        <div className={rightSidebarVisible && activeRightPanelId ? "flex" : "hidden"}>
      ...
      <div
        className="absolute"
        style={{ bottom: statusBarHeight, left: railColumnOffsets(dockEnv).sidebarRail }}
      >
        <BottomPanelSelector
