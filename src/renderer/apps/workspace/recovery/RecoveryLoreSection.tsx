...
export function RecoveryLoreSection({ context }: { context: WorkspaceContext | null }) {
    ...
    try {
      ...
      setFeedback(
        result
          ? {
              kind: "ok",
              text: t("workspace.recovery.lore.checkpointDone", { revision: result.revision })
            }
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      setFeedback({
        kind: "ok",
        text: t("workspace.recovery.lore.restoreDone", { version: label })
      });
  ...
  return (
    <section className="px-3 py-3">
      ...
      {confirming && (
        <p className="mt-2 text-xs text-fg-subtle">{t("workspace.recovery.lore.restoreExplain")}</p>
      )}
