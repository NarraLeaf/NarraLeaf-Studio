...
} from "@/lib/app/diagnostics/diagnosticsReport";
import { Services } from "@/lib/workspace/services/services";
import type {
  RecoveryService,
  RecoveryProbeState
} from "@/lib/workspace/services/core/RecoveryService";
...
export function RecoveryPanel() {
  ...
  const handleCopyAll = async () => {
    try {
      ...
      setFeedback({
        kind: "bad",
        text: t("workspace.shell.errorCopyFailed", { error: message(error) })
      });
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setFeedback({
          kind: "bad",
          text: t("workspace.shell.errorExportFailed", { error: result.error ?? "" })
        });
        return;
      }
      if (!result.data.canceled) {
        setFeedback({
          kind: "ok",
          text: t("workspace.shell.errorExported", { path: result.data.filePath ?? "" })
        });
      }
    } catch (error) {
      setFeedback({
        kind: "bad",
        text: t("workspace.shell.errorExportFailed", { error: message(error) })
      });
    } finally {
      setBusy(false);
    }
  ...
  const handleOpenFolder = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setFeedback({
          kind: "bad",
          text: t("workspace.recovery.tools.openFolderFailed", { error: result.error ?? "" })
        });
      }
    } catch (error) {
      setFeedback({
        kind: "bad",
        text: t("workspace.recovery.tools.openFolderFailed", { error: message(error) })
      });
    }
  };
      ...
      <section className="border-b border-edge-subtle px-3 py-3">
        <div className="group/help mb-2 flex items-center gap-2">
          <FieldLabel as="div" className="mb-0">
            {t("workspace.recovery.problems.title")}
          </FieldLabel>
  ...
  return (
    <li className="group relative rounded-md border border-edge bg-fill-subtle px-2 py-1.5">
      <button
        ...
        data-tip={
          copied ? t("workspace.recovery.problems.copied") : t("workspace.recovery.problems.copy")
        }
      >
...
function ProbeStatusIcon({ status }: { status: RecoveryProbeState["status"] }) {
  switch (status) {
    ...
    default:
      return (
        <span className="h-3.5 w-3.5 shrink-0 rounded-full border border-edge-strong" aria-hidden />
      );
  }
}
