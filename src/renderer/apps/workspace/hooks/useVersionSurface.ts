...
} from "@/lib/workspace/services/core/VersionControlService";
...
export function useVersionSurface(): VersionSurface {
  ...
  useEffect(() => {
    ...
    const read = () => {
      ...
      setShown(
        reason?.kind === "revision" ? { revision: reason.revision, label: reason.label } : null
      );
  ...
  const showRevision = useCallback(
    (revision: RevisionId, label?: string) => {
      if (!services) {
        return;
      }
      setBusy("revision");
  ...
  const restoreRevision = useCallback(
    async (revision: RevisionId, label?: string): Promise<boolean> => {
      if (!services || busy !== null) {
        return false;
      }
  ...
  const signInToServer = useCallback(
    async (authUrl: string, token: string): Promise<boolean> => {
      if (!services || busy !== null) {
        return false;
      }
      setBusy("remote");
   ...
   */
  const trustAuthority = useCallback(
    async (certificatePath: string): Promise<boolean> => {
      if (!services || busy !== null) {
        return false;
      }
      setBusy("remote");
  ...
  const keep = useMemo(() => new Set(shown ? [shown.revision] : []), [shown]);
...
function describeFailure(thrown: unknown): VersionFailure {
  ...
  switch (code) {
    ...
    case VcsErrorCode.Unavailable:
      return {
        text: translate("workspace.shell.versionControl.unavailable.installation"),
        tone: "failure"
      };
