...
export class ValidationService {
   ...
   */
  static async validatePathDrive(
    _path: string,
    _platformInfo?: any
  ...
  static async validateDirectory(
    path: string
  ): Promise<{ success: boolean; error?: string; data?: DirectoryValidationResult }> {
    try {
      ...
      const dirExists = this.unwrapFsResult(
        dirExistsResult.data,
        translate("wizard.validation.checkExistenceFailed")
      );
      ...
      const isDir = this.unwrapFsResult(
        isDirResult.data,
        translate("wizard.validation.checkIsDirFailed")
      );
      if (!isDir.success) {
      ...
      const entries = this.unwrapFsResult(
        listResult.data,
        translate("wizard.validation.listContentsFailed")
      );
      if (!entries.success) {
  ...
  private static unwrapFsResult<T>(
    result: FsRequestResult<T> | undefined,
    fallback: string
   ...
   */
  static async validateProjectDirectory(
    path: string,
    platformInfo?: any
  ): Promise<{
    success: boolean;
    ...
    try {
      ...
      if (validationResult.success && validationResult.data) {
        ...
        errors.directory =
          validationResult.error || translate("wizard.validation.validationFailed");
        return { success: false, errors };
      }
    } catch {
  ...
  static isStepValid(step: string, projectData: ProjectData, flow: ProjectFlow): boolean {
    switch (step) {
      ...
      case "import":
        return projectData.packagePath.trim() !== "" && projectData.location.trim() !== "";
      case "source":
      ...
      case "clone":
        return (
          parseVcsRemoteUrl(projectData.remoteUrl) !== null && projectData.location.trim() !== ""
        );
      ...
      case "review":
        return (
          this.isStepValid("project", projectData, flow) &&
