...
export class ProjectService {
  static async createProject(
    ...
    try {
      ...
      // Create directories
      throwException(
        await BaseFileSystemService.createDir(this.resolve(basePath, ProjectNameConvention.NLCache))
      );
      throwException(
        await BaseFileSystemService.createDir(this.resolve(basePath, ProjectNameConvention.Plugins))
      );
      throwException(
        await BaseFileSystemService.createDir(this.resolve(basePath, ProjectNameConvention.Assets))
      );
      throwException(
        await BaseFileSystemService.createDir(
          this.resolve(basePath, ProjectNameConvention.AssetsContent)
        )
      );
      throwException(
        await BaseFileSystemService.createDir(this.resolve(basePath, ProjectNameConvention.Scripts))
      );
      throwException(
        await BaseFileSystemService.createDir(this.resolve(basePath, ProjectNameConvention.Editor))
      );
      throwException(
        await BaseFileSystemService.createDir(
          this.resolve(basePath, ProjectNameConvention.EditorAssets)
        )
      );
      throwException(
        await BaseFileSystemService.createDir(
          this.resolve(basePath, ProjectNameConvention.EditorServices)
        )
      );
      throwException(
        await BaseFileSystemService.createDir(
          this.resolve(basePath, ProjectNameConvention.EditorUI)
        )
      );
      throwException(
        await BaseFileSystemService.createDir(
          this.resolve(basePath, ProjectNameConvention.EditorStory)
        )
      );
      throwException(
        await BaseFileSystemService.createDir(
          this.resolve(basePath, ProjectNameConvention.EditorStoryStories)
        )
      );
      ...
      throwException(
        await BaseFileSystemService.write(editorConfigPath, JSON.stringify(editorConfig), "utf-8")
      );
      ...
      throwException(
        await BaseFileSystemService.write(
          uiDocumentPath,
          JSON.stringify(uiDocument, null, 2),
          "utf-8"
        )
      );
      ...
      for (const type of Object.values(AssetType)) {
        const metadataPath = this.resolve(
          basePath,
          ProjectNameConvention.AssetsMetadataShard(type)
        );
        throwException(
          await BaseFileSystemService.write(metadataPath, JSON.stringify({}), "utf-8")
        );
      }

      // Folders and row order are sharded one level up, by sidebar section.
      for (const category of ASSET_CATEGORY_ORDER) {
        const groupsPath = this.resolve(
          basePath,
          ProjectNameConvention.AssetsGroupsShard(category)
        );
        ...
        throwException(
          await BaseFileSystemService.write(orderPath, EMPTY_ASSET_ORDER_TEXT, "utf-8")
        );
      }
  ...
  private static async enableVersionControl(projectPath: string): Promise<void> {
    try {
      ...
      if (!result.success) {
        console.warn(
          "[Wizard] Project created, but version control could not be enabled:",
          result.error
        );
      }
    } catch (error) {
