...
describe("CloneService", () => {
    ...
    it("accepts a clone that brought down a Studio project", async () => {
        ...
        const result = await CloneService.cloneProject(` lore://studio.example.lan:41337/my-game `, DESTINATION);
        ...
        expect(mocks.vcs.clone).toHaveBeenCalledWith("lore://studio.example.lan:41337/my-game", DESTINATION);
    ...
    it("refuses a clone that is a repository but not a Studio project", async () => {
        ...
        const result = await CloneService.cloneProject("lore://studio.example.lan:41337/notes", DESTINATION);
    ...
    it("passes the backend's own refusal through", async () => {
        mocks.vcs.clone.mockResolvedValue({ success: false, error: "D:/Projects/my-game is not empty" });
