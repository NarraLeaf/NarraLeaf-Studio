...
function graphFor(elementId: string) {
  ...
  const leadsTo = (fromId: string, port: string, toId: string): boolean =>
    edges.some(
      (edge) => edge.from.nodeId === fromId && edge.from.port === port && edge.to.nodeId === toId
    );
...
function assertPrompt(confirm: GraphNode, message: string, answer: string): void {
  ...
  const pins = blueprintNodeRegistry
    ...
    .pins.map((pin) => pin.id);
  expect(pins).toEqual(
    expect.arrayContaining(["in", "message", "button_1_label", "button_1_pressed", "dismissed"])
  );
}
...
describe("the questions the starter template asks before it takes something away", () => {
  ...
  it.each(TITLE_BUTTONS)(
    ...
    ({ elementId }) => {
      ...
      assertPrompt(
        confirm,
        "Return to the title screen? Unsaved progress is lost.",
        "Return to title"
      );
  ...
  it.each(SAVE_SLOTS.map((elementId, index) => ({ slot: index + 1, elementId })))(
    ...
    ({ slot, elementId }) => {
      ...
      const contains = source(
        branch.id,
        "condition",
        BLUEPRINT_NODE_TYPE_COLLECTION_ARRAY_CONTAINS
      );
      expect(source(contains.id, "array", BLUEPRINT_NODE_TYPE_GAME_SAVE_LIST_IDS).id).toBe(
        listIds.id
      );
      expect(source(contains.id, "item", BLUEPRINT_NODE_TYPE_LITERAL_STRING).params?.value).toBe(
        String(slot)
      );
