...
type Blueprint = {
  ...
  program: {
    graphs: {
      events: Record<string, { graph: { nodes: Record<string, GraphNode>; edges: GraphEdge[] } }>;
    };
  };
};
...
describe("the Confirm page in the starter template", () => {
  ...
  it("answers by closing itself with the index of the row that was pressed", () => {
    ...
    expect(pinIds(BLUEPRINT_NODE_TYPE_EVENT_HEAD_ITEM_CLICK)).toEqual(
      expect.arrayContaining(["then", "index"])
    );
    expect(pinIds(BLUEPRINT_NODE_TYPE_LAYER_CLOSE_SELF)).toEqual(
      expect.arrayContaining(["in", "result"])
    );
    expect(wired({ nodeId: click.id, port: "then" }, { nodeId: close.id, port: "in" })).toBe(true);
    expect(wired({ nodeId: click.id, port: "index" }, { nodeId: close.id, port: "result" })).toBe(
      true
    );
  });
  ...
  it.each([
    ...
    "draws its %s from a value binding rather than a literal",
    (_name, textIndex, sourceType, jsonPath) => {
      ...
      const blueprint = blueprints.find(
        (candidate) =>
          candidate.owner.kind === "widgetValue" && candidate.owner.elementId === text.id
      )!;
      ...
      expect(wired({ nodeId: source.id, port: "props" }, { nodeId: read.id, port: "json" })).toBe(
        true
      );
      expect(wired({ nodeId: read.id, port: "result" }, { nodeId: value.id, port: "value" })).toBe(
        true
      );
    }
  );
