...
export function useProjectWizard() {
  ...
  const [platformInfo] = useState<any>(null);
  const [defaultLocation, setDefaultLocation] = useState<string>("");
  const [validationErrors, setValidationErrors] = useState<ValidationErrors>({});
  const [directoryValidation, setDirectoryValidation] = useState<DirectoryValidationResult | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    setProjectData((prev) =>
      prev.location === suggested ? prev : { ...prev, location: suggested }
    );
   ...
   */
  const validateProjectDirectory = useCallback(
    async (path: string) => {
  ...
  const handleSelectDirectory = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data?.dest) {
        ...
        const validationResult = await ValidationService.validateProjectDirectory(
          selectedPath,
          platformInfo
        );
