...
export function SettingsTransferPanel() {
  ...
  const runExport = useCallback(async () => {
    ...
    try {
      ...
      if (!result.canceled) {
        setMessage({
          tone: "ok",
          text: t("settings.transfer.exported", { path: result.filePath ?? "" })
        });
      }
    } catch (error) {
  ...
  const skipped = plan
    ? plan.entries.filter((entry) => entry.verdict === "unknown" || entry.verdict === "invalid")
    : [];
    ...
    <div className="flex flex-col gap-3">
            ...
            <div className="max-h-48 overflow-y-auto">
              {plan.applicable.map((entry) => (
                <div key={entry.key} className="flex h-6 items-center gap-2 text-2xs">
                  ...
                  <span className="min-w-0 max-w-40 truncate text-fg">
              ...
              {skipped.map((entry) => (
                <p key={entry.key} className="truncate text-2xs text-fg-subtle">
                  {entry.verdict === "unknown"
                    ...
                    : t("settings.transfer.skippedInvalid", {
                        key: entry.key,
                        reason: entry.reason ?? ""
                      })}
                </p>
      ...
      {message && (
        <p
          className={cn(
            "truncate text-xs",
            message.tone === "ok" ? "text-fg-subtle" : "text-danger"
          )}
        >
          {message.text}
        </p>
