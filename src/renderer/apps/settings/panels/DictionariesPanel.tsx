  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        {installed === null ? (
          ...
          <div className="flex flex-col">
            {installed.map((entry) => (
              <div
                key={entry.code}
                className="flex min-h-7 items-center gap-3 rounded-md px-1 hover:bg-fill-subtle"
              >
      ...
      <SectionCard
        ...
        actions={
          <Button
            size="sm"
            variant="secondary"
            disabled={browsing || busy !== null}
            onClick={() => void browse()}
          >
            {t(
              available === null ? "settings.dictionaries.browse" : "settings.dictionaries.refresh"
            )}
          </Button>
        }
      >
        {available === null ? (
          <p className="px-1 py-1 text-xs text-fg-subtle">
            {browsing
              ? t("settings.dictionaries.browsing")
              : t("settings.dictionaries.available.prompt")}
          </p>
        ) : offered.length === 0 ? (
          <p className="px-1 py-1 text-xs text-fg-subtle">
            {t("settings.dictionaries.available.none")}
          </p>
        ) : (
          <div className="flex flex-col">
            {offered.map((entry) => (
        ...
        {failed && (
          <p className="px-1 pt-2 text-xs text-danger">{t("settings.dictionaries.failed")}</p>
        )}
      </SectionCard>
      ...
      <p className="text-xs leading-relaxed text-fg-subtle">
        {t("settings.dictionaries.noDictionaryLanguages")}
      </p>
    </div>
  );
}
