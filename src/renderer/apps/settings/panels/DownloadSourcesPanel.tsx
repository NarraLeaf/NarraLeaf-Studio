      ...
      setProbes((prev) => ({
        ...
        [index]: result?.success
          ...
          : {
              phase: "done",
              reachable: false,
              error: result?.error ?? t("settings.network.probeFailed")
            }
      }));
    },
    [t]
...
function ProbeSummary({ probes }: { probes: Record<number, ProbeState> }) {
  ...
  return (
    <span
      className={cn("min-w-0 truncate text-xs", latest.reachable ? "text-success" : "text-danger")}
    >
      {latest.reachable
