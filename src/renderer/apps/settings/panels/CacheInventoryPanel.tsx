...
export function CacheInventoryPanel() {
  ...
  const clear = useCallback(
    async (ids: CacheBucketId[], scope: CacheBucketId | "all") => {
      setBusy(scope);
      const result = await getInterface()
        .app.clearCaches(ids)
        .catch(() => null);
  ...
  const clearable = (report?.buckets ?? []).filter(
    (bucket) => !bucket.error && bucket.sizeBytes > 0
  );
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          onClick={() =>
            void clear(
              clearable.map((bucket) => bucket.id),
              "all"
            )
          }
        >
