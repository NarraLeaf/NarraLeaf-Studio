...
export function SettingsExplorer<T>({
  ...
  const handleInputCommit = useCallback(
    (entry: SettingEntry<T>) => {
      ...
      const rawValue =
        pendingInputs[id] ??
        String(getValue(entry.source, entry.descriptor) ?? entry.descriptor.defaultValue ?? "");
      ...
      const value =
        typeof parsed === "number" ? normalizeSettingNumber(entry.descriptor, parsed) : parsed;
  ...
  const renderControl = (entry: SettingEntry<T>) => {
    ...
    switch (descriptor.type) {
      ...
      case SettingValueType.Color: {
        ...
        const effectiveHex =
          (isCustom ? normalizeHexColor(displayValue) : descriptor.optionColors?.[displayValue]) ??
  ...
  const setCategoryRef = useCallback(
    (categoryKey: SettingCategory["key"], node: HTMLElement | null) => {
  ...
  return (
    <div className="h-full flex flex-col" onClick={() => panelFocusHandler?.()}>
      {showSearch && (
          ...
          <div className="space-y-5 px-3 py-3">
            {categoryEntriesToRender.map((entry) => (
              ...
              >
                ...
                <div className="space-y-0">{entry.entries.map(renderSetting)}</div>
              </section>
            ))}
          </div>
        )}
