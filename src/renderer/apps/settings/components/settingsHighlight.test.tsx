...
function PanelThatReadsIt() {
    ...
    return <button type="button" data-servers-add className={highlighted ? SETTINGS_HIGHLIGHT_RING : ""} />;
}
...
const CATEGORIES: SettingCategory[] = [{ key: "servers", label: "Servers", description: "", order: 0 }];
...
beforeEach(() => {
    ...
    Element.prototype.scrollTo = function scrollTo() { /* nothing to scroll */ };
});
...
describe("a panel Settings was opened at", () => {
    ...
    it("leaves the block unmarked when the panel marks a control of its own", () => {
        ...
        expect(document.querySelector("[data-servers-add]")?.className).toContain(SETTINGS_HIGHLIGHT_RING);
        expect(document.querySelector("[data-settings-panel]")?.className).not.toContain(SETTINGS_HIGHLIGHT_RING);
    });
