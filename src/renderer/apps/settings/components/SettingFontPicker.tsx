...
type LoadState = { status: "idle" } | { status: "loading" } | SystemFontsResult;
...
export function SettingFontPicker({
  ...
  const rows = useMemo<PickerRow[]>(() => {
    ...
    const matchedPresets = presets.filter(
      (preset) =>
        !needle ||
        preset.toLowerCase().includes(needle) ||
        presetLabel(preset).toLowerCase().includes(needle)
    );
    if (matchedPresets.length > 0) {
      result.push({
        kind: "header",
        key: "header:presets",
        label: t("settings.fontPicker.presets")
      });
    ...
    const matchedFamilies = needle
      ? families.filter((family) => matches(needle, family))
      : families;
    if (matchedFamilies.length > 0) {
      result.push({
        kind: "header",
        key: "header:installed",
        label: t("settings.fontPicker.installed")
      });
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: (index) =>
      rows[index]?.kind === "header" ? HEADER_ROW_HEIGHT_PX : FONT_ROW_HEIGHT_PX,
    overscan: 12,
  ...
  useLayoutEffect(() => {
    ...
    const place = () => {
      ...
      const maxHeight = Math.max(
        120,
        Math.min(PANEL_MAX_HEIGHT_PX, openAbove ? spaceAbove : spaceBelow)
      );
      setPanelStyle({
        position: "fixed",
        width,
  ...
  const triggerLabel = isEditorFontPreset(value)
    ? presetLabel(value)
    : value || presetLabel("Default");
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-edge-subtle px-2.5 py-2">
        ...
        <input
          ...
          aria-activedescendant={
            activeIndex >= 0 ? `setting-font-row-${rows[activeIndex]?.key}` : undefined
          }
