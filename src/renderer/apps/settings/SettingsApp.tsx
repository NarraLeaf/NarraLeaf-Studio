...
export function SettingsApp() {
  ...
  const [availability, setAvailability] = useState<
    Record<string, { enabled: boolean; reasonKey?: TranslationKey }>
  >({});
  useEffect(() => {
    let mounted = true;
  ...
  const describeAppSetting = useCallback(
    (setting: AppSettingDefinition): SettingDescriptor => {
      ...
      const spellcheckOptions =
        setting.key === SPELLCHECK_LANGUAGE_KEY && spellcheckLanguages
      ...
      const merged: Record<string, string> = {
        ...
        ...(spellcheckOptions
          ? Object.fromEntries(
              spellcheckLanguages!.map((code) => [code, spellcheckLanguageLabel(code)])
            )
          : {}),
      ...
      return {
        ...
        optionLabels: languageLocales
          ? Object.fromEntries(
              languageLocales.map((code) => [code, getLocaleMeta(code).nativeName])
            )
          : optionLabels,
  ...
  const invokeSettingAction = useCallback(async (setting: AppSettingDefinition) => {
   ...
   */
  const resetSettingRow = useCallback(async (setting: AppSettingDefinition) => {
  ...
  const navCategories = useMemo<SettingsNavCategory[]>(() => {
    return localizedCategories
      .map((category) => {
        const entries = getSettingsByCategory(category.key as AppSettingCategoryKey).map(
          (setting) => ({ descriptor: describeAppSetting(setting) })
        );
        const matched = filterCategoryEntries(category, entries, searchQuery);
        return matched ? { category, entries: matched.map((entry) => entry.descriptor) } : null;
      })
  ...
  useEffect(() => {
    ...
    void getInterface()
      ...
      .catch(() => undefined);
    const token = getInterface().app.onSettingsHighlight?.((highlight) =>
      ...
      >
        <aside className="flex w-64 shrink-0 flex-col gap-3 border-r border-edge-subtle bg-surface-sunken p-4">
          ...
          {localizedCategories.length > 0 ? (
            ...
            <p className="text-xs text-fg-subtle">{t("settings.noneExposed")}</p>
          )}
        </aside>
        ...
        <section className="min-w-0 flex-1 p-4">
          <SettingsExplorer
            ...
            getSettingsForCategory={(category) =>
              getSettingsByCategory(category as AppSettingCategoryKey)
            }
