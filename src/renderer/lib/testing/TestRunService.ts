...
export class TestRunService extends Service<TestRunService> implements ITestRunService {
    ...
    public async start(testId: TestId): Promise<string> {
        ...
        this.logToConsole("info", translate("test.console.started", { title: formatTestText(record.title) }));
    ...
    private async execute(active: ActiveRun): Promise<void> {
        ...
        this.logToConsole(consoleLevelForStatus(status), translate("test.console.finished", {
    ...
    private async launchGame(active: ActiveRun, options?: TestGameLaunchOptions): Promise<TestGameSession> {
        ...
        try {
            ...
            if (!result.success) {
                throw new Error(`Could not launch a game for the test: ${result.error ?? "no reason given"}`);
            }
            if (!result.data.ok) {
    ...
    private appendFinding(active: ActiveRun, finding: TestFinding): void {
        ...
        if (active.record.findings.length <= CONSOLE_FINDING_LIMIT) {
            this.logToConsole(FINDING_CONSOLE_LEVELS[finding.severity], translate("test.console.finding", {
    ...
    private logToConsole(level: TestLogLevel, message: string): void {
        this.tryGetConsole()?.log(TEST_CONSOLE_CHANNEL, level, message, { source: TEST_CONSOLE_SOURCE });
    }
