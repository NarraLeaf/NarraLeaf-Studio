  ...
  useLayoutEffect(() => {
    ...
    const updatePlacement = () => {
      ...
      if (menuPlacement === "auto") {
        ...
        const shouldOpenUp = dropdownRect.height > spaceBelow && spaceAbove >= dropdownRect.height;
  ...
  useLayoutEffect(() => {
    ...
    const positionPortalMenu = () => {
      ...
      if (menuPlacement === "above") {
        ...
        openAbove = menuHeight > spaceBelow && spaceAbove >= Math.min(menuHeight, spaceAbove);
      }
      ...
      const left = alignLeft ? trigger.left : Math.max(SELECT_MENU_GAP_PX, trigger.right - width);
      ...
      {options.map((option) => {
        ...
        const optionBody = (
          <>
            ...
            {option.icon && <div className="flex-shrink-0 text-fg-muted">{option.icon}</div>}
      ...
      >
        ...
        <span className="flex min-w-0 flex-1 items-baseline gap-1.5">
          <span className="truncate">
            {selectedOption ? optionLabel(selectedOption) : resolvedPlaceholder}
          </span>
...
export function SelectGroup({
  ...
  return (
    <div className={className}>
      <div className="px-3 py-1 text-xs font-semibold text-fg-muted tracking-wider">{label}</div>
