...
export function CardDescription({
  ...
  return <p className={cn("text-xs text-fg-muted mt-1", className)}>{children}</p>;
}
...
export function CardContent({
  ...
  return <div className={cn("text-sm text-fg", className)}>{children}</div>;
}
...
export function CardFooter({
  ...
  return (
    <div
      className={cn("mt-4 pt-3 border-t border-edge flex items-center justify-between", className)}
    >
      {children}
    </div>
  );
...
export function InteractiveCard({
  ...
  return (
    <Card variant={variant} size={size} onClick={onClick} hover={true} className={className}>
      <div className="flex items-start gap-3">
        {icon && <div className="flex-shrink-0 mt-0.5 text-fg-muted">{icon}</div>}
