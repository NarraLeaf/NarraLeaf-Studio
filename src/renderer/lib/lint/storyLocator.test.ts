...
const jump = (id: string): BlockSpec => ({ id, kind: "jump", payload: { targetSceneId: "nowhere" } });
...
const finding = (blockId?: string, sceneId: string | undefined = "sc1"): LintFinding => ({
    ...
    location: { kind: "story", storyId: "st1", storyName: "Chapter One", sceneId, sceneName: "Opening", blockId },
});
...
describe("createStoryRowLocator", () => {
    ...
    it("numbers rows down the document, descending into containers", () => {
        expect(["n1", "g", "c1", "c2", "j"].map(id => locate("st1", "sc1", id)?.line)).toEqual([1, 2, 3, 4, 5]);
    });
    ...
    it("clips a long line rather than carrying a paragraph into the report", () => {
        ...
        const one = createStoryRowLocator([story("st1", "C", [scene("sc1", "S", [narration("n1", long)])])]);
