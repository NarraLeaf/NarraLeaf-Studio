      ...
      for (const asset of ctx.assets) {
        ...
        if (!(await ctx.io.exists(asset.id))) {
          findings.push(
            assetFinding(
              "assets/unreadable",
              "lint.rule.assetsUnreadable.messageMissingBytes",
              asset
            )
          );
          continue;
        ...
        if (!probe.ok) {
          findings.push(
            assetFinding("assets/unreadable", "lint.rule.assetsUnreadable.message", asset)
          );
        }
      }
      return findings;
...
function shippedAssetIds(ctx: LintContext): ReadonlySet<string> {
  try {
    ...
    return new Set(
      answer.members.filter((member) => member.kind === "asset").map((member) => member.id)
    );
  } catch (error) {
