...
function story(id: string, name: string, scenes: StoryScene[], entrySceneId?: string): LintStoryEntry {
    const document = {
...
describe("story/invalid-command", () => {
    ...
    it("says nothing about a story whose rows all resolve", () => {
        expect(run("story/invalid-command", ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [narration("b1")])])))).toEqual([]);
    });
...
describe("story/goto-missing", () => {
    ...
    it("accepts a goto whose label is declared in the same scene", () => {
        const findings = run(
            "story/goto-missing",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [label("b0", "retry"), goto("b1", "retry")])])),
        );
    ...
    it("ignores a disabled goto", () => {
        const findings = run(
            "story/goto-missing",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [{ ...goto("b1", "retry"), disabled: true }])])),
        );
        expect(findings).toEqual([]);
    });
    ...
    it("ignores a goto inside a disabled container", () => {
        const findings = run(
            ...
            ctxWith(
                story("s1", "Main", [
                    scene("sc1", "Prologue", [
                        { id: "g1", kind: "control", payload: { control: "sequence" }, disabled: true, children: [goto("b1", "retry")] },
                    ]),
                ]),
            ),
        );
    ...
    it("does not see a label whose own row is disabled", () => {
        const findings = run(
            ...
            ctxWith(
                story("s1", "Main", [
                    scene("sc1", "Prologue", [{ ...label("b0", "retry"), disabled: true }, goto("b1", "retry")]),
                ]),
            ),
        );
        expect(findings).toHaveLength(1);
...
describe("story/label-duplicate", () => {
    it("reports the second declaration, not the first", () => {
        const findings = run(
            "story/label-duplicate",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [label("b1", "retry"), label("b2", "retry")])])),
        );
...
describe("story/label-unused", () => {
    it("reports a label nothing jumps to", () => {
        const findings = run(
            "story/label-unused",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [label("b1", "retry"), narration("b2")])])),
        );
    ...
    it("says nothing about a label a goto addresses", () => {
        const findings = run(
            "story/label-unused",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [label("b1", "retry"), goto("b2", "retry")])])),
        );
        expect(findings).toEqual([]);
    });
...
describe("story/jump-missing", () => {
    ...
    it("accepts a jump into a scene of the same story", () => {
        const findings = run(
            "story/jump-missing",
            ctxWith(
                story("s1", "Main", [scene("sc1", "Prologue", [jump("b1", "sc2")]), scene("sc2", "Chapter 1", [narration("b2")])]),
            ),
        );
        expect(findings).toEqual([]);
    });
    ...
    it("reports an empty jump target", () => {
        const findings = run("story/jump-missing", ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [jump("b1", "")])])));
        expect(findings).toHaveLength(1);
    });
    ...
    it("ignores a disabled jump", () => {
        const findings = run(
            "story/jump-missing",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [{ ...jump("b1", "gone"), disabled: true }])])),
        );
        expect(findings).toEqual([]);
    });
});
...
describe("story/empty-choice", () => {
    ...
    it("reports an option with no text", () => {
        const findings = run(
            "story/empty-choice",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [choice("b1", [option("o1", "Fight"), option("o2", "   ")])])])),
        );
    ...
    it("says nothing about a choice whose options all read", () => {
        const findings = run(
            "story/empty-choice",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [choice("b1", [option("o1", "Fight"), option("o2", "Flee")])])])),
        );
        expect(findings).toEqual([]);
    });
        ...
        const interpolated: BlockSpec = {
            ...
            payload: {
                ...
                text: {
                    ...
                    value: "",
                    role: "choiceText",
                    rich: [{ interpolation: { kind: "variable", target: { scope: "saved", variableId: "v1" } } }],
                },
            },
        };
        const findings = run(
    ...
    it("counts a choice whose only option is disabled as having none", () => {
        const findings = run(
            "story/empty-choice",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [choice("b1", [{ ...option("o1", "Fight"), disabled: true }])])])),
        );
        expect(findings).toHaveLength(1);
        expect(findings[0].messageKey).toBe("lint.rule.storyEmptyChoice.message");
    });
    ...
    it("ignores a disabled choice entirely", () => {
        const findings = run(
            "story/empty-choice",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [{ ...choice("b1", []), disabled: true }])])),
        );
        expect(findings).toEqual([]);
    });
    ...
    it("ignores a blank option under a disabled choice", () => {
        const findings = run(
            "story/empty-choice",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [{ ...choice("b1", [option("o1", "")]), disabled: true }])])),
        );
        expect(findings).toEqual([]);
    });
});
...
describe("story/dead-end", () => {
    ...
    it("says nothing about a scene ending in a goto", () => {
        const findings = run(
            "story/dead-end",
            ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [label("b0", "top"), goto("b1", "top")])])),
        );
        expect(findings).toEqual([]);
    });
    ...
    it("says nothing about a scene ending in a choice whose every option jumps", () => {
        const findings = run(
            ...
            ctxWith(
                story("s1", "Main", [
                    scene("sc1", "Prologue", [
                        choice("b1", [option("o1", "Fight", [jump("j1", "sc1")]), option("o2", "Flee", [jump("j2", "sc1")])]),
                    ]),
                ]),
            ),
        );
    ...
    it("reports a choice where one option falls through", () => {
        const findings = run(
            ...
            ctxWith(
                story("s1", "Main", [
                    scene("sc1", "Prologue", [
                        choice("b1", [option("o1", "Fight", [jump("j1", "sc1")]), option("o2", "Flee", [narration("n1")])]),
                    ]),
                ]),
            ),
        );
...
describe("story/unreachable-scene", () => {
        ...
        const findings = run(
            ...
            ctxWith(
                story(
                    "s1",
                    "Main",
                    [scene("sc1", "Prologue", [jump("b1", "sc2")]), scene("sc2", "Chapter 1", []), scene("sc3", "Cut", [])],
                    "sc1",
                ),
            ),
        );
    ...
    it("does not flag a scene a blueprint Start Game node opens", () => {
        const ctx = createTestLintContext({
            stories: [story("s1", "Main", [scene("sc1", "Prologue", []), scene("sc2", "Recollection", [])], "sc1")],
        ...
        const findings = run(
            ...
            ctxWith(
                story(
                    "s1",
                    "Main",
                    [scene("sc1", "Prologue", [{ ...jump("b1", "sc2"), disabled: true }]), scene("sc2", "Chapter 1", [])],
                    "sc1",
                ),
            ),
        );
...
describe("story/empty-scene", () => {
    it("reports a scene with no blocks", () => {
        const findings = run("story/empty-scene", ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [])])));
    ...
    it("says nothing about a scene with content", () => {
        expect(run("story/empty-scene", ctxWith(story("s1", "Main", [scene("sc1", "Prologue", [narration("b1")])])))).toEqual([]);
    });
