...
const setFromExpression = (
  id: string,
  target: StoryVariableRef,
  read: StoryVariableRef,
  name: string
): BlockSpec => ({
  id,
  kind: "action",
...
const binary = (
  ...
  left: StoryExpr,
  right: StoryExpr
): StoryExpr => ({ kind: "binary", op, left, right });
const varRead = (target: StoryVariableRef, name: string): StoryExpr => ({
  kind: "var",
  target,
  name
});
        ...
        program: {
          ...
          graphs: {
            events: {
              "ev-1": {
                id: "ev-1",
                graph: { nodes: { "n-1": { id: "n-1", type: "blueprint.story.sceneGet", params } } }
              }
            },
            functions: {}
          }
...
describe("variables/undeclared", () => {
      ...
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [
            ...
            setFromExpression(
              "b1",
              { scope: "scene", variableId: "v1" },
              { scope: "saved", variableId: "v2" },
              "bonus"
            )
          ])
        ])
        ...
        story("s1", "Main", [
          scene("sc1", "Prologue", [
            repeatUntil(
              "r1",
              "Gold >= 10",
              binary(">=", varRead({ scope: "scene", variableId: "v1" }, "Gold"), num(10))
            )
          ])
        ])
      ]
  ...
  it("reads a variable interpolated into a line", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [interpolatingLine("b1", { scope: "saved", variableId: "v9" })])
        ])
      ]
    });
  ...
  it("does not flag a persistent variable declared only in the project registry", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [setVariable("b1", { scope: "persistent", variableId: "pk-1" })])
        ])
      ],
  ...
  it("does not flag a saved variable declared only in the project registry", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [setVariable("b1", { scope: "saved", variableId: "sv-1" })])
        ])
      ],
  ...
  it("does not let a saved registry entry vouch for a persistent reference", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [setVariable("b1", { scope: "persistent", variableId: "sv-1" })])
        ])
      ],
      variableRegistry: [savedRegistryEntry("sv-1", "Affection")]
    });
  ...
  it("accepts a persistent variable a story row in ANOTHER story declares", () => {
    const ctx = createTestLintContext({
      stories: [
        ...
        story("s2", "Side", [
          scene("sc9", "Extras", [declaration("d1", "persistent", "Playthroughs", "pk-1")])
        ])
      ]
    });
    expect(run("variables/undeclared", ctx)).toEqual([]);
...
describe("variables/unused", () => {
  it("reports a declaration nothing reads or writes", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [scene("sc1", "Prologue", [declaration("v1", "scene", "Gold")])])
      ]
    });
  ...
  it("counts a saved declaration row read from another story", () => {
    const ctx = createTestLintContext({
      stories: [
        ...
        story("s2", "Side", [
          scene("sc9", "Extras", [setVariable("b9", { scope: "saved", variableId: "d1" })])
        ])
      ]
    });
    expect(run("variables/unused", ctx)).toEqual([]);
  ...
  it("reports a saved registry entry nothing reads, against the project", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [setVariable("b1", { scope: "saved", variableId: "other" })])
        ])
      ],
      variableRegistry: [savedRegistryEntry("sv-1", "Affection")]
    });
  ...
  it("does not let a use in one project scope excuse the other scope's entry", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [setVariable("b1", { scope: "saved", variableId: "shared" })])
        ])
      ],
      variableRegistry: [
        savedRegistryEntry("shared", "Affection"),
        registryEntry("shared-p", "Playthroughs", "shared")
      ]
    });
...
describe("variables/name-collision", () => {
  it("emits one finding per reported collision, anchored on the story row", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [declaration("d1", "persistent", "Playthroughs", "pk-1")])
        ])
      ],
  ...
  it("reports a saved collision too, anchored on the saved row", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [declaration("d1", "saved", "Affection", "sk-1")])
        ])
      ],
  ...
  it("does not anchor a saved collision on a persistent row with the same key", () => {
    const ctx = createTestLintContext({
      stories: [
        story("s1", "Main", [
          scene("sc1", "Prologue", [declaration("d1", "persistent", "Affection", "sk-1")])
        ])
      ],
...
describe("variables/random-outside-assignment", () => {
  ...
  it("deliberately exempts a /repeat until condition - re-testing it each iteration IS the loop", () => {
    const ctx = inScene([
      repeatUntil("r1", "random() < 0.1", binary("<", call("random", []), num(0.1)))
    ]);
    expect(run("variables/random-outside-assignment", ctx)).toEqual([]);
  });
  ...
  it("reports a roll in a choice option's hiddenWhen", () => {
    ...
    expect(findings).toHaveLength(1);
    expect(findings[0].messageKey).toBe(
  ...
  it("counts hiddenWhen and disabledWhen as two separate mistakes", () => {
    ...
    const ctx = inScene([
      choiceWithOption("ch1", "op1", { hiddenWhen: rolled(), disabledWhen: rolled() })
    ]);
  ...
  it("reports a roll interpolated inline into a line", () => {
    const ctx = inScene([
      interpolatingExpressionLine(
        "b1",
        expr("randomInt(1, 6)", call("randomInt", [num(1), num(6)]))
      )
    ]);
    ...
    expect(findings).toHaveLength(1);
    expect(findings[0].messageKey).toBe(
      "lint.rule.variablesRandomOutsideAssignment.messageInterpolation"
    );
    expect(findings[0].messageParams).toEqual({ fn: "randomInt" });
  });
