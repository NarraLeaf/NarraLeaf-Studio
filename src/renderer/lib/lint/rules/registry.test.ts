...
describe("lint rule registry", () => {
  it("contains exactly the planned rule set", () => {
    ...
    expect(LINT_RULES).toHaveLength(41);
  });
  ...
  it("prefixes every rule id with its category", () => {
    for (const rule of LINT_RULES) {
      expect(
        rule.id.startsWith(`${rule.category}/`),
        `${rule.id} is not in category ${rule.category}`
      ).toBe(true);
    }
  });
  ...
  it("partitions the registry by category with nothing lost", () => {
    ...
    expect(new Set(partitioned.map((rule) => rule.id))).toEqual(
      new Set(LINT_RULES.map((rule) => rule.id))
    );
  ...
  it("declares option specs only where they are called for", () => {
    ...
    expect(getLintRule("assets/oversized")?.options?.maxMegabytes).toEqual({
      kind: "number",
      default: 64,
      min: 1,
      max: 4096
    ...
    expect(overlong?.options?.maxChars).toEqual({
      kind: "number",
      default: 120,
      min: 1,
      max: 2000
