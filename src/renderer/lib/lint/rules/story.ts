...
function gotoTarget(block: StoryBlock): string | null {
  return block.kind === "control" && block.payload.control === "goto"
    ? block.payload.targetLabel.trim()
    : null;
}
  ...
  if (block.kind === "control") {
    ...
    if (block.payload.control === "condition") {
      ...
      const hasElse = branches.some(
        (child) =>
          child.kind === "control" &&
          child.payload.control === "conditionBranch" &&
          child.payload.branch === "else"
      );
...
function collectEntryPoints(ctx: LintContext): StoryEntryPointScan {
  const scan = scanStoryEntryPoints(
    ...
    (storyId, sceneId) =>
      Boolean(ctx.stories.find((story) => story.id === storyId)?.document.scenes[sceneId])
  );
  for (const entry of ctx.stories) {
