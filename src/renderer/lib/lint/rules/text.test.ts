...
const INTERPOLATION = {
  interpolation: {
    ...
    target: { scope: "scene" as const, variableId: "var-1" }
  }
};
...
describe("text/overlong", () => {
  it("fires one column over the maximum and not at it", async () => {
    ...
    expect(over[0].location).toMatchObject({
      kind: "story",
      sceneName: "Scene One",
      blockId: "b1"
    });
  ...
  it("does not charge for an interpolation's unknowable width", async () => {
    const segment = textSegment("t-1", "aaaaaaaaaa{gold}", "dialogue", [
      { text: "a".repeat(10) },
      INTERPOLATION
    ]);
    // Serialized for translators this is "aaaaaaaaaa{0}" - 13 columns, which would fire at 10.
    expect(
      await run("text/overlong", contextOf([dialogueBlock("b1", segment)]), { maxChars: 10 })
    ).toEqual([]);
  });
...
describe("text/empty", () => {
  it("reports a blank dialogue and a whitespace-only narration", async () => {
    ...
    expect(findings.map((entry) => entry.location)).toMatchObject([
      { blockId: "b1" },
      { blockId: "b2" }
    ]);
  });
  ...
  it("leaves a written line, a blank prompt, an interpolation-only line and an event-only line alone", async () => {
    const findings = await run(
      ...
      contextOf([
        ...
        dialogueBlock(
          "b3",
          textSegment("t-3", "", "dialogue", [{ event: { sound: { assetId: "a-1" } } }])
        )
      ])
    );
  ...
  it("ignores a disabled row", async () => {
    const ctx = contextOf([
      dialogueBlock("b1", textSegment("t-1", "", "dialogue"), { disabled: true })
    ]);
