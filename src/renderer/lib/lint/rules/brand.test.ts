...
function documentLinking(link: string): UIDocument {
    return {
        ...
        elements: {
            ...
            start: { id: "start", type: "nl.button", name: "Start", childrenIds: [], style: { backgroundColor: `nlbrand:${link}` } },
        },
...
describe("classifyBrandLink", () => {
    ...
    it("calls an id the palette does not have missing", () => {
        expect(classifyBrandLink(new BrandPalette(BUILTIN_BRAND_COLORS), "nope")).toEqual({ kind: "missing" });
    });
    ...
    it("names the entry the chain runs out at, rather than calling it a ring", () => {
        ...
        expect(classifyBrandLink(palette, "button.primary")).toEqual({ kind: "chain", missingId: "mine" });
    });
});
...
describe("brand/broken-link", () => {
    ...
    it("reports a link the palette has no entry for", async () => {
        const findings = await RULE.run(createTestLintContext({ uiDocument: documentLinking("nope") }), {});
    ...
    it("reports a link into a ring", async () => {
        ...
        const findings = await RULE.run(createTestLintContext({ uiDocument: documentLinking("a") }), {});
    ...
    it("names the missing entry when the chain runs out further down", async () => {
        ...
        const findings = await RULE.run(createTestLintContext({ uiDocument: documentLinking("button.primary") }), {});
