...
import {
  blueprintNodeJumpTarget,
  listBlueprintGraphSites,
  type BlueprintGraphSite
} from "../blueprintSites";
...
function execPortIds(
  entry: BlueprintNodeEditorCatalogEntry,
  kind: "input" | "output"
): ReadonlySet<string> {
  return new Set(
    entry.pins.filter((pin) => pin.kind === kind && pin.semantic === "exec").map((pin) => pin.id)
  );
}
...
function hasOutgoingExecEdge(ir: BlueprintGraphIr, node: BlueprintGraphNode): boolean {
  ...
  return (ir.edges ?? []).some(
    (edge) => edge.from.nodeId === node.id && outputs.has(edge.from.port)
  );
}
...
function runSaveFieldEmpty(ctx: LintContext): LintFinding[] {
  ...
  for (const site of listBlueprintGraphSites(ctx.blueprintDocument)) {
    const nodes = site.ir.nodes ?? {};
    const saveNodes = Object.values(nodes).filter(
      (node) => node.type === BLUEPRINT_NODE_TYPE_GAME_SAVE_WRITE
    );
    ...
    for (const node of saveNodes) {
      const willRun =
        entries.length > 0
          ...
          : (site.ir.edges ?? []).some(
              (edge) => edge.to.nodeId === node.id && edge.to.port === "in"
            );
      if (!willRun) {
        continue;
      }
