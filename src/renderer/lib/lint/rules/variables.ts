 ...
 */
const RANDOM_FUNCTIONS: ReadonlySet<StoryExprFunction> = new Set<StoryExprFunction>([
  "random",
  "randomInt"
]);
...
function collectBlueprintVariableUses(
  document: BlueprintDocument | null
): Record<keyof typeof BLUEPRINT_VARIABLE_PARAMS, Set<string>> {
  const found = {
  ...
  for (const node of eachBlueprintNode(document)) {
    for (const [scope, paramKey] of Object.entries(BLUEPRINT_VARIABLE_PARAMS) as [
      keyof typeof BLUEPRINT_VARIABLE_PARAMS,
      string
    ][]) {
 ...
 */
function collectProjectScopeIdentities(
  ctx: LintContext,
  ...
  },
  {
    ...
    run(ctx) {
      ...
      const usedAtProjectScope = (
        ...
        return variableAliases(id, storageKey).some(
          (alias) => uses.has(alias) || fromBlueprint.has(alias)
        );
      };
      ...
      const isUsed = (
        ...
        return usedAtProjectScope(
          declaration.payload.scope,
          declaration.id,
          declaration.payload.storageKey
        );
      };
      ...
      for (const { scope, collisions } of byScope) {
        for (const collision of collisions) {
          ...
          findings.push({
            ...
            location: site
              ? storyLocation(site.entry, site.scene, site.block.id)
              : { kind: "project" },
