...
function unit(
  assetId: string,
  sourceText: string,
  status: VoiceUnit["status"] = "linked"
): VoiceUnit {
...
describe("voice/missing", () => {
  ...
  it("stays silent when only the legacy per-row voiceAssetId is set", async () => {
    ...
    const findings = await run(
      "voice/missing",
      contextOf(
        [dialogueBlock("b1", textSegment("t-1", LINE, "dialogue"), { voiceAssetId: "asset-9" })],
        {}
      )
    );
        ...
        documents: new Map([
          [
            "ja",
            {
              ...
              locale: "ja",
              units: {
                "t-1": { target: JA_TAKE, sourceHash: hashSourceText(LINE), status: "translated" }
              }
            }
          ]
        ])
          ...
          [
            "ja",
            {
              ...
              units: {
                "t-1": {
                  target: "もう帰りましょう。",
                  sourceHash: hashSourceText(LINE),
                  status: "translated"
                }
              }
            }
