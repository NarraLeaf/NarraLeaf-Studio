...
describe("runLintRules", () => {
  ...
  it("reports a throwing rule and keeps sweeping", async () => {
    ...
    expect(
      report.entries.filter((entry) => entry.messageKey === "lint.message.ruleFailed")
    ).toHaveLength(1);
  ...
  it("numbers a rule's story rows for it, and reads them out in document order", async () => {
    ...
    expect(
      report.entries.map((entry) => entry.location.kind === "story" && entry.location.line)
    ).toEqual([1, 2]);
    expect(
      report.entries.map((entry) => entry.location.kind === "story" && entry.location.excerpt)
  ...
  it("stops at a cancellation and reports the unrun rules as skipped", async () => {
    ...
    const report = await runLintRules(createTestLintContext(), {
      rules,
  ...
  it("reports progress once per scheduled rule", async () => {
    ...
    await runLintRules(createTestLintContext(), {
      rules,
