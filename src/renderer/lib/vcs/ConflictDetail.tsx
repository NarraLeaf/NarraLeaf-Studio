        ...
        {entry === undefined || entry.status === "loading" ? (
          ...
          <div className="space-y-0.5">
            <p className="text-2xs text-fg-muted">
              {t("documentDiff.resolve.change.blocked.title")}
            </p>
            <p className="text-2xs text-fg-subtle">
              {t(mergeDocumentBlockedKey(entry.document.blocked))}
            </p>
          ...
          <>
            <p className="mb-1 text-2xs text-fg-subtle">
              {t("documentDiff.resolve.change.heading")}
            </p>
        ...
        {!conflict &&
          side !== undefined && (
            ...
            >
              {t(
                other === "mine"
                  ? "documentDiff.resolve.change.useMine"
                  : "documentDiff.resolve.change.useTheirs"
              )}
            </button>
          )}
      </div>
          ...
          {(["mine", "theirs"] as const).map((candidate) => (
            <button
              ...
              className={cn(
                ...
                side === candidate ? "border-primary bg-primary/10" : "border-edge hover:bg-fill"
              )}
            >
              ...
              <MergeValue
                view={describeMergeSide(candidate === "mine" ? decision.mine : decision.theirs)}
              />
            </button>
          ))}
        </div>
...
function MergeValue({ view }: { view: MergeValueView }) {
  ...
  if (view.absent) {
    return (
      <span className="block truncate text-2xs italic text-fg-subtle">
        {t("documentDiff.resolve.change.absent")}
      </span>
    );
  }
  return (
