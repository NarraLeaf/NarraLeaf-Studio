...
describe("buildDocumentChangeRows", () => {
  it("keeps the head of an already ordered list rather than picking from it", () => {
    const diff = buildDocumentDiff([leaf("a"), leaf("b"), leaf("c")], {
      tier: "structural",
      limit: 10
    });
  ...
  it("gives every row a distinct key, including two changes at the document root", () => {
    const root = (kind: DocumentChange["kind"]): DocumentChange => ({
      path: [],
      kind,
      label: { key: "documentDiff.opaque.changed" }
    });
...
describe("resolveDocumentChangeLabel", () => {
    ...
    const view = resolveDocumentChangeLabel(
      {
        ...
        kind: "changed",
        label: {
          key: "documentDiff.structural.property",
          params: { name: "volume", from: 0.8, to: 0.9 }
        },
        subject: "volume"
      },
      ...
      resolveDocumentChangeLabel(
        {
          path: ["title"],
          kind: "changed",
          label: { key: "documentDiff.structural.property", params: { name: "title" } },
          subject: "title"
        },
        translator
      ).detail
      ...
      resolveDocumentChangeLabel(
        {
          path: ["title"],
          kind: "changed",
          label: { key: "documentDiff.summary.title", params: { from: "a", to: "b" } },
          subject: "b"
        },
        translator
      )
  ...
  it("translates a summary count's name, and falls back to the raw identifier", () => {
    ...
    expect(
      resolveDocumentChangeLabel(
        {
          path: ["counts", "audioTracks"],
          kind: "changed",
          label: {
            key: "documentDiff.summary.count",
            params: { name: "audioTracks", from: 3, to: 4 }
          }
        },
        translator
      )
      ...
      resolveDocumentChangeLabel(
        {
          path: ["counts", "puppets"],
          kind: "added",
          label: { key: "documentDiff.summary.count", params: { name: "puppets", to: 2 } }
        },
        translator
      )
...
describe("documentDiffTierCaption", () => {
  ...
  it("gives a structural diff a different caption from a semantic one, which is the whole point", () => {
    const structural: DocumentDiff = buildDocumentDiff([leaf("a")], {
      tier: "structural",
      limit: 8
    });
    const semantic: DocumentDiff = buildDocumentDiff([leaf("a")], { tier: "semantic", limit: 8 });

    expect(documentDiffTierCaption(structural.tier)).not.toEqual(
      documentDiffTierCaption(semantic.tier)
    );
  });
});
