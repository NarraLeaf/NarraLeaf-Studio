...
describe("buildChangeIndex", () => {
  it("gives every file exactly one row, whatever is inside it", () => {
    ...
    const entries = [
      ...
      entry({
        path: "editor/story/stories/d/storydoc.json",
        tier: "structural",
        changes: 60,
        total: 400,
        complete: false
  ...
  it("groups by category, in a fixed order, with the count opening the group costs", () => {
    ...
    expect(index.groups.map((group) => group.category)).toEqual([
      "story",
      "assets",
      "localization"
    ]);
    ...
    expect(index.groups[0].rows.map((row) => row.path)).toEqual([
      "editor/story/index.json",
      "editor/story/stories/a/storydoc.json"
    ]);
  });

  it("starts a group closed once it is bigger than the threshold", () => {
    const many = Array.from({ length: GROUP_COLLAPSE_THRESHOLD + 1 }, (_, index) =>
      entry({ path: `assets/content/ab/cd/file${index}` })
    );
    const few = Array.from({ length: GROUP_COLLAPSE_THRESHOLD }, (_, index) =>
      entry({ path: `editor/story/stories/s${index}/storydoc.json` })
    );
  ...
  it("closes a group of two hundred, so it costs one line before it is opened", () => {
    const entries = Array.from({ length: 200 }, (_, index) =>
      entry({ path: `assets/content/ab/cd/file${index}`, changes: 5 })
    );
  ...
  it("sums a caveat onto the group once, never onto the rows", () => {
    ...
    const entries = Array.from({ length: 200 }, (_, index) =>
      entry({ path: `assets/content/ab/cd/file${index}`, tier: "structural" })
    );
