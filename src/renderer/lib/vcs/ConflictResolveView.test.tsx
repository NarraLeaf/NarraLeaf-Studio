...
const THAWED: WriteGuard = {
  writes: (ownDisabled = false, ownTooltip?: string) => ({
...
function ready(path: string, decisions: DocumentMergeDecision[]): MergeDocumentEntry {
  return {
    ...
    document: {
      path,
      decisions,
      conflicts: decisions.filter((one) => one.outcome === "conflict").length
    }
  };
}
...
function blocked(
  path: string,
  blocker: VcsMergeDocumentBlocker,
  detail?: string
): MergeDocumentEntry {
  return {
    status: "ready",
    document: { path, decisions: [], conflicts: 0, blocked: blocker, detail }
  };
}
...
const finish = (container: HTMLElement) =>
  container.querySelector<HTMLButtonElement>("[data-resolve-finish]")!;
...
describe("the three states of a row", () => {
  it("marks the undecided row and leaves the two decided ones unmarked", () => {
    ...
    expect([a, b, c].map((row) => row.getAttribute("data-resolve-decision"))).toEqual([
      "mine",
      "theirs",
      "none"
    ]);
...
describe("a file that cannot be merged change by change", () => {
  it("keeps both whole-file buttons and states the reason beside them", () => {
    const { container } = surface(
      ...
      {
        documents: {
          "assets/assets.metadata.image.json": blocked(
            "assets/assets.metadata.image.json",
            "read-only",
            "serialize refused"
          )
  ...
  it("offers the per-change control on a file that can take it", () => {
    const { container } = surface(
      ...
      {
        documents: {
          "editor/localization/en.json": ready("editor/localization/en.json", [
            decision("conflict", "greeting")
          ])
        }
      },
      "editor/localization/en.json"
