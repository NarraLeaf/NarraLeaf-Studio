...
const WIRE: BlueprintGraphEdge = {
  from: { nodeId: "n-head", port: "then" },
  to: { nodeId: "n-log", port: "in" }
};
...
function headDocument(): UIGraphDocument {
  ...
  return uigraphs(
    blueprint(
      "bp-1",
      "Main menu",
      [dragged, edited, node("n-wait", "blueprint.time.wait", 800, 0)],
      []
    )
  );
}
...
describe("every change is either marked or named", () => {
  ...
  it("puts a blueprint's own fields and its layer order on the excused list", () => {
    ...
    const head = uigraphs({
      ...blueprint("bp-1", "Start screen", [HEAD_NODE(), LOG_NODE()], [WIRE])
    });
  ...
  it("opens on the graph with the most changes", () => {
    ...
    const head = uigraphs(
      blueprint(
        "bp-1",
        "Main menu",
        [node("n-head", "blueprint.event.head.click", 40, 60), LOG_NODE()],
        [WIRE]
      ),
      quiet
    );
...
describe("the viewport both columns share", () => {
  ...
  it("answers a drawable box for a graph with no nodes at all", () => {
    expect(sharedGraphViewport([[], []], { width: 300, height: 200 })).toEqual({
      x: 0,
      y: 0,
      width: 300,
      height: 200,
...
describe("reading a path", () => {
  it("reads a blueprint graph's nodes and wires positionally", () => {
    ...
    expect(
      graphMaskTarget(["blueprints", "bp", "functions", "fn", "edges", "a:x->b:y"])?.target
    ).toEqual({ kind: "edge", edgeKey: "a:x->b:y" });
    expect(graphMaskTarget(["blueprints", "bp", "events", "ev"])?.target).toEqual({
      kind: "graph"
    });
    // A leaf handed over on its own is still about its node.
    expect(
      graphMaskTarget(["blueprints", "bp", "events", "ev", "nodes", "n1", "params"])?.target
  ...
  it("keys a graph by its blueprint as well as by its own id", () => {
    expect(graphKeyOf({ blueprintId: "a", slot: "events", graphId: "g" })).not.toBe(
      graphKeyOf({ blueprintId: "b", slot: "events", graphId: "g" })
    );
    expect(graphKeyOf({ blueprintId: null, slot: null, graphId: "g" })).not.toBe(
      graphKeyOf({ blueprintId: "a", slot: "events", graphId: "g" })
    );
