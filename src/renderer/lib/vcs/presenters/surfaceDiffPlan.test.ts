...
function baseDocument(): UIDocument {
  return uidoc(
    [
      surface("surf-title", "Title", "root-title"),
      surface("surf-options", "Options", "root-options")
    ],
    [
      element("root-title", "Root", {
        type: "nl.root",
        childrenIds: ["el-play", "el-quit", "el-exit"]
      }),
...
function headDocument(): UIDocument {
  return uidoc(
    ...
    [
      element("root-title", "Root", {
        type: "nl.root",
        childrenIds: ["el-quit", "el-play", "el-load"]
      }),
      element("el-play", "Play", { parentId: "root-title" }),
      element("el-quit", "Quit", { parentId: "root-title" }),
      element("el-load", "Load", { parentId: "root-title" }),
      element("root-options", "Root", { type: "nl.root", childrenIds: ["el-volume"] }),
      element("el-volume", "Volume", {
        parentId: "root-options",
        layout: { x: 8, y: 8, width: 100, height: 40 }
      })
    ],
    [
      {
        ...COMPONENT,
        elements: {
          "ce-root": element("ce-root", "Slot root", {
            layout: { x: 4, y: 0, width: 100, height: 40 }
          })
        }
      }
    ],
...
function maskFor(plan: ReturnType<typeof planOf>["plan"], elementId: string) {
  return plan.masks.find(
    (mask) => mask.target.kind === "element" && mask.target.elementId === elementId
  );
}
...
describe("which page the canvas opens on", () => {
  ...
  it("opens on the first page rather than on nothing when no page changed", () => {
    ...
    const { plan } = planOf(
      base,
      uidoc(base.surfaces, Object.values(base.elements), base.components, "Renamed")
    );
  ...
  it("still lists a page that only one side has, and says which", () => {
    ...
    const head = uidoc(
      ...
      Object.values(base.elements).filter(
        (one) => !one.id.includes("options") && one.id !== "el-volume"
      ),
      base.components
    );
...
describe("the scale both columns share", () => {
  ...
  it("is bounded by the height as well as the width", () => {
    expect(
      sharedSurfaceScale([{ width: 100, height: 1000 }], { width: 500, height: 250 })
    ).toBeCloseTo(0.25);
  });
...
describe("reading a path", () => {
  it("tells an element apart from the page it is on by length alone", () => {
    ...
    expect(surfaceMaskTarget(["surfaces", "s1", "designSize"])).toEqual({
      kind: "surface",
      surfaceId: "s1"
    });
    expect(surfaceMaskTarget(["surfaces", "s1", "elements", "e1"])).toEqual({
      kind: "element",
      surfaceId: "s1",
      elementId: "e1"
    });
    expect(surfaceMaskTarget(["surfaces", "s1", "elements", "e1", "layout"])).toEqual({
      kind: "element",
      surfaceId: "s1",
      elementId: "e1"
    });
