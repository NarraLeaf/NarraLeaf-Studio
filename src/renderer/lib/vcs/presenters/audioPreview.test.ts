...
describe("reducing samples to a waveform", () => {
  const ramp = (length: number, value: number): Float32Array =>
...
describe("decode budget", () => {
  it("measures a decode from the header rather than by attempting it", () => {
    ...
    expect(estimateDecodedBytes({ durationMs: 600_000, sampleRate: 44_100, channels: 2 })).toBe(
      600 * 44_100 * 2 * 4
    );
  });

  it("assumes stereo when the header does not say, because guessing low lets the big ones through", () => {
    expect(estimateDecodedBytes({ durationMs: 1_000, sampleRate: 44_100 })).toBe(
      estimateDecodedBytes({ durationMs: 1_000, sampleRate: 44_100, channels: 2 })
    );
  });

  it("refuses a track whose decode would not fit", () => {
    // Sixteen minutes of stereo is about 340 MB, five times over.
    expect(withinDecodeBudget({ durationMs: 960_000, sampleRate: 44_100, channels: 2 })).toBe(
      false
    );
  });
