...
describe("which files this draws", () => {
    it("claims what the comparison classified as a bitmap, name or no name", () => {
        ...
        expect(isBitmapEntry(entry("assets/content/99/55/3d15abb54213bad7203798a1adc4", {
            contentClass: "bitmap",
        }))).toBe(true);
...
describe("what a browser will decode", () => {
    ...
    const ascii = (text: string, ...rest: number[]): Uint8Array =>
        new Uint8Array([...[...text].map(character => character.charCodeAt(0)), ...rest, ...Array(16).fill(0)]);
    ...
    it("names the type of every still Chromium can draw", () => {
        ...
        expect(bitmapMediaType(new Uint8Array([0x42, 0x4d, 1, 1, 1, 1, 0, 0, 0, 0, 1]))).toBe("image/bmp");
...
describe("which comparisons two images support", () => {
    it("offers all three when the two are the same size", () => {
        expect(comparableModes({ width: 1024, height: 1024 }, { width: 1024, height: 1024 }))
            .toEqual(["side-by-side", "swipe", "difference"]);
    });
    ...
    it("leaves difference out when the sizes changed", () => {
        ...
        expect(comparableModes({ width: 1088, height: 1984 }, { width: 1024, height: 1024 }))
            .toEqual(["side-by-side", "swipe"]);
        expect(comparableModes({ width: 1024, height: 1024 }, { width: 1024, height: 512 }))
            .toEqual(["side-by-side", "swipe"]);
    });
...
describe("how the two are scaled", () => {
    ...
    it("takes the larger of each direction, which is neither image on its own", () => {
        expect(unionBox({ width: 1088, height: 512 }, { width: 640, height: 1984 }))
            .toEqual({ width: 1088, height: 1984 });
    });
