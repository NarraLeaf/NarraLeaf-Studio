...
describe("presenterFor", () => {
  ...
  it("falls past a presenter that declines the document", () => {
    ...
    expect(presenterFor(entry("editor/story/stories/a/storydoc.json", "story")).id).toBe(
      "test-story"
    );
    ...
    expect(presenterFor(entry("editor/audio/tracks.json", "audio-tracks"))).toBe(
      genericChangePresenter
    );
  });
