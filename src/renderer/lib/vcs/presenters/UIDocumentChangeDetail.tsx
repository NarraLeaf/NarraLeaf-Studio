...
export function UIDocumentChangeDetail({ entry, change, sides }: ChangePresenterProps) {
  ...
  const scale = sharedSurfaceScale([surface?.baseSize ?? null, surface?.headSize ?? null], {
  ...
  return (
    <CanvasShell
        ...
        plan.surfaces.length > 1 && (
          <Select
            ...
            options={plan.surfaces.map((option) => ({
              ...
              secondaryLabel:
                option.changes > 0 ? tn("documentDiff.shell.changes", option.changes) : undefined
            }))}
          />
        )
      }
      ...
      notes={
        <>
          {failure && (
            <CanvasNote tone="danger">{t(failure.key, { error: failure.error })}</CanvasNote>
          )}
          <UnmarkedNote
...
function SurfaceColumn(props: SurfaceColumnProps) {
  ...
  const measure = useCallback(() => {
    ...
    if (
      reported.current.length !== missing.length ||
  ...
  return (
    <figure className="flex min-w-0 flex-col gap-1">
      ...
      <CanvasColumn
        ...
        detail={size ? `${size.width} × ${size.height}` : null}
      />
    </figure>
  );
}
...
function sameBoxes(
  left: ReadonlyMap<number, MaskBox>,
  right: ReadonlyMap<number, MaskBox>
): boolean {
