...
const trackWidths = (container: HTMLElement): string[] =>
    [...container.querySelectorAll("canvas")].map(canvas => (canvas.parentElement as HTMLElement).style.width);
...
describe("two versions on one timeline", () => {
    ...
    it("states each side's numbers once, and marks the ones that changed", async () => {
        ...
        expect(container.textContent).toContain("documentDiff.presenter.audio.stereo → documentDiff.presenter.audio.mono");
    });
...
describe("when a sound cannot be shown", () => {
    it("says the decoder refused it rather than drawing an empty line", async () => {
        ...
        await waitFor(() => expect(container.textContent).toContain("documentDiff.presenter.audio.unreadable"));
