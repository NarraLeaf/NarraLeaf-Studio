...
export function readGraphs(document: UIGraphDocument | null): Map<string, GraphFacts> {
  ...
  const blueprints = recordOf(
    recordOf((document as Record<string, unknown> | null)?.blueprintDocument)?.blueprints
  );
  ...
  for (const [graphId, graph] of Object.entries(
    recordOf((document as Record<string, unknown> | null)?.graphs) ?? {}
  )) {
    const wrapper = recordOf(graph);
    if (!wrapper) continue;
...
function graphFacts(
  ...
  return {
    address,
    key: graphKeyOf(address),
    name: textOrNull(wrapper.name),
    blueprintName,
    nodes,
    edges
  };
...
export function sharedGraphViewport(
  ...
  const right =
    Math.max(...nodes.map((node) => node.x + GRAPH_NODE_WIDTH)) + GRAPH_VIEWPORT_PADDING;
  const bottom =
    Math.max(...nodes.map((node) => node.y + GRAPH_NODE_HEIGHT)) + GRAPH_VIEWPORT_PADDING;
