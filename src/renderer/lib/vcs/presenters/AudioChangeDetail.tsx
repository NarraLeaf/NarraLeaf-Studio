...
function Fact({ before, after }: { before: string | null; after: string | null }) {
    ...
    return <span className="text-2xs font-medium text-warning">{before} → {after}</span>;
}
    ...
    return (
        <figure className="flex items-center gap-2">
            <button
                type="button"
                disabled={!canPlay}
                aria-label={t(playing ? "documentDiff.presenter.audio.pause" : "documentDiff.presenter.audio.play")}
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-1">
                ...
                <div className="w-full">
                    ...
                    >
                        {side.peaks
                            ? <Waveform peaks={side.peaks} tone={which === "after" ? "text-primary" : "text-fg-muted"} />
                            : (
...
function useAudioSide(side: ComparisonSide | null, path: string, host: AudioHost): AudioSide {
    ...
    useEffect(() => {
        ...
        void context.decodeAudioData(copy)
            .then(buffer => {
                if (cancelled) return;
...
function usePlayback(host: AudioHost): Playback {
    ...
    const toggle = useCallback((which: Which, buffer: AudioBuffer) => {
