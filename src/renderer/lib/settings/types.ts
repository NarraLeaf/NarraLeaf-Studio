...
export type TypeofSettingSchema<T extends SettingValueType> = T extends SettingValueType.String
  ? string
  : T extends SettingValueType.Number
    ? number
    : T extends SettingValueType.Integer
      ? number
      : T extends SettingValueType.Boolean
        ? boolean
        : T extends SettingValueType.Enum
          ? string
          : T extends SettingValueType.Color
            ? string
            : T extends SettingValueType.Font
              ? string
              : T extends SettingValueType.Slider
                ? number
                : T extends SettingValueType.Action
                  ? null
                  : T extends SettingValueType.Custom
                    ? null
                    : never;
