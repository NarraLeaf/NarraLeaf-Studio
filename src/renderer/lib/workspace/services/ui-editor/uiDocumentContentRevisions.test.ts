...
function documentWithTwoSurfaces(): UIDocument {
    return {
        ...
        name: "UI",
        surfaces: [
            { id: "surface-a", name: "A", host: "app", kind: "appSurface", designSize: { width: 1280, height: 720 }, rootElementId: "root-a" },
            { id: "surface-b", name: "B", host: "app", kind: "appSurface", designSize: { width: 1280, height: 720 }, rootElementId: "root-b" },
        ],
        components: [],
        elements: {
...
describe("UIDocumentContentRevisions", () => {
    ...
    it("tracks components one by one", () => {
        ...
        const document = documentWithTwoSurfaces();
        document.components = [
            { id: "component-a", name: "A", rootElementId: "a-root", elements: { "a-root": element("a-root", null) } },
            { id: "component-b", name: "B", rootElementId: "b-root", elements: { "b-root": element("b-root", null) } },
        ];
    ...
    it("does not restart counting at 1 after a reset", () => {
        ...
        expect(revisions.getSurfaceContentRevision(document, 0, "surface-a")).toBeGreaterThanOrEqual(beforeReset);
    });
});
