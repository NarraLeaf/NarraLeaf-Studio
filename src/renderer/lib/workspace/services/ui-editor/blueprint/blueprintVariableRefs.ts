...
function variableFromDeclareNode(node: BlueprintGraphNode): BlueprintVariable | null {
  ...
  const defaultValue =
    valueType === "any"
      ? null
      : (normalizeLiteralValue(params.defaultValue) ??
        resolveBlueprintVariableDefaultValue(valueType));
  return {
    id,
    name,
    valueType,
...
export function getEffectiveBlueprintVariableRecord(
  blueprint: Blueprint
): Record<string, BlueprintVariable> {
  return Object.fromEntries(
    listEffectiveBlueprintVariables(blueprint).map((variable) => [variable.id, variable])
  );
}
...
export function createExplicitBlueprintVariableRef(
  blueprintId: string,
  variableId: string
): string {
...
export function parseBlueprintVariableRef(
  raw: unknown,
  currentBlueprintId: string
): BlueprintVariableRef | null {
...
function getSurfaceMainBlueprintId(
  doc: BlueprintDocument,
  surfaceId: string | undefined
): string | undefined {
  return surfaceId
    ? doc.ownerRecords[surfaceMainOwnerKey(surfaceId)]?.activeBlueprintId
    : undefined;
}
...
function pushGroup(
  out: VariableGroupInput[],
  used: Set<string>,
  group: VariableGroupInput | null
): void {
...
function currentBlueprintScope(blueprint: Blueprint): {
  kind: BlueprintVariableScopeKind;
  label: string;
} {
...
export function resolveBlueprintVariableRef(input: {
  ...
  const variable = blueprint
    ? getEffectiveBlueprintVariableRecord(blueprint)[ref.variableId]
    : undefined;
