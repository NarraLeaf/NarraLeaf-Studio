...
export function parseBlueprintFnRef(
  raw: unknown
): { blueprintId: string; headNodeId: string } | null {
  ...
  while (queue.length > 0) {
    ...
    for (const edge of edges) {
      if (
        ...
        execOutputPortIds.has(edge.from.port) &&
 ...
 */
export function resolveBlueprintFnReturnDecls(
  ir: BlueprintGraphIr,
  headNodeId: string
): BlueprintFnPinDecl[] {
  const nodes = ir.nodes ?? {};
  ...
  const returnNodeIds = Object.keys(nodes)
    .filter(
      (nodeId) => nodes[nodeId]?.type === BLUEPRINT_NODE_TYPE_FN_RETURN && reachable.has(nodeId)
    )
    .sort();
...
export function findBlueprintFnByRef(
...
export function listCallableBlueprintFns(
  ...
  return collectDeclaredBlueprintFns(doc).filter((decl) =>
    isBlueprintFnVisibleToOwner(decl.owner, caller)
  );
}
...
export function buildBlueprintFnSignatureSnapshot(
  decl: BlueprintFnDeclaration
): BlueprintFnSignatureSnapshot {
  return {
    name: decl.name,
...
function pinListsEqual(
  a: readonly BlueprintFnPinDecl[],
  b: readonly BlueprintFnPinDecl[]
): boolean {
  return (
    a.length === b.length &&
    a.every(
      (pin, i) =>
        pin.pinId === b[i].pinId && pin.name === b[i].name && pin.valueType === b[i].valueType
    )
  );
}
