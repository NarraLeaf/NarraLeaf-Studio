...
function testDocument(): BlueprintDocument {
  const blueprints: Record<string, Blueprint> = {
    ...
    "bp-s1": graphBlueprint(
      "bp-s1",
      { kind: "surfaceMain", surfaceId: "s1" },
      {
        nodes: {
    ...
    "bp-w1": graphBlueprint(
      "bp-w1",
      { kind: "widgetMain", surfaceId: "s1", elementId: "button" },
      {
        nodes: {
...
describe("fnCatalog", () => {
  it("collects declarations from active graph blueprints with params and reachable returns", () => {
    ...
    expect(surfaceFn.params).toEqual([
      { pinId: "param_1_value", name: "count", valueType: "integer" }
    ]);
    expect(surfaceFn.returns).toEqual([
      { pinId: "ret_1_value", name: "result", valueType: "string" }
    ]);
  ...
  it("round-trips fn refs and rejects malformed values", () => {
    ...
    expect(parseBlueprintFnRef(ref)).toEqual({
      blueprintId: "bp:with:colons",
      headNodeId: "head/1"
    });
  ...
  it("applies the scoping matrix", () => {
    ...
    const widgetDecl: BlueprintOwnerRef = {
      kind: "widgetMain",
      surfaceId: "s1",
      elementId: "button"
    };
    ...
    const callerWidgetS1: BlueprintOwnerRef = {
      kind: "widgetMain",
      surfaceId: "s1",
      elementId: "other"
    };
    const callerValueS1: BlueprintOwnerRef = {
      kind: "widgetValue",
      surfaceId: "s1",
      elementId: "other",
      propPath: "props.text"
    };
    const callerWidgetS2: BlueprintOwnerRef = {
      kind: "widgetMain",
      surfaceId: "s2",
      elementId: "x"
    };
    const callerComponent: BlueprintOwnerRef = {
