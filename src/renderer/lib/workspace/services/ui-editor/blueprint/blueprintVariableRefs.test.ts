...
describe("blueprintVariableRefs", () => {
  it("lists page, blueprint, and global variables and disambiguates duplicate names", () => {
    ...
    expect(
      options
        .filter((option) => option.name === "shared")
        .map((option) => option.disambiguationLabel)
    ).toEqual(["Page", "Blueprint", "Global"]);
    expect(
      options.find((option) => option.name === "globalOnly")?.disambiguationLabel
    ).toBeUndefined();
  });
  ...
  it("uses Var declarations for owner blueprints except Blueprint Value", () => {
    ...
    expect(listEffectiveBlueprintVariables(valueBlueprint).map((variable) => variable.id)).toEqual([
      "legacyValue"
    ]);
