...
describe("blueprint graph validation", () => {
  ...
  it("reports multiple incoming edges to one input pin", () => {
    const ir: BlueprintGraphIr = {
      ...
      edges: [
        ...
        {
          from: { nodeId: "secondSource", port: "result" },
  ...
  it("reports saved variable references that are empty or name a variable the registry lost", () => {
    ...
    expect(
      savedDiagnostics.map((d) => (d.target?.kind === "node" ? d.target.nodeId : undefined)).sort()
  ...
  it("validates Get Var references against Var declaration nodes", () => {
    ...
    expect(validateBlueprintDocumentGraphs(doc, "widget").map((d) => d.code)).toContain(
      "node.variable_id_invalid"
    );
  });
  ...
  it("reports inferred Var type mismatches without removing existing edges", () => {
    ...
    const graph =
      doc.blueprints.widget?.program.kind === "graph"
  ...
  it("reports float output connected to a json input", () => {
    ...
    const ir: BlueprintGraphIr = {
      ...
      edges: [
        { from: { nodeId: "source", port: "value" }, to: { nodeId: "target", port: "values" } }
      ]
    };
  ...
  it("allows float output connected to a string input", () => {
    ...
    const ir: BlueprintGraphIr = {
      ...
      edges: [
        { from: { nodeId: "source", port: "value" }, to: { nodeId: "target", port: "value" } }
      ]
    };
  ...
  it("allows Element Flush element outputs to feed multiple derived nodes", () => {
    ...
    const ir: BlueprintGraphIr = {
      ...
      edges: [
        ...
        {
          from: { nodeId: "flush", port: "element" },
          to: { nodeId: "getNormalized", port: "slider" }
        }
      ]
    };
  ...
  it("allows Element Click element outputs to feed multiple derived nodes", () => {
    ...
    const ir: BlueprintGraphIr = {
      ...
      edges: [
        ...
        {
          from: { nodeId: "click", port: "element" },
          to: { nodeId: "getNormalized", port: "slider" }
        }
      ]
    };
...
describe("blueprint fn validation", () => {
  ...
  it("reports orphan and multi-owner Fn Return nodes", () => {
    ...
    expect(validateBlueprintDocumentGraphs(orphan, "bp-a").map((d) => d.code)).toContain(
      "fn.return_orphan"
    );
    ...
    expect(validateBlueprintDocumentGraphs(shared, "bp-a").map((d) => d.code)).toContain(
      "fn.return_orphan"
    );
  });
