...
describe("blueprint graph editing", () => {
  ...
  it("creates Displayable Animate Property with editable motion defaults", () => {
    expect(
      createGraphNodeForPalette(BLUEPRINT_NODE_TYPE_DISPLAYABLE_ANIMATE_PROPERTY, "animate").params
    ).toEqual({
      property: "opacity",
      from: 0,
      to: 100,
  ...
  it("creates Displayable Get/Set Property with compact editor defaults", () => {
    expect(
      createGraphNodeForPalette(BLUEPRINT_NODE_TYPE_DISPLAYABLE_GET_PROPERTY, "getProperty").params
    ).toEqual({
      property: "position"
    });
    expect(
    ...
    expect(
      createGraphNodeForPalette(BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_PROPERTY, "setProperty").params
    ).toEqual({
      property: "opacity",
      value: 100
    });
    ...
    expect(
      createGraphNodeForPalette(BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_VARIANT, "setVariant").params
    ).toEqual({
