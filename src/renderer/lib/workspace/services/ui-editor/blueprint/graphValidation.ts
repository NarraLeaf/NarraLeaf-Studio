...
function isExecInputEdge(
  ...
  return entry.pins.some(
    (pin) => pin.id === edge.to.port && pin.kind === "input" && pin.semantic === "exec"
  );
}
...
function buildNodeValidationPaletteContext(ctx: {
  ...
  return {
    ...
    isBlueprintValueGraph:
      ctx.isBlueprintValueGraph ?? isBlueprintValueGraphOwner(ctx.blueprintOwner),
...
function validateBlueprintFnRules(
  ...
  if (returnEntries.length > 0) {
    ...
    for (const [headId] of headEntries) {
      ...
      for (const [returnId, returnNode] of owned.slice(1)) {
        if (
          fnPinDeclSignature(readBlueprintFnReturnPinDecls(returnNode.params)) !== authoritative
        ) {
          out.push({
            severity: "error",
            code: "fn.return_signature_conflict",
            message: translate("blueprint.diagnostics.fn.returnSignatureConflict", {
              node: returnId
            }),
  ...
  const recursiveFnRefs =
    callEntries.length > 0 ? collectRecursiveFnRefs(allDecls) : new Set<string>();
  for (const [nodeId, node] of callEntries) {
    ...
    const visible =
      decl && (!ctx.blueprintOwner || isBlueprintFnVisibleToOwner(decl.owner, ctx.blueprintOwner));
    if (!decl || !visible) {
      const snapshotName = readBlueprintFnSignatureSnapshot(node.params)?.name;
      out.push({
        severity: "error",
        code: "fn.call_target_not_found",
        message: translate("blueprint.diagnostics.fn.callTargetNotFound", {
          name: snapshotName ?? fnRef
        }),
        target: nodeTarget(nodeId)
      });
      continue;
  ...
  for (const hook of legacyHooks) {
    if (hook.binding.blueprintId !== blueprintId) {
      out.push({
        ...
        message: translate("blueprint.diagnostics.widget.legacyHookWrongBlueprint", {
          slot: hook.slotName
        })
      });
      continue;
    ...
    if (!supportedEventIds.has(hook.slotName)) {
      out.push({
        ...
        message: translate("blueprint.diagnostics.widget.legacyHookUnsupported", {
          slot: hook.slotName,
          type: element.type
        })
      });
...
export function validateBlueprintGraphIr(
  ...
  if (ctx.graphKind === "function") {
    const entries = Object.entries(nodes).filter(
      ([, n]) => n.type === BLUEPRINT_NODE_TYPE_FUNCTION_ENTRY
    );
    if (entries.length === 0) {
      out.push({
        severity: "error",
  ...
  if (ctx.graphKind === "function") {
    const fnEntryCount = Object.values(nodes).filter(
      (n) => n.type === BLUEPRINT_NODE_TYPE_FUNCTION_ENTRY
    ).length;
    if (fnEntryCount === 1) {
      try {
        const graph = adaptBlueprintGraphIr(
          ir,
          buildBlueprintRunGraphId("validate", ctx.blueprintId, ctx.graphId)
        );
  ...
  for (const edge of edges) {
    ...
    if (
      ...
      !isBlueprintFanOutOutputPin(fromNodeType, edge.from.port)
    ) {
  ...
  for (const [nid, n] of Object.entries(nodes)) {
    ...
    if (
      (n.type === BLUEPRINT_NODE_TYPE_PERSISTENT_SET ||
        n.type === BLUEPRINT_NODE_TYPE_PERSISTENT_GET) &&
      ...
      if (!vid || !ctx.validPersistentVariableIds.has(vid)) {
        out.push({
          ...
          message: translate("blueprint.diagnostics.node.persistentVariableIdInvalid", {
            node: nid
          }),
          target: { kind: "node", graphKind: ctx.graphKind, graphId: ctx.graphId, nodeId: nid }
        });
...
function validateStoryConditionReturnType(
  ...
  for (const [nodeId, node] of Object.entries(nodes)) {
    ...
    const sourceEntry = resolveBlueprintNodeEditorCatalogEntryForNode(
      sourceNode.type,
      sourceParams
    );
    const outPin = sourceEntry.pins.find(
      (pin) => pin.id === valueEdge.from.port && pin.kind === "output"
    );
...
export function validateBlueprintBindingsForBlueprint(
  doc: BlueprintDocument,
  blueprintId: string
): BlueprintGraphEditorDiagnostic[] {
  const bp: Blueprint | undefined = doc.blueprints[blueprintId];
...
export function validateBlueprintDocumentGraphs(
  ...
  const validPersistentVariableIds = new Set(
    registryPersistentVariables.map((variable) => variable.id)
  );
  const persistentVariableValueTypes = registryPersistentVariables.map((variable) => ({
    value: variable.id,
    valueType: variable.valueType
  }));
  const validSavedVariableIds = new Set(
    (options?.savedVariables ?? []).map((variable) => variable.id)
  );
  const out: BlueprintGraphEditorDiagnostic[] = [];
