...
const APPEARANCE_INSPECTOR_VARIANT_CACHE_KEY =
  "uiEditor.editingArea.appearanceInspectorVariantByElementId";

/** Editing-area cache: compact border "sides" row expanded (per element). */
const APPEARANCE_BORDER_SIDES_EXPANDED_CACHE_KEY =
  "uiEditor.editingArea.appearanceBorderSidesExpandedByElementId";
...
export class UIEditorStateService
  extends Service<UIEditorStateService>
  implements IUIEditorStateService
{
  ...
  protected async init(
    ...
    this.selectionUnsubscribe = this.uiStore.getEvents().on("selectionChanged", (selection) => {
    ...
    const variantCache = this.settingsService.getSync<Record<string, string>>(
      APPEARANCE_INSPECTOR_VARIANT_CACHE_KEY
    );
    ...
    const outlineCollapsed = this.settingsService.getSync<Record<string, boolean>>(
      OUTLINE_COLLAPSED_BRANCHES_KEY
    );
    ...
    const outlinePanelCollapsed = this.settingsService.getSync<boolean>(
      OUTLINE_PANEL_COLLAPSED_KEY
    );
  ...
  public adoptSurfaceViewport(surfaceId: string): SurfaceViewportFit | null {
    ...
    const fit = this.viewportFitBySurfaceId.get(surfaceId) ?? {
  ...
  public on<K extends keyof UIEditorStateEvents>(
    event: K,
  ...
  private normalizeViewport(transform: ViewportTransform): ViewportTransform {
    const scale = Math.max(
      SURFACE_ZOOM_MIN_SCALE,
      Math.min(SURFACE_ZOOM_MAX_SCALE, transform.scale)
    );
  ...
  private persistAppearanceInspectorUiCachesNow(): void {
    ...
    void this.settingsService
      .set(APPEARANCE_INSPECTOR_VARIANT_CACHE_KEY, variantRecord)
      .catch((err) => {
        console.warn(
          "[UIEditorStateService] failed to persist appearance inspector variant cache",
          err
        );
      });
    ...
    void this.settingsService
      .set(APPEARANCE_BORDER_SIDES_EXPANDED_CACHE_KEY, expandedRecord)
      .catch((err) => {
        console.warn(
          "[UIEditorStateService] failed to persist appearance border sides expanded cache",
          err
        );
      });
  }
...
function normalizeSmartSnapDetailSettings(raw: unknown): SmartSnapDetailSettings {
  ...
  return {
    snapElementLayout:
      typeof o.snapElementLayout === "boolean" ? o.snapElementLayout : d.snapElementLayout,
    snapElementBorder:
      typeof o.snapElementBorder === "boolean" ? o.snapElementBorder : d.snapElementBorder,
    snapCanvasLayout:
      typeof o.snapCanvasLayout === "boolean" ? o.snapCanvasLayout : d.snapCanvasLayout
  };
}

function areSmartSnapDetailSettingsEqual(
  a: SmartSnapDetailSettings,
  b: SmartSnapDetailSettings
): boolean {
  return (
