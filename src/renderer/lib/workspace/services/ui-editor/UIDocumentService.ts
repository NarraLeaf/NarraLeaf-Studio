...
} from "@/lib/ui-editor/widget-modules/builtin/list/types";
import {
...
const COMPONENT_LINKED_LAYOUT_KEYS = new Set<keyof UILayout>([
  "x",
  "y",
  "width",
  "height",
...
function createDuplicateName(baseName: string, existingNames: Set<string>): string {
  const base = translate("defaultDoc.pageCopy", {
    name: baseName.trim() || translate("defaultDoc.pageName")
  });
...
function remapSurfaceDuplicateReferenceValue<T>(
  value: T,
  ...
  if (typeof value === "string") {
    ...
    if (
      key &&
      ctx.componentIdMap &&
      isReferenceKey(key, "componentId") &&
      ctx.componentIdMap[value]
    ) {
...
function remapDuplicatedBlueprintOwner(
  owner: BlueprintOwnerRef,
  ctx: SurfaceDuplicateRemapContext
): BlueprintOwnerRef | null {
  ...
  if (owner.kind === "widgetMain" && owner.surfaceId === ctx.oldSurfaceId) {
    ...
    return newElementId
      ? { kind: "widgetMain", surfaceId: ctx.newSurfaceId, elementId: newElementId }
      : null;
  }
...
function createContainerTemplateProps(
  overrides: Partial<ContainerWidgetProps>
...
function collectComponentSubtreeElementIds(
...
function calculateElementsBounds(
  elements: UIElement[]
): UISurfaceDesignSize & { x: number; y: number } {
...
export class UIDocumentService extends Service<UIDocumentService> implements IUIDocumentService {
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "uiDocument",
      "workspace.shell.save.stores.uiDocument",
      this.autoSaver
    );
  ...
  public async load(): Promise<UIDocument> {
    ...
    const needsSave =
      schemaChanged || normalizedChanged || mainSurfaceChanged || flowLayoutsChanged;
    if (needsSave) {
  ...
  public getSurfaceContentRevision(surfaceId: string): number {
    return this.contentRevisions.getSurfaceContentRevision(
      this.getDocument(),
      this.revision,
      surfaceId
    );
  ...
  public getComponentContentRevision(componentId: string): number {
    return this.contentRevisions.getComponentContentRevision(
      this.getDocument(),
      this.revision,
      componentId
    );
  ...
  public moveElementsInSurface(
    ...
    const planned = planMoveElementsInSurface(
      document,
      surfaceId,
      elementIds,
      targetParentId,
  ...
  private mutateDocument(
    ...
    options: UIDocumentMutationOptions = {}
  ): void {
    const historyService = this.getHistoryService();
  ...
  private filterLinkedComponentLayoutPatch(layoutPatch: Partial<UILayout>): Partial<UILayout> {
    ...
    for (const [key, value] of Object.entries(layoutPatch) as Array<
      [keyof UILayout, UILayout[keyof UILayout]]
    >) {
  ...
  private normalizeSpecialChildSlots(document: UIDocument): UIDocument {
    ...
    for (const surface of document.surfaces) {
      ...
      const rawMountRecord =
        rawMount && typeof rawMount === "object" ? (rawMount as Record<string, unknown>) : {};
      surface.mount = {
        kind: "slot",
    ...
    for (const element of Object.values(document.elements)) {
      ...
      if (isListLikeWidgetType(element.type)) {
        ...
        const scrollbar =
          props.scrollbar && typeof props.scrollbar === "object"
            ? (props.scrollbar as Record<string, unknown>)
            : {};
        const trackElementId =
          typeof scrollbar.trackElementId === "string" ? scrollbar.trackElementId : null;
        const thumbElementId =
          typeof scrollbar.thumbElementId === "string" ? scrollbar.thumbElementId : null;
      ...
      if (element.type === "nl.slider") {
        const props = (element.props ?? {}) as Record<string, unknown>;
        const trackElementId =
          typeof props.trackElementId === "string" ? props.trackElementId : null;
        const handleElementId =
          typeof props.handleElementId === "string" ? props.handleElementId : null;
        for (const childId of element.childrenIds) {
          const child = document.elements[childId];
          ...
          const sliderSlot =
            childId === handleElementId ? "handle" : childId === trackElementId ? "track" : null;
          if (!sliderSlot) {
            continue;
          }
          child.extra = {
      ...
      if (element.type === UI_SWITCH_ELEMENT_TYPE) {
        ...
        const trackElementId =
          typeof props.trackElementId === "string" ? props.trackElementId : null;
        const thumbElementId =
          typeof props.thumbElementId === "string" ? props.thumbElementId : null;
        for (const childId of element.childrenIds) {
          const child = document.elements[childId];
          ...
          const switchSlot =
            childId === thumbElementId ? "thumb" : childId === trackElementId ? "track" : null;
          if (!switchSlot) {
            continue;
          }
          child.extra = {
  ...
  public createSurface(input: CreateSurfaceInput): UISurface {
    ...
    if (kind === "stageSurface") {
      const existing = this.getDocument().surfaces.find(
        (surface) =>
  ...
  public duplicateSurface(surfaceId: string, name?: string): UISurface | null {
    ...
    const sourceElementIds = Array.from(
      collectSubtreeElementIds(sourceDocument, sourceRootId)
    ...
    if (sourceBlueprintDocument) {
      for (const [ownerKey, ownerRecord] of Object.entries(sourceBlueprintDocument.ownerRecords)) {
        ...
        if (
          !owner ||
          !remapDuplicatedBlueprintOwner(owner, {
    ...
    localBp?.applyBlueprintMutation((bpDoc) => {
      for (const sourceOwnerRecord of Object.values(ownerRecordsToClone)) {
        ...
        if (newOwner.kind === "surfaceMain") {
          ...
          newOwnerKey = widgetValueOwnerKey(
            newOwner.surfaceId,
            newOwner.elementId,
            newOwner.propPath
          );
        ...
        for (const oldBlueprintId of sourceOwnerRecord.privateBlueprintIds) {
          ...
          const clonedBlueprint = cloneBlueprintForSurfaceDuplicate(
            sourceBlueprint,
            newBlueprintId,
            remapContext
          );
    ...
    for (const oldElementId of sourceElementIds) {
      ...
      copy.parentId = sourceElement.parentId
        ? (elementIdMap[sourceElement.parentId] ?? null)
        : null;
      ...
      if (copy.valueBindings) {
        copy.valueBindings = remapElementValueBindingBlueprintIds(
          copy.valueBindings,
          blueprintIdMap
        );
      }
  ...
  public importTemplateBundle(input: ImportTemplateBundleInput): ImportTemplateResult {
    ...
    try {
      const rawBlueprint =
        input.graphs && typeof input.graphs === "object"
    ...
    const importable = sourceDocument.surfaces.filter(
      (surface) => surface.id !== MAIN_APP_SURFACE_ID
    );
    const occupiedStageSlots = new Set<UIStageSlotId>(
      this.getDocument()
        .surfaces.filter(
          (surface): surface is UISurface & { kind: "stageSurface"; mount: UIStageSurfaceMount } =>
  ...
  private importTemplateComponents(
    ...
    const takenNames = new Set(
      (this.getDocument().components ?? []).map((component) => component.name)
    );
    ...
    for (const source of sourceComponents) {
      if (!source.elements?.[source.rootElementId]) {
        ...
        console.warn(
          `[UIDocumentService] template component "${source.name}" skipped (no root element)`
        );
        continue;
      }
      ...
      if (sourceBlueprintDocument) {
        for (const [oldBlueprintId, newBlueprintId] of Object.entries(blueprintIdMap)) {
          ...
          const cloned = remapSurfaceDuplicateReferenceValue(
  ...
  private importSingleSurface(
    ...
    const newSurface: UISurface =
      placement.kind === "stageSurface"
        ? {
            id: newSurfaceId,
            name: nextName,
            host: "player",
  ...
  public createComponentFromElements(
    surfaceId: string,
    elementIds: string[],
    name?: string
    ...
    const topLevelIds = filterToTopLevelMovers(document, elementIds).filter((id) => {
        ...
        copy.parentId =
          oldId === sourceRootId
            ...
            : source.parentId && elementIdMap[source.parentId]
              ...
              : null;
        copy.childrenIds = source.childrenIds
          .filter((childId) => elementIdMap[childId])
          .map((childId) => elementIdMap[childId]);
    ...
    if (blueprintDocument) {
      for (const oldElementId of Object.keys(elementIdMap)) {
        ...
        const sourceBlueprint = sourceBlueprintId
          ? blueprintDocument.blueprints[sourceBlueprintId]
          : undefined;
    ...
    if (blueprintDocument && Object.keys(blueprintIdMap).length > 0) {
      ...
      for (const [oldBlueprintId, newBlueprintId] of Object.entries(blueprintIdMap)) {
        ...
        carried.push({
          ownerKey: componentWidgetMainOwnerKey(componentId, newElementId),
          blueprint: cloned
        });
      }
      ...
      for (const element of Object.values(componentElements)) {
        if (element.behavior) {
          element.behavior = remapSurfaceDuplicateReferenceValue(
            element.behavior,
            remapContext
          ) as UIElement["behavior"];
        }
      }
    }
    ...
    const component: UIComponentDefinition = {
      ...
      name: sanitizeComponentName(
        name,
        selectedTopElements.length === 1
          ? (selectedTopElements[0].name ?? translate("defaultDoc.componentName"))
  ...
  public setComponentInstanceParam(elementId: string, paramId: string, value: string): void {
    ...
    this.mutateDocument(
      ...
      {
        history: surfaceId
          ? { surfaceId, mergeKey: `component-param:${elementId}:${paramId}` }
          : false
      }
    );
  }
  ...
  public duplicateComponent(componentId: string): UIComponentDefinition | null {
    ...
    for (const [oldId, element] of Object.entries(source.elements)) {
      ...
      copy.childrenIds = element.childrenIds
  ...
  public clearComponentElementBlueprintEvent(
    componentId: string,
    elementId: string,
    eventName: string
  ): void {
    ...
    if (current?.kind === "blueprintEvent") {
      const localBp = this.getContext().services.get<LocalBlueprintService>(
  ...
  public stripComponentBlueprintLayerBindings(
    componentId: string,
    blueprintId: string,
    layerEventId: string
  ): void {
  ...
  public reorderComponentChildren(
    componentId: string,
    parentId: string,
  ...
  public pasteComponentClipboardPayload(
    ...
    if (
      payload.v !== 1 ||
      payload.topLevelElementIds.length === 0 ||
  ...
  public createElement(
    ...
    const element: UIElement = {
      ...
      extra: isListLikeWidgetType(parent.type)
        ? ({
    ...
    const elementWithChildren: UIElement = {
      ...
      parentId: element.parentId,
      childrenIds:
        defaultChildren.length > 0 ? defaultChildren.map((child) => child.id) : element.childrenIds,
      layout: {
        ...element.layout,
        ...(defaultChildrenResult?.elementPatch?.layout ?? {})
      },
  ...
  public pasteClipboardPayload(
    ...
    if (
      !target ||
      !allowed.has(targetParentId) ||
      !isValidUIInsertParent(target) ||
      isLinkedUIComponentElement(target)
    ) {
      return { ok: false, reason: "invalid_target" };
    }
    if (beforeChildId != null) {
  ...
  public setElementBlueprintEvent(
    ...
    const beforeHistory =
      surfaceId && historyService ? historyService.captureSnapshot(surfaceId) : null;
    const localBp = this.getContext().services.get<LocalBlueprintService>(Services.LocalBlueprint);
    const bpDoc = localBp.getBlueprintDocument();
    const bp = bpDoc.blueprints[ref.blueprintId];
    const slot = bp?.program.kind === "graph" ? bp.program.graphs.events?.[ref.eventId] : undefined;
  ...
  public stripBlueprintLayerBindings(
    surfaceId: string,
    blueprintId: string,
    layerEventId: string
  ): void {
  ...
  private createDialogStageTemplate(
    ...
    const panelY = Math.max(
      0,
      designSize.height - panelHeight - Math.round(designSize.height * 0.04)
    );
    const panelInset = 28;
  ...
  private configureDefaultDialogBlueprints(
    surfaceId: UISurfaceId,
    template: DialogStageTemplate
  ): void {
    ...
    const nametagBlueprintId = localBp.ensureWidgetMain(
      surfaceId,
      template.nametagId,
      translate("defaultDoc.dialog.nametag"),
      "nl.text"
    );
    localBp.applyBlueprintMutation((doc) => {
    ...
    const avatarBlueprintId = localBp.ensureWidgetMain(
      surfaceId,
      template.avatarId,
      translate("defaultDoc.dialog.avatar"),
      "nl.image"
    );
    localBp.applyBlueprintMutation((doc) => {
  ...
  private configureDefaultChoiceBlueprints(
    surfaceId: UISurfaceId,
    template: ChoiceStageTemplate
  ): void {
    const localBp = this.getOptionalLocalBlueprintService();
    if (!localBp) {
      return;
    ...
    this.seedListItemTextValueBinding(
      template.itemTextId,
      "text",
      translate("defaultDoc.choice.text"),
      translate("defaultDoc.choice.itemText")
    );
  ...
  private configureDefaultNvlBlueprints(surfaceId: UISurfaceId, template: NvlStageTemplate): void {
    ...
    this.seedListItemTextValueBinding(
      template.nametagId,
      "nametag",
      translate("defaultDoc.nvl.nametag"),
      translate("defaultDoc.speaker")
    );
