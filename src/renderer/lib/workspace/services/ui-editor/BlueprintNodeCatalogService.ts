...
} from "@/lib/ui-editor/blueprint-nodes/defineBlueprintNode";
...
{
  private readonly dynamicSelectOptionsSources = new Map<
    string,
    DynamicSelectOptionsSourceRecord
  >();
  ...
  public register(def: BlueprintNodeDef, options?: PluginRegistrationOptions): void {
    ...
    const replaceExisting = Boolean(
      options?.replaceExisting && options.ownerPluginId && existingOwner === options.ownerPluginId
    );
    if (
      options?.ownerPluginId &&
      blueprintNodeRegistry.get(def.type) &&
  ...
  public registerDynamicSelectOptionsSource(
    ...
    if (options?.ownerPluginId && !normalized.startsWith(`${options.ownerPluginId}.`)) {
      throw new Error(
        `Dynamic select options source id must be prefixed with plugin id: ${options.ownerPluginId}`
      );
    }
  ...
  public getDynamicSelectOptions(): Record<string, BlueprintInspectorParamSelectOption[]> {
    ...
    for (const [sourceId, record] of this.dynamicSelectOptionsSources.entries()) {
      try {
        ...
        console.error(
          `[BlueprintNodeCatalogService] Dynamic options source failed: ${sourceId}`,
          error
        );
        options[sourceId] = [];
      }
  ...
  public getBlueprintNodeEditorCatalogEntry(
    type: string
  ): BlueprintNodeEditorCatalogEntry | undefined {
  ...
  private validatePluginOwnedNode(
