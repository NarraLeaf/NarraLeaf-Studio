...
describe("uiDocumentTreeMove flow layout normalization", () => {
  it("drops selected descendants when their ancestor is also selected", () => {
    ...
    expect(filterToTopLevelMovers(document, ["panel", "child", "sibling"])).toEqual([
      "panel",
      "sibling"
    ]);
  });
...
describe("uiDocumentTreeMove structural parts", () => {
  ...
  it("refuses to move a slider's track or handle out of the slider", () => {
    ...
    expect(planMoveElementsInSurface(document, "surface", ["track"], "panel", null)).toEqual({
      ok: false,
      reason: "invalid_movers"
    });
    expect(planMoveElementsInSurface(document, "surface", ["handle"], "root", null)).toEqual({
      ok: false,
      reason: "invalid_movers"
    });
  });
  ...
  it("refuses to move a switch's track or thumb out of the switch", () => {
    ...
    expect(planMoveElementsInSurface(document, "surface", ["thumb"], "panel", null)).toEqual({
      ok: false,
      reason: "invalid_movers"
    });
    expect(planMoveElementsInSurface(document, "surface", ["track"], "root", null)).toEqual({
      ok: false,
      reason: "invalid_movers"
    });
  });
