...
function createHarness(options: { withLocalBlueprint?: boolean; withHistory?: boolean } = {}) {
  ...
  const localBlueprintService = {
    ...
    ensureWidgetValueBlueprint: (input: {
      ...
      const id = createGraphBlueprint(
        `widget-value-${input.elementId}-${input.propPath}`,
...
describe("UIDocumentService surface creation", () => {
  ...
  it("creates app Pages with blocking exit animation enabled by default", () => {
    ...
    expect(initialDocument.surfaces[0]?.settings?.pageAnimation).toEqual(
  ...
  it("wires default Dialog template blueprints for Next, Nametag and Avatar updates", () => {
    ...
    expect(
      nextNodes.some((node: any) => node.type === BLUEPRINT_NODE_TYPE_EVENT_HEAD_MOUSE_CLICK)
    ).toBe(true);
    ...
    expect(elementClickTargets).toEqual(
    ...
    const nextIncomingEdges = nextGraph.edges.filter(
      (edge: any) => edge.to.nodeId === "dialog.next" && edge.to.port === "in"
    );
    expect(nextIncomingEdges).toHaveLength(7);
    const outgoingKeys = nextGraph.edges.map(
      (edge: any) => `${edge.from.nodeId}:${edge.from.port}`
    );
    ...
    const nametagNodeTypes = new Set(
      Object.values(nametagGraph.nodes).map((node: any) => node.type)
    );
    ...
    expect(
      Object.values(nametagGraph.nodes).filter(
        (node: any) => node.type === BLUEPRINT_NODE_TYPE_GAME_GET_NAMETAG
      )
    ).toHaveLength(2);
    ...
    const setAssetIncoming = avatarGraph.edges.filter(
      (edge: any) => edge.to.nodeId === "avatar.update.setImageAsset" && edge.to.port === "in"
    );
    expect(setAssetIncoming).toHaveLength(2);
    expect(avatarGraph.edges).toEqual(
      expect.arrayContaining([
        {
  ...
  it("creates Notification Game UI with a list-driven template and message value binding", () => {
    ...
    expect(itemText.valueBindings?.text).toMatchObject({
      kind: "blueprintValue",
      valueType: "string"
    });
    ...
    expect(
      valueNodes.some((node: any) => node.type === BLUEPRINT_NODE_TYPE_LIST_GET_ITEM_PROPS)
    ).toBe(true);
    expect(
      valueNodes.some(
        (node: any) =>
    ...
    expect(
      valueNodes.some((node: any) => node.type === BLUEPRINT_NODE_TYPE_DATA_RETURN_VALUE)
    ).toBe(true);
  });
  ...
  it("creates Choice Game UI with select wiring and item text value binding", () => {
    ...
    const valueBlueprint = blueprintDocument.blueprints[`widget-value-${itemText.id}-text`];
    const valueNodes = Object.values(
      valueBlueprint.program.graphs.events.init.graph.nodes
    ) as any[];
    expect(
      valueNodes.some(
        (node: any) =>
    ...
    expect(
      selectNodes.some((node: any) => node.type === BLUEPRINT_NODE_TYPE_EVENT_HEAD_ITEM_CLICK)
    ).toBe(true);
    expect(selectNodes.some((node: any) => node.type === BLUEPRINT_NODE_TYPE_GAME_CHOOSE)).toBe(
      true
    );
    const edgeKeys = selectGraph.edges.map((edge: any) => `${edge.from.port}->${edge.to.port}`);
  ...
  it("creates NVL Game UI with next wiring, nametag binding, and the NVL texts leaf", () => {
    ...
    expect(nametag.valueBindings?.text).toMatchObject({
      kind: "blueprintValue",
      valueType: "string"
    });
  ...
  it("returns the existing active Game UI when creating a duplicate slot", () => {
    ...
    const dialogSurfaces = service
      .getDocument()
      .surfaces.filter(
        (surface) => surface.kind === "stageSurface" && surface.mount.slotId === "dialog"
      );
  ...
  it("migrates legacy stage mounts and slot aliases", () => {
    ...
    for (const surface of migrated.surfaces.filter(
  ...
  it("renames the main Page display name while preserving the main surface id", () => {
    const { service } = createHarness();
    const mainSurface = service
      .getDocument()
      .surfaces.find((surface) => surface.id === MAIN_APP_SURFACE_ID);
  ...
  it("duplicates Pages with independent elements and private blueprints", () => {
    const { service, blueprintDocument, createGraphBlueprint } = createHarness({
    ...
    const duplicatedLinkedInstance = duplicatedChildren.find(
      (element) => getUIComponentLink(element)?.componentId === component.id
    )!;
    ...
    expect(getUIComponentLink(duplicatedLinkedInstance)).toEqual({
      componentId: component.id,
      linked: true
    });
    ...
    const duplicatedSurfaceBlueprintId =
      blueprintDocument.ownerRecords[`surfaceMain:${duplicated.id}`]?.activeBlueprintId;
    ...
    expect(blueprintDocument.ownerRecords[`surfaceMain:${source.id}`]?.activeBlueprintId).toBe(
      surfaceBlueprintId
    );
    expect(
      blueprintDocument.ownerRecords[`widgetMain:${source.id}:${button.id}`]?.activeBlueprintId
    ).toBe(widgetBlueprintId);
    expect(
      blueprintDocument.ownerRecords[`widgetValue:${source.id}:${button.id}:label`]
        ?.activeBlueprintId
    ).toBe(valueBlueprintId);
    expect(
      blueprintDocument.ownerRecords[
        `componentWidgetMain:${component.id}:${component.rootElementId}`
      ]?.activeBlueprintId
    ).toBe(componentBlueprintId);
    expect(
      Object.keys(blueprintDocument.ownerRecords).filter((key) =>
        key.startsWith("componentWidgetMain:")
      )
    ...
    expect(duplicatedSurfaceBlueprint.owner).toEqual({
      kind: "surfaceMain",
      surfaceId: duplicated.id
    });
    ...
    expect(duplicatedSurfaceBlueprint.bindings["bind-surface"].source.blueprintId).toBe(
      duplicatedSurfaceBlueprintId
    );
    expect(
      duplicatedSurfaceBlueprint.program.graphs.events.init.graph.nodes["node-self"].params
    ).toMatchObject({
      surfaceId: duplicated.id,
      targetSurfaceId: duplicated.id,
      elementId: duplicatedButton.id,
    ...
    expect(duplicatedWidgetBlueprint.bindings["bind-widget"].source.blueprintId).toBe(
      duplicatedWidgetBlueprintId
    );
    expect(
      duplicatedWidgetBlueprint.program.graphs.events.click.graph.nodes["node-target"].params
    ).toMatchObject({
      surfaceId: duplicated.id,
      elementId: duplicatedLabel.id,
...
describe("UIDocumentService component library", () => {
  ...
  it("keeps instance values pointed across a rename and a removal", () => {
    ...
    expect(getUIComponentLink(service.getDocument().elements[instance.id])?.params).toEqual({
      saveId: "7",
      label: ""
    });
  ...
  it("carries a widget blueprint into the component it extracts", () => {
    ...
    const hit: UIElement = {
      ...
      layout: { x: 0, y: 0, width: 200, height: 60 },
      behavior: {
        events: { mouseClick: { kind: "blueprintEvent", blueprintId: "bp-hit", eventId: "click" } }
      }
    ...
    const boundId = (copy.behavior?.events?.mouseClick as { blueprintId?: string } | undefined)
      ?.blueprintId;
    expect(boundId).toBeTruthy();
    expect(boundId).not.toBe("bp-hit");

    const cloned = blueprintDocument.blueprints[boundId!]!;
    expect(cloned.owner).toMatchObject({
      kind: "componentWidgetMain",
      componentId: component.id,
      elementId: copy.id
    });
    expect(
      blueprintDocument.ownerRecords[`componentWidgetMain:${component.id}:${copy.id}`]
        ?.activeBlueprintId
    ).toBe(boundId);
    ...
    const refParams = (
      cloned.program as never as {
        graphs: {
          events: Record<
            string,
            { graph: { nodes: Record<string, { params: Record<string, string> }> } }
          >;
        };
      }
    ...
    const clones = Object.values(blueprintDocument.blueprints).filter(
      (b) =>
        (b as { owner?: { kind?: string; componentId?: string } }).owner?.kind ===
          "componentWidgetMain" &&
  ...
  it("leaves an empty blueprint behind when it extracts", () => {
    ...
    blueprintDocument.blueprints["bp-empty"] = {
      ...
      program: {
        kind: "graph",
        graphs: {
          events: { click: { id: "click", graph: { nodes: {}, edges: [] } } },
          functions: {}
        }
      }
    } as never;
  ...
  it("wraps multi-selection components in a relative container root", () => {
    ...
    const component = service.createComponentFromElements(
      surface.id,
      [first.id, second.id],
      "Group"
    )!;
...
describe("UIDocumentService template import: components and naming", () => {
  ...
  it("copies a template's components into the library under fresh ids", () => {
    ...
    expect(service.getDocument().components?.map((component) => component.id)).toContain(
      imported.id
    );
...
describe("UIDocumentService template import: multi-surface templates", () => {
  ...
  it("repoints a frame at the sibling surface that arrived with it", () => {
    ...
    const frame = Object.values(service.getDocument().elements).find(
      (element) => element.type === "nl.frame"
    )!;
