...
function createHarness(
  initialDocument = documentWithPositions(0, 0),
  initialBlueprint = emptyBlueprintDocument()
) {
  const uidoc = {
...
describe("UIEditorHistoryService", () => {
  ...
  it("keeps undo and redo isolated per surface", () => {
    ...
    history.record({
      surfaceId: "surface-a",
      before: beforeA,
      after: history.captureSnapshot("surface-a")
    });
    ...
    history.record({
      surfaceId: "surface-b",
      before: beforeB,
      after: history.captureSnapshot("surface-b")
    });
  ...
  it("restores private blueprint resources coupled to UI edits", () => {
    ...
    expect(history.redo("surface-a")).toBe(true);
    expect(
      graphDocument.blueprintDocument.ownerRecords["widgetMain:surface-a:a"]?.activeBlueprintId
    ).toBe("bp-a");
  ...
  it("restores widgetValue blueprint resources coupled to UI edits", () => {
    ...
    expect(history.undo("surface-a")).toBe(true);
    expect(
      graphDocument.blueprintDocument.ownerRecords["widgetValue:surface-a:a:text"]
    ).toBeUndefined();
    ...
    expect(graphDocument.blueprintDocument.blueprints["bp-value-a"]?.owner.kind).toBe(
      "widgetValue"
    );
