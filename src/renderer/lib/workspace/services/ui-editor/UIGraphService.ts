...
import {
  createInitialBlueprintDocument,
  repairGlobalMainIfMissing
} from "./blueprint/blueprintFactories";
import {
  assertValidBlueprintDocument,
  BlueprintDocumentValidationError
} from "./blueprint/documentValidation";
...
export class UIGraphService extends Service<UIGraphService> implements IUIGraphService {
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "uiGraph",
      "workspace.shell.save.stores.uiGraph",
      this.autoSaver
    );
  ...
  private migrateIfNeeded(document: UIGraphDocument): UIGraphDocument {
    ...
    if (!document.blueprintDocument) {
      throw new RendererError(
        "uigraphs.json is missing blueprintDocument (Blueprint M2 required)."
      );
    }
...
): Record<string, BlueprintPersistentVariable> | null {
