...
import type { VoiceConfiguration, VoiceDocument, VoiceLocaleEntry } from "@shared/types/voice";
import type {
...
} from "./puppet/puppetDescriptionModel";
...
interface IProjectService extends IService {
  ...
  updateWebOptimizationConfiguration(
  ...
  importProjectIconSource(
    slot: string
  ): Promise<{ source: ProjectIconSource; bytes: Uint8Array } | null>;
...
interface IUIDocumentService extends IService {
  ...
  clearComponentElementBlueprintEvent(
    componentId: string,
    elementId: string,
    eventName: string
  ): void;
  stripComponentBlueprintLayerBindings(
    componentId: string,
    blueprintId: string,
    layerEventId: string
  ): void;
...
interface IAudioTrackService extends IService {
  ...
  updateTrack(
    id: string,
    patch: Partial<Omit<ProjectAudioTrack, "id" | "builtin" | "parentId">>
  ): void;
...
interface ILocalBlueprintService extends IService {
  ...
  captureBlueprintHistorySnapshot(
    blueprintId: string,
    ownerKey?: string
  ): {
    blueprintId: string;
  ...
  getReadonlyComponentWidgetMainSummary(
    componentId: string,
    element: UIElement
  ): ReadonlyBlueprintWidgetSummary;
  createField(
...
interface IUIEditorFontFaceService extends IService {
  acquire(assetId: string): Promise<{ ok: true; cssFamily: string } | { ok: false; error: string }>;
...
interface IUIEditorStateService extends IService {
  ...
  on<K extends keyof UIEditorStateEvents>(
    event: K,
    handler: (data: UIEditorStateEvents[K]) => void
  ): () => void;
}
...
interface IConsoleService extends IService {
  ...
  onChannelsChanged(
    handler: (event: { channels: readonly ConsoleChannelDefinition[] }) => void
  ): () => void;
  ...
  onProgressChanged(
    handler: (event: { channel: ConsoleChannelId; progress: ConsoleProgress | null }) => void
  ): () => void;
}
...
interface IAssetService extends IService {
  ...
  importLocalAssets<T extends AssetType>(
    type: T
  ...
  refreshRemoteAsset<T extends AssetType>(
...
interface IServiceAssetsService extends IService {
  writeStore<T extends Record<string, any>>(
    namespace: string,
    data: T
...
interface ILocalizationService extends IService {
  ...
  onDocumentChanged(
    handler: (event: { locale: string; document: LocalizationDocument }) => void
  ): () => void;
  flushPendingChanges(): Promise<void>;
}
...
export {
  ...
  IWorkspaceFreezeService,
  IWorkspaceReloadService,
  IVideoService,
  ICharacterService,
  IHistoryService,
  IUIDocumentService,
  IUIEditorHistoryService,
  IUIGraphService,
  ILocalBlueprintService,
  IUIBlueprintLifecycleCoordinator,
  IUIRuntimeBridgeService,
  IUIEditorFontFaceService,
  IUIEditorStateService,
  IDevModeService,
  IConsoleService,
  UIEditorStateEvents,
  IProjectDependencyService,
  IVoiceService,
  IVariableRegistryService,
  IAudioTrackService,
  IAppTagService,
  IBrandService,
  IDictionaryService,
