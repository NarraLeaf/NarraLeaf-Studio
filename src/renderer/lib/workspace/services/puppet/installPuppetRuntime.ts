...
export async function installPrebuiltPuppetRuntime(
    ...
    const existed = await required(appPrivilegedFacade.fs.isDirExists(target), "check the runtime directory");
    if (existed) {
        fail(`A runtime called "${backend}" is already installed. Remove it first, or install under another name.`);
    }

    if (source.kind === "directory") {
        const entries = await required(appPrivilegedFacade.fs.list(source.path), "read that folder");
        if (!entries.some(entry => entry.type !== "directory" && entry.fileName === PUPPET_RUNTIME_ENTRY_FILE)) {
            fail(
        ...
        await required(appPrivilegedFacade.fs.copyDir(source.path, target), "copy that folder into the project");
    } else {
...
export async function removePuppetRuntime(project: Porject, backend: string): Promise<void> {
    await required(appPrivilegedFacade.fs.deleteDir(runtimeDir(project, backend)), `remove the "${backend}" runtime`);
}
