...
function fakeContainer() {
    const container = {
        children: [] as unknown[],
        replaceChildren: vi.fn(() => { container.children = []; }),
    };
...
function options(source: string, patch: Partial<PuppetModelSessionOptions> = {}): PuppetModelSessionOptions {
    return {
...
describe("createPuppetModelSession", () => {
    ...
    it("resolves siblings against the bundle root, the way the engine does", async () => {
        ...
        expect(seen.resolveSibling("textures/page-0.png"))
            .toBe("app://fs/grant/models/alice/textures/page-0.png");
    ...
    it("empties the container on dispose even when the backend throws on the way out", async () => {
        ...
        const session = await createPuppetModelSession(options(`
            export default {
                name: "renderer-a",
                mount() {
    ...
    it("fails loudly when the module registers nothing under the requested name", async () => {
        await expect(createPuppetModelSession(options(`
    ...
    it("fails loudly when the module itself is broken", async () => {
        ...
        await expect(createPuppetModelSession(options("throw new Error('nope');")))
            .rejects.toBeInstanceOf(PuppetBackendUnavailableError);
    });
    ...
    it("pushes a complete state through to the backend", async () => {
        const session = await createPuppetModelSession(options(REPORTING_BACKEND));
        await session.apply({ motion: "walk", expression: null, skin: "default", params: {}, slots: {} });

        expect((globalThis as unknown as { __applied: unknown }).__applied)
            .toEqual({ motion: "walk", expression: null, skin: "default", params: {}, slots: {} });
    });
});
