...
describe("puppetDescriptionKey", () => {
  it("is the identity of the model/runtime pair, not its state on disk", () => {
    ...
    expect(
      puppetDescriptionKey({ assetId: "a", backend: "r", size: { width: 9, height: 9 } })
    ).toBe(key);
  });

  it("separates two entries within one bundle", () => {
    expect(puppetDescriptionKey({ assetId: "a", backend: "r", entry: "one.skel" })).not.toBe(
      puppetDescriptionKey({ assetId: "a", backend: "r", entry: "two.skel" })
    );
  });
...
describe("puppetDescriptionFingerprint", () => {
  it("is stable for unchanged inputs", () => {
    expect(puppetDescriptionFingerprint(baseFingerprint)).toBe(
      puppetDescriptionFingerprint({ ...baseFingerprint, options: { atlas: "model.atlas" } })
    );
  });
  ...
  it.each([
    ...
    expect(puppetDescriptionFingerprint({ ...baseFingerprint, ...patch })).not.toBe(
      puppetDescriptionFingerprint(baseFingerprint)
    );
  });

  it("carries the record version, so a shape change invalidates every file", () => {
    expect(puppetDescriptionFingerprint(baseFingerprint)).toMatch(
      new RegExp(`^${PUPPET_DESCRIPTION_CACHE_VERSION}\\.`)
    );
  });
});
...
describe("normalizePuppetDescription", () => {
  ...
  it("treats a degenerate size as no size", () => {
    expect(
      normalizePuppetDescription({ motions: [], size: { width: 0, height: 10 } })?.size
    ).toBeNull();
