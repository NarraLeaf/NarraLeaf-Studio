...
export class StoryService extends Service<StoryService> implements IStoryService {
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "story",
      "workspace.shell.save.stores.story",
      this.autoSaver
    );
  ...
  public async deleteStory(storyId: StoryId): Promise<boolean> {
    ...
    this.getHistoryService().pushCommand(projectHistoryScope(), {
      label: {
        key: "story.history.deleteStory" as TranslationKey,
      ...
      undo: async () => {
        ...
        if (storedDocument) {
          this.events.emit("documentChanged", {
            storyId,
            document: this.getStoryDocument(storyId)
          });
        }
      },
      redo: () => {
  ...
  public async loadStory(storyId: StoryId): Promise<StoryDocument> {
    ...
    if (!result.ok) {
      ...
      throw new RendererError(
        result.error.message || `Failed to read story document: ${entry.name}`
      );
    }
    try {
  ...
  public async reloadFromDisk(): Promise<void> {
    ...
    if (failures.length > 0) {
      throw new RendererError(
        `Could not re-read ${failures.length} story document(s): ${failures.join("; ")}`
      );
    }
  }
  ...
  public async loadAnimationAsset(
    ...
    const entry = this.getAnimationIndex().animations.find(
      (animation) => animation.id === animationId
    );
    if (!entry) {
      throw new RendererError(`Story animation not found: ${animationId}`);
    }
    const result = await this.getFileSystem().readJSON<StoryAnimationAsset>(
      this.getAnimationAssetPath(animationId)
    );
    if (!result.ok) {
      throw new RendererError(
  ...
  public getLoadedAnimationAsset(
    animationId: StoryAnimationAssetId
  ): StoryAnimationAsset | undefined {
  ...
  public async deleteAnimationAsset(animationId: StoryAnimationAssetId): Promise<boolean> {
    ...
    const storedEntry = JSON.parse(
      JSON.stringify(index.animations[position])
    ) as StoryAnimationIndexEntry;
  ...
  private removeAnimationAsset(animationId: StoryAnimationAssetId): void {
    ...
    void this.getFileSystem()
      .deleteFile(this.getAnimationAssetPath(animationId))
      .catch((err) => {
  ...
  public onPluginActionsChanged(
    handler: (actions: StoryPluginActionRegistration[]) => void
  ): () => void {
    return this.events.on("pluginActionsChanged", handler);
  }

  public onDocumentChanged(
    handler: (event: { storyId: StoryId; document: StoryDocument }) => void
  ): () => void {
  ...
  public deleteChapter(storyId: StoryId, chapterId: string): boolean {
    ...
    const name =
      this.getStoryDocument(storyId).chapters.find((c) => c.id === chapterId)?.name ?? "";
    let changed = false;
    this.mutateDocument(storyId, (document) => {
  ...
  public renameSceneVariable(
    storyId: StoryId,
    _sceneId: StorySceneId,
    variableId: string,
    name: string
  ): boolean {
  ...
  public setSceneVariableDefault(
    storyId: StoryId,
    _sceneId: StorySceneId,
    variableId: string,
  ...
  public setSavedVariableDefault(
    storyId: StoryId,
    variableId: string,
    value: StoryLiteralValue
  ): boolean {
  ...
  public clearSceneSnapshotValue(
    storyId: StoryId,
    sceneId: StorySceneId,
    snapshotId: string,
    refKey: string
  ...
  private mutateSceneSnapshot(
    ...
    let changed = false;
    this.mutateDocument(storyId, (document) => {
      const snapshot = document.scenes[sceneId]?.sceneSnapshots?.find(
  ...
  private createDeclaration(
    ...
    this.mutateDocument(storyId, (document) => {
      ...
      insertBlockInScene(scene, block, {
        parentId: null,
        beforeBlockId: scene.rootBlockIds[0] ?? null
      });
      created = {
        id,
  ...
  private updateDeclaration(
    storyId: StoryId,
    variableId: string,
    mutate: (payload: StoryDeclarationPayload) => void
  ): boolean {
    let changed = false;
    this.mutateDocument(storyId, (document) => {
  ...
  public updateScene(storyId: StoryId, sceneId: StorySceneId, patch: StorySceneUpdate): boolean {
    ...
    const nextName =
      patch.name !== undefined
        ? this.cleanName(patch.name, current.name || "Untitled Scene")
        : current.name;
    ...
    const nextBgm =
      patch.bgm !== undefined
        ? patch.bgm && this.cleanOptionalString(patch.bgm.assetId)
          ? patch.bgm
          : undefined
        : current.bgm;
    ...
    const hasDescriptionChange =
      patch.description !== undefined && nextDescription !== (current.description ?? "");
    const hasBackgroundChange =
      patch.defaultBackgroundAssetId !== undefined &&
      nextBackgroundAssetId !== currentBackgroundAssetId;
    const hasBgmChange =
      patch.bgm !== undefined &&
  ...
  public moveScene(
    storyId: StoryId,
    sceneId: StorySceneId,
    target: { chapterId: string; beforeSceneId?: string | null }
  ): boolean {
    let changed = false;
    this.mutateDocument(storyId, (document) => {
  ...
  public moveBlocks(
    storyId: StoryId,
    sceneId: StorySceneId,
    moves: { blockIds: StoryBlockId[]; target: BlockTarget }[]
  ): void {
    this.mutateDocument(storyId, (document) => {
      const scene = this.getSceneOrThrow(document, sceneId);
  ...
  private async syncLibraryAssetLocks(): Promise<void> {
    ...
    for (const entry of index.stories) {
      ...
      const result = await this.getFileSystem().readJSON<StoryDocument>(
        this.getStoryDocumentPath(entry.id)
      );
      if (!result.ok) {
        continue;
      }
  ...
  private getAnimationAssetPath(animationId: StoryAnimationAssetId): string {
    return this.getContext().project.resolve(
      ProjectNameConvention.EditorStoryAnimationDocument(animationId)
    );
  }
  ...
  private async ensureStoryDocumentDir(storyId: StoryId): Promise<void> {
    ...
    if (!exists.data) {
      ...
      if (!created.ok) {
        throw new RendererError(
          created.error.message || "Failed to create story document directory"
        );
      }
    }
  }
