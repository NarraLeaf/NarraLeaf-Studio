...
import {
  assertValidStoryEntityId,
  assertValidStoryId,
  isValidStoryEntityId,
  isValidStoryId
} from "@shared/utils/storyId";
...
export function normalizeStoryLibraryIndex(
...
export function normalizeStoryAnimationIndex(
  index: StoryAnimationIndex,
  now: string
): StoryAnimationIndex {
...
function migrateStoryDocumentV1toV2(document: StoryDocument): StoryDocument {
  ...
  for (const [sceneId, scene] of Object.entries(document.scenes)) {
    ...
    const cleanedScene = { ...scene, sceneVariables, blocks } as StoryScene &
      LegacyStorySceneFields &
      LegacyRegistrySceneFields;
...
function migrateBlockExpressionCondition(
  block: StoryBlock,
  ...
  const condition = block.payload.condition as
    | (StoryConditionRef & LegacyExpressionConditionFields)
    | undefined;
 ...
 */
function listStoryVariableEntries(
  document: StoryDocument,
  scene: StoryScene
): { name: string; ref: StoryVariableRef }[] {
...
function migrateStoryDocumentV5toV6(document: StoryDocument): StoryDocument {
  ...
  for (const [sceneId, scene] of Object.entries(document.scenes)) {
    const legacyScene = scene as StoryScene & LegacyRegistrySceneFields;
    const rows: StoryDeclarationBlock[] = Object.values(legacyScene.sceneVariables ?? {}).map(
      (def) => declarationRowFromDef("scene", def)
    );
    if (sceneId === homeSceneId) {
      rows.push(
        ...Object.values(legacyDoc.savedVariables ?? {}).map((def) =>
          declarationRowFromDef("saved", def)
        )
      );
    }
  ...
  const migrated = {
    ...
    schemaVersion: STORY_DOCUMENT_SCHEMA_VERSION,
    scenes
  } as StoryDocument & LegacyRegistryDocumentFields;
  delete migrated.savedVariables;
  return migrated;
}

function declarationRowFromDef(
  scope: "scene" | "saved",
  def: StorySceneVariableDefinition | StorySavedVariableDefinition
): StoryDeclarationBlock {
  return {
...
function migrateBlockLayerRef(
  block: StoryBlock,
  layerBlockIdsByName: Map<string, StoryBlockId>
): StoryBlock {
  if (
    block.kind !== "action" ||
    (block.payload.action !== "image" && block.payload.action !== "text")
  ) {
    return block;
  }
  ...
  if (name) {
    ...
    nextPayload.layer = sourceBlockId
      ? { kind: "custom", sourceBlockId, name }
      : { kind: "custom", name };
  }
  return { ...block, payload: nextPayload } as StoryBlock;
}

function migrateLegacySceneVariables(
  scene: StoryScene
): Record<string, StorySceneVariableDefinition> {
  ...
  if (block.kind === "nodeAction" && block.payload.action === "choiceOption") {
    return {
      ...
      payload: {
        ...
        disabledWhen: migrateConditionRef(
...
function migrateConditionRef(
  ...
  return {
    ...condition,
    target: migrateLegacyVariableRef(condition.target, sceneVariables, savedVariables)
  };
}
...
function migrateLegacyVariableRef(
  ...
  if (legacy?.scope === "gamePersistent") {
    return {
      scope: "saved",
      variableId: provisionSavedVariable(savedVariables, legacy.namespace ?? "", key)
    };
  }
...
function provisionSceneVariable(
  ...
  sceneVariables[id] = {
    id,
    name,
    valueType: inferStoryValueType(defaultValue),
    defaultValue,
    storageKey: id
  };
  return id;
...
export function normalizeStoryDocument(document: StoryDocument, now: string): StoryDocument {
  ...
  const entrySceneId =
    migrated.entrySceneId && scenes[migrated.entrySceneId]
...
export function normalizeStoryAnimationAsset(
  ...
  const normalizedSequences =
    sequences.length > 0 ? sequences : [createDefaultAnimationSequence(asset.id)];
  const config = {
...
export function updateBlockPayload(
  ...
  nextPayload: StoryBlock["payload"]
): void {
...
export function moveBlockInScene(
  ...
  const block = scene.blocks[blockId];
  const oldSiblings = block.parentId
  ...
  block.parentId = target.parentId;
  const nextSiblings = target.parentId
...
const NUMERIC_TRACK_PROPERTIES = new Set<StoryAnimationTrackProperty>([
  "opacity",
  "zoom",
  "scaleX",
  "scaleY",
...
function normalizeAnimationTrack(
  ...
  const keyframes = [...keyframesByTime.values()].sort(
...
function normalizeAnimationKeyframeValue(
  property: StoryAnimationTrackProperty,
  value: unknown
): StoryAnimationKeyframe["value"] | undefined {
  if (property === "position") {
    const props = normalizeTransformSequenceProps({
      position: value as StoryTransformSequenceProps["position"]
    });
...
function isAnimationTrackProperty(value: unknown): value is StoryAnimationTrackProperty {
  return (
    typeof value === "string" &&
    ANIMATION_TRACK_PROPERTIES.includes(value as StoryAnimationTrackProperty)
  );
}

function migrateAnimationSequencesToTimeline(
  sequences: StoryAnimationSequence[],
  animationId: string
): StoryAnimationTimeline {
  ...
  spans.forEach(({ sequence, endMs }, sequenceIndex) => {
    ...
    for (const [property, value] of Object.entries(props) as [
      StoryAnimationTrackProperty,
      unknown
    ][]) {
  ...
  const durationMs = Math.min(
    MAX_ANIMATION_DURATION_MS,
    Math.max(DEFAULT_ANIMATION_DURATION_MS, ...spans.map((span) => span.endMs))
  );
  return {
...
function normalizeAnimationSequences(
  sequences: StoryAnimationSequence[] | undefined
): StoryAnimationSequence[] {
...
function normalizeAnimationSequence(
  sequence: StoryAnimationSequence | undefined,
  index: number
): StoryAnimationSequence | null {
...
function normalizeTransformSequenceProps(
  props: StoryTransformSequenceProps | undefined
): StoryTransformSequenceProps {
...
function preserveTextIds(
  previous: StoryBlock["payload"],
  next: StoryBlock["payload"]
): StoryBlock["payload"] {
  if (isNodeTextPayload(previous) && isNodeTextPayload(next) && previous.action === next.action) {
    if (
      "text" in previous &&
      "text" in next &&
      isStoryTextSegment(previous.text) &&
      isStoryTextSegment(next.text)
    ) {
      return {
        ...next,
        text: {
    ...
    if (
      "prompt" in previous &&
      "prompt" in next &&
      isStoryTextSegment(previous.prompt) &&
      isStoryTextSegment(next.prompt)
    ) {
      return {
        ...next,
        prompt: {
 ...
 */
export function promoteTempSpeaker(
  document: StoryDocument,
  name: string,
  characterId: string
): number {
