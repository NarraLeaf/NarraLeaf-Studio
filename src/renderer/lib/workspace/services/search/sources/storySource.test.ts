...
function dialogueBlock(
    ...
    return {
        ...
        parentId: null,
        childrenIds: [],
        payload: { action: "dialogue", ...speaker, text: { value: text, textId: `t-${id}`, role: "dialogue" } },
    } as StoryBlock;
}
...
describe("extractStoryEntries: the speaker facet", () => {
    ...
    it("actually filters extracted entries - the regression", () => {
        ...
        const inko = querySearchIndex(entries, "morning speaker:Inko").find(g => g.group === "storyText");
        expect(inko?.hits.map(h => h.entry.text)).toEqual(["Good morning, Inko!"]);

        const passerBy = querySearchIndex(entries, "morning speaker:passer").find(g => g.group === "storyText");
