...
export const blueprintSource: SearchSource = {
    ...
    dependsOn: [Services.UIGraph, Services.LocalBlueprint, Services.BlueprintNodeCatalog, Services.UIDocument],
    extract: ctx => {
    ...
    // The blueprint document lives inside the graph document, so its mutations surface there.
    watch: (ctx, signal) =>
        ctx.services.get<UIGraphService>(Services.UIGraph).onGraphsChanged(() => signal.invalidate()),
