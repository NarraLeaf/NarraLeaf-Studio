...
export function passesFilters(entry: IndexedSearchEntry, filters: SearchFilters): boolean {
  ...
  if (
    filters.assetTypes &&
    !(fields?.assetType && filters.assetTypes.includes(fields.assetType.toLowerCase()))
  ) {
    return false;
  }
...
function matchEntryWith(entry: IndexedSearchEntry, matcher: CompiledMatcher): SearchHit | null {
  ...
  if (hits.length > 0) {
    ...
    return {
      entry,
      score,
      titleRanges: normalizeRanges(
        hits.map((range) => [range.start, range.end] as [number, number])
      ),
      matchReason: "text"
    };
  }
