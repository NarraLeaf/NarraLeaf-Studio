...
describe("querySearchIndex", () => {
  const entries = indexEntries([
    {
      ...
      fields: { sceneName: "Opening", speaker: "Inko" },
      target: { kind: "localizationKey", keyName: "x" }
    },
    {
      id: "2",
      group: "storyText",
      text: "It is a fine morning.",
      detail: "Main Story › Opening",
      fields: { sceneName: "Opening" },
      target: { kind: "localizationKey", keyName: "x" }
    },
    {
      id: "3",
  ...
  it("ANDs terms regardless of word order, across text and detail", () => {
    ...
    expect(forward.find((g) => g.group === "storyText")?.hits.map((h) => h.entry.id)).toEqual([
      "1"
    ]);
    expect(reversed.find((g) => g.group === "storyText")?.hits.map((h) => h.entry.id)).toEqual([
      "1"
    ]);
    // "inko" is in the title, "opening" only in the detail line - both must still match.
    expect(
      querySearchIndex(entries, "inko opening").find((g) => g.group === "storyText")?.hits
    ).toHaveLength(1);
  });
  ...
  it("highlights every matched term in the title", () => {
    const hit = querySearchIndex(entries, "good inko").find((g) => g.group === "storyText")
      ?.hits[0];
  ...
  it("honours a quoted phrase as a single term", () => {
    expect(
      querySearchIndex(entries, '"fine morning"')
        .find((g) => g.group === "storyText")
  ...
  it("narrows by a field filter from query syntax", () => {
    expect(
      querySearchIndex(entries, "morning speaker:inko")
        .find((g) => g.group === "storyText")
        ?.hits.map((h) => h.entry.id)
    ).toEqual(["1"]);
  ...
  it("intersects supplied filters with query-syntax filters", () => {
    const groups = querySearchIndex(entries, "morning type:storyText", {
      filters: { groups: ["variable"] }
    });
  ...
  it("lifts the cap for an expanded group only", () => {
    ...
    const groups = querySearchIndex(many, "morning", {
      maxPerGroup: 5,
      expandedGroups: ["storyText"]
    });
...
describe("querySearchIndex with a refined matcher", () => {
  ...
  it("leaves the term path exactly as it was when no matcher is supplied", () => {
    expect(
      querySearchIndex(entries, "morning")
        .flatMap((group) => group.hits.map((hit) => hit.entry.id))
        .sort()
    ).toEqual(["1", "2", "3"]);
  });
