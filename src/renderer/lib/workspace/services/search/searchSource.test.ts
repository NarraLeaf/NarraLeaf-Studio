...
function entry(id: string, text: string, detail?: string): SearchIndexEntry {
    return { id, group: "blueprintNode", text, detail, target: { kind: "localizationKey", keyName: "x" } };
}
...
describe("dedupSearchEntries", () => {
    it("collapses duplicates into one row carrying the count", () => {
        const out = dedupSearchEntries(
            [entry("1", "Image", "Layer 1"), entry("2", "Image", "Layer 1"), entry("3", "Image", "Layer 1")],
            byTitleAndDetail,
        );
    ...
    it("leaves a row that stands for itself alone without a count", () => {
        const out = dedupSearchEntries([entry("1", "Image", "Layer 1"), entry("2", "Image", "Layer 2")], byTitleAndDetail);
