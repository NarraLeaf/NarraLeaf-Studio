...
describe("planStoryReplace", () => {
    it("plans across several scenes of several stories, counting occurrences and not rows", () => {
        ...
        const side = storyDocument("s2", "Side", [scene("sc3", "Cafe", [{ id: "b5", text: "早上好，店长。" }])]);
    ...
    it("writes nothing while planning", () => {
        const document = storyDocument("s1", "Prologue", [scene("sc1", "Opening", [{ id: "b1", text: "hello" }])]);
    ...
    it("refuses the whole plan when any candidate has gone, and applies none of it", () => {
        const document = storyDocument("s1", "Prologue", [
            scene("sc1", "Opening", [{ id: "b1", text: "hello there" }, { id: "b2", text: "hello again" }]),
        ]);
        ...
        entries.push({
            ...
            id: "story:s1:sc1:b3",
            text: "hello, gone",
            target: { kind: "storyBlock", storyId: "s1", sceneId: "sc1", blockId: "b3", storyName: "Prologue", sceneName: "Opening" },
        });
        ...
        expect(plan.failures).toEqual([{ reason: "blockMissing", storyId: "s1", sceneId: "sc1", blockId: "b3" }]);
    ...
    it("reports a scene, a story and a text-less block as failures too", () => {
        ...
        const entries = entriesFor([storyDocument("s1", "Prologue", [
        ...
        entries.push({
            ...entries[0],
            id: "story:s7:sc7:b7",
            target: { kind: "storyBlock", storyId: "s7", sceneId: "sc7", blockId: "b7", storyName: "Gone", sceneName: "Gone" },
        });
    ...
    it("skips a candidate the live document no longer matches, without failing the plan", () => {
        const document = storyDocument("s1", "Prologue", [
            scene("sc1", "Opening", [{ id: "b1", text: "hello there" }, { id: "b2", text: "typed over" }]),
        ]);
    ...
    it("finds nothing for a pattern that would not compile", () => {
        ...
        const plan = planStoryReplace(ctx, { matcher: matcher("[", { regex: true }), replacement: "x" });
    ...
    it("leaves scene names alone even when the query is one", () => {
        ...
        entries.push({
            ...
            target: { kind: "storyScene", storyId: "s1", sceneId: "sc1", storyName: "Prologue", sceneName: "Corridor" },
        });
        const { ctx } = makeContext([document], entries);
...
describe("applyStoryReplace", () => {
    it("writes one batch per story", () => {
        ...
        expect(batches).toEqual([{ storyId: "s1", count: 2 }, { storyId: "s2", count: 1 }]);
    ...
    it("replaces one row on its own, as its own undo step", () => {
        const document = storyDocument("s1", "Prologue", [
            scene("sc1", "Opening", [{ id: "b1", text: "hello" }, { id: "b2", text: "hello" }]),
        ]);
...
describe("applyStoryReplace - undo", () => {
    function twoStoryWorkspace() {
        ...
        const two = storyDocument("s2", "Side", [scene("sc3", "Cafe", [{ id: "b3", text: "hello hello" }])]);
    ...
    it("undo, redo and undo again all restore the whole sweep in one press", () => {
        ...
        const after = () => [textOf(one, "sc1", "b1"), textOf(one, "sc2", "b2"), textOf(two, "sc3", "b3")];
    ...
    it("rebuilds the payload from the live block, so a field changed since is not reverted", () => {
        ...
        expect(history.undo(projectHistoryScope())).toBe(true);
        expect(textOf(document, "sc1", "b1")).toBe("hello");
        expect((document.scenes.sc1.blocks.b1.payload as { speakerName: string }).speakerName).toBe("Mika");
    });
