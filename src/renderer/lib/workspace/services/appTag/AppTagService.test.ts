...
describe("AppTagService", () => {
  ...
  it("resolves to the release tag on a project with no document", async () => {
    ...
    expect(service.resolveIdentity(null, BASE).displayName).toEqual({
      value: "My Game",
      overridden: false
    });
  });
  ...
  it("round-trips an override and restores it by removing the key", async () => {
    ...
    expect(service.resolveIdentity(created.id, BASE).displayName).toEqual({
      value: "My Game Demo",
      overridden: true
    });
  ...
  it("follows references through a rename, because they hold the id", async () => {
    ...
    expect(service.resolveIdentity(created.id, BASE).identifier).toEqual({
      value: "com.example.mygame.demo",
      overridden: true
    });
  });
  ...
  it("reads a stored list back with its overrides", async () => {
    ...
    expect(service.resolveIdentity("demo", BASE).displayName).toEqual({
      value: "My Game Demo",
      overridden: true
    });
  });
...
describe("AppTagService plugin config", () => {
  it("writes a global field on the project whatever variant is selected", async () => {
    ...
    expect(service.setPluginConfigValue(demo.id, buildField("appId", "global"), " 480 ")).toBe(
      true
    );
    await service.flushPendingChanges();
    ...
    expect(JSON.parse(files.get(DOCUMENT)!).pluginConfig).toEqual({
  ...
  it("writes a variant field on the variant, and on the project under the release tag", async () => {
    ...
    expect(service.resolvePluginConfigValue(APP_TAG_ID_RELEASE, field)).toEqual({
      value: "default",
      overridden: false
    });
    expect(service.resolvePluginConfigValue(demo.id, field)).toEqual({
      value: "beta",
      overridden: true
    });
  });
  ...
  it("keys a platform-scoped field per platform", async () => {
    ...
    expect(service.resolvePluginConfigValue(null, field, "windows")).toEqual({
      value: "1001",
      overridden: false
    });
    expect(service.resolvePluginConfigValue(null, field, "linux")).toEqual({
      value: "",
      overridden: false
    });
  });
  ...
  it("sweeps a variant entry for a field the project owns", async () => {
    ...
    expect(service.resolvePluginConfigValue("demo", field)).toEqual({
      value: "480",
      overridden: false
    });
  });
      ...
      tags: [
        {
          ...
          name: "Demo",
          overrides: {},
          pluginConfig: { "acme.absent": { token: "kept" } }
        }
      ],
  ...
  it("refuses a variant that does not exist", async () => {
    ...
    expect(
      service.setPluginConfigValue("no-such-tag", buildField("branch", "variant"), "beta")
    ).toBe(false);
...
describe("app tag ending page", () => {
  ...
  it("lets a variant state its own page, and restores it by removing the key", async () => {
    ...
    expect(service.resolveEndingSurface(demo.id)).toEqual({
