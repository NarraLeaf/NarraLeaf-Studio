...
export class AppTagService extends Service<AppTagService> implements IAppTagService {
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "appTags",
      "workspace.shell.save.stores.appTags",
      this.autoSaver
    );
  ...
  private takenNames(excludeId: string | null): string[] {
    return [
      ...
      ...this.getDocument()
        .tags.filter((tag) => tag.id !== excludeId)
        .map((tag) => tag.name)
    ];
  }
  ...
  public clearAllOverrides(id: string): boolean {
    ...
    this.applyTagMutation((tags) =>
      tags.map((tag) => (tag.id === id ? { ...tag, overrides: {} } : tag))
    );
    return true;
  }
  ...
  public clearAllPluginConfig(id: string): boolean {
    ...
    this.applyTagMutation((tags) =>
      tags.map((tag) => (tag.id === id ? { ...tag, pluginConfig: {} } : tag))
    );
    return true;
  }
  ...
  private writePluginConfigValue(
    ...
    const onProject =
      !isVariantScopedBuildConfig(field.scope) || !trimmedId || isBuiltinAppTagId(trimmedId);
