...
function buildDocument(): StoryDocument {
  const blocks: Record<string, StoryBlock> = {
    n1: block({
      ...
      payload: {
        action: "narration",
        text: { textId: "t-n1", value: "The rain kept falling.", role: "narration" }
      }
    }),
    d1: block({
      id: "d1",
    ...
    c1: block({
      ...
      payload: {
        action: "choice",
        prompt: { textId: "t-c1", value: "Stay or go?", role: "choicePrompt" }
      }
    }),
    o1: block({
      id: "o1",
      ...
      payload: {
        action: "choiceOption",
        text: { textId: "t-o1", value: "Stay", role: "choiceText" }
      }
    }),
    o2: block({
      id: "o2",
  ...
  return {
    ...
    id: "story-1",
    name: "Story",
    chapters: [{ id: "ch-1", name: "Chapter 1", sceneIds: ["scene-b"] }],
    scenes: {
      "scene-a": {
        id: "scene-a",
...
describe("deriveUnitState", () => {
  ...
  it("is untranslated without a unit or with an empty target", () => {
    ...
    expect(
      deriveUnitState(
        { target: "", sourceHash: hashSourceText(sourceText), status: "translated" },
        sourceText
      )
    ).toBe("untranslated");
  });
  ...
  it("is stale when the source changed after translation", () => {
    const unit = {
      target: "早く帰ろう。",
      sourceHash: hashSourceText("Old line"),
  ...
  it("buckets rows into completed / stale / untranslated", () => {
    ...
    const document: LocalizationDocument = {
      ...
      units: {
        "t-d1": {
          target: "早く帰ろう。",
          sourceHash: hashSourceText("We should go home."),
          status: "translated"
        },
        "t-c1": {
          target: "古い訳",
          sourceHash: hashSourceText("old prompt"),
          status: "translated"
        },
...
describe("buildTranslationExchangeRows", () => {
  ...
  const document: LocalizationDocument = {
    ...
    units: {
      "t-1": {
        target: "雨は降り続けた。",
        sourceHash: hashSourceText("The rain kept falling."),
        status: "translated"
      },
      // Translated against a line that has since been rewritten.
      "t-2": {
        target: "早く帰ろう。",
        sourceHash: hashSourceText("Let us go home."),
        status: "reviewed"
      },
      "key:menu.start": {
        target: "はじめる",
        sourceHash: hashSourceText("Start"),
        status: "reviewed",
        note: "keep it short"
      }
    }
  ...
  it("narrows to the work left when the scope says so", () => {
    expect(
      buildTranslationExchangeRows(units, document, "pending").map((row) => row.unitId)
    ).toEqual(["t-2", "t-3"]);
  });
