...
export class LocalizationService
  extends Service<LocalizationService>
  implements ILocalizationService
{
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "localization",
      "workspace.shell.save.stores.localization",
      this.autoSaver
    );
    ...
    const config = await this.updateConfiguration((config) => {
      if (code === config.sourceLocale && config.locales.length > 1) {
        throw new RendererError(
          "The source language cannot be removed while other languages exist"
        );
      }
  ...
  public async loadDocument(locale: string): Promise<LocalizationDocument> {
    ...
    const document =
      result.status === "missing" ? createEmptyLocalizationDocument(locale) : result.document;
  ...
  public async reloadFromDisk(): Promise<void> {
    ...
    if (failures.length > 0) {
      throw new RendererError(
        `Could not re-read ${failures.length} translation document(s): ${failures.join("; ")}`
      );
    }
  }
  ...
  public async loadKeys(): Promise<LocalizationKeysDocument> {
    ...
    const document =
      result.status === "missing" ? createEmptyLocalizationKeysDocument() : result.document;
