...
export type LocalizationUnitState =
  | "untranslated"
  | "machine"
  | "translated"
  | "reviewed"
...
export function deriveUnitState(
  unit: LocalizationUnit | undefined,
  sourceText: string
): LocalizationUnitState {
  if (!unit || !unit.target) {
    return "untranslated";
  }
...
function getLocalizableWidgetText(
  element: UIElement
): { prop: "text" | "label"; sourceText: string } | null {
...
export function extractUiTranslationRows(document: UIDocument): UiTranslationRow[] {
  ...
  for (const element of Object.values(document.elements)) {
    ...
    const surface = surfaceId
      ? document.surfaces.find((entry) => entry.id === surfaceId)
      : undefined;
