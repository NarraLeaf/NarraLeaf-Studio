...
const EDIT_SOURCES: EditSource[] = [
  ...
  },
  {
    ...
    label: "Characters",
    attach: (ctx, onEdit) =>
      ctx.services.get<CharacterService>(Services.Character).subscribe(onEdit)
  },
  {
...
export class ProjectStatsService extends Service<ProjectStatsService> {
  ...
  private subscribe(ctx: WorkspaceContext): void {
    try {
      ...
      this.subscriptions.push(buildService.onStateChanged((state) => this.handleBuildState(state)));
    } catch (error) {
  ...
  private authorIsPresent(): boolean {
    return (
      this.lastInteractionAt !== null && Date.now() - this.lastInteractionAt <= IDLE_TIMEOUT_MS
    );
  }
  ...
  private today(): ProjectStatsV1["days"][string] {
    ...
    if (!day) {
      ...
      const previous = previousKeys.length
        ? this.stats.days[previousKeys[previousKeys.length - 1]]
        : null;
