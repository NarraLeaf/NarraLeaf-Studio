 ...
 */
const AUDIO_TRACK_EDIT_LABEL: HistoryLabel = {
  key: "project.audio.history.edit" as TranslationKey
};
...
export class AudioTrackService extends Service<AudioTrackService> implements IAudioTrackService {
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "audioTracks",
      "workspace.shell.save.stores.audioTracks",
      this.autoSaver
    );
  ...
  private recordUndoStep(
    ...
    this.getContext()
      .services.get<HistoryService>(Services.History)
      .pushCommand(projectHistoryScope(), {
        label,
        undo: () => restore(before),
        redo: () => restore(after)
      });
  ...
  public createTrack(
    input?: Partial<Omit<ProjectAudioTrack, "id" | "builtin">>
  ): ProjectAudioTrack {
  ...
  public updateTrack(
    ...
    this.applyTrackMutation((tracks) =>
      tracks.map((track) => (track.id === id ? { ...track, ...patch, id: track.id } : track))
    );
  }
  ...
  public reparentTrack(id: string, parentId: string | null): boolean {
    ...
    this.applyTrackMutation((tracks) =>
      tracks.map((track) => (track.id === id ? { ...track, parentId } : track))
    );
    return true;
  }
    ...
    this.applyTrackMutation(
      (tracks) =>
        tracks
          ...
          .map((track) =>
            track.parentId === id ? { ...track, parentId: inheritedParent } : track
          ),
