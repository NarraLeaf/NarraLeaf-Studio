...
export class HistoryService extends Service<HistoryService> implements IHistoryService {
    ...
    public describe(): Array<{ scopeId: HistoryScopeId; undo: number; redo: number; registered: boolean }> {
    ...
    private step(direction: "undo" | "redo", scopeId?: HistoryScopeId): boolean {
        ...
        const put = direction === "undo"
            ? () => stack.restoreUndo(entry)
            : () => stack.restoreRedo(entry);
        const accept = direction === "undo"
            ? () => stack.acceptUndo(entry)
            : () => stack.acceptRedo(entry);
