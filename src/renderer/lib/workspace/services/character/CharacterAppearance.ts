...
export class CharacterAppearance {
  ...
  public setAvatar(key: string, entry: CharacterAvatarEntry | null): void {
    ...
    const next =
      entry && (entry.baked || entry.overrideAssetId || entry.portrait) ? { ...entry } : null;
    if (next) {
      avatars[key] = next;
    } else {
      delete avatars[key];
  ...
  public setAvatarAxisIds(axisIds: string[] | null): void {
    ...
    if (!layered) return;
    const valid = (axisIds ?? []).filter((axisId) =>
      layered.axes.some((axis) => axis.id === axisId)
    );
    if (valid.length > 0) {
  ...
  public getPuppetDefaultState(): PuppetDefaultState {
    ...
    return (
      normalizePuppetDefaultState(this.getPuppet()?.defaultState) ?? {
        motion: null,
        expression: null,
        skin: null
      }
  ...
  public findPsdSlot(path: string[]): { layerId: string; tagId?: string } | null {
    ...
    return axis?.tags.some((tag) => tag.id === slot.tagId)
      ? { layerId: slot.layerId, tagId: slot.tagId }
      : null;
  }
  ...
  public createSnapshot(name: string, tags: CharacterTagSelection): CharacterSnapshot | null {
    ...
    const snapshot: CharacterSnapshot = {
      id: newId("s"),
      name,
      tags: this.resolveTagSelection(tags)
    };
   ...
   */
  public resolveDrawList(selection: {
    poseId?: string | null;
    tags?: CharacterTagSelection | null;
  }): (string | null)[] {
    if (this.appearance.kind === "preset") {
   ...
   */
  public toLayeredDefinition(
    resolveAssetUrl: (assetId: string) => string | null
  ): ResolvedLayeredDefinition | null {
    const layered = this.layered;
    if (!layered) return null;
    ...
    const defaults = Object.values(this.defaultTagSelection()).filter((tagId) =>
      emitted.has(tagId)
    );
