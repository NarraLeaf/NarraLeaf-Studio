...
describe("collectCharacterDiagnostics", () => {
  ...
  it("measures against the declared canvas, and against the largest layer until one is declared", () => {
    ...
    const sizes = {
      [layer.id]: { width: 100, height: 200 },
      [second.id]: { width: 64, height: 64 }
    };
    // No canvas declared: the biggest layer stands in, so only the hat is off-canvas.
    const undeclared = collectCharacterDiagnostics(appearance, sizes).filter(
      (d) => d.code === "offCanvas"
    );
    ...
    const reordered = collectCharacterDiagnostics(appearance, sizes).filter(
      (d) => d.code === "offCanvas"
    );
    ...
    expect(
      collectCharacterDiagnostics(appearance, sizes).filter((d) => d.code === "offCanvas")
    ).toHaveLength(2);
  });
  ...
  it("reports two tags of one axis sharing a display name", () => {
    ...
    const duplicate = collectCharacterDiagnostics(appearance).find(
      (d) => d.code === "duplicateTag"
    );
  ...
  it("reports an axis whose declared default tag is missing, which the getter papers over", () => {
    ...
    const found = collectCharacterDiagnostics(appearance).find(
      (d) => d.code === "axisDefaultMissing"
    );
    expect(found).toMatchObject({
      severity: "warning",
      values: { axis: "Expression", name: "Happy" }
    });
  ...
  it("reports a look whose whole stack draws nothing, and stays quiet on an unstarted character", () => {
    ...
    appearance.setLayerOption(layer.id, happy.id, "asset-1");
    const found = collectCharacterDiagnostics(appearance).filter(
      (d) => d.code === "combinationNoArt"
    );
