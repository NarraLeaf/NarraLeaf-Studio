...
describe("enumerateCombinations", () => {
    it("is the cartesian product of the axes, labelled in axis order", () => {
        ...
        expect(combinations.map(c => c.labels.join("/"))).toEqual([
            "Uniform/Happy", "Uniform/Angry", "Casual/Happy", "Casual/Angry",
        ]);
    });
    ...
    it("flags a hole only in a layer that draws somewhere else", () => {
        ...
        expect(withHole.filter(c => c.missing.length > 0).map(c => c.labels.join("/")))
            .toEqual(["Uniform/Angry", "Casual/Angry"]);
        ...
        expect(enumerateCombinations(appearance).combinations.every(c => c.missing.length === 0)).toBe(true);
    });
