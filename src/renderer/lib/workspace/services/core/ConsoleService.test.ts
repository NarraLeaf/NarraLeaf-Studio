...
const bridgeMock = vi.hoisted(() => ({
  ...
  onBlueprintDebugEvent: vi.fn((_handler: (payload: unknown) => void) => ({
    cancel: () => undefined
  }))
}));
...
beforeEach(() => {
  ...
  bridgeMock.onConsoleLog.mockImplementation((_handler: (payload: unknown) => void) => ({
    cancel: () => undefined
  }));
  bridgeMock.onBlueprintDebugEvent.mockReset();
  bridgeMock.onBlueprintDebugEvent.mockImplementation((_handler: (payload: unknown) => void) => ({
    cancel: () => undefined
  }));
});
...
describe("ConsoleService", () => {
  ...
  it("routes Dev Mode console IPC payloads into the build channel", async () => {
    ...
    bridgeMock.onConsoleLog.mockImplementation((handler) => {
      consoleHandler = handler as (payload: {
        level: "verbose";
        message: string;
        source?: string;
      }) => void;
  ...
  it("exposes the built-in channels by default", () => {
    const service = new ConsoleService();
    expect(service.getChannels().map((channel) => channel.id)).toEqual([
      "blueprint",
      "build",
      "storage"
    ]);
  ...
  it("routes forwarded Dev Mode blueprint debug events into the blueprint channel", async () => {
    const service = new ConsoleService();
    let blueprintDebugHandler: (payload: {
      type: "devtools.log";
      level: string;
      message: string;
    }) => void = () => {
    ...
    bridgeMock.onBlueprintDebugEvent.mockImplementation((handler) => {
      blueprintDebugHandler = handler as (payload: {
        type: "devtools.log";
        level: string;
        message: string;
      }) => void;
