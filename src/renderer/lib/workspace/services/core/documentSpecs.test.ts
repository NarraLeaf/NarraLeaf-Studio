...
describe("document specs agree with ProjectNameConvention", () => {
  ...
  const of = (segments: readonly string[]) =>
    segments.map((segment) => segment.replace(/\/$/, "")).join("/");
  ...
  it("puts every adopted document where the convention says it goes", () => {
    ...
    expect(voiceDocumentSpec.pathFor({ locale: "ja" })).toBe(
      of(ProjectNameConvention.EditorVoiceDocument("ja"))
    );
    expect(localizationDocumentSpec.pathFor({ locale: "zh-CN" })).toBe(
      of(ProjectNameConvention.EditorLocalizationDocument("zh-CN"))
    );
  ...
  it("puts the wave-2 documents where the convention says they go", () => {
    expect(charactersSpec.pathFor()).toBe(
      of([...ProjectNameConvention.EditorServices, "character.json"])
    );
    expect(storyDocumentSpec.pathFor({ storyId: "abc" })).toBe(
      of(ProjectNameConvention.EditorStoryDocument("abc"))
    );
    expect(assetsMetadataSpec.pathFor({ type: AssetType.Image })).toBe(
      of(ProjectNameConvention.AssetsMetadataShard(AssetType.Image))
    );
  });
  ...
  it("claims every asset type's metadata shard", () => {
    for (const type of Object.values(AssetType)) {
      expect(
        assetsMetadataSpec.matches(of(ProjectNameConvention.AssetsMetadataShard(type))),
        type
      ).toBe(true);
    }
  });
