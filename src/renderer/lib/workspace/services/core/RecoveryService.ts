...
async function bringUp(
  ...
  service: Services
): Promise<RecoveryProbeState["detail"]> {
...
export class RecoveryService extends Service<RecoveryService> implements IRecoveryService {
  ...
  public seedFromBoot(failures: readonly ServiceInitFailure[], ctx: WorkspaceContext): void {
    for (const probe of PROBES) {
      ...
      if (failure) {
        this.setProbe(probe.id, {
          status: "failed",
          raw: describeRawError(failure.error),
          detail: null
        });
        ...
        const raw =
          getWorkspaceAnomalyReportCount(probe.source) > 0 ? this.latestRawFor(probe.source) : null;
        this.setProbe(
          probe.id,
          raw ? { status: "failed", raw, detail: null } : { status: "ok", raw: null, detail: null }
        );
      }
    }
    this.emit();
...
function probeService(id: RecoveryProbeId): Services | null {
  switch (id) {
    ...
    case "variables":
      return Services.VariableRegistry;
    case "audioTracks":
      return Services.AudioTracks;
    case "storyDocuments":
      return null;
  }
}
