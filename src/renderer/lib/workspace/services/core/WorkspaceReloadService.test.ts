...
describe("WorkspaceReloadService", () => {
    ...
    it("re-reads from disk, so a change made behind the workspace's back is picked up", async () => {
        ...
        expect(harness.variables.listEntries().map(entry => entry.name)).toEqual(["written_by_somebody_else"]);
    });
    ...
    it("puts a corrupt document into the refuses-to-save state instead of throwing", async () => {
        ...
        await expect(harness.variables.save(harness.variables.getRegistry())).rejects.toThrow(/could not be read/);
    ...
    it("keeps a participant that failed on what it had, and names it to the author", async () => {
        const harness = await createHarness();
        harness.stubs.storyReload.mockRejectedValueOnce(new Error("index.json is on a disconnected volume"));
    ...
    it("announces itself once per pass, after writes are writable again", async () => {
        ...
        expect(seen).toEqual([{ generation: 1, held: false }, { generation: 2, held: false }]);
        stop();
    });
});
