  ...
  implements IProjectDependencyService
{
  ...
  public async resolve(): Promise<ProjectDependencyResolution> {
    ...
    try {
      ...
      console.warn(
        "[ProjectDependencyService] could not list installed plugins; skipping resolve",
        error
      );
  ...
  public async rescan(): Promise<ProjectDependencyTable> {
    ...
    try {
      ...
      /* fall through - collectors tolerate an unloaded doc */
    }
    try {
  ...
  private collectBlueprintNodeUsage(
    usage: DependencyUsageRecord[],
  ...
  private collectWidgetUsage(usage: DependencyUsageRecord[], authoritative: Set<string>): void {
    ...
    let document;
    try {
      document = this.getContext()
        .services.get<UIDocumentService>(Services.UIDocument)
        .getDocument();
    } catch {
      return;
    }
  ...
  private async listInstalledPlugins(): Promise<InstalledPlugin[]> {
    ...
    if (!result.success || !result.data) {
      throw new Error(
        result.success
          ? "Plugin list response was empty"
          : (result.error ?? "Failed to list plugins")
      );
    }
...
export function buildDependencyTable(input: DependencyScanInput): ProjectDependencyTable {
  ...
  const existingById = new Map(
    (existing?.plugins ?? []).map((plugin) => [plugin.id, plugin] as const)
  );

  // Fold usage records into one accumulator per plugin.
  const accumulators = new Map<
    string,
    { hard: boolean; usedBy: Map<DependencyKind, Set<string>> }
  >();
...
): Partial<Record<DependencyKind, string[]>> {
