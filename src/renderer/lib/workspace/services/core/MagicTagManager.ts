...
export class MagicTagManager {
   ...
   */
  public static analyzeWithRegex(filenames: string[], regexPattern: string): MagicTagTemplate {
    if (filenames.length === 0) {
      throw new Error("No filenames provided for analysis");
    }
    ...
    if (captureGroups.length === 0) {
      throw new Error(
        "Regular expression must contain at least one named capture group. Example: (?<category>pattern)"
      );
    }
    ...
    for (const name of cleanedNames) {
      ...
      if (match && match.groups) {
        ...
        const segments = captureGroups.map((groupName) => (match.groups![groupName] || "").trim());
    ...
    const segmentCounts = allSegments.map((segs) => segs.filter((s) => s.length > 0).length);
