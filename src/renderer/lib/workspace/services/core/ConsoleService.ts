...
export class ConsoleService extends Service<ConsoleService> {
  ...
  public onProgressChanged(
    handler: (event: ConsoleServiceEvents["progressChanged"]) => void
  ): () => void {
    return this.events.on("progressChanged", handler);
  }

  public onFocusRequested(
    handler: (event: ConsoleServiceEvents["focusRequested"]) => void
  ): () => void {
  ...
  public onEntriesChanged(
    handler: (event: ConsoleServiceEvents["entriesChanged"]) => void
  ): () => void {
    return this.events.on("entriesChanged", handler);
  }

  public onChannelsChanged(
    handler: (event: ConsoleServiceEvents["channelsChanged"]) => void
  ): () => void {
