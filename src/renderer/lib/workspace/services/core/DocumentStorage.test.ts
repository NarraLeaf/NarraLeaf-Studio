...
describe("RendererDocumentStorage", () => {
  ...
  it("reports a missing file as null and every other failure as a throw", async () => {
    ...
    await expect(harness.storage.read("editor/variables.json")).rejects.toThrow(
      /PERMISSION_DENIED/
    );
  });
  ...
  it("creates the destination directory before copying, which is what makes quarantine possible", async () => {
    ...
    await harness.storage.copy(
      "editor/variables.json",
      ".nlstudio/quarantine/stamp/editor/variables.json"
    );

    expect(
      harness.files.get(join(ROOT, ".nlstudio", "quarantine", "stamp", "editor", "variables.json"))
    ).toBe("{oops");
  });
  ...
  it("refuses to address anything outside the project", async () => {
    ...
    await expect(harness.storage.write("D:/elsewhere/file.json", "{}")).rejects.toThrow(
      DocumentPathError
    );
    await expect(harness.storage.copy("editor/variables.json", "/tmp/leak.json")).rejects.toThrow(
      DocumentPathError
    );
  });
  ...
  it("carries an unreadable document into a quarantine directory that never existed", async () => {
    ...
    expect(result.status === "corrupt" && result.quarantinePath).toBe(
      ".nlstudio/quarantine/2026-07-27T14-32-11-123Z/editor/voice/ja.json"
    );
    expect(
      harness.files.get(
        join(
  ...
  it("does not quarantine while a revision is being shown - the working copy is not the broken one", async () => {
    ...
    try {
      ...
      expect(
        result.status === "corrupt" && String((result.quarantineFailure as Error)?.message)
