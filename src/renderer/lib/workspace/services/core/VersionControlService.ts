...
export class VersionControlService
  extends Service<VersionControlService>
  implements IVersionControlService
{
  ...
  public async getAvailability(): Promise<VcsAvailability> {
    if (!this.availability) {
      const pending: Promise<VcsAvailability> = getInterface()
        .vcs.getAvailability()
        .then(
          (result) => {
  ...
  public async readRevisionDocuments(
    ...
    for (const entry of result.data.documents) {
      documents.set(
        entry.path,
        entry.contentBase64 === null ? null : decodeUtf8(entry.contentBase64)
      );
    }
    return documents;
  }
  ...
  public async restoreRevision(
    ...
    if (freeze.isFrozen()) {
      ...
      await this.getContext()
        .services.get<WorkspaceReloadService>(Services.WorkspaceReload)
        .reload("restore");
    }
    return result.data;
  }
  ...
  public async diffRevisions(
    from: RevisionId,
    to: RevisionId
  ): Promise<VcsRevisionDiffResult | null> {
    if (!(await this.isAvailable())) return null;
