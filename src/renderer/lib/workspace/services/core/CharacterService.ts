...
} from "../character/migrateAppearance";
...
export class CharacterService extends Service<CharacterService> implements ICharacterService {
   ...
   */
  private async reportFlattenedForms(result: {
    status: string;
    normalized?: boolean;
  }): Promise<void> {
    ...
    this.getContext()
      .services.get<UIService>(Services.UI)
      .showError(
  ...
  private applyLoadResult(
    result: Awaited<ReturnType<typeof loadDocument<CharacterStoreDocument>>>
  ): void {
  ...
  private registerStore(
    characters: readonly StoredCharacter[] | undefined,
    groups: Record<string, CharacterGroup> | undefined
  ): void {
    if (groups) {
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      uiService.showError(
        "Failed to persist characters: " + (error instanceof Error ? error.message : String(error))
      );
      return;
    }
    this.dirty = false;
