 ...
 */
export class WorkspaceFreezeService
  extends Service<WorkspaceFreezeService>
  implements IWorkspaceFreezeService
{
  ...
  public async showRevision(
    ...
    if (source.origin.kind !== "revision") {
      ...
      throw new Error(
        "showRevision needs a revision source; use thaw() to return to the working tree."
      );
    }
  ...
  public thaw(): void {
    ...
    if (this.isReleaseHeld()) {
      ...
      console.warn(
        "[WorkspaceFreeze] refused to leave the view while the project files are being rewritten"
      );
      return;
    }
    const reload = this.getContext().services.get<WorkspaceReloadService>(Services.WorkspaceReload);
