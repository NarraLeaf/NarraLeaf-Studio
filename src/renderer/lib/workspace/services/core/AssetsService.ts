...
export class AssetsService extends Service<AssetsService> implements IAssetService {
   ...
   */
  public async transaction(mutator: (svc: this) => Promise<void> | void): Promise<void> {
    this.beginBatch();
    try {
      await mutator(this);
    } finally {
  ...
  private async flushPendingWrites(): Promise<void> {
    if (this.dirtyTypes.size > 0) {
      ...
      const results = await Promise.all(
        types.map(async (type) => ({ type, result: await this.writeAssetsMetadata(type) }))
      );
  ...
  private async flushPendingOrderWrites(): Promise<void> {
    if (
      this.dirtyOrderCategories.size === 0 ||
      !this.assetOrderManager ||
      !this.assetsMetadataManager ||
      !this.groupAssetsManager
    ) {
      return;
    }
    ...
    for (const { category, result } of results) {
      if (!result.ok) {
        this.dirtyOrderCategories.add(category);
        console.warn(
          `[AssetsService] failed to write ${category} asset order: ${result.error.message}`
        );
      }
    }
  }
   ...
   */
  public async fetch<T extends AssetType>(
    asset: Asset<T, AssetSource>
  ): Promise<RequestStatus<AssetData<T>>> {
    return this.getLocalAssetsManager().fetch(asset as Asset<T, AssetSource.Local>);
  }

  public async importLocalAssets<T extends AssetType>(
    type: T
  ...
  public async refreshRemoteAsset<T extends AssetType>(
    ...
    const applied = this.getAssetsMetadataManager().applyRemoteRefresh(
      asset,
      meta,
      changed ? digest : undefined
    );
  ...
  private getThumbnailCachePath(assetId: string): string {
    return this.getContext().project.resolve(
      ProjectNameConvention.EditorThumbnailCacheShard(assetId)
    );
  }
  ...
  private async ensureThumbnailDir(path: string): Promise<void> {
    ...
    if (!exists.ok) {
      throw new RendererError(
        exists.error?.message || "Failed to access thumbnail cache directory"
      );
    }
    if (!exists.data) {
      ...
      if (!created.ok) {
        throw new RendererError(
          created.error?.message || "Failed to create thumbnail cache directory"
        );
      }
    }
  }
  ...
  private async createThumbnailBuffer(buffer: Uint8Array): Promise<Uint8Array> {
    if (typeof document === "undefined" && typeof OffscreenCanvas === "undefined") {
      throw new RendererError(
        "Thumbnail generation requires a document or OffscreenCanvas context"
      );
    }

    const bufferSource = buffer.buffer.slice(
      buffer.byteOffset,
  ...
  private async writeAssetsMetadata(type: AssetType): Promise<FsRequestResult<void>> {
    ...
    const filesystemService = this.getContext().services.get<FileSystemService>(
      Services.FileSystem
    );
  ...
  public async deleteGroup(
    ...
    const blocked = await this.findDeleteBlocker(
      groupManager.collectGroupAssets(category, groupId, recursive),
      options
    );
    if (blocked) {
    ...
    history.pushCommand(projectHistoryScope(), {
      ...
      redo: async () => {
        ...
        for (const plan of restorable) {
          const live =
            this.getAssetsMetadataManager().getAssets()[plan.record.type][plan.record.id];
          if (live) {
  ...
  private async restoreGroupRecords(plan: AssetGroupsRestorePlan): Promise<void> {
    ...
    try {
      const orderManager = this.getAssetOrderManager();
      await orderManager.write(
        plan.category,
        orderManager.getAssetIds(plan.category),
        plan.groupOrder
      );
    } catch (error) {
  ...
  private async deleteGroupWithHistory(
    ...
    try {
      result = await groupManager.deleteGroup(category, groupId, recursive, {
  ...
  public async replaceAssetContent<T extends AssetType>(
    ...
    const written = await this.getLocalAssetsManager().writeAssetContentFromPath(
      asset as Asset<T, AssetSource.Local>,
      sourcePath
    );
  ...
  public async writeAssetTextContent<T extends AssetType>(
    ...
    const written = await this.getLocalAssetsManager().writeAssetContentText(
      asset as Asset<T, AssetSource.Local>,
      text,
      encoding
    );
   ...
   */
  public async duplicateAsset<T extends AssetType>(
    asset: Asset<T, AssetSource>
  ): Promise<RequestStatus<Asset<T, AssetSource.Local>>> {
    if (asset.source !== AssetSource.Local) {
   ...
   */
  public analyzeMagicTagsWithRegex(filenames: string[], regexPattern: string): MagicTagTemplate {
