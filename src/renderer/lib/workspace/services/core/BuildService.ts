 ...
 */
const BLOCKER_MESSAGE_KEYS: Record<
  ReleaseContentBlockerReason,
  "build.contentBlockedStartStory" | "build.contentBlockedScript" | "build.contentBlockedPlugin"
> = {
 ...
 */
const APP_TAG_GRAPH_MESSAGE_KEYS: Record<
  AppTagGraphRefusalReason,
  "build.appTagGraphUnresolved" | "build.appTagGraphUnknownNode" | "build.appTagGraphFnHead"
> = {
...
export class BuildService extends Service<BuildService> {
  ...
  public async start(request: GameBuildRequest): Promise<GameBuildStateSnapshot> {
    ...
    if (invalid.length > 0) {
      ...
      for (const ref of invalid) {
        consoleService?.log(
          ...
          "error",
          translate("build.invalidCommand", {
            story: ref.storyName,
            scene: ref.sceneName,
            source: ref.source
          }),
    ...
    if (unfoldable.length > 0) {
      ...
      for (const use of unfoldable) {
        consoleService?.log(
          BUILD_CONSOLE_CHANNEL,
          "error",
          translate("build.appTagUnresolved", {
            story: use.storyName,
            scene: use.sceneName,
            source: use.source
          }),
      ...
      this.updateState({
        ...
        finishedAt: Date.now(),
        platforms,
        error: translateN("build.appTagUnresolvedSummary", unfoldable.length, {
          count: unfoldable.length
        })
      });
      return this.state;
    }
    ...
    if (nestedCuts.length > 0) {
      ...
      for (const cut of nestedCuts) {
        consoleService?.log(
          BUILD_CONSOLE_CHANNEL,
          "error",
          translate("build.cutPointNested", {
            story: cut.storyName,
            scene: cut.sceneName,
      ...
      this.updateState({
        ...
        finishedAt: Date.now(),
        platforms,
        error: translateN("build.cutPointNestedSummary", nestedCuts.length, {
          count: nestedCuts.length
        })
      });
      return this.state;
    }
    ...
    // already in memory.
    const appTagGraphRefusal = await this.runAppTagGraphGate(
      startedAt,
      platforms,
      request.appTagId
    );
    ...
    const result = await getInterface().gameBuild.start(
      this.projectPath(),
      {
        kind: "surface",
   ...
   */
  private async collectUnfoldableAppTagUses(
    appTagId: string | undefined
  ): Promise<UnfoldableAppTagUse[]> {
  ...
  private async runReleaseContentGate(
    ...
    try {
      ...
      consoleService?.log(
        BUILD_CONSOLE_CHANNEL,
        "warning",
        "The build content check failed to run",
        {
          source: BUILD_CONSOLE_SOURCE
        }
      );
    ...
    for (const stale of answer.staleDeclarations) {
      consoleService?.log(
        BUILD_CONSOLE_CHANNEL,
        "warning",
        translate("build.contentStaleDeclaration", {
    ...
    if (answer.blockers.length > 0) {
      for (const blocker of answer.blockers) {
        consoleService?.log(
          BUILD_CONSOLE_CHANNEL,
          "error",
          translate(BLOCKER_MESSAGE_KEYS[blocker.reason], {
      ...
      consoleService?.log(BUILD_CONSOLE_CHANNEL, "error", refusal, {
        source: BUILD_CONSOLE_SOURCE
      });
    ...
    if (answer.removedScenes.length > 0) {
      ...
      consoleService?.log(
        BUILD_CONSOLE_CHANNEL,
        "info",
        translateN("build.contentKept", kept, {
          count: kept,
          variant: answer.appTagName
        }),
        { source: BUILD_CONSOLE_SOURCE }
      ...
      for (const removed of answer.removedScenes) {
        consoleService?.log(
          BUILD_CONSOLE_CHANNEL,
          "info",
          translate("build.contentDropped", {
  ...
  private async refuseOnTrimCoverageGaps(
    ...
    try {
      ...
      console.error(
        "[Build] the reference index could not be consulted for the content check",
        error
      );
      return null;
    }
    const touching = gaps.filter(
      (gap) =>
        !gap.slice ||
        gap.slice === "story" ||
  ...
  private async runAppTagGraphGate(
    ...
    const refusal = translateN("build.appTagGraphSummary", refused.length, {
   ...
   */
  private async collectUnfoldableSharedBlueprints(
    tagName: string
  ): Promise<UnfoldableAppTagGraph[]> {
    let assets: SharedBlueprintAsset[];
    try {
      assets = await this.getContext()
        .services.get<AssetsService>(Services.Assets)
        .listSharedBlueprints();
    } catch (error) {
      console.error("[Build] could not read the shared blueprints for the variant check", error);
      return [];
    }
    return assets.flatMap((asset) =>
      collectUnfoldableAppTagGraphsInBlueprint(asset.blueprint, { tagName }).map((graph) => ({
        ...graph,
        blueprintName: asset.name || graph.blueprintName
      }))
    );
  }
  ...
  private async runNetworkAllowlistGate(
    ...
    const refused = collectBlueprintNetworkNodes(document).filter(
      (site) => site.literalUrl && !isNetworkAddressAllowed(site.literalUrl, allowlist)
    );
    if (refused.length === 0) {
      return null;
    }
    ...
    const refusal = translateN("build.networkAllowlistSummary", refused.length, {
      count: refused.length
    });
    consoleService?.log(BUILD_CONSOLE_CHANNEL, "error", refusal, { source: BUILD_CONSOLE_SOURCE });
    this.updateState({
  ...
  private async runMediaGate(
    ...
    const refusal = translateN("build.mediaSummary", unplayable.length, {
  ...
  private async runLintGate(
    ...
    try {
      ...
      consoleService?.log(BUILD_CONSOLE_CHANNEL, "error", message, {
        source: BUILD_CONSOLE_SOURCE
      });
...
export function countBlockingLintFindings(
  ...
  return report.entries.filter((entry) => isBlockingLintSeverity(entry.severity, failBuildOn))
    .length;
}
...
export function formatLintFinding(entry: LintReportEntry): string {
  ...
  return (
    translate("lint.console.finding", {
      rule: entry.ruleId,
