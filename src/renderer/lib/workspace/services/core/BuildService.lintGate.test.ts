...
vi.mock("@/lib/app/bridge", () => ({
  ...
  getInterface: () => ({
    gameBuild,
    plugins: { list: async () => ({ success: true, data: { plugins: [] } }) }
  })
}));
        ...
        switch (id) {
          ...
          case Services.Reference:
            return {
              ...
              ensureReady: async () => {},
              getIndexResult: () => ({
                complete: (options.referenceGaps ?? []).length === 0,
                gaps: options.referenceGaps ?? []
              })
            };
...
describe("BuildService lint gate", () => {
    ...
    expect(
      lines.some(
        (line) =>
          ...
          line.level === "error" &&
          line.message.includes("lint.build.blocked(")
      )
    ).toBe(true);
    ...
    expect(
      lines.some(
        (line) =>
          line.channel === BUILD_CONSOLE_CHANNEL &&
          line.level === "info" &&
          line.message === "lint.build.blockedHint"
      )
    ).toBe(true);
  });
    ...
    expect(
      lines.some(
        (line) =>
          line.channel === BUILD_CONSOLE_CHANNEL &&
          line.level === "warning" &&
          line.message.includes("story/dead-end")
      )
    ).toBe(true);
    expect(
      lines.some((line) => line.level === "info" && line.message.includes("story/empty-scene"))
    ).toBe(true);
  ...
  it("builds on a clean report and says so", async () => {
    ...
    expect(
      lines.some(
        (line) => line.level === "success" && line.message.includes("lint.console.finished")
      )
    ).toBe(true);
  });
    ...
    blueprints: {
      bp1: {
        ...
        program: {
          ...
          graphs: {
            events: {
              ev1: { graph: { nodes: { n1: { id: "n1", type: "blueprint.network.fetch" } } } }
            },
            functions: {},
            macros: {}
          }
    ...
    expect(
      lines.some(
        (line) =>
          ...
          line.message.includes("Title Screen")
      )
    ).toBe(true);
  });
...
describe("countBlockingLintFindings", () => {
  ...
  it("reads entries, not the report's own counts", () => {
    ...
    const lying: LintReport = {
      ...report([finding("error")]),
      counts: { error: 99, warning: 0, info: 0 }
    };
...
describe("shouldBlockBuild", () => {
  ...
  it("blocks on a warning at the warning threshold", () => {
    expect(shouldBlockBuild(report([finding("warning")]), config({ failBuildOn: "warning" }))).toBe(
      true
    );
  });
    ...
    sc1: {
      ...
      blocks: {
        n1: {
          ...
          kind: "nodeAction",
          childrenIds: [],
          payload: { action: "narration", text: { textId: "t1", value: "line", role: "narration" } }
        },
        c1: {
          id: "c1",
          kind: "control",
      ...
      referenceGaps: [
        {
          ...
          slice: "ui",
          location: "Title.backgroundImage",
          affects: ["image"]
        }
      ]
    });
    ...
    expect(
      lines.some(
        (line) =>
          ...
          line.message.includes("Continue")
      )
    ).toBe(true);
  });
