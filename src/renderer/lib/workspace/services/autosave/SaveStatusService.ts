...
export class SaveStatusService extends Service<SaveStatusService> {
  ...
  protected async init(
    ...
    this.unobserveWrites = filesystemService.observeWrites((outcome) =>
      this.handleWriteOutcome(outcome)
    );
   ...
   */
  public reportUnreadableDocument(
    error: DocumentCorruptError,
    quarantinePath: string | null
  ): void {
    this.logStorage(
      "error",
      translate("workspace.shell.save.consoleUnreadable", {
        kind: error.kind,
        path: error.path,
        reason: error.reason
      })
    ...
    if (quarantinePath) {
      this.logStorage(
        "error",
        translate("workspace.shell.save.consoleQuarantined", { path: quarantinePath })
      );
    }
  ...
  private handleRefusedWrite(refusal: RefusedWrite): void {
    this.logStorage(
      "error",
      translate("workspace.shell.save.consoleFrozen", {
        path: refusal.path,
  ...
  private handleWriteOutcome(outcome: FsWriteOutcome): void {
    ...
    this.recordFailure(
      outcome.path,
      outcome.error?.code ?? FsRejectErrorCode.UNKNOWN,
  ...
  private recordFailure(path: string, code: FsRejectErrorCode, message: string): void {
    ...
    this.logStorage(
      "error",
      translate("workspace.shell.save.consoleFailed", {
        path,
        code,
        error: message,
  ...
  private logStorage(level: "error" | "success", message: string): void {
    try {
      this.getContext()
        ...
        .log(STORAGE_CONSOLE_CHANNEL, level, message, {
          source: "Storage"
        });
    } catch {
...
export function reportUnreadableDocument(
  ...
  try {
    ctx.services
      .get<SaveStatusService>(Services.SaveStatus)
