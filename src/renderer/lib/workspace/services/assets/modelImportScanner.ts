...
function findLive2dManifests(files: readonly string[]): ManifestHit[] {
  ...
  for (const file of files) {
    ...
    if (kind) {
      hits.push({
        path: file,
        format: live2dBundleFormat(kind),
        rank: kind === "cubism4" ? 100 : 60
      });
    }
  }
  return hits;
...
async function collectLive2dFor(
  hit: ManifestHit,
  root: string,
  fs: ModelScanFs
  ...
  const namesAModel =
    collected.references.some((reference) => reference.role === "moc") ||
...
async function collectSpineFor(
  ...
  const siblingAtlases = allFiles.filter(
    (file) => dirNameOf(file) === directory && isSpineAtlasName(fileNameOf(file))
  );

  const atlas =
    siblingAtlases.find((file) => stemOf(fileNameOf(file)) === stem) ??
  ...
  if (!atlas) {
    return {
      recognised: true,
      references: [],
      problems: [{ kind: "atlasMissing", path: `${stem}.atlas` }]
    };
  }
