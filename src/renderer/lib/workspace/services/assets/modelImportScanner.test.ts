 ...
 */
function fakeFs(tree: Record<string, string>, options: { unreadable?: string[]; rootUnreadable?: boolean } = {}): ModelScanFs {
...
async function scan(family: "live2d" | "spine", tree: Record<string, string>, options?: Parameters<typeof fakeFs>[1]) {
...
describe("scanFolderForModels — Live2D", () => {
    ...
    it("reports an unreadable or malformed manifest rather than treating it as complete", async () => {
        const [unreadable] = await scan("live2d", HIYORI, { unreadable: ["Hiyori/Hiyori.model3.json"] });
        expect(unreadable.problems).toEqual([{ kind: "manifestUnreadable", path: "Hiyori.model3.json" }]);

        const [malformed] = await scan("live2d", { ...HIYORI, "Hiyori/Hiyori.model3.json": "{ not json" });
        expect(malformed.problems).toEqual([{ kind: "manifestUnreadable", path: "Hiyori.model3.json" }]);
    });
    ...
    it("merges two models sharing one folder into one asset with a choice of entry", async () => {
        const [model] = await scan("live2d", {
            "pair/a.model3.json": JSON.stringify({ FileReferences: { Moc: "a.moc3", Textures: ["a.png"] } }),
            "pair/b.model3.json": JSON.stringify({ FileReferences: { Moc: "b.moc3", Textures: ["b.png"] } }),
            "pair/a.moc3": "moc",
            "pair/a.png": "png",
            "pair/b.moc3": "moc",
            "pair/b.png": "png",
    ...
    it("prefers a Cubism 4 manifest over a Cubism 2 one in the same folder", async () => {
        const [model] = await scan("live2d", {
            ...
            "mixed/new.model3.json": JSON.stringify({ FileReferences: { Moc: "m.moc3", Textures: ["t.png"] } }),
            "mixed/m.moc": "moc",
            "mixed/m.moc3": "moc3",
            "mixed/t.png": "png",
        });
...
describe("scanFolderForModels — Spine", () => {
    ...
    it("names the atlas it expected when there is none", async () => {
        const [model] = await scan("spine", { "raptor/raptor.skel": "skel", "raptor/raptor.png": "png" });
