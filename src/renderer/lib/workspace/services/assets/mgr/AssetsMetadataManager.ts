...
export class AssetsMetadataManager {
  ...
  public applyReplacedContent<T extends AssetType>(
    ...
    if (nextExt !== existingAsset.ext) {
      ...
      existingAsset.name = this.resolveNameForExtension(
        asset.type,
        existingAsset.name,
        existingAsset.id,
        nextExt
  ...
  public applyRemoteRefresh<T extends AssetType>(
    ...
    const metadata = this.getAssets();
    const existingAsset = metadata[asset.type][asset.id] as
      | Asset<T, AssetSource.Remote>
      | undefined;
    if (!existingAsset) {
  ...
  private async initAssetsMetadata(): Promise<void> {
    ...
    const tasks = files.map((file) =>
      filesystemService.ensureRegularFile(file.path, JSON.stringify({}), "utf-8")
    );
    ...
    for (const type of Object.values(AssetType)) {
      ...
      if (shardResult.ok) {
        ...
        if (parsed) {
          ...
          console.warn(
            `AssetsService: metadata shard corrupted, backing up and resetting: ${shardPath}`
          );
          const recoveryResult = await filesystemService.recoverCorruptedJsonFile(
            shardPath,
            JSON.stringify({}),
            "utf-8"
          );
          if (!recoveryResult.ok) {
            console.warn(
              `AssetsService: failed to recover corrupted metadata shard: ${shardPath}`,
