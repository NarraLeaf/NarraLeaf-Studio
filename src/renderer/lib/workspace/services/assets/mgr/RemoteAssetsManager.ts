...
export class RemoteAssetsManager {
  ...
  private chooseType(category: AssetCategory, url: string, fetched: RemoteAssetBytes): AssetType {
    ...
    if (declared) {
      const byMime = members.find((type) =>
        MIME_PREFIXES[type]?.some((prefix) => declared.startsWith(prefix))
      );
      if (byMime) {
        return byMime;
      }
  ...
  private async writeSnapshot<T extends AssetType>(
    ...
    const validation = await this.assetsService
      .getFileFormatValidator()
      .validateFileFormat(type, name, bytes);
    ...
    if (!dirExists.ok) {
      return {
        success: false,
        error: `Failed to check destination directory: ${dirExists.error?.message}`
      };
    }
    if (!dirExists.data) {
      ...
      if (!created.ok) {
        return {
          success: false,
          error: `Failed to create destination directory: ${destDir}. ${created.error?.message}`
        };
      }
    }
    ...
    if (!written.ok) {
      return {
        success: false,
        error: `Failed to store the remote asset: ${destPath}. ${written.error?.message}`
      };
    }
  ...
  private async refuseUnplayable<T extends AssetType>(
    ...
    const scratchDir = this.context.project.resolve(
   ...
   * gate below all read.
   */
  private resolveUniqueName<T extends AssetType>(
    type: T,
    url: string,
    ...
    if (!usable) {
      const sniffed = this.assetsService
        .getFileFormatValidator()
        .sniffExtension(type, fetched.bytes);
      if (sniffed) {
...
export function remoteMediaRefusal(outcome: MediaProbeOutcome | null): string | null {
  ...
  return (
    `NarraLeaf cannot play what this URL serves: ${describeVerdict(outcome.verdict)}. ` +
