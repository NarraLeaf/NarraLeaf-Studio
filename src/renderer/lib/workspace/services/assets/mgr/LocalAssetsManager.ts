...
export class LocalAssetsManager {
  ...
  public async fetch<T extends AssetType>(
    ...
    switch (asset.type) {
      case AssetType.Image:
        ...
        return (await this.assetsService.imageService.readLocalImage(
          asset as Asset<AssetType.Image>
        )) as RequestStatus<AssetData<T>>;
      case AssetType.Audio:
        if (!this.assetsService.audioService) {
          throw new RendererError("Audio service not initialized");
        }
        return (await this.assetsService.audioService.readLocalAudio(
          asset as Asset<AssetType.Audio>
        )) as RequestStatus<AssetData<T>>;
      case AssetType.Video:
        if (!this.assetsService.videoService) {
          throw new RendererError("Video service not initialized");
        }
        return (await this.assetsService.videoService.readLocalVideo(
          asset as Asset<AssetType.Video>
        )) as RequestStatus<AssetData<T>>;
      case AssetType.JSON:
        if (!this.assetsService.jsonService) {
          throw new RendererError("JSON service not initialized");
        }
        return (await this.assetsService.jsonService.readLocalJSON(path)) as RequestStatus<
          AssetData<T>
        >;
      case AssetType.Blueprint:
        ...
        return (await this.assetsService.blueprintService.readLocalBlueprint(
          path
        )) as RequestStatus<AssetData<T>>;
      case AssetType.Font:
        ...
        return (await this.assetsService.fontService.readLocalFont(
          asset as Asset<AssetType.Font>
        )) as RequestStatus<AssetData<T>>;
      case AssetType.Model:
        if (!this.assetsService.modelService) {
          throw new RendererError("Model service not initialized");
        }
        return (await this.assetsService.modelService.readLocalModel(
          asset as Asset<AssetType.Model>
        )) as RequestStatus<AssetData<T>>;
      case AssetType.Other:
        if (!this.assetsService.otherService) {
          throw new RendererError("Other service not initialized");
        }
        return (await this.assetsService.otherService.readLocalOther(
          asset as Asset<AssetType.Other>
        )) as RequestStatus<AssetData<T>>;
      default:
        return {
          success: false,
  ...
  public async writeAssetContentText<T extends AssetType>(
    ...
    if (!written.ok) {
      return {
        success: false,
        error: `Failed to write asset text: ${destPath}. ${written.error?.message}`
      };
    }
  ...
  public async createLocalAssetFromBytes<T extends AssetType>(
    ...
    if (!written.ok) {
      return {
        success: false,
        error: `Failed to write asset contents: ${destPath}. ${written.error?.message}`
      };
    }
  ...
  public async duplicateAsset<T extends AssetType>(
    ...
    if (!dirEnsure.ok) {
      return {
        success: false,
        error: `Failed to create destination directory: ${dirEnsure.error?.message}`
      };
    }
    ...
    if (!copyResult.success || !copyResult.data.ok) {
      const msg =
        copyResult.error || (copyResult.data as FsRequestResult<void, false>)?.error.message;
  ...
  private async importModelBundle<T extends AssetType>(
    ...
    if (!copyResult.success || !copyResult.data.ok) {
      ...
      return {
        success: false,
        error: `Failed to copy model bundle: ${sourceDir} to ${destPath}. ${message}`
      };
    }
  ...
  private async importLocalAsset<T extends AssetType>(
    type: T,
    path: string
  ): Promise<RequestStatus<Asset<T, AssetSource.Local>>> {
    if (isBundleAssetType(type)) {
  ...
  private async validateFileFormat<T extends AssetType>(
    type: T,
    path: string
  ): Promise<RequestStatus<void>> {
    const fsService = this.getContext().services.get<FileSystemService>(Services.FileSystem);
