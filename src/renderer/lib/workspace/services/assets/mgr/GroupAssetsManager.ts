...
import {
  legacyShardTypesFor,
  mergeLegacyGroupShards,
  normalizeAssetGroupRecords
} from "../assetCategoryShards";
...
export class GroupAssetsManager {
  ...
  public listOrderedGroups(category: AssetCategory): string[] {
    ...
    return reconcileAssetOrder(
      this.assetsService.getAssetOrderManager().getGroupIds(category),
  ...
  private categoryAssets(category: AssetCategory): Asset<AssetType, AssetSource>[] {
    ...
    return ASSET_CATEGORY_TYPES[category].flatMap((type) => Object.values(metadata[type])) as Asset<
      AssetType,
      AssetSource
    >[];
  }
  ...
  public async deleteGroup(
    ...
    const assetsInGroup = this.categoryAssets(category).filter((a) => a.groupId === groupId);
  ...
  public async duplicateGroup(
    ...
    const newGroupResult = await this.createGroup(
      category,
      `${originalGroup.name} Copy`,
      newParentGroupId
    );
  ...
  private async fetchAssetsGroups(): Promise<AssetGroupMap> {
    ...
    for (const category of ASSET_CATEGORY_ORDER) {
      const shardPath = this.getContext().project.resolve(
        ProjectNameConvention.AssetsGroupsShard(category)
      );
      const shardResult =
        await filesystemService.readJSON<Record<string, LegacyTypedAssetGroup>>(shardPath);
      if (shardResult.ok) {
   ...
   */
  private async createMigratedGroupShards(
    ...
    await Promise.all(
      categories.map(async (category) => {
        const path = this.getContext().project.resolve(
          ProjectNameConvention.AssetsGroupsShard(category)
        );
   ...
   * permanent loss.
   */
  private async readLegacyGroupShards(
    category: AssetCategory
  ): Promise<Record<string, AssetGroup>> {
   ...
   */
  private async removeEmptyParentGroups(
    category: AssetCategory,
    parentGroupId?: string
  ): Promise<void> {
    while (parentGroupId) {
