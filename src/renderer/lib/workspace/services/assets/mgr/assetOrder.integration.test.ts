...
beforeEach(() => {
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({ [IMAGE_C]: assetRecord(IMAGE_C), [IMAGE_A]: assetRecord(IMAGE_A), [IMAGE_B]: assetRecord(IMAGE_B) }),
        "utf-8",
    );
    fs.writeFileSync(
...
describe("migrating a real project that has no order file", () => {
    ...
    it("leaves both shards exactly as they were: byte-identical, still a bare record", async () => {
        const metadataBefore = fs.readFileSync(path.join(projectPath, "assets", "assets.metadata.image.json"), "utf-8");
        const groupsBefore = fs.readFileSync(path.join(projectPath, "assets", "assets.groups.image.json"), "utf-8");

        await openProject();

        expect(fs.readFileSync(path.join(projectPath, "assets", "assets.metadata.image.json"), "utf-8")).toBe(metadataBefore);
        expect(fs.readFileSync(path.join(projectPath, "assets", "assets.groups.image.json"), "utf-8")).toBe(groupsBefore);
    });
