...
describe("asset order, for a project that predates the order file", () => {
  it("loads with every asset present, in key order", async () => {
    ...
    expect(metadataManager.getOrderedAssets(AssetType.Image).map((asset) => asset.id)).toEqual([
      C,
      A,
      B
    ]);
    expect(groupManager.getGroups(AssetCategory.Image).map((group) => group.id)).toEqual([
      "group_2",
      "group_1"
    ]);
...
describe("asset order, once the order file exists", () => {
  ...
  it("drops an id the record no longer holds instead of handing out an empty row", async () => {
    ...
    expect(
      metadataManager.getOrderedAssets(AssetType.Image).every((asset) => asset !== undefined)
    ).toBe(true);
  });
...
describe("group order", () => {
  it("draws the stored order and appends a group created after it was written", async () => {
    ...
    expect(JSON.parse(lastWrite(harness.writes, IMAGE_ORDER)!).groupIds).toEqual([
      "group_2",
      "group_1",
      createdId
    ]);
