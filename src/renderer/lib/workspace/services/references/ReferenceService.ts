...
export class ReferenceService extends Service<ReferenceService> {
   ...
   */
  private rebuildTimers = new Map<
    string,
    { timer: ReturnType<typeof setTimeout>; action: () => void | Promise<void> }
  >();
  ...
  private resolveBlueprintAssetPins(nodeType: string): readonly BlueprintAssetPin[] | null {
    const catalog = this.getContext().services.get<BlueprintNodeCatalogService>(
    ...
    try {
      ...
      return catalog.resolveCatalogEntry(nodeType).pins.flatMap((pin) =>
        pin.assetRef
          ? [
              {
                pinId: pin.id,
  ...
  private rebuildStorySlice(storyId: string): void {
    ...
    try {
      ...
      const name =
        storyService.listStories().find((entry) => entry.id === storyId)?.name ?? storyId;
    ...
    await Promise.all(
      entries.map(async (entry) => {
        try {
          ...
          this.storyAnimationReferences.set(
            entry.id,
            extractStoryAnimationAssetReferences(animation)
          );
  ...
  private rebuildBlueprintSlice(): void {
    ...
    try {
      ...
      this.blueprintReferences = [];
      this.setSliceGaps("blueprint", [
        { reason: "sliceFailed", slice: "blueprint", location: BLUEPRINT_SLICE_LOCATION }
      ]);
    }
  }

  private rebuildUISlice(): void {
    const uiDocumentService = this.getContext().services.get<UIDocumentService>(
      Services.UIDocument
    );
    try {
      ...
      this.uiReferences = [];
      this.setSliceGaps("ui", [
        { reason: "sliceFailed", slice: "ui", location: UI_SLICE_LOCATION }
      ]);
    }
  }
  ...
  private rebuildCharacterSlice(): void {
    ...
    try {
      const characters: ReferenceScannableCharacter[] = characterService
        .listCharacter()
        .map((character) => {
            ...
            appearance.getKind() === "preset"
              ...
              : // A layered character references one image per layer *per tag*, so the slot key
                ...
                appearance.getLayers().flatMap((layer) => {
                  if (!layer.axisId) {
                    return [
                      { slot: `layer:${layer.id}`, detail: layer.name, assetId: layer.assetId }
                    ];
                  }
      ...
      this.characterReferences = [];
      this.setSliceGaps("character", [
        { reason: "sliceFailed", slice: "character", location: CHARACTER_SLICE_LOCATION }
      ]);
    }
  }
