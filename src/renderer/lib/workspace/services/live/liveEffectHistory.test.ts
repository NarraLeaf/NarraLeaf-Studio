...
describe("the stack Ctrl+Z reads inside a session", () => {
    it("has nothing to offer before anything has been done", () => {
        ...
        expect(history.plan("undo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null }))
    ...
    it("answers with the inverse of the last thing this window did", () => {
        ...
        expect(history.plan("undo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null })).toEqual({
            index: 0,
            direction: "undo",
    ...
    it("takes steps back one at a time and puts them back in the same order", () => {
        ...
        const first = history.plan("undo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null });
        ...
        const second = history.plan("undo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null });
        expect(second).toMatchObject({ index: 0, op: { op: "rename-story", name: "First" } });

        const redo = history.plan("redo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null });
    ...
    it("has nothing to put back until something has been taken back", () => {
        const history = new LiveEffectHistory();
        history.record({ effect: effect(1, renamed("Second")), before: { op: "rename-story", name: "Tale" } });
        expect(history.plan("redo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null }))
    ...
    it("leaves the stack where it was when the host refuses a step", () => {
        const history = new LiveEffectHistory();
        history.record({ effect: effect(1, renamed("Second")), before: { op: "rename-story", name: "Tale" } });
        const plan = history.plan("undo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null });
    ...
    it("refuses when the row the last operation was about is gone", () => {
        ...
        expect(history.plan("undo", { self: SELF, cast: EMPTY_CAST, document, assets: () => null, assetFolders: () => null })).toEqual({ impossible: "row-gone" });
    });
    ...
    it("carries the entries a deleted row came with into the insert that puts it back", () => {
        ...
        const plan = history.plan("undo", { self: SELF, cast: EMPTY_CAST, document, assets: () => null, assetFolders: () => null });
    ...
    it("does not attach those entries to an inverse that puts no row back", () => {
        ...
        expect(history.plan("undo", { self: SELF, cast: EMPTY_CAST, document: makeDocument(), assets: () => null, assetFolders: () => null })).not.toHaveProperty("derived");
    });
});
