...
export class DictionaryService extends Service<DictionaryService> implements IDictionaryService {
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "dictionary",
      "workspace.shell.save.stores.dictionary",
      this.autoSaver
    );
  ...
  private async publish(): Promise<void> {
    const localizationService = this.getContext().services.get<LocalizationService>(
    ...
    if (
      this.status?.language === next?.language &&
      this.status?.sourceLocale === next?.sourceLocale
    ) {
      this.status = next;
      return;
    }
