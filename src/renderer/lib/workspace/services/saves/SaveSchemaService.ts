...
export class SaveSchemaService extends Service<SaveSchemaService> implements ISaveSchemaService {
  ...
  protected async init(
    ...
    await registerAutoSaver(
      ...
      depend,
      "saveSchema",
      "workspace.shell.save.stores.saveSchema",
      this.autoSaver
    );
  ...
  public createField(input?: { name?: string; valueType?: SaveSchemaFieldType }): SaveSchemaField {
    ...
    const field: SaveSchemaField = {
      ...
      name,
      valueType,
      storageKey: deriveSaveSchemaStorageKey(
        name,
        id,
        this.listFields().map((f) => f.storageKey)
      ),
  ...
  private applySchemaMutation(
    mutator: (fields: Record<string, SaveSchemaField>) => Record<string, SaveSchemaField>
  ): void {
