...
describe("deriveVoiceUnitState", () => {
  it("returns missing without a unit or asset", () => {
    ...
    expect(deriveVoiceUnitState({ assetId: "", sourceHash: "", status: "linked" }, "Hi")).toBe(
      "missing"
    );
  });
  ...
  it("returns linked/approved when the source hash still matches", () => {
    ...
    expect(deriveVoiceUnitState({ assetId: "a", sourceHash: hash, status: "linked" }, "Hi")).toBe(
      "linked"
    );
    expect(deriveVoiceUnitState({ assetId: "a", sourceHash: hash, status: "approved" }, "Hi")).toBe(
      "approved"
    );
  });
  ...
  it("derives stale when the line text changed since import", () => {
    const hash = hashSourceText("Hi");
    expect(
      deriveVoiceUnitState({ assetId: "a", sourceHash: hash, status: "approved" }, "Hello")
    ).toBe("stale");
  });
});
...
describe("computeVoiceProgress", () => {
  it("counts covered/approved/stale/missing across the voiceable rows", () => {
    const rows = extractVoiceableRows(buildDocument()).map((row) => ({
      unitId: row.unitId,
  ...
  it("reports everything missing without a document", () => {
    ...
    expect(computeVoiceProgress(rows, undefined)).toEqual({
      total: 2,
      covered: 0,
      approved: 0,
      stale: 0,
