...
export function deriveVoiceUnitState(
  unit: VoiceUnit | undefined,
  sourceText: string
): VoiceUnitState {
...
export function computeVoiceProgress(
  ...
  for (const row of rows) {
    const state = deriveVoiceUnitState(
      document?.units[row.unitId],
      lineTextFor(row.unitId, row.sourceText)
    );
    if (state === "missing") {
      progress.missing += 1;
    } else if (state === "stale") {
