...
describe("TextEditorContributionService", () => {
  ...
  it("offers an action with no extensions on every document, and a scoped one only where it applies", () => {
    ...
    expect(textEditor.actionsForExtension("txt").map((def) => def.id)).toEqual([
    ...
    expect(textEditor.actionsForExtension("").map((def) => def.id)).toEqual([
      "test-plugin.uppercase"
    ]);
  });
...
describe("app.services.textEditor", () => {
  ...
  it("rejects ids that are not namespaced under the plugin id", () => {
    ...
    expect(() => app.services.textEditor.registerLanguage({ ...language, id: "mermaid" })).toThrow(
      /must be prefixed with/
    );
    expect(() =>
      app.services.textEditor.registerPreview({ ...preview, id: "other-plugin.preview" })
    ).toThrow(/must be prefixed with/);
    expect(() => app.services.textEditor.registerAction({ ...action, id: "uppercase" })).toThrow(
      /must be prefixed with/
    );
