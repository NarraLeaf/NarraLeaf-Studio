...
describe("parseKeybinding", () => {
  ...
  it("keeps literal ctrl as the Control key on both platforms", () => {
    ...
    expect(parseKeybinding("ctrl+tab", true)).toMatchObject({
      ctrl: true,
      meta: false,
      key: "tab"
    });
    expect(parseKeybinding("ctrl+tab", false)).toMatchObject({
      ctrl: true,
      meta: false,
      key: "tab"
    });
  ...
  it("stacks mod with other modifiers", () => {
    expect(parseKeybinding("mod+shift+z", true)).toMatchObject({
      meta: true,
      shift: true,
      key: "z"
    });
...
describe("matchesKeybinding", () => {
  ...
  it("rejects extra modifiers so mod+z and mod+shift+z stay distinct", () => {
    const parsed = parseKeybinding("mod+z", true);
    expect(matchesKeybinding(keyEvent({ key: "z", metaKey: true, shiftKey: true }), parsed)).toBe(
      false
    );
  });
});
...
describe("keybindingFromKeyboardEvent", () => {
  it("returns null for a bare modifier press", () => {
    expect(
      keybindingFromKeyboardEvent(keyEvent({ key: "Shift", shiftKey: true }), true)
    ).toBeNull();
