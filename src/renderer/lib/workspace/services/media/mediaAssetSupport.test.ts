...
describe("mediaSupportCheckKind", () => {
    it("probes sound and video, whatever they are called", () => {
        ...
        expect(mediaSupportCheckKind(asset({ type: AssetType.Video, name: "intro.mp4" }))).toBe("probe");
        expect(mediaSupportCheckKind(asset({ type: AssetType.Audio, name: "theme.mp3" }))).toBe("probe");
        expect(mediaSupportCheckKind(asset({ type: AssetType.Video, name: "recording.mkv" }))).toBe("probe");
    });
    ...
    it("checks only the three image formats no browser decodes, by name", () => {
        ...
        expect(mediaSupportCheckKind(asset({ type: AssetType.Image, name: "scan.TIFF" }))).toBe("image");
        expect(mediaSupportCheckKind(asset({ type: AssetType.Image, name: "cursor.xbm" }))).toBe("image");
    ...
    it("reads the extension off `ext` when the name does not carry one", () => {
        expect(mediaSupportCheckKind(asset({ type: AssetType.Image, name: "scan", ext: "tif" }))).toBe("image");
        // ...and does not append it twice when the name already ends with it.
        expect(mediaSupportCheckKind(asset({ type: AssetType.Image, name: "scan.tif", ext: "tif" }))).toBe("image");
    });
...
describe("mediaSupportRecordFromProbe", () => {
    ...
    it("calls a container swap convertible and not lossy", () => {
        const record = mediaSupportRecordFromProbe(probed({
            tier: "remux",
    ...
    it("marks a re-encode lossy, which is the only thing that changes the sentence shown", () => {
        ...
        expect(record?.target).toEqual({ kind: "reencode", container: "webm", video: "vp9", audio: "vorbis" });
    });

    it("has nothing to offer for a file with no streams", () => {
        const record = mediaSupportRecordFromProbe(probed({ tier: "refuse", reason: "no-streams" }, null));
    ...
    it("answers null when the probe answered nothing at all", () => {
        ...
        expect(mediaSupportRecordFromProbe({ status: "unavailable", detail: "", searched: [] })).toBeNull();
        expect(mediaSupportRecordFromProbe({ status: "failed", reason: "timeout", detail: "" })).toBeNull();
    });
});

describe("blocksShipping", () => {
    it("stops the build for both kinds of unplayable file", () => {
        expect(blocksShipping({ state: "convertible", target: { kind: "image", container: "png" }, durationUs: null, lossy: false })).toBe(true);
        expect(blocksShipping({ state: "unplayable", target: null, durationUs: null, lossy: false })).toBe(true);
    });

    it("lets a playable file through", () => {
        expect(blocksShipping({ state: "playable", target: null, durationUs: 1, lossy: false })).toBe(false);
    });
});
...
describe("the cache", () => {
    ...
    it("round-trips what it stored", () => {
        ...
        const parsed = parseMediaSupportCache(JSON.parse(JSON.stringify(serializeMediaSupportCache(entries))));
    ...
    it("forgets hashes the library no longer holds", () => {
        const entries = new Map([["live", record], ["gone", record]]);
