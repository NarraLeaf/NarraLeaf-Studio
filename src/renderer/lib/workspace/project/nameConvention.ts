...
export const ProjectNameConvention = {
   ...
   */
  AssetsGroupsShard: (category: AssetCategory) => [
    "assets",
    `assets.groups.${category}.json` as const
  ],
  /**
   ...
   */
  AssetsOrderShard: (category: AssetCategory) => [
    "assets",
    `assets.order.${category}.json` as const
  ],
  ...
  ProjectIconSource: (slot: string, ext: string) => [
    "resources",
    "icons",
    "source",
    `${slot}.${ext}` as const
  ...
  EditorThumbnailCacheShard: (id: string) => {
    ...
    return [
      ...
      "cache",
      "thumbnail",
      ...splitEncodedPathSegmentId(safeId),
      `${safeId}.png` as const
    ];
  },
  /**
  ...
  EditorPuppetDescriptionCacheShard: (key: string) => [
    ...
    "cache",
    "puppet",
    `${encodePathSegmentId(key)}.json` as const
  ],
  /**
  ...
  EditorStoryDocument: (storyId: string) => [
    "editor",
    "story",
    "stories",
    storyId,
  ...
  EditorStoryAnimationDocument: (animationId: string) => [
    "editor",
    "story",
    "animations",
    `${animationId}.json`
...
function splitEncodedPathSegmentId(id: string): [string, string, string] {
  return [id.slice(0, 2) || "id", id.slice(2, 4) || "id", id.slice(4) || "empty"];
}
