...
export function rankSpeakers(
  ...
  for (const [key, tally] of tallies) {
    rows.push({
      key,
      ...labelSpeaker(tally, characterName),
      lines: tally.lines,
      words: tally.words
    });
  }
...
function countBlueprints(ctx: WorkspaceContext): {
  ...
  for (const blueprint of Object.values(document.blueprints ?? {})) {
    ...
    const groups: Record<string, { graph?: BlueprintGraphIr }>[] = [
      events,
      functions,
      macros ?? {}
    ];
 ...
 */
function readCast(ctx: WorkspaceContext): {
  count: number;
...
async function collectTranslatableRows(
  ...
  const keysDocument =
    localizationService.getKeysIfLoaded() ??
    (await localizationService.loadKeys().catch(() => undefined));
...
async function computeLocalizationStats(
  ...
  for (const locale of config.locales) {
    ...
    try {
      ...
      stats.push({
        locale: locale.code,
        ...localizationService.computeProgress(rows, locale.code)
      });
    } catch {
...
export async function computeProjectStatsSnapshot(
  ctx: WorkspaceContext
  ...
  let uiSurfaces = 0;
  try {
    uiSurfaces = ctx.services.get<UIDocumentService>(Services.UIDocument).getDocument()
      .surfaces.length;
  } catch {
    uiSurfaces = 0;
  }
