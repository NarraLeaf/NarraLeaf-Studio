 ...
 */
export function usePuppetDescription(
  request: PuppetDescriptionRequest | null
): PuppetDescriptionView {
  ...
  useEffect(() => {
    ...
    void service.describe(pending, nonce > 0 ? { refresh: true } : undefined).then((next) => {
      if (!cancelled) {
        setResult(next);
        setLoading(false);
      }
 ...
 */
export function puppetDescriptionRequestFor(
  appearance: CharacterAppearance | undefined
): PuppetDescriptionRequest | null {
 ...
 */
export function puppetDescribeStatusKey(
  reason: PuppetDescriptionUnavailableReason | null | undefined
): TranslationKey {
  switch (reason) {
    case "no-model":
      return "characters.editor.puppet.describeNoModel";
    case "no-backend":
      return "characters.editor.puppet.describeNoBackend";
    case "backend-missing":
      return "characters.editor.puppet.describeBackendMissing";
    case "not-described":
      return "characters.editor.puppet.describeNotSupported";
    case "failed":
      return "characters.editor.puppet.describeFailed";
