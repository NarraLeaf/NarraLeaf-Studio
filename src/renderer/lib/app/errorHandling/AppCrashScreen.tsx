  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setFeedback({
          kind: "bad",
          text: t("crash.screen.exportFailed", { error: result.error ?? "" })
        });
        return;
      }
      ...
      setFeedback({
        kind: "ok",
        text: t("crash.screen.exported", { path: result.data.filePath ?? "" })
      });
    } catch (exportError) {
      setFeedback({
        kind: "bad",
    ...
    <div className="h-screen w-screen flex flex-col bg-surface text-fg">
      ...
      <div className="min-h-0 flex-1 overflow-auto flex items-center justify-center bg-surface p-6">
        <div className="w-full max-w-2xl">
          ...
          {flushMessage && (
            <p
              className={`mt-3 text-xs ${flush === "failed" ? "text-danger" : "text-fg-subtle"}`}
              role="status"
            >
              {flushMessage}
            </p>
