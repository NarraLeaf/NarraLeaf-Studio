...
import { APP_TAG_ID_RELEASE, appTagMechanismKey, RELEASE_APP_TAG, type ProjectAppTag } from "@shared/types/appTag";
...
const line = (id: string, value = "line"): StoryBlock =>
    block({ id, kind: "nodeAction", payload: { action: "narration", text: { textId: `t-${id}`, value, role: "narration" } } });
...
function skeleton(): ReleaseContentStory {
    const scenes = [
        ...
        scene("chapter-two", "Chapter Two", [line("c1"), jump("c-jump", "recollection"), jump("c-end", "ending")]),
...
const WIRED_SCENE_PIN = [{ from: { nodeId: "n-src", port: "value" }, to: { nodeId: "n-1", port: "sceneId" } }];
...
describe("release content scenes", () => {
    it("keeps what the demo can still reach and drops the rest", () => {
        ...
        expect(sceneNames(answer)).toEqual(["Prologue", "Fork", "Path North", "Path South", "Recollection"]);
        expect(answer.removedScenes.map(removed => removed.sceneName))
            .toEqual(["Chapter Two", "Ending", "Orphan"]);
    });
    ...
    it("says which scene is the story's marked entry", () => {
        ...
        expect(answer.members.find(member => member.name === "Prologue")?.provenance)
            .toEqual({ kind: "storyEntryScene" });
    });
    ...
    it("names the node behind a scene a blueprint starts", () => {
        ...
        expect(answer.removedScenes.map(removed => removed.sceneName)).toEqual(["Chapter Two", "Ending"]);
    });
...
describe("release content beyond scenes", () => {
    it("says a surface, a key and a plugin are in because every package has them", () => {
        const answer = solveReleaseContent(input({
            ...
            plugins: [{ id: "narraleaf.gallery", name: "Gallery", runtimeCapabilities: ["store", "events"] }],
        }));

        for (const kind of ["surface", "localizationKey", "plugin"] as const) {
            expect(answer.members.find(member => member.kind === kind)?.provenance)
                .toEqual({ kind: "shipsWithEveryVariant" });
        }
    });
...
function storyReference(assetId: string, sceneId: string, blockId: string): AssetReference {
    return {
        ...
        target: { kind: "storyBlock", storyId: "story-1", sceneId, blockId, storyName: "Skeleton", sceneName: sceneId },
    };
}
...
describe("release content blockers", () => {
    it("stops a build whose Start Story node picks its scene while the game runs", () => {
        const answer = solveReleaseContent(input({
            blueprints: [graphBlueprint(startStory({ storyId: "story-1", sceneId: "prologue" }), WIRED_SCENE_PIN)],
        }));
    ...
    it("stops a build carrying a TypeScript blueprint", () => {
        ...
        expect(answer.blockers.map(blocker => [blocker.reason, blocker.location]))
            .toEqual([["scriptBlueprint", "Launcher"]]);
    });
    ...
    it("takes the author's declaration instead of stopping, and keeps what it names", () => {
        const answer = solveReleaseContent(input({
            blueprints: [graphBlueprint(startStory({ storyId: "story-1", sceneId: "prologue" }), WIRED_SCENE_PIN)],
            projectDeclaredScenes: { [START_STORY_MECHANISM]: [{ storyId: "story-1", sceneId: "ending" }] },
        }));
    ...
    it("reports a declaration naming a scene the project no longer has", () => {
        const answer = solveReleaseContent(input({
            blueprints: [graphBlueprint(startStory({ storyId: "story-1", sceneId: "prologue" }), WIRED_SCENE_PIN)],
            projectDeclaredScenes: { [START_STORY_MECHANISM]: [{ storyId: "story-1", sceneId: "deleted" }] },
        }));
