...
function tokensToRuns(
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (!source) {
      report(
        "unknownRun",
        `run ${token.index} is not in this row's snapshot; the marker was dropped`
      );
      continue;
    }
    if (token.close) {
      report(
        "unknownRun",
        `closing marker for run ${token.index} has no opening marker; it was dropped`
      );
      continue;
    }
    if (!isTextRun(source)) {
    ...
    if (closeAt < 0) {
      report(
        "unknownRun",
        `run ${token.index} is styled text but its closing marker is gone; the marker was dropped`
      );
      continue;
    }
...
export function exportStoryScript(
  ...
  const out: string[] = [
    `${HEADER_DIRECTIVE} ${STORY_SCRIPT_FORMAT_VERSION}`,
    `${STORY_DIRECTIVE} ${document.id}`
  ];
  ...
  for (const sceneId of sceneIds) {
    ...
    for (const { block, depth } of walkSceneWithDepth(scene)) {
      ...
      out.push(
        roundtrip
          ? `${line}${ANCHOR_SEPARATOR}${STORY_SCRIPT_ANCHOR_OPEN}${anchor}${STORY_SCRIPT_ANCHOR_CLOSE}`
          : line
      );
    }
  }
  ...
  switch (shape) {
    ...
    case "dialogue":
      return escapeLineStart(
        `${escapeBoundarySpaces(escapeInline(options.speaker(scene, block.id)))}: ${text}`
      );
    default:
...
const ANCHOR_SUFFIX = new RegExp(
  `${ANCHOR_SEPARATOR}${STORY_SCRIPT_ANCHOR_OPEN}(\\d+)${STORY_SCRIPT_ANCHOR_CLOSE}$`
);
...
export function parseStoryScript(text: string): StoryScriptParseResult {
  ...
  for (let index = 1; index < lines.length; index += 1) {
    ...
    if (raw.startsWith("#")) {
      ...
      if (name === SCENE_DIRECTIVE) {
        ...
        sections.push({
          sceneId,
          name: space < 0 ? "" : rest.slice(space + 1).trim(),
          origin: "",
          lines: []
        });
        continue;
  ...
  if (dataAt < 0) {
    return parseError(
      "dataMissing",
      "the file carries no #data snapshot, so nothing can be restored from it"
    );
  }
  ...
  try {
    ...
    return parseError(
      "dataCorrupt",
      `the #data snapshot is not valid JSON: ${String(error)}`,
      dataAt + 1
    );
  }
...
function parseLine(raw: string, lineNumber: number): ParsedStoryScriptLine {
  ...
  if (
    content.startsWith(STORY_SCRIPT_OPAQUE_PREFIX) ||
    content === STORY_SCRIPT_OPAQUE_PREFIX.trim()
  ) {
    line.shape = "opaque";
    return line;
  }
  ...
  if (
    content.startsWith(STORY_SCRIPT_OPTION_PREFIX) ||
    content === STORY_SCRIPT_OPTION_PREFIX.trim()
  ) {
...
function planScene(
  parsed: ParsedStoryScriptScene,
  input: StoryScriptPlanInput
): StoryScriptScenePlan {
  const diagnostics: StoryScriptDiagnostic[] = [];
  const stats: StoryScriptSceneStats = {
    unchanged: 0,
    edited: 0,
    added: 0,
    removed: 0,
  ...
  return {
    ...
    stale:
      ...
      digestOfText(canonicalSceneJson(live)) !== parsed.origin,
    missing: !live,
    diagnostics
  };
}
...
function mergeLine(line: ParsedStoryScriptLine, context: MergeContext): MergeNode | null {
  ...
  if (line.anchor === undefined) {
    if (line.shape === "opaque") {
      ...
      report(
        "opaqueWithoutAnchor",
        "a » line with no anchor names no action, so no row was created"
      );
      return null;
    }
  ...
  if (!source) {
    report(
      "unknownAnchor",
      `anchor ${line.anchor} is not in this file's snapshot; the line was dropped`
    );
    return null;
  }
  ...
  if (snapshotShape === "opaque" || line.shape === "opaque") {
    ...
    if (duplicate) {
      report(
        "duplicateAnchor",
        `anchor ${line.anchor} appears more than once; this copy is a new row`,
        "warning"
      );
  ...
  if (duplicate) {
    ...
    const clone = applyTextEdit(
      cloneBlock(source, context.generateId),
      source,
      line,
      context,
      () => {}
...
function assembleScene(
  ...
  for (const node of nodes) {
    ...
    if (
      node.origin !== "snapshot" &&
      node.block.kind === "nodeAction" &&
      node.block.payload.action === "choiceOption"
    ) {
...
function countPositions(
  ...
  for (const node of nodes) {
    ...
    const beforeSiblings = before.parentId
      ? (snapshot.blocks[before.parentId]?.childrenIds ?? [])
      : snapshot.rootBlockIds;
    const afterSiblings = after.parentId
      ? (scene.blocks[after.parentId]?.childrenIds ?? [])
      : scene.rootBlockIds;
    const moved =
