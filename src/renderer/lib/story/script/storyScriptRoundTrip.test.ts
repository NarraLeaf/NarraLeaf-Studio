  ...
  for (let index = 0; index < count; index += 1) {
    switch (rng.int(5)) {
      ...
      case 1: {
        ...
        runs.push(
          marks ? { text: rng.pick(TEXT_FRAGMENTS), marks } : { text: rng.pick(TEXT_FRAGMENTS) }
        );
        break;
      }
      case 2:
...
function randomOpaqueBlock(rng: Rng, id: string, newId: () => string): StoryBlock {
  switch (rng.int(12)) {
    ...
    case 3:
      return block(id, "action", {
        action: "setVariable",
        target: { scope: "scene", variableId: "v-scene" },
        value: 3
      });
    case 4:
...
function randomRow(rng: Rng, depth: number, newId: () => string): Built {
  ...
  if (roll === 0) {
    ...
    for (let index = 0; index < 1 + rng.int(3); index += 1) {
      const option = block(newId(), "nodeAction", {
        action: "choiceOption",
        text: randomSegment(rng, "choiceText", newId)
      });
      markDisabled(rng, option);
      options.push({
        block: option,
        children: depth < 2 ? randomRows(rng, depth + 1, newId, 2) : []
      });
    }
  ...
  const row = block(id, "nodeAction", {
    action: "narration",
    text: randomSegment(rng, "narration", newId)
  });
  markDisabled(rng, row);
  return { block: row, children: [] };
}
...
export function randomScene(seed: number): { document: StoryDocument; sceneId: string } {
  ...
  const document: StoryDocument = {
    ...
    name: "Story",
    chapters: [
      { id: "0c0c0c0c-0c0c-4c0c-8c0c-0c0c0c0c0c0c", name: "Chapter", sceneIds: [sceneId] }
    ],
...
export const exportOptions: StoryScriptExportOptions = {
  ...
  speaker: (scene, blockId) => {
    ...
    return target.payload.characterId
      ? nameOf(target.payload.characterId)
...
function editableSegment(target: StoryBlock | undefined): StoryTextSegment | null {
  ...
  if (
    target.kind === "nodeAction" &&
    (target.payload.action === "narration" ||
      target.payload.action === "dialogue" ||
      target.payload.action === "choiceOption")
  ) {
    return target.payload.text;
  }
  return null;
...
function runsOf(segment: StoryTextSegment): StoryRichRun[] {
  return segment.rich && segment.rich.length > 0
    ? normalizeRuns(segment.rich)
    : segment.value
      ? [{ text: segment.value }]
      : [];
}
