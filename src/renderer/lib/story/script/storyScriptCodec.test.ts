...
function makeBlock(id: string, kind: StoryBlock["kind"], payload: unknown, childrenIds: string[] = []): StoryBlock {
...
function fixture(): StoryDocument {
    const blocks: StoryBlock[] = [
        makeBlock(N1, "nodeAction", { action: "narration", text: segment("t1", "早上好", "narration") }),
        makeBlock(D1, "nodeAction", { action: "dialogue", characterId: ALICE, text: segment("t2", "你好", "dialogue") }),
        makeBlock(NT1, "note", { text: segment("t3", "给译者的备注", "note") }),
        makeBlock(A1, "action", { action: "setBackground", assetId: "bg-1", transition: { kind: "dissolve", durationMs: 500 } }),
        makeBlock(C1, "nodeAction", { action: "choice" }, [O1]),
        makeBlock(O1, "nodeAction", { action: "choiceOption", text: segment("t4", "跟他打招呼", "choiceText") }),
    ...
    return {
        ...
        id: STORY_ID,
        name: "Story",
        chapters: [{ id: "ffffffff-ffff-4fff-8fff-fffffffffff1", name: "Chapter", sceneIds: [SCENE_ID] }],
...
const exportOptions: StoryScriptExportOptions = {
    ...
    speaker: (scene, blockId) => {
        ...
        return target.payload.characterId ? CHARACTERS[target.payload.characterId] ?? "" : target.payload.speakerName ?? "";
    },
};
...
describe("story script merge rules", () => {
    ...
    it("gives every copy after the first a fresh id and a fresh textId", () => {
        ...
        const clone = Object.values(plan.scene.blocks).find(target => target.id !== N1 && textOf(target)?.value === "早上好");
    ...
    it("creates a row with fresh UUID v4 ids for an unanchored prose line", () => {
        ...
        const created = Object.values(plan.scene.blocks).find(target => textOf(target)?.value === "作者新写的一行");
...
describe("story script speakers", () => {
    it("rebinds a dialogue row when the label resolves to another character", () => {
        ...
        const after = plan.scene.blocks[D1];
        expect(after.kind === "nodeAction" && after.payload.action === "dialogue" && after.payload.characterId).toBe(SANAE);
    ...
    it("carries an unknown label as a temp speaker and clears the character", () => {
        ...
        const after = plan.scene.blocks[D1];
        expect(after.kind === "nodeAction" && after.payload.action === "dialogue" && after.payload.speakerName).toBe("？？？");
        expect(after.kind === "nodeAction" && after.payload.action === "dialogue" && "characterId" in after.payload).toBe(false);
    });
    ...
    it("reports a speaker change it can see when no resolver was supplied, and ignores it", () => {
        ...
        const after = plan.scene.blocks[D1];
        expect(after.kind === "nodeAction" && after.payload.action === "dialogue" && after.payload.speakerName).toBe("旁白甲");
    ...
    it("leaves a character-bound row alone, silently, when no resolver was supplied", () => {
        ...
        expect(after.kind === "nodeAction" && after.payload.action === "dialogue" && after.payload.characterId).toBe(ALICE);
        expect(textOf(after)?.value).toBe("你好，改过的台词");
        expect(plan.diagnostics).toEqual([]);
    });
});
...
describe("story script staleness", () => {
    ...
    it("reports whether the file belongs to the open story", () => {
        ...
        expect(planStoryScriptImport({ script: parsed.script, live: document, generateId: idFactory() }).storyMatches).toBe(true);
        expect(planStoryScriptImport({ script: parsed.script, live: other, generateId: idFactory() }).storyMatches).toBe(false);
    });
});
