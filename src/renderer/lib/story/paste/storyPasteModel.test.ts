...
function mappings(entries: Record<string, SpeakerMappingTarget>): Record<string, SpeakerMappingTarget> {
    return Object.fromEntries(Object.entries(entries).map(([label, target]) => [speakerMemoryKey(label), target]));
}

function planFor(text: string, entries: Record<string, SpeakerMappingTarget> = {}): PastePlan {
    return planPastedRows({ split: splitPastedText(text, inferPasteSeparator(text)), mappings: mappings(entries) });
}
...
describe("inferPasteSeparator", () => {
    ...
    it("rejects a separator whose labels never repeat, and keeps one whose labels do", () => {
        ...
        const numbered = Array.from({ length: 200 }, (_, index) => `第${index + 1}节：他继续往前走了很久`).join("\n");
        expect(inferPasteSeparator(numbered)).toEqual({ kind: "none" });

        const cast = ["林", "早苗", "老板", "女孩"];
        const conversation = Array.from({ length: 200 }, (_, index) => `${cast[index % cast.length]}：还没到时候`).join("\n");
...
describe("splitPastedText", () => {
    ...
    it("never matches a line with a label but no text", () => {
        ...
        expect(split.lines[0]).toEqual({ index: 0, raw: "label chapter_one:", text: "label chapter_one:" });
    ...
    it("returns an empty split for an empty paste", () => {
        expect(splitPastedText("", { kind: "fullwidthColon" })).toEqual({ lines: [], speakers: [], narrationCount: 0 });
    });
...
describe("planPastedRows", () => {
    ...
    it("resolves a mapped character and collects the ones still to create, once each", () => {
        ...
        expect(plan.rows[2]).toEqual({ kind: "dialogue", text: "他说：「走吧」", characterId: "cccccccc-cccc-4ccc-8ccc-ccccccccccc1" });
        expect(plan.rows[3]).toEqual({ kind: "dialogue", text: "我还没准备好。", speakerName: "早苗", pendingCharacterName: "早苗" });
    });
    ...
    it("uses the tally's spelling, so one mapping decision cannot create two characters", () => {
        ...
        expect(plan.rows.map(row => (row.kind === "dialogue" ? row.pendingCharacterName : null))).toEqual(["Alice", "Alice"]);
    });
...
describe("materializePastedRows", () => {
    it("builds narration and dialogue blocks with fresh UUID v4 ids and one textId each", () => {
        ...
        expect(blocks[0].payload).toEqual({ action: "narration", text: { textId: expect.any(String), role: "narration", value: "第一章 雨" } });
    ...
    it("falls back to the bare name when a pending character was never created", () => {
        const plan = planFor(CHINESE_MANUSCRIPT, { 林: { kind: "createCharacter" } });
        const { blocks } = materializePastedRows(plan, { generateId: makeIdFactory(), createdCharacterIds: {} });
        expect(blocks[2].payload).toEqual({
            action: "dialogue",
            speakerName: "林",
            text: { textId: expect.any(String), role: "dialogue", value: "他说：「走吧」" },
...
describe("routeStoryPaste", () => {
    ...
    it("prefers Studio's own block payload over the text beside it", () => {
        expect(routeStoryPaste({ storyBlocksPayload: blocksPayload, plainText: CHINESE_MANUSCRIPT, plainRequested: true })).toEqual({ kind: "blocks" });
    });

    it("ignores a payload that is absent, unparseable or empty", () => {
        for (const payload of ["", "{not json", JSON.stringify({ kind: "narraleaf.story.actions", roots: [] })]) {
            expect(routeStoryPaste({ storyBlocksPayload: payload, plainText: CHINESE_MANUSCRIPT, plainRequested: false })).toEqual({
                kind: "wizard",
    ...
    it("sends a Story Script file to Import Script, ahead of both the single-line and wizard paths", () => {
        ...
        expect(routeStoryPaste({ storyBlocksPayload: "", plainText: file, plainRequested: false })).toEqual({ kind: "scriptFile" });
        expect(routeStoryPaste({ storyBlocksPayload: "", plainText: "#nlscript v1", plainRequested: false })).toEqual({ kind: "scriptFile" });
    });

    it("takes a single line straight in, wizard or not", () => {
        expect(routeStoryPaste({ storyBlocksPayload: "", plainText: "  林：走吧。  \n", plainRequested: false })).toEqual({ kind: "single" });
        expect(routeStoryPaste({ storyBlocksPayload: "", plainText: "", plainRequested: false })).toEqual({ kind: "single" });
    });

    it("honours an explicit plain paste, and otherwise opens the wizard", () => {
        expect(routeStoryPaste({ storyBlocksPayload: "", plainText: CHINESE_MANUSCRIPT, plainRequested: true })).toEqual({
            kind: "plain",
            text: CHINESE_MANUSCRIPT,
        });
        expect(routeStoryPaste({ storyBlocksPayload: "", plainText: CHINESE_MANUSCRIPT, plainRequested: false })).toEqual({
            kind: "wizard",
            text: CHINESE_MANUSCRIPT,
        });
    });
