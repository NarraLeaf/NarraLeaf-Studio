 ...
 */
export type StoryVariableNameLookups = Pick<
  StoryRowLookups,
  "scene" | "scenes" | "projectVariableName"
>;
...
export function getStorySceneName(
...
export function getStoryContainerHeaderInfo(block: StoryBlock): StoryContainerHeaderInfo | null {
  if (block.kind === "control") {
    ...
    if (payload.control === "condition") {
      ...
      return {
        pill: translate("story.containerHeader.condition"),
        commandId: "if",
        role: "condition",
        hasCondition: false
      };
    ...
    if (payload.control === "repeat") {
      ...
      return {
        pill: translate("story.containerHeader.repeat"),
        commandId: "repeat",
        role: "group",
        hasCondition: false,
        repeatTimes: payload.times ?? 1
      };
    }
    if (payload.control === "parallel") {
      return {
        pill: translate("story.containerHeader.parallel"),
        commandId: "parallel",
        role: "group",
        hasCondition: false
      };
    ...
    if (payload.control === "race") {
      return {
        pill: translate("story.containerHeader.race"),
        commandId: "race",
        role: "group",
        hasCondition: false
      };
    ...
    return {
      pill: translate("story.containerHeader.sequence"),
      commandId: "sequence",
      role: "group",
      hasCondition: false
    };
  ...
  if (block.kind === "action" && block.payload.action === "nvl") {
    return {
      pill: translate("story.containerHeader.nvl"),
      commandId: "nvl",
      role: "nvl",
      hasCondition: false
    };
  ...
  if (block.kind === "nodeAction" && block.payload.action === "choice") {
    return {
      pill: translate("story.containerHeader.menu"),
      commandId: "menu",
      role: "menu",
      hasCondition: false
    };
 ...
 */
export function storyContainerChain(
  scene: StoryScene,
  blockId: StoryBlockId
): { blockId: StoryBlockId; info: StoryContainerHeaderInfo }[] {
...
export function storyBlockBadge(block: StoryBlock): StoryBlockBadge {
  if (block.kind === "nodeAction") {
    if (block.payload.action === "narration")
      return badge("narration", "story.badge.narration", "character");
    if (block.payload.action === "dialogue")
      return badge("dialogue", "story.badge.dialogue", "character");
  ...
  if (block.kind === "action") {
    if (block.payload.action === "setBackground")
      return badge("background", "story.badge.background", "scene");
    if (block.payload.action === "character")
      return badge("character", "story.badge.character", "character");
    if (block.payload.action === "audio") return badge("audio", "story.badge.audio", "sound");
    if (block.payload.action === "setVariable")
      return badge("variable", "story.badge.variable", "data");
    ...
    if (block.payload.action === "displayable") {
      if (block.payload.operation === "transform")
        return badge("transform", "story.badge.transform", "image");
    ...
    if (block.payload.action === "blueprint")
      return badge("blueprint", "story.badge.blueprint", "utils");
  ...
  if (block.kind === "declaration") {
    return badge(
      "declaration",
      `story.badge.declare.${block.payload.scope}` as TranslationKey,
      "data"
    );
  }
 ...
 */
export function variableRefShortLabel(
  ...
  if (ref.scope === "persistent") {
    ...
    return (
      lookups.projectVariableName?.("persistent", ref.variableId) ??
  ...
  return (
    (ref.scope === "saved" ? lookups.projectVariableName?.("saved", ref.variableId) : null) ??
...
export function storyConditionSummary(
  ...
  const operator = translate(
    `story.condition.op${conditionOperatorSuffix(condition.operator)}` as TranslationKey
  );
  const suffix =
...
function conditionOperatorSuffix(
...
function describeAssignment(
  ...
  const readsTarget = (node: StoryExpr) =>
    node.kind === "var" && storyVariableRefKey(node.target) === targetKey;
 ...
 */
const CAMERA_PAN_PLACEMENTS: Record<number, StagePlacement> = (
 ...
 */
export function storyCameraPanPlacement(
  position: Extract<StoryActionPayload, { action: "camera" }>["position"]
): StagePlacement | null {
...
export function describeStoryBlock(block: StoryBlock, lookups: StoryRowLookups): string {
  ...
  if (block.kind === "nodeAction") {
    ...
    if (payload.action === "narration")
      return payload.text.value || translate("story.describe.narration");
    if (payload.action === "dialogue")
      return `${storyCharacterName(lookups, payload.characterId)}: ${payload.text.value || translate("story.describe.dialogue")}`;
    if (payload.action === "choice")
      return `${translate("story.describe.choice")}${payload.prompt?.value ? ` - ${payload.prompt.value}` : ""}`;
  ...
  if (block.kind === "action") {
    ...
    if (payload.action === "setBackground") {
      const named = payload.assetId
        ? lookups.assetName
          ? (lookups.assetName(payload.assetId) ?? translate("story.background.missingImage"))
          : payload.assetId
        : null;
    ...
    if (payload.action === "character") {
      const name = payload.characterId
        ? storyCharacterName(lookups, payload.characterId)
        : payload.objectName || translate("story.describe.characterFallback");
      ...
      const operation = verbWord(
        payload,
        translate(`story.describe.charOp.${payload.operation}` as TranslationKey)
      );
      ...
      if (
        ...
        (payload.operation === "expression" && payload.puppetName !== undefined)
      ) {
    ...
    if (payload.action === "audio") {
      const named =
        payload.assetId && lookups.assetName
    ...
    if (payload.action === "setVariable")
      return describeAssignment(payload, variableRefShortLabel(payload.target, lookups));
    if (payload.action === "wait")
      return payload.mode === "duration"
        ? translate("story.describe.waitDuration", {
            seconds: storyMsToSeconds(payload.durationMs ?? 0)
          })
        : translate("story.describe.waitClick");
    if (payload.action === "image")
      return translate("story.describe.image", {
        operation: verbWord(payload, payload.operation),
        name: payload.objectName || translate("story.describe.unnamed")
      });
    if (payload.action === "displayable")
      return `${verbWord(payload, payload.operation)} ${resolveDisplayableTargetRef(scene, payload.target).label || translate("story.describe.targetFallback")}`;
    if (payload.action === "text")
      return translate("story.describe.text", {
        operation: verbWord(payload, payload.operation),
        name: payload.objectName || translate("story.describe.unnamed")
      });
    if (payload.action === "layer") {
      const layerName =
        payload.operation === "create"
          ...
          : resolveStoryLayerRef(scene, layerActionTargetRef(payload.target, payload.objectName))
              .name || translate("story.describe.unnamed");
      return translate("story.describe.layer", {
        operation: verbWord(payload, payload.operation),
        name: layerName
      });
    }
    if (payload.action === "video")
      return translate("story.describe.video", {
        operation: verbWord(payload, payload.operation),
        name: payload.objectName || translate("story.describe.unnamed")
      });
    if (payload.action === "vfx")
      return translate("story.describe.vfx", {
        operation: verbWord(payload, payload.operation),
        name: payload.objectName || translate("story.describe.unnamed")
      });
  ...
  if (block.kind === "control") {
    ...
    if (block.payload.control === "conditionBranch")
      return translate("story.describe.branch", { branch: block.payload.branch });
    ...
    if (block.payload.control === "label")
      return translate("story.describe.label", {
        name: block.payload.name || translate("story.describe.unnamed")
      });
    if (block.payload.control === "goto")
      return translate("story.describe.goto", {
        name: block.payload.targetLabel || translate("story.describe.unnamed")
      });
  ...
  if (block.kind === "jump") {
    return translate("story.describe.jump", {
      scene: getStorySceneName(scenes, block.payload.targetSceneId)
    });
  }
...
export type StoryRowLabel = (
  key: "story.quickParam.jumpLabel" | "story.quickParam.waitLabel"
) => string;
...
export function storyActionRowFragments(
  ...
  const valueInToken =
    params.length > 0 &&
    ((block.kind === "action" && block.payload.action === "wait") || block.kind === "jump");
...
export function storyRowFragments(
  ...
  if (container) {
    ...
    if (container.repeatTimes !== undefined) {
      fragments.push({
        kind: "text",
        text: `${container.repeatTimes} ${translate("story.repeat.times")}`
      });
    }
  ...
  if (segment) {
    const placeholder =
      options.editingPlaceholders === false ? "" : getStoryEmptyTextPlaceholder(block);
    fragments.push({
      kind: "text",
...
export function storyRowSpeaker(
  block: StoryBlock,
  lookups: StoryRowLookups
): StoryRowCharacter | null {
 ...
 * already reads there.
 */
export function storyRowSentence(
  block: StoryBlock,
  lookups: StoryRowLookups,
  options: StoryRowOptions = {}
): string {
  ...
  return storyRowFragments(block, lookups, defaultRowLabel, options)
    .map((fragment) =>
      fragment.kind === "text" ? fragment.text : quickParamText(fragment.param, sceneName)
    )
...
export function projectStoryRow(
  block: StoryBlock,
  lookups: StoryRowLookups,
  options: StoryRowOptions = {}
): StoryRowProjection {
