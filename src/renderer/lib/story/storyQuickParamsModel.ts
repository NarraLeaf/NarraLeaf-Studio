...
function durationParam(
  ...
  label: string,
  ms: number,
  presetsMs: number[] | undefined,
  apply: (ms: number) => StoryBlock["payload"]
): QuickParam {
  return {
    id,
    label,
...
export function getQuickParams(block: StoryBlock): QuickParam[] {
    ...
    return [
      {
        ...
        apply: (next) =>
          next.kind === "scene"
            ? { ...payload, targetSceneId: next.sceneId ?? payload.targetSceneId }
            : payload
      }
    ];
  }
  ...
  if (payload.action === "wait") {
    ...
    return [
      durationParam("duration", "", payload.durationMs ?? 0, WAIT_PRESETS_MS, (ms) => ({
        ...payload,
        mode: "duration",
        durationMs: ms
      }))
  ...
  if (payload.action === "setBackground") {
    ...
    return [
      durationParam("d", "d", transition.durationMs ?? 0, undefined, (ms) => ({
        ...payload,
        transition: { ...transition, durationMs: ms }
      }))
    ];
  }
  if (
    payload.action === "character" &&
    (payload.operation === "enter" || payload.operation === "exit")
  ) {
    ...
    return [
      durationParam("d", "d", transform.durationMs ?? 0, undefined, (ms) => ({
        ...payload,
        transform: { ...transform, durationMs: ms }
      }))
    ];
  }
  if (payload.action === "camera") {
    ...
    return payload.operation === "motion"
      ? []
      : [
          durationParam("d", "d", payload.durationMs ?? 0, undefined, (ms) => ({
            ...payload,
            durationMs: ms
          }))
        ];
