...
describe("computeScreenRatioFrame", () => {
  ...
  it("returns null for degenerate design sizes", () => {
    ...
    expect(
      computeScreenRatioFrame({ designSize: { width: -1920, height: 1080 }, preset })
    ).toBeNull();
    expect(
...
describe("safe-area preset table", () => {
  it("is a real device list, every entry consistent across its two orientations", () => {
    ...
    expect(new Set(SAFE_AREA_PRESETS.map((p) => p.family))).toEqual(
      new Set(["iphone", "ipad", "android"])
    );
  ...
  it("carries the sourced iOS numbers, with mirrored landscape side insets", () => {
    ...
    for (const preset of SAFE_AREA_PRESETS.filter(
      (p) => p.family === "iphone" && p.portrait.insets.top > 0
    )) {
  ...
  it("carries the AOSP cutout shapes, each with the overlay's own numbers", () => {
    ...
    for (const preset of SAFE_AREA_PRESETS.filter(
      (p) => p.family === "android" && p.id !== "android-cutout-double"
    )) {
    ...
    const shapes = SAFE_AREA_PRESETS.filter((p) => p.family === "android").map((p) =>
      JSON.stringify([p.portrait.insets, p.landscape.insets])
    );
  ...
  it("takes the orientation from the project setting, not from the design size", () => {
    ...
    expect(computeSafeAreaFrame({ designSize: DESIGN_1920x1080, preset })!.orientation).toBe(
      "landscape"
    );
    // `auto` locks nothing, so there is no answer to read and the design size decides.
    expect(
      computeSafeAreaFrame({ designSize: DESIGN_1920x1080, preset, mobileOrientation: "auto" })!
        .orientation
    ).toBe("landscape");
    expect(resolveSafeAreaOrientation({ width: 1080, height: 1920 }, "auto")).toBe("portrait");
    expect(resolveSafeAreaOrientation({ width: 1080, height: 1920 }, "landscape")).toBe(
      "landscape"
    );
  });
...
describe("orientation selection", () => {
  ...
  it("reads the matching orientation out of the preset", () => {
    ...
    const tall = computeSafeAreaFrame({
      designSize: { width: 1080, height: 2340 },
      preset: island
    })!;
...
describe("computeSafeAreaFrame — math", () => {
  ...
  it("returns null for degenerate inputs", () => {
    ...
    expect(
      computeSafeAreaFrame({ designSize: { width: 0, height: 1080 }, preset: device })
    ).toBeNull();
    expect(
...
describe("stage fit: cover", () => {
  it("inscribes the screen rect in the design rect instead of wrapping it", () => {
    ...
    const contain = computeScreenRatioFrame({
      designSize: DESIGN_1920x1080,
      preset: aspect("21:9")
    })!;
  ...
  it("puts the strips inside the design under cover and outside it under contain", () => {
    for (const stageFit of ["contain", "cover"] as const) {
      ...
      const inside = strips.every(
        (s) => s.x >= 0 && s.y >= 0 && s.x + s.width <= 1920 && s.y + s.height <= 1080
      );
  ...
  it("costs MORE safe area than contain — there is no bar left to absorb the inset", () => {
    ...
    const cover = computeSafeAreaFrame({
      designSize: DESIGN_1920x1080,
      preset,
      stageFit: "cover"
    })!;
...
describe("computeUnsafeBands", () => {
  it("covers only the home-indicator strip for a 16:9 design on a notched iPhone", () => {
    ...
    const iphones = SAFE_AREA_PRESETS.filter(
      (p) => p.family === "iphone" && p.portrait.insets.top > 0
    );
  ...
  it("clamps insets that swallow the whole design rather than inverting geometry", () => {
    const frame = computeSafeAreaFrameForGeometry({
      ...
      geometry: {
        ...
        insets: { left: 0, right: 0, top: 400, bottom: 0 }
      },
