...
} from "@/lib/ui-editor/snapping/types";
...
function hasResizeAxisCrossed(
  initialSignedSize: number | undefined,
  currentSignedSize: number | undefined
) {
...
export function resolveGroupDragTranslate(
  ...
  return childBeforeTranslate
    ? coerceTranslate(childBeforeTranslate)
    : coerceTranslate(groupBeforeTranslate);
}
...
export function useMoveableHandlers({
  ...
  const finalizeResize = useCallback(() => {
    ...
    selectedTargets.forEach((target) => {
      ...
      if (
        element?.type === "nl.image" &&
        (cached?.width !== undefined || cached?.height !== undefined)
      ) {
    ...
    (e: OnResizeGroup) => {
      ...
      e.events.forEach((event) => {
        ...
        target.style.transform = isFlowChild
          ...
          : (event.drag?.transform ??
            buildTransform(translateX, translateY, initialLayout?.rotation));
        resizeCache.current.set(elementId, {
          width: event.width,
          height: event.height,
