...
export function ensureNormalizedLayout(
  elementId: string,
  layout: UILayout,
  ...
  documentService.updateElementLayout(
    elementId,
    {
      x: normalized.x,
      y: normalized.y,
...
export function computeResizeAxis1D(
  direction: number,
  startSize: number,
  delta: number
): ResizeAxisResult {
...
export function computeResizePreview(
  ...
  const { translateX, translateY } = computeResizeTranslate(
    layout,
    xAxis,
    yAxis,
    xAxis.size,
