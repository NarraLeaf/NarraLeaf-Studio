...
export function useUIEditorKeybindings(params: UseUIEditorKeybindingsParams): void {
  ...
  const keybindings = useMemo<KeybindingDefinition[]>(() => {
    ...
    const bindMod = (
      mod: "ctrl" | "meta",
      defs: Array<{ suffix: string; key: string; handler: () => void }>
    ) =>
      defs.map((d) => ({
        id: `${d.suffix}-${mod}`,
        key: `${mod}+${d.key}`,
  ...
  useKeybindings({
    ...
    enabled:
      enabled &&
      Boolean(surfaceId && documentService && localBlueprint && historyService && stateService),
    when: and(
      whenEditorFocused(tabId),
      fromGetter(() => !isTypingInField())
    ),
