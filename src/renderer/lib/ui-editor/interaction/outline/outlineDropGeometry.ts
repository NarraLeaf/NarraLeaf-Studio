...
export function resolveBeforeChildIdForOutlineGap(
  ...
  const moverSet = new Set(movers);
  const visualWithoutMovers = getOutlineVisualChildren(targetParent).filter(
    (id) => !moverSet.has(id)
  );
  ...
  return documentInsertIndex >= documentChildren.length
    ? null
    : (documentChildren[documentInsertIndex] ?? null);
}
