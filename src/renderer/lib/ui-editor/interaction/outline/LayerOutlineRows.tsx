...
export function isOutlineGapDropData(value: unknown): value is OutlineGapDropData {
  ...
  return (
    data.kind === "outline-gap" &&
    typeof data.parentId === "string" &&
    typeof data.visualIndex === "number"
  );
}
      ...
      >
        ...
        <button
          ...
          className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-fg-subtle transition hover:bg-fill-subtle hover:text-fg disabled:cursor-not-allowed disabled:opacity-25 disabled:hover:bg-transparent disabled:hover:text-fg-subtle ${
            visible
              ? "opacity-0 group-hover/outline-row:opacity-100 group-focus-within/outline-row:opacity-100"
              : "opacity-100"
          }`}
...
export function OutlineGapDropZone({
  ...
  const expandedHeight = terminalChildDrop
    ? OUTLINE_TERMINAL_GAP_HEIGHT
    : OUTLINE_ACTIVE_GAP_HEIGHT;
