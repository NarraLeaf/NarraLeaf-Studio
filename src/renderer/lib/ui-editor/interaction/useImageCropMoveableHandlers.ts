...
function isDefaultCropPlacement(placement: ImageFillCropPlacement): boolean {
  return (
    Math.abs(placement.leftPct - DEFAULT_RECTANGLE_CROP_PLACEMENT.leftPct) <
      CROP_PLACEMENT_EPSILON &&
    Math.abs(placement.topPct - DEFAULT_RECTANGLE_CROP_PLACEMENT.topPct) < CROP_PLACEMENT_EPSILON &&
    Math.abs(placement.widthPct - DEFAULT_RECTANGLE_CROP_PLACEMENT.widthPct) <
      CROP_PLACEMENT_EPSILON &&
    Math.abs(placement.heightPct - DEFAULT_RECTANGLE_CROP_PLACEMENT.heightPct) <
      CROP_PLACEMENT_EPSILON
  );
}
...
export function useImageCropMoveableHandlers(config: ImageCropHandlersConfig) {
  ...
  const updatePlacement = useCallback(
    (
      ...
      const prev =
        gestureBaseRef.current ?? resolveInitialCropPlacement(container, imageTarget, prevFill);
