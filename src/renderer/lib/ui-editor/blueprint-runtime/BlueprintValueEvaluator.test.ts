...
describe("Blueprint Value evaluator", () => {
    ...
    const graph: BlueprintGraphIr = {
      nodes: {
        ...
        get: {
          ...
          params: { variableId: surfaceVariableRef }
        },
        ret: { id: "ret", type: BLUEPRINT_NODE_TYPE_DATA_RETURN_VALUE, params: {} }
      },
      edges: [
  ...
  it("tracks Element Text property dependencies while resolving return data pins", async () => {
    const graph: BlueprintGraphIr = {
      ...
      edges: [
        { from: { nodeId: "head", port: "then" }, to: { nodeId: "ret", port: "in" } },
        {
          from: { nodeId: "element", port: "element" },
          to: { nodeId: "getText", port: "element" }
        },
  ...
  it("tracks Slider mapped value dependencies while resolving return data pins", async () => {
    const graph: BlueprintGraphIr = {
      ...
      edges: [
        ...
        {
          from: { nodeId: "element", port: "element" },
          to: { nodeId: "getValue", port: "slider" }
        },
    ...
    await expect(
      evalValue(valueDocument(graph), hostAdapter("Text", undefined, 42))
    ).resolves.toEqual({
      returned: true,
      value: 42,
  ...
  it("allows Call Fn in value graphs and returns the fn result", async () => {
    ...
    adapter.blueprintRuntime!.invokeBlueprintFn = async () => ({
      returns: { ret_1_value: "from-fn" }
    });
  ...
  it("blocks effectful element write nodes before they can run", async () => {
    ...
    ).rejects.toThrow(/not allowed in Blueprint Value/);
