...
describe("Get Component Param", () => {
  ...
  it("reads as empty with no instance, no chosen param, or a param that no longer exists", async () => {
    ...
    expect(
      await dispatchAndReadLoggedParam({ paramId: "", componentParams: { saveId: "7" } })
    ).toBe("");
    expect(
      await dispatchAndReadLoggedParam({ paramId: "removed", componentParams: { saveId: "7" } })
    ).toBe("");
  });
});
