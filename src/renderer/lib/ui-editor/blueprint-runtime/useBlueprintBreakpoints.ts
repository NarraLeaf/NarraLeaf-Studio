...
export function useBlueprintBreakpoints(
  projectPath: string | null | undefined
): UseBlueprintBreakpointsResult {
  const stateKey = useMemo(
    () => (projectPath ? blueprintBreakpointsStateKey(projectPath) : null),
    [projectPath]
  );
  ...
  const update = useCallback(
    (
      target: BlueprintBreakpointTarget,
      apply: (existing: BlueprintBreakpoint | undefined) => BlueprintBreakpoint | null
    ) => {
