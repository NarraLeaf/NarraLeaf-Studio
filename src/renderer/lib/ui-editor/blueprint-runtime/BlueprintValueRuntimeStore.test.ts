...
} from "./BlueprintValueRuntimeStore";
...
async function resolveSwitchChecked(input: {
  ...
  await waitFor(() => {
    expect(store.getResolvedValue("surface", "switch", "checked", "bp-switch").hasResolved).toBe(
      true
    );
  });
  return {
    resolved: store.getResolvedValue("surface", "switch", "checked", "bp-switch").value,
    merged: mergeElementWithBlueprintValues(input.document.elements.switch!, "surface", store).props
      ?.checked
  };
}
...
describe("BlueprintValueRuntimeStore", () => {
  ...
  it("keeps one pending rerun while evaluation is in flight", async () => {
    ...
    store.sync({
      ...
      blueprintDocument: blueprintDocument(
        literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_STRING, "first")
      ),
      hostAdapter
    });
    store.sync({
      ...
      persistentVariables: {},
      blueprintDocument: blueprintDocument(
        literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_STRING, "second")
      ),
      hostAdapter
    });
    store.sync({
      ...
      persistentVariables: {},
      blueprintDocument: blueprintDocument(
        literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_STRING, "third")
      ),
      hostAdapter
    });
  ...
  it("merges resolved button labels", async () => {
    ...
    await waitFor(() => {
      expect(store.getResolvedValue("surface", "button", "label", "bp-button").hasResolved).toBe(
        true
      );
    });
  ...
  it("merges resolved Page params as JSON objects", async () => {
    ...
    await waitFor(() => {
      expect(store.getResolvedValue("surface", "frame", "params", "bp-frame").hasResolved).toBe(
        true
      );
    });
  ...
  it("merges resolved Slider values as clamped and snapped floats", async () => {
    ...
    await waitFor(() => {
      expect(store.getResolvedValue("surface", "slider", "value", "bp-slider").hasResolved).toBe(
        true
      );
    });
  ...
  it.each([
    {
      name: "a boolean literal true",
      graph: literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_BOOLEAN, true),
      expected: true
    },
    {
      name: "a boolean literal false",
      graph: literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_BOOLEAN, false),
      expected: false
    },
    {
      name: 'the string "true"',
      graph: literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_STRING, "true"),
      expected: true
    },
    {
      name: 'the string "false"',
      graph: literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_STRING, "false"),
      expected: false
    },
    {
      name: "the number 1",
      graph: literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_FLOAT, 1),
      expected: true
    },
    {
      name: "a JSON object",
      graph: literalInitGraph(BLUEPRINT_NODE_TYPE_LITERAL_JSON, { enabled: true }),
      expected: false
    },
