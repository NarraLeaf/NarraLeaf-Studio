  ...
  for (const el of elements) {
    ...
    if (visible === false) {
      out.push({
        ...
        severity: "warning",
        source: "interaction",
        message: translate("blueprint.diagnostics.interaction.hiddenEvents", {
    ...
    if (visible !== false && op <= 0.01) {
      out.push({
        ...
        severity: "warning",
        source: "interaction",
        message: translate("blueprint.diagnostics.interaction.opaqueEvents", {
          name: el.name ?? el.type
        }),
    ...
    if (
      !isUIElementFlowLayoutChild(document, el) &&
      width * height > 0 &&
      width * height < MIN_HIT_AREA
    ) {
      out.push({
        id: `ix:small-hit:${el.id}`,
        severity: "warning",
        source: "interaction",
        message: translate("blueprint.diagnostics.interaction.smallHit", {
          name: el.name ?? el.type
        }),
