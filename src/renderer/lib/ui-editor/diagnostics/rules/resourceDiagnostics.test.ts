...
describe("collectResourceDiagnostics", () => {
  ...
  it("names both halves a puppet widget can be missing", () => {
    ...
    expect(collectResourceDiagnostics([puppet("p2", { assetId: "m" })]).map((d) => d.id)).toEqual([
      "res:puppet-backend:p2"
    ]);
    expect(collectResourceDiagnostics([puppet("p3", { backend: "b" })]).map((d) => d.id)).toEqual([
      "res:puppet:p3"
    ]);
  ...
  it("reports the WebGL context budget rather than truncating silently", () => {
    ...
    const configured = Array.from({ length: SURFACE_PUPPET_CONTEXT_BUDGET + 1 }, (_, index) =>
      puppet(`p${index}`, { assetId: "m", backend: "b" })
    );
    ...
    expect(collectResourceDiagnostics(configured)).toEqual([
      expect.objectContaining({
        ...
        severity: "warning",
        message:
          `This Surface draws ${SURFACE_PUPPET_CONTEXT_BUDGET + 1} models at once; ` +
  ...
  it("counts only the puppets that would actually draw against the budget", () => {
    ...
    const unconfigured = Array.from({ length: SURFACE_PUPPET_CONTEXT_BUDGET + 4 }, (_, index) =>
      puppet(`p${index}`, { backend: "b" })
    );

    expect(collectResourceDiagnostics(unconfigured).map((d) => d.id)).not.toContain(
      "res:puppet-context-budget"
    );
  });
});
