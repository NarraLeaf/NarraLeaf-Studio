...
function normalizeDurationInput(value: number): number {
  return (
    Math.round(Math.min(MAX_DURATION_SECONDS, Math.max(MIN_DURATION_SECONDS, value)) * 100) / 100
  );
}
...
function AnimationPhaseBlock({
  ...
  const directionOptions = useMemo(
    () => [
      {
        id: "left",
        label: t("widgetChrome.pageAnimation.directionLeft"),
        icon: <ArrowLeft className="h-4 w-4" />
      },
      {
        id: "right",
        label: t("widgetChrome.pageAnimation.directionRight"),
        icon: <ArrowRight className="h-4 w-4" />
      },
      {
        id: "up",
        label: t("widgetChrome.pageAnimation.directionUp"),
        icon: <ArrowUp className="h-4 w-4" />
      },
      {
        id: "down",
        label: t("widgetChrome.pageAnimation.directionDown"),
        icon: <ArrowDown className="h-4 w-4" />
      },
      {
        id: "angle",
        label: t("widgetChrome.pageAnimation.angle"),
    ...
    <div className="space-y-2 rounded-md border border-edge bg-fill-subtle p-2">
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_92px] gap-2">
        <label className="min-w-0 space-y-1">
          <span className="block text-2xs font-medium tracking-wide text-fg-subtle">
            {t("widgetChrome.pageAnimation.preset")}
          </span>
          <Select
            fullWidth
        ...
        <label className="min-w-0 space-y-1">
          <span className="block text-2xs font-medium tracking-wide text-fg-subtle">
            {t("widgetChrome.pageAnimation.seconds")}
          </span>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_76px] gap-2">
        <div className="min-w-0 space-y-1">
          <span className="block text-2xs font-medium tracking-wide text-fg-subtle">
            {t("widgetChrome.pageAnimation.direction")}
          </span>
          <IconButtonSegGroup
        ...
        <label className="min-w-0 space-y-1">
          <span className="block text-2xs font-medium tracking-wide text-fg-subtle">
            {t("widgetChrome.pageAnimation.angle")}
          </span>
          <NumericDraftEnhancedInput
    ...
    <div className="space-y-2 rounded-md border border-edge bg-fill-subtle p-2">
      <div className="flex min-h-7 items-center justify-between gap-2">
        ...
        <label className="flex min-w-0 items-center gap-2 text-2xs text-fg-muted">
          ...
          <Switch checked={waitsForChildren} size="sm" onCheckedChange={onWaitsForChildrenChange} />
        </label>
      </div>
      <label className="grid grid-cols-[minmax(0,1fr)_92px] items-center gap-2">
        <span className="min-w-0 text-2xs font-medium tracking-wide text-fg-subtle">
...
export function PageAnimationEditor({
  ...
  const effectiveSettings = inherited
    ...
    : ownSettings;
  const [preview, setPreview] = useState<{ phase: "enter" | "exit"; tick: number }>({
    phase: "enter",
    tick: 0
  });
