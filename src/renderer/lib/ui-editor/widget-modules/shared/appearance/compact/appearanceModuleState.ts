...
export type ContainerAppearanceModuleId =
  | "background"
  | "stroke"
  | "corners"
  | "transform"
  | "effects";
export type ButtonAppearanceModuleId =
  | "background"
  | "border"
  | "spacing"
  | "transform"
...
export const CONTAINER_MODULE_KEYS: Record<
  ContainerAppearanceModuleId,
  readonly ContainerAppearancePropertyKey[]
> = {
  background: [
    "fillType",
    "backgroundColor",
...
export const BUTTON_MODULE_KEYS: Record<
  ButtonAppearanceModuleId,
  readonly ButtonAppearancePropertyKey[]
> = {
  background: [
    "fillType",
    "backgroundColor",
...
export const TEXT_MODULE_KEYS: Record<
  TextAppearanceModuleId,
  readonly TextAppearancePropertyKey[]
> = {
...
export function isExclusiveTrueState(
  ...
  const keys = Object.keys(conditions).filter(
    (k) => conditions[k as keyof AppearanceSystemCondition] !== undefined
  );
  if (keys.length !== 1) {
    return false;
  }
