...
export function CompactContainerAppearance({
  ...
  useEffect(() => {
    (["background", "stroke", "corners", "transform", "effects"] as const).forEach((mid) => {
      ...
      if (
        m !== "default" &&
        !moduleFullyHasExclusiveState(variant, CONTAINER_MODULE_KEYS[mid], m)
      ) {
  ...
  const getStroke = (key: ContainerAppearancePropertyKey) =>
    getRowValueForModuleEdit(variant, key, strokeMode);
  const getCorners = (key: ContainerAppearancePropertyKey) =>
    getRowValueForModuleEdit(variant, key, cornersMode);
  ...
  const patchTransform = (key: ContainerAppearancePropertyKey, value: AppearanceRowValue) => {
    commitVariant(
      updateRowValueForModuleEditOrEnsure(
        variant,
        CONTAINER_MODULE_KEYS.transform,
        key,
        transformMode,
        value
      )
  ...
  const uniformDisplay = showUniformPlaceholder
    ? ""
    : String(cornerAdvanced ? brTL : borderRadiusUniform);
  const uniformPlaceholder = showUniformPlaceholder ? "-" : undefined;

  const toggleCornerAdvanced = () => {
    const next = !cornerAdvanced;
    let v = variant;
    v = updateRowValueForModuleEditOrEnsure(
      v,
      CONTAINER_MODULE_KEYS.corners,
      "cornerAdvanced",
      cornersMode,
    ...
    <div className="space-y-3 min-w-0">
      ...
      >
        <BorderStrokeCompactRows
          ...
          onBorderColorChange={(next: ColorValue) =>
            patchStroke("borderColor", serializeColorValue(next))
          }
            ...
            <div className="flex items-center gap-1 min-w-0">
              <NumericDraftEnhancedInput
                ...
                onFiniteNumber={(radius) => {
                  ...
                  commitVariant(
                    patchManyInModule(variant, CONTAINER_MODULE_KEYS.corners, cornersMode, patch)
                  );
                }}
                inputMode="numeric"
                type="number"
        ...
        {cornerAdvanced && (
          <>
            <div className="flex gap-2 flex-nowrap mt-2">
              ...
              >
                <span className="text-2xs font-medium px-1">
                  {t("widgetAppearance.corners.link")}
                </span>
              </button>
            </div>
      ...
      >
        <div className="flex flex-wrap gap-2 min-w-0">
          <div className="flex min-w-[6rem] flex-1 flex-col gap-1">
            <span className="text-xs font-medium text-fg-muted">
              {t("widgetAppearance.transform.xOffset")}
            </span>
            <div className="flex items-center gap-1 min-w-0">
              <NumericDraftEnhancedInput
          ...
          <div className="flex min-w-[6rem] flex-1 flex-col gap-1">
            <span className="text-xs font-medium text-fg-muted">
              {t("widgetAppearance.transform.yOffset")}
            </span>
            <div className="flex items-center gap-1 min-w-0">
              <NumericDraftEnhancedInput
        ...
        <div className="mt-2 flex min-w-0 flex-col gap-1">
          <span className="text-xs font-medium text-fg-muted">
            {t("widgetAppearance.transform.zoom")}
          </span>
          <div className="flex items-center gap-1 min-w-0">
            <NumericDraftEnhancedInput
              committedDisplay={formatPercentDisplay(
                readFiniteNumber(getTransform("transformScale"), 1)
              )}
        ...
        <div className="mt-2 flex min-w-0 flex-col gap-1">
          <span className="text-xs font-medium text-fg-muted">
            {t("widgetAppearance.transform.rotation")}
          </span>
          <div className="flex items-center gap-1 min-w-0">
            <NumericDraftEnhancedInput
        ...
        <div className="mt-2 flex min-w-0 flex-col gap-1">
          <span className="text-xs font-medium text-fg-muted">
            {t("widgetAppearance.transform.opacity")}
          </span>
          <div className="flex items-center gap-1 min-w-0">
            <NumericDraftEnhancedInput
              committedDisplay={formatPercentDisplay(
