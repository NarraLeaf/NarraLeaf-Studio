...
export function CompactButtonAppearance({
  ...
  const getBorder = (key: ButtonAppearancePropertyKey) =>
    getRowValueForModuleEdit(variant, key, borderMode);
  const getSpacing = (key: ButtonAppearancePropertyKey) =>
    getRowValueForModuleEdit(variant, key, spacingMode);
  ...
  const patchCursor = (value: unknown) => {
    commitVariant(
      updateRowValueForModuleEditOrEnsure(variant, ["cursor"], "cursor", "default", value as never)
    );
  };
        ...
        <div className="pt-2 space-y-1 min-w-0">
          <BorderStrokeCompactRows
            ...
            onBorderColorChange={(next: ColorValue) =>
              patchBorder("borderColor", serializeColorValue(next))
            }
