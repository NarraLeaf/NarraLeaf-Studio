...
export const APPEARANCE_TWEEN_EASING_OPTIONS: {
  value: AppearanceTransitionTweenEasing;
...
export function isMotionCapableContainerAppearanceKey(
  key: string
): key is ContainerAppearancePropertyKey {
  return (CONTAINER_ANIMATABLE_KEYS as readonly string[]).includes(key);
}

export function isMotionCapableButtonAppearanceKey(
  key: string
): key is ButtonAppearancePropertyKey {
...
export function moduleHasAnyAppearanceTransition(
  variant: AppearanceVariant,
  keys: readonly string[]
): boolean {
  return listAnimatableKeysForModule(keys).some((key) =>
    hasAppearanceGroupTransition(variant, key)
  );
}

/** True if any variant has a transition on any animatable key in the module. */
export function moduleHasAnyAppearanceTransitionInModel(
...
export function countModuleAppearanceTransitions(
  variant: AppearanceVariant,
  keys: readonly string[]
): number {
  return listAnimatableKeysForModule(keys).filter((key) =>
    hasAppearanceGroupTransition(variant, key)
  ).length;
}

export function getDefaultAppearanceTransition(
  type: AppearanceFieldTransition["type"] = "tween"
): AppearanceFieldTransition {
  return JSON.parse(
    JSON.stringify(type === "spring" ? DEFAULT_SPRING_TRANSITION : DEFAULT_TWEEN_TRANSITION)
  );
}
...
export function formatAppearanceTransitionSummary(
  transition: AppearanceFieldTransition | null | undefined
): string {
  if (!transition) {
...
export function toRuntimeMotionTransition(
