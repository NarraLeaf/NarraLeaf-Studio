...
function deriveContainerMotionVisibility(
  model: AppearanceModel
): Record<ContainerAppearanceModuleId, boolean> {
  return {
...
function deriveButtonMotionVisibility(
  model: AppearanceModel
): Record<ButtonAppearanceModuleId, boolean> {
  return {
...
function deriveTextMotionVisibility(
  model: AppearanceModel
): Record<TextAppearanceModuleId, boolean> {
  return {
...
function cloneVariantShallow(
  source: AppearanceVariant,
  id: string,
  name: string
): AppearanceVariant {
  return {
...
export function AppearanceAuthoringPanel({
  ...
  const [containerMotionVisibility, setContainerMotionVisibility] = useState(
    DEFAULT_CONTAINER_MOTION_VISIBILITY
  );
  const [buttonMotionVisibility, setButtonMotionVisibility] = useState(
    DEFAULT_BUTTON_MOTION_VISIBILITY
  );
  const [textMotionVisibility, setTextMotionVisibility] = useState(DEFAULT_TEXT_MOTION_VISIBILITY);

  const setContainerModuleMode = useCallback(
    (module: ContainerAppearanceModuleId, mode: ModuleEditMode) => {
      setContainerModuleModes((prev) => ({ ...prev, [module]: mode }));
    },
    []
  );

  const setButtonModuleMode = useCallback(
    (module: ButtonAppearanceModuleId, mode: ModuleEditMode) => {
  ...
  const setContainerMotionVisible = useCallback(
    (module: ContainerAppearanceModuleId, visible: boolean) => {
      setContainerMotionVisibility((prev) => ({ ...prev, [module]: visible }));
    },
    []
  );

  const setButtonMotionVisible = useCallback(
    (module: ButtonAppearanceModuleId, visible: boolean) => {
  ...
  const body = (
    <div className="space-y-3 min-w-0">
      <div className="flex flex-wrap gap-2 items-center min-w-0">
        ...
        <button
          type="button"
          data-tip={t("widgetAppearance.variant.addTitle")}
          aria-label={t("widgetAppearance.variant.addTitle")}
        ...
        <button
          type="button"
          data-tip={t("widgetAppearance.variant.setDefaultTitle")}
          aria-label={t("widgetAppearance.variant.setDefaultTitle")}
        ...
        <button
          type="button"
          data-tip={t("widgetAppearance.variant.deleteTitle")}
          aria-label={t("widgetAppearance.variant.deleteTitle")}
      ...
      <div className="min-w-0">
        <label className="text-2xs tracking-wide text-fg-subtle block mb-1">
          {t("widgetAppearance.variant.nameLabel")}
        </label>
        <EnhancedInput
