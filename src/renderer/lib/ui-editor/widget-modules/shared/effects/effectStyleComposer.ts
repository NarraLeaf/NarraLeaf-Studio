 ...
 */
export function composeChromeEffectLayers(
  effects: ElementEffectValues
): ComposedChromeEffectLayers {
  const outerParts = [
...
export function mergeBoxShadowLayers(
  user: string | undefined,
  strokeSimulatedInset: string | undefined
): string | undefined {
  const u = user?.trim() ?? "";
...
export type ListHostEffectStyle = Pick<
  CSSProperties,
  "filter" | "backdropFilter" | "mixBlendMode" | "boxShadow"
>;
