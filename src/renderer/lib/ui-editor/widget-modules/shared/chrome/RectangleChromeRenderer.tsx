...
export function RectangleChromeRenderer({
  ...
  const parsedBg = parseColorValue(String(props.backgroundColor ?? ""), {
    hex: "#FFFFFF",
    alpha: 1
  });
  const colorFill =
  ...
  const parsedStroke = parseColorValue(String(props.borderColor ?? ""), {
    hex: "#FFFFFF",
    alpha: 1
  });
  ...
  const ts =
    Number.isFinite(props.transformScale) && props.transformScale > 0 ? props.transformScale : 1;
  ...
  if (wantsStroke && props.strokeAlign === "outside") {
    ...
    assignMotionTransition(
      rootTransition,
      "outlineWidth",
      firstTransition(appearanceTransitions, ["borderWidth"])
    );
    assignMotionTransition(
      rootTransition,
      "outlineColor",
  ...
  const hasAnimatedBackdrop = Object.prototype.hasOwnProperty.call(
    rootTransition,
    "backdropFilter"
  );
  ...
  const rootBaseStyle: CSSProperties = {
    ...
    ...(!hasAnimatedFilter && composedEffects.rootFilter
      ? { filter: composedEffects.rootFilter }
      : {}),
    ...(!hasAnimatedBackdrop && composedEffects.backdropFilter
      ? { backdropFilter: composedEffects.backdropFilter }
      : {})
  };
  ...
  const chromeChildren = useClipIsolation ? (
    <div style={innerClipStyle}>{chromeInner}</div>
  ) : (
    chromeInner
  );
