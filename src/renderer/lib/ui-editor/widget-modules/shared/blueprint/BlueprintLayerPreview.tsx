...
function inferEdgeColor(fromPort: string, toPort: string): string {
  ...
  return port.includes("value") || port.includes("result") || port.includes("data")
    ? "#f59e0b"
    : "#22d3ee";
}
...
function describePreviewNode(
  ...
  const pinRows = Math.max(
    inputPins.length,
    outputPins.length,
    entry.supportsDynamicInputPins ? 1 : 0
  );
  ...
  const inputs = inputPins
    .filter((pin) => pin.semantic === "data")
    .map((pin) => pin.label ?? pin.id);
...
const MiniBlueprintNode = memo(function MiniBlueprintNode({
  data
}: NodeProps<Node<MiniPreviewNodeData>>) {
          ...
          <div className="flex flex-1 justify-between gap-2 px-2 py-1.5">
            <div className="flex min-w-0 flex-col gap-1">
              {inputs.map((label, index) => (
                <span
                  key={`in-${index}`}
                  className="flex items-center gap-1 truncate text-2xs text-fg-muted"
                >
