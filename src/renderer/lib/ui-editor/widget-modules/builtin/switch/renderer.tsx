...
export function SwitchRenderer(props: WidgetRendererProps) {
  ...
  const trackElement = useMemo(
    () => findSwitchPart(element, document, "track"),
    [document, element]
  );
  const thumbElement = useMemo(
    () => findSwitchPart(element, document, "thumb"),
    [document, element]
  );
  ...
  const childrenIds = [trackElement?.id, thumbElement?.id].filter((id): id is string =>
    Boolean(id)
  );
  ...
  return (
    <div
      ...
      data-ui-switch-pending={
        pendingChecked === null ? undefined : pendingChecked ? "true" : "false"
      }
      tabIndex={0}
