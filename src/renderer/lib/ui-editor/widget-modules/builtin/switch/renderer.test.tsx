...
function stubLayout(view: RenderResult): void {
  ...
  for (const node of view.container.querySelectorAll<HTMLElement>(
    '[role="switch"], [data-ui-element-id]'
  )) {
...
function variantOf(call: RenderCall | undefined, id: string): string | undefined {
  ...
  return (override?.extra as { runtimeVariantOverrideId?: string } | undefined)
    ?.runtimeVariantOverrideId;
}
...
describe("SwitchRenderer", () => {
  it("flips both parts to the on variant while checked, and leaves geometry alone", () => {
    ...
    expect(lastCall(renderCalls)?.elementOverrides?.track?.layout).toEqual(
      document.elements.track!.layout
    );
    expect(lastCall(renderCalls)?.elementOverrides?.thumb?.layout).toEqual(
      document.elements.thumb!.layout
    );
  });
  ...
  it("toggles on release of a press that never moved, and dispatches changed then turnedOn", async () => {
    ...
    expect(dispatchElementBlueprintEvent.mock.calls[0]?.[2]).toEqual({
      checked: true,
