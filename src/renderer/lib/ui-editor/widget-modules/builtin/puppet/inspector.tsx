...
function describeStatusKey(
  reason: PuppetDescriptionUnavailableReason | null | undefined
): TranslationKey {
  switch (reason) {
    case "no-model":
      return "widgets.puppet.describeNoModel";
    case "no-backend":
      return "widgets.puppet.describeNoBackend";
    case "backend-missing":
      return "widgets.puppet.describeBackendMissing";
    case "not-described":
      return "widgets.puppet.describeNotSupported";
    case "failed":
      return "widgets.puppet.describeFailed";
    default:
      return "widgets.puppet.describeOk";
  }
}
...
function PuppetBackendField(props: CustomFieldProps<UIInspectorData>) {
  ...
  useEffect(() => {
    ...
    void listProjectPuppetRuntimes(context.project)
      .then((names) => {
        if (!cancelled) setInstalled(names);
      })
      .catch(() => {
          ...
          defineField<D, any>({
            ...
            fields: [
              defineField<D, any>({
                ...
                brandPalette: true,
                getValue: (d: D) =>
                  parseColorValue(getLiveChromeProps(d).backgroundColor, {
                    hex: "#FFFFFF",
                    alpha: 1
                  }),
              ...
              defineField<D, any>({
                ...
                brandPalette: true,
                getValue: (d: D) =>
                  parseColorValue(getLiveChromeProps(d).borderColor, { hex: "#FFFFFF", alpha: 1 }),
                setValue: (d: D, value: ColorValue) =>
