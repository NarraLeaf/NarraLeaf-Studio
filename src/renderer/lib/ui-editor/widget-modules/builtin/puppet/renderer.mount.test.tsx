...
} from "@/lib/ui-editor/runtime/game/surfacePuppetHosts";
...
describe("a configured puppet widget on a visible canvas", () => {
  ...
  it("counts what is drawn when it does have to refuse one", async () => {
    ...
    installIntersectionObserver(true);
    const views = Array.from({ length: SURFACE_PUPPET_CONTEXT_BUDGET + 1 }, () =>
      renderWidget({ assetId: "hiyori", backend: "live2d" })
    );
    await settle();
    ...
    expect(refused.container.textContent ?? "").toContain(
      `${SURFACE_PUPPET_CONTEXT_BUDGET} models are already drawn`
    );
