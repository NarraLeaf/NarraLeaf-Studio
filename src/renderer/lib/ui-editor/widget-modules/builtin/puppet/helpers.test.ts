...
function element(props: Record<string, unknown> = {}, layout?: Partial<UIElement["layout"]>): UIElement {
    return {
        id: "puppet-1",
...
describe("puppet widget helpers", () => {
    ...
    it("builds the engine's complete PuppetState", () => {
        ...
        const state = puppetWidgetState(getPuppetProps(element({
            motion: "wave",
            skin: "summer",
            params: { blink: 0.5 },
            slots: { weapon: null },
    ...
    it("asks for nothing until both a model and a runtime are chosen", () => {
        ...
        expect(puppetWidgetRequest(getPuppetProps(element({
            assetId: "m",
            backend: "b",
    ...
    it("does not put the pose in the request", () => {
        ...
        const request = puppetWidgetRequest(getPuppetProps(element({
            assetId: "m",
            backend: "b",
            motion: "idle",
            skin: "summer",
    ...
    it("gives the backend a box of at least one pixel", () => {
        ...
        expect(puppetWidgetSize(element({}, { width: -120, height: 0 }))).toEqual({ width: 120, height: 1 });
        expect(puppetWidgetSize(element({}, { width: 100.4, height: 99.6 }))).toEqual({ width: 100, height: 100 });
    });
});
