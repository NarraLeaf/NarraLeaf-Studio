...
function SliderValueLiteralEditor({
  data,
  liveElement
}: {
    ...
    <CompactModuleCard title={t("widgets.slider.value")}>
      ...
      <div className="mt-2 flex flex-wrap gap-2 min-w-0">
        ...
        <SliderNumberControl
          ...
          onFiniteNumber={(step) =>
            patchSlider(props.data, { step: Math.max(0, finiteOr(step, 0)) })
          }
        />
      </div>
...
function SliderPartsField(props: CustomFieldProps<UIInspectorData>) {
  ...
  const handleExists = Boolean(
    current.handleElementId && document.elements[current.handleElementId]
  );
  const stateService =
