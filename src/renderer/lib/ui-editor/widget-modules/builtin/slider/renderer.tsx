...
export function SliderRenderer(props: WidgetRendererProps) {
  ...
  const trackElement = useMemo(
    () => findSliderPart(element, document, "track"),
    [document, element]
  );
  const handleElement = useMemo(
    () => findSliderPart(element, document, "handle"),
    [document, element]
  );
  ...
  const scheduleSliderFlush = useCallback(() => {
    if (!blueprintRuntime) {
      return;
    }
    if (flushFrameRef.current !== null) {
  ...
  const dispatchCoalescedValueChanged = useCallback(() => {
    ...
    void blueprintRuntime
      .dispatchElementBlueprintEvent(element.id, "valueChanged", payload)
      .finally(() => {
  ...
  const queueValueChanged = useCallback(
  ...
  const valueFromPointer = useCallback(
    (clientX: number, clientY: number, pointerOffsetFromHandleCenter = 0): number => {
      ...
      const ratio = sliderProps.orientation === "horizontal" ? ratioOnTrack : 1 - ratioOnTrack;
  ...
  const setRuntimeValue = useCallback(
    async (nextValue: number, dispatchChange: boolean) => {
      ...
      const current = runtimeStore.setSliderProperties(runtimeElementKey, authoredProps, {
        value: nextValue
      }).value;
    ...
    [authoredProps, queueValueChanged, runtimeElementKey, runtimeStore, scheduleSliderFlush]
  );

  const handlePointerDown = useCallback(
    (event: PointerEvent<HTMLDivElement>) => {
      if (!canRunSliderInteraction || !blueprintRuntime || event.button !== 0) {
        return;
      }
      ...
      void (async () => {
        ...
        await blueprintRuntime.dispatchElementBlueprintEvent(element.id, "dragStart", {
          value: firstValue
        });
    ...
    trackElement && handleElement
      ? {
          [handleElement.id]: {
            ...
            style: {
              ...(handleElement.style ?? {}),
              cursor: canRunSliderInteraction
