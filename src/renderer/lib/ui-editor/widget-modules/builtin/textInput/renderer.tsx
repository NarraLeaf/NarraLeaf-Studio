...
export function TextInputRenderer(props: WidgetRendererProps) {
  ...
  const value = (
    runtimeStore?.getTextInputProperties(runtimeElementKey) ?? resolveTextInputRuntimeValue(p)
  ).value;
  ...
  const setRuntimeValue = useCallback(
    (nextValue: string, dispatchChange: boolean): string => {
      ...
      const current = runtimeStore.setTextInputProperties(runtimeElementKey, p, {
        value: nextValue
      }).value;
  ...
  const handleChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      const next = clampTextInputValue(
        coerceInputModeText(p.inputMode, event.target.value),
        p.maxLength
      );
  ...
  const resolvedCursor = interactionDisabled
    ? "not-allowed"
    : v.cursor !== "auto"
      ? v.cursor
      : canRunInteraction
