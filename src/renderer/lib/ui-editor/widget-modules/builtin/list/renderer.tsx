...
import type { UIListItemScope, UIListScrollbarPartStyle } from "@shared/types/ui-editor/list";
...
function ListItemRenderEvent(props: {
  ...
  useEffect(() => {
    ...
    void runtime.dispatchElementBlueprintEvent(
      elementId,
      "itemRender",
...
function resolveBoundItems(
  props: ReturnType<typeof getListProps>,
  runtimeData: WidgetRendererProps["runtimeData"]
): unknown[] | null {
...
function layoutWithMainAxisOffset(layout: UILayout, horizontal: boolean, offset: number): UILayout {
  return horizontal ? { ...layout, x: layout.x + offset } : { ...layout, y: layout.y + offset };
}
...
function resolveAuthoredThumbLayout(
  ...
  const travel = Math.max(
    0,
    axisSize(track.layout, horizontal) - axisSize(thumb.layout, horizontal)
  );
...
export function ListRenderer(props: WidgetRendererProps) {
  ...
  const items = runtimeListItems
    ? [...runtimeListItems]
    : (boundItems ??
      (p.previewItems.length > 0
        ? p.previewItems
        : Array.from({ length: p.previewCount }, (_, i) => ({ index: i }))));
  ...
  const dispatchListItemEvent = useCallback(
    (
      eventName: "itemClick" | "itemHover" | "selectionChanged",
      scope: UIListItemScope,
  ...
  const fallbackThumbLength = Math.max(
    ...
    Math.min(
      fallbackTrackLength,
      (metrics.viewport / Math.max(metrics.content, metrics.viewport)) * fallbackTrackLength
    )
  );
  const fallbackThumbTravel = Math.max(1, fallbackTrackLength - fallbackThumbLength);
  const fallbackThumbOffset = overflowAvailable
    ? (metrics.offset / scrollRange) * fallbackThumbTravel
    : 0;
  const authoredThumbLength = scrollbarThumbElement
    ? axisSize(scrollbarThumbElement.layout, horizontalScrollbar)
    : fallbackThumbLength;
  ...
  const scrollToRatio = useCallback(
    (clientPosition: number, trackRect: DOMRect, thumbLength: number, thumbTravel: number) => {
      ...
      const screenToDesign =
        trackScreenLength > 0 ? (thumbLength + thumbTravel) / trackScreenLength : 1;
  ...
  const startThumbDrag = useCallback(
    (
      ...
      const trackScreenLength = trackRect
        ? horizontalScrollbar
          ? trackRect.width
          : trackRect.height
        : 0;
    ...
    (event: PointerEvent<HTMLDivElement>) => {
      ...
      if (targetId === scrollbarThumbElement.id) {
        startThumbDrag(
          ...
          authoredThumbTravel,
          trackRect,
          authoredThumbLength + authoredThumbTravel
        );
        return;
      }
  ...
  const handleContentDragPointerDown = useCallback(
    (event: PointerEvent<HTMLDivElement>) => {
      ...
      const scrollbarIds = new Set(
        [scrollbarTrackElement?.id, scrollbarThumbElement?.id].filter((id): id is string =>
          Boolean(id)
        )
      );
    ...
    [p.dragContentScroll, p.repeatDirection, scrollbarThumbElement?.id, scrollbarTrackElement?.id]
  );
  ...
  const hasAuthoredScrollbar = Boolean(
    scrollbarTrackElement && scrollbarThumbElement && renderChildren
  );
  const authoredScrollbar =
    showScrollbar &&
    hasAuthoredScrollbar &&
    scrollbarTrackElement &&
    scrollbarThumbElement &&
    renderChildren ? (
      <>
  ...
  const fallbackScrollbar =
    showScrollbar && !hasAuthoredScrollbar ? (
      <div
