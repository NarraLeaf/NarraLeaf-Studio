...
function patchListScrollbarProps(
  data: UIInspectorData,
  partial: Partial<UIListScrollbarProps>
): void {
      ...
      <div ref={anchorRef} className="flex min-w-0 w-full flex-col gap-1 self-start">
        ...
        <div className="flex min-w-0 flex-nowrap items-stretch gap-2">
          ...
          <InspectOnlyButton
            ...
            aria-label={
              sidesOpen
                ? t("widgets.list.contentPaddingPerSideClose")
                : t("widgets.list.contentPaddingPerSideEdit")
            }
...
function ScrollbarCustomizeField(props: CustomFieldProps<UIInspectorData>) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {hasAuthoredParts
          ? t("widgets.list.scrollbarPartsCreated")
          : t("widgets.list.customizeScrollbar")}
      </Button>
      {hasAuthoredParts ? (
...
function ListScrollbarField(props: CustomFieldProps<UIInspectorData>) {
  ...
  return (
    <div className="space-y-2 min-w-0">
      <CompactModuleCard title={t("widgets.list.behavior")}>
        ...
        <div className="mt-2 flex flex-wrap gap-2 min-w-0">
          ...
          <ListSelectControl
            ...
            onChange={(value) =>
              patchScrollbar({ visibility: value as UIListScrollbarProps["visibility"] })
            }
          />
        </div>
      </CompactModuleCard>
      ...
      <CompactModuleCard
        title={authored ? t("widgets.list.contentReserve") : t("widgets.list.defaultMetrics")}
      >
        {authored ? (
          <ListNumberControl
            ...
            onFiniteNumber={(value) =>
              patchScrollbar({ contentInset: clampListSpacingPx(value, 64) })
            }
            max={64}
          />
        ) : (
          <div className="flex flex-wrap gap-2 min-w-0">
            <ListNumberControl
              ...
              onFiniteNumber={(value) =>
                patchScrollbar({ thickness: clampListMetricPx(value, 2, 64) })
              }
              min={2}
              max={64}
            />
            ...
            <ListNumberControl
              ...
              onFiniteNumber={(value) =>
                patchScrollbar({ minThumbLength: clampListMetricPx(value, 8, 256) })
              }
              min={8}
              max={256}
            />
...
function ListEffectsField(props: CustomFieldProps<UIInspectorData>) {
  ...
  const live = props.data.documentService.getDocument().elements[el.id] ?? el;
  const flat = getListProps(live);
  return (
    <StaticEffectsSection
      effects={flat.effects}
      onChange={(next) => {
        const docEl = props.data.documentService.getDocument().elements[el.id] ?? live;
...
export function createListInspector(ctx: InspectorContext) {
  ...
  const patchScrollbarPart = (
    part: "trackStyle" | "thumbStyle",
    partial: Partial<UIListScrollbarPartStyle>
  ) => {
    const current = getListProps(documentService.getDocument().elements[element.id] ?? element);
    patchScrollbar({
      [part]: {
          ...
          defineField<D, any>({
            ...
            fields: [
              ...
              defineField<D, any>({
                ...
                brandPalette: true,
                getValue: (d: D) =>
                  parseColorValue(getLiveListProps(d).scrollbar.trackStyle.backgroundColor, {
                    hex: "#FFFFFF",
                    alpha: 1
                  }),
                setValue: (_d: D, value: ColorValue) =>
                  patchScrollbarPart("trackStyle", {
              ...
              defineField<D, any>({
                ...
                setValue: (_d: D, value: ImageFill) =>
                  patchScrollbarPart("trackStyle", { imageFill: value, fillType: "image" })
              }),
              defineField<D, any>({
                ...
                brandPalette: true,
                getValue: (d: D) =>
                  parseColorValue(getLiveListProps(d).scrollbar.thumbStyle.backgroundColor, {
                    hex: "#FFFFFF",
                    alpha: 1
                  }),
                setValue: (_d: D, value: ColorValue) =>
                  patchScrollbarPart("thumbStyle", {
                    backgroundColor: serializeColorValue(value),
                    fillType: "color",
                    fillOpacity: 1
                  })
              ...
              defineField<D, any>({
                ...
                setValue: (_d: D, value: ImageFill) =>
                  patchScrollbarPart("thumbStyle", { imageFill: value, fillType: "image" })
              }),
              defineField<D, any>({
