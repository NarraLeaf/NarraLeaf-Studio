...
export function resolveListItemsBindingArray(
  ...
  if (binding.kind === "globalState") {
    ...
    value =
      props && Object.prototype.hasOwnProperty.call(props, binding.key)
        ? props[binding.key]
        : undefined;
  } else {
...
function normalizePartStyle(
  value: unknown,
  fallback: UIListScrollbarPartStyle
): UIListScrollbarPartStyle {
  const raw =
    value && typeof value === "object" ? (value as Partial<UIListScrollbarPartStyle>) : {};
  return {
    ...defaultListScrollbarPartStyle,
    ...fallback,
    ...raw,
    ...
    fillType:
      raw.fillType === "image" ? "image" : raw.fillType === "color" ? "color" : fallback.fillType,
    backgroundImage:
...
function normalizeScrollbar(value: unknown): UIListScrollbarProps {
  ...
  return {
    ...
    minThumbLength: clampNumber(
      raw.minThumbLength,
      defaultListScrollbarProps.minThumbLength,
      8,
      256
    ),
    ...
    trackElementId:
      typeof raw.trackElementId === "string" && raw.trackElementId ? raw.trackElementId : null,
    thumbElementId:
      typeof raw.thumbElementId === "string" && raw.thumbElementId ? raw.thumbElementId : null
  };
}
...
export function getListProps(element: UIElement): ListWidgetProps {
  ...
  return {
    ...
    previewItems: Array.isArray(raw.previewItems)
      ? raw.previewItems
      : defaultListWidgetProps.previewItems,
    itemKeyPath:
      typeof raw.itemKeyPath === "string" ? raw.itemKeyPath : defaultListWidgetProps.itemKeyPath,
    ...
    contentPaddingTop: clampNumber(
      raw.contentPaddingTop,
      defaultListWidgetProps.contentPaddingTop,
      0,
      512
    ),
    contentPaddingRight: clampNumber(
      raw.contentPaddingRight,
      defaultListWidgetProps.contentPaddingRight,
      0,
      512
    ),
    contentPaddingBottom: clampNumber(
      raw.contentPaddingBottom,
      defaultListWidgetProps.contentPaddingBottom,
      0,
      512
    ),
    contentPaddingLeft: clampNumber(
      raw.contentPaddingLeft,
      defaultListWidgetProps.contentPaddingLeft,
      0,
      512
    ),
    templateDirection:
