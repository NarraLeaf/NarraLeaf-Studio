...
vi.mock("@/apps/workspace/modules/properties/framework/utils/colorUtils", () => ({
  parseColorValue: (_raw: string | undefined, fallback: { hex: string; alpha?: number }) =>
    fallback,
...
function createStateService() {
  ...
  const listeners = new Set<
    (payload: { previous: TextEditOverride; next: TextEditOverride }) => void
  >();
  return {
    ...
    on(
      event: string,
      listener: (payload: { previous: TextEditOverride; next: TextEditOverride }) => void
    ) {
