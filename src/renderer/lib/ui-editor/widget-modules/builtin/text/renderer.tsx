...
import {
  beginInlineTextEdit,
  resolveInlineTextEditHost
} from "@/lib/ui-editor/interaction/inlineTextEdit";
...
export function TextRenderer({
  ...
  const [interactionOverride, setInteractionOverride] = useState(
    () => stateService?.getInteractionOverride() ?? null
  );
  ...
  useEffect(() => {
    ...
    return stateService.on("interactionOverrideChanged", (payload) => {
      ...
      const isHere =
        next?.kind === "textEdit" && next.surfaceId === surface.id && next.elementId === element.id;
      ...
      if (isHere && !wasHere) {
        const nextDraft = getTextProps(
          documentService.getDocument().elements[element.id] ?? element
        ).text;
  ...
  const inspectorVariantId = useEditorAppearanceInspectorVariant(
    element.id,
  ...
  const resolveCtx = {
    variantOverrideId:
      listScopedVariantId ?? runtimeState.variantOverrideId ?? inspectorVariantId ?? null,
    signals: runtimeState.signals
  };
  const p = resolveTextVisualProps(element, flatProps.appearance ?? undefined, resolveCtx);
  const appearanceTransitions = resolveTextAppearanceTransitions(
    flatProps.appearance ?? undefined,
    resolveCtx
  );
  ...
  const textBodyStyle: CSSProperties = {
    ...
    ...(!useEffectShell && effectTextStyle.mixBlendMode
      ? { mixBlendMode: effectTextStyle.mixBlendMode }
      : {})
  };
  ...
  assignMotionTransition(
    textTransition,
    "fontSize",
    firstTransitionForKeys(appearanceTransitions, ["fontSize"])
  );
  assignMotionTransition(
    textTransition,
    "color",
    firstTransitionForKeys(appearanceTransitions, ["color"])
  );
  assignMotionTransition(
    textTransition,
    "lineHeight",
    firstTransitionForKeys(appearanceTransitions, ["lineHeight"])
  );
  ...
  const filterTransition = firstTransitionForKeys(appearanceTransitions, [
    "effectBlur",
    "effectFilter"
  ]);
  ...
  const handleTextareaBlur = useCallback(
    (e: FocusEvent<HTMLTextAreaElement>) => {
      ...
      const openedMsAgo =
        editOpenedAtRef.current > 0 ? performance.now() - editOpenedAtRef.current : Infinity;
      const relatedTarget = e.relatedTarget instanceof Element ? e.relatedTarget : null;
      const relatedElementNode = relatedTarget?.closest(
        "[data-ui-element-id]"
      ) as HTMLElement | null;
      const relatedInsideSameElement = relatedElementNode?.dataset.uiElementId === element.id;
      const relatedIsEditable = Boolean(
        relatedTarget?.closest("textarea, input, [contenteditable='true']")
      );
