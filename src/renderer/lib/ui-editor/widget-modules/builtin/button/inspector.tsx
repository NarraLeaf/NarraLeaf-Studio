...
function ButtonAppearanceField(props: CustomFieldProps<UIInspectorData>) {
  ...
  const panelAppearance = isUsableAppearanceModel(appearance)
    ? appearance
    : createInitialButtonAppearance(flat);
  ...
  renderLiteralEditor: ({ data, liveElement }) => {
    ...
    return (
      <DraftTextInput
        ...
        readCommittedValue={() =>
          getButtonProps(data.documentService.getDocument().elements[liveElement.id] ?? liveElement)
            .label
        }
        onCommit={(next) => {
                    ...
                    items: [
                      {
                        ...
                        render: ({ data, onSaving }: InlineRowItemContext<D>) => {
                          const current = getButtonProps(data.element);
                          const stored = parseColorValue(current.color, {
                            hex: "#FFFFFF",
                            alpha: 1
                          });
