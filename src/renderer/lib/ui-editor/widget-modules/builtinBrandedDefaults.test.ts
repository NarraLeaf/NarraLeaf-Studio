    ...
    for (const { type, props, fallbacks } of BRANDED_WIDGETS) {
        it(`${type}: a new element carries brand links`, () => {
            ...
            for (const prop of props) {
                expect(isBrandLink(created[prop] as string), `${type}.${prop} is "${String(created[prop])}"`).toBe(true);
            }
        });
        ...
        it(`${type}: the initial appearance rows follow the links too`, () => {
            ...
            const variant = appearance!.variants.find(candidate => candidate.id === appearance!.defaultVariantId);
            ...
            for (const prop of props) {
                ...
                expect(isBrandLink(group.rows[0]?.value as string), `${type} appearance row "${prop}"`).toBe(true);
            }
        });
    }
});
