...
export type ExecuteGraphOptions = {
  ...
  valueExecution?: Pick<
    NonNullable<BehaviorNodeExecutionContext["valueExecution"]>,
    "trackDependency"
  >;
  signal?: AbortSignal;
  fnCallDepth?: number;
...
export async function executeGraph(options: ExecuteGraphOptions): Promise<ExecuteGraphResult> {
  ...
  try {
    while (cursor) {
      ...
      try {
        result = await abortablePromise(
          Promise.resolve(definition.execute(context)),
          options.signal,
          node.id
        );
        throwIfBlueprintExecutionCancelled(options.signal, node.id);
        ...
        throw err instanceof BlueprintGraphExecutionError
          ? err
          : new BlueprintGraphExecutionError(message, nodeId);
      } finally {
        if (trace) {
