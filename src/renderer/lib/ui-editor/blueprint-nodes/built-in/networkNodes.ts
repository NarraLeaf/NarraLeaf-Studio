...
import {
  isResponseBodyLimitReached,
  readResponseBody,
  storeResponseBody
} from "./responseBodyStore";
...
function requireLocals(ctx: NetworkExecuteCtx): Record<string, unknown> {
  if (!ctx.blueprintLocals) {
    throw new BlueprintGraphExecutionError(
      "Network: no execution scope to hold the response",
      ctx.node.id
    );
  }
...
export const networkBlueprintNodes: BlueprintNodeDef[] = [
  {
    ...
    async execute(ctx) {
      ...
      return {
        ...
        outputValues: {
          response:
            result.body === null ? null : storeResponseBody(locals, ctx.node.id, result.body),
