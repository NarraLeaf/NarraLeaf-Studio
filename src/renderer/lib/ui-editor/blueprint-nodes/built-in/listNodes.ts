...
function writeNode(input: {
  ...
  return {
    ...
    isLatent: true,
    pins: elementTarget
      ? [execIn, execNext, listIn, ...(input.pins ?? [])]
  ...
  if (ref) {
    ...
    if (currentSurfaceId && ref.surfaceId !== currentSurfaceId) {
      throw new BlueprintGraphExecutionError(
        "List node can only target the current Surface",
        ctx.node.id
      );
    }
...
export const listBlueprintNodes: BlueprintNodeDef[] = [
  ...
  writeNode({
    ...
    execute: (ctx) => {
      ...
      const index = Math.max(
        0,
        Math.min(items.length, toInteger(readPin(ctx, "index"), items.length))
      );
      const next = [...items];
  ...
  writeNode({
    ...
    execute: async (ctx) => {
      ...
      const item = readPin(ctx, "item");
      const index = api.widget
        .getListProperties(listId)
        .items.findIndex((value) => jsonEquals(value, item));
      if (index >= 0) {
  ...
  writeNode({
    ...
    target: "self",
    execute: async (ctx) => {
      await requireHostApi(ctx).widget.setListSelectedIndex(
        resolveListElementId(ctx, "self"),
  ...
  writeNode({
    ...
    target: "self",
    execute: async (ctx) => {
      await requireHostApi(ctx).widget.scrollListToIndex(
        resolveListElementId(ctx, "self"),
        toInteger(readPin(ctx, "index"), 0)
      );
      return { nextPort: "next" };
    }
  }),
  ...
  writeNode({
    ...
    target: "element",
    execute: (ctx) =>
      setItems(ctx, [...currentItems(ctx, "element"), readPin(ctx, "item")], "element")
  }),
  writeNode({
  ...
  writeNode({
    ...
    execute: async (ctx) => {
      await requireHostApi(ctx).widget.setListSelectedIndex(
        resolveListElementId(ctx, "element"),
        toInteger(readPin(ctx, "index"), -1)
      );
      return { nextPort: "next" };
    }
