...
function writeNode(input: {
    ...
    return {
        ...
        isPure: false,
        isLatent: true,
        pins: elementTarget ? [execIn, execNext, switchIn, ...input.pins] : [execIn, execNext, ...input.pins],
        magicElementTarget: elementTarget ? SWITCH_MAGIC_TARGET : undefined,
        scope: elementTarget ? undefined : SWITCH_SCOPE,
...
function runtimeSwitchRef(ctx: Parameters<BlueprintNodeDef["execute"]>[0], target: "self" | "element") {
    const api = requireHostApi(ctx);
    const ref = normalizeBlueprintElementRefValue(readPin(ctx, "switch"));
    if (ref) {
        if (ref.elementType !== SWITCH_ELEMENT_TYPE) {
            throw new BlueprintGraphExecutionError(
                translate("blueprint.runtimeError.elementWrongKind", { kind: widgetKindName(SWITCH_ELEMENT_TYPE) }),
                ctx.node.id,
            );
        }
 ...
 */
async function toggleCurrentSwitch(ctx: Parameters<BlueprintNodeDef["execute"]>[0], target: "self" | "element") {
    const { api, elementId } = runtimeSwitchRef(ctx, target);
...
export const switchBlueprintNodes: BlueprintNodeDef[] = [
    ...
    writeNode({
        ...
        target: "self",
        execute: ctx => patchCurrentSwitch(ctx, { checked: toBooleanValue(readPin(ctx, "checked"), false) }, "self"),
    }),
    writeNode({
    ...
    writeNode({
        ...
        pins: [boolIn("checked", "Checked")],
        target: "element",
        execute: ctx => patchCurrentSwitch(ctx, { checked: toBooleanValue(readPin(ctx, "checked"), false) }, "element"),
    }),
    writeNode({
