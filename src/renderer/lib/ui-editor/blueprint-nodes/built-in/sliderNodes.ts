...
function writeNode(input: {
  ...
  return {
    ...
    isLatent: true,
    pins: elementTarget
      ? [execIn, execNext, sliderIn, ...input.pins]
...
function runtimeSliderRef(
  ...
  if (ref) {
    if (ref.elementType !== SLIDER_ELEMENT_TYPE) {
      throw new BlueprintGraphExecutionError(
        "Slider node requires an nl.slider element",
        ctx.node.id
      );
    }
    ...
    if (currentSurfaceId && ref.surfaceId !== currentSurfaceId) {
      throw new BlueprintGraphExecutionError(
        "Slider node can only target the current Surface",
        ctx.node.id
      );
    }
  ...
  if (target === "element") {
    throw new BlueprintGraphExecutionError(
      "Slider Element node requires a Slider input",
      ctx.node.id
    );
  }
...
export const sliderBlueprintNodes: BlueprintNodeDef[] = [
  ...
  writeNode({
    ...
    target: "self",
    execute: (ctx) =>
      patchCurrentSlider(ctx, { value: toFiniteNumber(readPin(ctx, "value"), 0) }, "self")
  }),
  writeNode({
  ...
  writeNode({
    ...
    target: "element",
    execute: (ctx) =>
      patchCurrentSlider(ctx, { value: toFiniteNumber(readPin(ctx, "value"), 0) }, "element")
  }),
  writeNode({
