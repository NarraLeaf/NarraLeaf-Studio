...
function isBlueprintNodeDefArray(value: unknown): value is BlueprintNodeDef[] {
    return (
        Array.isArray(value) &&
        value.length > 0 &&
        value.every(entry => typeof entry === "object" && entry !== null && typeof (entry as BlueprintNodeDef).type === "string")
    );
}
...
describe("function entry node", () => {
    ...
    it("is offered by the palette in an empty function graph", () => {
        ...
        expect(isBlueprintNodeAllowedInGraphContext(def!, paletteContext({ hasFunctionEntry: false }))).toBe(true);
    });
    ...
    it("is refused once the function graph already has an entry", () => {
        ...
        expect(isBlueprintNodeAllowedInGraphContext(def, paletteContext({ hasFunctionEntry: true }))).toBe(false);
    });
    ...
    it("is refused in an event graph", () => {
        ...
        expect(isBlueprintNodeAllowedInGraphContext(def, paletteContext({ graphKind: "event" }))).toBe(false);
    });
