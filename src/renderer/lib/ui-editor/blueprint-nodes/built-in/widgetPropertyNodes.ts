...
function resolveTargetElementId(
  ...
  if (mode === "self") {
    ...
    if (!elementId) {
      throw new BlueprintGraphExecutionError(
        `${target.label} node requires a widget execution owner`,
        ctx.node.id
      );
    }
    return elementId;
  ...
  if (!ref) {
    throw new BlueprintGraphExecutionError(
      `${target.label} Element node requires an Element input`,
      ctx.node.id
    );
  }
  ...
  if (currentSurfaceId && ref.surfaceId !== currentSurfaceId) {
    throw new BlueprintGraphExecutionError(
      `${target.label} Element node can only target the current Surface`,
      ctx.node.id
    );
  }
...
function readOptionalFramePropsPatch(ctx: Parameters<BlueprintNodeDef["execute"]>[0]): {
...
function readNode(input: {
  ...
  return {
    ...
    magicElementTarget: elementTarget
      ? { inputPinId: "element", elementTypes: [input.target.elementType] }
      : undefined,
    scope: elementTarget
      ? undefined
      : { ownerKinds: ["widgetMain"], widgetElementTypes: [input.target.elementType] },
    execute: () => ({})
  };
}
  ...
  const nodes: BlueprintNodeDef[] = [
    ...
    writeNode({
      ...
      execute: async (ctx) => {
        ...
        await api.widget.setVisible(
          elementId,
          toBooleanValue(readPin(ctx, "visible"), current.visible)
        );
    ...
    writeNode({
      ...
      execute: async (ctx) => {
        ...
        const elementId = resolveTargetElementId(ctx, target, mode);
        const current = api.widget.getCommonProperties(elementId);
        await api.widget.setEnabled(
          elementId,
          toBooleanValue(readPin(ctx, "enabled"), current.enabled)
        );
        return { nextPort: "next" };
      }
    })
  ...
  return [
    ...
    writeNode({
      ...
      execute: async (ctx) => {
        const api = requireHostApi(ctx);
        const elementId = resolveTargetElementId(ctx, target, mode);
        await api.widget.setButtonProperties(elementId, {
    ...
    writeNode({
      ...
      execute: async (ctx) => {
        ...
        await requireHostApi(ctx).widget.setImageProperties(
          resolveTargetElementId(ctx, target, mode),
          {
            asset: normalizeBlueprintImageAssetValue(
              assetInput === undefined ? legacyAssetIdInput : assetInput
            )
          }
        );
        return { nextPort: "next" };
