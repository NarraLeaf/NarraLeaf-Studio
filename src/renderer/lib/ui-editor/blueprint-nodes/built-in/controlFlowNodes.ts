...
function createForLoopState(
  ctx: BehaviorNodeExecutionContext
): Extract<LoopState, { kind: "forLoop" }> {
...
function createForEachState(
  ctx: BehaviorNodeExecutionContext
): Extract<LoopState, { kind: "forEach" }> | null {
...
function executeBooleanBranch(
  ...
  return { nextPort: conditionValue ? truePort : falsePort };
}
...
function executeIfElse(ctx: BehaviorNodeExecutionContext) {
  const dynamicConditionPins = readDynamicInputPinIds(
    ctx.params,
    IF_ELSE_DYNAMIC_BRANCH_PINS_KEY
