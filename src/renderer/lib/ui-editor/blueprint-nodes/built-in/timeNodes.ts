...
export function readBlueprintTimeUnit(
...
export const timeBlueprintNodes: BlueprintNodeDef[] = [
  ...
  timeNode({
    ...
    pins: [numberIn("milliseconds", "Milliseconds"), out("result", "Result", "string")],
    inspectorParams: [
      {
        key: BLUEPRINT_TIME_PARAM_STYLE,
        label: "Layout",
  ...
  timeNode({
    ...
    pins: [timestampIn("timestamp", "Timestamp"), out("result", "Result", "string")]
  }),
  timeNode({
    type: BLUEPRINT_NODE_TYPE_TIME_IS_SAME_DAY,
    displayName: "Is Same Day",
    keywords: ["time", "date", "same", "day", "today", "compare", "calendar"],
    pins: [timestampIn("a", "A"), timestampIn("b", "B"), out("result", "Result", "boolean")]
  }),
  timeNode({
    ...
    pins: [timestampIn("timestamp", "Timestamp"), out("result", "Result", "float")]
  }),
  timeNode({
