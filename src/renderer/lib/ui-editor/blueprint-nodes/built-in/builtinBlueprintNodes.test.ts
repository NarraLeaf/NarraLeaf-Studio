...
const NO_DECLARED_LINKS: BlueprintHostApiRuntime["navigation"]["openExternal"] = async () => ({
  outcome: "refused",
  error: "not declared"
});
...
function createPageNavigationHostAdapter(
  ...
  fullscreen: { current: boolean; setCalls: boolean[] } | undefined = {
    current: false,
    setCalls: []
  },
    ...
    blueprintRuntime: {
      ...
      hostApi: {
        ...
        game: {
          ...
          getSpeakerColor: () => normalizeBlueprintRGBAColor(options.speakerColor ?? null),
...
describe("built-in blueprint nodes", () => {
  it("registers documented event, page, broadcast, string, text, slider, and debug nodes", () => {
    ...
    expect(
      frameBlueprintNodes.find((def) => def.type === BLUEPRINT_NODE_TYPE_FRAME_GET_PARAM)
        ?.hideInPalette
    ).toBe(true);
    ...
    expect(blueprintNodeRegistry.get(BLUEPRINT_NODE_TYPE_FLOW_SKIP_DELAY)?.displayName).toBe(
      "Skip Delay"
    );
    ...
    expect(blueprintNodeRegistry.get(BLUEPRINT_NODE_TYPE_SLIDER_GET_VALUE)?.displayName).toBe(
      "Get Value"
    );
  ...
  it("registers Var as a pinless blueprint-scope declaration node", () => {
    ...
    expect(def?.inspectorParams?.map((param) => param.key)).toEqual([
      "name",
      "valueType",
      "defaultValue"
    ]);
  ...
  it("projects Get Var and Set Var value pins from the selected variable type", () => {
    ...
    const getEntry = blueprintNodeRegistry.resolveCatalogEntryForNode(
      BLUEPRINT_NODE_TYPE_LOCAL_GET,
      {
        variableId: "score",
    ...
    const setEntry = blueprintNodeRegistry.resolveCatalogEntryForNode(
      BLUEPRINT_NODE_TYPE_LOCAL_SET,
      {
        variableId: "score",
        [BLUEPRINT_NODE_PARAM_VARIABLE_VALUE_TYPE]: "integer"
  ...
  it("executes Page navigation and Frame page switching through host APIs", async () => {
    ...
    await executeGraph({
      graph: {
        ...
        edges: [{ from: { nodeId: "props", port: "value" }, to: { nodeId: "set", port: "props" } }]
      },
      entry: { start: { nodeId: "set", port: "in" } },
  ...
  it("reads and writes the App window fullscreen state", async () => {
    const readFullscreenInto = async (fullscreen: {
      current: boolean;
      setCalls: boolean[];
      ...
      await executeGraph({
        graph: {
          ...
          edges: [
            ...
            {
              from: { nodeId: "get", port: "isFullscreen" },
    ...
    expect(await readFullscreenInto({ current: true, setCalls: [] })).toMatchObject({
      state: true
    });
    expect(await readFullscreenInto({ current: false, setCalls: [] })).toMatchObject({
      state: false
    });

    const runSetFullscreen = async (mode: unknown, initial: boolean): Promise<boolean[]> => {
        ...
        graph: {
          ...
          nodes: {
            ...
            opened: {
              id: "opened",
              type: BLUEPRINT_NODE_TYPE_LOCAL_SET,
              params: { variableId: "opened" }
            },
            failed: {
              id: "failed",
              type: BLUEPRINT_NODE_TYPE_LOCAL_SET,
              params: { variableId: "failed" }
            }
          },
          edges: [
      ...
      resolveDataPinValue(
        ...
        0,
        {
          hostAdapter: createGameSaveHostAdapter({
            speakerAvatar: toBlueprintImageAsset("asset-avatar-angry")
          })
        }
      )
  ...
  it("executes game preference getter and setter nodes through host APIs", async () => {
    ...
    await executeGraph({
      graph: {
        ...
        edges: [
          {
            from: { nodeId: "autoForward", port: "next" },
            to: { nodeId: "gameSpeed", port: "in" }
          },
          {
            from: { nodeId: "gameSpeed", port: "next" },
            to: { nodeId: "voiceEndMode", port: "in" }
          },
    ...
    await executeGraph({
      graph: {
        ...
        edges: [{ from: { nodeId: "cps", port: "cps" }, to: { nodeId: "capture", port: "value" } }]
      },
      entry: { start: { nodeId: "capture", port: "in" } },
  ...
  it("executes game save nodes through host APIs", async () => {
    ...
    await executeGraph({
      graph: {
        ...
        edges: [
          {
            from: { nodeId: "capture", port: "value" },
            to: { nodeId: "write", port: "screenshot" }
          }
        ]
      },
      entry: { start: { nodeId: "write", port: "in" } },
    ...
    await executeGraph({
      graph: {
        ...
        edges: [
          ...
          {
            from: { nodeId: "metadata", port: "metadata" },
            to: { nodeId: "capture", port: "value" }
          }
        ]
      },
      ...
      await executeGraph({
        graph: {
          ...
          edges: [
            ...
            {
              from: { nodeId: "line", port: "line" },
              to: { nodeId: "captureLine", port: "value" }
            },
            {
              from: { nodeId: "captureLine", port: "next" },
              to: { nodeId: "captureSpeaker", port: "in" }
            },
            {
              from: { nodeId: "line", port: "speaker" },
              to: { nodeId: "captureSpeaker", port: "value" }
            },
            {
              from: { nodeId: "captureSpeaker", port: "next" },
              to: { nodeId: "captureExists", port: "in" }
            },
            {
              from: { nodeId: "line", port: "exists" },
              to: { nodeId: "captureExists", port: "value" }
            }
          ]
        },
  ...
  it("executes the auto save nodes through host APIs", async () => {
    ...
    await executeGraph({
      graph: {
        ...
        edges: [
          {
            from: { nodeId: "autoList", port: "next" },
            to: { nodeId: "captureEntries", port: "in" }
          },
          {
            from: { nodeId: "autoList", port: "entries" },
            to: { nodeId: "captureEntries", port: "value" }
          },
          {
            from: { nodeId: "captureEntries", port: "next" },
            to: { nodeId: "captureCount", port: "in" }
          },
          {
            from: { nodeId: "autoList", port: "count" },
            to: { nodeId: "captureCount", port: "value" }
          }
        ]
      },
    ...
    await executeGraph({
      graph: {
        ...
        edges: [
          ...
          {
            from: { nodeId: "latest", port: "hasAutoSave" },
            to: { nodeId: "captureHas", port: "value" }
          }
        ]
      },
  ...
  it("reads the dialogue backlog and restores the game from a history entry", async () => {
    ...
    await executeGraph({
      graph: {
        ...
        edges: [
          {
            from: { nodeId: "backlog", port: "next" },
            to: { nodeId: "captureEntries", port: "in" }
          },
          {
            from: { nodeId: "backlog", port: "entries" },
            to: { nodeId: "captureEntries", port: "value" }
          },
          {
            from: { nodeId: "captureEntries", port: "next" },
          ...
          {
            from: { nodeId: "backlog", port: "count" },
            to: { nodeId: "captureCount", port: "value" }
          }
        ]
      },
  ...
  it("uses class.md palette categories for the new node groups", () => {
    ...
    const gameReadyHead = eventHeadBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_EVENT_HEAD_GAME_READY
    );
    ...
    const startStory = gameBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_GAME_START_STORY
    );
    ...
    expect(startStory?.pins.filter((pin) => pin.id !== "in").every((pin) => pin.optional)).toBe(
      true
    );
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_GET_NAMETAG)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["nametag"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_IS_IN_GAME)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["isInGame"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_IS_GAME_OVERLAY)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["isGameOverlay"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_QUIT)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in"]);
    expect(
      gameBlueprintNodes.find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_QUIT)?.inspectorParams
    ).toEqual([
    ...
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_NEXT)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_SKIP)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next"]);
    const setSentenceSpeedNode = gameBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_GAME_SET_SENTENCE_SPEED
    );
    expect(setSentenceSpeedNode?.pins.map((pin) => pin.id)).toEqual(["in", "next", "cps"]);
    expect(setSentenceSpeedNode?.pins.find((pin) => pin.id === "cps")).toMatchObject({
      label: "CPS"
    });
    ...
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_SAVE_LOAD)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "id"]);
    const saveGameNode = gameBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_GAME_SAVE_WRITE
    );
    ...
    expect(saveGameNode?.pins.find((pin) => pin.id === "metadata")).toMatchObject({
      optional: true
    });
    ...
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_SAVE_GET_METADATA)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "id", "metadata"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_SAVE_DELETE)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "id"]);
    expect(
      frameBlueprintNodes
    ...
    expect(
      frameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_PAGE_GO)
        ?.pins.find((pin) => pin.id === "props")?.optional
    ).toBe(true);
    expect(
      frameBlueprintNodes
    ...
    expect(
      frameBlueprintNodes
        ...
        ?.pins.map((pin) => pin.id)
    ).toEqual(["isTransitioning"]);
    expect(
      frameBlueprintNodes.find((def) => def.type === BLUEPRINT_NODE_TYPE_PAGE_GO)?.inspectorParams
    ).toEqual([
      {
        key: "surfaceId",
        label: "Page",
    ...
    expect(persistentVariableBlueprintNodes.every((def) => def.category === "Variables")).toBe(
      true
    );
    ...
    const frameSetPage = widgetPropertyBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FRAME_WIDGET_SET_PAGE
    );
    ...
    expect(elementFrameSetPage?.pins.map((pin) => pin.id)).toEqual([
      "in",
      "next",
      "element",
      "props"
    ...
    const buttonSetPointer = widgetPropertyBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_BUTTON_SET_POINTER
    );
    ...
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_CONTINUE_EVENT_BUBBLE)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_STOP_EVENT_BUBBLE)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next"]);
  ...
  it("adds Displayable variant and generic property nodes", () => {
    const displayableTypes = new Set(
      elementBlueprintNodes.filter((def) => def.category === "Displayable").map((def) => def.type)
    );
    const elementTypes = new Set(
      elementBlueprintNodes.filter((def) => def.category === "Element").map((def) => def.type)
    );

    expect([...displayableTypes]).toEqual(
    ...
    expect(displayableTypes.has(BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_ANIMATE_PROPERTY)).toBe(
      false
    );
    ...
    const setVariant = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_VARIANT
    );
    ...
    const getProperty = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_GET_PROPERTY
    );
    ...
    expect(getProperty?.inspectorParams?.[0]).not.toMatchObject({
      options: expect.arrayContaining([{ value: "variant", label: "Variant" }])
    });
    const setProperty = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_PROPERTY
    );
    expect(setProperty?.pins.map((pin) => pin.id)).toEqual(["in", "next", "value"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_DISPLAY)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "display"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_DISPLAY)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "element", "display"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_PROPERTY)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "element", "value"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_ANIMATE_PROPERTY)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "animation"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_ANIMATE_PROPERTY)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "element", "animation"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_STOP_ANIMATION)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "animation"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_STOP_ANIMATION)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next", "animation"]);
    expect(
      elementBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_ANIMATE_PROPERTY)
        ?.pins.find((pin) => pin.id === "animation")
    ).toMatchObject({
      kind: "output",
      semantic: "data",
    ...
    expect(
      elementBlueprintNodes.find((def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_GET_POSITION)
        ?.hideInPalette
    ).toBe(true);
    expect(
      elementBlueprintNodes.find((def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_GET_VARIANT)
        ?.hideInPalette
    ).toBe(true);
    expect(
      elementBlueprintNodes.find(
        (def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_GET_POSITION
      )?.hideInPalette
    ).toBe(true);
    expect(
      elementBlueprintNodes.find(
        (def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_GET_VARIANT
      )?.hideInPalette
    ).toBe(true);
    ...
    expect(
      legacyWidgetVariantNodes
        .filter((def) => def.type.endsWith(".setVariant"))
        .flatMap((def) => def.pins)
        .some((pin) => pin.id === "variantId")
    ).toBe(false);

    const animate = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_ANIMATE_PROPERTY
    );
  ...
  it("keeps structured literal editor metadata locked to fixed schemas", () => {
    const stringLiteral = dataBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_LITERAL_STRING
    );
    const rectLiteral = dataBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_LITERAL_RECT
    );

    expect(stringLiteral?.displayName).toBe("String");
    expect(stringLiteral?.pins.find((pin) => pin.id === "value")?.label).toBe("String");
    expect(stringLiteral?.inspectorParams?.[0]).toMatchObject({
      key: "value",
      label: "String",
      kind: "string"
    });
  ...
  it("resolves Data literals and explicit conversions", () => {
    ...
    expect(
      resolveDataPinValue(
        {
          nodes: {
            check: { type: BLUEPRINT_NODE_TYPE_DATA_IS_EMPTY_VALUE, params: { value: {} } }
          },
          edges: []
        },
        "check",
  ...
  it("resolves JSON parse, path read, existence, and stringify nodes", () => {
    ...
    expect(
      resolveDataPinValue(graph, "parse", "result", graph.nodes.parse.params, undefined)
    ).toEqual({
      user: { profile: { name: "Ada" } },
      items: [10, 20]
    });
    expect(
      resolveDataPinValue(graph, "getName", "result", graph.nodes.getName.params, undefined)
    ).toBe("Ada");
    expect(
      resolveDataPinValue(
        graph,
        "getArrayItem",
        "result",
        graph.nodes.getArrayItem.params,
        undefined
      )
    ).toBe(20);
    expect(
      resolveDataPinValue(graph, "hasName", "result", graph.nodes.hasName.params, undefined)
    ).toBe(true);
    expect(
      resolveDataPinValue(graph, "stringify", "result", graph.nodes.stringify.params, undefined)
    ).toBe('{"ok":true}');
  });
  ...
  it("resolves JSON make object, make array, and array length nodes", () => {
    ...
    const graph = {
      ...
      edges: [
        { from: { nodeId: "array", port: "result" }, to: { nodeId: "length", port: "value" } }
      ]
    };

    expect(
      resolveDataPinValue(graph, "object", "result", graph.nodes.object.params, undefined)
    ).toEqual({
      name: "Ada",
      score: 42
    });
    expect(
      resolveDataPinValue(
        ...
        "legacyObject",
        "result",
        graph.nodes.legacyObject.params,
        undefined
      )
    ).toEqual({
      legacyName: "Ada",
    ...
    expect(
      resolveDataPinValue(graph, "array", "result", graph.nodes.array.params, undefined)
    ).toEqual([true, null]);
    expect(
      resolveDataPinValue(graph, "length", "length", graph.nodes.length.params, undefined)
    ).toBe(2);
  });
  ...
  it("resolves JSON set, remove, merge object, and clone nodes", () => {
    ...
    expect(
      resolveDataPinValue(graph, "remove", "result", graph.nodes.remove.params, undefined)
    ).toEqual({
      user: { name: "Ada", score: 42 },
      items: [20]
    });
    expect(
      resolveDataPinValue(graph, "merge", "result", graph.nodes.merge.params, undefined)
    ).toEqual({
      name: "Ada",
      score: 42,
      ok: true
    });
    const cloned = resolveDataPinValue(
      graph,
      "clone",
      "result",
      graph.nodes.clone.params,
      undefined
    );
  ...
  it("exposes element-derived palette entries only after a bound Element Literal is present", () => {
    ...
    expect(
      baseEntries.some((entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_TEXT_GET_TEXT)
    ).toBe(false);
    expect(
      baseEntries.some(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_GET_VISIBLE
      )
    ).toBe(false);
    expect(
      baseEntries.some(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_GET_PROPERTY
      )
    ).toBe(false);
    expect(
      baseEntries.some(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_PROPERTY
      )
    ).toBe(false);
    expect(
      baseEntries.some(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_ANIMATE_PROPERTY
      )
    ).toBe(false);
    expect(
      baseEntries.some(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_VARIANT
      )
    ).toBe(false);
    ...
    const getTextEntry = derivedEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_TEXT_GET_TEXT
    );
    ...
    expect(
      derivedEntries.some((entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_TEXT_SET_TEXT)
    ).toBe(true);
    expect(
      derivedEntries.some(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_GET_VISIBLE
  ...
  it("deduplicates element-derived palette entries when several compatible refs exist", () => {
    ...
    expect(entriesOfType(BLUEPRINT_NODE_TYPE_ELEMENT_IMAGE_SET_CROP_RECT)[0]?.category).toBe(
      "Element"
    );
    expect(
      entriesOfType(BLUEPRINT_NODE_TYPE_ELEMENT_IMAGE_SET_CROP_RECT)[0]?.magicElementRef
    ).toBeUndefined();
    expect(entriesOfType(BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_PROPERTY)[0]?.category).toBe(
      "Element"
    );
    expect(
      entriesOfType(BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_PROPERTY)[0]?.magicElementRef
    ).toBeUndefined();
  });
  ...
  it("exposes Set Frame Page as an Element-category derived element entry", () => {
    ...
    expect(
      baseEntries.some((entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_FRAME_SET_PAGE)
    ).toBe(false);
    ...
    const setPage = derivedEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_FRAME_SET_PAGE
    );
  ...
  it("scopes ImageAsset nodes to Image owners or bound Image elements", () => {
    ...
    expect(
      surfaceEntries.some((entry) => entry.type === BLUEPRINT_NODE_TYPE_IMAGE_ASSET_LITERAL)
    ).toBe(false);
    expect(
      surfaceEntries.some((entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_IMAGE_SET_ASSET)
    ).toBe(false);
    ...
    expect(
      imageOwnerEntries.some((entry) => entry.type === BLUEPRINT_NODE_TYPE_IMAGE_ASSET_LITERAL)
    ).toBe(true);
    const setSelf = imageOwnerEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_IMAGE_SET_ASSET
    );
    ...
    expect(
      imageOwnerEntries
        .find((entry) => entry.type === BLUEPRINT_NODE_TYPE_IMAGE_SET_FIT_MODE)
        ?.pins.some((pin) => pin.id === "element")
    ).toBe(false);
    expect(
      imageOwnerEntries
        .find((entry) => entry.type === BLUEPRINT_NODE_TYPE_IMAGE_GET_CROP_RECT)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["leftPct", "topPct", "widthPct", "heightPct"]);
    ...
    expect(
      derivedEntries.some((entry) => entry.type === BLUEPRINT_NODE_TYPE_IMAGE_ASSET_LITERAL)
    ).toBe(true);
    const getImage = derivedEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_IMAGE_GET_ASSET
    );
    expect(getImage?.category).toBe("Element");
    expect(getImage?.pins.find((pin) => pin.id === "asset")).toMatchObject({
      valueType: BLUEPRINT_VALUE_TYPE_IMAGE_ASSET_NULLABLE
    });
    const setFitMode = derivedEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_IMAGE_SET_FIT_MODE
    );
    ...
    const getCropRect = derivedEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_IMAGE_GET_CROP_RECT
    );
    expect(getCropRect?.category).toBe("Element");
    expect(getCropRect?.pins.map((pin) => pin.id)).toEqual([
      "element",
      "leftPct",
      "topPct",
      "widthPct",
    ...
    expect(
      derivedEntries.find(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_STOP_ANIMATION
      )?.category
    ).toBe("Element");
    ...
    expect(
      imageWidgetDerivedEntries.find(
        (entry) => entry.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_PROPERTY
      )?.category
    ).toBe("Element");
    expect(
  ...
  it("resolves completed Math, Boolean, and Compare nodes", () => {
    ...
    expect(
      resolveDataPinValue(graph, "modulo", "result", graph.nodes.modulo.params, undefined)
    ).toBe(1);
    ...
    expect(resolveDataPinValue(graph, "round", "result", graph.nodes.round.params, undefined)).toBe(
      3
    );
    expect(resolveDataPinValue(graph, "floor", "result", graph.nodes.floor.params, undefined)).toBe(
      2
    );
    expect(resolveDataPinValue(graph, "ceil", "result", graph.nodes.ceil.params, undefined)).toBe(
      3
    );
    ...
    expect(resolveDataPinValue(graph, "and", "result", graph.nodes.and.params, undefined)).toBe(
      false
    );
    expect(resolveDataPinValue(graph, "or", "result", graph.nodes.or.params, undefined)).toBe(true);
    expect(resolveDataPinValue(graph, "not", "result", graph.nodes.not.params, undefined)).toBe(
      true
    );
    expect(resolveDataPinValue(graph, "xor", "result", graph.nodes.xor.params, undefined)).toBe(
      true
    );
    expect(
      resolveDataPinValue(graph, "equalStrict", "result", graph.nodes.equalStrict.params, undefined)
    ).toBe(false);
    expect(
      resolveDataPinValue(
        graph,
        "notEqualStrict",
        "result",
        graph.nodes.notEqualStrict.params,
        undefined
      )
    ).toBe(true);
    expect(
      resolveDataPinValue(graph, "greater", "result", graph.nodes.greater.params, undefined)
    ).toBe(true);
    expect(
      resolveDataPinValue(
        graph,
        "greaterEqual",
        "result",
        graph.nodes.greaterEqual.params,
        undefined
      )
    ).toBe(true);
    expect(resolveDataPinValue(graph, "less", "result", graph.nodes.less.params, undefined)).toBe(
      true
    );
    expect(
      resolveDataPinValue(graph, "lessEqual", "result", graph.nodes.lessEqual.params, undefined)
    ).toBe(true);
  });
  ...
  it("exposes and executes the If flow node", () => {
    ...
    const ifNode = controlFlowBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FLOW_IF
    )!;
    expect(
      ifNode.execute({
        graph: {
    ...
    const ifElseNode = controlFlowBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FLOW_IF_ELSE
    )!;
    expect(
      ifElseNode.execute({
        graph: {
    ...
    const ifElseCatalog = blueprintNodeRegistry.resolveCatalogEntryForNode(
      BLUEPRINT_NODE_TYPE_FLOW_IF_ELSE,
      {
    ...
    expect(ifElseCatalog.pins.map((pin) => pin.id)).toEqual([
      "in",
      "condition",
      "if_1_condition",
      "then",
  ...
  it("executes string switch, bounded loops, and zero-duration delay flow nodes", async () => {
    ...
    const switchNode = controlFlowBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FLOW_SWITCH_STRING
    )!;
    const switchCatalog = blueprintNodeRegistry.resolveCatalogEntryForNode(
      BLUEPRINT_NODE_TYPE_FLOW_SWITCH_STRING,
      {
    ...
    const forEachNode = controlFlowBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FLOW_FOR_EACH
    )!;
    ...
    const whileNode = controlFlowBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FLOW_WHILE
    )!;
    ...
    const whileContext = {
      graph: {
        ...
        edges: [
          {
            from: { nodeId: "condition", port: "value" },
            to: { nodeId: "while", port: "condition" }
          }
        ]
      },
    ...
    const delayNode = controlFlowBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FLOW_DELAY
    )!;
    const skipDelayNode = controlFlowBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_FLOW_SKIP_DELAY
    )!;
  ...
  it("skips a pending Delay through its Timer token", async () => {
    ...
    try {
      ...
      await expect(delayExecution).resolves.toEqual({
  ...
  it("registers the Game UI slot nodes with pure/latent gating and slot widget palettes", async () => {
    ...
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_GET_NOTIFICATIONS)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["notifications"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_GET_CHOICE_COUNT)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["count"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_IS_NVL_MODE)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["isNvlMode"]);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_IS_TEXT_READ)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["isRead"]);
    expect(
    ...
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_CLEAR_TEXT_READ)
        ?.pins.map((pin) => pin.id)
    ).toEqual(["in", "next"]);
    expect(
      gameBlueprintNodes.find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_CLEAR_TEXT_READ)
        ?.isPure
    ).toBe(false);
    expect(
      gameBlueprintNodes
        .find((def) => def.type === BLUEPRINT_NODE_TYPE_GAME_CHOOSE)
        ?.pins.map((pin) => pin.id)
  ...
  it("scopes event heads by owner and widget capability without duplicate click aliases", () => {
    ...
    expect(surfacePaletteTypes.has(BLUEPRINT_NODE_TYPE_EVENT_HEAD_WINDOW_CLOSE_REQUESTED)).toBe(
      true
    );
    ...
    expect(buttonPaletteTypes.has(BLUEPRINT_NODE_TYPE_EVENT_HEAD_WINDOW_CLOSE_REQUESTED)).toBe(
      false
    );
    ...
    const listGetItems = listEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_LIST_GET_ITEMS
    );
    ...
    const sliderGetValue = sliderEntries.find(
      (entry) => entry.type === BLUEPRINT_NODE_TYPE_SLIDER_GET_VALUE
    );
  ...
  it("filters widget event heads by the active event layer slot when provided", () => {
    ...
    expect(listItemClickPaletteTypes.has(BLUEPRINT_NODE_TYPE_EVENT_HEAD_SELECTION_CHANGED)).toBe(
      false
    );
  ...
  it("executes Text write nodes and resolves Text read nodes against the current widget owner", async () => {
    ...
    const setNode = textBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_TEXT_SET_TEXT
    )!;
    await Promise.resolve(
      setNode.execute({
        graph: {
    ...
    const setColorNode = textBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_TEXT_SET_TEXT_COLOR
    )!;
    await Promise.resolve(
  ...
  it("resolves Displayable Get Property against self and element targets without exposing Variant ids", () => {
    ...
    const hostAdapter = {
      ...
      blueprintRuntime: {
        hostApi: {
          widget: {
            getDisplayableProperties: (elementId: keyof typeof displayableProps) =>
              displayableProps[elementId],
  ...
  it("executes Displayable opacity writes and animations as editor percentages", async () => {
    ...
    let displayableMotion:
      | { target?: Record<string, unknown>; transition?: Record<string, unknown> }
      | undefined;
      ...
      blueprintRuntime: {
        hostApi: {
          widget: {
            ...
            animateDisplayable: async (
              _elementId: string,
              request: { target?: Record<string, unknown>; transition?: Record<string, unknown> }
            ) => {
    ...
    const setOpacity = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_PROPERTY
    )!;
    await Promise.resolve(
      setOpacity.execute({
    ...
    const setDisplay = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_DISPLAY
    )!;
    await Promise.resolve(
      setDisplay.execute({
        graph: {
    ...
    const animateOpacity = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_ANIMATE_PROPERTY
    )!;
    ...
    const setOffset = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_SET_PROPERTY
    )!;
    await Promise.resolve(
      setOffset.execute({
  ...
  it("requests held Displayable x animations without pre-patching layout", async () => {
    ...
    const animateX = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_ANIMATE_PROPERTY
    )!;
    const animateXNode = {
      id: "animateX",
  ...
  it("stops Displayable animations through AnimationToken input", async () => {
    const stopAnimation = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_DISPLAYABLE_STOP_ANIMATION
    )!;
    const stoppedAnimationIds: string[] = [];
    const hostAdapter = {
      host: "player",
    ...
    await expect(
      Promise.resolve(
        stopAnimation.execute(createContext({ kind: "animation", id: "animation:one" }))
      )
    ).resolves.toEqual({
      nextPort: "next"
    });
    await expect(
      Promise.resolve(stopAnimation.execute(createContext({ kind: "timer", id: "animation:one" })))
    ).resolves.toEqual({
      nextPort: "next"
    });
    await expect(
      Promise.resolve(stopAnimation.execute(createContext("not-an-animation")))
  ...
  it("executes Element Displayable property writes and animations through Element refs", async () => {
    ...
    const setElementOpacity = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_PROPERTY
    )!;
    const setNode = {
      id: "setOpacity",
    ...
    const setElementDisplay = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_SET_DISPLAY
    )!;
    ...
    const animateElementOpacity = elementBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_ANIMATE_PROPERTY
    )!;
    const animateNode = {
      id: "animateOpacity",
  ...
  it("executes Slider write nodes and resolves Slider read nodes against the current widget owner", async () => {
      ...
      blueprintRuntime: {
        ...
        hostApi: {
          widget: {
            ...
            setSliderProperties: async (
              _elementId: string,
              patch: Partial<UISliderRuntimeValue>
            ) => {
    ...
    const setValueNode = sliderBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_SLIDER_SET_VALUE
    )!;
    await Promise.resolve(
    ...
    const setRangeNode = sliderBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_SLIDER_SET_RANGE
    )!;
    await Promise.resolve(
  ...
  it("executes Button Set Pointer nodes", async () => {
    ...
    await executeButtonNode(BLUEPRINT_NODE_TYPE_BUTTON_SET_POINTER, "setPointerCrosshair", {
      cursor: "crosshair"
    });
  ...
  it("executes Image write nodes and resolves Image reads", async () => {
    ...
    await executeImageNode(BLUEPRINT_NODE_TYPE_IMAGE_SET_ASSET, "setImage", {
    ...
    await executeImageNode(BLUEPRINT_NODE_TYPE_IMAGE_SET_FIT_MODE, "setFitMode", {
      fitMode: "contain"
    });
...
describe("fn blueprint nodes", () => {
  ...
  it("resolves fn head parameter pins with per-pin labels and types", () => {
    const headDef =
      blueprintNodeRegistry.get(FN_HEAD_TYPE) ??
      fnBlueprintNodes.find((def) => def.type === FN_HEAD_TYPE)!;
  ...
  it("synthesizes Call Fn pins from the signature snapshot", () => {
    const callDef = fnBlueprintNodes.find((def) => def.type === FN_CALL_TYPE)!;
    const pins = resolveEffectiveBlueprintNodePins(callDef, {
      __fnSignatureSnapshot: CALL_SNAPSHOT
    });
    ...
    expect(resolveEffectiveBlueprintNodePins(callDef, {}).map((pin) => pin.id)).toEqual([
      "in",
      "next"
    ]);
  });
...
describe("sound node transport", () => {
  function soundHostAdapter(log: { play: unknown[]; ops: unknown[] }): UIHostAdapter {
    ...
    hostApi.sound = {
      ...
      setVolume: async (handle, volume, fadeMs) =>
        void log.ops.push({ op: "setVolume", handle, volume, fadeMs }),
      seek: async (handle, timeMs) => void log.ops.push({ op: "seek", handle, timeMs }),
      isPlaying: () => true,
      resolveElementVolume: (input) => input.volume ?? 1,
      subscribeMixerChanges: () => () => undefined,
      getTrackVolume: (trackId) => (trackId === "alice" ? 0.3 : 1),
      setTrackVolume: async (trackId, volume) =>
        void log.ops.push({ op: "setTrackVolume", trackId, volume })
    };
    return adapter;
  }
...
describe("track volume nodes", () => {
  ...
  it("serves Get Track Volume's pin through the resolver, keyed by the picked track", () => {
    registerCoreBlueprintNodes();
    const read = (params: Record<string, unknown>): unknown =>
      resolveDataPinValue(
...
describe("character data nodes", () => {
  ...
  function resolveSpeakerColor(speakerColor?: string | null): unknown {
    ...
    return resolveDataPinValue(
      {
        nodes: { speaker: { type: BLUEPRINT_NODE_TYPE_GAME_GET_SPEAKER_COLOR, params: {} } },
        edges: []
      },
      "speaker",
      "color",
  ...
  it("reads a named character's name, colour and avatar", () => {
    ...
    expect(resolveCharacterPin("characterColor", params, [ALICE])).toEqual({
      r: 64,
      g: 168,
      b: 196,
      a: 1
    });
    expect(resolveCharacterPin("characterAvatar", params, [ALICE])).toEqual({
      kind: "imageAsset",
      assetId: "asset-alice-avatar"
    });
  ...
  it("degrades visibly when the referenced character is gone", () => {
    ...
    expect(resolveCharacterPin("characterColor", params, [])).toEqual({
      r: 255,
      g: 255,
      b: 255,
      a: 1
  ...
  it("separates a character with no colour from one that is missing", () => {
    ...
    expect(resolveCharacterPin("characterColor", params, [MUTE])).toEqual({
      r: 255,
      g: 255,
      b: 255,
      a: 1
  ...
  it("registers both nodes in the Game category and offers them to value graphs", () => {
    ...
    const speaker = gameBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_GAME_GET_SPEAKER_COLOR
    )!;
    expect(speaker.isPure).toBe(true);
    expect(speaker.pins.map((pin) => pin.id)).toEqual(["color"]);

    const character = gameBlueprintNodes.find(
      (def) => def.type === BLUEPRINT_NODE_TYPE_GAME_GET_CHARACTER
    )!;
    expect(character.isPure).toBe(true);
    expect(character.pins.map((pin) => pin.id)).toEqual([
      "name",
      "characterColor",
      "characterAvatar",
      "found"
