...
function toggleIntoLocalGraph(toggleType: string, elementRef: boolean) {
    ...
    if (elementRef) {
        ...
        edges.push({ from: { nodeId: "ref", port: "element" }, to: { nodeId: "toggle", port: "switch" } });
    }
    return {
        id: "toggleGraph",
        ...
        await executeGraph({
            graph: {
                ...
                nodes: {
                    ...
                    literal: { id: "literal", type: BLUEPRINT_NODE_TYPE_LITERAL_BOOLEAN, params: { value: true } },
                },
                edges: [{ from: { nodeId: "literal", port: "value" }, to: { nodeId: "set", port: "checked" } }],
            },
        ...
        await expect(
            executeGraph({
                graph: {
                    ...
                    edges: [{ from: { nodeId: "ref", port: "element" }, to: { nodeId: "set", port: "switch" } }],
                },
                entry: { start: { nodeId: "set", port: "in" } },
                hostAdapter: createSwitchHostAdapter(store),
                executionOwner: SELF_OWNER,
