...
function writeNode(input: {
  ...
  return {
    ...
    isLatent: true,
    pins: elementTarget
      ? [execIn, execNext, textInputIn, ...input.pins]
...
function runtimeTextInputRef(
  ...
  if (!elementId) {
    throw new BlueprintGraphExecutionError(
      "Text Input node requires a Text Input target",
      ctx.node.id
    );
  }
...
export const textInputBlueprintNodes: BlueprintNodeDef[] = [
  ...
  writeNode({
    ...
    target: "self",
    execute: (ctx) =>
      patchCurrentTextInput(ctx, { value: toStringValue(readPin(ctx, "value")) }, "self")
  }),
  writeNode({
  ...
  writeNode({
    ...
    target: "element",
    execute: (ctx) =>
      patchCurrentTextInput(ctx, { value: toStringValue(readPin(ctx, "value")) }, "element")
  }),
  writeNode({
