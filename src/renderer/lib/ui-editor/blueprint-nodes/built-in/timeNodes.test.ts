...
describe("Time blueprint nodes", () => {
  ...
  it("rolls out-of-range Make Time fields the way a date picker would", () => {
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_MAKE, "timestamp", { year: 2026, month: 9, day: 0 })
    ).toBe(localTime(2026, 8, 31));
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_MAKE, "timestamp", { year: 2026, month: 13, day: 1 })
    ).toBe(localTime(2027, 1, 1));
  });

  it("keeps a two-digit year as itself rather than the 1900s", () => {
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_PARTS, "year", {
        timestamp: readPin(BLUEPRINT_NODE_TYPE_TIME_MAKE, "timestamp", {
          year: 50,
          month: 1,
          day: 1
        })
  ...
  it("clamps Add Time to the shorter month instead of rolling past it", () => {
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_ADD, "result", {
        timestamp: jan31,
        amount: 1,
        [BLUEPRINT_TIME_PARAM_UNIT]: "months"
      })
    ).toBe(localTime(2026, 2, 28));
    // 2028 is a leap year, so the same add lands one day later there.
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_ADD, "result", {
        timestamp: localTime(2028, 1, 31),
        amount: 1,
        [BLUEPRINT_TIME_PARAM_UNIT]: "months"
      })
  ...
  it("adds fixed units by arithmetic and years by the calendar", () => {
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_ADD, "result", {
        timestamp: base,
        amount: 90,
        [BLUEPRINT_TIME_PARAM_UNIT]: "minutes"
      })
    ).toBe(base + 90 * 60_000);
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_ADD, "result", {
        timestamp: base,
        amount: -2,
        [BLUEPRINT_TIME_PARAM_UNIT]: "years"
      })
  ...
  it("measures Time Difference in fractions for fixed units and whole steps for months", () => {
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_DIFFERENCE, "difference", {
        from,
        to: from + 5_400_000,
        [BLUEPRINT_TIME_PARAM_UNIT]: "hours"
      })
    ).toBe(1.5);
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_DIFFERENCE, "difference", {
        from,
        to: localTime(2026, 2, 28),
        [BLUEPRINT_TIME_PARAM_UNIT]: "months"
      })
    ).toBe(1);
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_DIFFERENCE, "difference", {
        from: localTime(2026, 6, 1),
        to: from,
        [BLUEPRINT_TIME_PARAM_UNIT]: "months"
      })
    ).toBe(-4);
  ...
  it("substitutes Format Time pattern tokens against the local calendar", () => {
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_FORMAT, "result", {
        timestamp,
  ...
  it("formats a duration as a clock, hours accumulating past a day", () => {
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_FORMAT_DURATION, "result", {
        milliseconds: span
      })
    ).toBe("30:02:03");
    expect(
    ...
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_FORMAT_DURATION, "result", {
        milliseconds: 65_000,
        [BLUEPRINT_TIME_PARAM_STYLE]: "hoursMinutesSeconds"
      })
    ).toBe("00:01:05");
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_FORMAT_DURATION, "result", {
        milliseconds: -65_000,
        [BLUEPRINT_TIME_PARAM_STYLE]: "minutesSeconds"
      })
    ).toBe("-01:05");
  });
  ...
  it("splits a duration into bounded parts plus totals", () => {
    ...
    const part = (id: string) =>
      readPin(BLUEPRINT_NODE_TYPE_TIME_DURATION_PARTS, id, { milliseconds: span });
    ...
    expect(readPin(BLUEPRINT_NODE_TYPE_TIME_DURATION_PARTS, "negative", { milliseconds: -1 })).toBe(
      true
    );
  });
  ...
  it("reads a bare date as local midnight and round-trips ISO through To ISO String", () => {
    expect(readPin(BLUEPRINT_NODE_TYPE_TIME_PARSE, "timestamp", { value: "2026-08-14" })).toBe(
      localTime(2026, 8, 14)
    );
  ...
  it("compares calendar days rather than elapsed hours", () => {
    ...
    expect(readPin(BLUEPRINT_NODE_TYPE_TIME_IS_SAME_DAY, "result", { a: morning, b: night })).toBe(
      true
    );
    // Half an hour apart, and a different day.
    expect(readPin(BLUEPRINT_NODE_TYPE_TIME_IS_SAME_DAY, "result", { a: night, b: nextDay })).toBe(
      false
    );
    expect(readPin(BLUEPRINT_NODE_TYPE_TIME_START_OF_DAY, "result", { timestamp: night })).toBe(
      localTime(2026, 8, 14)
    );
  });
  ...
  it("reports the local zone offset in minutes", () => {
    ...
    expect(readPin(BLUEPRINT_NODE_TYPE_TIME_ZONE_OFFSET, "offsetMinutes", { timestamp })).toBe(
      -new Date(timestamp).getTimezoneOffset()
    );
    expect(typeof readPin(BLUEPRINT_NODE_TYPE_TIME_ZONE_OFFSET, "name", { timestamp })).toBe(
      "string"
    );
  });
  ...
  it("answers nothing when Format Time Localized is asked for neither half", () => {
    expect(
      readPin(BLUEPRINT_NODE_TYPE_TIME_FORMAT_LOCALIZED, "result", {
  ...
  it("picks the relative unit by size", () => {
    ...
    const relative = (to: number, style?: string) =>
      readPin(BLUEPRINT_NODE_TYPE_TIME_FORMAT_RELATIVE, "result", {
        from: now,
        to,
        locale: "en-US",
        ...(style ? { [BLUEPRINT_TIME_PARAM_STYLE]: style } : {})
      });
...
function createSaveHostAdapter(
  times: Record<string, { savedAt: number; createdAt: number }>
): UIHostAdapter {
  return {
    host: "player",
    blueprintRuntime: {
  ...
  return {
    ...
    nodes: {
      ...
      savedAt: {
        ...
        params: { variableId: "savedAt" }
      },
      createdAt: {
        id: "createdAt",
        type: BLUEPRINT_NODE_TYPE_LOCAL_SET,
        params: { variableId: "createdAt" }
      },
      exists: {
        id: "exists",
        type: BLUEPRINT_NODE_TYPE_LOCAL_SET,
  ...
  it("publishes a slot's stamps to downstream data pins", async () => {
    ...
    await executeGraph({
      ...
      hostAdapter: createSaveHostAdapter({
        "slot-1": { savedAt: 1_700_000_000_000, createdAt: 1_600_000_000_000 }
      }),
