...
import { BLUEPRINT_FLOW_DELAY_TOKEN_PIN_ID, createDelayTimerToken } from "./flowTimerTokens";
...
const MATH_UNARY_OPS: Record<
  string,
  "increment" | "decrement" | "abs" | "round" | "floor" | "ceil"
> = {
...
function resolveMathVariadicNumbers(
  ...
  for (const pin of dataIn) {
    const n = toFiniteNumber(
      resolveInput(graph, nodeId, pin.id, params, blueprintLocals, depth, runtime)
    );
    if (Number.isNaN(n)) {
      return null;
    }
...
function resolveMathRandomResult(
  ...
  const minRaw = toFiniteNumber(
    resolveInput(graph, nodeId, "min", params, blueprintLocals, depth, runtime)
  );
  const maxRaw = toFiniteNumber(
    resolveInput(graph, nodeId, "max", params, blueprintLocals, depth, runtime)
  );
...
function resolveBooleanNodeResult(
  ...
  const a = toBlueprintBoolean(
    resolveInput(graph, nodeId, "a", params, blueprintLocals, depth, runtime)
  );
  if (op === "not") {
    return !a;
  }
  const b = toBlueprintBoolean(
    resolveInput(graph, nodeId, "b", params, blueprintLocals, depth, runtime)
  );
  switch (op) {
    case "and":
      return a && b;
...
function createJsonContainerForNextSegment(
...
function resolveJsonMakeArrayResult(
  ...
  return resolveEffectiveBlueprintNodePins(def, params)
    ...
    .map((pin) =>
      toJsonSafeValue(resolveInput(graph, nodeId, pin.id, params, blueprintLocals, depth, runtime))
    );
}
...
function resolveStringNodeOutput(
  ...
  const any = (pin: string) =>
    resolveInput(graph, nodeId, pin, params, blueprintLocals, depth, runtime);
...
function resolveBroadcastNodeOutput(
  ...
  const eventName = toBlueprintString(
    resolveInput(graph, nodeId, "event", params, blueprintLocals, depth, runtime)
  ).trim();
  if (!eventName) {
    return 0;
  }
...
function resolveFrameNodeOutput(
  ...
  const key = toBlueprintString(
    resolveInput(graph, nodeId, "key", params, blueprintLocals, depth, runtime)
  ).trim();
...
function resolvePageNodeOutput(portId: string, runtime?: DataPinResolveRuntime): unknown {
  ...
  if (portId === "isEntering") {
    return (
      runtime?.hostAdapter?.blueprintRuntime?.getSurfaceTransitionState?.().isEntering === true
    );
  }
...
function resolveGameNodeOutput(
  ...
  if (preference && portId === preference.portId) {
    return runtime?.hostAdapter?.blueprintRuntime?.hostApi?.game.getPreference(
      preference.key as never
    );
  }
  return undefined;
}
...
function resolveTextReadByIdNodeOutput(
  ...
  const textId = String(
    resolveInput(graph, nodeId, "textId", params, blueprintLocals, depth, runtime) ?? ""
  ).trim();
  if (!textId) {
    return false;
  }
...
function resolveAppTagNodeOutput(nodeType: string, portId: string): unknown {
  if (
    nodeType !== BLUEPRINT_NODE_TYPE_GAME_GET_APP_TAG ||
    portId !== BLUEPRINT_APP_TAG_OUTPUT_PIN_ID
  ) {
    return undefined;
  }
...
function sameSurfaceElementRef(
  ref: BlueprintElementRef | undefined,
  runtime?: DataPinResolveRuntime
...
function resolveElementInputRef(
  ...
  return sameSurfaceElementRef(
    normalizeBlueprintElementRefValue(
      resolveInput(graph, nodeId, "element", params, blueprintLocals, depth, runtime)
    ),
    runtime
  );
}
...
function resolveListElementIdInput(
  ...
  const ref = sameSurfaceElementRef(
    normalizeBlueprintElementRefValue(
      resolveInput(graph, nodeId, "list", params, blueprintLocals, depth, runtime)
    ),
    runtime
  );
...
function resolveElementDisplayableNodeOutput(
  ...
  if (type === BLUEPRINT_NODE_TYPE_ELEMENT_DISPLAYABLE_GET_VARIANT && portId === "variantId") {
    return read(
      "props.appearance.defaultVariantId",
      api.widget.getCommonProperties(ref.elementId).variantId ?? ""
    );
  }
...
function resolveSliderNodeOutput(
  ...
  const ref = isElementTarget
    ? sameSurfaceElementRef(
        normalizeBlueprintElementRefValue(
          resolveInput(graph, nodeId, "slider", params, blueprintLocals, depth, runtime)
        ),
        runtime
      )
    : undefined;
...
function resolveSwitchNodeOutput(
  ...
  const ref = isElementTarget
    ? sameSurfaceElementRef(
        normalizeBlueprintElementRefValue(
          resolveInput(graph, nodeId, "switch", params, blueprintLocals, depth, runtime)
        ),
        runtime
      )
    : undefined;
...
function resolveWidgetPropertyNodeOutput(
  ...
  try {
    ...
    if (action === "getVariant" && portId === "variantId") {
      return read(
        "props.appearance.defaultVariantId",
        api.widget.getCommonProperties(elementId).variantId ?? ""
      );
    }
    ...
    if (key === "image" && action === "getImageAsset" && portId === "asset") {
      return read(
        "props.imageFill.assetId",
        api.widget.getImageProperties(elementId).asset ?? null
      );
    }
    ...
    if (
      (key === "frame" || key === "frameWidget") &&
      action === "getTargetPage" &&
      portId === "targetSurfaceId"
    ) {
      return read(
        "props.targetSurfaceId",
        api.widget.getFrameProperties(elementId).targetSurfaceId ?? ""
      );
    }
    if (
      (key === "frame" || key === "frameWidget") &&
...
function resolveTimeNodeOutput(
  ...
  if (type === BLUEPRINT_NODE_TYPE_TIME_FORMAT) {
    ...
    return formatBlueprintTime(
      ts("timestamp"),
      pattern.length > 0 ? pattern : DEFAULT_TIME_FORMAT_PATTERN
    );
  }
  if (type === BLUEPRINT_NODE_TYPE_TIME_FORMAT_LOCALIZED) {
    return portId === "result"
      ? formatBlueprintTimeLocalized({
          ...
          locale: str("locale"),
          dateStyle: readBlueprintTimeDisplayStyle(
            params,
            BLUEPRINT_TIME_PARAM_DATE_STYLE,
            "medium"
          ),
...
function resolveDataNodeOutput(
  ...
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_ARRAY_LENGTH && portId === "length") {
    return normalizeArrayValue(
      resolveInput(graph, nodeId, "array", params, blueprintLocals, depth, runtime)
    ).length;
  }
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_ARRAY_GET && portId === "item") {
    const array = normalizeArrayValue(
      resolveInput(graph, nodeId, "array", params, blueprintLocals, depth, runtime)
    );
    const index = toInteger(
      resolveInput(graph, nodeId, "index", params, blueprintLocals, depth, runtime),
      -1
    );
  ...
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_ARRAY_SET) {
    ...
    const out = [...array];
    out[index] = toJsonSafeValue(
      resolveInput(graph, nodeId, "item", params, blueprintLocals, depth, runtime)
    );
    return out;
  }
  ...
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_ARRAY_SLICE) {
    ...
    const start = clampInteger(
      resolveInput(graph, nodeId, "start", params, blueprintLocals, depth, runtime),
      0,
      array.length,
      0
    );
    const end = clampInteger(
      resolveInput(graph, nodeId, "end", params, blueprintLocals, depth, runtime),
      start,
      array.length,
      array.length
    );
  ...
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_ARRAY_JOIN) {
    ...
    const separator = toBlueprintString(
      resolveInput(graph, nodeId, "separator", params, blueprintLocals, depth, runtime)
    );
    return array.map(toBlueprintString).join(separator);
  }
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_OBJECT_KEYS) {
    const object = toJsonSafeValue(
      resolveInput(graph, nodeId, "object", params, blueprintLocals, depth, runtime)
    );
  ...
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_OBJECT_VALUES) {
    ...
    return isJsonObjectRecord(object)
      ? Object.values(object).map((value) => toJsonSafeValue(value))
      : [];
  }
  ...
  if (type === BLUEPRINT_NODE_TYPE_COLLECTION_OBJECT_SET_FIELD) {
    ...
    const field = toBlueprintString(
      resolveInput(graph, nodeId, "field", params, blueprintLocals, depth, runtime)
    );
    const value = resolveInput(graph, nodeId, "value", params, blueprintLocals, depth, runtime);
  ...
  if (type === BLUEPRINT_NODE_TYPE_DATA_JSON_SET) {
    ...
    const path = toBlueprintString(
      resolveInput(graph, nodeId, "path", params, blueprintLocals, depth, runtime)
    );
...
function resolveSelfOutput(
  ...
  if (portId === "value") {
    ...
    if (
      selfNode.type === BLUEPRINT_NODE_TYPE_LITERAL_FLOAT ||
      selfNode.type === BLUEPRINT_NODE_TYPE_LITERAL_NUMBER
    ) {
  ...
  if (
    selfNode.type === BLUEPRINT_NODE_TYPE_FLOW_DELAY &&
    portId === BLUEPRINT_FLOW_DELAY_TOKEN_PIN_ID
  ) {
  ...
  if (
    ...
    (selfNode.type === BLUEPRINT_NODE_TYPE_LAYER_CONFIRM &&
      (portId === "index" || portId === "label"))
  ) {
    return readBlueprintNodeOutputValue(blueprintLocals, nodeId, portId);
  }
  ...
  if (selfNode.type === BLUEPRINT_NODE_TYPE_MATH_RANDOM_FLOAT && portId === "result") {
    return resolveMathRandomResult(
      graph,
      nodeId,
      false,
      selfNode.params ?? {},
  ...
  if (booleanOp && portId === "result") {
    return resolveBooleanNodeResult(
      graph,
      nodeId,
      booleanOp,
      selfNode.params ?? {},
  ...
  if (compareOp && portId === "result") {
    return resolveCompareNodeResult(
      graph,
      nodeId,
      compareOp,
      selfNode.params ?? {},
  ...
  if (selfNode.type === BLUEPRINT_NODE_TYPE_BROADCAST_GET_LISTENER_COUNT) {
    return resolveBroadcastNodeOutput(
      graph,
      nodeId,
      portId,
      selfNode.params ?? {},
  ...
  const selfDisplayableOutput = resolveSelfDisplayableNodeOutput(
    selfNode.type,
    portId,
    selfNode.params ?? {},
    runtime
...
export function resolveDataPinValue(
  ...
  const edge = graph.edges?.find(
    (e) => e.to.nodeId === consumerNodeId && e.to.port === consumerPortId
  );
  if (!edge) {
