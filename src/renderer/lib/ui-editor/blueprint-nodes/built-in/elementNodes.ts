...
const DISPLAYABLE_WIDGET_TYPES = [
  "nl.container",
  "nl.text",
  "nl.image",
  "nl.button",
...
const DISPLAYABLE_ANIMATION_PROPERTIES = [
  "opacity",
  "offsetX",
  "offsetY",
  "x",
...
const DISPLAYABLE_ANIMATION_EASINGS = [
  "linear",
  "easeIn",
  "easeOut",
  "easeInOut",
...
const DISPLAYABLE_SET_PROPERTIES = [
  "x",
  "y",
  "offsetX",
  "offsetY",
...
const floatIn = (id: string, label: string): BlueprintNodePinDef =>
  dataIn(id, label, "float", true);
const booleanIn = (id: string, label: string): BlueprintNodePinDef => dataIn(id, label, "boolean");
const colorIn = (id: string, label: string): BlueprintNodePinDef =>
  dataIn(id, label, BLUEPRINT_VALUE_TYPE_RGBA_COLOR);
const jsonIn = (id: string, label: string): BlueprintNodePinDef => dataIn(id, label, "json");
const animationTokenIn = (
  id = DISPLAYABLE_ANIMATION_OUTPUT_PIN_ID,
  label = "Animation"
): BlueprintNodePinDef => dataIn(id, label, BLUEPRINT_VALUE_TYPE_ANIMATION_TOKEN);
...
const animationTokenOut = (
  id = DISPLAYABLE_ANIMATION_OUTPUT_PIN_ID,
  label = "Animation"
): BlueprintNodePinDef => out(id, label, BLUEPRINT_VALUE_TYPE_ANIMATION_TOKEN);
...
function displayableWriteNode(input: {
  ...
  return {
    ...
    magicElementTarget: elementTarget
      ...
      : undefined,
    pins: elementTarget
      ? [execIn, execNext, genericElementIn, ...(input.pins ?? [])]
    ...
    scope: elementTarget
      ...
      : {
          ...
          widgetElementTypes: [...(input.elementTypes ?? DISPLAYABLE_WIDGET_TYPES)]
        },
    execute: input.execute
  };
}
...
function createDisplayableAnimationToken(
  ctx: Parameters<BlueprintNodeDef["execute"]>[0]
): BlueprintAnimationToken {
...
function resolveElementId(
  ...
  if (!ref) {
    throw new BlueprintGraphExecutionError(
      "Element node requires a bound element input",
      ctx.node.id
    );
  }
  if (ref.elementType !== expectedElementType) {
    throw new BlueprintGraphExecutionError(
      `Element node expected ${expectedElementType}, got ${ref.elementType}`,
      ctx.node.id
    );
  }
  ...
  if (currentSurfaceId && ref.surfaceId !== currentSurfaceId) {
    throw new BlueprintGraphExecutionError(
      "Element node can only target the current Surface",
      ctx.node.id
    );
  }
...
function resolveDisplayableTargetElementId(
  ...
  if (target === "self") {
    ...
    if (!elementId) {
      throw new BlueprintGraphExecutionError(
        "Displayable node requires a widget execution owner",
        ctx.node.id
      );
    }
    return elementId;
  ...
  if (!ref) {
    throw new BlueprintGraphExecutionError(
      "Displayable Element node requires an Element input",
      ctx.node.id
    );
  }
  if (!allowedElementTypes.includes(ref.elementType)) {
    throw new BlueprintGraphExecutionError(
      `Displayable Element node cannot target ${ref.elementType}`,
      ctx.node.id
    );
  }
  const currentSurfaceId = ctx.executionOwner?.surfaceId;
  if (currentSurfaceId && ref.surfaceId !== currentSurfaceId) {
    throw new BlueprintGraphExecutionError(
      "Displayable Element node can only target the current Surface",
      ctx.node.id
    );
  }
  return ref.elementId;
...
async function setDisplayableVariant(
  ctx: Parameters<BlueprintNodeDef["execute"]>[0],
  target: "self" | "element"
) {
...
async function setDisplayableProperty(
  ctx: Parameters<BlueprintNodeDef["execute"]>[0],
  target: "self" | "element"
) {
  const api = requireHostApi(ctx);
  ...
  if (property === "visible") {
    await api.widget.setDisplayableProperties(elementId, {
      visible: toBooleanValue(raw, current.visible)
    });
    return { nextPort: "next" };
  }
  const fallback =
...
async function setDisplayableDisplay(
  ctx: Parameters<BlueprintNodeDef["execute"]>[0],
  target: "self" | "element"
) {
  const api = requireHostApi(ctx);
...
async function animateDisplayableProperty(
  ctx: Parameters<BlueprintNodeDef["execute"]>[0],
  target: "self" | "element"
) {
  const api = requireHostApi(ctx);
  ...
  const shouldCommitAbsolutePositionOnComplete =
    (property === "x" || property === "y") && !resetOnComplete;
...
async function stopDisplayableAnimation(ctx: Parameters<BlueprintNodeDef["execute"]>[0]) {
  const api = requireHostApi(ctx);
  const animation = normalizeBlueprintAnimationToken(
    readPin(ctx, DISPLAYABLE_ANIMATION_OUTPUT_PIN_ID)
  );
...
function readTargetText(ctx: Parameters<BlueprintNodeDef["execute"]>[0]): {
  elementId: string;
...
async function continueElementEventBubble(ctx: Parameters<BlueprintNodeDef["execute"]>[0]) {
  ...
  if (!runtime?.continueElementEventBubble) {
    throw new BlueprintGraphExecutionError(
      "Continue Event Bubble requires a UI event runtime",
      ctx.node.id
    );
  }
  const elementId = ctx.executionOwner?.elementId;
  if (!elementId) {
    throw new BlueprintGraphExecutionError(
      "Continue Event Bubble requires an element event owner",
      ctx.node.id
    );
  }
  ...
  if (!eventName) {
    throw new BlueprintGraphExecutionError(
      "Continue Event Bubble requires an active event",
      ctx.node.id
    );
  }
...
function isElementEventBubbleStopped(ctx: Parameters<BlueprintNodeDef["execute"]>[0]): boolean {
  return Boolean(
    ctx.eventControl?.isPropagationStopped() ||
    ctx.blueprintLocals?.[EVENT_BUBBLE_STOPPED_LOCAL_KEY]
  );
}
...
function stopElementEventBubble(ctx: Parameters<BlueprintNodeDef["execute"]>[0]) {
  ...
  if (!eventName) {
    throw new BlueprintGraphExecutionError(
      "Stop Event Bubble requires an active event",
      ctx.node.id
    );
  }
...
export const elementBlueprintNodes: BlueprintNodeDef[] = [
  ...
  textWriteNode({
    ...
    execute: (ctx) => {
      ...
      return patchTargetText(ctx, {
        text: `${current.text}${toStringValue(readPin(ctx, "text"), "")}`
      });
    }
  }),
  textWriteNode({
  ...
  textWriteNode({
    ...
    execute: (ctx) =>
      patchTargetText(ctx, { fontAssetId: toFontAssetId(readPin(ctx, "fontAssetId"), null) })
  }),
  textReadNode({
  ...
  textWriteNode({
    ...
    execute: (ctx) =>
      patchTargetText(ctx, { fontSize: Math.max(1, toFiniteNumber(readPin(ctx, "fontSize"), 16)) })
  }),
  textReadNode({
  ...
  textWriteNode({
    ...
    execute: (ctx) =>
      patchTargetText(ctx, {
        lineHeight: Math.max(0.1, toFiniteNumber(readPin(ctx, "lineHeight"), 1.4))
      })
  }),
  textReadNode({
