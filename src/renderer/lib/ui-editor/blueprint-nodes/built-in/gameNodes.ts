...
function createPreferenceDataPin(
 ...
 */
function resolveSaveMetadataToWrite(
  ctx: Parameters<NonNullable<BlueprintNodeDef["execute"]>>[0]
): unknown {
...
function resolveSaveScreenshot(
  ctx: Parameters<NonNullable<BlueprintNodeDef["execute"]>>[0]
): boolean {
  const value = resolveDataPinValue(
    ctx.graph,
...
function hasDataInputValue(
  ctx: Parameters<NonNullable<BlueprintNodeDef["execute"]>>[0],
  portId: string
): boolean {
  return (
    Object.prototype.hasOwnProperty.call(ctx.params, portId) ||
    ctx.graph.edges?.some((edge) => edge.to.nodeId === ctx.node.id && edge.to.port === portId) ===
      true
  );
}
...
function resolveSentenceCps(ctx: Parameters<NonNullable<BlueprintNodeDef["execute"]>>[0]): number {
  const portId =
    hasDataInputValue(ctx, "cps") || !hasDataInputValue(ctx, "speed") ? "cps" : "speed";
  const value = resolveDataPinValue(
    ctx.graph,
    ctx.node.id,
    portId,
...
function resolvePreferenceValue(
  ...
  const resolvedValue = resolveDataPinValue(
    ctx.graph,
    ctx.node.id,
    meta.pinId,
    ctx.params,
  ...
  if (meta.valueType === "string") {
    ...
    if (meta.key === "voiceEndMode" && text !== "fade" && text !== "stop" && text !== "none") {
      throw new BlueprintGraphExecutionError(
        'Voice End Mode must be "fade", "stop", or "none"',
        ctx.node.id
      );
    }
...
const gamePreferenceBlueprintNodes: BlueprintNodeDef[] = GAME_PREFERENCE_NODE_META.flatMap(
  (meta) => {
    const setterNode = createPreferenceSetterNode(meta);
    return setterNode
      ? [createPreferenceGetterNode(meta), setterNode]
      : [createPreferenceGetterNode(meta)];
  }
);
