...
describe("visited blueprint nodes", () => {
    ...
    it("keeps both readers pure and non-latent, so a function graph still accepts them", () => {
        ...
        for (const type of [BLUEPRINT_NODE_TYPE_GAME_IS_SCENE_VISITED, BLUEPRINT_NODE_TYPE_GAME_IS_OPTION_PICKED]) {
    ...
    it("reads Is Scene Visited as true for a visited scene and false for one never entered", async () => {
        ...
        await expect(readPin(BLUEPRINT_NODE_TYPE_GAME_IS_SCENE_VISITED, "isVisited", { sceneId: "scene-1" }, state))
            .resolves.toBe(true);
        await expect(readPin(BLUEPRINT_NODE_TYPE_GAME_IS_SCENE_VISITED, "isVisited", { sceneId: "scene-2" }, state))
            .resolves.toBe(false);
    });
    ...
    it("reads Is Option Picked as true only for an option that was actually picked", async () => {
        ...
        await expect(readPin(BLUEPRINT_NODE_TYPE_GAME_IS_OPTION_PICKED, "isPicked", { optionId: "opt-a" }, state))
            .resolves.toBe(true);
        // `opt-b` sat in the same menu as `opt-a`; being shown is not being picked.
        await expect(readPin(BLUEPRINT_NODE_TYPE_GAME_IS_OPTION_PICKED, "isPicked", { optionId: "opt-b" }, state))
            .resolves.toBe(false);
    });
    ...
    it("reads false rather than undefined when nothing is picked in the inspector", async () => {
        ...
        await expect(readPin(BLUEPRINT_NODE_TYPE_GAME_IS_SCENE_VISITED, "isVisited", {}, EMPTY)).resolves.toBe(false);
        await expect(readPin(BLUEPRINT_NODE_TYPE_GAME_IS_OPTION_PICKED, "isPicked", {}, EMPTY)).resolves.toBe(false);
    });
