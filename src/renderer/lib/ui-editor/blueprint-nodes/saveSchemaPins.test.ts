...
import { resolveEffectiveBlueprintNodePins, saveSchemaFieldIdFromPin, saveSchemaPinId } from "./effectivePins";
...
describe("save schema pins", () => {
    ...
    it("grows one input pin per field on Save Game, in schema order", () => {
        ...
        expect(grown.map(pin => pin.id)).toEqual([saveSchemaPinId("f-chapter"), saveSchemaPinId("f-play-time")]);
    ...
    it("grows the same fields as outputs on Get Save Metadata", () => {
        setActiveSaveSchemaFields([CHAPTER, PLAY_TIME]);
        const pins = resolveEffectiveBlueprintNodePins(def(BLUEPRINT_NODE_TYPE_GAME_SAVE_GET_METADATA), {});
        const grown = pins.filter(pin => saveSchemaFieldIdFromPin(pin.id) !== null);
        expect(grown.map(pin => pin.id)).toEqual([saveSchemaPinId("f-chapter"), saveSchemaPinId("f-play-time")]);
    ...
    it("keeps the raw metadata pin on both nodes once fields exist", () => {
        ...
        expect(resolveEffectiveBlueprintNodePins(def(BLUEPRINT_NODE_TYPE_GAME_SAVE_WRITE), {})
            .some(pin => pin.id === "metadata" && pin.kind === "input")).toBe(true);
        expect(resolveEffectiveBlueprintNodePins(def(BLUEPRINT_NODE_TYPE_GAME_SAVE_GET_METADATA), {})
            .some(pin => pin.id === "metadata" && pin.kind === "output")).toBe(true);
    });

    it("addresses a pin by field id, so a rename does not move it", () => {
        setActiveSaveSchemaFields([CHAPTER]);
        const before = resolveEffectiveBlueprintNodePins(def(BLUEPRINT_NODE_TYPE_GAME_SAVE_WRITE), {})
            .find(pin => saveSchemaFieldIdFromPin(pin.id) !== null);
        setActiveSaveSchemaFields([{ ...CHAPTER, name: "Act", storageKey: "chapter" }]);
        const after = resolveEffectiveBlueprintNodePins(def(BLUEPRINT_NODE_TYPE_GAME_SAVE_WRITE), {})
            .find(pin => saveSchemaFieldIdFromPin(pin.id) !== null);
