...
export function isBlueprintNodeAllowedInBlueprintValueGraph(
  def: BlueprintNodeGraphContextDef
): boolean {
  if (def.role === "eventHead") {
    return (
      def.type === BLUEPRINT_NODE_TYPE_EVENT_HEAD_INIT ||
...
function matchesBlueprintNodeScope(
  def: BlueprintNodeGraphContextDef,
  ctx: BlueprintPaletteContext
): boolean {
...
function listCompatibleMagicElementRefs(
  ...
  return refs
    ...
    .map((ref) =>
      ref.targetPortId === target.inputPinId ? ref : { ...ref, targetPortId: target.inputPinId }
    );
}
...
class BlueprintNodeDefinitionsRegistry {
   ...
   */
  public resolveCatalogEntryForNode(
    type: string,
  ...
  private validateDynamicInputPins(def: BlueprintNodeDef): void {
    ...
    if (!d.storageKey.trim()) {
      throw new Error(
        `[BlueprintNodeRegistry] Node ${def.type} dynamicInputPins.storageKey is empty`
      );
    }
    if (!d.generatedIdPrefix.trim()) {
      throw new Error(
        `[BlueprintNodeRegistry] Node ${def.type} dynamicInputPins.generatedIdPrefix is empty`
      );
    }
    if (d.pinLabelParamKey !== undefined && !d.pinLabelParamKey.trim()) {
      throw new Error(
        `[BlueprintNodeRegistry] Node ${def.type} dynamicInputPins.pinLabelParamKey is empty`
      );
    }
    if (d.pinValueTypeParamKey !== undefined && !d.pinValueTypeParamKey.trim()) {
      throw new Error(
        `[BlueprintNodeRegistry] Node ${def.type} dynamicInputPins.pinValueTypeParamKey is empty`
      );
    }
    if (d.pinValueTypeOptions !== undefined && d.pinValueTypeOptions.length === 0) {
      throw new Error(
        `[BlueprintNodeRegistry] Node ${def.type} dynamicInputPins.pinValueTypeOptions is empty`
      );
    }
