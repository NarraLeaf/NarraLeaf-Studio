...
function readElementInputRef(input: {
  ...
  const edge = input.ir.edges?.find(
    (item) =>
      item.to.nodeId === input.nodeId && item.to.port === "element" && item.from.port === "element"
  );
...
export function resolveBlueprintSetFramePageTargetContext(input: {
  ...
  if (
    input.nodeType === BLUEPRINT_NODE_TYPE_FRAME_WIDGET_SET_PAGE &&
    input.owner.kind === "widgetMain"
  ) {
