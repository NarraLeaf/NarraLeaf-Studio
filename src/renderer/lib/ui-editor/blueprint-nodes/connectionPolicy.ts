...
function resolvePinValueType(input: {
  ...
  if (
    ...
    return (
      readParamString(input.params, BLUEPRINT_NODE_PARAM_VARIABLE_VALUE_TYPE) ?? input.pinValueType
    );
  }
  return input.pinValueType;
}

function areDataValueTypesCompatible(
...
export function isValidBlueprintPinConnection(params: {
  ...
  const src = blueprintNodeRegistry.resolveCatalogEntryForNode(
    params.sourceType,
    params.sourceParams
  );
  const tgt = blueprintNodeRegistry.resolveCatalogEntryForNode(
    params.targetType,
    params.targetParams
  );
