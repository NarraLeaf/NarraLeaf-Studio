...
describe("computeZoomedViewportTransform", () => {
  ...
  it("anchors on the free box, not the whole pane, so the chrome does not drag the view", () => {
    ...
    const withoutInsets = computeZoomedViewportTransform({
      current,
      container: CONTAINER,
      nextScale: 2
    })!;
  ...
  it("holds the zoom inside the range the viewport accepts", () => {
    const current = { scale: 1, offsetX: 0, offsetY: 0 };
    expect(
      computeZoomedViewportTransform({ current, container: CONTAINER, nextScale: 40 })!.scale
    ).toBe(SURFACE_ZOOM_MAX_SCALE);
    expect(
      computeZoomedViewportTransform({ current, container: CONTAINER, nextScale: 0.001 })!.scale
  ...
  it("refuses a pane it cannot measure, or a zoom that is not a number", () => {
    const current = { scale: 1, offsetX: 0, offsetY: 0 };
    expect(
      computeZoomedViewportTransform({ current, container: { width: 0, height: 0 }, nextScale: 2 })
    ).toBeNull();
    expect(
      computeZoomedViewportTransform({ current, container: CONTAINER, nextScale: Number.NaN })
    ).toBeNull();
  });
});
...
describe("areViewportTransformsEqual", () => {
  ...
  it("separates transforms a viewer would see apart", () => {
    expect(
      areViewportTransformsEqual(
        { scale: 0.5, offsetX: 100, offsetY: 40 },
        { scale: 0.51, offsetX: 100, offsetY: 40 }
      )
    ).toBe(false);
    expect(
      areViewportTransformsEqual(
        { scale: 0.5, offsetX: 100, offsetY: 40 },
        { scale: 0.5, offsetX: 108, offsetY: 40 }
      )
    ).toBe(false);
  });
});
