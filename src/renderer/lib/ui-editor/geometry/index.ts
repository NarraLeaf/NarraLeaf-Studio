...
function mapRect(
  value: Rect2D,
  mapper: (point: Point2D) => Point2D,
  sizeMapper?: (size: { width: number; height: number }) => { width: number; height: number }
): Rect2D {
  const point = mapper({ x: value.x, y: value.y });
  const size = sizeMapper
    ? sizeMapper({ width: value.width, height: value.height })
...
export function viewportToSurface(
  value: Point2D | Rect2D,
