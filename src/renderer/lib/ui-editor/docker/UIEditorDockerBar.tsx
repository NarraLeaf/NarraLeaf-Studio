...
function PaletteDockerBar({
  ...
  const overflowActive =
    componentsActive || overflowEntries.some((entry) => entry.module.type === activeInsertType);
          ...
          return (
            <button
              ...
              className={`flex h-8 w-full items-center gap-2 px-3 text-left text-xs transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
                isActive ? "bg-primary/20 text-fg" : "text-fg-muted hover:bg-fill hover:text-fg"
              }`}
              data-tip={
                readOnly.active
                  ? readOnly.reason
    ...
    case "number": {
      return (
        <div
          ...
          onMouseDownCapture={stopPointerPropagation}
        >
          {item.label && <span className="text-2xs text-fg-subtle select-none">{item.label}</span>}
          <DeferredNumberInput
            value={item.value}
...
function wrapMultiSelectItem(base: DockerBarItem, sources: DockerBarItem[]): DockerBarItem | null {
  switch (base.kind) {
    case "button": {
      const buttonSources = sources.filter(
        (source) => source.kind === "button"
      ) as DockerBarButton[];
    ...
    case "select": {
      const selectSources = sources.filter(
        (source) => source.kind === "select"
      ) as DockerBarSelect[];
    ...
    case "number": {
      const numberSources = sources.filter(
        (source) => source.kind === "number"
      ) as DockerBarNumberInput[];
...
function ElementDockerBar({ items, moduleName }: { items: DockerBarItem[]; moduleName: string }) {
  return (
...
function MultiSelectDockerBar({ items }: { items: DockerBarItem[] }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <span className="text-2xs text-fg-subtle font-medium mr-1 select-none">
        {t("widgetChrome.docker.multiple")}
      </span>
      <div className="w-px h-5 bg-fill mx-0.5" />
      {items.map((item) => (
...
function ComponentPreview({
  ...
  const scale =
    frameWidth > 0 ? Math.min(frameWidth / designWidth, previewHeight / designHeight) : 0;
  ...
  return (
    <div
      ref={frameRef}
      className="h-[104px] overflow-hidden rounded-md border border-edge bg-surface-canvas"
    >
            ...
            <div className="flex h-14 shrink-0 items-center gap-3 border-b border-edge px-6">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-fg">
                  {t("widgetChrome.docker.componentLibrary")}
                </div>
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
              {filtered.length === 0 ? (
                <div className="flex h-full items-center justify-center text-sm text-fg-subtle">
                  {components.length === 0
                    ? t("widgetChrome.docker.noComponents")
                    : t("widgetChrome.docker.noMatchingComponents")}
                </div>
              ) : (
                  ...
                  {filtered.map((component) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div className="mt-2 truncate text-xs font-medium text-fg">
                          {component.name}
                        </div>
...
export function UIEditorDockerBar({
  ...
  const multiSelectItems = useMemo<DockerBarItem[]>(() => {
    ...
    const itemGroups = selectedElements.map((element) => {
      ...
      return (
        mod.createMultiSelectDockerBarItems?.(context) ?? mod.createDockerBarItems?.(context) ?? []
      );
    });
    ...
    for (const baseItem of firstGroup) {
      ...
      for (const group of restGroups) {
        const match = group.find(
          (other) => other.id === baseItem.id && other.kind === baseItem.kind
        );
        if (!match) {
          missing = true;
          break;
