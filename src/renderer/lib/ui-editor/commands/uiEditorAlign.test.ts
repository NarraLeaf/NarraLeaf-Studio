...
describe("UI editor distribute", () => {
    ...
    it("equalises the gaps between three boxes, leaving the extremes put", () => {
        ...
        expect(patches(makeDocument(), ["a", "b", "c"], "distributeHorizontal")).toEqual({ b: { x: 225 } });
    });
    ...
    it("equalises unequal gaps across four boxes", () => {
        ...
        const result = computeUiEditorAlignPatches(doc, "surface", selection(["a", "b", "c", "panel"]), "distributeHorizontal");
    ...
    it("orders by position, not by selection order", () => {
        ...
        const result = computeUiEditorAlignPatches(doc, "surface", selection(["a", "b", "c"]), "distributeHorizontal");
    ...
    it("distributes vertically on the y axis", () => {
        ...
        const result = computeUiEditorAlignPatches(doc, "surface", selection(["a", "b", "c"]), "distributeVertical");
...
describe("UI editor align, negative extents", () => {
    ...
    it("measures a negative-height box by its absolute extent", () => {
        ...
        const result = computeUiEditorAlignPatches(doc, "surface", selection(["a", "b", "c"]), "bottom");
...
describe("UI editor align, excluded elements", () => {
    ...
    it("skips slider track and handle slots", () => {
        const doc = makeDocument();
        doc.elements.panel = element("panel", "nl.slider", "root", { x: 300, y: 400, width: 200, height: 100 }, [
            "inner",
        ]);
...
describe("UI editor align, idempotence and guards", () => {
    ...
    it("ignores a selection that belongs to another surface", () => {
        const doc = makeDocument();
        expect(computeUiEditorAlignPatches(doc, "surface", selection(["a", "b", "c"], "other"), "left")).toEqual({});
...
describe("uiEditorAlign write path", () => {
    ...
    it("writes every moved element in one call", () => {
        ...
        expect(uiEditorAlign(documentService, "surface", selection(["a", "b", "c"]), "horizontalCenter")).toBe(true);
    ...
    it("writes nothing when the selection is already aligned", () => {
        ...
        expect(uiEditorAlign(documentService, "surface", selection(["a", "b", "c"]), "left")).toBe(false);
    ...
    it("writes nothing for a selection on another surface", () => {
        ...
        expect(uiEditorAlign(documentService, "surface", selection(["a", "b", "c"], "other"), "left")).toBe(false);
        expect(updateElementLayouts).not.toHaveBeenCalled();
    });
});
