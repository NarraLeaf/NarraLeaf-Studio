...
import { sortElementIdsByPreorder } from "@/lib/workspace/services/ui-editor/uiDocumentTreeMove";
...
function moversAlreadyAtFrontEnd(
  childrenIds: readonly string[],
  moverSet: ReadonlySet<string>
): boolean {
  const idxs: number[] = [];
  for (let i = 0; i < childrenIds.length; i++) {
 ...
 * @internal Exported for unit tests
 */
export function computeBringForwardOrder(
  childrenIds: readonly string[],
 ...
 * @internal Exported for unit tests
 */
export function computeSendBackwardOrder(
  childrenIds: readonly string[],
  blockIds: readonly string[]
): string[] | null {
  const set = new Set(blockIds);
  ...
  if (op === "bringToFront" || op === "sendToBack") {
    for (const [parentId, ids] of byParent) {
      ...
      if (op === "bringToFront") {
        ...
        const r = documentService.moveElementsInSurface(
          surfaceId,
          ordered,
          parentId,
          firstNonMover
