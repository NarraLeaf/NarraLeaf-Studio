...
import {
  isButtonAppearanceKey,
  isContainerAppearanceKey,
  isTextAppearanceKey
} from "./appearanceWhitelist";
...
function pickLastMatchingRowValue(
...
function patchRectangleLikeEffects(
  target: RectangleLikeProps,
  patch: Partial<ElementEffectValues>
): void {
...
function applyContainerKey(
  ...
  switch (key) {
    ...
    case "effectInnerShadow": {
      patchRectangleLikeEffects(target, {
        effectInnerShadow: normalizeElementEffectValues({ effectInnerShadow: raw })
          .effectInnerShadow
      });
      break;
    }
...
function applyButtonKey(
  target: ButtonResolvedVisualProps,
...
function applyTextKey(
  target: TextResolvedVisualProps,
...
function isUsableAppearance(
  appearance: AppearanceModel | null | undefined
): appearance is AppearanceModel {
...
export function resolveAppearanceDisplayableOpacity(
  ...
  const hasConditions = Boolean(
    resolved.row.conditions && Object.keys(resolved.row.conditions).length > 0
  );
...
export function resolveImageRectangleLike(
  ...
  const defaultVariant =
    ...
    null;
  const next: RectangleLikeProps = { ...baseline };
  const defaultImageFillGroup = defaultVariant?.propertyGroups.find(
    (group) => group.key === "imageFill"
  );
  ...
  if (next.fillType === "color") {
    ...
    const raw = fillOpacityGroup
      ? pickLastMatchingRowValue(fillOpacityGroup.rows, ctx.signals)
      : undefined;
    if (raw !== undefined) {
