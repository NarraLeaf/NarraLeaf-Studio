...
export class WidgetRuntimeStateStore {
  ...
  setTextInputProperties(
    ...
    const current =
      this.textInputProperties.get(elementId) ?? resolveTextInputRuntimeValue(currentProps);
  ...
  clearDisplayableMotionById(
    motionId: string
  ): { elementId: string; motion: UIDisplayableMotionOverride } | null {
  ...
  getSignalsForElement(
    elementId: string,
    interactionDisabled: boolean | undefined
  ): SystemInteractionSignals {
    return {
