...
export function buildSurfaceAnimationPlan(input: PlanInput): SurfaceAnimationPlan {
  ...
  const visit = (
    ...
    const enterDurationMs = durationMs(
      settings.enter,
    ...
    const enterStartMs =
      enterOriginMs +
      (enterDurationMs > 0 ? delayMs(settings.enterDelaySeconds, reducedMotion) : 0);
 ...
 */
export function getSurfaceAnimationPlan(
  input: PlanInput & { cache?: boolean }
): SurfaceAnimationPlan {
  if (input.cache !== true) {
...
export function getSurfaceAnimationTimings(input: {
  ...
  const enterMs = plan
    ? plan.enterTotalMs
    : getPageAnimationDurationMs(settings, "enter", reducedMotion);
  const exitMs = plan
    ? plan.exitTotalMs
    : getPageAnimationDurationMs(settings, "exit", reducedMotion);
  return {
    plan,
    enterMs,
    exitMs,
