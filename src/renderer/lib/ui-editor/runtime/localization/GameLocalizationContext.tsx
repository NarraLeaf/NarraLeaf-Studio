...
export function useLocalizedWidgetText(input: LocalizedWidgetTextInput): string {
  ...
  if (keyName) {
    return (
      resolveLocalizedUnitText(runtime.bundle, locale, localizationKeyUnitId(keyName)) ??
  ...
  return (
    resolveLocalizedUnitText(runtime.bundle, locale, uiTextUnitId(input.elementId, input.prop)) ??
    input.sourceText
  );
}
