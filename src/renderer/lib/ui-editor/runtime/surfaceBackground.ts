...
export function getSurfaceBackgroundColor(surface: UISurface): string {
  const stored =
    ...
    (surface.kind === "stageSurface" ? "transparent" : "#ffffff");
...
function parseAlpha(value: string | undefined): number | null {
  ...
  const alpha = trimmed.endsWith("%") ? Number(trimmed.slice(0, -1)) / 100 : Number(trimmed);
...
export function getEditorSurfaceAreaBackgroundColor(surface: UISurface): string | undefined {
  ...
  return isTransparentBackground(getSurfaceBackgroundColor(surface))
    ? EDITOR_SURFACE_AREA_BACKGROUND
    : undefined;
}
