...
describe("puppetPackRuntimes", () => {
  ...
  it("answers null for a backend this game did not publish", () => {
    ...
    expect(
      listPackPuppetBackendSources(fakeBridge(), fakePack({ puppetRuntimes: undefined }))
    ).toEqual([]);
  ...
  it("confines resolveFile to the backend's own directory", async () => {
    ...
    await expect(source.resolveFile("./core.wasm")).resolves.toBe(
      "nlgame://runtime/puppet/renderer-a/core.wasm"
    );
    await expect(source.resolveFile("nested\\page-0.png")).resolves.toBe(
      "nlgame://runtime/puppet/renderer-a/nested/page-0.png"
    );
  ...
  it("points a model bundle at its entry file, not at its asset id", () => {
    ...
    expect(resolvePackModelBundleUrl(fakeBridge(), fakePack(), "model-alice")).toBe(
      "nlgame://asset/model-alice/alice/alice.model.json"
    );
  });

  it("falls back to the bare id for an asset that declares no bundle entry", () => {
    expect(resolvePackModelBundleUrl(fakeBridge(), fakePack(), "plain-image")).toBe(
      "nlgame://asset/plain-image"
    );
  });
