...
const say = (id: string, value = "Text", parentId: string | null = null) =>
  block(
    id,
    "nodeAction",
    { action: "narration", text: { textId: `${id}-text`, value, role: "narration" } },
    parentId
  );
...
describe("compileStagePreviewToNlr", () => {
  ...
  it("orders statements as seeds, injection script, markers, and target", async () => {
    ...
    expect(types[2].some((type) => type?.includes("say") || type?.includes("character"))).toBe(
      true
    );
  ...
  it("inserts the posed marker and reveal gate between the pose and the before-marker", async () => {
    ...
    expect(types[3]).toEqual(["script:action"]);
    expect(types[4].some((type) => type?.includes("say") || type?.includes("character"))).toBe(
      true
    );
    ...
    expect(compiled.diagnostics).toEqual([
      {
        level: "warning",
        blockId: "jump",
        message: "Preview holds before the jump instead of leaving the scene."
      }
    ]);
  });
  ...
  it("keeps the full menu when the target is a choice block", async () => {
    const blocks: Record<string, StoryBlock> = {
      choice: block(
        "choice",
        "nodeAction",
        { action: "choice", prompt: { textId: "t", value: "Pick", role: "choicePrompt" } },
        null,
        ["option"]
      ),
      option: block(
        "option",
        "nodeAction",
        { action: "choiceOption", text: { textId: "t2", value: "Go", role: "choiceText" } },
        "choice",
        ["option-say"]
      ),
    ...
    expect(
      statements.some((statement) => Array.isArray((statement as { choices?: unknown })?.choices))
    ).toBe(true);
  });
  ...
  describe("continuous playback", () => {
      ...
      const document = baseDocument(
        {
          ...
          optA: block(
            "optA",
            "nodeAction",
            { action: "choiceOption", text: { textId: "ta", value: "A", role: "choiceText" } },
            "choice",
            ["a-say"]
          ),
          ...
          optB: block(
            "optB",
            "nodeAction",
            { action: "choiceOption", text: { textId: "tb", value: "B", role: "choiceText" } },
            "choice",
            ["b-say"]
          ),
    ...
    it("holds before a scene jump and reports where playback ended", async () => {
      ...
      expect(compiled.playbackStop).toEqual({
        reason: "jump",
        blockId: "jump",
      ...
      expect(compiled.diagnostics).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            level: "warning",
            blockId: "jump",
            message: expect.stringContaining("Scene 2")
          })
        ])
      );
    });
