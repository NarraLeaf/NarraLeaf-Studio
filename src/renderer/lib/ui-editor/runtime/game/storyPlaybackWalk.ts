...
export function collectStoryPlaybackPlan(
  ...
  while (cursor && guard++ < MAX_STEPS) {
    const parent: StoryBlock | undefined = cursor.parentId
      ? scene.blocks[cursor.parentId]
      : undefined;
...
class PlanBuilder {
  ...
  constructor(
    ...
    private readonly followJumps: boolean = false
  ) {}
 ...
 */
export function groupPlaybackStepsByNvl(
  steps: readonly StoryPlaybackStep[]
): { insideNvl: boolean; steps: StoryPlaybackStep[] }[] {
