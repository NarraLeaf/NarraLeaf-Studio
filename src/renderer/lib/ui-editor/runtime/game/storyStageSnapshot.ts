...
export type StoryStageSnapshot = {
  ...
  builtinLayerProps: {
    ...
    displayableLayer: Record<string, unknown>;
  };
...
export function collectSavedVariableView(
  ...
  return buildMergedVariableView(
    Object.values(savedVariables ?? {}),
 ...
 */
export function savedVariableDefsFromView(
...
export function computeStoryStageSnapshot(input: {
  ...
  const animations =
    input.animations instanceof Map
      ? input.animations
...
class SnapshotWalker {
  ...
  private builtinLayerProps = {
    backgroundLayer: {} as Record<string, unknown>,
    displayableLayer: {} as Record<string, unknown>
  };
  ...
  run(): StoryStageSnapshot {
    if (this.targetBlockId === null) {
      ...
      if (!this.reachedTarget) {
        this.diagnostic(
          this.targetBlockId,
          translate("story.preview.diagnostics.targetUnreachable")
        );
      }
    }
    return {
  ...
  private visitBlock(blockId: string, insideNvl: boolean): void {
    ...
    if (block.kind === "control") {
      ...
      if (
        ...
        (block.payload.until !== undefined || (block.payload.times ?? 1) !== 1)
      ) {
  ...
  private visitCondition(
    ...
    for (const branch of branches) {
      ...
      if (
        ...
        this.evaluateCondition(branch.payload.condition, branch.id)
      ) {
        this.visitList(branch.childrenIds, insideNvl);
        return;
      }
  ...
  private evaluateCondition(condition: StoryConditionRef | undefined, blockId: string): boolean {
    ...
    if (condition.kind === "expression") {
      if (!isStoryExpressionEvaluable(condition.expression.ast)) {
        this.diagnostic(
          blockId,
          translate("story.preview.diagnostics.conditionUnresolved", {
            expression: condition.expression.source
          })
        );
        return false;
      }
      ...
      return isTruthy(
        evaluateStoryExpression(condition.expression.ast, this.expressionEnv(blockId))
      );
    }
  ...
  private applyCharacter(
    ...
    if (payload.operation === "exit") {
      ...
      record.props = mergeTransformProps(
        record.props,
        this.finalProps(
    ...
    if (payload.operation === "move") {
      record.props = mergeTransformProps(
        record.props,
        this.finalProps(payload.transform, "none", block.id)
      );
      return;
    }
    ...
    if (payload.operation === "enter") {
      ...
      record.props = mergeTransformProps(
        record.props,
        this.finalProps(payload.transform, "show", block.id)
      );
    }
  }
  ...
  private applyImage(
    ...
    if (payload.operation === "show" || payload.operation === "create") {
      ...
      record.props = mergeTransformProps(
        record.props,
        this.finalProps(payload.transform, "hide", block.id)
      );
    }
  }
  ...
  private applyLayer(
    ...
    if (
      ...
      const visibility: VisibilityTransformMode =
        payload.operation === "transform" ? "none" : payload.operation;
      ...
      if (targetProps.record) {
        ...
        this.builtinLayerProps[targetProps.builtin] = mergeTransformProps(
          this.builtinLayerProps[targetProps.builtin],
          props
        );
      }
    }
  ...
  private applyCamera(
    ...
    switch (payload.operation) {
      case "motion":
        ...
        camera.props = mergeTransformProps(
          camera.props,
          this.finalProps(payload.motion, "none", block.id)
        );
        return;
      case "pan":
  ...
  private resolveLayerTargetProps(payload: Extract<StoryActionPayload, { action: "layer" }>): {
    ...
    if (target?.kind === "custom") {
      ...
      const name =
        source?.kind === "action" && source.payload.action === "layer"
  ...
  private applyDisplayable(
    ...
    if (operation === "show" || operation === "hide" || operation === "transform") {
      ...
      if (bucket.record) {
        ...
        this.builtinLayerProps[bucket.builtinLayer] = mergeTransformProps(
          this.builtinLayerProps[bucket.builtinLayer],
          props
        );
      }
      return;
  ...
  private resolveDisplayableBucket(
    payload: Extract<StoryActionPayload, { action: "displayable" }>,
    blockId: string
  ): {
    record?: StageSnapshotDisplayable;
    ...
    const record = this.displayables.get(
      this.key(kind === "text" ? "text" : kind === "layer" ? "layer" : "image", resolved.name)
    );
    if (!record) {
      this.diagnostic(
        blockId,
        translate("story.preview.diagnostics.displayableNotFound", {
          target:
            resolved.label ||
            resolved.name ||
  ...
  private resolveAssignedValue(
    ...
    if (!isStoryExpressionEvaluable(payload.expression.ast)) {
      this.diagnostic(
        block.id,
        translate("story.preview.diagnostics.assignmentUnresolved", {
          expression: payload.expression.source
        })
      );
      return undefined;
    }
  ...
  private expressionEnv(blockId: string): StoryExpressionEnv {
    return {
      ...
      invoke: (_blueprintId, name) => {
        this.diagnostic(
          blockId,
          translate("story.preview.diagnostics.blueprintCallEmpty", { name })
        );
        return undefined;
      }
    };
  ...
  private finalProps(
    ...
    return storyTransformRefFinalProps(transform, visibility, this.animations, (message) =>
      this.diagnostic(blockId, message)
    );
  }
