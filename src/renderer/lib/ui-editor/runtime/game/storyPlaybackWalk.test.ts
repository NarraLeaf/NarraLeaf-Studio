...
describe("collectStoryPlaybackPlan", () => {
  ...
  it("skips sibling condition branches when climbing out of one", () => {
    const scene = makeScene(
      {
        ...
        i1: say("i1", "ifB"),
        elseB: block("elseB", "control", { control: "conditionBranch", branch: "else" }, "cond", [
          "e1"
        ]),
        e1: say("e1", "elseB"),
        after: say("after")
    ...
    expect(collectStoryPlaybackPlan(scene, "elseB").steps[0]).toMatchObject({
      blockId: "elseB",
      bodyOnly: true
    });
  ...
  it("stops immediately when playback starts on a jump row", () => {
    const scene = makeScene({ jump: block("jump", "jump", { targetSceneId: "scene-2" }) }, [
      "jump"
    ]);
