...
function walkActions(root: AnyAction, story: unknown, seen = new Set<AnyAction>()): AnyAction[] {
  ...
  seen.add(root);
  const children =
    typeof root.getFutureActions === "function"
...
function choiceDocument(options: { sceneName?: string; optionAText?: string } = {}): StoryDocument {
  const blocks: Record<string, StoryBlock> = {
    choice: {
      ...
      payload: {
        action: "choice",
        prompt: { textId: "t-prompt", value: "Which?", role: "choicePrompt" }
      }
    },
    "opt-a": {
      id: "opt-a",
    ...
    "opt-b": {
      ...
      payload: {
        action: "choiceOption",
        text: { textId: "t-b", value: "Leave", role: "choiceText" }
      }
    },
    leave: {
      id: "leave",
...
describe("visited record - compiler", () => {
  ...
  it("records the scene id when the scene starts", async () => {
    ...
    expect(fake.read(compiled.visitedNamespaceName, STORY_VISITED_SCENES_KEY)).toEqual([
      "scene-1",
      "scene-2"
    ]);
  });
  ...
  it("records an option only when it is PICKED, not when the menu shows it", async () => {
    const { compiled, sceneHeadScript, optionHead, optionCount } =
      await compileAndDissect(choiceDocument());
    const fake = createFakeStorable();
    ...
    expect(fake.read(compiled.visitedNamespaceName, STORY_VISITED_OPTIONS_KEY)).not.toContain(
      "opt-b"
    );
  });
  ...
  it("keys on ids, so renaming the scene and rewriting the option text hits the same record", async () => {
    ...
    const after = await compileAndDissect(
      choiceDocument({
    ...
    expect(afterStore.read(namespace, STORY_VISITED_SCENES_KEY)).toEqual(
      beforeStore.read(namespace, STORY_VISITED_SCENES_KEY)
    );
    expect(afterStore.read(namespace, STORY_VISITED_OPTIONS_KEY)).toEqual(
      beforeStore.read(namespace, STORY_VISITED_OPTIONS_KEY)
    );
