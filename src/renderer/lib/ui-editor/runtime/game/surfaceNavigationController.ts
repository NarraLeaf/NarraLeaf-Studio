...
export function createSurfaceNavigationOpenUpdate<Entry extends SurfaceNavigationEntry>(input: {
  ...
  const currentVisual =
    input.activeEntry ?? input.visibleEntries[input.visibleEntries.length - 1] ?? null;
  ...
  return {
    ...
    pendingUnderlayReadyKey:
      waitForExit || input.currentHiddenForGame || holdCurrentUntilEnterComplete
        ? null
        : nextEntry.key,
...
export function createSurfaceNavigationCloseUpdate<Entry extends SurfaceNavigationEntry>(input: {
   ...
   * a lower one drops every page above it in a single transition, which is what clearing the page
  ...
  const targetIndex = Math.max(
    0,
    Math.min(input.targetIndex ?? input.navStack.length - 2, input.navStack.length - 2)
  );
  ...
  return {
    ...
    pendingWaitEntry: waitForExit ? nextEntry : null,
    pendingUnderlayReadyKey:
      waitForExit || input.targetHiddenForGame || holdCurrentUntilEnterComplete
        ? null
        : nextEntry.key,
    pendingRemoveAfterEnterKey: holdCurrentUntilEnterComplete ? nextEntry.key : null,
    transitionDurationMs,
