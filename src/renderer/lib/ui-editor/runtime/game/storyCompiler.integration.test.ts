...
import {
  ...
  resolveBundleEntry,
  STORY_WHILE_LOOP_MAX_ITERATIONS
...
const EMPTY_APPEARANCE: CharacterAppearanceSummary = {
  kind: "preset",
  poses: [],
  defaultPoseId: null
};
...
function collectActionTree(action: any, story: unknown, seen = new Set<any>()): any[] {
  ...
  seen.add(action);
  const children =
    typeof action.getFutureActions === "function"
...
function getDisplayableTransformProps(actions: any[]): Record<string, unknown>[] {
  return getDisplayableTransforms(actions).flatMap(
    (transform) => transform?.sequences?.map((sequence) => sequence.props ?? {}) ?? []
  );
}

function getDisplayableTransforms(actions: any[]): {
  sequences?: { props?: Record<string, unknown>; options?: Record<string, unknown> }[];
  config?: Record<string, unknown>;
}[] {
  return actions
    .filter((action) => action?.type === "displayable:applyTransform")
    .flatMap((action) => {
      const transform = action.contentNode?.getContent?.()[0] as
        | {
            sequences?: { props?: Record<string, unknown>; options?: Record<string, unknown> }[];
            config?: Record<string, unknown>;
          }
...
describe("compileStudioStoryToNlr", () => {
  it("compiles core scene actions, resolves assets, and assigns stable ids", async () => {
    ...
    expect(compiled.actionIdBindings.map((binding) => binding.blockId)).toEqual(
      expect.arrayContaining(["bg", "say", "wait", "jump"])
    );
    expect(
      compiled.actionIdBindings.every(
        (binding) => DevTools.getStaticId(binding.action) === binding.staticId
      )
    ).toBe(true);
    expect(
      compiled.actionIdBindings.find((binding) => binding.blockId === "say")?.staticId
    ).toContain("text-say");
  });
      ...
      enter: {
        ...
        payload: {
          ...
          transform: {
            preset: "center",
            durationMs: 300,
            props: { zoom: 0.5, xoffset: 24, yoffset: -12 }
          }
        }
      }
    };
      ...
      characters: [
        {
          ...
          appearance: {
            kind: "preset",
            poses: [{ id: "pose-Neutral", name: "Neutral", assetId: "asset-alice-neutral" }],
  ...
  it("compiles hidden story animation assets into NarraLeaf transform sequences", async () => {
    ...
    expect(compiled.diagnostics).toEqual([]);
    expect(showTransform?.sequences?.[0]?.props).toEqual(
      expect.objectContaining({
        opacity: 1,
    ...
    expect(showTransform?.sequences?.[0]?.options).toEqual(
      expect.objectContaining({
        duration: 420,
        ease: "easeOut",
        delay: 40,
  ...
  it("compiles keyframe timeline tracks into grouped NarraLeaf transform sequences", async () => {
      ...
      timeline: {
        ...
        tracks: [
            ...
            keyframes: [
              {
                id: "position-start",
                timeMs: 0,
                value: { xalign: 0.5, yalign: 0.55, xoffset: -120 },
                easing: "linear"
              },
              {
                id: "position-end",
                timeMs: 420,
                value: { xalign: 0.5, yalign: 0.55, xoffset: 0 },
                easing: "easeOut"
              }
            ]
          },
          {
            id: "track-zoom",
            property: "zoom",
            keyframes: [{ id: "zoom-end", timeMs: 420, value: 1.1, easing: "easeOut" }]
          }
        ]
      }
    };
    ...
    const byBlock = (blockId: string) =>
      compiled.actionIdBindings
        .filter((binding) => binding.blockId === blockId)
        .flatMap((binding) => collectActionTree(binding.action, compiled.story));
    const showFinal = getDisplayableTransforms(byBlock("show"))[0]?.sequences?.find(
      (sequence) => sequence.options?.duration === 420
    );
    const moveFinal = getDisplayableTransforms(byBlock("transform"))[0]?.sequences?.find(
      (sequence) => sequence.options?.duration === 420
    );

    expect(compiled.diagnostics).toEqual([]);
    expect(showFinal?.options).toEqual(
      expect.objectContaining({ duration: 420, ease: "easeOut", at: 0 })
    );
    expect(showFinal?.props).toEqual(
      expect.objectContaining({
        opacity: 1,
    ...
    expect(moveFinal?.props).toEqual(
      expect.objectContaining({
        position: expect.objectContaining({ xalign: 0.5, yalign: 0.55, xoffset: 0 }),
        zoom: 1.1
      })
  ...
  it("keeps character enter asset resolution when using animation transforms", async () => {
    const animation: StoryAnimationAsset = {
      ...
      sequences: [
        {
          id: "step-1",
          props: { position: { xalign: 0.5, yalign: 0.5 } },
      ...
      characters: [
        {
          ...
          appearance: {
            kind: "preset",
            poses: [{ id: "pose-base", name: "base", assetId: "asset-alice" }],
            defaultPoseId: "pose-base"
          }
        }
      ],
      animations: { [animation.id]: animation },
  ...
  it("reuses story animation assets across displayable target kinds", async () => {
        ...
        tracks: [
          {
            id: "track-zoom",
            property: "zoom",
            keyframes: [{ id: "zoom-end", timeMs: 240, value: 1.1, easing: "easeOut" }]
          }
        ]
      }
    };
    ...
    expect(compiled.diagnostics).toEqual([]);
    expect(textTransform?.sequences?.at(-1)?.props).toEqual(
      expect.objectContaining({ opacity: 1, zoom: 1.1 })
    );
    expect(layerTransform?.sequences?.at(-1)?.props).toEqual(
      expect.objectContaining({ zoom: 1.1 })
    );
    ...
    const blocks: Record<string, StoryBlock> = {
      ...
      img: {
        ...
        payload: {
          ...
          objectName: "hero",
          assetId: "asset-hero",
          layer: { kind: "custom", sourceBlockId: "layer" }
        }
      },
      caption: {
        id: "caption",
        ...
        payload: {
          ...
          objectName: "cap",
          text: "Hi",
          layer: { kind: "default", layer: "background" }
        }
      }
    };
  ...
  it("still compiles a layer-action transform (back-compat: no longer offered in the UI)", async () => {
    ...
    expect(compiled.diagnostics).toEqual([]);
    expect(getDisplayableTransformProps(byBlock("bgZoom"))).toContainEqual(
  ...
  it("transforms built-in singletons (scene background + built-in layer) via displayable targets", async () => {
    ...
    expect(compiled.diagnostics).toEqual([]);
    expect(getDisplayableTransformProps(byBlock("bg"))).toContainEqual(
      expect.objectContaining({ zoom: 1.25 })
    );
    expect(getDisplayableTransformProps(byBlock("fgLayer"))).toContainEqual(
  ...
  it("compiles displayable visual effects on an existing stage image", async () => {
    ...
    expect(compiled.diagnostics).toEqual([]);
    expect(compiled.actionIdBindings.map((binding) => binding.blockId)).toEqual(
      expect.arrayContaining(["reveal", "darken", "wipe"])
    );
  });
  ...
  describe("inline expression events", () => {
        ...
        document: baseDocument(
          {
            enter,
            ...eventDialogue({
              event: { expression: { characterId: "char-alice", pose: "pose-angry" } }
            })
          },
          ["enter", "say"]
        ),
    ...
    it("compiles a sound-only event into a TextEvent carrying the SE", async () => {
      const compiled = await compileStudioStoryToNlr({
        document: baseDocument(eventDialogue({ event: { sound: { assetId: "asset-sting" } } }), [
          "say"
        ]),
        sceneId: "scene-1",
        characters: [alice],
    ...
    it("warns and omits an event whose character image cannot be resolved", async () => {
      const compiled = await compileStudioStoryToNlr({
        document: baseDocument(
          eventDialogue({
            event: { expression: { characterId: "char-ghost", formName: "angry" } }
          }),
          ["say"]
        ),
        sceneId: "scene-1",
      ...
      expect(compiled.diagnostics).toEqual([
        {
          level: "warning",
          blockId: "say",
          message: "Inline event: character image source not found for char-ghost."
        }
      ]);
  ...
  describe("character nametag fallbacks", () => {
    ...
    async function compileSpeaker(
      ...
      expect(compiled.diagnostics).toEqual([]);
      const sayAction = compiled.actionIdBindings.find((binding) => binding.blockId === "say")
        ?.action as any;
    ...
    it.each([
      ...
      const speaker = await compileSpeaker([
        { id: "char-alice", name, appearance: EMPTY_APPEARANCE }
      ]);
    ...
    it("falls back to a neutral label rather than leaking the characterId UUID", async () => {
      ...
      const unnamed = await compileSpeaker(
        [{ id: uuid, name: "", appearance: EMPTY_APPEARANCE }],
        uuid
      );
    ...
    it("still binds one Character instance per characterId when names collide", async () => {
      ...
      const speakers = ["say", "say2"].map((blockId) => {
        const action = compiled.actionIdBindings.find((binding) => binding.blockId === blockId)
          ?.action as any;
  ...
  describe("temp speakers", () => {
    function speakerLine(
      id: string,
      payload: { characterId?: string; speakerName?: string }
    ): StoryBlock {
      return {
        id,
        kind: "nodeAction",
    ...
    async function compileSpeakers(
      blocks: Record<string, StoryBlock>,
      characters: DevModeCharacterSummary[] = []
    ) {
      const compiled = await compileStudioStoryToNlr({
    ...
    it("falls back to speakerName when the characterId no longer resolves", async () => {
      const [speaker] = await compileSpeakers({
        say: speakerLine("say", { characterId: "char-gone", speakerName: "Alice" })
      });
      expect(speaker.state.name).toBe("Alice");
    });
  ...
  it("compiles choice, condition and variables", async () => {
    ...
    expect(compiled.actionIdBindings.map((binding) => binding.blockId)).toEqual(
      expect.arrayContaining(["option-child", "condition", "set-var"])
    );
    expect(compiled.diagnostics).toEqual([]);
  });
  ...
  it("validates persistent references against the declared set", async () => {
    ...
    const setDeclared: StoryBlock = {
      ...
      payload: {
        action: "setVariable",
        target: { scope: "persistent", variableId: "flag" },
        value: true
      }
    };
    const setGhost: StoryBlock = {
      ...
      payload: {
        action: "setVariable",
        target: { scope: "persistent", variableId: "ghost" },
        value: true
      }
    };
    const compiled = await compileStudioStoryToNlr({
      document: baseDocument(
        { "flag-decl": persistentDecl, "set-declared": setDeclared, "set-ghost": setGhost },
        ["flag-decl", "set-declared", "set-ghost"]
      ),
      sceneId: "scene-1"
    });
    ...
    expect(compiled.diagnostics).toContainEqual({
      level: "warning",
      blockId: "set-ghost",
      message: "Persistent variable not found; the assignment was skipped."
    });
    expect(compiled.diagnostics.find((d) => d.blockId === "set-declared")?.message).toContain(
      "require Dev Mode host persistence"
    );
    expect(
      compiled.diagnostics.some(
        (d) => d.blockId === "set-declared" && d.message.includes("not found")
      )
    ).toBe(false);
  });
  ...
  it("seeds declared scene-local defaults at the scene head and compiles declaration rows to nothing", async () => {
    ...
    const statementTypes = statements.map((statement) =>
      DevTools.chainToActions(statement as any)
      ...
      expect(compiled.diagnostics).toEqual([
        {
          level: "warning",
          blockId: "set-var",
          message: "Expression `gone + 1` did not resolve; the assignment was skipped."
        }
      ]);
    });
  ...
  it("maps the custom transition kinds onto real NLR transitions without diagnostics", async () => {
    ...
    const kinds: StoryTransitionRef["kind"][] = [
      "softWipe",
      "blinds",
      "slide",
      "softIris",
  ...
  it("keeps the stored shape of `slide` untouched by the native-Push switch", async () => {
    ...
    const stored: StoryTransitionRef = {
      kind: "slide",
      durationMs: 400,
  ...
  function findTransition(
    ...
    return visit(
      compiled.actionIdBindings.map((binding) => binding.action),
      0
    );
  }
});
...
describe("compileStudioStoryToNlr localization", () => {
  ...
  it("keeps untranslated segments as plain compiled prompts (no dynamic wrapper)", async () => {
    const compiled = await compileStudioStoryToNlr({
      document: baseDocument({ say: narrationBlock("say", "text-other", "没有翻译的行。") }, [
        "say"
      ]),
      sceneId: "scene-1",
        ...
        text: {
          ...
          rich: [
            { text: "你好，" },
            {
              interpolation: {
                kind: "variable",
                target: { scope: "persistent", variableId: "playerName" }
              }
            },
            { text: "！" }
          ]
...
describe("compileStudioStoryToNlr voice", () => {
  ...
  it("carries a voiceId for a line voiced in some other language", async () => {
    ...
    expect(compiled.getVoicePlayback?.("text-say")).toEqual({
      src: "nlr://asset-ja-say",
      busId: "voice"
    });
  });
  ...
  it("keeps the legacy per-line voiceAssetId as an inline fallback", async () => {
    const compiled = await compileStudioStoryToNlr({
      document: baseDocument(
        { say: dialogueBlock("say", "text-legacy", "hi", { voiceAssetId: "asset-voice" }) },
        ["say"]
      ),
      sceneId: "scene-1",
      characters: [
  ...
  it("compiles /camera onto story.camera and clamps every numeric input", async () => {
    ...
    const cameraBlock = (
      id: string,
      payload: Extract<StoryBlock["payload"], { action: "camera" }>
    ): StoryBlock => ({
      id,
      kind: "action",
      parentId: null,
    ...
    const blocks: Record<string, StoryBlock> = {
      ...
      rotate: cameraBlock("rotate", {
        action: "camera",
        operation: "rotate",
        rotation: 15,
        durationMs: 400
      }),
      dark: cameraBlock("dark", {
        action: "camera",
        operation: "darken",
        darkness: 2,
        durationMs: 500
    ...
    expect(propsOf("pan")).toEqual([
      expect.objectContaining({ position: expect.objectContaining({ xalign: 0.25, yalign: 0.5 }) })
    ]);
    ...
    const blocks: Record<string, StoryBlock> = {
      shot: {
        ...
        payload: {
          action: "camera",
          operation: "motion",
          motion: { mode: "animation", animationId: animation.id }
        }
      },
  ...
  it("compiles /label and /goto, and refuses the two shapes the engine would refuse", async () => {
    const control = (
      id: string,
      payload: Extract<StoryBlock["payload"], { control: string }>
    ): StoryBlock => ({
      id,
      kind: "control",
      parentId: null,
    ...
    const typeOf = (blockId: string) =>
      compiled.actionIdBindings.find((binding) => binding.blockId === blockId)?.action as
        | { type?: string }
        | undefined;
    ...
    expect(compiled.diagnostics).toEqual([
      {
        level: "error",
        blockId: "dupe",
        message: 'Label "start" is declared more than once in this scene.'
      },
      {
        level: "error",
        blockId: "nowhere",
        message: "Go to target label not found in this scene: elsewhere"
      }
    ]);
  });
    ...
    expect(compiled.diagnostics).toEqual([
      {
        level: "error",
        blockId: "miscased",
        message: "Go to target label not found in this scene: START"
      }
    ]);
  });
  ...
  it("compiles /vfx onto one Vfx, showing on create and clamping its knobs", async () => {
    const vfxBlock = (
      id: string,
      payload: Extract<StoryBlock["payload"], { action: "vfx" }>
    ): StoryBlock => ({
      id,
      kind: "action",
      parentId: null,
    ...
    const actionOf = (blockId: string) =>
      compiled.actionIdBindings.find((binding) => binding.blockId === blockId)?.action as any;
    ...
    expect(actionOf("create")?.callee?.config).toMatchObject({
      blendMode: "screen",
      opacity: 1,
      zIndex: 3,
      fit: "cover"
  ...
  it("compiles the video transport operations, converting seek to seconds", async () => {
    const videoBlock = (
      id: string,
      payload: Extract<StoryBlock["payload"], { action: "video" }>
    ): StoryBlock => ({
      id,
      kind: "action",
      parentId: null,
    ...
    const blocks: Record<string, StoryBlock> = {
      ...
      seek: videoBlock("seek", {
        action: "video",
        operation: "seek",
        objectName: "clip",
        timeMs: 3500
      }),
      rewind: videoBlock("rewind", {
        action: "video",
        operation: "seek",
        objectName: "clip",
        timeMs: -1000
    ...
    const contentOf = (blockId: string) =>
      compiled.actionIdBindings
        .filter((binding) => binding.blockId === blockId)
  ...
  it("compiles /rename onto the same Character the dialogue rows speak through", async () => {
    ...
    const actionsOf = (blockId: string) =>
      compiled.actionIdBindings
        .filter((binding) => binding.blockId === blockId)
        .flatMap((binding) => collectActionTree(binding.action, compiled.story));
  ...
  it("warns when a persistent name is declared in both the registry and a story row (M-VAR merged view)", async () => {
    ...
    expect(compiled.diagnostics).toContainEqual({
      ...
      blockId: undefined,
      message:
        'Persistent variable "Score" is declared in both the variable registry and a story row; references are ambiguous.'
    });
  });
        ...
        text: {
          ...
          rich: [
            { text: "Chapter " },
            {
              interpolation: {
                kind: "variable",
                target: { scope: "persistent", variableId: "key_chapter" }
              }
            }
          ]
        }
  ...
  it("seeds registry-backed saved variables into the saved namespace beside story-declared ones (M-VAR)", async () => {
    ...
    const savedPersistent = (
      compiled.story as unknown as { persistent: unknown[] }
    ).persistent.find(
      (entry) => DevTools.getNamespaceName(entry as never) === compiled.savedNamespaceName
    ) as { defaultContent: Record<string, unknown> } | undefined;
  ...
  it("warns when a saved name is declared in both the registry and a story row (M-VAR merged view)", async () => {
    ...
    expect(compiled.diagnostics).toContainEqual({
      ...
      blockId: undefined,
      message:
        'Saved variable "Gold" is declared in both the variable registry and a story row; references are ambiguous.'
    });
  });
});
...
describe("dialog avatars", () => {
  ...
  function resolveAvatar(
    ...
    const character = compiled.characters.get(characterId) as unknown as
      | { config?: { avatar?: unknown } }
      | undefined;
  ...
  it("resolves a preset character's avatar from the pose src the engine reports", async () => {
    ...
    expect(resolveAvatar(compiled, "char-alice", { currentSrc: "nlr://asset-angry" })).toBe(
      "nlr://asset-avatar-angry"
    );
    expect(resolveAvatar(compiled, "char-alice", { currentSrc: "nlr://asset-neutral" })).toBe(
      "nlr://asset-avatar-neutral"
    );
  });
  ...
  it("resolves a layered character's avatar from the active tags", async () => {
        ...
        layers: [
          {
            ...
            name: "Face",
            axisId: "mood",
            options: { happy: "asset-happy", sad: "asset-sad" }
          }
        ],
        avatars: {
    ...
    expect(resolveAvatar(compiled, "char-bob", { tags: ["happy"] })).toBe(
      "nlr://asset-avatar-happy"
    );
  });
  ...
  it("falls back to the character default when the speaker has no differential on stage", async () => {
    ...
    expect(resolveAvatar(compiled, "char-alice", { currentSrc: "nlr://asset-stranger" })).toBe(
      "nlr://asset-avatar-default"
    );
  });
  ...
  it("gives a character that never appears on stage its default avatar", async () => {
    ...
    expect(resolveAvatar(compiled, "char-alice", {})).toBe("nlr://asset-avatar-default");
    expect(compiled.avatarAssetIdByUrl.get("nlr://asset-avatar-default")).toBe(
  ...
  it("does not overwrite a staged character's live resolver with the flat default", async () => {
    ...
    const compiled = await compileStudioStoryToNlr({
      document: baseDocument({ ...enterBlock("char-alice"), ...sayBlock("char-alice") }, [
        "enter",
        "say"
      ]),
      sceneId: "scene-1",
  ...
  it("keeps avatars out of the scene preloader, which caches them where nothing can read them", async () => {
    ...
    const srcManager = (
      compiled.scene as unknown as { srcManager: { src: { type: string; src: unknown }[] } }
    ).srcManager;
    const preloaded = srcManager.src
    ...
    expect([...compiled.avatarAssetIdByUrl]).toEqual(
...
describe("puppet characters", () => {
  ...
  function characterBlock(
    ...
    return {
      row: {
        ...
        parentId: null,
        childrenIds: [],
        payload: { action: "character", operation, characterId } as Extract<
          StoryActionPayload,
          { action: "character" }
        >
      }
  ...
  it("resolves an entry override against the bundle the asset resolved to", async () => {
    const compiled = await compileStudioStoryToNlr({
      ...
      characters: [
        {
          ...PUPPET,
          appearance: { ...PUPPET.appearance, entry: "alt/other.json" } as typeof PUPPET.appearance
        }
      ],
      resolveAssetUrl: async (assetId) => `nlr://bundle/${assetId}/model.json`
    });
    ...
    expect(puppetConfig(compiled, "char-doll")).toMatchObject({
      src: "nlr://bundle/asset-model/alt/other.json"
    });
  });
  ...
  it("resolves an entry the way the engine resolves a sibling", () => {
    ...
    expect(resolveBundleEntry(entry, "windows\\path.json")).toBe(
      "nlr://bundle/a/windows/path.json"
    );
  ...
  it("reports a puppet with no model rather than emitting a sourceless element", async () => {
    const compiled = await compileStudioStoryToNlr({
      ...
      characters: [
        {
          ...PUPPET,
          appearance: { ...PUPPET.appearance, assetId: null } as typeof PUPPET.appearance
        }
      ],
      resolveAssetUrl: async (assetId) => `nlr://${assetId}`
    });
  ...
  it("reports a puppet that names no runtime", async () => {
    const compiled = await compileStudioStoryToNlr({
      ...
      sceneId: "scene-1",
      characters: [
        { ...PUPPET, appearance: { ...PUPPET.appearance, backend: "" } as typeof PUPPET.appearance }
      ],
      resolveAssetUrl: async (assetId) => `nlr://${assetId}`
    });
  ...
  it("compiles the three state channels onto the same element the stage rows use", async () => {
    const stateRow = (
      id: string,
      operation: "expression" | "setMotion" | "setSkin",
      puppetName?: string
    ): StoryBlock => ({
      id,
      kind: "action",
      ...
      payload: {
        ...
        operation,
        characterId: "char-doll",
        ...(puppetName !== undefined ? { puppetName } : {})
      }
    });
    const compiled = await compileStudioStoryToNlr({
      document: baseDocument(
    ...
    const actionsOf = (blockId: string) =>
      compiled.actionIdBindings
        .filter((binding) => binding.blockId === blockId)
        .flatMap((binding) => collectActionTree(binding.action, compiled.story));
    const one = (blockId: string, type: string) =>
      actionsOf(blockId).find((action) => action?.type === type);
    ...
    expect(compiled.sceneElements?.["scene-1"]?.puppets.size).toBe(1);
    expect(one("motion", "puppet:setMotion")?.callee).toBe(
      compiled.sceneElements?.["scene-1"]?.puppets.get("char-doll")
    );
  });
  ...
  it("compiles a parameter row into one setParam per entry", async () => {
    ...
    expect(compiled.diagnostics).toEqual([]);
    expect(setParams.map((action) => action?.contentNode?.getContent?.())).toEqual([
      ["ParamAngleX", 12],
      ["ParamAngleY", -7.5]
    ]);
  ...
  it("refuses a state channel on a character Studio draws itself", async () => {
    ...
    expect(compiled.diagnostics.some((entry) => /not drawn by a runtime/.test(entry.message))).toBe(
      true
    );
  });
...
describe("story audio", () => {
  ...
  it("still warns when a control row addresses music no scene or row set", async () => {
    ...
    expect(
      compiled.diagnostics.some((entry) => /No background music is set/.test(entry.message))
    ).toBe(true);
  });
  ...
  describe("audio buses", () => {
    ...
    it("a dangling track id falls back to the built-in for the operation's own bus", async () => {
      ...
      expect(compiled.diagnostics).toEqual([]);
      expect((compiled.sceneElements?.["scene-1"].sounds.get("impact") as any).config.type).toBe(
        "sound"
      );
    });
    ...
    it("a bus under a root of the author's own still carries its own id", async () => {
      ...
      expect(compiled.diagnostics).toEqual([]);
      expect((compiled.sceneElements?.["scene-1"].sounds.get("wind") as any).config.type).toBe(
        "t_field"
      );
    });
    ...
    it("reports a second row that names a different track for a handle that already has one", async () => {
      ...
      expect(
        compiled.diagnostics.some((entry) =>
          /already playing on the "Ambience" track/.test(entry.message)
        )
      ).toBe(true);
      expect((compiled.sceneElements?.["scene-1"].sounds.get("rain") as any).config.type).toBe(
        "t_amb"
      );
    });
    ...
    describe("per-character voice", () => {
      ...
      it("an unset character stays on `voice`, and the voice map stays a bare URL", async () => {
        ...
        expect((compiled.scenes["scene-1"] as any).config?.voices?.["text-say"]).toBe(
          "nlr://asset-ja-say"
        );
      });
...
describe("repeat until", () => {
  ...
  async function compileLoop(
    until: StoryConditionRef | undefined,
    extra: Record<string, StoryBlock> = {}
  ) {
  ...
  it("refuses a loop whose condition cannot resolve, rather than emitting one that never ends", async () => {
    const { compiled, spy } = await compileLoop({
      ...
      expression: {
        source: "nope",
        ast: { kind: "var", target: { scope: "scene", variableId: "nope" }, name: "nope" }
      }
    });
    expect(spy).not.toHaveBeenCalled();
    expect(compiled.diagnostics.some((d) => d.level === "error" && d.blockId === "loop")).toBe(
      true
    );
  });
    ...
    expect(outside.diagnostics).toEqual([
      {
        level: "error",
        blockId: "brk",
        message: "Break is not inside a repeat group; there is no loop for it to leave."
      }
    ]);
  });
});
      ...
      document: baseDocument(
        {
          cut: {
            ...
            parentId: null,
            childrenIds: [],
            payload: { control: "cut", appTagId: "tag-demo" }
          },
