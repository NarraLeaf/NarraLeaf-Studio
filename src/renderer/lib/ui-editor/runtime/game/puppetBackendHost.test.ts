...
describe("loadPuppetBackends", () => {
    it("registers a default-exported backend object", async () => {
        const game = fakeGame();
        const results = await loadPuppetBackends(game, [moduleSource(
            "plain",
            "export default { name: 'renderer-a', mount() { return {}; } };",
        )], silent);
    ...
    it("calls a factory with the game and the module's file resolver", async () => {
        const game = fakeGame();
        await loadPuppetBackends(game, [moduleSource(
            "factory",
    ...
    it("keeps loading after a module fails, so one bad runtime cannot cost the others", async () => {
        const game = fakeGame();
        const log = vi.fn();
        const results = await loadPuppetBackends(game, [
            { id: "missing", url: "data:text/javascript;base64,***not-base64***", resolveFile: () => Promise.reject(new Error("no")) },
