...
export function compileStoryActionBlueprintToScript(input: CompileStoryActionScriptInput): unknown {
  ...
  if (bp.program.kind !== "graph") {
    input.onDiagnostic?.(
      "Story Action Blueprint is not a graph blueprint; the action was skipped."
    );
    return null;
  }
 ...
 */
export function evaluateStoryActionBlueprintValueSync(
  input: CompileStoryActionScriptInput,
  ctx: ScriptCtx
): unknown {
  const bp = resolveActiveStoryActionBlueprint(input.blueprintDocument, input.blueprintId);
  ...
  for (const eventGraph of Object.values(bp.program.graphs.events ?? {})) {
    ...
    const graph = adaptBlueprintGraphIr(
      ir,
      buildBlueprintRunGraphId("storyActionValue", bp.id, eventGraph.id)
    );
...
async function runStoryActionOnCall(env: StoryActionExecutionEnv): Promise<unknown> {
  ...
  for (const eventGraph of Object.values(bp.program.graphs.events ?? {})) {
    ...
    const graph = adaptBlueprintGraphIr(
      ir,
      buildBlueprintRunGraphId("storyAction", bp.id, eventGraph.id)
    );
    for (const headId of headIds) {
...
async function invokeStoryActionFn(options: {
  ...
  const visible =
    ...
    (decl.owner.kind === "storyAction" &&
      input.sceneFnCatalog.blueprintIds.has(decl.owner.blueprintId));
  if (!visible) {
  ...
  const graph = adaptBlueprintGraphIr(
    decl.ir,
    buildBlueprintRunGraphId("storyActionFn", decl.blueprintId, decl.graphId)
  );
  const result = await executeGraph({
    graph,
