 ...
 *
 * One walk and one set of warmers for both places a game is drawn. The packaged game asks through
 * `src/runtime/renderer/surfaceResourcePreload.ts` with its pack; a Dev Mode window asks with its
 * bundle. Each hands in the two things only it knows: where an asset's bytes are, and the font
 * registry its own widgets read - a typeface warmed into any other registry is a typeface every text
 * widget then loads again.
 *
 * The walk has to be one. If Dev Mode waited on a different set than the shipped game warms, an
 * author would watch a title screen arrive complete and ship one that pops in, or the reverse.
 */

export const SURFACE_WARMUP_TIMEOUT_MS = 10_000;

/**
 * Everything the walk reads, and nothing that says which runtime is asking.
 *
 * `manifestIds` is the set of ids the build says exist, or null when there is nothing to check
 * against - a protected build ships no manifest, and a Dev Mode window has the project itself.
 */
export type SurfaceWarmupSource = {
    uidoc: UIDocument;
     ...
     * Absent where a caller only warms what a screen draws, which is all a single surface needs.
     */
    blueprintDocument?: BlueprintDocument;
};

/**
 * What a caller can say about an asset's kind: the three fields a pack's manifest carries for it.
 * A Dev Mode window answers `type` alone, from the library.
 */
export type SurfaceWarmupKindHint = Pick<GameRuntimeAssetManifestEntry, "type" | "ext" | "mimeType">;

/** Register a typeface in the registry this runtime's widgets read. */
export type SurfaceWarmupFontLoader = (assetId: string, url: string) => Promise<unknown>;
 ...
 */
function collectProjectFontAssetIds(ctx: CollectContext, fontAssetIds: readonly string[]): void {
...
function newContext(source: SurfaceWarmupSource): CollectContext {
    return {
        document: source.uidoc,
        manifestIds: source.manifestIds,
        assetIds: new Set(),
...
/** The assets one surface draws with, the frames and components it pulls in, and the project fonts. */
...
/** Every surface's assets, the first surface's leading, for a pass that warms the whole interface. */
export function collectWarmupAssetIds(source: SurfaceWarmupSource, firstSurfaceId: string): {
    firstSurfaceAssetIds: string[];
    assetIds: string[];
} {
 ...
 *
 * The walk above reads the UI document, and a graph is not in it: a button's click sound
 ...
 * naming an asset set warms every member, for the reason `fetchableAssetIds` gives. Filtered against
 * the manifest the same way, which drops nothing in a protected build: that one ships none.
 */
export function collectBlueprintWarmupAssets(source: SurfaceWarmupSource): { assetId: string; kind: PreloadKind }[] {
    const found = new Map<string, PreloadKind>();
    for (const graph of blueprintDocumentGraphs(source.blueprintDocument)) {
        forEachBlueprintAssetSlot(graph, slot => {
            ...
            for (const assetId of members ? new Set(Object.values(members)) : [stored]) {
                if (source.manifestIds && !source.manifestIds.has(assetId)) {
                    continue;
                }
 ...
 */
function kindFromEntry(entry: SurfaceWarmupKindHint | undefined): PreloadKind | null {
...
/**
 * Warm one asset the way its kind is warmed.
 *
 * A font is warmed by registering it, through the registry the widgets read. Registering rather
 * than merely fetching is the whole point: the bytes alone would leave every text widget to build a
 * `FontFace` of its own.
 */
async function preloadAsset(input: {
    assetId: string;
    entry: SurfaceWarmupKindHint | undefined;
...
/** How a warm-up pass reaches its assets: everything but the walk's own input. */
export type SurfaceWarmupAccess = {
    assetUrl: (assetId: string) => string;
    /** What the build's manifest says the asset is, when there is one to ask. */
    entryFor: (assetId: string) => SurfaceWarmupKindHint | undefined;
    loadFont: SurfaceWarmupFontLoader;
    timeoutMs?: number;
    onProgress?: RuntimePreloadProgress;
};

/** Warm one surface's assets and the project fonts, and nothing else. */
export async function warmSurfaceAssets(input: SurfaceWarmupAccess & {
    source: SurfaceWarmupSource;
    surfaceId: string;
}): Promise<RuntimeSurfacePreloadResult> {
    const assetIds = collectSurfaceWarmupAssetIds(input.source, input.surfaceId);
    const failed: string[] = [];
    let loaded = 0;
    let settled = 0;
    let completed = false;
    ...
    await Promise.race([
        preloadAll,
        new Promise(resolve => setTimeout(resolve, input.timeoutMs ?? SURFACE_WARMUP_TIMEOUT_MS)),
    ]);
...
/**
 * Warm the whole interface, and say as soon as the first screen can show.
 *
 * # The wait is the first screen, not the interface
 *
 * This used to be one wait over every asset of every page: a packaged game held its first frame -
 * and the story compile behind it - until a settings page's backdrop and a gallery's thumbnails had
 * all decoded. `onFirstSurfaceSettled` is the moment the first screen's own assets (and the project's
 * fonts, which every page is set in) have settled, and it is the only thing a caller should gate on.
 * Everything else keeps warming afterwards, a few at a time, ahead of the player getting there. A page
 * opened before its assets arrive still draws correctly - a picture decodes on first use and the
 * page's own prepaint waits for its fonts - it is only later than it would have been.
 *
 * Progress counts the first screen's assets alone, because they are now the whole of the wait.
 *
 * # What is warmed besides the interface document
 *
 * Every asset the source's blueprints name ({@link collectBlueprintWarmupAssets}), a blueprint's
 * sounds first of all: the first thing a player does on a title screen is press something, and a
 * click sound is small. Measured on the shipped skeleton with a heavy Load page, they arrived 2.4 s in
 * when they queued behind that page's pictures.
 *
 ...
 */
export async function warmInterfaceAssets(input: SurfaceWarmupAccess & {
    source: SurfaceWarmupSource;
    ...
    const { firstSurfaceAssetIds, assetIds: interfaceAssetIds } = collectWarmupAssetIds(input.source, input.firstSurfaceId);
    const firstSurfaceAssetSet = new Set(firstSurfaceAssetIds);
    const blueprintAssets = collectBlueprintWarmupAssets(input.source)
    ...
    const timeoutMs = input.timeoutMs ?? SURFACE_WARMUP_TIMEOUT_MS;
    const failed: string[] = [];
    ...
    const preloadAll = (async () => {
        ...
        await Promise.all(Array.from(
            { length: Math.min(BACKGROUND_WARMUP_CONCURRENCY, background.length) },
            () => worker(),
        ));
        await firstPass;
        completed = true;
