...
describe("surface animation plan", () => {
  ...
  it("staggers direct children and reports the subtree as animated up the chain", () => {
    const plan = buildSurfaceAnimationPlan({
      elements: table(
        element("root", ["a", "b"]),
        element("a", [], FADE_IN_OUT),
        element("b", [], FADE_IN_OUT)
      ),
      rootElementId: "root",
    ...
    const plan = buildSurfaceAnimationPlan({
      elements: table(
        ...
        element(
          "loud",
          [],
          settings({ exit: "fade", exitDurationSeconds: 0.1, exitDelaySeconds: 0.2 })
        )
      ),
      rootElementId: "root"
    });
...
describe("surface animation timings", () => {
  const elements = table(
    ...
    element(
      "a",
      [],
      settings({ enter: "fade", exit: "fade", enterDurationSeconds: 0.2, exitDurationSeconds: 0.3 })
    )
  );
  const surface = {
    rootElementId: "root",
