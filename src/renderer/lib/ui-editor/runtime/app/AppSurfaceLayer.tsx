...
export type AppSurfaceLayerNavEntry = SurfaceNavigationEntry<
  PageProps,
  SurfaceNavigationPresentation
> & {
...
export function AppSurfaceLayer(
  props: AppSurfaceLayerCommonProps & {
  ...
  const [surfaceRuntimeSubscriptionsReadyKey, setSurfaceRuntimeSubscriptionsReadyKey] = useState<
    string | null
  >(null);
  ...
  const surfaceBlueprintLifecycleReady =
    blueprintLifecycleReady && surfaceRuntimeSubscriptionsReady;
  ...
  useEffect(() => {
    if (hostAdapterBundle.hostAdapter.blueprintRuntime) {
      hostAdapterBundle.hostAdapter.blueprintRuntime.getSurfaceTransitionState = () =>
    ...
    (commands: readonly LifecycleCommand[]) => {
      executeLifecycleCommands(commands, {
        ...
        dispatchSurfaceEvent: (command) => {
          void hostAdapterBundle.hostAdapter.blueprintRuntime?.dispatchSurfaceBlueprintEvent?.(
            command.eventName
          );
        },
  ...
  const handleBeforeExit = useCallback(
    (entryKey: string) => {
      ...
      runTransitionCommands(
        lifecycleRef.current.beforeExit(hostAdapterBundle.runtimeScopeId, surface.id)
      );
    },
    [entry.key, hostAdapterBundle.runtimeScopeId, lifecycleRef, runTransitionCommands, surface.id]
  );
  ...
  const handleEnterComplete = useCallback(
    (entryKey: string) => {
      if (entryKey === entry.key) {
        runTransitionCommands(
          lifecycleRef.current.enterComplete(hostAdapterBundle.runtimeScopeId, surface.id)
        );
...
export function AppSurfaceLayerWithAdapter(
  props: AppSurfaceLayerCommonProps & {
    core: BlueprintRuntimeCore | null;
    createHostAdapterBundle: (
      entry: AppSurfaceLayerNavEntry,
      surface: UISurface
    ) => HostAdapterBundle | null;
  }
) {
  const { core, entry, surface, createHostAdapterBundle } = props;
