...
function makeSave(head: PlayHead, options: SaveOptions = {}): SavedGame {
    ...
    return {
        ...
        game: {
            ...
            asyncStackModels: [],
            history: head.backlog.map(text => ({ actionId, element: { type: "say", text }, snapshot: null })),
        },
...
const PLAYING: PlayHead = { sceneId: "scene-a", line: 3, backlog: ["one", "two"], audio: "bgm:town" };

describe("loadSaveIntoGame", () => {
    it("advances the run when the save loads", async () => {
        const harness = createHarness({ head: PLAYING });
        const saved = makeSave({ sceneId: "scene-a", line: 9, backlog: ["one", "two", "three"], audio: "bgm:night" });
...
describe("compareSaveStory", () => {
    ...
    it("separates this build from any other", () => {
        ...
        expect(compareSaveStory(makeSave(PLAYING, { storyHash: "other" }), LIVE_STORY_HASH)).toBe("otherStory");
    });
});
...
describe("collectUnresolvedSaveReferences", () => {
    ...
    it("descends into nested and async stacks, and into loop bodies", () => {
        ...
        saved.game.stackModel.items.push({
            ...
            action: "action-1",
            stackWaitType: "all",
            stacks: [{ items: [{ type: "action", actionType: "character:say", action: "action-nested-gone" }] }],
        });
