...
import { useBlueprintRuntimeCore } from "@/lib/ui-editor/runtime/game/useBlueprintRuntimeCore";
import {
...
import {
  createPuppetStageHandle,
  loadPuppetBackends
} from "@/lib/ui-editor/runtime/game/puppetBackendHost";
...
import { AppSurfaceLayerWithAdapter, type AppSurfaceLayerNavEntry } from "./AppSurfaceLayer";
...
import {
  ...
  mountSurfaceLayer,
  type SurfaceLayerEntry
} from "./layers/LayerStackController";
...
export function GameApp(props: GameAppProps): ReactNode {
      ...
      try {
        ...
        if (
          ...
          (typeof stored === "string" &&
            localization.locales.some((locale) => locale.code === stored))
        ) {
          return;
        }
        const candidates =
  ...
  const translateCharacterName = useCallback(
    (name: string | null): string | null => {
      ...
      return (
        resolveLocalizedUnitText(localization, locale, characterTranslationUnitId(character.id)) ??
        name
      );
    },
   ...
   * resolves to null.
   */
  const resolveSpeakerCharacterId = useCallback(
    (sourceName: string | null): string | null => {
      if (!sourceName) {
        return null;
      }
  ...
  const [widgetPatchesByScope, setWidgetPatchesByScope] = useState<
  ...
  const layerStack = useMemo(
    () => providedLayerStack ?? new LayerStackController(),
    [providedLayerStack]
  );
  ...
  const pendingEnvReadyRef = useRef(
  ...
  const startStoryInGameRef = useRef<
    | ((request: DevModeStartStoryRequest, options?: { forceReinit?: boolean }) => Promise<void>)
    | null
  >(null);
  ...
  const pendingGameStartsRef = useRef(
    new Map<string, { resolve: () => void; reject: (error: Error) => void }>()
  );
  ...
  const { isInGame, requireLiveGame: requireActiveLiveGame } = useMemo(
    () =>
      createSessionGate<LiveGame>({
  ...
  const playHeadBlockId = useCallback((): string | undefined => {
    ...
    return nlrCompiledRef.current?.actionIdBindings.find((binding) => binding.staticId === actionId)
      ?.blockId;
  }, []);
  /**
  ...
  const reportFailure = useCallback(
    (error: unknown, options?: { prefix?: string }) => {
  ...
  const handleSurfaceLayerPrepaintReady = useCallback(
    (entryKey: string) => {
  ...
  const isGameHiddenEntry = useCallback((entry: GameAppNavEntry | null | undefined): boolean => {
    return Boolean(
      entry && studioPageHiddenForGameRef.current && gameHiddenNavKeysRef.current.has(entry.key)
    );
  }, []);
  ...
  const openSurface = useCallback(
    (surfaceId: string, props?: PageProps, options?: OpenSurfaceOptions): Promise<void> => {
      ...
      return navigation.open({
        ...
        createNextEntry: (waitForExit) =>
          createNavEntry(target.id, "forward", waitForExit, props, presentation)
      });
    },
    [
      bundle,
  ...
  const closeToIndex = useCallback(
    ...
    [bundle, isGameHiddenEntry, navigation, prefersReducedMotion, resetSurfaceInteractionReadiness]
  );
  ...
  const showLayer = useCallback(
    (request: BlueprintLayerShowRequest): string => {
  ...
  const hideLayerGroup = useCallback(
  ...
  const waitLayer = useCallback(
    (handle: string): Promise<unknown> => {
   ...
   */
  const closeOwnLayer = useCallback(
    (runtimeScopeId: string, result: unknown): boolean => {
      return layerStack.closeWithResult(runtimeScopeId, result);
    },
    [layerStack]
  );

  const isLayerMounted = useCallback(
  ...
  const isSceneVisitedInGame = useCallback(
  ...
  const isOptionPickedInGame = useCallback(
  ...
  const resolveRunningStoryDocument = useCallback(() => {
    ...
    return (
      ...
      Object.values(bundle.storyLibrary?.documents ?? {}).find(
        (document) => document.id === storyId
      )
    );
  }, [bundle]);
   ...
   */
  const applyImportedSavedProgress = useCallback(
    (document: GameProgressDocumentV1): void => {
      const liveGame = nlrLiveGameRef.current;
      ...
      if (
        ...
        document.visitedSceneIds.length > 0 &&
        storable.hasNamespace(visitedNamespaceName)
      ) {
  ...
  const exportProgressInGame = useCallback(async (): Promise<{
    ...
    const savedVariables =
      liveGame && compiled?.savedNamespaceName
        ? await collectGameProgressVariables(defs.saved, readSavedProgressValue)
        : {};
    const persistentVariables = core
      ? await collectGameProgressVariables(defs.persistent, (key) =>
          core.scopeBridge.persistenceGetAsync(key)
        )
      : {};
    const anchorSceneId =
    ...
    const anchor: GameProgressAnchor | null = anchorSceneId
      ? {
          ...
          sceneRuntimeName:
            storyDocument?.scenes[anchorSceneId]?.runtimeName ||
            storyDocument?.scenes[anchorSceneId]?.name ||
            anchorSceneId
        }
      : null;
    const visitedSceneIds =
      liveGame && compiled?.visitedNamespaceName
        ? readStoryVisitedIds(
            liveGame.getStorable(),
            compiled.visitedNamespaceName,
            STORY_VISITED_SCENES_KEY
          )
        : [];
   ...
   */
  const reportSaveCaptureFailure = useCallback(
  ...
  const readVoiceLocale = useCallback((): string => {
    ...
    if (
      typeof stored === "string" &&
      stored &&
      voice.voicedLocales.some((entry) => entry.code === stored)
    ) {
      return stored;
    }
   ...
   * is the other one.
   */
  const soundTransport = useMemo(
    () =>
      createSoundTransport({
        getLiveGame: () => nlrLiveGameRef.current,
        ...
        createSound: ({ src, busId, loop, volume, assetId }) =>
          new Sound({
            src,
   ...
   */
  const playVoiceUnit = useCallback(
    async (unitId: string): Promise<boolean> => {
      ...
      try {
        ...
        host.log(
          "warning",
          `Play Voice: ${error instanceof Error ? error.message : String(error)}`
        );
        return false;
      }
    },
    ...
    () => ({
      ...
      relaunch: async ({ sceneId, startBlockId, snapshotId }) => {
        ...
        await start(
          {
            storyId: request.storyId,
            sceneId: sceneId ?? request.sceneId,
            startBlockId,
            snapshotId
          },
          { forceReinit: true }
  ...
  const quitGame = useCallback(
    async (surfaceId: string): Promise<void> => {
  ...
  const handleStoryEnd = useCallback(() => {
    ...
    void quitGame(endingSurfaceId).catch((error) => {
      ...
      host.log(
        "error",
        `[${host.id}] the ending page could not be opened: ${normalizeError(error)}`
      );
    });
  }, [endingSurfaceId, host, quitGame]);

  const writeSave = useCallback(
    async (id: string, metadata?: unknown, screenshot?: boolean) => {
  ...
  const loadSave = useCallback(
    async (id: string): Promise<SaveLoadOutcome> => {
   ...
   */
  const loadSaveAction = useCallback(
  ...
  const listAutoSaves = useCallback(async (): Promise<AutoSaveEntry[]> => {
    ...
    const entries = await Promise.all(
      ids.map(async (id): Promise<AutoSaveEntry | null> => {
    ...
    return entries
      .filter((entry): entry is AutoSaveEntry => entry !== null)
   ...
   */
  const getSaveTimes = useCallback(
    async (id: string): Promise<SaveRecordTimes | null> => {
      const record = await host.saveStore.read(id);
      if (!record) {
        return null;
      }
   ...
   */
  const getSaveLine = useCallback(
    async (id: string): Promise<SaveRecordLine | null> => {
      const record = await host.saveStore.read(id);
      if (!record) {
        return null;
      }
  ...
  const getSavePreview = useCallback(
    async (id: string): Promise<BlueprintImageAsset | null> => {
  ...
  const compileStoryRequest = useCallback(
    async (request: DevModeStartStoryRequest): Promise<CompiledNlrStory> => {
      ...
      if (!storyDocument) {
        const indexedStoryIds =
          bundle.storyLibrary?.index.stories.map((story) => story.id).join(", ") || "(none)";
        const documentStoryIds =
          Object.values(bundle.storyLibrary?.documents ?? {})
      ...
      const compileInput: Parameters<typeof compileStudioStoryToNlr>[0] & {
        ...
        localization:
          bundle.localization && core
            ...
            : undefined,
        voice:
          bundle.voice && core ? { ...bundle.voice, getVoiceLocale: readVoiceLocale } : undefined,
  ...
  const mountNlrSession = useCallback(
    async (
      ...
      rejectPendingGameStarts(
        new NlrSessionSupersededError("NLR environment superseded by a newer session")
      );
      ...
      audioBusPersistenceRef.current = await attachAudioBusPersistence({
        mixer: (game as { audioBuses?: Parameters<typeof attachAudioBusPersistence>[0]["mixer"] })
          .audioBuses,
        read: (key) => core.scopeBridge.persistenceGetAsync(key),
        write: (key, value) => core.scopeBridge.persistenceSetAsync(key, value),
  ...
  const startStoryInGame = useCallback(
    async (
      ...
      await mountNlrSession(compiled, {
        storyRequest: { storyId, sceneId, startBlockId, snapshotId }
      });
      await enterMountedGame();
    },
  ...
  const initDefaultSceneEnvironment = useCallback(
    async (request: DevModeStartStoryRequest): Promise<void> => {
  ...
  const createHostAdapterBundle = useCallback(
    (entry: AppSurfaceLayerNavEntry, surface: UISurface) => {
      if (!core) {
        return null;
      }
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      host.log(
        "warning",
        `[${host.id}] NLR environment preload timed out; starting surface system`
      );
      finish();
  ...
  const visibleLayers = layers
    .map((layer) => {
      const layerSurface = bundle.ui.uidoc.surfaces.find(
  ...
  const renderedLayerKeys = new Set(
    nlrPreloadDone ? visibleLayers.map((item) => item.layer.key) : []
  );
    ...
    return {
      ...
      mountSurface: (input) => {
        ...
        const executor = {
          ...
          closeScope: (scopeId: string, reason: string) =>
            core.executionManager.closeScope(scopeId, reason),
      ...
      getWidgetRuntimePatches: (input) =>
        widgetPatchesByScopeRef.current[input.runtimeScopeId] ?? {}
    };
  }, [
    bundle,
    goBack,
    ...
    void (async () => {
      try {
        ...
        if (err instanceof NlrSessionSupersededError) {
          ...
          host.log(
            "info",
            `[${host.id}] NLR hot reload restart superseded by a newer bundle revision`
          );
          return;
        }
  ...
  useEffect(() => {
    ...
    // would refuse it.
    const preference = (game as { preference?: { getPreference?: (key: string) => unknown } })
      .preference;
    ...
    const controller = createSkipRunController({
      ...
      isBlocked: () => {
        ...
        return (
          readPreference("skipReadText") === true &&
  ...
  useEffect(() => {
    if (
      ...
      !hostAdapterBundle ||
      !activeSurface ||
      !host.subscribeFullscreenChanged
    ) {
      return;
    }
    ...
    <NlrStageLayer
      ...
      onLiveGameReady={async (sessionId, liveGame) => {
        ...
        if (dialogGameState) {
          textReadTrackerRef.current = createTextReadTracker({
            ...
            setMirror: (value) =>
              core.scopeBridge.globalSet(BLUEPRINT_GAME_TEXT_READ_STATE_KEY, value),
        ...
        nlrCurrentActionTokenRef.current = liveGame.onCurrentActionChange(({ actionId }) => {
          ...
          currentActionListenersRef.current.forEach((listener) => {
            try {
              ...
              host.log(
                "warning",
                `[${host.id}] current-action listener failed: ${normalizeError(error)}`
              );
            }
          });
        });
      ...
      onError={(err, errorSessionId) => {
        ...
        if (nlrSession?.id !== errorSessionId) {
          host.log(
            "warning",
            `stale NLR session error (${errorSessionId}): ${normalizeError(err)}`
          );
          return;
        }
