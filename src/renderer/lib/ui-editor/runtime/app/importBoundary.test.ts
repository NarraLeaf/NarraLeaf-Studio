...
const FORBIDDEN_IMPORTS = [
  {
    pattern: /^@\/apps(?:\/|$)/,
    reason: "App-specific UI must stay outside the shared game app layer."
  },
  {
    pattern: /^@\/lib\/app\/bridge(?:\/|$)/,
    reason: "Studio renderer IPC bridge must be injected via the host interface."
  },
  {
    pattern: /^@\/lib\/plugins(?:\/|$)/,
    reason: "Studio plugin runtime is not part of the shared game app layer."
  },
  {
    pattern: /^@\/lib\/workspace(?:\/|$)/,
    reason: "Workspace services/hooks must not be consumed by the shared game app layer."
  },
  {
    pattern: /^@services(?:\/|$)/,
    reason: "Workspace service aliases must not be consumed by the shared game app layer."
  },
  {
    pattern: /^@\/components\/AppLayout(?:\/|$)/,
    reason: "Studio window layout must not be inherited by the shared game app layer."
  }
];
  ...
  it("does not import Studio app, bridge, workspace, or plugin modules", async () => {
    ...
    for (const filePath of await listSourceFiles(APP_ROOT)) {
      ...
      for (const match of source.matchAll(STATIC_IMPORT_PATTERN)) {
        ...
        if (forbidden) {
          violations.push(
            `${path.relative(APP_ROOT, filePath)} imports ${specifier}: ${forbidden.reason}`
          );
        }
      }
    }
