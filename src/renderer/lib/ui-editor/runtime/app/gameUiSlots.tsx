...
export function createGameUiSlotComponents(input: {
  ...
  return {
    ...
    menu: choiceSurface
      ...
      : undefined,
    nvlDialog: nvlSurface ? createNvlSlotComponent(slotHostOptions, nvlSurface) : undefined,
    onStageNode: onStageSurface ? createOnStageSlotNode(slotHostOptions, onStageSurface) : undefined
  };
}
...
export function createNlrGameWithGameUi(input: {
  ...
  const { width, height, contentContainerId, slots, minStageSize, audioBuses, hostOwnsSkipKey } =
    input;
  const game = new Game({
    app: { debug: false },
...
export type LiveGameUiCallbacks = Pick<
...
export function createLiveGameUiCallbacks(deps: LiveGameUiCallbackDeps): LiveGameUiCallbacks {
  ...
  return {
    ...
    getNotificationsInGame: (): BlueprintGameNotification[] => {
      ...
      const manager = (
        gameState as { notificationMgr?: { toArray?: () => unknown } } | null | undefined
      )?.notificationMgr;
    ...
    isNvlModeInGame: (): boolean => {
      const gameState = getLiveGame()?.getGameState?.();
      const nvlState = (
        gameState as { getNvlState?: () => { active?: unknown } } | null | undefined
      )?.getNvlState?.();
    ...
    setGamePreferenceInGame: async (
      ...
      const preference = requireLiveGame(`Set ${key} Preference`).game.preference as {
        setPreference: (
          preferenceKey: BlueprintGamePreferenceKey,
          preferenceValue: BlueprintGamePreferenceValue
        ) => void;
      };
