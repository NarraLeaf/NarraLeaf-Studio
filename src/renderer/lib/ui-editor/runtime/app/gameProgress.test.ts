...
describe("applying a document", () => {
    it("carries both saved and persistent values through a round trip", async () => {
        ...
        const document = buildGameProgressDocument("com.example.sablehours", {
            storyId: "story-1",
        ...
        expect(applyGameProgressVariables(SAVED, document.savedVariables, (key, value) => {
            freshLive[key] = value;
        })).toEqual(["gold", "route"]);
        expect(applyGameProgressVariables(PERSISTENT, document.persistentVariables, (key, value) => {
            freshApp[key] = value;
...
describe("what the node hands out", () => {
    it("reports the anchor's scene id, and blank when there is no anchor", () => {
        const anchored = buildGameProgressDocument("k", {
            storyId: "s",
            savedVariables: {},
            persistentVariables: {},
            anchor: { sceneId: "scene-3", sceneRuntimeName: "chapter-two" },
        ...
        const unanchored = buildGameProgressDocument("k", {
            storyId: "",
            savedVariables: {},
