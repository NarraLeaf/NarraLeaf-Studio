...
import {
  dialogSlotRuntimeScopeId,
  findStageSurfaceForSlot,
  stageSlotRuntimeScopeId
} from "./stageSlots";
...
describe("findStageSurfaceForSlot", () => {
  it("returns the stage surface mounted on the slot", () => {
    ...
    const doc = docWith([{ id: "s-app", kind: "appSurface" }, dialog]);
...
describe("stageSlotRuntimeScopeId", () => {
  it("builds the stable per-session scope id for each slot", () => {
    ...
    expect(stageSlotRuntimeScopeId("sess-1", "choice", "surf-3")).toBe(
      "nlr:sess-1:slot:choice:surf-3"
    );
    expect(stageSlotRuntimeScopeId("sess-1", "nvl", "surf-4")).toBe("nlr:sess-1:slot:nvl:surf-4");
    expect(stageSlotRuntimeScopeId("sess-1", "onStage", "surf-5")).toBe(
      "nlr:sess-1:slot:onStage:surf-5"
    );
  });
