...
export class LayerStackController {
  ...
  public show(request: SurfaceLayerMountRequest): string {
    ...
    const group =
      typeof request.group === "string" && request.group.trim() ? request.group.trim() : null;
  ...
  public isPresent(key: string): boolean {
    return (
      this.layers.some((layer) => layer.key === key) ||
      this.queued.some((layer) => layer.key === key)
    );
  }
  ...
  private isGroupTaken(group: string): boolean {
    return (
      this.layers.some((layer) => layer.group === group) ||
      this.queued.some((layer) => layer.group === group)
    );
  }
