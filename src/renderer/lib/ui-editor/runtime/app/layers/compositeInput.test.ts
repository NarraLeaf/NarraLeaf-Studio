...
function expectMatchesLegacy(state: CompositeInputState): void {
  ...
  for (const entry of state.pageEntries) {
    expect(resolution.interactiveKeys.has(entry.key), `interactive: ${entry.key}`).toBe(
      legacyAnswer(state, entry.key)
    );
    expect(resolution.keyboardOwnerKey === entry.key, `keyboard: ${entry.key}`).toBe(
      legacyAnswer(state, entry.key)
    );
  }
}
