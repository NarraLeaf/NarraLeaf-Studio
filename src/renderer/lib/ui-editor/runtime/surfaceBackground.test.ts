...
function createSurface(
  ...
  kind: UISurface["kind"] = "appSurface"
): UISurface {
  const base = {
    id: "surface",
    name: "Surface",
...
describe("surfaceBackground", () => {
  ...
  it("shows the editor outline only when the effective surface background is below 20 percent opacity", () => {
    expect(shouldShowEditorSurfaceLowOpacityOutline(createSurface(undefined, "appSurface"))).toBe(
      false
    );
    expect(shouldShowEditorSurfaceLowOpacityOutline(createSurface(undefined, "stageSurface"))).toBe(
      true
    );
    expect(shouldShowEditorSurfaceLowOpacityOutline(createSurface("rgba(10, 20, 30, 0.19)"))).toBe(
      true
    );
    expect(shouldShowEditorSurfaceLowOpacityOutline(createSurface("rgba(10, 20, 30, 0.2)"))).toBe(
      false
    );
  });

  it("keeps the white editor backing limited to fully transparent Game UI surfaces", () => {
    expect(getEditorSurfaceAreaBackgroundColor(createSurface("transparent", "stageSurface"))).toBe(
      "#ffffff"
    );
    expect(
      getEditorSurfaceAreaBackgroundColor(createSurface("rgba(10, 20, 30, 0.1)", "stageSurface"))
    ).toBeUndefined();
    expect(
      getEditorSurfaceAreaBackgroundColor(createSurface("transparent", "appSurface"))
    ).toBeUndefined();
  });
});
...
describe("surface background image", () => {
  ...
  it("reads a stored background image and defaults an unusable fill mode", () => {
    ...
    expect(
      getSurfaceBackgroundImage(withBackgroundImage({ assetId: "  ", fillMode: "tile" }))
    ).toBeNull();
    expect(
      getSurfaceBackgroundImage(withBackgroundImage({ assetId: "asset-1", fillMode: "crop" }))
    ).toEqual({
      assetId: "asset-1",
      fillMode: "cover"
    });
    expect(
      getSurfaceBackgroundImage(withBackgroundImage({ assetId: "asset-1", fillMode: "tile" }))
    ).toEqual({
      assetId: "asset-1",
      fillMode: "tile"
    });
  ...
  it("maps each fill mode onto the background CSS that draws it", () => {
    ...
    expect(surfaceBackgroundImageStyle("blob:one", "contain")).toMatchObject({
      backgroundSize: "contain"
    });
    expect(surfaceBackgroundImageStyle("blob:one", "stretch")).toMatchObject({
  ...
  it("escapes the url so a quoted asset path cannot break out of the CSS value", () => {
    expect(surfaceBackgroundImageStyle('blob:a"b\\c', "cover").backgroundImage).toBe(
      'url("blob:a\\"b\\\\c")'
    );
  });
  ...
  it("thins the picture for a page presented over a running game, by the same amount as its colour", () => {
    ...
    expect(getSurfaceLayerBackgroundImageOpacity("gameOverlay")).toBe(
      GAME_OVERLAY_BACKGROUND_ALPHA
    );
  });
});
