...
import type { NormalizedPluginManifestV2, PluginBuildConfigFieldContribution, RuntimePluginDescriptor } from "@shared/types/plugins";
...
describe("app.game.config", () => {
    ...
    it("reads the values of the plugin it belongs to and no other's", async () => {
        ...
        const itch = await writePlugin("acme.itch", [textField("appId")], { appId: "not-the-steam-one" });
    ...
    it("answers nothing for a field the manifest never declared", async () => {
        const plugin = await writePlugin("acme.undeclared", [textField("appId")], { appId: "480", sneaked: "in" });
        const lines: string[] = [];
