...
const runtimeWidgetRenderers = new Map<
  string,
  { ownerPluginId: string; def: RuntimeWidgetRendererDef }
>();
...
function createRuntimePluginApp(
  ...
  const readConfig = (key: string): string | null => {
    ...
    const declared = descriptor.manifest.contributes.buildConfig.find(
...
function buildCapabilityDomains(
  ...
  const declared = new Set<PluginRuntimeCapability>(
    descriptor.manifest.contributes.runtimeCapabilities
  );
  const domains: Partial<RuntimePluginGame> = {};

  /** Declared but unbacked is worth saying out loud — it is why nothing happens. */
  const unavailable = (capability: string): void =>
    log(
      "warning",
  ...
  if (declared.has("saves.read")) {
    ...
    if (!backend) {
      ...
      domains.saves = {
        ...
        ...(declared.has("saves.write") && write
          ? { write: (id, metadata) => write(id, metadata) }
          : {}),
  ...
  if (sidecarIds.length > 0) {
    ...
    if (!backend) {
      log(
        "info",
        "sidecars are not available in this environment (web and mobile shells have no child processes)"
      );
    } else {
      domains.sidecar = {
        available: (sidecarId) =>
          sidecarIds.includes(sidecarId) && backend.available(pluginId, sidecarId),
