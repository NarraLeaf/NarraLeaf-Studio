...
function namespaceOf(session: RuntimePluginStorySession, name: string): StorableNamespaceLike | null {
    if (!name) {
        return null;
    }
    try {
...
export function readRuntimePluginVariable(
    ...
    const namespaceName = scope === "saved"
...
export function snapshotEngineScopes(
    session: RuntimePluginStorySession | null,
): { scene: RuntimePluginStateSnapshot; saved: RuntimePluginStateSnapshot } {
