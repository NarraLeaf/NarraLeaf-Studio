...
describe("plugin store key namespacing", () => {
  ...
  it("produces a key the persistence validator accepts", () => {
    ...
    expect(
      PERSISTENCE_KEY_PATTERN.test(pluginStoreKey(pluginId, "narraleaf.gallery.unlocked"))
    ).toBe(true);
  });
  ...
  it("namespaces plugins apart from each other", () => {
    expect(pluginStoreKey("narraleaf.quick-save", "slot")).not.toBe(
      pluginStoreKey("narraleaf.gallery", "slot")
    );
  });
});
