...
class RuntimePluginEventHub {
  ...
  public constructor(
    private readonly log: (level: RuntimePluginLogLevel, message: string) => void
  ) {}
...
export class RuntimePluginHostController {
  ...
  public attachSession(attachment: RuntimePluginSessionAttachment): void {
    ...
    if (!bundle) {
      ...
      this.log(
        "warning",
        "runtime plugin host: game session attached before the blueprint runtime"
      );
    }
  ...
  public bindShellEvents(): () => void {
    ...
    if (this.shell.subscribeFullscreenChanged) {
      tokens.push(
        this.shell.subscribeFullscreenChanged((isFullscreen) => {
    ...
    if (this.shell.subscribeCloseRequested) {
      tokens.push(
        this.shell.subscribeCloseRequested(() => {
  ...
  private bindEngineEvents(liveGame: LiveGame): void {
    ...
    push(
      liveGame.onMenuChoose((payload: unknown) => {
        this.hub.emit("choiceMade", { text: readText(payload) });
      })
    );
    push(
      liveGame.onCharacterPrompt((payload: unknown) => {
    ...
    push(
      gameState.events.on("event:state.scene.mount", (scene: Scene) => {
    ...
    push(
      gameState.events.on("event:state.scene.unmount", (scene: Scene) => {
        const sceneId = this.sceneIdByScene.get(scene) ?? null;
    ...
    push(
      gameState.events.on("event:state.player.lineEnd", () => {
      ...
      state: {
        ...
        set: (scope: RuntimePluginStateScope, key: string, value: unknown) => {
          ...
          if (!written) {
            this.log(
              "warning",
              `runtime plugin state.set ignored: no "${scope}" variable named "${key}"`
            );
            return;
          }
        ...
        onChange: (listener) => {
          ...
          this.engineSnapshot = snapshotEngineScopes(this.session);
          this.persistentSnapshot = snapshotPersistentScope(
            this.session,
            this.attachment?.scope ?? null
          );
          return () => {
