 ...
 */
export function createDevModeBlueprintHostAdapter(
  options: DevModeBlueprintHostAdapterOptions
): UIHostAdapter {
  const { bundle, surface, runtimeScopeId, scopeBridge, debug, hostApi, executionManager } =
    options;
  ...
  const readRuntimeElement = (
    elementId: string,
    componentId?: UIComponentId
  ): UIElement | undefined => {
    if (componentId) {
      return document.components?.find((component) => component.id === componentId)?.elements[
        elementId
      ];
    }
  ...
  const dispatchElementBlueprintEventNow: UIHostAdapterBlueprintRuntime["dispatchElementBlueprintEvent"] =
    async (elementId, eventName, eventPayload, eventOptions) => {
      const flushedElement =
        eventName === "flush"
          ? readRuntimeElement(elementId, eventOptions?.componentId)
          : undefined;
  ...
  blueprintRuntime.dispatchElementBlueprintEvent = async (
    elementId,
    eventName,
    eventPayload,
    eventOptions
  ...
  blueprintRuntime.continueElementEventBubble = async (
    elementId,
    eventName,
    eventPayload,
    eventOptions
