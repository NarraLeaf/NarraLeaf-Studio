...
describe("createDevModeBlueprintHostAdapter", () => {
  it("continues a widget event bubble from child to parent", async () => {
    ...
    await adapter.blueprintRuntime?.dispatchElementBlueprintEvent("child", "mouseClick", {
      x: 4,
      y: 5,
      button: 0
    });
  ...
  function createUnlistenedChildFixture(childType: string) {
    ...
    return {
      adapter,
      readParentLocal,
      cleanup: () => releaseBlueprintWidgetLocals("surface", "parent", parentBlueprintId)
    };
  }
