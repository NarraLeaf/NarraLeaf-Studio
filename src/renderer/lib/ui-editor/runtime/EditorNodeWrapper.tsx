...
export function EditorNodeWrapper({
  ...
  const appearance = (element.props as { appearance?: AppearanceModel | null } | undefined)
    ?.appearance;
  const listScopedVariantId =
    typeof (element.extra as { runtimeVariantOverrideId?: unknown } | undefined)
  ...
  const appearanceResolveCtx = {
    variantOverrideId:
      listScopedVariantId ?? runtimeElementState.variantOverrideId ?? inspectorVariantId ?? null,
    signals: runtimeElementState.signals
  };
  const appearanceOpacity = resolveAppearanceDisplayableOpacity(appearance, {
  ...
  const effectiveOpacity = hasRuntimeOpacityOverride
    ? layoutOpacity
    : (appearanceOpacity ?? layoutOpacity);
  const baseRotation = layout.rotation ?? 0;
  const isResetPhase = Boolean(
    displayableMotion?.resetOnComplete && resetMotionId === displayableMotion.id
  );
  ...
  const dispatchWidgetEvent = useCallback(
    (
      ...
      if (
        !interactive ||
        !blueprintRuntime ||
        eventControl?.isPropagationStopped() ||
        !isDirectElementEvent(target)
      ) {
        return false;
      }
  ...
  const localMousePayload = useCallback(
    (
      e: MouseEvent<HTMLDivElement> | PointerEvent<HTMLDivElement> | WheelEvent<HTMLDivElement>
  ...
  const motionRunKey = displayableMotion
    ? `${displayableMotion.id}:${isResetPhase ? "reset" : "run"}`
    : null;
  ...
  return (
    <motion.div
      ...
      onPointerEnter={
        interactive && (widgetRuntimeStore || blueprintRuntime) ? onPointerEnter : undefined
      }
      onPointerLeave={
        interactive && (widgetRuntimeStore || blueprintRuntime) ? onPointerLeave : undefined
      }
      onPointerDown={
        interactive && (widgetRuntimeStore || blueprintRuntime) ? onPointerDown : undefined
      }
      onPointerUp={
        interactive && (widgetRuntimeStore || blueprintRuntime) ? onPointerUp : undefined
      }
