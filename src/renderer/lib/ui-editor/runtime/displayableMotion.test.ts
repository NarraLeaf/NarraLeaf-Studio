...
describe("displayable Motion props", () => {
  it("only sends explicitly animated properties to Motion", () => {
    expect(buildDisplayableMotionAnimateTarget({ x: [-500, 0] }, fallback)).toEqual({
      x: [-500, 0]
    });
    expect(buildDisplayableMotionInitialTarget({ x: [-500, 0] }, fallback)).toEqual({ x: -500 });
    expect(buildDisplayableMotionAnimateTarget({ opacity: [0, 1] }, fallback)).toEqual({
      opacity: [0, 1]
    });
    expect(buildDisplayableMotionInitialTarget({ opacity: [0, 1] }, fallback)).toEqual({
      opacity: 0
    });
  });
  ...
  it("lets Motion continue from the current value without injecting an initial target", () => {
    ...
    expect(buildDisplayableMotionAnimateTarget({ opacity: fromCurrent }, fallback)).toEqual({
      opacity: 1
    });
