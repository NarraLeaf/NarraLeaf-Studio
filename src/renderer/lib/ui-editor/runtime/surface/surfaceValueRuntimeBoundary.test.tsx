...
function rendererRegistry(): ElementRendererRegistry {
  return new ElementRendererRegistry([
    ...
    {
      type: "nl.text",
      render: (props) => <span>[{String(props.element.props?.text ?? "")}]</span>
    },
  ...
  it("gives every list row its own item field instead of one shared value", async () => {
    ...
    const view = mount(
      ...
      valueBlueprintDocument({
        blueprintId: "bp-value",
        elementId: "row",
        graph: listItemFieldGraph("line")
      })
    );
    await settle();

    expect(renderedTexts(view.container)).toEqual([
      "[First line]",
      "[Second line]",
      "[Third line]"
    ]);
