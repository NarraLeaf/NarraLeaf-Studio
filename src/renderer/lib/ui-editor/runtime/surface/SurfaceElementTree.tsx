...
export type NestedSurfaceRuntime = {
  ...
  mountSurface?(
    input: NestedSurfaceRuntimeInput & { hostAdapter: UIHostAdapter }
  ): void | (() => void);
  getWidgetRuntimePatches?(
    input: NestedSurfaceRuntimeInput
  ): Record<string, DevModeWidgetRuntimePatch> | undefined;
};
...
function SurfaceValueRuntimeBoundary(props: SurfaceElementTreeProps) {
  const { document, surface, hostAdapter, blueprintBindingContext } = props;
  ...
  return (
    <SurfaceElementTreeContent {...props} valueRuntime={valueRuntime} bindingTick={bindingTick} />
  );
}
...
function renderSurfaceElementTreeWithValueRuntime(
  ...
  return (
    <WidgetRuntimeScopeProvider
      runtimeScopeId={hostAdapter.blueprintRuntime?.runtimeScopeId ?? null}
    >
      {tree}
    </WidgetRuntimeScopeProvider>
  );
...
function defaultFrameRuntimeScopeId(
  input: Omit<NestedSurfaceRuntimeInput, "runtimeScopeId">
): string {
  const parentScope =
...
function NestedSurfaceRenderer(props: {
  ...
  const targetSurface = targetSurfaceId
    ? document.surfaces.find((surface) => surface.id === targetSurfaceId)
    : undefined;
  const invalidLabel =
  ...
  const runtimeScopeId = useMemo(() => {
    ...
    return (
      nestedSurfaceRuntime?.createRuntimeScopeId?.(runtimeBaseInput) ??
      defaultFrameRuntimeScopeId(runtimeBaseInput)
    );
  ...
  useEffect(() => {
    ...
    pendingRemoveAfterEnterKeyRef.current = holdCurrentUntilEnterComplete
      ? runtimeInput.runtimeScopeId
      : null;
...
function NestedSurfaceInstance(props: {
  ...
  useEffect(
    () => nestedSurfaceRuntime?.mountSurface?.({ ...runtimeInput, hostAdapter }),
...
function renderLinkedComponentInstanceContent(input: {
  ...
  const componentInstanceKey = buildUIComponentInstanceKey(
    input.instanceKey,
    input.instanceElement.id
  );
  ...
  const animatedContent =
    animationTiming && animated && animationTiming.selfAnimated ? (
      <ElementAnimationLayer
        ...
        reducedMotion={animationPlan?.reducedMotion === true}
      >
        {content}
      </ElementAnimationLayer>
    ) : (
