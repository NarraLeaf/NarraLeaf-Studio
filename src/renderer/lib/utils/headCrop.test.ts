...
describe("findHeadCrop", () => {
  ...
  it("reports nothing when the sprite is a face close-up", () => {
    ...
    expect(
      findHeadCrop(sprite(100, 100, [{ from: 5, to: 95, left: 10, right: 90 }]), 100, 100)
    ).toBeNull();
  });
  ...
  const REAL_SPRITES: Record<string, { extents: number[]; head: [number, number] }> = {
    ...
    YouKi: {
      extents: [
        6, 10, 12, 17, 22, 24, 26, 24, 27, 29, 30, 32, 34, 34, 36, 36, 38, 38, 38, 38, 39, 39, 38,
        37, 37, 38, 40, 45, 43, 44, 51, 47, 48, 49, 49, 47, 50, 47, 47, 46, 45, 40, 38, 34, 32, 31,
        29, 32, 34, 36, 36, 38, 38, 39, 40, 41, 41, 42, 43, 43, 45, 45, 45, 46, 46, 45, 44, 44, 45,
        44, 42, 40, 40, 40
      ],
    ...
    Nattou: {
      extents: [
        5, 7, 8, 2, 3, 3, 3, 16, 17, 19, 21, 24, 26, 27, 29, 31, 31, 33, 33, 35, 35, 35, 37, 37, 37,
        38, 40, 44, 43, 39, 40, 41, 42, 43, 43, 45, 40, 40, 39, 39, 38, 37, 37, 35, 35, 35, 37, 37,
        37, 37, 37, 38, 38, 38, 39, 40, 40, 40, 40, 41, 42, 42, 43, 43, 43, 45, 45, 45, 46, 46, 47,
        48, 48, 50, 50
      ],
      head: [0, 43]
    }
