 ...
 */
export const DEVELOPER_MENU_ROW_IDS: ReadonlySet<string> = new Set(
  ALL_KINDS.map(developerCopyIdRowId)
);
...
export function appendDeveloperIdSection(
  ...
  const separator: ContextMenuDef =
    items.length > 0 && !endsWithSeparator(items)
