...
type PermissionOf<K extends PluginInstallPermission["kind"]> = Extract<
  PluginInstallPermission,
  { kind: K }
>;
  ...
  return (
    <div className={cn("space-y-3", className)}>
          ...
          {groups.sidecars.map((permission, index) => {
            ...
            return (
              <PermissionRow key={`${permission.id}-${index}`} tone="warning">
                 ...
                 */}
                {kind ? <div className="mt-0.5 text-xs text-warning/80">{kind}</div> : null}
      ...
      {groups.runtime.length > 0 ? (
        <PermissionGroup
          label={t("pluginPermission.permissions.section.runtime")}
          rounded={rounded}
        >
      ...
      {groups.network.length > 0 ? (
        <PermissionGroup
          ...
          rounded={rounded}
        >
          <PermissionRow>{t("pluginPermission.permissions.section.networkNote")}</PermissionRow>
      ...
      {groups.studio.length > 0 ? (
        <PermissionGroup label={t("pluginPermission.permissions.section.studio")} rounded={rounded}>
          {groups.studio.map((permission, index) => (
            <PermissionRow key={index}>{describePluginInstallPermission(permission)}</PermissionRow>
          ))}
        </PermissionGroup>
      ) : null}
    </div>
