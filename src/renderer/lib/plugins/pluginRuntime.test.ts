...
describe("createPluginApp disposal", () => {
  const descriptor = {
    ...
    manifest: {
      ...
      contributes: {
        blueprintNodes: ["test-plugin.node"],
        widgets: ["test-plugin.widget"],
        locales: []
      },
      permissions: []
    },
  ...
  it("reclaims panel, action, group, keybinding, widget, and dynamic-source registrations in reverse order", () => {
    const { ctx, calls, store, panelDisposer, keybindingDisposer, dynamicSourceDisposer } =
      createFakeContext();
  ...
  it("rejects blueprint node registrations not declared in manifest contributes", () => {
    ...
    expect(() =>
      app.services.blueprintNodes.register({ type: "test-plugin.undeclared" } as any)
  ...
  it("keeps hostAdapter out of the context a plugin node's execute receives", async () => {
    ...
    expect(Object.keys(pluginCtx.game).sort()).toEqual([
      "blueprintNodes",
      "config",
      "data",
      "log",
  ...
  it("confines a plugin's action group to a menu of its own", () => {
    ...
    app.services.ui.actions.registerGroup({
      ...
      label: "Evil",
      menuSlot: "edit",
      actions: [{ id: "test-plugin:paste", label: "Paste", menuRole: "paste", onClick: vi.fn() }]
    } as any);
      ...
      items: [
        {
          ...
          label: "More",
          items: [{ id: "test-plugin:copy", label: "Copy", menuRole: "copy", onClick: vi.fn() }]
        }
      ]
    } as any);
  ...
  it("rejects widget registrations not declared in manifest contributes", () => {
    ...
    expect(() =>
      app.services.widgets.register({
        type: "test-plugin.undeclared-widget"
      } as unknown as UIWidgetModule)
  ...
  it("keeps disposing after one disposer throws", () => {
    ...
    uiService.keybindings.register.mockReturnValueOnce(
      vi.fn(() => {
