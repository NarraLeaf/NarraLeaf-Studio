...
export function usePluginCatalog(hooks?: PluginCatalogHooks): PluginCatalog {
    ...
    const installLocal = useCallback(() => void runTask(t("plugins.task.installing"), async () => {
 ...
 */
async function restoreQuietly(hooks: PluginCatalogHooks | undefined, pluginId: string): Promise<void> {
    try {
