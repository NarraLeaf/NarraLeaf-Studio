...
function loadAssetSelector(): Promise<AssetSelectorComponent> {
  assetSelectorLoader ??= import("@/apps/workspace/modules/assets/components/AssetSelector").then(
...
export function PluginPanelHeader({
  ...
  return (
    <div
      className={cx(
        "flex items-start justify-between gap-3 border-b border-edge pb-3 mb-3",
        className
      )}
      {...props}
    >
...
export function PluginPanelToolbar({ className, children, ...props }: PluginPanelToolbarProps) {
  return (
...
export function PluginPanelRow({
  ...
  return (
    <div
      className={cx("flex min-w-0 items-center justify-between gap-3 py-1.5", className)}
      {...props}
    >
      <div className="min-w-0">
        {label && <div className="text-sm text-fg truncate">{label}</div>}
        {description && (
          <div className="mt-0.5 text-xs text-fg-subtle leading-relaxed">{description}</div>
        )}
        {children}
      </div>
...
export function PluginPanelEmptyState({
  ...
  return (
    <div
      className={cx(
        "flex min-h-36 flex-col items-center justify-center rounded-md border border-dashed border-edge px-4 py-6 text-center",
        className
      )}
      {...props}
    >
      ...
      {description && (
        <div className="mt-1 max-w-xs text-xs leading-relaxed text-fg-subtle">{description}</div>
      )}
