...
const librarySpec: DocumentSpec<Library> = defineDocumentSpec<Library>({
    ...
    summarize: document => ({title: document.locale, counts: [{key: "units", value: Object.keys(document.units).length}]}),
});

const canonicalLibrary = librarySpec.serialize({schemaVersion: 2, locale: "en-US", units: {b: "two", a: "one"}});
...
describe("loadDocument", () => {
    ...
    it("reports bytes that are valid but not canonical as not normalized", async () => {
        storage.files.set(DOCUMENT_PATH, JSON.stringify({schemaVersion: 2, locale: "en-US", units: {}}));
    ...
    it("accepts a Windows path and normalises it", async () => {
        ...
        expect((await loadDocument(librarySpec, storage, "editor\\voice\\en-US.json")).status).toBe("loaded");
    });

    it("rejects a path that is not project-relative", async () => {
        await expect(loadDocument(librarySpec, storage, "D:/game/editor/voice/en-US.json"))
...
describe("loadDocument: corruption containment", () => {
    ...
    it("wraps a spec that throws something other than DocumentCorruptError", async () => {
        ...
        const result = await loadDocument(throwingSpec, storage, "editor/localization/en.json", {now: () => AT});
    ...
    it("still reports corruption when quarantine itself fails", async () => {
        ...
        expect(result.status === "corrupt" && (result.quarantineFailure as Error).message).toBe("disk full");
    ...
    it("uses the injected clock, so two failures in one session do not collide", async () => {
        ...
        expect(second.status === "corrupt" && second.quarantinePath)
            .toBe(`.nlstudio/quarantine/2026-07-27T14-32-12-000Z/${DOCUMENT_PATH}`);
    });
});
...
describe("loadDocument: no file contents can make it throw", () => {
    ...
    it("survives a sweep of hostile file contents", async () => {
        const contents: Record<string, string> = {
            ...
            wideObject: JSON.stringify(Object.fromEntries(Array.from({length: 2000}, (_, i) => [`k${i}`, i]))),
    ...
    it("round-trips the contents it does accept", async () => {
        for (const text of ["{}", "[]", "null", "42", "{\"__proto__\": 1}", "{\"a\": -0}", "{\"a\": 1e-400}"]) {
            const {result, storage} = await loadContents(text);
...
describe("quarantinePathFor", () => {
    it("keeps the original path under a timestamped directory", () => {
        expect(quarantinePathFor("editor/story/stories/abc/storydoc.json", AT))
            .toBe(`.nlstudio/quarantine/${STAMP}/editor/story/stories/abc/storydoc.json`);
    });
    ...
    it("normalises the original path so a Windows path cannot escape the quarantine tree", () => {
        expect(quarantinePathFor("editor\\voice\\en-US.json", AT)).toBe(`.nlstudio/quarantine/${STAMP}/${DOCUMENT_PATH}`);
...
describe("saveDocument", () => {
    ...
    it("writes canonical bytes", async () => {
        await saveDocument(librarySpec, storage, DOCUMENT_PATH, {schemaVersion: 2, locale: "en-US", units: {b: "two", a: "one"}});
    ...
    it("does not touch the file when serialisation rejects the document", async () => {
        ...
        await expect(saveDocument(librarySpec, storage, DOCUMENT_PATH, {
            schemaVersion: 2,
            locale: "en-US",
    ...
    it("rejects a path that is not project-relative", async () => {
        await expect(saveDocument(librarySpec, storage, "/etc/passwd", {schemaVersion: 2, locale: "x", units: {}}))
            .rejects.toThrow(DocumentPathError);
    });
});
