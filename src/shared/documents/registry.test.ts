...
describe("DocumentRegistry.resolve", () => {
    ...
    it("accepts Windows separators, which is how callers will hand paths over", () => {
        ...
        expect(registry.resolve("assets\\assets.metadata.audio.json")?.spec.kind).toBe("assets-metadata");
    });

    it("gives a literal path priority over a parameterised one that also matches it", () => {
        expect(registry.resolve("editor/story/animations/index.json")?.spec.kind).toBe("story-animation-index");
        expect(registry.resolve("editor/story/animations/fade-in.json")?.spec.kind).toBe("story-animation");
    ...
    it("throws rather than silently missing when handed a path that is not project-relative", () => {
        ...
        expect(() => registry.resolve("D:/Projects/game/editor/ui/uidoc.json")).toThrow(DocumentPathError);
...
describe("DocumentRegistry.register", () => {
    it("refuses the same path twice", () => {
        ...
        expect(() => registry.register(fixtureSpec("ui-graphs", ["editor/ui/uidoc.json"])))
    ...
    it("refuses the same path expressed through equivalent patterns", () => {
        ...
        expect(() => registry.register(fixtureSpec("voice", ["editor/localization/<code>.json"])))
            .toThrow(DocumentRegistrationError);
    });
    ...
    it("refuses an overlap where neither pattern is more specific", () => {
        ...
        expect(() => registry.register(fixtureSpec("voice", ["editor/story/<name>.json"])))
            .toThrow(/neither is more specific/);
    });

    it("refuses a spec declaring one path twice", () => {
        const registry = new DocumentRegistry();

        expect(() => registry.register(fixtureSpec("story", [
    ...
    it("refuses a second spec for the same kind", () => {
        ...
        expect(() => registry.register(fixtureSpec("voice", ["editor/voice/legacy.json"])))
            .toThrow(/already registered/);
    });

    it("refuses a spec with no paths", () => {
        expect(() => new DocumentRegistry().register(fixtureSpec("voice", [])))
            .toThrow(DocumentRegistrationError);
    });

    it("accepts a spec owning several unrelated paths", () => {
        const registry = new DocumentRegistry();
        registry.register(fixtureSpec("story", [
            "editor/story/index.json",
            "editor/story/stories/<storyId>/storydoc.json",
        ]));

        expect(registry.resolve("editor/story/index.json")?.spec.kind).toBe("story");
        expect(registry.resolve("editor/story/stories/x/storydoc.json")?.parameters).toEqual({storyId: "x"});
    });
...
describe("defineDocumentSpec", () => {
    ...
    it("builds paths from parameters, and agrees with matches()", () => {
        ...
        expect(story.pathFor({storyId: "1f0a-77"})).toBe("editor/story/stories/1f0a-77/storydoc.json");
        ...
        for (const [spec, parameters] of [[story, {storyId: "x"}], [locale, {locale: "fr"}], [assets, {type: "audio"}]] as const) {
    ...
    it("serialises to canonical JSON unless the spec says otherwise", () => {
        expect(fixtureSpec("story", ["editor/story/index.json"]).serialize({b: 1, a: 2}))
            .toBe("{\n  \"a\": 2,\n  \"b\": 1\n}\n");
