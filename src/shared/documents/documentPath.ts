...
export function normalizeDocumentPath(value: string): string {
    ...
    if (value.includes("\0")) {
        throw new DocumentPathError(`A document path may not contain a NUL character: ${JSON.stringify(value)}`);
    }

    const normalized = value.replace(/\\/g, "/");
    if (normalized.startsWith("/") || /^[A-Za-z]:\//.test(normalized)) {
        throw new DocumentPathError(`A document path must be relative to the project root, got: ${value}`);
    }

    const segments = normalized.split("/");
    if (segments.some(segment => segment === "" || segment === "." || segment === "..")) {
        throw new DocumentPathError(`A document path may not contain empty, "." or ".." segments: ${value}`);
    }
 ...
 */
export function matchDocumentPath(pattern: DocumentPathPattern, relativePath: string): Record<string, string> | null {
    ...
    for (let index = 0; index < segments.length; index += 1) {
        ...
        parameters[expected.name] = actual.slice(expected.prefix.length, actual.length - expected.suffix.length);
    }
...
export function documentPathParameterNames(pattern: DocumentPathPattern): string[] {
    return pattern.segments
        .filter((segment): segment is Extract<DocumentPathSegment, {kind: "parameter"}> => segment.kind === "parameter")
...
export function formatDocumentPath(
    ...
    const path = pattern.segments.map(segment => {
    ...
    if (!roundTrip || documentPathParameterNames(pattern).some(name => roundTrip[name] !== parameters[name])) {
        throw new DocumentPathError(
 ...
 */
export function documentPathPatternsOverlap(a: DocumentPathPattern, b: DocumentPathPattern): boolean {
 ...
 */
export function documentPathPatternSubsumes(a: DocumentPathPattern, b: DocumentPathPattern): boolean {
    return a.segments.length === b.segments.length
...
function segmentsOverlap(a: DocumentPathSegment, b: DocumentPathSegment): boolean {
    ...
    if (a.kind === "literal") {
        return literalMatchesParameter(a.text, b as Extract<DocumentPathSegment, {kind: "parameter"}>);
    }
    if (b.kind === "literal") {
...
function literalMatchesParameter(text: string, parameter: Extract<DocumentPathSegment, {kind: "parameter"}>): boolean {
    ...
    // that cannot exist.
    return text.startsWith(parameter.prefix)
