...
function chapterRows(base: StoryDocument, head: StoryDocument): DocumentChange[] {
  ...
  for (const entry of diffKeyed(baseChapters, headChapters)) {
    ...
    if (!entry.base || !entry.head) {
      ...
      rows.push(
        change(path, entry.kind, entry.head ? LABEL.chapterAdded : LABEL.chapterRemoved, {
    ...
    if (!sameJsonValue(entry.base.name, entry.head.name)) {
      rows.push(
        change([...path, "name"], "changed", LABEL.chapterRenamed, {
    ...
    if (!sameJsonValue(wasScenes, nowScenes)) {
      rows.push(
        change([...path, "sceneIds"], "changed", LABEL.chapterScenes, {
          params: {
    ...
    if (!sameJsonValue(entry.base.meta, entry.head.meta)) {
      rows.push(
        change([...path, "meta"], "changed", LABEL.documentField, {
          params: { field: "meta" },
          subject: name
        })
      );
...
function sharedSceneIds(base: StoryDocument, head: StoryDocument): Set<string> {
  ...
  return new Set(
    Object.keys(headScenes).filter((id) => Object.prototype.hasOwnProperty.call(baseScenes, id))
  );
}
...
function sceneLeaves(
  ...
  if (!sameJsonValue(base.name, head.name)) {
    leaves.push(
      change([...path, "name"], "changed", LABEL.sceneRenamed, {
        params: fromToParams(base.name, head.name),
        subject: authoredName(head.name)
      })
    );
  ...
  for (const field of SCENE_FIELDS) {
    ...
    if (!sameJsonValue(left, right)) {
      leaves.push(
        change([...path, field], presence(left, right), LABEL.sceneField, {
...
function blockLeaves(
  ...
  for (const entry of diffKeyed(base.blocks ?? {}, head.blocks ?? {})) {
    ...
    if (!entry.base || !entry.head) {
      ...
      push(
        change(blockPath, entry.kind, entry.head ? LABEL.blockAdded : LABEL.blockRemoved, {
    ...
    if (entry.base.kind !== entry.head.kind) {
      push(
        change([...blockPath, "kind"], "changed", LABEL.blockKind, {
    ...
    if (!sameJsonValue(entry.base.payload, entry.head.payload)) {
      push(
        change([...blockPath, "payload"], "changed", LABEL.blockChanged, { subject }),
        entry.key
      );
    }
    ...
    if (!sameJsonValue(entry.base.childrenIds, entry.head.childrenIds)) {
      push(
        change([...blockPath, "childrenIds"], "moved", LABEL.blockOrder, { subject }),
        entry.key
      );
    }
    ...
    if (!sameJsonValue(entry.base.diagnosticsMeta, entry.head.diagnosticsMeta)) {
      push(
        change([...blockPath, "diagnosticsMeta"], "changed", LABEL.blockField, {
...
function blockSubject(block: StoryBlock | undefined): string | undefined {
  ...
  return (
    authoredName(previewValue(text?.value)) ??
