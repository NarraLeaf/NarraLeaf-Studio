...
function characterLeaves(
  path: readonly string[],
  base: StoredCharacter,
  head: StoredCharacter
): DocumentChange[] {
  ...
  if (!sameJsonValue(baseProfile.name, headProfile.name)) {
    leaves.push(
      change([...path, "name"], "changed", LABEL.renamed, {
  ...
  for (const field of PROFILE_FIELDS) {
    if (!sameJsonValue(baseProfile[field], headProfile[field])) {
      leaves.push(
        change(
          [...path, field],
          presence(baseProfile[field], headProfile[field]),
          LABEL.profileField,
          {
...
function appearanceLeaves(
  ...
  if (!base || !head) {
    return sameJsonValue(base, head)
      ? []
      : [
          change([...path], presence(base, head), LABEL.appearanceField, {
            params: { field: "appearance" }
          })
        ];
  }
  if (base.kind !== head.kind) {
    return [
      change([...path, "kind"], "changed", LABEL.kindChanged, {
...
function presetLeaves(
  path: readonly string[],
  base: PresetAppearance,
  head: PresetAppearance
): DocumentChange[] {
  const leaves: DocumentChange[] = [];
  ...
  for (const entry of diffKeyed(basePoses, headPoses)) {
    ...
    if (!entry.base || !entry.head) {
      ...
      leaves.push(
        change(posePath, entry.kind, entry.head ? LABEL.poseAdded : LABEL.poseRemoved, {
    ...
    if (!sameJsonValue(entry.base.assetId, entry.head.assetId)) {
      leaves.push(
        change([...posePath, "assetId"], "changed", LABEL.poseAsset, {
    ...
    if (!sameJsonValue(entry.base.name, entry.head.name)) {
      leaves.push(
        change([...posePath, "name"], "changed", LABEL.poseRenamed, {
    ...
    if (
      !sameJsonValue(entry.base.folder, entry.head.folder) ||
      !sameJsonValue(entry.base.portrait, entry.head.portrait)
    ) {
      leaves.push(
        change(posePath, "changed", LABEL.poseChanged, {
          subject: authoredName(entry.head.name)
        })
      );
    }
  ...
  if (!sameJsonValue(base.defaultPoseId, head.defaultPoseId)) {
    leaves.push(
      change([...path, "defaultPoseId"], "changed", LABEL.defaultPose, {
...
function layeredLeaves(
  path: readonly string[],
  base: LayeredAppearance,
  head: LayeredAppearance
): DocumentChange[] {
  const leaves: DocumentChange[] = [];
  ...
  for (const entry of diffKeyed(byId<CharacterAxis>(base.axes), byId<CharacterAxis>(head.axes))) {
    ...
    if (!entry.base || !entry.head) {
      leaves.push(
        change(axisPath, entry.kind, entry.head ? LABEL.axisAdded : LABEL.axisRemoved, {
          subject: authoredName(present?.name)
        })
      );
      continue;
    ...
    leaves.push(
      change(axisPath, "changed", LABEL.axisChanged, { subject: authoredName(entry.head.name) })
    );
  }

  for (const entry of diffKeyed(
    byId<CharacterLayer>(base.layers),
    byId<CharacterLayer>(head.layers)
  )) {
    const layerPath = [...path, "layers", entry.key];
    if (!entry.base || !entry.head) {
      const present = (entry.head ?? entry.base) as CharacterLayer;
      leaves.push(
        change(layerPath, entry.kind, entry.head ? LABEL.layerAdded : LABEL.layerRemoved, {
          subject: authoredName(present?.name)
        })
      );
      continue;
    ...
    if (!sameJsonValue(entry.base.assetId, entry.head.assetId)) {
      leaves.push(
        change([...layerPath, "assetId"], "changed", LABEL.layerAsset, { subject: layerName })
      );
    }
    ...
    for (const option of diffKeyed(entry.base.options, entry.head.options)) {
      ...
      leaves.push(
        change([...layerPath, "options", option.key], option.kind, LABEL.layerOptionAsset, {
    ...
    if (
      !sameJsonValue(entry.base.name, entry.head.name) ||
      !sameJsonValue(entry.base.axisId, entry.head.axisId)
    ) {
      leaves.push(
        change(layerPath, "changed", LABEL.layerChanged, {
          params: fromToParams(entry.base.name, entry.head.name),
          subject: layerName
        })
      );
  ...
  for (const field of LAYERED_FIELDS) {
    if (!sameJsonValue(base[field], head[field])) {
      leaves.push(
        change([...path, field], presence(base[field], head[field]), LABEL.appearanceField, {
          params: { field }
        })
      );
    }
  ...
  if (!sameJsonValue(base.snapshots, head.snapshots)) {
    leaves.push(
      change([...path, "snapshots"], "changed", LABEL.appearanceField, {
        params: { field: "snapshots" }
      })
    );
  }
  leaves.push(...avatarLeaves(path, base.avatars, head.avatars));
...
function puppetLeaves(
  path: readonly string[],
  base: PuppetAppearance,
  head: PuppetAppearance
): DocumentChange[] {
  const leaves: DocumentChange[] = [];
...
function avatarLeaves(
  ...
  return diffKeyed(base, head).map((entry) =>
    change([...path, "avatars", entry.key], entry.kind, LABEL.avatarChanged, {
...
function orderChanged(
  base: readonly { id?: unknown }[] | undefined,
  head: readonly { id?: unknown }[] | undefined
): boolean {
  ...
  return (
    right.every((id) => inBase.has(id)) &&
    (base ?? []).some((element, index) => element?.id !== head?.[index]?.id)
  );
}
