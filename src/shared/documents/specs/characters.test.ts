...
function preset(
  id: string,
  name: string,
  poses: PresetAppearance["poses"],
  defaultPoseId: string | null = null
): StoredCharacter {
  return {
    profile: {
      id,
...
async function loadText(text: string) {
  ...
  return loadDocument(charactersSpec, storage, PATH, {
...
describe("characters spec: reading and writing", () => {
  ...
  it("round-trips a cast with every optional field absent", async () => {
    const document = store(
      preset("alice", "Alice", [{ id: "p1", name: "angry", assetId: "asset-1" }], "p1")
    );
              ...
              poses: [
                {
                  ...
                  folder: "school",
                  assetId: "a1",
                  portrait: { x: 0, y: 0, w: 1, h: 1 }
                }
              ],
              defaultPoseId: "p1",
              avatars: {
                p1: { baked: "fp", overrideAssetId: null, portrait: { x: 0, y: 0, w: 1, h: 1 } }
              }
            }
          }
        }
  ...
  it("refuses a store a newer Studio wrote, before the migration can touch it", async () => {
    const result = await loadText(
      JSON.stringify({
        ...
        characters: [
          { profile: { id: "a", name: "A", appearance: { kind: "hologram", frames: [] } } }
        ]
      })
    );
  ...
  it("refuses a characters field that is not an array", async () => {
    ...
    expect(result.status).toBe("corrupt");
    expect(result.status === "corrupt" && result.error.reason).toContain(
  ...
  it("migrates a pre-rework store into something that can actually be saved", async () => {
        ...
        characters: [
          {
            profile: {
              ...
              nicknames: [],
              appearance: {
                forms: [{ name: "school", variantAssets: { angry: { data: { id: "asset-1" } } } }]
              }
            }
          }
        ]
    ...
    await expect(
      saveDocument(charactersSpec, storage, PATH, result.document)
...
describe("characters spec: diff", () => {
  ...
  it("reports a changed differential as ONE row naming the character and the variant", () => {
    ...
    expect(row.children![0].path).toEqual([
      "characters",
      "alice",
      "appearance",
      "poses",
  ...
  it("says the same thing for a layered character, naming the tag rather than the layer id", () => {
    const appearance = (angryAsset: string): LayeredAppearance => ({
      ...
      axes: [
        { id: "x1", name: "expression", tags: [{ id: "t1", name: "angry" }], defaultTagId: "t1" }
      ],
      layers: [{ id: "l1", name: "face", axisId: "x1", assetId: null, options: { t1: angryAsset } }]
    });

    const result = diff(
      store(layered("alice", "Alice", appearance("a1"))),
      store(layered("alice", "Alice", appearance("a9")))
    );
  ...
  it("reports a kind switch once, without listing the poses it discarded", () => {
    const base = store(
      preset("alice", "Alice", [{ id: "p1", name: "angry", assetId: "a1" }], "p1")
    );
    const head = store(
      layered("alice", "Alice", { kind: "layered", canvas: null, axes: [], layers: [] })
    );
  ...
  it("reports a reordered cast as one row about the order, not a row per character", () => {
    ...
    expect(result.changes[0]).toMatchObject({
      kind: "moved",
      label: { key: "documentDiff.characters.castOrder" }
    });
  });
  ...
  it("gives identical documents an empty, complete diff", () => {
    ...
    expect(diff(document, structuredClone(document))).toMatchObject({
      changes: [],
      total: 0,
      complete: true
    });
  ...
  it("respects the budget, sorts before it truncates, and says how much is missing", () => {
    const many = (count: number, asset: string) =>
      store(
        ...Array.from({ length: count }, (_, index) =>
          preset(`c${index}`, `Char ${String(index).padStart(2, "0")}`, [
            { id: "p1", name: "angry", assetId: asset }
          ])
        )
      );
    ...
    expect(result.changes.map((change) => change.subject)).toEqual([
      "Char 00",
      "Char 01",
      "Char 02",
      "Char 03"
  ...
  it("never throws, whatever shape it is handed", () => {
    const broken = {
      characters: [null, { profile: null }, { profile: { id: "a" } }]
    } as unknown as CharacterStoreDocument;
