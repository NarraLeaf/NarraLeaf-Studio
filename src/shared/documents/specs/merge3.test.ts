...
function contextFor(
  path: string,
  kind: "localization" | "assets-metadata",
  text: string
...
function reparseLibrary(document: LocalizationDocument): LocalizationDocument {
  ...
  return localizationDocumentSpec.parse(
    JSON.parse(text),
    contextFor(LOCALE_PATH, "localization", text)
  );
}
...
function reparseShard(document: AssetsMetadataShard): AssetsMetadataShard {
  ...
  return assetsMetadataSpec.parse(
    JSON.parse(text),
    contextFor(IMAGE_SHARD, "assets-metadata", text)
  );
}
...
describe("assets-metadata merge3", () => {
  ...
  it("merges two people importing different assets with no conflict at all", () => {
    ...
    expect(
      merged.decisions.map((one) => [one.path.join("/"), one.outcome, one.label?.key])
    ).toEqual([
