...
function story(scenes: StoryScene[], chapters?: StoryChapter[]): StoryDocument {
  return {
    ...
    name: "A Story",
    chapters: chapters ?? [
      { id: "ch1", name: "Chapter One", sceneIds: scenes.map((one) => one.id) }
    ],
...
function textOf(document: StoryDocument, sceneId: string, blockId: string): unknown {
  return (blockOf(document, sceneId, blockId).payload as { text?: { value?: unknown } }).text
    ?.value;
}
...
describe("story merge3: what merges", () => {
  it("merges two people working in different scenes with nothing to decide", () => {
    const base = story([
      ...
      scene("s2", "Market", [block("b2", "wares")])
    ]);
  ...
  it("merges two rows of one scene, one edited by each side", () => {
    ...
    const mine = story([
      scene("s1", "Prologue", [block("b1", "hello there"), block("b2", "goodbye")])
    ]);
    const theirs = story([
      scene("s1", "Prologue", [block("b1", "hello"), block("b2", "farewell")])
    ]);
  ...
  it("answers an array BOTH sides rearranged with one decision for the whole array", () => {
    ...
    const chapters = (order: string[]): StoryChapter[] =>
      order.map((id) => ({ id, name: id, sceneIds: [] }));
    ...
    expect((merged.decisions[0].mine.value as StoryChapter[]).map((one) => one.id)).toEqual([
      "ch3",
      "ch1",
      "ch2"
    ]);
    expect((merged.decisions[0].theirs.value as StoryChapter[]).map((one) => one.id)).toEqual([
      "ch2",
      "ch3",
      "ch1"
    ]);
  ...
  it("does not auto-take a document field only one side holds when there is no base", () => {
    ...
    expect(merged.decisions.find((one) => one.path[0] === "entrySceneId")?.theirs.present).toBe(
      false
    );
  });
...
describe("story merge3: the refusal", () => {
  const base = story([
    scene("s1", "Prologue", [block("b1", "a"), block("b2", "b"), block("b3", "c")])
  ]);
  ...
  it("refuses rather than throwing, whatever it is handed", () => {
    ...
    expect(() =>
      merge3(base, base, { ...base, scenes: null } as unknown as StoryDocument)
    ).not.toThrow();
  });
  ...
  it("refuses when one side deleted a row the other edited", () => {
    ...
    const mine = story([
      scene("s1", "Prologue", [base.scenes.s1.blocks.b1, base.scenes.s1.blocks.b3])
    ]);
    const theirs = story([
      scene("s1", "Prologue", [
  ...
  it("does NOT refuse when both sides made the SAME rearrangement", () => {
    ...
    const mine = story([
      scene(
        "s1",
        "Prologue",
        same.map((id) => base.scenes.s1.blocks[id])
      )
    ]);
    const theirs = story([
      scene(
        "s1",
        "Prologue",
        same.map((id) => (id === "b1" ? block("b1", "a, rewritten") : base.scenes.s1.blocks[id]))
      )
    ]);
