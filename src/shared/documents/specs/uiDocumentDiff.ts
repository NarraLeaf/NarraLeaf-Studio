...
export function diffUIDocument(
  ...
  if (!sameJsonValue(base?.name, head?.name)) {
    rows.push(
      DOCUMENT_SLOT,
      change(["name"], "changed", LABEL.renamed, {
...
function surfaceRows(left: DocumentIndex, right: DocumentIndex, rows: RankedRows): FoldedSurfaces {
  ...
  for (const entry of diffKeyed(left.surfaces, right.surfaces)) {
    ...
    if (!entry.base || !entry.head) {
      ...
      rows.push(
        slot,
        change(path, entry.kind, entry.head ? LABEL.surfaceAdded : LABEL.surfaceRemoved, {
...
function surfaceLeaves(
  ...
  if (!sameJsonValue(base.name, head.name)) {
    leaves.push(
      change([...path, "name"], "changed", LABEL.surfaceRenamed, {
        params: fromToParams(base.name, head.name),
  ...
  if (!sameJsonValue(base.designSize, head.designSize)) {
    ...
    leaves.push(
      size
        ? change([...path, "designSize"], "changed", LABEL.surfaceDesignSize, {
            params: size,
            subject
          })
        : change([...path, "designSize"], "changed", LABEL.surfaceField, {
            params: { field: "designSize" },
            subject
          })
    );
  }
  ...
  for (const field of SURFACE_FIELDS) {
    ...
    if (!sameJsonValue(was, now)) {
      leaves.push(
        change([...path, field], "changed", LABEL.surfaceField, { params: { field }, subject })
      );
    }
  }
  return leaves;
...
function componentRows(left: DocumentIndex, right: DocumentIndex, rows: RankedRows): void {
  for (const entry of diffKeyed(left.components, right.components)) {
    ...
    if (!entry.base || !entry.head) {
      ...
      rows.push(
        slot,
        change(path, entry.kind, entry.head ? LABEL.componentAdded : LABEL.componentRemoved, {
          subject: authoredName(present?.name),
    ...
    if (!sameJsonValue(entry.base.name, entry.head.name)) {
      children.push(
        change([...path, "name"], "changed", LABEL.componentRenamed, {
    ...
    for (const field of COMPONENT_FIELDS) {
      ...
      if (!sameJsonValue(was, now)) {
        children.push(
          change([...path, field], "changed", LABEL.componentField, { params: { field }, subject })
        );
      }
    }
    ...
    for (const element of diffKeyed(elementsOf(entry.base), elementsOf(entry.head))) {
      rows.push(
        ...
        elementRow(
          ["components", entry.key, "elements", element.key],
          element.base,
          element.head,
          shared
        )
...
function elementRows(
  ...
  for (const entry of diffKeyed(left.elements, right.elements)) {
    ...
    const path =
      owner === undefined ? ["elements", entry.key] : ["surfaces", owner, "elements", entry.key];
...
function elementRow(
  ...
  if (!base || !head) {
    ...
    return change(
      path,
      head ? "added" : "removed",
      head ? LABEL.elementAdded : LABEL.elementRemoved,
      {
        subject: authoredName(present?.name)
      }
    );
...
function elementLeaves(
  ...
  if (!sameJsonValue(base.name, head.name)) {
    leaves.push(
      change([...path, "name"], "changed", LABEL.elementRenamed, {
        params: fromToParams(base.name, head.name),
        subject: authoredName(head.name) ?? subject
      })
    );
  ...
  if (!sameJsonValue(base.type, head.type)) {
    leaves.push(
      change([...path, "type"], "changed", LABEL.elementType, {
  ...
  for (const field of ELEMENT_FIELDS) {
    ...
    if (!sameJsonValue(was, now)) {
      leaves.push(
        change([...path, field], "changed", LABEL.elementField, { params: { field }, subject })
      );
    }
  }
  return leaves;
...
function indexDocument(document: UIDocument | undefined): DocumentIndex {
  ...
  const surfaceOrder = surfaceList
    ...
    .filter(
      (id): id is string =>
        typeof id === "string" && Object.prototype.hasOwnProperty.call(surfaces, id)
    );
  for (const surfaceId of surfaceOrder) {
    const tree = walkTree(
      elements,
      surfaces[surfaceId]?.rootElementId,
      surfaceOf.has.bind(surfaceOf)
    );
  ...
  for (const componentId of componentOrder) {
    ...
    componentTreeOf.set(
      componentId,
      walkTree(own, components[componentId]?.rootElementId, () => false)
    );
  }

  const orphans = Object.keys(elements)
    .filter((id) => !surfaceOf.has(id))
    .sort();
  return {
    surfaces,
    surfaceOrder,
...
function childIdsIn(element: UIElement, shared: ReadonlySet<string>): string[] {
  ...
  return Array.isArray(children)
    ? children.filter((id) => typeof id === "string" && shared.has(id))
    : [];
}
...
function sharedKeys(
  ...
  return new Set(
    Object.keys(head).filter((key) => Object.prototype.hasOwnProperty.call(base, key))
  );
}
...
const componentElementSlot = (componentId: string, elementId: string): string =>
  `x${SLOT_SEPARATOR}${componentId}${SLOT_SEPARATOR}${elementId}`;
