...
describe("ui-document spec: reading", () => {
  ...
  it("refuses a document a newer Studio wrote", () => {
    expect(() =>
      parseUiDocument({ ...uidoc([], []), schemaVersion: UI_DOCUMENT_SCHEMA_VERSION + 1 })
    ).toThrow(/newer version of Studio/);
  });

  it("refuses surfaces, components or elements of the wrong shape", () => {
    expect(() => parseUiDocument({ ...uidoc([], []), surfaces: {} })).toThrow(
      /"surfaces" must be an array/
    );
    expect(() => parseUiDocument({ ...uidoc([], []), components: 7 })).toThrow(
  ...
  it("counts surfaces, components and elements, component elements included", () => {
    ...
    const summary = uiDocumentSpec.summarize(
      uidoc(
        [surface("surf-1", "Title", "el-root")],
...
describe("ui-document spec: diff", () => {
  ...
  it("says nothing at all about a pure re-serialization", () => {
    ...
    expect(diffUi(document, reserialized(document))).toMatchObject({
      changes: [],
      total: 0,
      complete: true,
      tier: "semantic"
  ...
  it("addresses a removed element where it used to be", () => {
    ...
    const removed = diffUi(twoElementDocument(), head).changes.find(
  ...
  it("tells a re-parented element from an edited one", () => {
    ...
    const moved = diffUi(base, head)
      .changes.flatMap((change) => change.children ?? [])
      .filter((child) => child.kind === "moved");

    expect(moved.map((child) => [child.path, child.label.key])).toEqual([
      [
        ["surfaces", "surf-1", "elements", "el-root", "childrenIds"],
        "documentDiff.uiDocument.elementOrder"
      ],
      [
        ["surfaces", "surf-1", "elements", "el-z", "childrenIds"],
        "documentDiff.uiDocument.elementOrder"
      ],
      [
        ["surfaces", "surf-1", "elements", "el-a", "parentId"],
        "documentDiff.uiDocument.elementMoved"
      ]
    ]);
  });
  ...
  it("addresses a component's own elements under the component", () => {
    ...
    const result = diffUi(uidoc([], [], [define("Slot")]), uidoc([], [], [define("Empty slot")]));
  ...
  it("falls back to the flat address for an element under no root", () => {
    const orphaned = (name: string): UIDocument =>
      uidoc(
        [surface("surf-1", "Title", "el-root")],
    ...
    expect(
      diffUi(orphaned("Loose"), orphaned("Adrift")).changes.map((change) => change.path)
  ...
  it("never puts a generated id in a subject", () => {
    const nameless = (width: number): UIDocument => ({
      ...uidoc(
        ...
        [
          element("el-root", "", { type: "nl.root", childrenIds: ["el-z"] }),
          element("el-z", "", { parentId: "el-root" })
        ]
      ),
      name: ""
    });
  ...
  it("names the design size with all four numbers, or not at all", () => {
    const sized = (designSize: unknown): UIDocument =>
      uidoc(
    ...
    expect(
      diffUi(sized({ width: 1920, height: 1080 }), sized({ width: 1280, height: 720 })).changes[0]
        .children![0]
    ).toMatchObject({
    ...
    expect(
      diffUi(sized({ width: 1920 }), sized({ width: 1280, height: 720 })).changes[0].children![0]
        .label.key
    ).toBe("documentDiff.uiDocument.surfaceField");
  });

  it("never throws on a malformed document, and says less instead", () => {
    expect(() =>
      diffUi({} as UIDocument, { surfaces: null, elements: 7 } as unknown as UIDocument)
    ).not.toThrow();
...
function node(
  id: string,
  type: string,
  overrides: Partial<BlueprintGraphNode> = {}
): BlueprintGraphNode {
...
function withNodes(
  source: Blueprint,
  mutate: (nodes: Record<string, BlueprintGraphNode>) => void
): Blueprint {
...
describe("ui-graphs spec: reading", () => {
  ...
  it("refuses a document a newer Studio wrote, or one shaped wrongly", () => {
    expect(() =>
      parseUiGraphs({ ...uigraphs(), schemaVersion: UI_GRAPH_DOCUMENT_SCHEMA_VERSION + 1 })
    ).toThrow(/newer version of Studio/);
    expect(() => parseUiGraphs({ ...uigraphs(), graphs: [] })).toThrow(/"graphs"/);
    expect(() => parseUiGraphs({ ...uigraphs(), blueprintDocument: [] })).toThrow(
      /"blueprintDocument" must be an object/
    );
    expect(() => parseUiGraphs({ ...uigraphs(), blueprintDocument: { blueprints: [] } })).toThrow(
      /"blueprints"/
    );
  });
...
describe("ui-graphs spec: diff", () => {
  it("says nothing at all about a pure re-serialization", () => {
    ...
    expect(diffGraphs(document, reserialized(document))).toMatchObject({
      changes: [],
      total: 0,
      complete: true,
      tier: "semantic"
  ...
  it("reports a rewire but not the wires a deleted node took with it", () => {
    const three = (edges: BlueprintGraphEdge[]): UIGraphDocument =>
      uigraphs(
        blueprint(
          "bp-1",
          "Main menu",
    ...
    const deleted = diffGraphs(
      three([wire("n-head", "n-log")]),
      uigraphs(
        blueprint(
          "bp-1",
          "Main menu",
  ...
  it("never throws on a malformed document, and says less instead", () => {
    expect(() =>
      diffGraphs({} as UIGraphDocument, { graphs: 7 } as unknown as UIGraphDocument)
    ).not.toThrow();
    expect(() =>
      diffGraphs(
        { blueprintDocument: { blueprints: { "bp-1": 5 } } } as unknown as UIGraphDocument,
        {
          blueprintDocument: { blueprints: { "bp-1": { id: "bp-1", program: null } } }
