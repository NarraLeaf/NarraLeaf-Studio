...
describe("compileDocumentPathPattern", () => {
    it("accepts one parameter per segment, with literal text either side", () => {
        expect(compileDocumentPathPattern("editor/story/stories/<storyId>/storydoc.json").segments).toHaveLength(5);
...
describe("matchDocumentPath", () => {
    ...
    it("captures parameters from real project paths", () => {
        expect(matchDocumentPath(storyDocument, "editor/story/stories/9f1c/storydoc.json")).toEqual({storyId: "9f1c"});
        expect(matchDocumentPath(locale, "editor/localization/zh-CN.json")).toEqual({locale: "zh-CN"});
        expect(matchDocumentPath(assetsMetadata, "assets/assets.metadata.image.json")).toEqual({type: "image"});
...
describe("pattern overlap and specificity", () => {
    ...
    const animationDocument = compileDocumentPathPattern("editor/story/animations/<animationId>.json");
    ...
    it("sees no overlap between patterns of different lengths", () => {
        expect(documentPathPatternsOverlap(
