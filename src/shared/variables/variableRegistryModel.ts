...
export function normalizePersistentValueType(
 ...
 */
export function registryEntryFromBlueprintPersistent(
...
function withDefaultValue(value: unknown): Pick<VariableRegistryEntry, "defaultValue"> {
  return value === undefined
    ? {}
    : { defaultValue: value as VariableRegistryEntry["defaultValue"] };
}
...
export function migrateVariableRegistryToLatest(raw: unknown): VariableRegistry {
  ...
  for (const [id, value] of Object.entries(rawEntries)) {
    ...
    entries[id] = {
      ...
      valueType: normalizePersistentValueType(
        typeof value.valueType === "string" ? value.valueType : undefined
      ),
 ...
 */
export function buildPersistentRuntimeTable(
  registry: VariableRegistry
