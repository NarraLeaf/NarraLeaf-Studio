...
export const update = {
  ...
  unsupported: {
    macos:
      "macOS では Studio 自身が更新をインストールできない。新しいバージョンをダウンロードしてアプリを置き換える",
    development: "開発ビルドは自身を更新できない",
    platform:
      "このビルドは自身で更新をインストールできない。リリースページから新しいバージョンをダウンロードする"
  },
  setting: {
    checkOnLaunch: {
      label: "起動時に更新を確認",
      description:
        "Studio の起動直後に GitHub へ 1 回だけ問い合わせる。ダウンロードが自動で始まることはない"
    }
  },
  notification: {
