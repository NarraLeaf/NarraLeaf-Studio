...
export const plugins = {
  ...
  workspace: {
    ...
    activity: {
      ...
      suppressedHint:
        "入っているバージョンが、このプロジェクトを作ったときのバージョンと合わない。更新するか、「プロジェクト → 依存関係」でプロジェクト側の依存表を更新する",
      failed: "読み込みに失敗"
    },
    // 読み込み時に一度だけ知らせる。プロジェクトの依存表がはじいたプラグインを名指しする。
    suppressedNotice:
      "このプロジェクトでは読み込まれていない：{names}。入っているバージョンが、プロジェクトを作ったときのバージョンと合わない。プラグインパネルを見る",
    ...
    recoveryNotice:
      "復旧モードではプラグインを読み込まない。ここでの変更は、次に通常どおり開いたときに反映される",
