...
export const plugins = {
    ...
    task: {
        ...
        // 読み込みに失敗したプラグインの再試行。記録から失敗が消え、もう一度読み込まれる。
        starting: "プラグインを起動中…",
        started: "プラグインを起動した",
    },
    error: {
    ...
    dependencies: {
        ...
        // バージョンのために無効になったプラグイン。無効を解く「調べ直す」は「プロジェクト ▸ アプリ」にあり、ここにはない。
        rescanInProject: "「プロジェクト ▸ アプリ」の「調べ直す」で、入っているバージョンを使える",
        task: {
    ...
    workspace: {
        ...
        activity: {
            ...
            // 無効を解くのは「調べ直す」だけなので、ボタンの名前と場所を書く。
            suppressedHint: "入っているバージョンが、このプロジェクトを作ったときのバージョンと合わない。「プロジェクト ▸ アプリ」の「調べ直す」で、入っているバージョンを使える",
            failed: "読み込みに失敗",
        },
