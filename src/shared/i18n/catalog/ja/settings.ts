...
export const settings = {
  ...
  items: {
    ...
    tooltipDelay: {
      label: "ヒントの遅延",
      description:
        "ポインターがコントロールに止まってからヒントが出るまでの時間。同じツールバー内では最初の一つだけが待つ"
    },
    reduceMotion: {
      label: "動きを減らす",
    ...
    editorFontFamily: {
      ...
      description:
        "シーンエディタのストーリー文に使う書体。この端末に入っているフォントならどれでも選べる",
    ...
    spellcheckLanguage: {
      ...
      noDictionary:
        "このプロジェクトの言語のスペル辞書が入っていないため、本文に印は付かない。プロジェクト辞書はこのプロジェクトの用語を保持し続ける",
      options: {
    ...
    detachedEditorOnClose: {
      ...
      description:
        "独立したウィンドウで開いたエディタを、ワークスペースに戻すか、ウィンドウごと閉じるか",
      options: {
    ...
    versionControlAuthor: {
      label: "作者名",
      description:
        "コミットとチェックポイントに記録される。空のままなら NarraLeaf Studio と記録する",
      fromServer:
        "このインストールがサインインしているサーバーから取得されます。自分の名前を記録するにはサインアウトしてください。"
    },
    ...
    resetAllPreferences: {
      label: "すべての設定をリセット",
      description: "すべての設定を既定に戻す。プロジェクト、その履歴、統計には手を付けない",
      action: "リセット",
      confirm: "すべてリセット"
    }
  },
  ...
  dictionaries: {
    ...
    noDictionaryLanguages:
      "中国語と日本語にはスペル辞書がなく、今後も出ない：どちらも語を分けて書かないので、単語リストと照らし合わせるものがない。この二つで書かれたプロジェクトに印は付かない",
    installed: {
      title: "この機体にあるもの",
      emptyTitle: "辞書はまだない",
  ...
  servers: {
    ...
    probe: {
      ...
      untrusted: "そのアドレスのサーバーは信頼されなかった",
      failed: "そのアドレスを確認できなかった"
    },
    problems: {
      scheme: "サインイン先アドレスは https:// または ucs-auth:// で始まる必要がある",
      token: "このサーバーが発行したトークンではない",
      address: "このトークンにはサインイン先が書かれていない",
      server: "このトークンには対象のサーバーが書かれていない",
      certificate: "この端末はそのアドレスが提示した証明書を信頼していない",
      unreachable: "そのアドレスから応答がない",
    ...
    cache: {
      ...
      buckets: {
        ...
        spellcheckDictionaries: {
          label: "スペル辞書",
          description:
            "スペルチェック用にダウンロードした単語リスト。プロジェクト自身の用語はここにはない"
        },
        psdImports: {
          label: "PSD 読み込みの残り物",
  ...
  transfer: {
    ...
    apply: "適用",
    exportHint:
      "設定をそのままの JSON ファイルとして書き出す。ワークスペースの背景、コミットに記録する名前、最近のプロジェクト、統計、ウィンドウのレイアウトはこの端末に残る",
    exported: "{path} に保存した",
