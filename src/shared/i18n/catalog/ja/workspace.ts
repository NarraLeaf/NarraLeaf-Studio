...
export const workspace = {
  localization: {
    panel: {
      languagesTitle: "言語",
      languagesHint:
        "ゲーム自身の言語。原文の言語はストーリーを書いている言語で、ほかはそれを元に翻訳する",
      addLanguage: "言語を追加",
      ...
      importSummary:
        "翻訳 {applied} 件を読み込んだ（変更なし {unchanged}、対応不明 {unknown}、空のため飛ばした {skippedEmpty}）"
    },
    settings: {
      menu: "言語の設定…",
      title: "{name} の言語の設定",
  ...
  voice: {
    ...
    table: {
      ...
      outdatedHint:
        "このテイクを読み込んだ後に行が変わっている。クリップを読み込み直すと最新として扱う",
  ...
  recovery: {
    ...
    intro:
      "検査を走らせると、その部分だけを読み込んで結果を報告する。読めたものは普段どおり見て回れる",
    problems: {
      title: "見つかった問題",
      count: "{count}",
    ...
    lore: {
      ...
      checkpointNothing:
        "記録するものがない。現在のバージョンがすでにこれらのファイルと一致している",
      checkpointFailed: "バージョンを記録できなかった：{error}",
      restore: "このバージョンを復元",
      restoreConfirm: "{version} を復元するか",
      restoreExplain:
        "プロジェクトのすべてのファイルが、そのバージョンの中身に置き換わる。先に現在の状態をバージョンとして記録し、復元もさらに 1 つのバージョンとして足される。消えるバージョンはない",
      cancel: "キャンセル",
    ...
    offer: {
      ...
      detailOne:
        "ファイルを 1 つ読めなかったので、このウィンドウにはプロジェクトの一部が欠けている。よくある原因は、保存の中断、同期やバックアップの道具による同時書き込み、プラグイン。いま編集すると、この欠けた状態を無事なファイルの上に書いてしまうことがある",
      detailMany:
        "{count} 個のファイルを読めなかったので、このウィンドウにはプロジェクトの一部が欠けている。よくある原因は、保存の中断、同期やバックアップの道具による同時書き込み、プラグイン。いま編集すると、この欠けた状態を無事なファイルの上に書いてしまうことがある",
      enter: "復旧モードで開く"
    },
  ...
  shell: {
    ...
    save: {
      ...
      frozenDetailRevision:
        "バージョン {version} を開いている。バージョンを開いている間は何も保存されない",
      ...
      server: {
        ...
        signIn: {
          ...
          trust: {
            ...
            title: "このサーバーを信頼しますか",
            vouched:
              "貼り付けたトークンはこの認証局を指しており、そのアドレスで応答しているのもこの認証局です。",
            compare:
              "この接続とは別の手段で、サーバーの管理者から伝えられた指紋と照合してください。",
            authorityLabel: "発行元",
            fingerprintLabel: "指紋",
            meaning:
              "この認証局の鍵を持つものは、どのアドレスに対しても証明書を発行でき、このアカウントはそれを信じます。影響を受けるのはこのコンピューターのこのアカウントだけです。",
            manual:
              "このシステムにはアカウントごとの信頼ストアがないため、Studio では実行できません。次を実行してから、もう一度サインインしてください。",
            copy: "コマンドをコピー",
            confirm: "信頼する",
            cancel: "キャンセル"
          },
          ...
          reach: {
            ...
            notPermitted:
              "サインインしましたが、このアカウントにはこのプロジェクトが割り当てられていません。サーバーの管理者に権限を依頼してください。",
            dataPortSilent: "サインインしましたが、サーバー本体が応答しませんでした。"
          },
          problem: {
            scheme: "サインインアドレスは https:// または ucs-auth:// で始まる必要があります。",
            token:
              "これはこのサーバーが発行したトークンではありません。受け取ったトークン全体を貼り付けてください。",
