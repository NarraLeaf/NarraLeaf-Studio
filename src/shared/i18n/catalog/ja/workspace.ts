...
export const workspace = {
    ...
    team: {
        ...
        liveFrozenWhat: "セッション中はストーリー・キャスト・翻訳と音声、そしてアセットライブラリ全体（ファイルそのものを含む）が保存される。このプロジェクトの他の内容は最新の状態のまま読み取り専用になる",
        ...
        save: {
            ...
            // ライブセッションは自分のストーリーだけを保存してほかを拒否するため、
            // 上のタイトルは作者が実際に入力しているファイルについては偽になる。
            frozenTitleSession: "このファイルは保存されない",
            frozenDetailSession: "ライブセッション中。保存されるのはそのストーリーとキャストだけで、ほかを変更するにはセッションから抜ける",
