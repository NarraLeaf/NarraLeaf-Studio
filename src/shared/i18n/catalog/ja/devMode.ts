...
export const devMode = {
    ...
    issues: {
        // ドロワーの他のパネルと同じく一語。
        title: "問題",
        // 英語カタログを参照。どちらも動いているゲームが報告したものだけを数えるので、実行を名指す。
        // 素の「失敗したものはない」はプロジェクト全体への判定に読まれるが、ここはそれを言えない。
        empty: "今回の実行で失敗したものはない",
        ...
        onSurface: "{surface} 上",
        onSurfaceGone: "このプロジェクトにないページ上",
        // 英語カタログを参照。第 3 の場所としてのプラグインと、外したプラグインを名指す理由。
        onPlugin: "プラグイン {plugin}",
        pluginNotDeclared: "{plugin} はインストールされているが、このプロジェクトのプラグイン依存に入っていない。このセッションでもビルドでも読み込まれない。「プロジェクト ▸ アプリ」で依存表を取り直す",
        pluginUnusable: "{plugin} はこのプロジェクトのプラグイン依存に入っているが、読み込めない。このセッションでもビルドでも読み込まれない。「プロジェクト ▸ アプリ」で状態を確認する",
    ...
    /**
     * レイヤーのパネル。いま画面に載っているものを下から上へ全部並べ、入力を誰が持っているかを示す。
     *
     * 各行は「それが何か」ではなく「それが何を受け取るか」を言う。パネルを開くのはまさにそれを
     * 知りたいときだから。モーダルなレイヤーが 1 枚あれば、その下はすべてクリックを受け取らなくなり、
     * スタックのどこか 1 行だけがキーを受け取る。
     */
    layers: {
        // ドロワーの他のパネルと同じく一語。
        title: "レイヤー",
        ...
        offScreen: "画面に出ていない",
        offScreenNote: "同じ id のサーフェスがこのプロジェクトに無いあいだ、そのレイヤーは画面に出ない",
        exitPending: "まだ退場中のレイヤーがある",
        queued: "グループの順番待ち",
