  ...
  rule: {
    assetsUnused: {
      ...
      messageIndexUnresolved:
        "使われていないアセットを一覧にできない：{location} が指すアセットを特定できない",
    ...
    storyCutPointOrphan: {
      ...
      message:
        "このプロジェクトにはビルドバリアントが無いので、このカットポイントは何も終わらせない。バリアントを足すか、この行を消す"
    },
    storyCutPointUnreachable: {
      title: "届かないカットポイント",
      description: "どこからもたどり着けないシーンにあるカットポイント",
      message:
        "このシーンにはどこからも到達しないので、このカットポイントはどのビルドも終わらせない"
    },
    ...
    variablesRandomOutsideAssignment: {
      ...
      message:
        "{fn}() はこの条件を判定するたびに引き直されるので、確かめるたびに分岐が変わりうる。/set で一度だけ引き、その変数を判定する",
      messageChoiceOption:
        "{fn}() はメニューを描くたびに引き直されるので、この選択肢がちらつく。/set で一度だけ引き、その変数を判定する",
      messageInterpolation:
        "{fn}() はこの行を描くたびに引き直されるので、描き直すたびに値が変わる。/set で一度だけ引き、その変数を表示する"
    },
    textOverlong: {
      title: "長すぎる行",
    ...
    brandBrokenLink: {
      ...
      messageChain:
        "{where} が使っている {color} は {missing} へつながっているが、その色は配色に無い",
