...
export const build = {
  ...
  signing: {
    ...
    removeConfirmDetail:
      "鍵の情報がこの端末から削除される。これを使うプロジェクトは、読み込み直すまで署名なしでビルドされる",
    removeAction: "取り除く",
    chooseFile: "選ぶ…",
    noFile: "なし",
    expires: "{date} に期限切れ",
    ...
    macIdentityEmpty:
      "この Mac のキーチェーンにコード署名用の証明書がない。キーチェーンアクセスで入れるか、証明書ファイルを読み込む",
  ...
  preflight: {
    ...
    "version-invalid":
      "バージョン {version} はセマンティックバージョンとして不正で、ビルドは失敗する",
    ...
    "icon-low-resolution":
      "{platform} のアイコンが {minimum}×{minimum} より小さく、引き伸ばして入る",
    "icon-stale":
      "{platform} のアイコンが用意されていない。「プロジェクト ▸ アプリ」を開いて焼き直す",
    ...
    "plugin-secret-unavailable":
      "{plugin} の {field} は設定済みだが、その値はこの端末にない。秘密の値がプロジェクトと一緒に動くことはない。{platforms} をビルドするには、ここでもう一度入力する",
    ...
    "web-unprotected":
      "Web 書き出しにアセットの保護は効かない。そのファイル群は保護されずに配布される",
    "progress-carry-unsupported":
      "{blueprints} は版と版のあいだで進行状況を引き継ぐが、{platform} のビルドはそれを拒む。" +
      "ページには書き込める共有ファイルがないので、どちらのノードも失敗の枝に進む",
    "web-lossy-images":
      "非可逆の画像再圧縮が有効なので、書き出す画像は品質 {quality} で再エンコードされ、失われた情報は戻らない",
    "mobile-template-missing": "モバイルのシェルテンプレートを使えない：{reason}",
    "mobile-payload-too-large":
      "このプロジェクトのアセット（{size}）は、モバイルのパッケージに収まる大きさを超えている",
    "version-uncodable":
      "バージョン {version} は Android のバージョンコードに変換できない。メジャーは 2099 まで、マイナーとパッチは 999 まで",
    "appid-android-adjusted":
      "アプリ ID の {appId} は Android のパッケージ名として不正なので、ビルドは {applicationId} で出す",
    "bundleid-ios-adjusted":
      "アプリ ID の {appId} は iOS のバンドル識別子として不正なので、ビルドは {bundleId} で出す",
    // 各社の警告画面の名前は出さない。「Gatekeeper」も「SmartScreen」も作者の語彙ではなく、
    // 起きることはどちらも同じ。詳しい話は `build` のヘルプトピックにある。
    unsigned:
      "コード署名していない。プレイヤーが初めて開くとき、セキュリティの警告が出ることがある",
    "unsigned-android":
      "手元のデバッグ用の識別情報で署名している。これはサイドロードにしか使えず、これで署名した AAB は Google Play のアップロード鍵として使えない。自分の識別情報で署名するにはリリース用キーストアを選ぶ",
    // 証明書チェーンの注意はここに置く。作者が .p12 を書き出すのはこの文を読んでいる最中で、
    // 末端だけを書き出すと署名の段階でそのまま失敗する。
    "unsigned-ios":
      "この .ipa は署名されておらず、iOS は署名のないものを一切インストールしない。Apple の署名資格情報を選ぶ。.p12 はキーチェーンアクセスから発行元の証明書チェーンごと書き出す。そうしないと署名に失敗する",
    "signing-credential-missing":
      "{platform} の署名資格情報がこの端末にない。鍵の情報がプロジェクトと一緒に運ばれることはない。ここで読み込むか、選択を外して {platform} を署名なしでビルドする",
    "signing-credential-expired":
      "{platform} の署名証明書は今日の時点で有効ではない（有効期間は {notBefore} から {notAfter}）ので、署名は失敗する。発行元で更新し、新しいものを読み込む",
    "signing-credential-expiring":
      "{platform} の署名証明書は {notAfter} に期限が切れる。それより前に署名したビルドは有効なまま。以降は更新した証明書が要る",
    "signing-secret-unavailable":
      "{platform} の署名資格情報のパスワードをこの端末では読めない。資格情報を読み込み直して保存し直す",
    "signing-tool-missing":
      "{platform} のビルドに署名するには {tool} が必要だが、この端末に入っていない。入れて PATH を通してから、このダイアログを開き直す",
    "signing-host-unsupported":
      "この端末は {host} で動いており、選んだ資格情報では {platform} 向けに署名できない。その秘密鍵は当該プラットフォームにしかないシステムのサービスの中にある。この対象は {platform} の端末でビルドする",
    "signing-needs-network":
      "{platform} のビルドへの署名にはネットワーク接続が要る。このビルドのそれ以外はオフラインでも動く",
    "signing-macos-identity-missing":
      "{identity} という名前の証明書がこの Mac のキーチェーンにない。キーチェーンアクセスで入れるか、ここで別の証明書を選ぶ",
    "signing-macos-identity-unusable":
      "証明書 {identity} では署名できない。期限が切れている、秘密鍵がない、発行元のチェーンが揃っていない、のいずれか。どれなのかはキーチェーンアクセスで開くと分かる",
    "signing-macos-not-developer-id":
      "{identity} は「Developer ID Application」の証明書ではない。この Mac ではビルドが動くが、他人の Mac では Gatekeeper が拒み、Apple の公証も通らない",
    "signing-android-not-play":
      "署名済みの APK はサイドロードや itch.io などのストアで使える。Google Play が受け取るのは AAB だけ。Android の対象で AAB 形式を有効にすると作られる",
    "signing-ios-profile-mismatch":
      "アプリ ID の {bundleId} はプロビジョニングプロファイルの対象外で、そのプロファイルは {profileAppId} 向けに発行されている。プロジェクトの識別子を変えるか、対象に合うプロファイルを読み込む",
    "cross-build-download":
      "{platforms} 向けのクロスビルドは、初回だけ Electron をダウンロードする。以降はキャッシュを使う",
    "output-not-writable": "{outputDir} に書き込めない",
    "output-not-empty": "出力フォルダが空ではない。このビルドは名前の一致するファイルを上書きする"
  },
  webStaticNotice:
    "Web ビルドは、どのウェブサーバーにも置ける静的サイト。アセットの暗号化と HTTP の制限は効かない",
  toast: {
  ...
  appTagGraphUnresolved:
    "{blueprint} / {graph} でアプリタグが固定の値にならない。バリアント名と比べるか、その値をそのまま使う",
  appTagGraphUnknownNode:
    "{blueprint} / {graph} はバリアントを判定しつつ、このビルドが読めないノードも使っている。バリアントの判定をそのノードの無いグラフへ移す",
  appTagGraphFnHead:
    "{blueprint} / {graph} のバリアントの判定が、Fn があるかどうかを決めている。その Fn を、判定が決める分岐の外へ出す",
  appTagGraphSummary: {
    other:
      "ビルドを中止：バリアントの判定が固定の値にならないブループリントのグラフが {count} 件ある。コンソールを見る"
  },
  /**
   ...
   */
  cutPointNested:
    "{story} / {scene} の {variant} のカットポイントが、条件か群の中にある。シーンの最上位へ移す",
  ...
  contentBlockedStartStory:
    "{location} のゲーム開始ノードは、実行中にシーンを決める。インスペクタでシーンを選ぶか、{variant} のバリアントで開始しうるシーンを並べる",
  contentBlockedScript:
    "ブループリント {location} は TypeScript で書かれていて、どのシーンでも開始できる。{variant} のバリアントで開始しうるシーンを並べる",
  contentBlockedPlugin:
    "{location} プラグインはどのシーンでも開始できる。{variant} のバリアントで開始しうるシーンを並べる",
  contentBlockedSummary: {
    other:
      "ビルドを中止：{variant} のビルドが読めないシーンを開始しうるものが {count} 件ある。コンソールを見る"
  },
  /** 並べたシーンが、その後で削除された。止めはせず知らせるだけ。作者の答え自体はまだ有効。 */
  contentStaleDeclaration:
    "{variant} のバリアントで {location} に並べたシーンが、もうプロジェクトに無い",
   ...
   */
  contentCoverageGap:
    "{location} を読めなかったので、{variant} のビルドが何を落とすかを決められない",
  /** ドキュメント 1 件ではなく索引全体が欠けているときに `{location}` に入る言葉。 */
  contentCoverageWholeProject: "プロジェクト",
  contentCoverageSummary: {
    other:
      "ビルドを中止：{variant} のビルドはシーンを落とすが、{count} 件のドキュメントを読めなかった。コンソールを見る"
  },
  /**
   ...
   * その要求がもう要らない場合がある。
   */
  networkNodeDisallowed:
    "{blueprint} はネットワーク要求を行うが、このプロジェクトはそれを許可していない",
  networkSummary: {
    other:
      "ビルドを中止：動かせないネットワークノードが {count} 件ある。プロジェクト設定で HTTP の許可を有効にするか、そのノードを取り除く"
  },
  networkAddressNotAllowlisted:
    "{blueprint} は {url} を要求するが、このアドレスはプロジェクトのネットワーク要求許可一覧にない",
  networkAllowlistSummary: {
    one: "ビルドを中止した。{count} 件のアドレスがネットワーク要求許可一覧にない。プロジェクト設定で追加するか、ノードを修正する",
    other:
      "ビルドを中止した。{count} 件のアドレスがネットワーク要求許可一覧にない。プロジェクト設定で追加するか、ノードを修正する"
  },
