...
export const story = {
  ...
  script: {
    ...
    importFailed:
      "「{scene}」で読み込みが止まった。{total} 個中 {applied} 個のシーンを書き、残りはそのまま",
    line: "{line} 行目",
    stat: {
    ...
    parseError: {
      ...
      dataMissing:
        "このスクリプトはシーンのデータを持たないので読み込めない。確認用の書き出しは読むだけのもの",
    ...
    diag: {
      ...
      shapeMismatchAction:
        "アクションの行が文に書き換えられていた。アクションを残し、その編集は捨てた",
      ...
      speakerUnresolved:
        "この行はキャラクターと結びついていないので、元の話者名を残した。文のほうは変わっている"
    }
  },
  ...
  flow: {
    ...
    route: {
      ...
      stopsHere: "ここで止まる",
      stopsHereTitle:
        "道はここで止まるが、ここはエンディングではない。訪れたシーンへ戻ったか、選択肢の先に何も書かれていない",
      diagnostics: {
        unreachableEndings: {
  ...
  layerField: {
    ...
    notOnStageTitle:
      "この名前のレイヤーはこのシーンの前の行で宣言されていない。すでにあるレイヤーを選ぶ",
    hint: "レイヤー",
    createNew: "レイヤーを新規作成"
  },
  appearance: {
  ...
  rows: {
    ...
    placeholderDialogueContinue:
      "{name} として書き続ける。{trigger} でキャラクターのアクションを挿入",
  ...
  sceneEditor: {
    ...
    emptyHint:
      "このシーンは空。新しい行で {trigger} を打つとコマンドを選べる。地の文を書いてもよい",
  ...
  preview: {
    ...
    diagnostics: {
      ...
      targetUnreachable:
        "プレビューの対象にシーンのルートから到達できない。代わりにシーンの末尾を表示している",
      ...
      assignmentUnresolved:
        "式 `{expression}` を解決できなかった。プレビューではこの代入を飛ばした",
  ...
  command: {
    ...
    jump: {
      label: "ジャンプ",
      detail: "別のシーンへ移る。いまのシーンは降ろされる。/goto とは違う"
    },
    ...
    motion: {
      label: "モーション",
      detail: "ランタイムが描くキャラクターに再生させるモーションを決める"
    },
    param: {
      label: "パラメータ",
      detail: "ランタイムが描くキャラクターのモデルの数値パラメータを 1 つ決める"
    },
    ...
    repeat: {
      label: "繰り返し",
      detail: "中のアクションを決めた回数だけ実行する。条件で回すなら /until を使う"
    },
    // 詳細だけが、語そのものでは言えない 1 点を言う。`until` は *止まる* ときを言うので、
    // 群は条件が偽の間だけ動く。
    until: {
      label: "まで繰り返し",
      detail: "条件が成り立つまで、中のアクションを繰り返す。条件は毎回、実行の前に判定される"
    },
    ...
    goto: {
      label: "ラベルへ",
      detail: "再生位置をこのシーンのラベルへ移す。/jump と違い、シーンは動き続ける"
    },
    // 切る行為ではなく、できあがる行の名前を付ける。「カット」だけでは編集の切り取りに読める。
    // 名前が背負えない半分、すなわちこの行が 1 つのビルドだけのものだという事実は詳細に置く。
    cut: {
      label: "カットポイント",
      detail: "あるビルドバリアントのストーリーをこの行で終わらせる。他のビルドにこの行は入らない"
    },
    ...
    camera: {
      label: "カメラ",
      detail: "ステージカメラをパン、ズーム、回転、または暗くする。姿勢はシーンをまたいで残る"
    },
