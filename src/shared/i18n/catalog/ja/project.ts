...
export const project = {
    ...
    dependencies: {
        ...
        empty: "プラグインへの依存はない",
        // 一覧の上のバナー。行がとっている状態ごとに一文ずつ、状態とそれを直す場所を書く。
        // 各文は自分の原因だけを言うので、入っていないプラグインを「バージョンが合わない」とは言わない。語は各行（つまりプラグインパネル）と同じ。
        banner: {
            missing: {
                other: "このプロジェクトが使うプラグインのうち {count} 個が入っていない。プラグインパネルからインストールする",
            },
            // 無効を解くのはバナーの横の「調べ直す」なので、文はそれで終わる。
            held: {
                other: "{count} 個のプラグインがこのプロジェクトでは無効。入っているバージョンが、このプロジェクトを作ったときのバージョンと合わない。「調べ直す」で入っているバージョンを使える",
            },
            needsAuthorization: {
                other: "このプロジェクトが使うプラグインのうち {count} 個は承認が必要。プラグインパネルで承認する",
            },
            disabled: {
                other: "このプロジェクトが使うプラグインのうち {count} 個が無効。プラグインパネルで有効にする",
            },
            failed: {
                other: "このプロジェクトが使うプラグインのうち {count} 個が読み込みに失敗した。エラーはプラグインパネルに出ている",
            },
            outdated: {
                other: "{count} 個のプラグインが、このプロジェクトを作ったときのバージョンより古い",
            },
            // データのみの依存が別のメジャーバージョンにある状態。読み込みは続くので無効にはならない。
            incompatible: {
                other: "{count} 個のプラグインの入っているバージョンが、このプロジェクトを作ったときのバージョンと合わない",
            },
        },
        status: {
            ready: "利用可能",
            ...
            missing: "未インストール",
            incompatible: "非対応",
            // 作者がプラグインパネルで切ったため、貢献するものがどこにも読み込まれない状態。
            // プラグインパネルのスイッチの横に出る語と同じもの。一つの事実に一つの語。
            disabled: "無効",
            // 入っているバージョンがプロジェクトの依存と合わないため、Studio がこのプロジェクトへ読み込まない状態。
            // 作者の操作ではなく Studio 側の判断なので、スイッチの語は使えない。
            // 言い方はプラグインパネルの同じ状態に合わせる。
