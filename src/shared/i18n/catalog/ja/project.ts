...
export const project = {
  ...
  details: {
    ...
    copyrightTextHelper:
      "ゲームと並べて COPYRIGHT.txt として配布する。空のままならファイルは作られない",
    descriptionPlaceholder: "プロジェクトの説明…",
    required: "必須"
  },
  userData: {
    description:
      "配布したゲームがプレイヤーのセーブと進行を置く場所。フォルダの名前は識別子から決まるので、" +
  ...
  game: {
    ...
    autoSaveSlotsDescription:
      "自動セーブはこの数のスロットを古いものから順に使い回す。プレイヤー自身のセーブスロットとは別"
  },
  ...
  preferences: {
    ...
    // 読んでいる作者の役に立たない。
    intro:
      "何も変えていないプレイヤーにとっての各設定の初期値。プレイヤーはすべて変えられ、変えた内容は保たれる",
    group: {
      dialogue: "ダイアログ",
      skipping: "スキップ",
    ...
    voiceEndMode: {
      ...
      description:
        "行が終わったときクリップをどうするか。どれを選んでも、2 つのボイスが同時に鳴ることはない",
      option: {
        stop: "クリップを止める",
        fade: "クリップをフェードアウトする",
        none: "そのまま鳴らし続ける"
  ...
  settings: {
    ...
    networkPolicyWebHint:
      "Web 書き出しは HTTP で配信されるため「ネットワークを使わない」は適用できない。許可一覧はページのポリシーで適用される",
    networkAllowlist: {
      title: "ネットワーク要求の許可一覧",
      description: "1 行に 1 つ、アドレスかホストのパターンを書く",
      matchHint:
        "ホストだけを書くと、そのホスト配下のすべてのパスを指す。* は先頭のホストラベルの置き換え（*.example.com）か、パスの末尾（/v1/*）に使える。スキーム、ホスト、ポートは完全一致",
      placeholder: "https://api.example.com/*",
      invalid:
        "http:// か https:// のアドレスを入れる。* は先頭のホストラベルの置き換えか、パスの末尾にだけ使える",
      add: "アドレスを追加",
      remove: "アドレスを削除",
      fromPlugins: "インストール済みプラグインの宣言",
      sidecarNote:
        "プラグインが同梱するプログラムはゲームのプロセスの外で動き、この許可一覧の対象ではない"
    },
    encryptAssetsTitle: "アセットを暗号化",
    encryptAssetsDescription:
      "パッケージとプレビューのビルドで、アセット、プラグインのコード、ストーリーのバンドルを暗号化する。開発モードには影響しない",
    encryptAssetsWebHint:
      "Web 書き出しには当てはまらない。Web ビルドは常にアセットの保護なしで配布される",
    // 署名の群をまとめた 1 行。署名できるプラットフォームには、この端末でビルドできるかに
    // 関わらず行が並ぶ。証明書はそれを使うビルドの何日も前に用意するもので、その準備こそが
    // これがビルドのダイアログではなくパネルにある理由。
    signingDescription:
      "どの資格情報でどのプラットフォームに署名するか。証明書とパスワードはこの端末に留まり、プロジェクトはどれを使うかだけを持つ",
    webLosslessImagesTitle: "画像を WebP に変換",
    webLosslessImagesDescription:
      "書き出す画像を、そのほうが小さくなる場合に可逆 WebP へ再エンコードする",
    webLosslessImagesHint:
      "変換のたびに元の画像とピクセル単位で比べ、まったく同じにデコードされない限り捨てる。Android と iOS のビルドは同じ書き出しを配信するので、そちらにも効く",
    webPrecompressTitle: "テキストを事前圧縮",
    webPrecompressDescription:
      "サイトのスクリプト、スタイル、ストーリーのデータについて Brotli と Gzip の版も書き出す",
    webPrecompressHint:
      "使うのは、事前圧縮したファイルを配信するよう設定したサーバーだけ。それ以外のホストは元のファイルを配信し、これらを無視する",
    webLossyImagesTitle: "画像を再圧縮",
    webLossyImagesDescription:
      "書き出す画像を非可逆 WebP へ再エンコードする。かなり小さくなるが、失われた情報は戻らない",
     ...
     */
    stageFitDescription:
      "モバイル向けのビルドと開発モードに効く。デスクトップと Web は常に黒帯で収める",
    stageFit: {
      contain: "黒帯で収める",
      cover: "埋めて切り取る"
    },
  ...
  dependencies: {
    ...
    banner: {
      blocked:
        "ここで無効になっているプラグインがある。入っているバージョンが合わない。更新するか入れ直す",
