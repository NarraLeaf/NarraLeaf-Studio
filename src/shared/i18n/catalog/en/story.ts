...
export const story = {
  ...
  script: {
    ...
    importFailed:
      'Import stopped at "{scene}": {applied} of {total} scenes were written, the rest were left unchanged.',
    line: "Line {line}",
    stat: {
    ...
    parseError: {
      ...
      dataMissing:
        "This script carries no scene data, so it cannot be imported. Review exports are read-only.",
    ...
    diag: {
      ...
      shapeMismatchAction:
        "An action line was rewritten as prose; the action was kept and the edit dropped.",
      shapeMismatchText:
        "A text line was rewritten as an action line; the text was kept and the edit dropped.",
      ...
      speakerUnresolved:
        "This line binds no character, so the original speaker name was kept. Its text still changed."
    }
  },
  ...
  flow: {
    ...
    route: {
      ...
      stopsHereTitle:
        "A path stops here but this is not an ending: it looped back to a visited scene, or an option has nothing written after it",
      diagnostics: {
        unreachableEndings: {
    ...
    edge: {
      ...
      confirmRemoveDetail: {
        ...
        other:
          "All {count} jumps are removed from {source}. This can be undone in the scene editor."
      },
  ...
  layerField: {
    ...
    notOnStageTitle:
      "No layer with this name is declared earlier in this scene, pick an existing layer",
    hint: "Layer",
  ...
  rows: {
    ...
    placeholderDialogueContinue:
      "Keep writing as {name}, or {trigger} to insert a character action",
  ...
  sceneEditor: {
    ...
    emptyHint:
      "This scene is empty. Type {trigger} on a new row to pick a command, or write a line of narration.",
  ...
  preview: {
    ...
    diagnostics: {
      ...
      targetUnreachable:
        "Preview target is not reachable from the scene root; previewing the scene end instead.",
      ...
      conditionUnresolved:
        "Condition `{expression}` did not resolve; it evaluates false in the preview.",
      blueprintConditionFalse: "Blueprint condition evaluates false in the preview.",
      persistentConditionDefaults:
        "Persistent-variable condition evaluates against defaults in the preview.",
      ...
      persistentAssignmentSkipped:
        "Persistent-variable assignments are not applied in the preview.",
      assignmentUnresolved:
        "Expression `{expression}` did not resolve; the assignment was skipped in the preview.",
  ...
  command: {
    ...
    param: {
      label: "Parameter",
      detail: "Set one numeric parameter of a runtime-drawn character's model"
    },
    ...
    repeat: {
      label: "Repeat",
      detail: "Run the enclosed actions a set number of times. For a condition instead, use /until"
    },
    ...
    until: {
      label: "Until",
      detail:
        "Repeat the enclosed actions until a condition becomes true. The condition is checked before each pass"
    },
    ...
    goto: {
      label: "Go to",
      detail: "Move the play head to a label in this scene. Unlike /jump, the scene keeps running"
    },
    ...
    cut: {
      label: "Cut point",
      detail: "End one build variant's story at this line. Other builds do not have this line"
    },
    ...
    camera: {
      label: "Camera",
      detail: "Pan, zoom, rotate or darken the stage camera. Kept across scenes"
    },
