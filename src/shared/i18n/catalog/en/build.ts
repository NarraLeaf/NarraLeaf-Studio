...
export const build = {
  ...
  signing: {
    ...
    removeConfirmDetail:
      "Its key material is deleted from this machine. Projects that use it build unsigned until it is imported again.",
    removeAction: "Remove",
    chooseFile: "Choose…",
    noFile: "None",
    ...
    macIdentityEmpty:
      "No code-signing certificate in this Mac's keychain. Install one in Keychain Access, or import a certificate file.",
  ...
  preflight: {
    ...
    "icon-low-resolution":
      "The {platform} icon is smaller than {minimum}×{minimum} and ships upscaled.",
    ...
    "plugin-secret-unavailable":
      "{plugin}'s {field} was set on another machine and its value is not here. Enter it again to build {platforms}.",
    ...
    "web-unprotected":
      "Asset protection does not apply to the web export; its files ship unprotected.",
    ...
    "web-lossy-images":
      "Lossy image recompression is on, so the exported images are re-encoded at quality {quality} and lose detail permanently.",
    "mobile-template-missing": "The mobile shell templates are unavailable: {reason}",
    "mobile-payload-too-large":
      "This project's assets ({size}) exceed what a mobile package can hold.",
    "version-uncodable":
      "Version {version} cannot be encoded as an Android version code (major up to 2099, minor and patch up to 999).",
    "appid-android-adjusted":
      "The app id {appId} is not a valid Android package name; the build ships {applicationId}.",
    "bundleid-ios-adjusted":
      "The app id {appId} is not a valid iOS bundle identifier; the build ships {bundleId}.",
    ...
    "unsigned-android":
      "Signed with a local debug identity, which is only good for sideloading; an AAB signed with it is not usable as a Google Play upload key. Choose a release keystore to sign it under your own identity.",
    ...
    "unsigned-ios":
      "This .ipa is unsigned, and iOS installs nothing unsigned. Choose an Apple signing credential. Export the .p12 from Keychain Access with its issuing certificate chain, or signing fails.",
    "signing-credential-missing":
      "The {platform} signing credential is not on this machine; key material never travels with a project. Import it here, or clear the selection to build {platform} unsigned.",
    "signing-credential-expired":
      "The {platform} signing certificate is not valid today ({notBefore} to {notAfter}), so signing will fail. Renew it with the issuer and import the replacement.",
    "signing-credential-expiring":
      "The {platform} signing certificate expires on {notAfter}. Builds signed before then stay valid; later ones need a renewed certificate.",
    "signing-secret-unavailable":
      "The password for the {platform} signing credential cannot be read on this machine. Import the credential again to store it afresh.",
    "signing-tool-missing":
      "Signing the {platform} build needs {tool}, which is not installed on this machine. Install it, add it to PATH, then reopen this dialog.",
    "signing-host-unsupported":
      "This machine runs {host} and cannot sign for {platform} with the selected credential: its private key lives in a system service that only exists on that platform. Build this target on a {platform} machine.",
    "signing-needs-network":
      "Signing the {platform} build needs a network connection. Everything else about this build works offline.",
    "signing-macos-identity-missing":
      "No certificate named {identity} is in this Mac's keychain. Install it in Keychain Access, or choose another certificate here.",
    "signing-macos-identity-unusable":
      "The certificate {identity} cannot sign: it has expired, its private key is missing, or its issuing chain is incomplete. Open it in Keychain Access to see which.",
    "signing-macos-not-developer-id":
      '{identity} is not a "Developer ID Application" certificate. The build runs on this Mac, but Gatekeeper refuses it on anyone else\'s and Apple will not notarize it.',
    "signing-android-not-play":
      "A signed APK works for sideloading and for stores such as itch.io. Google Play takes only AAB packages: turn on the AAB format under the Android target to produce one.",
    "signing-ios-profile-mismatch":
      "The app id {bundleId} is not covered by the provisioning profile, which is issued for {profileAppId}. Change the project identifier, or import the profile that covers it.",
    "cross-build-download":
      "Cross-building for {platforms} downloads Electron on first use (cached afterwards).",
    "output-not-writable": "Cannot write to {outputDir}.",
    "output-not-empty":
      "The output folder is not empty. This build overwrites files with matching names."
  },
  webStaticNotice:
    "The Web build is a static site for any web server. Asset encryption and the HTTP restriction do not apply to it.",
  toast: {
  ...
  appTagUnresolvedSummary: {
    ...
    other:
      "Build stopped: {count} AppTag expressions do not reduce to a fixed value. See the console."
  },
  /**
   ...
   */
  appTagGraphUnresolved:
    "App Tag does not reduce to a fixed value in {blueprint} / {graph}. Compare it with a variant name, or use its value directly.",
  appTagGraphUnknownNode:
    "{blueprint} / {graph} tests the variant and also uses a node this build cannot read. Move the variant test to a graph without that node.",
  appTagGraphFnHead:
    "The variant test in {blueprint} / {graph} decides whether an Fn exists. Move the Fn out of the branch it decides.",
  appTagGraphSummary: {
    one: "Build stopped: {count} blueprint graph does not reduce its variant test to a fixed value. See the console.",
    other:
      "Build stopped: {count} blueprint graphs do not reduce their variant tests to a fixed value. See the console."
  },
  /**
   ...
   */
  cutPointNested:
    "The cut point for {variant} in {story} / {scene} is inside a condition or a group. Move it to the top level of the scene.",
  cutPointNestedSummary: {
    one: "Build stopped: {count} cut point is not at the top level of its scene. See the console.",
    other:
      "Build stopped: {count} cut points are not at the top level of their scene. See the console."
  },
  /**
   ...
   */
  contentBlockedStartStory:
    "A Start Game node in {location} picks its scene while the game runs. Pick the scene in the inspector, or list the scenes it can start in the {variant} variant.",
  contentBlockedScript:
    "The blueprint {location} is written in TypeScript and can start any scene. List the scenes it can start in the {variant} variant.",
  contentBlockedPlugin:
    "The {location} plugin can start any scene. List the scenes it can start in the {variant} variant.",
  contentBlockedSummary: {
    one: "Build stopped: {count} place can start a scene the {variant} build cannot read. See the console.",
    other:
      "Build stopped: {count} places can start a scene the {variant} build cannot read. See the console."
  },
  /** A listed scene that has since been deleted. A warning, not a stop: the answer still stands. */
  contentStaleDeclaration:
    "A scene listed for {location} in the {variant} variant is no longer in the project.",
   ...
   */
  contentCoverageGap:
    "{location} could not be read, so what the {variant} build leaves out cannot be decided.",
  ...
  contentCoverageSummary: {
    ...
    other:
      "Build stopped: the {variant} build removes scenes and {count} documents could not be read. See the console."
  },
  /**
  ...
  networkSummary: {
    ...
    other:
      "Build stopped: {count} network nodes cannot run. Change the network policy in project settings, or remove the nodes."
  },
  networkAddressNotAllowlisted:
    "{blueprint} requests {url}, which is not on this project's network request allowlist.",
  networkAllowlistSummary: {
    ...
    other:
      "Build stopped: {count} addresses are not on the network request allowlist. Add the addresses in project settings, or change the nodes."
  },
