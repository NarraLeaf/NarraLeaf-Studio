...
export const workspace = {
  localization: {
    panel: {
      ...
      languagesHint:
        "Languages of the game itself. The source language is the one the story is written in; the rest are translated against it.",
      ...
      removeConfirmDetail:
        "Translations stay on disk and come back if the language is added again.",
      openTable: "Open translation table",
      progress: "{completed}/{total} translated",
      staleCount: "{count} to review",
      importSummary:
        "Imported {applied} translations ({unchanged} unchanged, {unknown} unknown, {skippedEmpty} empty skipped)"
    },
    settings: {
      ...
      fallbackHint:
        "An entry with no translation here uses this language, then the source language.",
    ...
    exchange: {
      ...
      localeMismatchDetail:
        "The translations are imported into the selected language regardless of what the file declares."
    },
    table: {
      storyLabel: "Source",
      ...
      staleHint:
        "The source line changed after this translation. Save it again to mark it current.",
  ...
  voice: {
    panel: {
      ...
      removeConfirm: "Remove {name}?",
      removeConfirmDetail:
        "Voice assignments stay on disk and come back if the language is added again.",
    ...
    table: {
      ...
      outdatedHint:
        "The line changed after this take was imported. Import the clip again to mark it current.",
  ...
  recovery: {
    ...
    intro:
      "Running a check loads that part of the project and reports the result. Whatever loads can be browsed as usual.",
    problems: {
    ...
    lore: {
      ...
      restoreExplain:
        "Every file in the project is replaced with that version's contents. The current state is recorded as a version first, and the restore is added as another. No version is removed.",
      cancel: "Cancel",
      restoreDone: "Restored {version}. Reopening as a normal workspace.",
      restoreUnrecorded:
        "The files were restored, but the new version could not be recorded: {error}",
    ...
    offer: {
      ...
      detailOne:
        "A file could not be read, so part of the project is missing from this window. Common causes: an interrupted save, a sync or backup tool writing at the same time, a plugin. Editing now can write this incomplete state over the files that are still intact.",
      detailMany:
        "{count} files could not be read, so part of the project is missing from this window. Common causes: an interrupted save, a sync or backup tool writing at the same time, a plugin. Editing now can write this incomplete state over the files that are still intact.",
      enter: "Open in recovery mode"
    },
  ...
  shell: {
    ...
    save: {
      ...
      frozenDetailMerge:
        "A merge is unfinished. Finish it from the version panel to resume saving.",
    ...
    reload: {
      ...
      failedDetail:
        "These still show their previous contents: {stores}. Reopen the project to read them again.",
      ...
      server: {
        ...
        signIn: {
          ...
          trust: {
            ...
            vouched:
              "The token you pasted names this authority, and this is the authority answering at that address.",
            compare:
              "Check this fingerprint against the one whoever runs the server gives you, over something other than this connection.",
            ...
            meaning:
              "Anything holding this authority's key can then issue a certificate for any address, and this account will believe it. Only this account on this computer is affected.",
            manual:
              "This system has no per-account trust store, so Studio cannot do it. Run this, then sign in again:",
          ...
          reach: {
            ...
            notPermitted:
              "Signed in, but this account has not been given this project. Ask whoever runs the server for access.",
