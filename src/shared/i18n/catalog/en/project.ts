...
export const project = {
  ...
  details: {
    ...
    copyrightTextHelper:
      "Shipped beside the game as COPYRIGHT.txt. Left empty, no file is shipped.",
  ...
  userData: {
    description:
      "Where a shipped game keeps the player's saves and progress. The folder is named " +
  ...
  game: {
    ...
    autoSaveIntervalDescription:
      "How often to check. Nothing is written unless the story advanced.",
    autoSaveIntervalUnit: "s",
    autoSaveSlotsTitle: "Autosaves kept",
    autoSaveSlotsDescription:
      "Autosaves rotate through this many slots, oldest first. They are separate from the player's own save slots."
  },
  ...
  preferences: {
    ...
    intro:
      "The starting value of each setting for a player who has not changed it. Players can change all of them, and their choices are kept.",
    group: {
      dialogue: "Dialogue",
      skipping: "Skipping",
    ...
    showDialog: {
      ...
      description:
        "When off, the game starts with the dialogue box hidden, in the state the player's hide-UI toggle produces."
    },
    skip: {
    ...
    voiceEndMode: {
      ...
      description:
        "What happens to the clip when its line ends. Two voice clips never play at once, whichever option is selected.",
      option: {
        stop: "Stop the clip",
  ...
  audio: {
    ...
    loopDescription:
      "Clips played on this track repeat unless the action that plays them says otherwise.",
    duplicate: "Duplicate",
    delete: "Delete",
  ...
  settings: {
    ...
    networkPolicyDetail: {
      ...
      allowlist:
        "Only addresses on the allowlist below can be requested. Every other request is refused.",
      any: "The game can request any address over HTTP or HTTPS."
    },
    networkPolicyWebHint:
      "The Web export is served over HTTP and cannot enforce the off position. It does enforce the allowlist, through the page policy.",
    networkAllowlist: {
      title: "Network request allowlist",
      description: "One address or host pattern per row.",
      matchHint:
        "A host on its own covers every path under it. * can replace the first host label (*.example.com) or end a path (/v1/*). The scheme, the host and the port must match exactly.",
      placeholder: "https://api.example.com/*",
      invalid:
        "Enter an http:// or https:// address. Use * as a leading host label or at the end of a path.",
      add: "Add address",
      remove: "Remove address",
      fromPlugins: "Declared by installed plugins",
      sidecarNote:
        "A program a plugin ships runs outside the game process and is not covered by the allowlist."
    },
    encryptAssetsTitle: "Encrypt assets",
    encryptAssetsDescription:
      "Encrypt assets, plugin code and the story bundle in packaged and previewed builds. Does not affect Dev Mode.",
    encryptAssetsWebHint:
      "Not applicable to the Web export: Web builds always ship without asset protection.",
    ...
    signingDescription:
      "Which credential signs each platform. Certificates and passwords stay on this machine; the project stores only which one to use.",
    webLosslessImagesTitle: "Convert images to WebP",
    webLosslessImagesDescription:
      "Re-encode exported images as lossless WebP where that is smaller.",
    webLosslessImagesHint:
      "Each conversion is compared with the original pixel by pixel and discarded unless it decodes identically. Android and iOS builds serve the same exported site, so this applies to them too.",
    webPrecompressTitle: "Precompress text files",
    webPrecompressDescription:
      "Write Brotli and Gzip copies of the site's scripts, styles and story data.",
    webPrecompressHint:
      "Only a server set up to serve precompressed files uses them. Every other host serves the originals and ignores these.",
    webLossyImagesTitle: "Recompress images",
    webLossyImagesDescription:
      "Re-encode exported images as lossy WebP. Much smaller, and the lost detail cannot be recovered.",
    webLossyQualityTitle: "Image quality",
    webLossyQualityDescription: "WebP quality used when recompressing, from 1 to 100.",
    webSharedWithMobileHint:
      "Android and iOS builds serve the same exported site, so this applies to them too.",
  ...
  dependencies: {
    ...
    banner: {
      blocked:
        "Some plugins are disabled here: their installed version is incompatible. Update or reinstall them.",
      warnings:
