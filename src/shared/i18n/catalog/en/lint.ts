...
export const lint = {
  rule: {
    assetsUnused: {
      ...
      messageIndexUnresolved:
        "Unused assets not listed: {location} points at an asset that cannot be identified",
    ...
    storyCutPointOrphan: {
      ...
      message:
        "This project has no build variant, so this cut point ends nothing. Add a variant, or delete the row"
    },
    ...
    variablesRandomOutsideAssignment: {
      ...
      message:
        "{fn}() is re-rolled every time this condition is tested, so the branch can change between checks. Roll it once with /set, then test that variable",
      messageChoiceOption:
        "{fn}() is re-rolled every time the menu draws, so this option flickers. Roll it once with /set, then test that variable",
      messageInterpolation:
        "{fn}() is re-rolled every time this line draws, so the value changes on every redraw. Roll it once with /set, then show that variable"
    },
    textOverlong: {
      title: "Overlong line",
    ...
    brandBrokenLink: {
      ...
      messageChain:
        "{where} uses {color}, which links on to {missing} - a color the palette does not have",
