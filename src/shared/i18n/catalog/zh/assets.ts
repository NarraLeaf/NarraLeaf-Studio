...
export const assets = {
    ...
    import: {
        ...
        retry: "重试",
        remoteTitle: "导入远程资产",
        remoteDescription: "粘贴直链；文件会立即下载并随项目保存",
        remoteInvalidUrl: "输入有效的 URL",
    ...
    mediaConvert: {
        ...
        intro: "转换将在项目中生成新文件，源文件不受影响",
        convertingTitle: "正在转换",
        convertingIntro: "转换完成后自动导入",
        group: {
            lossless: "可以转换",
            losslessHint: "视频和音频保持原样",
            lossy: "可以转换，但会损失一些质量",
            lossyHint: "视频和音频会被重新编码，耗时更长，质量有损失",
            refused: "无法导入",
        },
        becomes: "转换为 .{ext}",
        refusal: {
            notMedia: "该文件不是音频或视频文件",
            noStreams: "该文件不含音频或视频流",
        },
        state: {
            waiting: "等待中",
            done: "完成",
    ...
    support: {
        ...
        needsConvertingRemoteHint: "该文件在游戏中无法播放，以链接方式保存的文件无法转换；在 Studio 之外完成转换后作为文件添加",
        notPlayable: "无法播放",
    ...
    modelImport: {
        ...
        unreadable: "无法读取该文件夹",
        tooManyFiles: "该文件夹包含 {count} 个文件，数量过多，无法逐个核对；选择模型所在层级的目录",
    },
    /**
    ...
    live: {
        ...
        refusedTitle: "未导入到会话",
        refusedQuotaOne: "{name} 超过该项目可同时传输的容量，待传输中的文件到达后再导入",
        refusedQuotaMany: "{count} 个文件超过该项目可同时传输的容量，待传输中的文件到达后再导入",
        refusedOne: "{name} 无法发送到服务器",
        refusedMany: "{count} 个文件无法发送到服务器",
        replaceRefused: "该文件无法发送到服务器",
        replaceRefusedQuota: "该文件超过该项目可同时传输的容量",
    },
    newTextFile: {
        title: "新建文本文件",
        prompt: "为文件命名。带扩展名则原样保留；不带时使用 .txt",
