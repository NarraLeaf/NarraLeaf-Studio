...
export const project = {
  ...
  userData: {
    description:
      "发布后的游戏把玩家的存档与进度存放在这里。目录名取自标识符，所以改应用名称不会挪动它。",
    copy: "复制位置",
    copied: "位置已复制",
    copyFailed: "无法复制位置",
    platform: {
  ...
  game: {
    ...
    autoSaveSlotsDescription:
      "自动存档在指定数量的槽位间轮转，最旧的先被覆盖，与玩家自己的存档槽相互独立"
  },
  preferences: {
  ...
  settings: {
    ...
    networkPolicyWebHint:
      "Web 导出通过 HTTP 提供，无法执行「不允许联网」；白名单在 Web 导出中通过页面策略执行",
    networkAllowlist: {
      title: "网络请求白名单",
      description: "每行一个地址或主机模式",
      matchHint:
        "只写主机表示该主机下的所有路径；* 可以替换首个主机标签（*.example.com），或出现在路径末尾（/v1/*）；协议、主机与端口需完全一致",
    ...
    webLosslessImagesHint:
      "每次转换都会与原图逐像素比对，解码结果不一致即丢弃；Android 与 iOS 构建使用同一份导出站点，因此该设置对它们同样生效",
    ...
    webLossyImagesDescription:
      "将导出的图像重编码为有损 WebP；体积小得多，但损失的画面细节无法恢复",
