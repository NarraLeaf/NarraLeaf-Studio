...
export const settings = {
  ...
  items: {
    ...
    maxActiveEditors: {
      label: "最大活动编辑器数",
      description:
        "同时保持加载并保留滚动位置与焦点的编辑器标签数（{min}-{max}）；其余标签在重新打开时重新加载"
    },
    ...
    localizedCommands: {
      label: "指令跟随界面语言",
      description:
        "关闭后，动作创建中的指令名、参数名与取值保持英文；无论显示哪种语言，英文写法始终可用"
    },
    hideParamNames: {
      label: "指令只显示参数值",
    ...
    spellcheckLanguage: {
      ...
      noDictionary:
        "尚未安装本工程所用语言的拼写词典，正文中不会标出任何内容。工程词典仍然保存着本工程自己的词条",
      options: {
    ...
    electronBuilderBinariesMirror: {
      label: "构建工具下载镜像",
      description:
        "构建时下载安装器工具（NSIS、AppImage、代码签名辅助程序）所用的镜像地址，留空则使用官方源"
    },
    downloadRewrites: {
      label: "下载地址替换"
    },
  ...
  dictionaries: {
    ...
    noDictionaryLanguages:
      "中文和日语没有拼写词典，以后也不会有：这两种语言词与词之间不分开写，词表无从比对。用它们写的工程不会被标出任何内容",
    installed: {
      title: "本机已有",
      emptyTitle: "还没有词典",
  ...
  transfer: {
    ...
    apply: "应用",
    exportHint:
      "把设置写入一份纯 JSON 文件；工作区背景图、提交版本上的署名、最近项目、统计数据和窗口布局仅保留在本机",
    exported: "已保存到 {path}",
