...
export const launcher = {
    ...
    servers: {
        ...
        problem: {
            ...
            // 只有发布时才会遇到，而这个页面在「为某台服务器新建项目」时就会发布。
            // 名字就是项目自己的 app id，审阅页已经写过，这里不重复。
            nameTaken: "这台服务器上已经有另一个项目叫这个名字",
            // 这里不重复名字：这个页面按项目自己的 app id 发布，审阅页已经写过，
            // 而这行旁边就是列表，能看到它已经以什么名字在服务器上。
            alreadyPublished: "这个项目已经以另一个名字在这台服务器上了",
            unknown: "无法读取这台服务器",
        },
        // 在本机建好、但服务器没有接收的项目。项目本身是完整的，缺的只是与服务器的连接，
        ...
        detail: {
            ...
            // 那种情况在下面有自己的一行。
            unread: "服务器还没有读取这个项目",
            // 写在 unread 之前并取代它：没人往里面发送过内容的项目，本来就没有项目文件可读，
            // 两句话说的是同一件事。
            empty: "该项目尚未收到任何内容",
            versions: "最近的版本",
            noVersions: "没有记录任何版本",
        ...
        // 别人的实时会话，以及所有加入的入口。它们在这里而不在编辑器里，因为加入往往要先取得
        // 项目，而取得项目需要一个没有打开项目的窗口——就是这一个。
        live: {
            title: "实时会话",
            untitled: "未命名的会话",
            hostedBy: "由 {name} 主持",
            members: "{count} 人在会话中",
            join: "加入",
            // 「需要同意」的房间按下这个并不会立刻放人进去，而是把一个问题递到主持方面前。
            // 写「加入」等于承诺了别的事。
            ask: "申请加入",
            passcodePrompt: "有口令？",
            // 口令错和口令没人用是同一句，因为服务器答的就是同一句——见加入规则。
            noSuchCode: "没有实时会话用这个口令",
            unreachable: "该会话所在的项目不在这台服务器列出的清单里",
        },
        discussion: {
            title: "讨论",
            empty: "该项目还没有备注。",
