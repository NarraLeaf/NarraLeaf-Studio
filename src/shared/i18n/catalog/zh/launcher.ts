...
export const launcher = {
    ...
    // 这里不重复。
    servers: {
        // 打开「添加服务器」对话框的那一个控件，在视图切换那条线的右端，以及空状态里。
        // 只作提示与标签，不出现为可见文字。
        manage: "添加服务器",
        empty: {
            title: "没有服务器",
        ...
        here: "已在本机",
        // 服务器读过该仓库，里面没有内容。只有服务器给出的数字确实为零时才写；
        // 还没读过的服务器什么都不说，这里也就什么都不写。
        nothingSent: "尚未发送内容",
        ...
        problem: {
            ...
            // 只有发布时才会遇到，而这个页面在「为某台服务器新建工程」时就会发布。
            // 名字就是工程自己的 app id，审阅页已经写过，这里不重复。
            nameTaken: "这台服务器上已经有另一个项目叫这个名字",
            unknown: "无法读取这台服务器",
        },
        // 在本机建好、但服务器没有接收的项目。工程本身是完整的，缺的只是与服务器的连接，
        // 这个对话框负责把它打开，让版本轨道再试一次。
        unsent: {
            title: "项目未发送",
            message: "{name} 已在本机创建，未发送到 {server}",
            open: "打开项目",
            close: "关闭",
        },
        ...
        detail: {
            ...
            // 那种情况在下面有自己的一行。
            unread: "服务器还没有读取这个项目",
            // 写在 unread 之前并取代它：没人往里面发送过内容的项目，本来就没有工程文件可读，
            // 两句话说的是同一件事。
            empty: "该项目尚未收到任何内容",
            versions: "最近的版本",
            noVersions: "没有记录任何版本",
        ...
        // 服务器上关于某个项目的讨论，走会话读取。只有支持讨论的服务器才会出现。
        discussion: {
            title: "讨论",
            empty: "该项目还没有备注。",
            placeholder: "写一条备注",
