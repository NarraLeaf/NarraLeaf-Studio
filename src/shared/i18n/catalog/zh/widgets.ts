import type { LocaleNamespace } from "../types";

export const widgets = {
  tabs: {
    properties: "属性",
    interaction: "交互"
  },
  blueprint: {
    controlLabel: "控制蓝图"
  },
  appearance: {
    title: "外观",
    modulesHelp: "紧凑型模块，支持按模块单独覆盖状态（可在标题栏菜单中添加或移除）"
  },
  content: "内容",
  textLabel: "文本",
  direction: "方向",
  horizontal: "水平",
  vertical: "垂直",
  gap: "间距",
  perSidePx: "各边（px）",
  sides: {
    top: "上",
    right: "右",
    bottom: "下",
    left: "左"
  },
  typography: {
    title: "排版",
    font: "字体",
    weight: "字重",
    regular: "常规",
    semibold: "半粗",
    bold: "粗体",
    lineWrap: "换行",
    wrapWords: "按词",
    wrapCharacters: "按字符",
    wrapNone: "不换行",
    alignment: "对齐",
    verticalAlignment: "垂直对齐",
    alignLeft: "左对齐",
    alignCenter: "水平居中",
    alignRight: "右对齐",
    alignTop: "顶端对齐",
    alignMiddle: "垂直居中",
    alignBottom: "底端对齐",
    lineHeightHint: "行高（无单位）",
    italic: "斜体",
    enableItalic: "启用斜体",
    disableItalic: "禁用斜体"
  },
  localization: {
    title: "本地化",
    textKey: "文本键",
    none: "无",
    createKey: "新建键…",
    createKeyTitle: "新建本地化键",
    keyName: "键名",
    keyNamePlaceholder: "menu.start…",
    keyNameHint: "可包含字母、数字，以及位于其间的点 / 下划线 / 连字符",
    sourceText: "源文本",
    sourceTextPlaceholder: "无译文时显示的文本",
    invalidKeyName: "键名只能包含字母、数字，以及位于其间的点 / 下划线 / 连字符",
    keyExists: "已存在名为“{name}”的键",
    keyServiceUnavailable: "当前环境不支持本地化"
  },
  list: {
    title: "列表",
    sectionPreview: "预览",
    runtimeItems: "运行时项",
    runtimePreviewOnly: "仅预览",
    runtimePageState: "页面状态数组",
    runtimeAppState: "应用状态数组",
    runtimePageProps: "页面属性数组",
    stateKey: "状态键",
    stateKeyPlaceholder: "choices",
    propName: "属性名",
    propNamePlaceholder: "choices",
    previewCount: "预览数量",
    sectionLayout: "布局",
    sectionTemplate: "项模板",
    sectionScrollbar: "滚动条",
    sectionEffects: "效果",
    trackColor: "轨道颜色",
    trackImage: "轨道图片",
    thumbColor: "滑块颜色",
    thumbImage: "滑块图片",
    flow: "排列",
    cellContent: "单元内容",
    dragToScroll: "拖动内容滚动",
    behavior: "行为",
    enabled: "启用",
    side: "位置",
    visibility: "可见性",
    visibilityAuto: "自动",
    visibilityAlways: "始终",
    visibilityHidden: "隐藏",
    contentReserve: "内容预留",
    defaultMetrics: "默认尺寸",
    inset: "内缩",
    thickness: "粗细",
    minThumb: "最小滑块",
    contentPadding: "内容内边距",
    contentPaddingAll: "所有边的内容内边距",
    contentPaddingSide: "内容内边距 {side}",
    contentPaddingPerSide: "各边内容内边距",
    contentPaddingPerSideEdit: "编辑各边内容内边距",
    contentPaddingPerSideClose: "关闭各边内容内边距",
    contentPaddingDialog: "各边内容内边距",
    customizeScrollbar: "自定义滚动条",
    scrollbarPartsCreated: "已创建滚动条部件",
    scrollbarPartsAuthoredHint: "轨道与滑块是列表大纲中已定义的元素，选中即可编辑其外观",
    scrollbarCustomizeHint: "创建可编辑的轨道与滑块元素，且不会改变项目模板"
  },
  slider: {
    title: "滑块",
    value: "值",
    min: "最小值",
    max: "最大值",
    step: "步长",
    parts: "部件",
    track: "轨道",
    handle: "手柄",
    repairParts: "修复部件"
  },
  switch: {
    title: "开关",
    state: "状态",
    defaultChecked: "默认开启",
    defaultCheckedHint: "切换画布预览的初始状态，不是玩家的运行时状态",
    interactionDisabled: "禁用交互",
    parts: "部件",
    track: "轨道",
    thumb: "滑钮",
    repairParts: "修复部件",
    recomputeTravel: "按当前尺寸重算行程"
  },
  textInput: {
    title: "文本输入框",
    value: "值",
    valueHint: "初始文本；玩家输入的内容由运行中的游戏保存，不会写回本文档",
    placeholder: "占位文本",
    placeholderKey: "占位文本键",
    placeholderKeyHint: "占位文本会展示给玩家：绑定键以便翻译",
    sectionBehavior: "行为",
    inputMode: "输入模式",
    inputModeText: "文本",
    inputModePassword: "密码",
    inputModeNumber: "数字",
    maxLength: "最大长度",
    maxLengthHint: "0 表示不限制",
    readOnly: "只读",
    disabled: "禁用",
    color: "颜色"
  },
  button: {
    title: "按钮",
    sectionText: "文本",
    sectionColor: "颜色",
    sectionBehavior: "行为",
    interactionDisabled: "禁用交互",
    localizeLabel: "本地化标签"
  },
  text: {
    title: "文本",
    localizeText: "本地化文本"
  },
  frame: {
    title: "页面",
    page: "页面",
    props: "参数",
    animation: "动画",
    usePageAnimation: "使用页面动画",
    pageLogic: "页面组件逻辑",
    scale: "缩放",
    selectPage: "选择页面",
    missingPage: "缺少页面",
    targetNotPage: "目标不是页面",
    pageLoopBlocked: "已阻止页面循环",
    previewUnavailable: "页面预览不可用",
    pageRootMissing: "缺少页面根元素"
  },
  container: {
    title: "容器",
    childLayout: "子级布局",
    mode: "模式",
    modeFree: "自由（绝对定位子级）",
    modeStack: "堆叠（弹性）",
    modeScroll: "滚动",
    scrollAxis: "滚动轴",
    verticalStack: "垂直堆叠",
    horizontalStack: "水平堆叠",
    alignCross: "对齐（交叉轴）",
    justifyMain: "对齐（主轴）",
    start: "起始",
    center: "居中",
    end: "末端",
    stretch: "拉伸",
    spaceBetween: "两端对齐",
    spaceAround: "环绕对齐",
    gapHint: "子级之间的间距",
    overflow: "溢出",
    overflowHidden: "隐藏",
    overflowVisible: "可见",
    padding: "内边距",
    paddingAllSides: "所有边的内边距",
    paddingAllTitle: "内边距（所有边）",
    paddingSide: "内边距 {side}",
    perSidePadding: "各边内边距",
    perSidePaddingEdit: "编辑各边内边距",
    perSidePaddingClose: "关闭各边内边距",
    paddingDialog: "各边内边距"
  },
  image: {
    title: "图片"
  },
  video: {
    title: "视频",
    sectionSource: "来源",
    sectionPlayback: "播放",
    sectionBox: "外框",
    asset: "视频文件",
    assetNone: "未选择视频",
    assetChoose: "选择视频",
    assetMissing: "资产缺失（{id}）",
    poster: "封面",
    posterNone: "未选择封面",
    posterChoose: "选择封面",
    fit: "填充",
    fitHint: "画面如何填满外框",
    fitNone: "原始尺寸",
    loop: "循环",
    muted: "静音",
    autoplay: "自动播放",
    autoplayHint: "仅在游戏中生效；编辑画布始终暂停，预览请使用工具条",
    volume: "音量",
    volumeHint: "0 到 1；运行时蓝图的音量引脚会覆盖此值",
    audioTrack: "音轨",
    audioTrackDefault: "音效",
    playbackRate: "速度",
    controls: "原生控件",
    controlsHint: "在画面上显示浏览器自带的播放条",
    preload: "预加载",
    preloadHint: "播放前预取的数据量；画布始终加载元数据，以便显示一帧画面",
    preloadNone: "不预加载",
    preloadMetadata: "仅元数据",
    preloadAuto: "自动",
    backdrop: "背景色",
    backdropHint: "视频比例填不满外框时露出的部分",
    previewPlay: "预览播放",
    previewPause: "暂停预览",
    previewRestart: "回到首帧"
  },
  puppet: {
    title: "模型",
    sectionModel: "模型",
    sectionState: "姿态",
    sectionBox: "外框",
    runtimeNotice:
      "Studio 不提供模型渲染器；将渲染器放到 runtimes/puppet/<名称>/index.js，然后在下方选择",
    model: "模型包",
    modelNone: "未选择模型包",
    modelChoose: "选择模型",
    modelMissing: "资产缺失（{id}）",
    backend: "运行时",
    backendNone: "未选择运行时",
    backendHint: "runtimes/puppet 下每个运行时一个目录",
    backendNoneInstalled: "本工程的 runtimes/puppet 下没有任何运行时",
    options: "运行时选项",
    optionsHint: "一个 JSON 对象，原样转交运行时",
    optionsInvalid: "不是 JSON 对象，本次修改未保存",
    motion: "动作",
    expression: "表情",
    skin: "皮肤",
    stateNone: "无",
    skinDefault: "模型默认",
    describing: "正在向模型询问它包含什么…",
    describeOk: "名称来自模型本身",
    describeNoModel: "选择模型包后才能列出名称",
    describeNoBackend: "选择运行时后才能列出模型的名称",
    describeBackendMissing: "本机未安装该运行时，名称需要手动输入",
    describeNotSupported: "该运行时不描述模型，名称需要手动输入",
    describeFailed: "读取模型失败，名称需要手动输入",
    redescribe: "重新读取",
    clearState: "复位",
    clearStateHint: "清空动作、表情、皮肤、参数与插槽",
    backdrop: "背景色",
    backdropHint: "画在模型后面；默认关闭，让画面本身透出来",
    placeholderUnconfigured: "请选择模型包与运行时",
    placeholderNoModel: "此处无法读取该模型包",
    placeholderBackendMissing: "本工程没有名为“{backend}”的运行时",
    placeholderLoading: "正在加载模型…",
    placeholderError: "运行时出错：{error}",
    placeholderBudget: "未绘制：本窗口已经绘制了 {drawn} 个模型",
    placeholderRuntimeNote: "Studio 不提供渲染器，需要作者自备"
  },
  rectangleInspector: {
    cornerRadius: "圆角半径",
    cornerJoin: "拐角连接",
    cornerTL: "左上",
    cornerTR: "右上",
    cornerBL: "左下",
    cornerBR: "右下",
    toggleCornerBreakdown: "切换圆角拆分",
    layer: "图层",
    toggleLayerVisibility: "切换图层可见性",
    fill: "填充",
    fillType: "填充类型",
    imageFill: "图片填充",
    toggleFillVisibility: "切换填充可见性",
    border: "边框",
    borderStyle: "边框样式",
    borderSides: "边框各边",
    toggleBorderVisibility: "切换边框可见性",
    moreBorderOptions: "更多边框选项"
  },
  blueprintValue: {
    sliderTitle: "滑块值",
    switchCheckedTitle: "开关开启状态",
    textInputValueTitle: "文本输入框值",
    pagePropsTitle: "页面参数值",
    buttonTextTitle: "按钮文本值",
    textTitle: "文本值",
    literalValue: "字面值",
    static: "静态",
    nameValue: "{name} 值",
    nameProps: "{name} 参数",
    nameLabel: "{name} 标签",
    nameText: "{name} 文本"
  },
  defaults: {
    button: {
      name: "按钮",
      label: "按钮"
    },
    choiceList: {
      name: "选项列表",
      choiceA: "选项 A",
      choiceB: "选项 B",
      choiceC: "选项 C"
    },
    container: {
      name: "容器"
    },
    dialog: {
      name: "句子",
      text: "当前对白将显示在这里"
    },
    frame: {
      name: "页面"
    },
    image: {
      name: "图片"
    },
    list: {
      name: "列表"
    },
    notificationList: {
      name: "通知列表",
      message1: "通知消息",
      message2: "另一条消息"
    },
    nvl: {
      name: "NVL 文本",
      text: "对白条目文本将显示在这里"
    },
    nvlList: {
      name: "NVL 列表",
      speaker: "说话人"
    },
    root: {
      name: "根"
    },
    slider: {
      name: "滑块",
      track: "滑块轨道",
      handle: "滑块手柄"
    },
    switch: {
      name: "开关",
      track: "开关轨道",
      thumb: "开关滑钮",
      onVariant: "开启"
    },
    text: {
      name: "文本",
      text: "文本"
    },
    textInput: {
      name: "文本输入框"
    },
    video: {
      name: "视频"
    },
    // 存储的类型名不点名任何渲染器，作者面明确点名两家——用户裁决 2026-07-29。
    puppet: {
      name: "Spine2D / Live2D 模型"
    }
  },
  unsupportedType: "不支持的类型：{type}"
} satisfies LocaleNamespace<"widgets">;
