...
export const build = {
  ...
  signing: {
    ...
    macIdentityEmpty:
      "本机钥匙串里没有代码签名证书；请在「钥匙串访问」中安装一张，或改用证书文件导入",
  ...
  preflight: {
    ...
    "cut-point-inert":
      "{scene}（{story}）中的截断点没有从 {variant} 里去掉任何内容，该构建仍会带上整个剧本",
    // 只针对真的会截短剧本的变体；两种回答都算作答：选一个页面，或者在该变体上选「不显示任何页面」，
    // 让画面停在最后一帧。
    "variant-ending-missing":
      "{variant} 会提前结束剧本，但没有指定结束后显示的页面。请在 项目 ▸ 应用 ▸ 变体 中选择",
    // 句子里不带数量：对话框用的是普通翻译函数，选不了复数形式，而且这个数字并不比场景名多说什么。
    "variant-branch-uncut":
      "从 {scene}（{story}）出发的部分路线上没有截断点，{variant} 会把这些路线整段发出去",
    ...
    "plugin-secret-unavailable":
      "{plugin} 的「{field}」在其他设备上设置，本机没有它的值。在此重新输入即可构建 {platforms}",
    ...
    "sidecar-target-missing":
      "{plugin} 没有为 {platform} 提供 {sidecar} 程序，它所支撑的功能在这份产物里不会生效",
    ...
    "version-uncodable":
      "项目版本 {version} 无法编码为 Android 版本号（主版本号最大 2099，次版本号与修订号最大 999）",
    ...
    "unsigned-android":
      "使用本地调试签名，仅供旁加载安装，这样签出的 AAB 也不能用作 Google Play 的上传密钥；选择你自己的 release keystore 即可用你的身份签名",
    "unsigned-ios":
      "这份 .ipa 未签名，而 iOS 不允许安装任何未签名应用；请选择一份 Apple 签名凭据；从钥匙串导出 .p12 时要连同签发链一起导出，否则签名会失败",
    "signing-credential-missing":
      "本机没有本工程为 {platform} 指定的签名凭据，密钥材料不会随工程流转；请在此导入，或清除该选择以未签名方式构建 {platform}",
    "signing-credential-expired":
      "{platform} 签名证书不在有效期内（{notBefore} 至 {notAfter}），签名会失败；请向签发方续期并导入新证书",
    "signing-credential-expiring":
      "{platform} 签名证书将于 {notAfter} 到期；在此之前签出的产物仍然有效，之后的构建需要续期后的证书",
    "signing-secret-unavailable":
      "本机无法读取 {platform} 签名凭据的密码；重新导入一次即可重新保存密码",
    "signing-tool-missing":
      "为 {platform} 签名需要 {tool}，本机没有安装；请安装并确保它在 PATH 中，然后重新打开本对话框",
    "signing-host-unsupported":
      "本机是 {host}，无法用所选凭据为 {platform} 签名：它的私钥由只存在于对应平台的系统服务保管；请在 {platform} 机器上构建这个目标",
    "signing-needs-network": "为 {platform} 签名需要联网；构建的其余环节都可离线完成",
    "signing-macos-identity-missing":
      "本机钥匙串里没有名为 {identity} 的证书；请在「钥匙串访问」中安装它，或在此改选其他证书",
    "signing-macos-identity-unusable":
      "证书 {identity} 无法用于签名：它已过期、私钥不在、或签发链不完整；请在「钥匙串访问」中打开它确认原因",
    "signing-macos-not-developer-id":
      "{identity} 不是「Developer ID Application」证书；产物能在本机运行，但在别人的 Mac 上会被 Gatekeeper 拒绝，Apple 也不会为它公证",
    "signing-android-not-play":
      "签名后的 APK 适用于旁加载安装，以及 itch.io 等接受 APK 的平台；Google Play 只接受 AAB 包，在 Android 目标下打开 AAB 格式即可产出",
    "signing-ios-profile-mismatch":
      "应用 ID {bundleId} 不在描述文件的覆盖范围内，该描述文件签发给的是 {profileAppId}；请修改工程标识符，或导入与之匹配的描述文件",
  ...
  appTagGraphUnresolved:
    "{blueprint} / {graph} 中的变体没有得出固定值，请把它与变体名比较，或直接使用它的值",
  appTagGraphUnknownNode:
    "{blueprint} / {graph} 既判断了变体，又使用了本次构建无法读取的节点，请把变体判断移到不含该节点的图中",
  appTagGraphFnHead:
    "{blueprint} / {graph} 中的变体判断决定了一个 Fn 是否存在，请把该 Fn 移出它决定的分支",
  appTagGraphSummary: {
  ...
  contentBlockedStartStory:
    "{location} 中的开始游戏节点在运行时才确定场景。请在检查器中选定场景，或在 {variant} 变体中列出它可以开始的场景",
  contentBlockedScript:
    "蓝图 {location} 使用 TypeScript 编写，可以开始任意场景。请在 {variant} 变体中列出它可以开始的场景",
  contentBlockedPlugin:
    "{location} 插件可以开始任意场景。请在 {variant} 变体中列出它可以开始的场景",
  ...
  networkAllowlistSummary: {
    ...
    other:
      "构建已停止：{count} 个地址不在网络请求白名单内。请在工程设置中添加这些地址，或修改对应节点"
  },
  mediaUnchecked: {
