...
export const workspace = {
    ...
    shell: {
        ...
        versionControl: {
            ...
            // 版本」一样整体重读一遍。
            restoring: "正在恢复到这个版本…",
            // 恢复唯一一种「失败时文件已经换过了」的失败：重写已经做完，只有记录它的那次提交没成。
            // 先说他的项目现在是什么样，再说错误——因为作者本来会得出的结论「失败了所以什么都没
            // 发生」正好与事实相反，然后他就在一个悄悄退回上周的项目上继续干活。`{action}` 是
            // 「记录一个版本」那个按钮，从按钮自己的文案取，免得这句话指向一个已经改了名的控件。
            restoreNotRecordedTitle: "文件已经恢复了，但这个版本没记录下来",
            restoreNotRecordedDetail:
                "你的项目文件现在是版本 {version} 的内容。把它记录成一个新版本时失败了（{error}）。"
                + "什么都没丢——按「{action}」就能自己把它记下来。",
            ...
            changes: "变更",
            refreshChanges: "检查变更",
            // 记录一个版本的按钮。用「记录」而不是「提交」：这一整块说的都是「版本」，而一个从没
            // 用过版本控制的作者没有理由认识那个词。
            commit: "记录一个版本",
            ...
            // 唯一一种会挡住「记录版本」的变更——所以它被单独标出来，也因此排在清单最前面，
            // 而不是按路径落在它本来的位置上。
            changeConflict: "未解决的冲突",
            // 清单有上限。这件事要说出来：一个悄悄停在第五十条的列表会被读成「一共就这些」，
            // 作者会以为自己看全了要记录的东西，然后就去记录了。
