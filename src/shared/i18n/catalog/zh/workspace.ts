...
export const workspace = {
    ...
    recovery: {
        ...
        windowTitle: "NarraLeaf Studio — 恢复模式",
        title: "恢复模式",
        subtitle: "该工程以只读方式打开，未加载任何插件。",
        copied: "诊断信息已复制到剪贴板。",
        openFolderFailed: "无法打开工程目录：{error}",
        banner: {
            title: "恢复模式",
            detail: "此处的任何操作都不会写入你的工程，插件也未加载。",
            exit: "退出恢复模式",
        },
        guide: {
            title: "把这个工程救回来",
            step1: "先看左侧的报错，每一条都附有原始错误和它来自哪个文件。",
            step2: "逐个运行加载检查，弄清工程的哪些部分还能正常读取。",
            step3: "要反馈问题就导出日志；想自己看那个文件，就打开工程目录。",
            step4: "如果损坏之前提交过版本，直接恢复到那个版本——通常这是最快的办法。",
            readOnly: "退出恢复模式会把这个窗口重新打开为普通工作区。",
        },
        anomalies: {
            title: "发现的问题（{count}）",
            empty: "启动过程中没有报出任何问题。可以运行下面的加载检查继续排查。",
            showRaw: "显示原始错误",
        },
        probes: {
            title: "加载检查",
            hint: "下面这些都还没有加载。点一项就会真的去加载它，并把结果原样显示出来。",
            run: "运行",
            rerun: "重新运行",
            runAll: "全部运行",
            project: "工程清单",
        ...
        offer: {
            message: "这个工程没有正常加载",
            detail: "有 {count} 处问题是被跳过而不是被修好的，你现在看到的内容可能不完整。恢复模式会把这个窗口以只读、不加载插件的方式重新打开，并显示原始报错。尚未自动保存的改动不会被写入。",
            enter: "进入恢复模式",
        },
        operations: {
