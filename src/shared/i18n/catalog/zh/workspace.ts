...
export const workspace = {
  localization: {
    panel: {
      ...
      importSummary:
        "已导入 {applied} 条翻译（{unchanged} 条未变更，{unknown} 条未知，{skippedEmpty} 条空译文已跳过）"
    },
    settings: {
      menu: "语言设置…",
      title: "{name}的语言设置",
  ...
  recovery: {
    ...
    lore: {
      ...
      restoreExplain:
        "工程中的每个文件都会被替换为该版本的内容；当前状态会先提交为一个版本，恢复结果作为新版本追加，不会删除任何版本",
      cancel: "取消",
    ...
    offer: {
      message: "该工程未能正常加载",
      detailOne:
        "有一个文件无法读取，本窗口中缺少工程的一部分内容；常见原因：保存被中断、同步或备份工具同时写入、插件干扰；此时继续编辑可能把残缺的状态写回磁盘，覆盖仍然完好的文件",
      detailMany:
        "有 {count} 个文件无法读取，本窗口中缺少工程的一部分内容；常见原因：保存被中断、同步或备份工具同时写入、插件干扰；此时继续编辑可能把残缺的状态写回磁盘，覆盖仍然完好的文件",
      enter: "以恢复模式打开"
    },
    operations: {
  ...
  shell: {
    ...
    versionControl: {
      ...
      restoreConfirmDetail:
        "项目文件会被替换为该版本的内容；当前状态会先记录为一个检查点，不会删除任何版本",
      ...
      server: {
        ...
        signIn: {
          ...
          trust: {
            ...
            fingerprintLabel: "指纹",
            meaning:
              "持有该机构密钥的任何一方，都能为任意地址签发证书，而这个账户都会相信。受影响的只有这台电脑上的这个账户。",
            manual:
              "这个系统没有按账户的信任库，Studio 无法代劳。请运行下面这条命令，然后重新登录：",
            copy: "复制命令",
            confirm: "信任",
            cancel: "取消"
          },
