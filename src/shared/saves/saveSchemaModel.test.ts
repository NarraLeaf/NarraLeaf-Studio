...
describe("deriveSaveSchemaStorageKey", () => {
  ...
  it("uniquifies against keys already taken", () => {
    ...
    expect(deriveSaveSchemaStorageKey("Chapter", "id-3", ["chapter", "chapter_2"])).toBe(
      "chapter_3"
    );
  });
...
describe("migrateSaveSchemaToLatest", () => {
  ...
  it("keys every field by its map key, whatever the record says", () => {
    ...
    const migrated = migrateSaveSchemaToLatest({
      schemaVersion: 1,
      fields: {
        real: { id: "stale", name: "Chapter", valueType: "string", storageKey: "chapter", order: 0 }
      }
    });
  ...
  it("fills in what a hand-edited record left out", () => {
    ...
    expect(migrated.fields.a).toEqual({
      id: "a",
      name: "a",
      valueType: "string",
      storageKey: "a",
...
describe("saveSchemaFieldFallback", () => {
  it("prefers the author's configured default", () => {
    expect(
      saveSchemaFieldFallback(field({ id: "a", valueType: "string", defaultValue: "Prologue" }))
    ).toBe("Prologue");
  });
  ...
  it("keeps a configured default that is falsy", () => {
    ...
    expect(saveSchemaFieldFallback(field({ id: "a", valueType: "integer", defaultValue: 0 }))).toBe(
      0
    );
    expect(
      saveSchemaFieldFallback(field({ id: "a", valueType: "boolean", defaultValue: false }))
    ).toBe(false);
...
describe("buildSaveMetadataFromFields", () => {
  ...
  it("writes by storage key, not by field id", () => {
    ...
    expect(buildSaveMetadataFromFields(fields, { f1: "Chapter 2", f2: 120 })).toEqual({
      chapter: "Chapter 2",
      play_time: 120
    });
  });
...
describe("readSaveMetadataFields", () => {
  ...
  it("reads back by field id, so a pin is addressed by identity", () => {
    expect(readSaveMetadataFields(fields, { chapter: "Chapter 2", play_time: 120 })).toEqual({
      f1: "Chapter 2",
      f2: 120
    });
  });
  ...
  it("gives a field declared after the save was written its configured default", () => {
    ...
    expect(readSaveMetadataFields(fields, { chapter: "Chapter 2" })).toEqual({
      f1: "Chapter 2",
      f2: 0
    });
  });
