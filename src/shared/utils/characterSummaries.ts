...
function mapAvatars(raw: unknown): Record<string, CharacterAvatarSummaryEntry> | undefined {
  ...
  for (const [key, value] of Object.entries(raw as Record<string, unknown>)) {
    ...
    if (baked || overrideAssetId) {
      avatars[key] = {
        ...(baked ? { baked } : {}),
        ...(overrideAssetId ? { overrideAssetId } : {})
      };
    }
  }
 ...
 */
function mapPuppetDefaultState(
  raw: unknown
): { motion: string | null; expression: string | null; skin: string | null } | null {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return null;
  }
...
function mapAppearance(appearance: unknown): CharacterAppearanceSummary {
    ...
    return {
      ...
      options:
        raw.options && typeof raw.options === "object" && !Array.isArray(raw.options)
          ...
          : {},
      ...(mapPuppetDefaultState(raw.defaultState)
        ? { defaultState: mapPuppetDefaultState(raw.defaultState)! }
        : {})
    };
  }
  ...
  if (kind === "layered") {
    ...
    return {
      ...
      layers: (Array.isArray(raw.layers) ? raw.layers : []).flatMap((entry) => {
        ...
        const options =
          rawLayer.options && typeof rawLayer.options === "object"
            ? Object.fromEntries(
                Object.entries(rawLayer.options).map(([tagId, assetId]) => [
                  tagId,
                  trimmed(assetId) || null
                ])
              )
            : undefined;
        return [
  ...
  return {
    ...
    poses,
    defaultPoseId:
      defaultPoseId && poses.some((pose) => pose.id === defaultPoseId)
        ? defaultPoseId
        : (poses[0]?.id ?? null),
 ...
 */
export function mapCharacterStoreEntriesToSummaries(
