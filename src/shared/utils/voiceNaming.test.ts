...
function tokens(overrides: Partial<Parameters<typeof formatVoiceFilename>[1]> = {}) {
    return { scene: "Prologue", index: 1, character: "Alice", locale: "ja", unitId: "t-1", ...overrides };
}
...
describe("formatVoiceFilename", () => {
    ...
    it("keeps CJK names in the filename", () => {
        expect(formatVoiceFilename(PATTERN, tokens({ scene: "序章", character: "優希" }))).toBe("序章_001_優希");
    });
});

describe("matchKeyForFilename", () => {
    it("ignores extension, folders, case and cosmetic punctuation", () => {
        expect(matchKeyForFilename("takes/Prologue_001_Alice.wav")).toBe(matchKeyForFilename("prologue 001-alice.mp3"));
    });
    ...
    it("keeps CJK apart instead of collapsing it to the index", () => {
        const a = matchKeyForFilename(formatVoiceFilename(PATTERN, tokens({ scene: "序章", character: "優希" })));
        const b = matchKeyForFilename(formatVoiceFilename(PATTERN, tokens({ scene: "第一章", character: "優希" })));
    ...
    it("matches full-width digits against half-width ones", () => {
        expect(matchKeyForFilename("序章_００１_優希.wav")).toBe(matchKeyForFilename("序章_001_優希.wav"));
    });
