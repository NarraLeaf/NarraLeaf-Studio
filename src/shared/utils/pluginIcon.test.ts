...
describe("validatePluginIconBytes", () => {
    ...
    it("rejects an icon past the byte budget", () => {
        expect(validatePluginIconBytes(png(512, 512, PLUGIN_ICON_MAX_BYTES + 1), "icon.png"))
            .toMatch(/at most 512 KB/);
    });

    it("rejects bytes whose format contradicts the extension", () => {
        expect(validatePluginIconBytes(png(512, 512), "icon.webp"))
            .toMatch(/is a PNG file with a \.webp name/);
    });

    it("rejects bytes that are not a readable image", () => {
        const svg = Uint8Array.from([...'<svg xmlns="http://www.w3.org/2000/svg"></svg>'].map(c => c.charCodeAt(0)));
