...
describe("mapCharacterStoreEntriesToSummaries", () => {
  it("maps a named character", () => {
    const summaries = mapCharacterStoreEntriesToSummaries([
      entry({ id: "char-alice", name: "Alice" })
    ]);
  ...
  it.each([
    ...
    const [summary] = mapCharacterStoreEntriesToSummaries([
      entry({ id: "char-alice", name: "Alice", voiceTrackId })
    ]);
  ...
  it("does not judge whether the colour is readable", () => {
    const [summary] = mapCharacterStoreEntriesToSummaries([
      entry({ id: "char-alice", color: "#FFFFFF" })
    ]);
  ...
  it("skips entries with no usable id", () => {
    expect(
      mapCharacterStoreEntriesToSummaries([
        entry({ id: "", name: "Alice" }),
        entry({ name: "Bob" }),
        { profile: null },
        null
