...
function dep(
  overrides: Partial<ProjectPluginDependency> & Pick<ProjectPluginDependency, "id">
...
function installed(
  entries: Array<Partial<InstalledPluginInfo> & Pick<InstalledPluginInfo, "id">>
): InstalledPluginInfo[] {
...
describe("resolveDependencies", () => {
  it("marks a matching plugin satisfied and not suppressed", () => {
    const result = resolveDependencies(
      table([dep({ id: "a.b", authoredVersion: "1.2.0" })]),
      installed([{ id: "a.b", version: "1.4.0" }])
    );
  ...
  it("suppresses a hard dependency whose major is incompatible", () => {
    const result = resolveDependencies(
      table([dep({ id: "a.b", authoredVersion: "1.0.0" })]),
      installed([{ id: "a.b", version: "2.0.0" }])
    );
  ...
  it("does not suppress a soft dependency even when incompatible", () => {
    const result = resolveDependencies(
      table([dep({ id: "a.b", hard: false, authoredVersion: "1.0.0" })]),
      installed([{ id: "a.b", version: "2.0.0" }])
    );
    expect(result.entries[0].status).toBe("incompatible");
  ...
  it("reports an older same-major install as outdated (warn, not suppressed)", () => {
    const result = resolveDependencies(
      table([dep({ id: "a.b", authoredVersion: "1.5.0" })]),
      installed([{ id: "a.b", version: "1.2.0" }])
    );
    ...
    const normalized = normalizeProjectDependencyTable({
      ...
      plugins: [
        ...
        { authoredVersion: "1.0.0" },
        {
          ...
          usedBy: { blueprintNode: ["a.plugin.x", "a.plugin.x", 5] }
        }
      ]
    });
