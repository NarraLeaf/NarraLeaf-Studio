...
export const NO_INSTALL_PERMISSIONS_COPY =
  "No privileged Studio controls are included in this install approval.";

export function describePluginInstallPermissions(
...
export function describePluginInstallPermission(permission: PluginInstallPermission): string {
  switch (permission.kind) {
    ...
    case "sidecar":
      return (
        `${describeSidecarKind(permission.sidecarKind)} (${singleLine(permission.id, "sidecar")}` +
        `${permission.platforms.length > 0 ? `, for ${permission.platforms.join(", ")}` : ""})`
      );
    case "buildDependency":
      return (
        `Download binaries while building your game (${singleLine(permission.id, "dependency")}` +
    ...
    case "externalLink":
      ...
      // readable.
      return (
        "In your game: send the player to " +
    ...
    case "network":
      ...
      // answer.
      return (
        "In your game: request data from " +
        (permission.patterns.length > 0
          ? permission.patterns.map((pattern) => singleLine(pattern, "declared address")).join(", ")
          : "declared addresses")
      );
...
function covers(granted: PluginInstallPermission, requested: PluginInstallPermission): boolean {
  ...
  if (granted.kind === "filesystem" && requested.kind === "filesystem") {
    return coversFileSystemMode(granted.mode, requested.mode) && coversPath(granted, requested);
  }
  ...
  if (granted.kind === "buildDependency" && requested.kind === "buildDependency") {
    return (
      granted.id === requested.id && requested.hosts.every((host) => granted.hosts.includes(host))
    );
  }
