...
describe("resolveGameRuntimeInitialBackgroundColor without brand links", () => {
    ...
    it("defaults app surfaces to white and stage surfaces to black", () => {
        ...
        expect(resolveGameRuntimeInitialBackgroundColor(packWith({
    ...
    it("takes a configured literal, in every spelling the shell accepts", () => {
        ...
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("rgb(64, 168, 196)"))).toBe("#40a8c4");
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("rebeccapurple"))).toBe("rebeccapurple");
    });

    it("falls back to black for a transparent or unreadable literal", () => {
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("transparent"))).toBe("#000000");
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("rgba(255, 255, 255, 0)"))).toBe("#000000");
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("color-mix(in srgb, red, blue)")))
            .toBe("#000000");
    });
});
...
describe("resolveGameRuntimeInitialBackgroundColor with brand links", () => {
    ...
    it("resolves a link to the palette's literal", () => {
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:background", brand)))
            .toBe("#f7f9fb");
    });

    it("follows a chain of links to the literal at the end", () => {
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:ring", brand)))
            .toBe("#f7f9fb");
    });
    ...
    it("keeps the RGB channels of a translucent link and drops a fully transparent one", () => {
        ...
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:primary/0.5", brand)))
            .toBe("#40a8c4");
        // ...but none of it is the surface asking for no background at all, which has no opaque form.
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:primary/0", brand)))
            .toBe("#000000");
    });
    ...
    it("resolves a stored alpha segment against a translucent entry by replacing it", () => {
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:translucent/0.8", brand)))
            .toBe("#40a8c4");
        ...
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:translucent/0", brand)))
            .toBe("#000000");
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:translucent", brand)))
            .toBe("#40a8c4");
    });
    ...
    it("reads the seeded palette when the pack predates the brand field", () => {
        ...
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:background")))
            .toBe("#101317");
    });
    ...
    it("keeps the original fallback for a link that resolves to nothing", () => {
        ...
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:missing", brand)))
            .toBe("#000000");
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:orphan", brand)))
            .toBe("#000000");
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:loop", brand)))
            .toBe("#000000");
    });
    ...
    it("leaves a malformed link alone rather than repairing it", () => {
        ...
        expect(resolveGameRuntimeInitialBackgroundColor(packWithBackground("nlbrand:primary/5", brand)))
            .toBe("#000000");
    });
});
