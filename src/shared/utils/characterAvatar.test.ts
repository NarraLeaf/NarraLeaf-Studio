...
describe("characterAvatarKeyFromTags", () => {
  ...
  it("agrees with the key the baker enumerated", () => {
    expect(characterAvatarKeys(layered)).toContain(
      characterAvatarKeyFromTags(layered, ["sad", "coat"])
    );
  });
...
describe("characterAvatarKeys", () => {
  ...
  it("enumerates the cartesian product of the avatar axes", () => {
    expect(characterAvatarKeys(layered).sort()).toEqual([
      "coat+happy",
      "coat+sad",
      "happy+shirt",
      "sad+shirt"
...
describe("characterAvatarAssetId", () => {
  it("round-trips ids that contain the separator", () => {
    ...
    expect(parseCharacterAvatarAssetId(id)).toEqual({
      characterId: "char:with:colons",
      key: "coat+sad"
    });
  });
...
describe("resolveCharacterAvatarAssetId", () => {
  ...
  it("prefers the author's override over the bake", () => {
    const withBoth = {
      ...
      appearance: {
        ...preset,
        avatars: { p2: { baked: true, overrideAssetId: "asset-hand-drawn" } }
  ...
  it("addresses the bake by its synthetic id", () => {
    ...
    expect(resolveCharacterAvatarAssetId(withBake, "p2")).toBe(
      characterAvatarAssetId("alice", "p2")
    );
  });
