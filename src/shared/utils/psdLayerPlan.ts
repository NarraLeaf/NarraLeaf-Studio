...
export function planImport(
  ...
  for (const leaf of kept) {
    if (leaf.clipping) {
      ...
      const target =
        base && keptPaths.has(joinPath(base.path)) ? host.get(joinPath(base.path)) : null;
      if (target) {
  ...
  for (const leaf of bases) {
    if (leaf.group && axisNames.has(leaf.group)) {
      ...
      slots.push({
        kind: "switch",
        axis: leaf.group,
        options: (byGroup.get(leaf.group) ?? []).map((member) => ({
          tag: member.name,
          leaf: member
        }))
      });
...
export function toBakeTargets(plan: ImportPlan): PsdBakeTarget[] {
  const attachmentsFor = (leaf: PsdLeaf): PsdBakeTarget => {
    ...
    return attached.length === 0
      ...
      : {
          path: leaf.path,
          mergeFrom: attached.map((entry) => ({ path: entry.leaf.path, clip: entry.clip }))
        };
  };
