...
describe("entryFileName", () => {
  ...
  it("round-trips identically under the renderer path polyfill", () => {
    ...
    for (const fileName of FILE_NAMES) {
      expect(entryFileName({ name: parse(fileName).name, ext: extname(fileName) || null })).toBe(
        fileName
      );
    }
  });
...
describe("splitFileEntry", () => {
  ...
  it("keeps `name` the stem (extension stripped), not the whole filename", () => {
    ...
    expect(splitFileEntry("archive.tar.gz")).toEqual({
      name: "archive.tar",
      ext: ".gz",
      fileName: "archive.tar.gz"
    });
    expect(splitFileEntry(".gitignore")).toEqual({
      name: ".gitignore",
      ext: null,
      fileName: ".gitignore"
    });
    expect(splitFileEntry("README")).toEqual({ name: "README", ext: null, fileName: "README" });
    expect(splitFileEntry("trailing.")).toEqual({
      name: "trailing",
      ext: ".",
      fileName: "trailing."
    });
