...
describe("classifyMediaSupport / accept", () => {
  ...
  it("takes H.264 + AAC in MP4", () => {
    expect(classifyMediaSupport(report(ISOBMFF, [video("h264"), audio("aac", 1)])).tier).toBe(
      "accept"
    );
  });
...
describe("classifyMediaSupport / remux", () => {
  ...
  it("sends VP8 + Vorbis in AVI to WebM", () => {
    expect(classifyMediaSupport(report("avi", [video("vp8"), audio("vorbis", 1)])).target).toEqual({
      kind: "remux",
      container: "webm",
      audioOnly: false
    });
  ...
  it("sends PCM in AIFF to WAV rather than wrapping it in WebM", () => {
    expect(classifyMediaSupport(report("aiff", [audio("pcm_s16le")])).target).toEqual({
      kind: "remux",
      container: "wav",
      audioOnly: true
    });
...
describe("classifyMediaSupport / reencode", () => {
  ...
  it("covers every audio codec measured as undecodable", () => {
    for (const codec of ["ac3", "alac", "wmav2"]) {
      ...
      expect(verdict.target, codec).toEqual({
        kind: "reencode",
        container: "mp4",
        video: null,
        audio: "aac"
  ...
  it("sends an audio-only file to AAC in MP4 and one with video to VP9 in WebM", () => {
    ...
    expect(classifyMediaSupport(report(MATROSKA, [audio("ac3")])).target).toEqual({
      kind: "reencode",
      container: "mp4",
      video: null,
      audio: "aac"
    });
    expect(classifyMediaSupport(report(MATROSKA, [video("hevc"), audio("ac3", 1)])).target).toEqual(
  ...
  it("names both encoders when the file has video and audio", () => {
    expect(classifyMediaSupport(report(ISOBMFF, [video("hevc"), audio("aac", 1)])).target).toEqual({
      kind: "reencode",
      container: "webm",
      video: "vp9",
      audio: "vorbis"
...
describe("containerNames", () => {
  ...
  it("still recognises ISO-BMFF when FFmpeg adds or drops an alias", () => {
    ...
    expect(classifyMediaSupport(report("mov,mp4,m4a,3gp,3g2", [video("h264")])).tier).toBe(
      "accept"
    );
...
describe("remuxContainerFor", () => {
  it("prefers a single audio codec's native container", () => {
    expect(remuxContainerFor([{ index: 0, kind: "audio", codec: "flac", decodable: true }])).toBe(
      "flac"
    );
    expect(remuxContainerFor([{ index: 0, kind: "audio", codec: "mp3", decodable: true }])).toBe(
      "mp3"
    );
    expect(remuxContainerFor([{ index: 0, kind: "audio", codec: "opus", decodable: true }])).toBe(
      "ogg"
    );
  });
...
describe("parseProbeOutput", () => {
  it("reads a real report", () => {
    const parsed = parseProbeOutput(
      JSON.stringify({
        format: { format_name: ISOBMFF, duration: "1.0" },
  ...
  it("drops junk entries instead of trusting the shape", () => {
    const parsed = parseProbeOutput(
      JSON.stringify({
        ...
        streams: [null, 7, { codec_type: "audio", codec_name: "aac" }]
      })
    );
    expect(parsed?.streams).toHaveLength(1);
...
describe("probeDurationUs", () => {
  ...
  it("reads the duration back out of a parsed report", () => {
    const parsed = parseProbeOutput(
      JSON.stringify({ format: { format_name: "matroska,webm", duration: "2.5" } })
    );
