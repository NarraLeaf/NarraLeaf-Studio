...
function wav(): Buffer {
    ...
    return concat(
        Buffer.from("RIFF"), u32le(0), Buffer.from("WAVE"),
        Buffer.from("fmt "), u32le(16),
        u16le(1), u16le(2), u32le(44100), u32le(byteRate), u16le(4), u16le(16),
        Buffer.from("data"), u32le(byteRate * 2),
    );
}
...
function oggVorbis(): Buffer {
    return concat(
        Buffer.from("OggS"), [0x00, 0x02],
        Buffer.alloc(8), u32le(1), u32le(0), u32le(0),
        [0x01, 30],
        [0x01], Buffer.from("vorbis"), u32le(0),
        [0x02], u32le(48000),
        Buffer.alloc(8),
    );
}
...
describe("Ogg", () => {
    ...
    it("reports Opus at the rate it decodes to, not the rate it was encoded from", () => {
        const opus = concat(
            ...
            Buffer.alloc(8), u32le(1), u32le(0), u32le(0),
            [0x01, 19],
            Buffer.from("OpusHead"), [0x01, 0x01], u16le(312), u32le(16000),
            Buffer.alloc(8),
        );
...
function mp3(withXing: boolean): Buffer {
    ...
    return concat(frame, Buffer.alloc(32), Buffer.from("Xing"), u32be(0x01), u32be(1000), Buffer.alloc(16));
}
...
describe("MPEG audio", () => {
    ...
    it("skips an ID3 tag to find the first frame", () => {
        const tagged = concat(
            Buffer.from("ID3"), [0x04, 0x00, 0x00],
    ...
    it("refuses the reserved version and layer encodings rather than guessing", () => {
        expect(readMediaHeader(concat([0xff, 0xf3, 0x90, 0x00], Buffer.alloc(64)))?.container)
            .toBe("mp3");
...
describe("ISO base media", () => {
    ...
    it("names the container and nothing else when moov is not in the prefix", () => {
        ...
        const noIndex = concat(box("ftyp", Buffer.from("isom\0\0\0\0isom")), box("mdat", Buffer.alloc(256)));
    ...
    it("treats a sound track's zero size as no size", () => {
        ...
        expect(readMediaHeader(mp4(1000, 3000, 0, 0))).toEqual({ container: "iso-bmff", durationMs: 3000 });
    });
});
...
function webm(durationTicks: number, width: number, height: number): Buffer {
    ...
    const info = ebml([0x15, 0x49, 0xa9, 0x66], concat(
        ebml([0x2a, 0xd7, 0xb1], concat([0x0f, 0x42, 0x40])),
        ebml([0x44, 0x89], duration),
    ));
    const tracks = ebml([0x16, 0x54, 0xae, 0x6b], ebml([0xae], ebml([0xe0], concat(
        ebml([0xb0], u16be(width)),
        ebml([0xba], u16be(height)),
    ))));
    return concat(
...
function font(family: string): Buffer {
    ...
    const nameTable = concat(
        u16be(0), u16be(1), u16be(18),
        u16be(3), u16be(1), u16be(0x0409), u16be(1), u16be(text.length), u16be(0),
        text,
    );
    return concat(
        u32be(0x00010000), u16be(1), u16be(16), u16be(0), u16be(0),
        Buffer.from("name"), u32be(0), u32be(28), u32be(nameTable.length),
        nameTable,
    );
}
...
describe("fonts", () => {
    ...
    it("never inflates a compressed font to read a string", () => {
        expect(readFontHeader(concat(Buffer.from("wOFF"), Buffer.alloc(60)))).toEqual({ container: "woff" });
        expect(readFontHeader(concat(Buffer.from("wOF2"), Buffer.alloc(60)))).toEqual({ container: "woff2" });
    });
