 ...
 *
 * Put the two halves back together with {@link entryFileName} before building a path. This module
 * deliberately has no imports: `FileStat` is declared in `@shared/utils/fs`, which pulls in
 * `fs/promises` and `mime-types`, and the renderer only ever imports that module for its types.
 */
