...
import {
  ...
  inferStoryExpressionType,
  storyExprTypeFits
} from "./storyExpressionEval";
import {
  createStoryExpressionScope,
  formatStoryExpr,
  formatStoryExpressionName,
  parseStoryExpression
} from "./storyExpressionParser";
...
const VARIABLES = [
  ...
  {
    name: "chapter",
    ref: { scope: "persistent", variableId: "chapter_global" } as StoryVariableRef
  },
...
describe("parseStoryExpression", () => {
  ...
  it("accepts non-ASCII identifiers", () => {
    ...
    expect(evaluateStoryExpression(expression.ast, { read: (ref) => VALUES[keyOf(ref)] })).toBe(
      101
    );
  });
});
...
describe("collection functions", () => {
  ...
  it("rewrites without touching the value it was given", () => {
    ...
    for (const source of [
      'setKey(flags, "ch3", true)',
      'removeKey(flags, "ch1")',
      'removeKey(flags, "nope")'
    ]) {
...
describe("failure handling", () => {
  it("never throws, and marks an unparseable tree as non-evaluable", () => {
    for (const source of [
      ...
      expect(isStoryExpressionEvaluable(expression.ast)).toBe(false);
      expect(() =>
        evaluateStoryExpression(expression.ast, { read: () => undefined })
      ).not.toThrow();
    }
  });
  ...
  it("holds the invariant the compiler relies on: any issue means a non-evaluable tree", () => {
    ...
    for (const source of sources) {
      ...
      expect(issues.length, `expected an issue for \`${source}\``).toBeGreaterThan(0);
      expect(
        isStoryExpressionEvaluable(expression.ast),
        `\`${source}\` parsed to an evaluable tree`
      ).toBe(false);
    }
  });
...
describe("visited / picked / invoke", () => {
  const ENTITIES = {
    scenes: [
      { id: "sc_prologue", name: "序章" },
      { id: "sc_two", name: "第 二 章" }
    ],
    options: [
    ...
    blueprints: [
      { id: "bp_bonus", name: "bonus" },
      { id: "bp_default", name: "Story Value" },
      { id: "bp_min", name: "min" }
    ]
  };
  ...
  it("stores the id, never the name", () => {
    ...
    expect(picked.expression.ast).toEqual({
      kind: "visited",
      target: { kind: "option", blockId: "opt_refuse" },
      name: "那句拒绝"
    });
  });
  ...
  it("keeps hitting the same id after the scene is renamed and the option reworded", () => {
    ...
    expect(parseStoryExpression("visited(序章)", renamed).issues.map((i) => i.code)).toEqual([
      "unknownVisitedTarget"
    ]);
  ...
  it("takes a quoted entity name, so a name with spaces is addressable", () => {
    ...
    expect(expression.ast).toEqual({
      kind: "visited",
      target: { kind: "scene", sceneId: "sc_two" },
      name: "第 二 章"
    });
  });
  ...
  it("calls a blueprint whose name is quoted, including one shadowing a built-in", () => {
    expect(parse("'Story Value'()").expression.ast).toEqual({
      kind: "invoke",
      blueprintId: "bp_default",
      name: "Story Value"
    });
    // The escape from the collision above, without renaming anything.
    expect(parse("'min'()").expression.ast).toEqual({
      kind: "invoke",
      blueprintId: "bp_min",
      name: "min"
    });
  ...
  it("composes into a larger expression", () => {
    ...
    expect(
      evaluateStoryExpression(expression.ast, { read: () => undefined, invoke: () => 21 })
    ).toBe(42);
  });
  ...
  it("stays total when the blueprint throws", () => {
    ...
    expect(() =>
      evaluateStoryExpression(expression.ast, { read: () => undefined, invoke: explode })
    ).not.toThrow();
    expect(
      evaluateStoryExpression(expression.ast, { read: () => undefined, invoke: explode })
    ).toBe(1);
    ...
    expect(
      evaluateStoryExpression(parse("bonus()").expression.ast, { read: () => undefined })
    ).toBe(null);
  });

  it("infers boolean for a record read and unknown for a call", () => {
    const infer = (source: string) =>
      inferStoryExpressionType(parse(source).expression.ast, (ref) => TYPES[keyOf(ref)]);
  ...
  it("collects the blueprints a tree calls, and no variables", () => {
    ...
    expect(collectStoryExpressionInvocations(expression.ast)).toEqual([
      { blueprintId: "bp_bonus", name: "bonus" }
    ]);
  ...
  it("prints back to source that re-parses to the same tree", () => {
    ...
    for (const source of sources) {
      ...
      expect(parseStoryExpression(printed, WORLD).expression.ast, source).toEqual(
...
describe("AppTag", () => {
  ...
  it("reads as the build-variant constant, whatever its case", () => {
    for (const source of ["AppTag", "apptag", "APPTAG"]) {
      expect(parseWith(source).expression.ast, source).toEqual({
        kind: "call",
        fn: "appTag",
        args: []
      });
  ...
  it("wins the bare name from a variable that shares it", () => {
    ...
    expect(parseStoryExpression("AppTag", scope).expression.ast).toEqual({
      kind: "call",
      fn: "appTag",
      args: []
    });
    // Quoting is how that variable is still reachable - and how the printer spells it back.
    expect(parseStoryExpression("'AppTag'", scope).expression.ast).toMatchObject({
      kind: "var",
      name: "AppTag"
    });
  ...
  it("is a string, so a comparison is a condition and an assignment fits a string", () => {
    const infer = (source: string) =>
      inferStoryExpressionType(parseWith(source).expression.ast, () => undefined);
  ...
  it("reports a name no variant has, and only where the caller can enumerate them", () => {
    ...
    expect(parseWith('AppTag == "demo"', tags).issues).toEqual([
      { code: "unknownAppTagName", span: { start: 0, end: 16 }, name: "demo" }
    ]);
