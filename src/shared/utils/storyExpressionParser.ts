...
const FUNCTION_ARITY: Readonly<
  Record<StoryExprFunction, { min: number; max: number; label: string }>
> = {
  ...
  while (index < source.length) {
    ...
    if (operator) {
      tokens.push({
        type: "operator",
        text: operator,
        span: { start: index, end: index + operator.length }
      });
...
class ExpressionParser {
  ...
  private parseVisitedCall(token: Token, call: StoryVisitedCall): StoryExpr {
    ...
    if (resolved === "ambiguous") {
      ...
      return {
        kind: "invalid",
        source: this.source.slice(token.span.start, this.tokens[this.position - 1].span.end)
      };
    }
    if (!resolved) {
 ...
 */
export function parseStoryExpression(
  source: string,
  scope: StoryExpressionScope
): StoryExpressionParse {
...
export function formatStoryExpressionName(name: string): string {
  ...
  const bare =
    name.length > 0 && !keyword && isIdentifierStart(name[0]) && [...name].every(isIdentifierPart);
...
export function createStoryExpressionScope(
  ...
  const byName = (
    list: readonly StoryExpressionNamedEntry[] | undefined,
    name: string
  ): StoryExpressionNameResolution => {
  ...
  return {
    ...
    lookupIn: (scope, name) => {
      const needle = normalize(name);
      return (
        entries.find((entry) => entry.ref.scope === scope && normalize(entry.name) === needle)
          ?.ref ?? null
      );
    },
    lookupVisited: (call, name) =>
      byName(call === "visited" ? entities.scenes : entities.options, name),
