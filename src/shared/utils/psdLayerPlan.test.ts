...
describe("toBakeTargets", () => {
  ...
  it("keeps a merged layer that has nothing underneath as a layer of its own", () => {
    ...
    expect(toBakeTargets(planImport(leaves, { Shade: "merge" }))).toEqual([
      { path: ["Shade"] },
      { path: ["Body"] }
    ]);
  });
...
describe("clipping masks", () => {
  ...
  it("folds a clipped layer into its base instead of giving it a slot", () => {
    ...
    expect(plan.slots.map((slot) => (slot.kind === "constant" ? slot.name : slot.axis))).toEqual([
      "Body",
      "Hat"
    ]);
  ...
  it("drops a clipped layer whose base is not imported rather than spreading it", () => {
    ...
    expect(plan.dropped.map((entry) => `${entry.leaf.name}:${entry.reason}`)).toEqual([
      "Body:hidden",
      "Blush:clip-base-dropped"
    ]);
    expect(plan.slots.map((slot) => (slot.kind === "constant" ? slot.name : slot.axis))).toEqual([
      "Hat"
    ]);
    ...
    expect(toBakeTargets(plan)).toEqual([
      {
        path: ["Body"],
        mergeFrom: [
          { path: ["Shade"], clip: false },
          { path: ["Blush"], clip: true }
        ]
      }
    ]);
  });
...
describe("planImport slots", () => {
  ...
  it("lists every tag of an axis with the leaf it comes from", () => {
    ...
    expect(outfit?.kind === "switch" && outfit.options.map((option) => option.tag)).toEqual([
      "Uniform",
      "Casual"
    ]);
  });
...
describe("unsupportedBlends and hidden layers", () => {
  it("does not ask about a layer that is being dropped anyway", () => {
    const leaves = flattenLeaves([
      leaf("Scratch", ["Scratch"], { hidden: true, blendMode: "multiply" })
    ]);
...
describe("merging onto a group", () => {
  ...
  it("still lands once when the group below is not an axis", () => {
    ...
    expect(toBakeTargets(planImport(flattenLeaves(single), { Shade: "merge" }))).toEqual([
      { path: ["Hat", "Beret"], mergeFrom: [{ path: ["Shade"], clip: false }] }
    ]);
  });
...
describe("estimateImportCost", () => {
  ...
  it("flags a sheet that is heavy by layer count or by memory", () => {
    ...
    const many = Array.from({ length: IMPORT_LAYER_WARNING + 1 }, (_, i) =>
      leaf(`L${i}`, [`L${i}`])
    );
    expect(
      estimateImportCost(planImport(flattenLeaves(many), {}), { width: 8, height: 8 }).heavy
    ).toBe(true);
  });
});
