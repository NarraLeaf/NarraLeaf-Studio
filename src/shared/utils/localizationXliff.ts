...
export function serializeTranslationXliff(document: TranslationExchangeDocument): string {
  ...
  for (const row of document.rows) {
    ...
    lines.push(
      `      <trans-unit${xmlAttributes({ id: row.unitId, resname: row.unitId, "xml:space": "preserve" })}>`
    );
    ...
    if (row.context) {
      lines.push(
        `        <note${xmlAttributes({ from: "developer", annotates: "source" })}>${encodeSegment(row.context)}</note>`
      );
    }
    if (row.note) {
      lines.push(
        `        <note${xmlAttributes({ from: "translator" })}>${encodeSegment(row.note)}</note>`
      );
    }
