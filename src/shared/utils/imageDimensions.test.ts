...
function png(width: number, height: number, { leadingChunk = false } = {}): Uint8Array {
    ...
    chunks.push(0, 0, 0, 13, ...ascii("IHDR"), ...be32(width), ...be32(height), 8, 6, 0, 0, 0, 0, 0, 0, 0);
...
describe("readImageDimensions", () => {
    ...
    it("reads PNG dimensions past a chunk that precedes IHDR", () => {
        expect(readImageDimensions(png(256, 128, { leadingChunk: true })))
            .toEqual({ format: "png", width: 256, height: 128 });
    });

    it("reads JPEG dimensions from the start-of-frame marker", () => {
        expect(readImageDimensions(jpeg(300, 200))).toEqual({ format: "jpeg", width: 300, height: 200 });
    });

    it("reads lossy WebP dimensions", () => {
        const payload = [0, 0, 0, 0x9d, 0x01, 0x2a, 0x00, 0x02, 0x00, 0x02];
        expect(readImageDimensions(webp("VP8 ", payload))).toEqual({ format: "webp", width: 512, height: 512 });
    });
    ...
    it("reads lossless WebP dimensions", () => {
        ...
        expect(readImageDimensions(webp("VP8L", [0x2f, ...le32(bits >>> 0)])))
            .toEqual({ format: "webp", width: 512, height: 256 });
    });

    it("reads extended WebP canvas dimensions", () => {
        const payload = [0, 0, 0, 0, 0xff, 0x01, 0x00, 0xff, 0x01, 0x00];
        expect(readImageDimensions(webp("VP8X", payload))).toEqual({ format: "webp", width: 512, height: 512 });
    });

    it("returns null for bytes that are not a readable image", () => {
        expect(readImageDimensions(Uint8Array.from(ascii("<svg xmlns=\"http://www.w3.org/2000/svg\" />")))).toBeNull();
