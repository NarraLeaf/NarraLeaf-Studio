...
const HIYORI = [
    ...
    ...Array.from({ length: 10 }, (_, i) => `motions/Hiyori_m${String(i + 1).padStart(2, "0")}.motion3.json`),
];
...
describe("detectModelBundleEntry", () => {
    ...
    it("falls back to the Spine JSON skeleton when no .skel was exported", () => {
        const detection = detectModelBundleEntry(["spineboy.atlas", "spineboy.png", "spineboy-pro.json"]);
...
describe("sortBundlePaths / bundleListingFingerprint", () => {
    ...
    it("is stable across enumeration order but moves when the file set changes", () => {
        ...
        expect(bundleListingFingerprint([...HIYORI, "extra.png"])).not.toBe(bundleListingFingerprint(HIYORI));
