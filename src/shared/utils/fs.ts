...
export class Fs {
  ...
  public static ensureRegularFile(
    path: string,
    data: string,
    encoding: BufferEncoding = "utf-8"
  ): Promise<FsRequestResult<void>> {
   ...
   */
  public static writeFileNoFollow(
    path: string,
    data: string,
    encoding: BufferEncoding = "utf-8"
  ): Promise<FsRequestResult<void>> {
  ...
  public static recoverCorruptedJsonFile(
    path: string,
    replacement: string,
    encoding: BufferEncoding = "utf-8"
  ): Promise<FsRequestResult<void>> {
  ...
  public static isWritableDir(dirPath: string): Promise<FsRequestResult<boolean>> {
    return (async () => {
      try {
        ...
        return { ok: false, error: this.createError(error) } satisfies FsRequestResult<
          boolean,
          false
        >;
      }
  ...
  public static getFiles(dir: string, ext?: string | string[]): Promise<FsRequestResult<string[]>> {
    return this.wrap(
      fs.readdir(dir, { withFileTypes: true }).then((files) => {
        ...
        return files
          .filter(
            (file) => file.isFile() && (extSet.size === 0 || extSet.has(path.extname(file.name)))
          )
  ...
  private static async accumulateDirectorySize(
    dir: string,
   ...
   */
  private static async writeFileAtomicCore(
    targetPath: string,
    data: Buffer,
  ...
  private static async createTempSibling(
    targetPath: string
  ): Promise<{ handle: Awaited<ReturnType<typeof fs.open>>; tempPath: string }> {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      const tempPath = path.join(
        directory,
        `.${baseName}.${randomBytes(8).toString("hex")}${ATOMIC_WRITE_TEMP_SUFFIX}`
      );
      try {
  ...
  private static async replaceOpenFileContents(
  ...
  private static async writeNewBackup(src: string, data: Buffer): Promise<void> {
    for (let attempt = 0; attempt < 10; attempt++) {
      const backupPath =
        attempt === 0 ? `${src}.bak` : `${src}.bak.${Date.now()}.${process.pid}.${attempt}`;
