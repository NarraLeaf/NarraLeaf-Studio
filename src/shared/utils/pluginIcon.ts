...
export function inspectPluginIconBytes(
  ...
  if (!probe) {
    return {
      error: `Plugin icon is not a readable ${PLUGIN_ICON_EXTENSIONS.join("/").toUpperCase()} image`
    };
  }
  ...
  if (probe.width > PLUGIN_ICON_MAX_DIMENSION) {
    return {
      error: `Plugin icon must be at most ${PLUGIN_ICON_MAX_DIMENSION}x${PLUGIN_ICON_MAX_DIMENSION} (got ${probe.width}x${probe.height})`
    };
  }
  if (probe.width < PLUGIN_ICON_MIN_DIMENSION) {
    return {
      error: `Plugin icon must be at least ${PLUGIN_ICON_MIN_DIMENSION}x${PLUGIN_ICON_MIN_DIMENSION} (got ${probe.width}x${probe.height})`
    };
  }
