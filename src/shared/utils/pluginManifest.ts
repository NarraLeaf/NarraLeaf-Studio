...
export function validatePluginManifest(value: unknown): PluginManifestValidationResult {
  ...
  const declared = value.permissions === undefined ? [] : validatePermissions(value.permissions);
...
function validateContributes(
  value: unknown,
  pluginId: string
): Required<PluginContributes> | string {
  ...
  const unknownKeys = Object.keys(value).filter(
    (key) => !CONTRIBUTES_KEYS.includes(key as (typeof CONTRIBUTES_KEYS)[number])
  );
...
function validateExternalLinks(value: unknown, pluginId: string): string[] | string {
  ...
  for (const item of value) {
    ...
    if (!key) {
      return (
        `Plugin contributes.externalLinks entry is not an address pattern: ${pattern}. ` +
...
function validateNetworkPatterns(value: unknown, pluginId: string): string[] | string {
  ...
  for (const item of value) {
    ...
    if (!key) {
      return (
        `Plugin contributes.network entry is not an address pattern: ${pattern}. ` +
    ...
    if (parsed.pathname === "/" && !parsed.search && !parsed.hash) {
      return (
        `Plugin contributes.network entry "${pattern}" names only the path "/". ` +
...
function validateBuildConfig(
  ...
  for (const item of value) {
    ...
    if (!PLUGIN_BUILD_CONFIG_TYPES.includes(type as PluginBuildConfigType)) {
      return (
        `Plugin contributes.buildConfig["${key}"] type must be one of: ` +
    ...
    if (!PLUGIN_BUILD_CONFIG_SCOPES.includes(scope as PluginBuildConfigScope)) {
      return (
        `Plugin contributes.buildConfig["${key}"] scope must be one of: ` +
...
function validateBuildConfigPlatforms(
  ...
  for (const item of value) {
    ...
    if (!BUILD_CONFIG_PLATFORMS.includes(platform as GameBuildPlatform)) {
      return (
        `Plugin contributes.buildConfig["${key}"] names an unknown platform: ${String(item)} ` +
...
function validateRuntimeCapabilities(value: unknown): PluginRuntimeCapability[] | string {
  ...
  for (const item of value) {
    ...
    if (!PLUGIN_RUNTIME_CAPABILITIES.includes(capability as PluginRuntimeCapability)) {
      return (
        `Unknown plugin runtime capability: ${capability} ` +
...
function validateSidecars(
  ...
  for (const item of value) {
    ...
    const startupTimeoutMs = readPositiveInt(
      ...
      SIDECAR_DEFAULTS.startupTimeoutMs
    );
    ...
    const shutdownTimeoutMs = readPositiveInt(
      item.shutdownTimeoutMs,
      SIDECAR_DEFAULTS.shutdownTimeoutMs
    );
...
function validateSidecarTargets(
  ...
  for (const [platformKey, rawTarget] of Object.entries(value)) {
    if (!isPluginBinaryPlatformKey(platformKey)) {
      return (
        `Plugin sidecar "${sidecarId}" has an unsupported platform key: ${platformKey} ` +
...
function validateLocaleContributions(value: unknown): PluginLocaleContribution[] | string {
  ...
  for (const item of value) {
    ...
    const nativeName =
      typeof item.nativeName === "string" && item.nativeName.trim()
...
function validateEntries(value: unknown): PluginManifestEntries | string {
  ...
  const unknownKeys = Object.keys(value).filter(
    (key) => !PLUGIN_ENTRY_TARGETS.includes(key as PluginEntryTarget)
  );
  if (unknownKeys.length > 0) {
