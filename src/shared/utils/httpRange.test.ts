...
describe("resolveSingleByteRange", () => {
  ...
  it("resolves bounded, open-ended, and suffix ranges", () => {
    expect(resolveSingleByteRange("bytes=0-99", 1000)).toEqual({
      kind: "partial",
      start: 0,
      end: 99
    });
    expect(resolveSingleByteRange("bytes=200-", 1000)).toEqual({
      kind: "partial",
      start: 200,
      end: 999
    });
    expect(resolveSingleByteRange("bytes=-100", 1000)).toEqual({
      kind: "partial",
      start: 900,
      end: 999
    });
    expect(resolveSingleByteRange("bytes=-2000", 1000)).toEqual({
      kind: "partial",
      start: 0,
      end: 999
    });
  ...
  it("clamps end positions past the payload", () => {
    expect(resolveSingleByteRange("bytes=990-2000", 1000)).toEqual({
      kind: "partial",
      start: 990,
      end: 999
    });
