...
function readMatroska(bytes: Uint8Array): MediaHeader | null {
  ...
  walkEbml(bytes, 0, bytes.length, (id, start, end) => {
    switch (id) {
      case 0x2ad7b1:
        timecodeScale = ebmlUint(bytes, start, end) || timecodeScale;
        break;
      case 0x4489:
        scaledDuration = ebmlFloat(bytes, start, end);
        break;
      case 0xb0:
        found.width = ebmlUint(bytes, start, end) || found.width;
        break;
      case 0xba:
        found.height = ebmlUint(bytes, start, end) || found.height;
        break;
      case 0x9f:
        found.channels = ebmlUint(bytes, start, end) || found.channels;
        break;
      case 0xb5: {
...
function readMp3(bytes: Uint8Array, totalSize?: number): MediaHeader | null {
  ...
  const durationMs =
    xingDuration(bytes, offset, version, mode, perFrame, sampleRate) ??
...
export function readFontHeader(bytes: Uint8Array): FontHeader | null {
  ...
  if (bytes.length >= 4 && view(bytes).getUint32(0) === 0x00020001)
    return { container: "embedded-opentype" };
...
function readSfntFamily(bytes: Uint8Array, offsetTable: number): string | undefined {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const text = platform === 1 ? ascii(bytes, at, length) : utf16be(bytes, at, length);
    if (!text) {
      continue;
    }
