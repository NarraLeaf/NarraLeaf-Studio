...
describe("parseSettingsDocument", () => {
  ...
  it("refuses a version it does not read", () => {
    expect(() =>
      parseSettingsDocument(JSON.stringify({ formatVersion: 99, settings: {} }))
    ).toThrow(/version 99/);
  });

  it("refuses a document with no settings object", () => {
    expect(() => parseSettingsDocument(JSON.stringify({ formatVersion: 1 }))).toThrow(
      SettingsDocumentError
    );
    expect(() => parseSettingsDocument(JSON.stringify({ formatVersion: 1, settings: [] }))).toThrow(
      SettingsDocumentError
    );
  });
...
describe("validateSettingValue", () => {
  it("checks booleans, numbers and their bounds", () => {
    ...
    expect(validateSettingValue({ key: "x", kind: "boolean" }, "true")).toBe(
      "expected true or false"
    );
    expect(validateSettingValue({ key: "x", kind: "number", min: 1, max: 5 }, 3)).toBeNull();
    expect(validateSettingValue({ key: "x", kind: "number", min: 1, max: 5 }, 0)).toContain(
      "minimum"
    );
    expect(validateSettingValue({ key: "x", kind: "number", min: 1, max: 5 }, 9)).toContain(
      "maximum"
    );
    expect(validateSettingValue({ key: "x", kind: "number" }, Number.NaN)).toBe(
      "expected a number"
    );
  });
...
describe("planSettingsImport", () => {
  ...
  it("reports an out-of-range value with its reason, and does not apply it", () => {
    const plan = planSettingsImport(doc({ "ui.zoomPercent": 9000 }), SPECS, {
      "ui.zoomPercent": 100
    });
  ...
  it("compares structurally, so an equal array is not a change", () => {
    ...
    const plan = planSettingsImport(doc({ "network.downloadRewrites": rules }), SPECS, {
