 ...
 */
export type StoryExpressionInvoker = (
  blueprintId: string,
  name: string
...
export function evaluateStoryExpression(
  expr: StoryExpr,
  env: StoryExpressionEnv
): StoryLiteralValue {
  switch (expr.kind) {
    case "literal":
      return expr.value;
...
function evaluateInvoke(
  expr: Extract<StoryExpr, { kind: "invoke" }>,
  env: StoryExpressionEnv
): StoryLiteralValue {
  if (!env.invoke) {
    return null;
...
function evaluateBinary(
  expr: Extract<StoryExpr, { kind: "binary" }>,
  env: StoryExpressionEnv
): StoryLiteralValue {
  ...
  if (expr.op === "&&") {
    return (
      isTruthy(evaluateStoryExpression(expr.left, env)) &&
...
function evaluateCall(
  expr: Extract<StoryExpr, { kind: "call" }>,
  env: StoryExpressionEnv
): StoryLiteralValue {
 ...
 */
function elementAt(
  collection: StoryLiteralValue,
  key: StoryLiteralValue,
  fallback: StoryLiteralValue
): StoryLiteralValue {
 ...
 */
export function storyExprTypeFits(
  inferred: StoryExprType,
  declared: StoryVariableValueType
): boolean {
