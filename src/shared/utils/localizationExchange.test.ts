...
describe.each(TRANSLATION_EXCHANGE_FORMATS)("%s exchange round-trip", (format) => {
  ...
  it("is detected from its own file name", () => {
    const text = serializeTranslationExchange(format, document(AWKWARD_ROWS));
    expect(
      detectTranslationExchangeFormat(`zh-CN.${format === "xliff" ? "xlf" : format}`, text)
    ).toBe(format);
  });
});
...
describe("exchange envelope", () => {
  ...
  it("carries the language tags in every format that has a place for them", () => {
    for (const format of TRANSLATION_EXCHANGE_FORMATS) {
      const parsed = parseTranslationExchange(
        format,
        serializeTranslationExchange(format, document(AWKWARD_ROWS))
      );
      if (format === "csv") {
...
describe("format detection", () => {
  ...
  it("falls back to the content when the extension says nothing", () => {
    ...
    expect(detectTranslationExchangeFormat("hand-back.txt", '<?xml version="1.0"?><xliff/>')).toBe(
      "xliff"
    );
...
describe("xliff specifics", () => {
  ...
  it("writes 1.2 with the unit id on both id and resname", () => {
    const text = serializeTranslationExchange(
      "xliff",
      document([row({ unitId: "t-1", source: "Hi", target: "嗨", status: "translated" })])
    );
...
<xliff version="1.2"><file source-language="en" target-language="ja"><body>
  ...
  <trans-unit id="e"><source>E</source><target state="new"></target></trans-unit>
    ...
    expect(parsed.rows.map((unit) => `${unit.unitId}:${unit.status}`)).toEqual([
      "a:reviewed",
      "b:machine",
      "c:machine",
      "d:translated",
...
<xliff xmlns="urn:oasis:names:tc:xliff:document:2.0" version="2.0" srcLang="en" trgLang="fr">
 <file id="f1">
  ...
  it("refuses a file that is not XLIFF, and says what it got", () => {
    expect(parse("<html><body>nope</body></html>").errors).toEqual([
      "Not an XLIFF file: the root element is <html>"
    ]);
    expect(parse("not xml at all").errors).toEqual(["Not a readable XML file"]);
    expect(parse('<xliff version="1.2"></xliff>').errors).toEqual([
      "This XLIFF file has no translation units"
    ]);
  });
});
...
describe("po specifics", () => {
  ...
  it("writes paragraphs in the multi-line form PO editors display", () => {
    const text = serializeTranslationExchange(
      "po",
      document([row({ unitId: "t-1", source: "one\ntwo", target: "" })])
    );
...
`);
    expect(parsed.rows).toEqual([
      row({ unitId: "t-1", source: "one file", target: "eine Datei", status: "translated" })
    ]);
  });
...
describe("json specifics", () => {
  ...
  it("reads the shape Studio wrote", () => {
    ...
    expect(JSON.parse(text)).toMatchObject({
      format: "narraleaf-translation",
      sourceLocale: "en",
      targetLocale: "zh-CN",
      project: "My Game"
    });
    expect(parse(text).rows).toEqual([
      row({ unitId: "t-1", source: "Hi", target: "嗨", status: "translated" })
    ]);
  });
  ...
  it("reads a list of rows under any of the usual id spellings", () => {
    const parsed = parse(
      '[{"unit_id": "t-1", "translation": "嗨"}, {"id": "t-2", "target": "再见"}]'
    );
    expect(parsed.rows).toEqual([
      row({ unitId: "t-1", target: "嗨" }),
      row({ unitId: "t-2", target: "再见" })
    ]);
  });
