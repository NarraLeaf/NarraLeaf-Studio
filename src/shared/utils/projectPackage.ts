...
const PROJECT_PACKAGE_MAGIC = new Uint8Array([0x4e, 0x4c, 0x53, 0x50, 0x4b, 0x47, 0x00, 0x01]);
...
export function decodeProjectPackage(bytes: Uint8Array): ProjectPackagePayload {
  ...
  const decoded = msgpack.decode(
    bytes.slice(PROJECT_PACKAGE_MAGIC.length)
  ) as Partial<ProjectPackagePayload>;
  ...
  return {
    ...
    projectName:
      typeof decoded.projectName === "string" && decoded.projectName.trim()
        ? decoded.projectName
        : "Imported Project",
    projectIdentifier:
      typeof decoded.projectIdentifier === "string" ? decoded.projectIdentifier : undefined,
    directories,
    files
  };
}
...
export function shouldExcludeProjectPackagePath(relativePath: string): boolean {
  ...
  if (
    fileName === ".ds_store" ||
    fileName === "thumbs.db" ||
    fileName.endsWith(PROJECT_PACKAGE_EXTENSION)
  ) {
    return true;
  }
