...
beforeEach(async () => {
    ...
    env = { platform: "win32", appDataDir: path.join(root, "AppData"), homeDir: path.join(root, "home") };
});
...
describe("resolveGameProgressDirectory", () => {
    it("sits under the roaming app data root on Windows", () => {
        expect(resolveGameProgressDirectory({ platform: "win32", appDataDir: "C:\\AD", homeDir: "C:\\home" }))
    ...
    it("honours an absolute XDG_DATA_HOME on Linux and ignores a relative one", () => {
        ...
        expect(resolveGameProgressDirectory({ ...base, xdgDataHome: "/data" }))
            .toBe(path.join("/data", "NarraLeaf", "progress"));
        // Per the XDG specification a relative value is to be ignored, not resolved against cwd.
        expect(resolveGameProgressDirectory({ ...base, xdgDataHome: "share" }))
            .toBe(path.join("/home/p", ".local", "share", "NarraLeaf", "progress"));
        expect(resolveGameProgressDirectory(base))
            .toBe(path.join("/home/p", ".local", "share", "NarraLeaf", "progress"));
    });
});
...
describe("the progress file", () => {
    it("round-trips a playthrough, saved and persistent values included", async () => {
        const written = await writeGameProgressFile(env, KEY, {
            storyId: "story-1",
    ...
    it("refuses to act at all for a build that carries no key", async () => {
        expect((await writeGameProgressFile(env, "  ", {
            storyId: "",
            savedVariables: {},
            persistentVariables: {},
            anchor: null,
