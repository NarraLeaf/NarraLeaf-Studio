...
describe("validatePluginManifest", () => {
  ...
  it("accepts a runtime-only manifest", () => {
    ...
    expect(
      (result as { manifest: { entries: Record<string, string> } }).manifest.entries.studio
    ).toBeUndefined();
  });
...
describe("validatePluginManifest — capability/permission alignment", () => {
  ...
  it("carries the sidecar kind, so a node sidecar cannot pass for an executable one", () => {
    ...
    const executable = validatePluginManifest(
      fullManifest({ contributes: onlySidecar("executable") })
    );
...
describe("validatePluginManifest contributes.buildConfig", () => {
  ...
  it("normalizes a declared field and defaults the optional halves away", () => {
    const result = validatePluginManifest(
      buildConfigManifest([{ key: "appId", label: "Steam App ID", type: "text", scope: "global" }])
    );
    ...
    const [field] = (
      result as { manifest: { contributes: { buildConfig: Record<string, unknown>[] } } }
  ...
  it("refuses a field nothing on screen would identify", () => {
    const result = validatePluginManifest(
      buildConfigManifest([{ key: "appId", label: "   ", type: "text", scope: "global" }])
    );

    expect(result).toMatchObject({
      ok: false,
      error: expect.stringContaining("must declare a label")
    });
  });
  ...
  it("accepts the fixture package's manifest as it sits on disk", async () => {
    const manifestPath = path.join(
      fileURLToPath(
        new URL("./__fixtures__/plugins/narraleaf.steam-appid-fixture/", import.meta.url)
      ),
      "manifest.json"
    );
...
describe("validatePluginManifest contributes.externalLinks", () => {
  ...
  it("keeps the author's own spelling, in order, and derives one permission carrying all of it", () => {
    const result = validatePluginManifest(
      linksManifest(["steam://*", "https://store.steampowered.com/app/*"])
    );
  ...
  it("rejects a blank entry", () => {
    expect(validatePluginManifest(linksManifest(["https://x.example.com/", "   "]))).toMatchObject({
      ok: false
    });
  ...
  it("rejects a duplicate, on the canonical form rather than the raw string", () => {
    expect(
      validatePluginManifest(linksManifest(["https://x.example.com/a", "https://x.example.com/a"]))
  ...
  it("rejects a declaration with no runtime entry to use it", () => {
    expect(
      validatePluginManifest(linksManifest(["steam://*"], { studio: "main.js" }))
    ).toMatchObject({ ok: false });
  });
});
