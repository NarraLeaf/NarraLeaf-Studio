...
const rule = (from: string, to: string, enabled = true): DownloadRewriteRule => ({
  from,
  to,
  enabled
});
...
describe("rewriteDownloadUrl", () => {
  ...
  it("refuses a downgrade to http", () => {
    const outcome = rewriteDownloadUrl(GITHUB_ZIP, [
      rule("https://github.com/", "http://mirror.example/")
    ]);
    expect(outcome.url).toBe(GITHUB_ZIP);
  ...
  it("refuses a replacement that does not compose into a URL", () => {
    const outcome = rewriteDownloadUrl(GITHUB_ZIP, [
      rule("https://github.com/", "mirror.example/")
    ]);
    expect(outcome.url).toBe(GITHUB_ZIP);
...
describe("resolveDownloadSource", () => {
  ...
  it("uses the configured value, trimmed", () => {
    expect(resolveDownloadSource("  https://mirror/  ", "https://official/")).toBe(
      "https://mirror/"
    );
  });
});
...
describe("describeRewrite", () => {
  ...
  it("explains a refusal, so an ignored mirror is not silent", () => {
    expect(describeRewrite(GITHUB_ZIP, { url: GITHUB_ZIP, refused: "not-https" })).toContain(
      "not https"
    );
    expect(describeRewrite(GITHUB_ZIP, { url: GITHUB_ZIP, refused: "unparseable" })).toContain(
      "valid URL"
    );
  });
});
