...
describe("listBlueprintEventIds", () => {
    it("falls back to record key order when nothing is listed", () => {
        ...
        expect(listBlueprintEventIds(graphs(layers("zeta", "alpha", "mid")))).toEqual(["zeta", "alpha", "mid"]);
    });
    ...
    it("ignores a malformed carrier instead of throwing", () => {
        ...
        expect(listBlueprintEventIds({ events: layers("a", "b"), eventIds: [7, null, "b"] })).toEqual(["b", "a"]);
    });
});
...
describe("listBlueprintFunctionIds", () => {
    ...
    it("drops a listed graph that is gone and appends one that was never listed", () => {
        expect(listBlueprintFunctionIds(fnGraphs(["a", "b", "c"], ["c", "ghost", "a"]))).toEqual(["c", "a", "b"]);
    });
...
describe("captureBlueprintDocumentEventOrder", () => {
    ...
    it("does not invent a functions carrier on a program that has no functions record", () => {
        ...
        captureBlueprintDocumentFunctionOrder({ blueprints: { visual: { program: { graphs: graphIndex } } } });
        expect(graphIndex).not.toHaveProperty("functionIds");
    });
    ...
    it("survives shapes it does not recognise", () => {
        ...
        expect(() => captureBlueprintDocumentEventOrder({ blueprints: { a: { program: {} } } })).not.toThrow();
        expect(() =>
            captureBlueprintDocumentEventOrder({ blueprints: { a: { program: { graphs: { events: 3 } } } } }),
        ).not.toThrow();
    });
});
