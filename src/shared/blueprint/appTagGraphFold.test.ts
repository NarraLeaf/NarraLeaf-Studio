...
function edgeList(ir: BlueprintGraphIr): string[] {
  return (ir.edges ?? [])
    .map((e) => `${e.from.nodeId}.${e.from.port}->${e.to.nodeId}.${e.to.port}`)
    .sort();
}
...
describe("foldAppTagInBlueprintGraph", () => {
  ...
  it("ends the flow when the taken arm is wired to nothing", () => {
    ...
    source.edges = (source.edges ?? []).filter(
      (edge) => !(edge.from.nodeId === "if" && edge.from.port === "true")
    );
  ...
  it("decides a Switch String on both a fixed and an added case", () => {
    ...
    expect(nodeIds(foldAppTagInBlueprintGraph(source, { tagName: "Gold" }).ir)).toEqual([
      "head",
      "other"
    ]);
  });
  ...
  it("refuses a folding graph that holds a node type the table does not know", () => {
    ...
    expect(folded.ir).toBe(source);
    expect(folded.refusals).toEqual([
      { reason: "unknownNode", nodeId: "plugin", nodeType: "acme.plugin.doThing" }
    ]);
  });
  ...
  it("refuses a fold that would delete a Fn head", () => {
    ...
    source.edges = [
      ...
      ...(source.edges ?? []).filter(
        (edge) => !(edge.from.nodeId === "if" && edge.from.port === "false")
      ),
    ...
    expect(folded.ir).toBe(source);
    expect(folded.refusals).toEqual([
      { reason: "fnHeadRemoved", nodeId: "fn", nodeType: BLUEPRINT_NODE_TYPE_FN_HEAD }
    ]);
  });
...
describe("applyAppTagToBlueprintDocument", () => {
  ...
  it("returns the same document when nothing in it names the variant", () => {
    const document = documentWith(
      graph([{ id: "head", type: BLUEPRINT_NODE_TYPE_EVENT_HEAD_INIT }], [])
    );
