...
function assertBlueprintProgram(raw: unknown): void {
  ...
  if (kind === "graph") {
    ...
    if (!isRecord(graphs)) {
      throw new SharedBlueprintAssetParseError(
        "blueprint.program.graphs required for graph program"
      );
    }
    ...
    if (!isRecord(graphs.functions)) {
      throw new SharedBlueprintAssetParseError(
        "blueprint.program.graphs.functions must be an object"
      );
    }
    return;
  }
  if (kind === "scriptModule") {
    if (!isRecord(raw.source)) {
      throw new SharedBlueprintAssetParseError(
        "blueprint.program.source required for scriptModule"
      );
    }
    return;
  }
