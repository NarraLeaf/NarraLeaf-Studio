...
export function foldAppTagInBlueprintGraph(
    ...
    const unknown = Object.values(nodes).find(node => node && blueprintNodeExecPins(node.type) === null);
    if (unknown) {
        return {
            ir,
            mentioned: true,
    ...
    const removedFnHead = [...flowRemoved].find(nodeId => nodes[nodeId]?.type === BLUEPRINT_NODE_TYPE_FN_HEAD);
    if (removedFnHead) {
    ...
    for (const nodeId of FoldableGraph.sweepOrphans(
        graph.survivingNodes(removed),
        edges,
        [...appTagNodeIds, ...feedersOf(ir.edges ?? [], flowRemoved)],
    )) {
        removed.add(nodeId);
    }
    ...
    for (const edge of edges) {
        ...
        substitutions.set(`${edge.from.nodeId} ${edge.from.port}`, { source: edge.from, value: pin.value });
    }
    ...
    if (substitutions.size > 0) {
        const orphaned = FoldableGraph.sweepOrphans(
            nodesOut,
            edges,
            [...appTagNodeIds, ...[...substitutions.values()].map(entry => entry.source.nodeId)],
        );
        for (const nodeId of orphaned) {
            delete nodesOut[nodeId];
        }
        edges = edges.filter(edge => !orphaned.has(edge.from.nodeId) && !orphaned.has(edge.to.nodeId));
    }
...
function feedersOf(edges: readonly BlueprintGraphEdge[], removed: ReadonlySet<string>): string[] {
    return edges.filter(edge => removed.has(edge.to.nodeId) && !removed.has(edge.from.nodeId))
        .map(edge => edge.from.nodeId);
}

/** A node id no node in `nodes` answers to, derived from the source so a reader can trace it. */
function uniqueLiteralNodeId(nodes: Readonly<Record<string, BlueprintGraphNode>>, sourceNodeId: string): string {
...
class FoldableGraph {
    ...
    private hasTaintedInput(nodeId: string): boolean {
        return this.edges.some(
            edge => edge.to.nodeId === nodeId && this.outputValue(edge.from.nodeId, edge.from.port).tainted,
        );
    }
    ...
    private decideBranch(node: BlueprintGraphNode | undefined): BranchDecision | null {
        ...
        if (node.type === BLUEPRINT_NODE_TYPE_FLOW_IF) {
            ...
            return { takenPort: Boolean(condition.value) ? "true" : "false" };
        }
    ...
    private decideIfElse(node: BlueprintGraphNode): BranchDecision | null {
        const added = readVariadicPinIds(node.params, BLUEPRINT_IF_ELSE_BRANCH_PINS.storageKey)
    ...
    public planFlowRemoval(decisions: ReadonlyMap<string, BranchDecision>): Set<string> {
        ...
        const after = this.reachable(this.executionRoots(decisions), nodeId => decisions.get(nodeId) ?? null);
    ...
    private isExecutionEdge(edge: BlueprintGraphEdge): boolean {
        ...
        return Boolean(source) && (blueprintNodeExecOutputPinIds(source) ?? []).includes(edge.from.port);
    }
    ...
    private reachable(
        ...
        while (queue.length > 0) {
            ...
            const ports = decision
                ? (decision.takenPort ? [decision.takenPort] : [])
                : blueprintNodeExecOutputPinIds(this.nodes[nodeId]) ?? [];
            for (const edge of this.edges) {
                if (edge.from.nodeId === nodeId && ports.includes(edge.from.port) && !reached.has(edge.to.nodeId)) {
    ...
    public assemble(nodes: Record<string, BlueprintGraphNode>, edges: BlueprintGraphEdge[]): BlueprintGraphIr {
...
type GraphCarrier = { id: string; name?: string; graph?: BlueprintGraphIr; meta?: Record<string, unknown> };
