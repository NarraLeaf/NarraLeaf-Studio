...
function walkStrings(
  ...
  if (Array.isArray(node)) {
    node.forEach((item, index) =>
      walkStrings(item, `${path}[${index}]`, visit, ancestors, depth + 1)
    );
  } else {
...
function claimSubtree(
  ...
  while (stack.length > 0) {
    ...
    const children =
      isRecord(element) && Array.isArray(element.childrenIds) ? element.childrenIds : [];
  ...
  for (const entry of components) {
    ...
    for (const [elementId, element] of Object.entries(owned)) {
      ...
      pushReferences(
        element,
        "",
        { trail: [label, elementLabel(element, elementId)], elementId },
        out
      );
    }
  }
