        ...
        components: [
            {
                ...
                elements: {
                    badgeRoot: { id: "badgeRoot", type: "nl.container", style: { backgroundColor: "nlbrand:primary" } },
                },
            },
        ],
    };
...
describe("collectBrandLinkReferences", () => {
    it("finds links nested anywhere under an element, not only on known props", () => {
        ...
        const byPath = new Map(refs.map(ref => [`${ref.location.elementId ?? ref.location.surfaceId}:${ref.location.propPath}`, ref.id]));
    ...
    it("names the site by surface and element, and keeps the ids beside it", () => {
        ...
        expect(start?.where).toBe(["Main Menu", "Start", "style.backgroundColor"].join(BRAND_REFERENCE_SEPARATOR));
        expect(start?.location).toEqual({ surfaceId: "s1", elementId: "start", propPath: "style.backgroundColor" });
    });
    ...
    it("falls back to the element type when it was never named", () => {
        ...
        expect(plain?.where).toBe(["Main Menu", "nl.text", "style.color"].join(BRAND_REFERENCE_SEPARATOR));
    });
    ...
    it("survives an element tree that loops through childrenIds", () => {
        const uidoc = {
            ...
            elements: {
                a: { id: "a", type: "nl.container", childrenIds: ["b"], style: { color: "nlbrand:primary" } },
...
describe("countBrandLinkReferences", () => {
    it("counts every site an id is used at", () => {
        const counts = countBrandLinkReferences(collectBrandLinkReferences({ uidoc: sampleUiDocument() }));
