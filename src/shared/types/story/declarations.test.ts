...
function declaration(
  ...
  opts?: { disabled?: boolean; defaultValue?: number }
): StoryDeclarationBlock {
  return {
    id,
    kind: "declaration",
...
describe("describeDeclaration", () => {
  ...
  it("JSON-encodes a string default so its quotes stay visible on the row", () => {
    expect(describeDeclaration(typedDeclaration("title", "string", "hi"))).toBe(
      'title: string = "hi"'
    );
  });
...
describe("declaration scans and document order", () => {
  it("lists declarations in tree order, not the order the record happens to hold them", () => {
    ...
    expect(listSceneDeclarationBlocks(scene).map((block) => block.id)).toEqual([
      "declared-second",
      "declared-first"
    ]);
  ...
  it("places a nested declaration where its container sits, not after every root row", () => {
    ...
    const scene: StoryScene = {
      ...
      blocks: {
        ...
        [branch.id]: branch,
        [tail.id]: tail,
        [nested.id]: { ...nested, parentId: branch.id }
      }
    };
    expect(listSceneDeclarationBlocks(scene).map((block) => block.id)).toEqual([
      "top",
