...
function scene(blocks: StoryBlock[], rootBlockIds = blocks.filter(block => !block.parentId).map(block => block.id)): StoryScene {
    return {
        id: "scene-1",
        name: "Scene",
        runtimeName: "scene",
...
function label(id: string, name: string, extra: Partial<StoryBlock> = {}): StoryBlock {
    return { id, kind: "control", parentId: null, childrenIds: [], payload: { control: "label", name }, ...extra } as StoryBlock;
}

describe("listSceneLabels", () => {
    it("reads labels in document order, trimmed, skipping unnamed ones", () => {
        const found = listSceneLabels(scene([label("a", " intro "), label("b", "  "), label("c", "retry")]));
        expect(found).toEqual([{ blockId: "a", name: "intro" }, { blockId: "c", name: "retry" }]);
    });
    ...
    it("reaches labels nested inside containers", () => {
        ...
        const branch: StoryBlock = { id: "if", kind: "control", parentId: null, childrenIds: ["deep"], payload: { control: "sequence" } };
    ...
    it("drops a disabled label, and every label under a disabled ancestor", () => {
        ...
        const container: StoryBlock = { id: "grp", kind: "control", parentId: null, childrenIds: ["under"], payload: { control: "sequence" }, disabled: true };
    ...
    it("survives a corrupted childrenIds cycle rather than spinning", () => {
        const loop: StoryBlock = { id: "grp", kind: "control", parentId: null, childrenIds: ["grp", "in"], payload: { control: "sequence" } };
