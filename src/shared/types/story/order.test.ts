...
function leaf(
  ...
  return {
    ...
    parentId,
    childrenIds: [],
    payload: { text: { textId: id, value: id, role: "note" } },
    ...extra
  } as StoryBlock;
}
...
describe("listSceneBlocksInDocumentOrder", () => {
  it("returns the tree depth first, not the order the record holds", () => {
    ...
    expect(
      listSceneBlocksInDocumentOrder(scene("s", blocks, ["grp", "after"])).map((block) => block.id)
  ...
  it("skips a pruned block together with everything under it", () => {
    ...
    const found = listSceneBlocksInDocumentOrder(scene("s", blocks, ["off", "on"]), {
  ...
  it("omits a block the tree does not reach, and never returns one twice", () => {
    ...
    expect(
      listSceneBlocksInDocumentOrder(scene("s", blocks, ["grp"])).map((block) => block.id)
    ).toEqual(["grp", "in"]);
  });
...
describe("deriveUnassignedSceneIds", () => {
  ...
  it("returns nothing when every scene has a chapter", () => {
    const document = documentWith(
      [scene("only")],
      [{ id: "ch-1", name: "ch-1", sceneIds: ["only"] }]
    );
