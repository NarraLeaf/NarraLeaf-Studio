...
export const STORY_EXPR_FUNCTIONS = [
    ...
    "list", "dict", "get", "keys",
    "push", "removeAt", "setKey", "removeKey",
    "hasKey", "indexOf", "contains", "join", "slice", "concat",
...
export type StoryExpr =
     ...
     */
    | { kind: "visited"; target: StoryVisitedRef; /** Author-facing name, for display and repair only - never used to resolve. */ name: string }
    /**
     ...
     */
    | { kind: "invoke"; blueprintId: string; /** Author-facing name, for display and repair only - never used to resolve. */ name: string }
    /**
 ...
 */
export function collectStoryExpressionInvocations(expr: StoryExpr): { blueprintId: string; name: string }[] {
