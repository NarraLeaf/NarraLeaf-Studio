...
describe("matchesExternalLinkPattern — hosts", () => {
    it("covers subdomains at any depth under a leading wildcard", () => {
        expect(matchesExternalLinkPattern("https://*.example.com/*", "https://a.example.com/x")).toBe(true);
        expect(matchesExternalLinkPattern("https://*.example.com/*", "https://b.a.example.com/x/y")).toBe(true);
    });
    ...
    it("refuses a host that merely starts with the declared one", () => {
        ...
        expect(matchesExternalLinkPattern(
            "https://*.example.com/*",
    ...
    it("refuses the bare domain under a leading wildcard", () => {
        ...
        expect(matchesExternalLinkPattern("https://*.example.com/*", "https://example.com/x")).toBe(false);
    });

    it("refuses a host that ends with the declared labels but is not beneath them", () => {
        expect(matchesExternalLinkPattern("https://*.example.com/*", "https://notexample.com/x")).toBe(false);
        // `endsWith(".example.com")` would pass this; label comparison does not.
        expect(matchesExternalLinkPattern("https://*.example.com/*", "https://evil-example.com/x")).toBe(false);
    });
...
describe("matchesExternalLinkPattern — scheme and port", () => {
    it("refuses a scheme mismatch", () => {
        expect(matchesExternalLinkPattern("https://store.example.com/", "http://store.example.com/")).toBe(false);
    ...
    it("refuses a port mismatch, and treats a default port as no port", () => {
        expect(matchesExternalLinkPattern("https://example.com/x", "https://example.com:8443/x")).toBe(false);
        expect(matchesExternalLinkPattern("https://example.com:8443/x", "https://example.com/x")).toBe(false);
        expect(matchesExternalLinkPattern("https://example.com:8443/x", "https://example.com:8443/x")).toBe(true);
        // `:443` on https is the default; URL removes it from both sides, so they are one authority.
        expect(matchesExternalLinkPattern("https://example.com:443/x", "https://example.com/x")).toBe(true);
    });
});

describe("matchesExternalLinkPattern — paths", () => {
    it("treats a trailing star as a segment-boundary prefix", () => {
        expect(matchesExternalLinkPattern("https://store.example.com/app/*", "https://store.example.com/app/480")).toBe(true);
        expect(matchesExternalLinkPattern("https://store.example.com/app/*", "https://store.example.com/app/480/reviews")).toBe(true);
    });

    it("refuses a path that shares a prefix but not a segment", () => {
        // The path half of the same trap: `/appeal` starts with `/app`.
        expect(matchesExternalLinkPattern("https://store.example.com/app/*", "https://store.example.com/appeal")).toBe(false);
        expect(matchesExternalLinkPattern("https://store.example.com/app/*", "https://store.example.com/appeal/480")).toBe(false);
    });
    ...
    it("matches only itself when the pattern carries no wildcard", () => {
        ...
        expect(matchesExternalLinkPattern(pattern, "https://store.example.com/app/480/reviews")).toBe(false);
    ...
    it("requires an exact match for a query the pattern does name", () => {
        expect(matchesExternalLinkPattern("https://x.example.com/s?q=1", "https://x.example.com/s?q=1")).toBe(true);
        expect(matchesExternalLinkPattern("https://x.example.com/s?q=1", "https://x.example.com/s?q=2")).toBe(false);
        expect(matchesExternalLinkPattern("https://x.example.com/s?q=1", "https://x.example.com/s")).toBe(false);
        expect(matchesExternalLinkPattern("https://x.example.com/s#a", "https://x.example.com/s#b")).toBe(false);
    });
});
...
describe("matchesExternalLinkPattern — credentials and denied schemes", () => {
    it("refuses a candidate carrying userinfo, however well its host reads", () => {
        ...
        expect(matchesExternalLinkPattern(
            "https://*.example.com/*",
            "https://store.example.com@evil.test/x",
        )).toBe(false);
        expect(matchesExternalLinkPattern(
            "https://store.example.com/",
    ...
    it("refuses a pattern carrying userinfo, so one can never be declared", () => {
        ...
        expect(matchesExternalLinkPattern("https://evil@store.example.com/", "https://evil@store.example.com/")).toBe(false);
    });
...
describe("matchesExternalLinkPattern — malformed input", () => {
    ...
    it("trims a candidate the way the declaration was trimmed", () => {
        expect(matchesExternalLinkPattern("https://x.example.com/a", " https://x.example.com/a ")).toBe(true);
    });
});
...
describe("isValidExternalLinkPattern", () => {
    ...
    it("gives two spellings of one address the same key, so a duplicate is findable", () => {
        expect(externalLinkPatternKey("HTTPS://STORE.EXAMPLE.COM/app/*"))
            .toBe(externalLinkPatternKey("https://store.example.com/app/*"));
        expect(externalLinkPatternKey("https://a.example.com/x"))
            .not.toBe(externalLinkPatternKey("https://b.example.com/x"));
    });
});
...
describe("isExternalLinkPatternDeclared", () => {
    ...
    it("passes an address any one pattern covers, and refuses the rest", () => {
        ...
        expect(isExternalLinkPatternDeclared(declared, "https://store.steampowered.com/app/480")).toBe(true);
        expect(isExternalLinkPatternDeclared(declared, "steam://run/480")).toBe(true);
        expect(isExternalLinkPatternDeclared(declared, "https://store.steampowered.com.evil.test/app/480")).toBe(false);
        expect(isExternalLinkPatternDeclared(declared, "https://store.steampowered.com/appeal")).toBe(false);
    });
    ...
    it("keeps one plugin's list from covering another's addresses", () => {
        ...
        expect(isExternalLinkPatternDeclared(itch, "https://store.steampowered.com/app/480")).toBe(false);
