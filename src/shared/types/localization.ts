...
export function normalizeLocalizationConfiguration(value: unknown): LocalizationConfiguration {
  ...
  const sourceLocale =
    isValidLocaleCode(record.sourceLocale) && seen.has(record.sourceLocale)
      ? record.sourceLocale
      : "";
  return { sourceLocale, locales };
}
...
export function isLocalizationEnabled(config: LocalizationConfiguration): boolean {
  return (
    Boolean(config.sourceLocale) &&
    config.locales.some((locale) => locale.code !== config.sourceLocale)
  );
}
 ...
 */
export function normalizeLocalizationDocument(
  value: unknown,
  locale: LocaleCode
  ...
  for (const [unitId, raw] of Object.entries(record.units as Record<string, unknown>)) {
    ...
    document.units[unitId] = {
      ...
      status:
        unit.status === "machine" || unit.status === "translated" || unit.status === "reviewed"
          ? unit.status
          : "untranslated",
...
export function resolveLocaleChain(
  config: Pick<GameLocalizationBundle, "sourceLocale" | "locales">,
  locale: LocaleCode
): LocaleCode[] {
