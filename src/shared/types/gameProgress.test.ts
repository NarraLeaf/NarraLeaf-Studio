...
describe("gameProgressKey", () => {
  it("is the same string for every variant of one project", () => {
    ...
    expect(resolveAppTagIdentity(RELEASE_APP_TAG, BASE).identifier.value).not.toBe(
      resolveAppTagIdentity(DEMO, BASE).identifier.value
    );
  ...
  it("distinguishes two different projects", () => {
    expect(gameProgressKey(BASE)).not.toBe(
      gameProgressKey({ ...BASE, identifier: "com.example.other" })
    );
  });
...
describe("normalizeGameProgressDocument", () => {
  const written = buildGameProgressDocument(
  ...
  it("refuses a document from a newer build rather than half-reading it", () => {
    expect(
      normalizeGameProgressDocument({ ...written, schemaVersion: GAME_PROGRESS_SCHEMA_VERSION + 1 })
    ).toBeNull();
