...
describe("mediaConvertTargetExtension: audio-only MP4 is written .m4a", () => {
  it("names a re-encode to MP4 with no video track .m4a", () => {
    expect(
      mediaConvertTargetExtension({ kind: "reencode", container: "mp4", video: null, audio: "aac" })
    ).toBe("m4a");
  });
  ...
  it("names a REMUX to MP4 with no video track .m4a", () => {
    ...
    expect(mediaConvertTargetExtension({ kind: "remux", container: "mp4", audioOnly: true })).toBe(
      "m4a"
    );
  });

  it("leaves an MP4 that carries video alone, on both routes", () => {
    expect(mediaConvertTargetExtension({ kind: "remux", container: "mp4", audioOnly: false })).toBe(
      "mp4"
    );
    expect(
      mediaConvertTargetExtension({
        kind: "reencode",
        container: "mp4",
        video: "vp9",
        audio: "aac"
  ...
  it("does not rewrite containers that are not MP4", () => {
    expect(mediaConvertTargetExtension({ kind: "remux", container: "wav", audioOnly: true })).toBe(
      "wav"
    );
    expect(
      mediaConvertTargetExtension({ kind: "remux", container: "webm", audioOnly: false })
    ).toBe("webm");
    expect(
      mediaConvertTargetExtension({
        kind: "reencode",
