 ...
 */
export type MacosKeychainCredential = SigningCredentialBase &
 ...
 */
export type MacosAppleCredential = SigningCredentialBase &
  AppleNotarizationFields & {
    kind: "macos-apple";
...
export const SIGNING_CREDENTIAL_KINDS: readonly SigningCredentialKind[] = Object.keys(
  SIGNING_CREDENTIAL_PLATFORM
) as SigningCredentialKind[];
...
export function isSigningCredentialKind(value: unknown): value is SigningCredentialKind {
  return (
    typeof value === "string" &&
    Object.prototype.hasOwnProperty.call(SIGNING_CREDENTIAL_PLATFORM, value)
  );
}
...
export const APPLE_NOTARIZATION_FIELDS = [
  "notaryKeyFile",
  "notaryKeyId",
  "notaryIssuerId"
] as const;
 ...
 */
export function signingNotarizes(
  value: Partial<AppleNotarizationFields> | null | undefined
): boolean {
  return (
    Boolean(value) &&
    APPLE_NOTARIZATION_FIELDS.every((field) => Boolean((value as Record<string, unknown>)[field]))
  );
}
 ...
 */
export type SigningInspectUnavailableReason =
  | "no-certificate"
  | "unsupported-format"
  | "unreadable";
