...
describe("the seeded palette", () => {
  it("gives every seed an id a link can address", () => {
    for (const seed of BUILTIN_BRAND_COLORS) {
      expect(parseBrandLink(`nlbrand:${seed.id}`), seed.id).toEqual({
        id: seed.id,
        alpha: 1,
        alphaExplicit: false
      });
  ...
  it("groups the dotted slots and leaves the flat semantic colours out", () => {
    expect(BRAND_CONTROL_GROUPS.map((group) => group.id)).toEqual([
      "button",
      "container",
      "text",
      "textInput"
...
describe("normalizeProjectBrandColors", () => {
  it("seeds a palette that has never been written", () => {
    expect(normalizeProjectBrandColors([]).map((entry) => entry.id)).toEqual(
      BUILTIN_BRAND_COLORS.map((seed) => seed.id)
    );
  ...
  it("puts a missing seed back, in front, in seed order", () => {
    ...
    const reseeded = BUILTIN_BRAND_COLORS.filter((seed) => seed.id !== "primary").map(
      (seed) => seed.id
    );
  ...
  it("keeps the author's order when nothing is missing", () => {
    ...
    expect(normalizeProjectBrandColors(authored).map((entry) => entry.id)).toEqual(
  ...
  it("re-seeds the value of a seeded entry the file blanked out", () => {
    ...
    expect(colors.find((entry) => entry.id === "primary")?.value).toBe(
      builtinBrandColor("primary")?.value
    );
  });
