 ...
 */
export function normalizeProjectDependencyTable(
  value: unknown
): ProjectDependencyTable | undefined {
...
function normalizeDependency(value: unknown): ProjectPluginDependency | null {
  ...
  const authoredVersion =
    typeof value.authoredVersion === "string" ? value.authoredVersion.trim() : "";
    ...
    for (const kind of DEPENDENCY_KINDS) {
      ...
      if (Array.isArray(list)) {
        const ids = Array.from(
          new Set(list.filter((item): item is string => typeof item === "string"))
        ).sort();
        if (ids.length > 0) {
          usedBy[kind] = ids;
        }
