...
describe("app tag list", () => {
  it("answers the release tag in a project that has never heard of tags", () => {
    expect(listAppTags([])).toEqual([
      expect.objectContaining({ id: APP_TAG_ID_RELEASE, builtin: true })
    ]);
    ...
    const stored = normalizeProjectAppTags([
      {
        id: "demo",
        name: "Demo",
        overrides: { displayName: "  ", version: " 1.1.0 ", nonsense: "x" }
      }
    ]);
...
describe("app tag inheritance", () => {
  ...
  it("follows the project after a key is restored, rather than freezing what was inherited", () => {
    ...
    expect(resolveAppTagIdentity(restored, laterBase).version).toEqual({
      value: "2.0.0",
      overridden: false
    });
  });
...
describe("app tag references", () => {
  it("counts every stored reference and reports zero for a tag nothing points at", () => {
    ...
    expect(countAppTagReferences(documents, ["demo", "bonus", "unused"])).toEqual({
      demo: 2,
      bonus: 1,
      unused: 0
    });
...
describe("app tag plugin config", () => {
  ...
  it("reads a variant's own value, and the project's when it states none", () => {
    ...
    expect(
      resolveAppTagPluginConfigValue(demo, base, buildField("acme.steam", "branch", "variant"))
    ).toEqual({ value: "beta", overridden: true });
    expect(
      resolveAppTagPluginConfigValue(demo, base, buildField("acme.steam", "appId", "variant"))
    ).toEqual({ value: "480", overridden: false });
    expect(
      resolveAppTagPluginConfigValue(demo, base, buildField("acme.steam", "unset", "variant"))
  ...
  it("keys a platform-scoped field per platform", () => {
    ...
    expect(resolveAppTagPluginConfigValue(demo, {}, field, "windows")).toEqual({
      value: "1001",
      overridden: true
    });
    expect(resolveAppTagPluginConfigValue(demo, {}, field, "macos")).toEqual({
      value: "",
      overridden: false
    });
  });
  ...
  it("drops a variant entry for a field the project owns, and leaves undeclared keys alone", () => {
    ...
    expect(
      variantStorablePluginConfig(stored, [
        buildField("acme.steam", "appId", "global"),
...
describe("app tag scene declarations", () => {
  ...
  it("keeps a declaration whose scene the project no longer has", () => {
    ...
    expect(
      normalizeAppTagReachableScenes({ [NODE]: [{ storyId: "gone", sceneId: "gone" }] })
  ...
  it("omits the record entirely when nothing is declared", () => {
    ...
    expect(
      normalizeProjectAppTags([{ id: "demo", name: "Demo", reachableScenes: {} }])[0]
        .reachableScenes
    ).toBeUndefined();
  });
});
...
describe("app tag ending page", () => {
  it("reads a variant's own page, and the project's when it states none", () => {
    ...
    expect(resolveAppTagEndingSurface(inheriting, "surface-credits")).toEqual({
      value: "surface-credits",
      overridden: false
    });
    expect(resolveAppTagEndingSurface(stating, "surface-credits")).toEqual({
      value: "surface-thanks",
      overridden: true
    });
  });
  ...
  it("treats a stated blank page as a statement, not as inheritance", () => {
    ...
    expect(resolveAppTagEndingSurface(stored[0], "surface-credits")).toEqual({
      value: "",
      overridden: true
    });
  });
  ...
  it("gives the release tag the project's page and nothing of its own", () => {
    expect(resolveAppTagEndingSurface(resolveAppTag([], "release"), "surface-credits")).toEqual({
      value: "surface-credits",
      overridden: false
    });
  });
