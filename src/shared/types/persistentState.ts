 ...
 * // Results in: string
 */
export type ObjectPathValue<T, P extends ObjectPaths<T>> = P extends `${infer K}.${infer Rest}`
  ? K extends keyof T
...
export type PathValue<T, P extends string> =
  P extends ObjectPaths<T> ? ObjectPathValue<T, P> : never;
...
export interface ITypedPersistentState<T> {
   ...
   */
  get<K extends ObjectPaths<T>>(
    key: K,
    defaultValue: ObjectPathValue<T, K>
  ): Promise<ObjectPathValue<T, K>>;
