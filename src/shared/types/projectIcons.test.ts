...
describe("project icon set", () => {
    ...
    describe("legacy migration", () => {
        ...
        it("freezes every inset to zero so migrated projects ship what they shipped before", () => {
            const set = normalizeProjectIconSet({ macos: legacyIcon("resources/icons/app-icon-macos.png") });
        ...
        it("treats a slot with no usable path as unconfigured", () => {
            const set = normalizeProjectIconSet({ windows: { path: "   " }, linux: legacyIcon("resources/icons/l.png") });
    ...
    describe("v2 normalization", () => {
        const v2 = (overrides: Record<string, unknown>) => normalizeProjectIconSet({
            version: 2,
        ...
        it("falls back to the default inset when the value is not a number", () => {
            expect(v2({ specs: { android: { inset: "0.2" } } }).specs.android.inset)
                .toBe(PROJECT_ICON_TARGET_DEFAULTS.android.inset);
        });
        ...
        it("accepts an explicit null background but rejects a malformed colour", () => {
            ...
            expect(v2({ specs: { ios: { background: "white" } } }).specs.ios.background).toBe(DEFAULT_OPAQUE_BACKGROUND);
            expect(v2({ specs: { ios: { background: "#aabbcc" } } }).specs.ios.background).toBe("#AABBCC");
        });
    ...
    describe("outputs", () => {
        ...
        it("declares web's two named sizes and makes only apple-touch opaque", () => {
            expect(outputsForTarget("web").map(output => output.id)).toEqual(["web-favicon", "web-apple-touch"]);
        ...
        it("forces an opaque output onto white when the spec keeps transparency", () => {
            ...
            expect(resolveIconBackground(spec, findProjectIconOutput("ios"))).toBe(DEFAULT_OPAQUE_BACKGROUND);
    ...
    describe("fingerprint", () => {
        ...
        it("is stable for the same inputs", () => {
            expect(projectIconFingerprint({ sourceHash: "aa", spec, output }))
                .toBe(projectIconFingerprint({ sourceHash: "aa", spec, output }));
        });
        ...
        it("moves when the source, the recipe, or the output changes", () => {
            ...
            expect(projectIconFingerprint({ sourceHash: "aa", spec: { ...spec, inset: 0.2 }, output })).not.toBe(base);
            expect(projectIconFingerprint({ sourceHash: "aa", spec, output: findProjectIconOutput("linux") })).not.toBe(base);
        });
        ...
        it("ignores the source's timestamp and display name", () => {
            const withMeta = {
                sourceHash: "aa",
                spec: {
                    override: { path: "a.png", sourceName: "a.png", mediaType: "image/png", updatedAt: "2026-01-01" },
                    inset: 0.1,
                    background: null,
                },
                output,
        ...
        it("tracks the resolved background, so an opaque output re-bakes when its colour changes", () => {
            ...
            expect(projectIconFingerprint({ sourceHash: "aa", spec: transparent, output: ios }))
                .toBe(projectIconFingerprint({ sourceHash: "aa", spec: white, output: ios }));
            expect(projectIconFingerprint({ sourceHash: "aa", spec: { ...white, background: "#000000" }, output: ios }))
