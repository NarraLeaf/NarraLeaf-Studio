...
describe("predictGameBuildArtifacts", () => {
  ...
  it("predicts the AAB alone when that is all the Android target asked for", () => {
    expect(
      predictGameBuildArtifacts({ ...base, targets: [{ platform: "android", formats: ["aab"] }] })
    ).toEqual([
...
describe("gameBuildArtifactBaseName", () => {
  ...
  it("names a variant's build from the project and the edition", () => {
    ...
    expect(gameBuildArtifactBaseName("My Game", null)).not.toBe(
      gameBuildArtifactBaseName("My Game", "Demo")
    );
  });
...
describe("mobileExportFileName", () => {
  it("ends the name in the requested format, not in the platform's one package", () => {
    ...
    expect(mobileExportFileName("android", "apk", "MyGame", "1.2.0")).toBe(
      "MyGame-1.2.0-android.apk"
    );
    expect(mobileExportFileName("android", "aab", "MyGame", "1.2.0")).toBe(
      "MyGame-1.2.0-android.aab"
    );
...
describe("platform membership predicates", () => {
  it("classifies every platform exactly once", () => {
    ...
    expect(all.filter((p) => !isDesktopBuildPlatform(p) && !isMobileBuildPlatform(p))).toEqual([
      "web"
    ]);
  });
});
...
describe("deriveAndroidVersionCode", () => {
  ...
  it("stays monotonic across successive releases", () => {
    const codes = ["0.9.9", "0.10.0", "1.0.0", "1.0.1", "1.1.0", "2.0.0"].map((v) =>
      deriveAndroidVersionCode(v)!
    );
