...
describe("the seeded buses", () => {
    ...
    it("carries no channel and no fades", () => {
        for (const track of BUILTIN_AUDIO_TRACKS) {
            expect(Object.keys(track).sort()).toEqual(["builtin", "id", "loop", "name", "parentId", "volume"]);
        }
    });
    ...
    it("names a seeded bus for every fallback channel, and each one really exists", () => {
        for (const [channel, id] of Object.entries(DEFAULT_AUDIO_TRACK_ID)) {
            expect(BUILTIN_AUDIO_TRACKS.find(entry => entry.id === id), channel).toBeDefined();
...
describe("normalizeProjectAudioTrack", () => {
    ...
    it("falls back rather than storing a volume that is not a number", () => {
        ...
        expect(normalizeProjectAudioTrack({ id: "a", volume: Number.POSITIVE_INFINITY })!.volume).toBe(1);
    ...
    it("lets a seeded bus be renamed, re-parented and re-tuned", () => {
        ...
        expect(track).toEqual({
            id: AUDIO_TRACK_ID_BGM, name: "Score", parentId: "submix", volume: 0.6, loop: false, builtin: true,
        });
    });
});
...
describe("normalizeProjectAudioTracks", () => {
    ...
    it("puts a missing seed back, in front, without disturbing what is there", () => {
        ...
        expect(ids(tracks)).toEqual([AUDIO_TRACK_ID_BGM, AUDIO_TRACK_ID_SOUND, "ambience", AUDIO_TRACK_ID_VOICE]);
    });

    it("preserves the order the author arranged the tracks in", () => {
        const tracks = normalizeProjectAudioTracks([
            custom({ id: "ui" }), { id: AUDIO_TRACK_ID_BGM }, custom({ id: "ambience" }),
            { id: AUDIO_TRACK_ID_SOUND }, { id: AUDIO_TRACK_ID_VOICE }, custom({ id: "stinger" }),
        ]);
        expect(ids(tracks)).toEqual([
            "ui", AUDIO_TRACK_ID_BGM, "ambience", AUDIO_TRACK_ID_SOUND, AUDIO_TRACK_ID_VOICE, "stinger",
        ]);
    });
    ...
    it("keeps a stored seed's edits rather than resetting it to the seed", () => {
        ...
        expect(tracks.find(track => track.id === AUDIO_TRACK_ID_BGM))
            .toMatchObject({ name: "Score", volume: 0.5, loop: false, builtin: true });
    });
    ...
    it("drops the entries that have no id and keeps the rest", () => {
        ...
        expect(ids(tracks)).toEqual([AUDIO_TRACK_ID_BGM, AUDIO_TRACK_ID_SOUND, AUDIO_TRACK_ID_VOICE, "ui"]);
    });
    ...
    it("keeps a parent that really is there", () => {
        const tracks = normalizeProjectAudioTracks([custom({ id: "alice", parentId: AUDIO_TRACK_ID_VOICE })]);
...
describe("a bad tree degrades rather than throwing", () => {
    ...
    it("never throws, whatever the document says", () => {
        expect(() => normalizeProjectAudioTracks([
            { id: "a", parentId: "b" }, { id: "b", parentId: "a" },
            { id: "c", parentId: "c" }, { id: "d", parentId: "nope" },
        ])).not.toThrow();
    });
    ...
    it("hoists the first track past the depth cap, and its subtree follows it up", () => {
        ...
        for (const track of tracks) {
            expect(resolveAudioTrackChain(tracks, track.id).length, track.id)
                .toBeLessThanOrEqual(AUDIO_TRACK_MAX_DEPTH + 1);
        }
    });

    it("still guarantees the three seeds when the rest of the document is a ring", () => {
        const tracks = normalizeProjectAudioTracks([{ id: "a", parentId: "b" }, { id: "b", parentId: "a" }]);
        for (const seed of BUILTIN_AUDIO_TRACKS) {
            expect(tracks.find(track => track.id === seed.id), seed.id).toBeDefined();
        }
    });
});
...
describe("the tree helpers", () => {
    ...
    it("lists the direct children of a bus, and of master", () => {
        ...
        expect(ids(audioTrackChildren(tracks, null)))
            .toEqual([AUDIO_TRACK_ID_BGM, AUDIO_TRACK_ID_SOUND, AUDIO_TRACK_ID_VOICE]);
    });

    it("collects a whole subtree, excluding the root of it", () => {
        expect([...audioTrackDescendantIds(tracks, AUDIO_TRACK_ID_VOICE)].sort())
            .toEqual(["alice", "alice-whisper", "bob"]);
...
describe("the document", () => {
    ...
    it("never writes an explicit undefined", () => {
        ...
        expect(document.tracks.every(track => Object.values(track).every(value => value !== undefined))).toBe(true);
    });
});
...
describe("migrating a v1 document", () => {
    ...
    it("renames the v1 seeds onto the engine's channel names, keeping the author's edits", () => {
        const document = migrateProjectAudioTrackDocument(v1([
            { id: "music", name: "Score", channel: "bgm", gain: 0.8, fadeInMs: 800, fadeOutMs: 800, loop: true },
        ...
        expect(ids(document.tracks)).toEqual([AUDIO_TRACK_ID_BGM, AUDIO_TRACK_ID_SOUND, AUDIO_TRACK_ID_VOICE]);
        expect(document.tracks[0]).toEqual({
            id: AUDIO_TRACK_ID_BGM, name: "Score", parentId: null, volume: 0.8, loop: true, builtin: true,
        });
    ...
    it("turns a custom track's channel into its parent bus", () => {
        ...
        expect(document.tracks.find(track => track.id === "ambience")).toEqual({
            id: "ambience", name: "Ambience", parentId: AUDIO_TRACK_ID_BGM, volume: 0.5, loop: true,
        });
    });
    ...
    it("clamps a v1 gain above one into 0..1", () => {
        const document = migrateProjectAudioTrackDocument(v1([
            { id: "loud", name: "Loud", channel: "sound", gain: 2, fadeInMs: 0, fadeOutMs: 0, loop: false },
            { id: "quieter", name: "Quieter", channel: "sound", gain: 1.25, fadeInMs: 0, fadeOutMs: 0, loop: false },
        ]));
    ...
    it("leaves a v1 seed alone when its new id is already taken", () => {
        ...
        expect(document.tracks.find(track => track.id === "bgm")).toMatchObject({ name: "Mine", builtin: true });
        expect(document.tracks.find(track => track.id === "music"))
            .toMatchObject({ name: "Score", parentId: AUDIO_TRACK_ID_BGM, volume: 0.5 });
    });
    ...
    it("does not re-migrate a v2 document", () => {
        ...
        expect(already.tracks.find(track => track.id === "alice"))
            .toMatchObject({ parentId: AUDIO_TRACK_ID_VOICE, volume: 0.7 });
    });
    ...
    it("treats a document with no schemaVersion as v1", () => {
        ...
        expect(document.tracks.find(track => track.id === "ambience"))
            .toMatchObject({ parentId: AUDIO_TRACK_ID_VOICE, volume: 0.25 });
    });
});

describe("resolveAudioTrack", () => {
    const tracks = normalizeProjectAudioTracks([custom({ id: "ambience", parentId: AUDIO_TRACK_ID_BGM })]);
...
describe("the bus chain", () => {
    ...
    it("runs from the clip's own bus up to master", () => {
        expect(ids(resolveAudioTrackChain(tracks, "whisper"))).toEqual(["whisper", "alice", AUDIO_TRACK_ID_VOICE]);
...
describe("resolveAudioTrackPlayback", () => {
    ...
    it("routes to the track's own bus and takes its loop default", () => {
        expect(resolveAudioTrackPlayback(bgm)).toEqual({ busId: AUDIO_TRACK_ID_BGM, volume: 1, loop: true });
    });
    ...
    it("ignores an override that is not a number", () => {
        expect(resolveAudioTrackPlayback(bgm, { volume: Number.NaN }))
            .toEqual({ busId: AUDIO_TRACK_ID_BGM, volume: 1, loop: true });
    });
...
describe("resolveMixedElementVolume", () => {
    ...
    it("is the authored volume when nothing attenuates it", () => {
        expect(resolveMixedElementVolume({ busId: AUDIO_TRACK_ID_SOUND, volume: 0.8 }, tracks)).toBe(0.8);
    });
    ...
    it("applies the seeded bus's player slider to a clip nested below it", () => {
        expect(resolveMixedElementVolume({ busId: "alice", volume: 1 }, tracks, { voiceVolume: 0.5 })).toBe(0.25);
        expect(resolveMixedElementVolume({ busId: "alice", volume: 1 }, tracks, { bgmVolume: 0 })).toBe(0.5);
    });

    it("obeys the master slider, so muting the game really mutes a host element", () => {
        expect(resolveMixedElementVolume({ busId: AUDIO_TRACK_ID_BGM, volume: 1 }, tracks, { globalVolume: 0 })).toBe(0);
    });

    it("treats an unreadable preference as unity rather than as silence", () => {
        expect(resolveMixedElementVolume(
            { busId: AUDIO_TRACK_ID_BGM, volume: 1 },
            tracks,
            { globalVolume: null, bgmVolume: Number.NaN },
        )).toBe(1);
    });
});
...
describe("countAudioTrackReferences", () => {
    ...
    it("reports zero for every known track when nothing references them", () => {
        expect(countAudioTrackReferences([{ scenes: [] }], known)).toEqual({ [AUDIO_TRACK_ID_BGM]: 0, ambience: 0 });
    });
    ...
    it("ignores a trackId that names something other than an audio track", () => {
        ...
        expect(countAudioTrackReferences([timeline], known)).toEqual({ [AUDIO_TRACK_ID_BGM]: 0, ambience: 0 });
    });

    it("does not count an id that merely appears under some other key", () => {
        expect(countAudioTrackReferences([{ name: "ambience", id: "ambience" }], known).ambience).toBe(0);
    });
