...
describe("blueprint keyboard bindings", () => {
  it("formats legacy key names and captured keyboard combos", () => {
    ...
    expect(
      formatBlueprintKeyboardBindingFromEvent({ key: "s", ctrlKey: true, shiftKey: true })
    ).toBe("Ctrl+Shift+S");
  ...
  it("matches legacy single-key bindings without requiring modifier state", () => {
    expect(blueprintKeyboardBindingMatchesEvent("escape", { key: "Escape", ctrlKey: true })).toBe(
      true
    );
    expect(blueprintKeyboardBindingMatchesEvent("escape", { key: "Enter", ctrlKey: true })).toBe(
      false
    );
  });
  ...
  it("matches explicit keyboard combos by key and modifier state", () => {
    ...
    expect(blueprintKeyboardBindingMatchesEvent("Ctrl+S", { key: "s", ctrlKey: false })).toBe(
      false
    );
    expect(
      blueprintKeyboardBindingMatchesEvent("Ctrl+S", { key: "s", ctrlKey: true, shiftKey: true })
    ).toBe(false);
  });
});
...
describe("blueprint event head dispatch resolution", () => {
  it("matches new surface event heads", () => {
    ...
    expect(collectSurfaceEventHeadNodeIdsForDispatch(nodes, "beforeSurfaceExit")).toEqual([
      "before"
    ]);
    expect(collectSurfaceEventHeadNodeIdsForDispatch(nodes, "afterSurfaceEnter")).toEqual([
      "after"
    ]);
  ...
  it("matches mounted widget lifecycle event heads", () => {
    ...
    expect(
      collectBlueprintEventHeadNodeIdsForDispatch(nodes, "beforeSurfaceExit", "nl.button")
    ).toEqual(["before"]);
    expect(
      collectBlueprintEventHeadNodeIdsForDispatch(nodes, "afterSurfaceEnter", "nl.button")
    ).toEqual(["after"]);
    expect(collectBlueprintEventHeadNodeIdsForDispatch(nodes, "unmount", "nl.button")).toEqual([
      "unmount"
    ]);
  ...
  it("filters game preference change heads by the selected preference key", () => {
    ...
    expect(
      collectGlobalEventHeadNodeIdsForDispatch(nodes, "gamePreferenceChanged", { key: "bgmVolume" })
    ).toEqual(["any", "bgm"]);
    expect(
      collectSurfaceEventHeadNodeIdsForDispatch(nodes, "gamePreferenceChanged", {
        key: "voiceVolume"
      })
    ).toEqual(["any", "voice"]);
    // A preference key with no matching specific head still fires the wildcard head.
    expect(
      collectGlobalEventHeadNodeIdsForDispatch(nodes, "gamePreferenceChanged", { key: "skip" })
    ).toEqual(["any"]);
  });
  ...
  it("dispatches the fullscreen change head to both global and surface owners", () => {
    ...
    expect(
      collectGlobalEventHeadNodeIdsForDispatch(nodes, "windowFullscreenChanged", {
        isFullscreen: true
      })
    ).toEqual(["fs"]);
    expect(
      collectSurfaceEventHeadNodeIdsForDispatch(nodes, "windowFullscreenChanged", {
        isFullscreen: false
      })
    ).toEqual(["fs"]);
    // The head carries no inspector filter, so it fires regardless of payload.
    expect(collectGlobalEventHeadNodeIdsForDispatch(nodes, "windowFullscreenChanged")).toEqual([
      "fs"
    ]);
  });
  ...
  it("dispatches the window close request head to both global and surface owners", () => {
    ...
    expect(collectGlobalEventHeadNodeIdsForDispatch(nodes, "windowCloseRequested")).toEqual([
      "close"
    ]);
    expect(collectSurfaceEventHeadNodeIdsForDispatch(nodes, "windowCloseRequested")).toEqual([
      "close"
    ]);
  });
});
