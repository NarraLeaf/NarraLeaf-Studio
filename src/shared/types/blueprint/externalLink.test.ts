...
describe("core external links", () => {
  it("opens an address the author wrote, in the form it will be opened as", () => {
    expect(resolveCoreExternalLink({ url: " https://store.example.com/app/480 " })).toEqual({
      allowed: true,
      url: "https://store.example.com/app/480"
    });
  });
  ...
  it("refuses an address carrying credentials, which reads as one host and goes to another", () => {
    expect(resolveCoreExternalLink({ url: "https://store.example.com@evil.test/" }).allowed).toBe(
      false
    );
...
describe("plugin external links", () => {
  it("opens an address the named plugin declared, in the form it was asked for", () => {
    expect(
      resolvePluginExternalLinkAmong(PLUGINS, "acme.steam", { url: " steam://run/480 " })
    ).toEqual({ allowed: true, url: "steam://run/480" });
    expect(
      resolvePluginExternalLinkAmong(PLUGINS, "acme.itch", { url: "https://itch.io/game/7" })
  ...
  it("refuses one plugin the addresses another declared", () => {
    ...
    expect(
      resolvePluginExternalLinkAmong(PLUGINS, "acme.itch", { url: "steam://run/480" }).allowed
    ).toBe(false);
    expect(
      resolvePluginExternalLinkAmong(PLUGINS, "acme.steam", { url: "https://itch.io/game/7" })
        .allowed
    ).toBe(false);
  });
  ...
  it("refuses a plugin that declared nothing, and an id naming no plugin at all", () => {
    expect(
      resolvePluginExternalLinkAmong(PLUGINS, "acme.quiet", { url: "steam://run/480" }).allowed
    ).toBe(false);
    expect(
      resolvePluginExternalLinkAmong(PLUGINS, "acme.invented", { url: "steam://run/480" }).allowed
    ).toBe(false);
    expect(resolvePluginExternalLinkAmong(PLUGINS, "", { url: "steam://run/480" }).allowed).toBe(
      false
    );
    expect(
      resolvePluginExternalLinkAmong(undefined, "acme.steam", { url: "steam://run/480" }).allowed
    ).toBe(false);
    expect(
      resolvePluginExternalLinkAmong([], "acme.steam", { url: "steam://run/480" }).allowed
    ).toBe(false);
  });

  it("names the plugin and the address when it refuses", () => {
    const decision = resolvePluginExternalLinkAmong(PLUGINS, "acme.itch", {
      url: "https://evil.test/"
    });
  ...
  it("keeps the two regimes from answering each other's question", () => {
    ...
    expect(
      resolvePluginExternalLinkAmong(PLUGINS, "acme.itch", { url: "https://anywhere.test/" })
        .allowed
    ).toBe(false);
  ...
  it("refuses a blank address without consulting anything", () => {
    expect(resolvePluginExternalLink("acme.steam", { url: "   " }, ["steam://*"]).allowed).toBe(
      false
    );
