...
describe("isVoiceEnabled", () => {
  it("is true only once a voice language exists", () => {
    ...
    expect(
      isVoiceEnabled(
        normalizeVoiceConfiguration({ voicedLocales: [{ code: "ja", displayName: "日本語" }] })
      )
    ).toBe(true);
  });
});
...
describe("voiceLineText", () => {
  ...
  it("reads the dubbed language's line when the project translates into it", () => {
    expect(voiceLineText(doc({ "t-1": { target: "家に帰ろう。" } }), "t-1", "Let's go home.")).toBe(
      "家に帰ろう。"
    );
  });
  ...
  it("falls back to the source line when there is no translation", () => {
    ...
    expect(voiceLineText(doc({ "t-1": { target: "   " } }), "t-1", "Let's go home.")).toBe(
      "Let's go home."
    );
  });
});
