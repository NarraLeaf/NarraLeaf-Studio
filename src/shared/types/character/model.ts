...
/**
 * The persisted character model.
 *
 * Shared rather than renderer-owned because the character store is a versioned document
 * (`editor/services/character.json`) and its spec - parse, canonical serialize, semantic diff - runs
 * in the MAIN process, which cannot import a renderer module. The renderer's
 * `services/character/types.ts` re-exports every name here, so nothing that edits a character had to
 * move with it; what moved is the definition of what is on disk.
 *
 * Types only, plus the one predicate that narrows the appearance union. Nothing here reads a file,
 * and nothing here knows what a service is.
 */

/**
 * A portrait framing rect in normalized (0–1) image coordinates.
 *
 * Structurally identical to the renderer's `NormalizedCrop`, which is what the head-crop detector
 * produces; the renderer's `character/types.ts` asserts the two stay assignable in both directions,
 * because this one is what gets written to disk and that one is what the detector hands over.
 */
 ...
 */
export function isPuppetAppearance(
  appearance: ICharacterAppearance | null | undefined
): appearance is PuppetAppearance {
...
/**
 * One character exactly as it sits in the store: the editor profile plus its appearance.
 *
 * The renderer's `CharacterProfileConfig` is this shape - it is what `CharacterProfile.toJSON()`
 * returns and what `CharacterProfile.fromJSON()` takes. Spelled here as well because the document
 * spec has to name the thing it parses without reaching into the renderer, and because a diff over
 * two revisions never builds a `CharacterProfile` at all.
 */
export interface StoredCharacterProfile extends CharacterEditorProfile {
  appearance: ICharacterAppearance;
}

/** One entry of the store's `characters` array - `Character.toJSON()`'s shape. */
export interface StoredCharacter {
  profile: StoredCharacterProfile;
}
