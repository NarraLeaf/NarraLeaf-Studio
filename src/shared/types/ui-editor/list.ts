...
export function isListLikeWidgetType(elementType: string | null | undefined): boolean {
  return (
    ...
    (UI_LIST_LIKE_WIDGET_TYPES as readonly string[]).includes(elementType)
  );
}
...
export function isUIListItemsBindingKind(value: unknown): value is UIListItemsBinding["kind"] {
  return (
    typeof value === "string" && (UI_LIST_ITEMS_BINDING_KINDS as readonly string[]).includes(value)
  );
}
...
export function getUIListChildSlot(
  extra: Record<string, unknown> | undefined
): UIListChildSlot | null {
  const slot = extra?.listSlot;
  return slot === "itemTemplate" || slot === "scrollbarTrack" || slot === "scrollbarThumb"
    ? slot
    : null;
}
