...
describe("UI video prop normalizer", () => {
    ...
    it("clamps volume into 0-1 and playback rate into the range Chromium accepts", () => {
        ...
        expect(normalizeVideoProps({ playbackRate: 999 }).playbackRate).toBe(UI_VIDEO_MAX_PLAYBACK_RATE);
    ...
    it("keeps booleans authored as booleans and ignores truthy strings", () => {
        expect(normalizeVideoProps({ loop: true, autoplay: true, muted: true, controls: true })).toMatchObject({
            loop: true,
            autoplay: true,
            muted: true,
            controls: true,
