...
export function clampContainerStackSpacingPx(v: number): number {
  ...
  return Math.max(
    ...
    Math.min(CONTAINER_STACK_SPACING_ABS_MAX_PX, v)
  );
}

export function parseContainerLayoutKind(
...
export function getContainerChildLayoutParticipation(
  kind: ContainerLayoutKind
): ContainerChildLayoutParticipation {
 ...
 */
export function resolveContainerScrollViewportOverflow(
  axis: ContainerScrollAxis
): ContainerScrollViewportOverflow {
  const isY = axis === "y";
  return {
