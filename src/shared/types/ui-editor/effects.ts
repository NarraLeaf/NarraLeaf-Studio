...
function clampLayer(partial: Partial<EffectShadowLayer>): EffectShadowLayer {
  return {
    ...
    color:
      typeof partial.color === "string" && partial.color.trim() ? partial.color.trim() : "#000000"
  };
}
...
function normalizeEffectShadowField(raw: unknown): EffectShadowStored | null {
  ...
  if (typeof raw === "object" && raw !== null) {
    ...
    if ("offsetX" in o || "layer" in o) {
      const layer = (
        "layer" in o && o.layer && typeof o.layer === "object"
...
export function normalizeElementEffectValues(raw: unknown): ElementEffectValues {
  ...
  const bb =
    typeof o.effectBackgroundBlur === "number"
      ? o.effectBackgroundBlur
      : Number(o.effectBackgroundBlur);
  return {
...
export function effectShadowStoredToCss(
  value: EffectShadowStored | null,
  slot: ShadowSlotKind
): string {
  if (!value) {
    return "";
...
export const WIDGET_EFFECT_KINDS_BY_TYPE: Record<string, readonly VisualEffectKind[]> = {
  ...
  "nl.container": ["blur", "backgroundBlur", "shadow", "innerShadow", "blend", "glow", "filter"],
