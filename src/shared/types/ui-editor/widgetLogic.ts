...
const SURFACE_LIFECYCLE_EVENTS: readonly WidgetLogicEventDef[] = [
  {
    ...
    description:
      "Fires before the current Surface starts its exit animation while the element is still mounted.",
    dispatchKind: "lifecycle",
  ...
  },
  {
    ...
    description:
      "Fires after the current Surface finishes its enter animation while the element is still mounted.",
    dispatchKind: "lifecycle",
...
const PUPPET_EVENTS: readonly WidgetLogicEventDef[] = [
  ...
  },
  {
    ...
    displayName: "Error",
    description:
      "Fires when the runtime was found and then failed. A missing runtime is not an error.",
    dispatchKind: "lifecycle",
...
export function getWidgetLogicApi(
