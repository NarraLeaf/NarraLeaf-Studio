...
export function parseShadowLikeFragment(
  raw: string
): { ok: true; value: ShadowLikeLayer } | { ok: false } {
  let s = raw.trim();
  if (!s) {
    return { ok: false };
  }
...
export function serializeShadowLikeLayer(
  value: ShadowLikeLayer,
  mode: ShadowSerializeMode
): string {
...
export function layerDataToShadowLike(
  layer: EffectShadowLayerData,
  inset: boolean
): ShadowLikeLayer {
  return { inset, ...layer };
}
 ...
 */
export function shadowLayerDataToCss(
  layer: EffectShadowLayerData,
  mode: ShadowSerializeMode
): string {
