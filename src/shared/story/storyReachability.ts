...
export function scanStoryEntryPoints(
  ...
  for (const carrier of carriers) {
    for (const node of Object.values(carrier.graph.nodes ?? {})) {
      ...
      const missing = START_STORY_TARGET_PINS.filter(
        (pin) => !isTargetDecided(carrier.graph, node, pin)
      );
      if (missing.length > 0) {
        undecidable.push({
...
export function* blueprintGraphCarriers(
...
function* carriersOf(blueprint: Blueprint): Generator<BlueprintGraphCarrier> {
  ...
  const slots: readonly {
    graphKind: BlueprintGraphCarrier["graphKind"];
    entries: Record<string, { graph?: BlueprintGraphIr } | undefined>;
  }[] = [
