...
function scene(id: string, blocks: StoryBlock[], rootBlockIds?: string[]): StoryScene {
  return {
    ...
    runtimeName: id,
    rootBlockIds:
      rootBlockIds ?? blocks.filter((entry) => entry.parentId === null).map((entry) => entry.id),
...
describe("reachableSceneIds", () => {
  it("follows a jump and nothing else", () => {
    const built = document(
      [scene("sc1", [goto("b1", "retry"), jump("b2", "sc2")]), scene("sc2", []), scene("sc3", [])],
      "sc1"
    );
  ...
  it("prefers the marked entry over either fallback", () => {
    const built = document(
      [scene("sc1", []), scene("sc2", [jump("b1", "sc3")]), scene("sc3", [])],
      "sc2"
    );
...
describe("traceReachableScenes", () => {
  ...
  it("reports the document-order fallback as its own reason", () => {
    ...
    expect(traceReachableScenes(built, { fallback: "documentOrder" }).get("sc1")).toEqual({
      kind: "documentOrder"
    });
  });
  ...
  it("keeps a live reason for a scene something later also names", () => {
    ...
    expect(traceReachableScenes(built, { fallback: "none" }).get("sc2")).toEqual({
      kind: "jump",
      fromSceneId: "sc1",
      blockId: "b1"
    });
  ...
  it("agrees with the id-only answer, member for member and in order", () => {
    const built = document(
      [scene("sc1", [jump("b1", "sc2")]), scene("sc2", []), scene("sc3", [])],
      "sc1"
    );
    const options = { fallback: "none" } as const;

    expect([...traceReachableScenes(built, options).keys()]).toEqual([
      ...reachableSceneIds(built, options)
    ]);
  });
});
...
describe("scanStoryEntryPoints", () => {
  ...
  it("names both targets when a node picked neither", () => {
    const scan = scanStoryEntryPoints(
      blueprintGraphCarriers([startStoryBlueprint({})]),
      everyScene
    );
  ...
  it("reads the same blueprint the same way through either container", () => {
    ...
    expect([...blueprintDocumentGraphCarriers(asDocument(blueprint))]).toEqual([
      ...blueprintGraphCarriers([blueprint])
    ]);
  });
