...
function cut(id: string, appTagId = DEMO, extra?: { disabled?: boolean; parentId?: string }): StoryBlock {
...
describe("collectCutPoints", () => {
    it("says a row with content after it removes something", () => {
        ...
        expect(collectCutPoints(built)).toEqual([expect.objectContaining({
            sceneId: "s1",
            blockId: "c",
            appTagId: DEMO,
            removes: true,
    ...
    it("counts only content, so a tail of other cut points is still nothing", () => {
        const built = story([scene("s1", [line("a"), cut("c"), cut("d", "tag-other")], ["a", "c", "d"])]);

        expect(collectCutPoints(built)[0].removes).toBe(false);
    });

    it("counts only shipped content, so a disabled tail is still nothing", () => {
        const built = story([scene("s1", [
            line("a"),
            cut("c"),
            block({ id: "b", kind: "nodeAction", disabled: true, payload: { action: "narration", text: { textId: "t", value: "b", role: "narration" } } }),
        ], ["a", "c", "b"])]);
    ...
    it("leaves a disabled row out, since it decides nothing", () => {
        const built = story([scene("s1", [line("a"), cut("c", DEMO, { disabled: true }), line("b")], ["a", "c", "b"])]);
...
describe("collectUncutForks", () => {
    it("reports a fork where one route ends and the other carries on", () => {
        ...
        expect(collectUncutForks(built, DEMO)).toEqual([expect.objectContaining({
            sceneId: "s1",
            uncutBranches: 1,
        })]);
    });
