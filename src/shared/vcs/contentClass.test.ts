...
const ASSET_TYPES = path.resolve(
  HERE,
  "../../renderer/lib/workspace/services/assets/assetTypes.ts"
);
  ...
  for (const [type, expected] of Object.entries(EXPECTED)) {
    it(`classifies every ${type} extension as ${expected}`, () => {
      const wrong = (table.get(type) ?? [])
        .filter(
          (extension) => contentClassOf(`a/b.${extension}`) !== (DELIBERATE[extension] ?? expected)
        )
...
const HEADERS: Readonly<Record<string, { bytes: number[]; expected: ContentClass }>> = {
  ...
  webp: {
    bytes: [...ascii("RIFF"), 0x24, 0, 0, 0, ...ascii("WEBP"), ...ascii("VP8 ")],
    expected: "bitmap"
  },
  wav: {
    bytes: [...ascii("RIFF"), 0x24, 0, 0, 0, ...ascii("WAVE"), ...ascii("fmt ")],
    expected: "audio"
  },
  avi: {
    bytes: [...ascii("RIFF"), 0x24, 0, 0, 0, ...ascii("AVI "), ...ascii("LIST")],
    expected: "video"
  },
  ...
  collection: {
    bytes: [...ascii("ttcf"), 0x00, 0x01, 0x00, 0x00, 0, 0, 0, 2, 0, 0, 0, 0x0c],
    expected: "font"
  },
